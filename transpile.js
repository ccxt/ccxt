...
//     let regex3 = /[^a-zA-Z0-9_](?:let|const)\s+(?:\[([^\]]+)\]|([a-zA-Z0-9_]+))/g // local variables
...
function transpileDerivedExchangeClasses (sourceFolder) {
    ...
    fs.readdirSync (sourceFolder).filter (file => file.includes ('.js')).map (file => {
        ...
        function addPythonClassDeclaration (array) {
            array.push ('')
            array.push ('')
        ...
        function addPHPClassDeclaration (array) {
            array.push ('')
            array.push ('class ' + className + ' extends ' + baseClass + ' {')
            array.push ('')
        }

        addPythonClassDeclaration (python2)
        addPythonClassDeclaration (python3)
        addPHPClassDeclaration (php)
        ...
        for (let i = 0; i < methods.length; i++) {
            ...
            let regex2 = /(async |)([\S]+)\s\(([^)]*)\)\s*{/g // signature line
            let matches = regex2.exec (signature)
            let keyword = matches[1]      // async or not
            let method = matches[2]       // method name
            let args = matches[3].trim ()

            log.green ('-----------------------------------------------')
            log.yellow (matches, keyword, method, args)

            // convert method to underscore notation
            method = method.replace (/[A-Z]/g, match => '_' + match.toLowerCase ())
            ...
            let python3Body = regexAll (body, pythonRegexes).replace (/$\s*$/gm, '').replace (/\'([абвгдеёжзийклмнопрстуфхцчшщъыьэюя]+)\'/gm, "u'$1'")
