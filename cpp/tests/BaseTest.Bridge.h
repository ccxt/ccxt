...
inline bool equals (const ccxt::any& rawA, const ccxt::any& rawB) {
    ...
    const ccxt::any a = ::wsToPlain (rawA);
    const ccxt::any b = ::wsToPlain (rawB);
    if (ccxt::isList (a)) {
        if (!ccxt::isList (b)) {
            return false;
        }
        const auto& x = ccxt::any_cast<ccxt::list> (a);
        const auto& y = ccxt::any_cast<ccxt::list> (b);
        if (x.size () != y.size ()) {
            return false;
        }
    ...
    if (ccxt::isDict (a)) {
        ...
        const auto& x = ccxt::any_cast<ccxt::dict> (a);
        const auto& y = ccxt::any_cast<ccxt::dict> (b);
...
inline bool deepEqual (ccxt::Exchange& exchange, const ccxt::any& a, const ccxt::any& b) {
    return isEqual (exchange.json (a), exchange.json (b));
}

inline void assertDeepEqual (ccxt::Exchange& exchange, const ccxt::any& skippedProperties,
                             const ccxt::any& method, const ccxt::any& a, const ccxt::any& b) {
    (void) skippedProperties;
    if (!deepEqual (exchange, a, b)) {
        throw ccxt::BaseError (
            "two dicts do not match: " + ccxt::any_cast<std::string> (toString (exchange.json (a)))
            + " != " + ccxt::any_cast<std::string> (toString (exchange.json (b)))
            + " [" + ccxt::any_cast<std::string> (toString (method)) + "]");
    }
}
...
inline std::shared_future<ccxt::any> testLanguageSpecific () {
