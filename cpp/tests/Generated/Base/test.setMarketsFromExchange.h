             ...
             [=]() mutable -> std::any {
               ...
               assertTrue(isTrue((!isEqual(
                              ...
                              isTrue((isGreaterThan(
                                  ...
                                  0))),
                          std::string("Markets should be loaded in exchange1"));
               ...
               try {
                 ...
                 assertTrue(!isTrue(trueClause),
                            std::string("Should have thrown an error when "
               ...
               try {
                 exchange2.setMarketsFromExchange(
                     nonloadedExchange); // exchange2 has no markets yet
                 assertTrue(!isTrue(trueClause),
                            std::string("Should have thrown error when sharing "
                                        "from exchange without markets"));
               } catch (const std::exception &error) {
                 assertTrue(trueClause);
               }
               ...
               for (std::any i = 0; isLessThan(i, getArrayLength(neededProps));
                 ...
                 assertDeepEqual(emptyExchange, ccxt::dict{}, methodName,
                                 emptyExchange.getProperty(
                                     exchange1, ::getValue(neededProps, i)),
                                 emptyExchange.getProperty(
                                     exchange2, ::getValue(neededProps, i)));
               }
               // Verify that modifying one exchange's markets modifies the
               // other
               // ::getValue(exchange1, std::string("markets"))['ETH/USD'] = {
               // 'id': 'EthUsd', 'symbol': 'ETH/USD', 'base': 'ETH', 'quote':
               // 'USD', 'baseId': 'Eth', 'quoteId': 'Usd', 'type': 'spot',
               // 'spot': true }; assert ('ETH/USD' in ::getValue(exchange2,
