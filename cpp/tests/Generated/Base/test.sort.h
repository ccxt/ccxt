...
void testSort() {
  ...
  assertDeepEqual(exchange, std::any{}, std::string("sort"),
                  ...
                  ccxt::list{std::string("a"), std::string("b"),
                             std::string("c"), std::string("d")});
  // todo 1: atm, `sort` is only meant for strings. we should update to support
  // numerics todo 2: add test for above 10, eg: 1, 2, 10, 20, 21
  // // integers (single-digit, safe for cross-language lexicographic/numeric
  // consistency) assertDeepEqual (exchange, undefined, 'sort', exchange.sort ([
  // 3, 1, 2 ]), [ 1, 2, 3 ]); assertDeepEqual (exchange, undefined, 'sort',
  // exchange.sort ([ 5, 3, 1, 4, 2 ]), [ 1, 2, 3, 4, 5 ]); assertDeepEqual
  // (exchange, undefined, 'sort', exchange.sort ([ 0, 3, 1, 2 ]), [ 0, 1, 2, 3
  // ]);
  // // floats (values chosen so lexicographic order matches numeric order)
  // assertDeepEqual (exchange, undefined, 'sort', exchange.sort ([ 1.5,
