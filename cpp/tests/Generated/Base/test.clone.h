...
void testClone() {
  ...
  assertTrue(isEqual(::getValue(simpleOrig, std::string("x")), 1),
             std::string("clone A: mutating clone must not change original x"));
  assertTrue(
      isEqual(::getValue(simpleOrig, std::string("y")), std::string("hello")),
      std::string("clone A: mutating clone must not change original y"));
  // mutating the original must not affect an already-taken clone
  ::setValue(simpleOrig, std::string("x"), 42);
  assertTrue(isEqual(::getValue(simpleClone, std::string("x")), 999),
             std::string("clone A: mutating original must not change clone x"));
  // -------------------------------------------------------------------------
  ...
  assertTrue(
      isEqual(::getValue(nestedOrig, std::string("top")),
              std::string("original")),
      std::string(
          "clone B: top-level scalar independence – original unchanged"));
  assertTrue(
      isEqual(::getValue(nestedClone, std::string("top")),
              std::string("cloned")),
      std::string("clone B: top-level scalar independence – clone updated"));
  ...
  assertTrue(isEqual(::getValue(emptyClone, std::string("newKey")),
                     std::string("injected")),
             std::string("clone C: can add key to clone of empty object"));
  // -------------------------------------------------------------------------
  ...
  assertTrue(isEqual(::getValue(withUndef, std::string("present")),
                     std::string("yes")),
             std::string("clone D: mutating clone must not change original"));
  // -------------------------------------------------------------------------
  ...
  assertTrue(
      isEqual(::getValue(masterOrig, std::string("a")), 1),
      std::string("clone E: original a untouched after clone1 mutation"));
  ...
  assertTrue(!isTrue((inOp(clone2, std::string("d")))),
             std::string("clone E: clone2 must not inherit clone1 extra key"));
  // mutate clone2 differently
  ::setValue(clone2, std::string("b"), 200);
  assertTrue(isEqual(::getValue(clone1, std::string("b")), 2),
             std::string("clone E: clone1.b unaffected by clone2 mutation"));
  assertTrue(isEqual(::getValue(masterOrig, std::string("b")), 2),
             std::string("clone E: original.b unaffected by clone2 mutation"));
}
