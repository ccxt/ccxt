...
const guard = (x) => { const st = x.parent; return x.operatorToken.kind === ts.SyntaxKind.EqualsEqualsEqualsToken && ts.isIfStatement(st) && st.expression === x && !st.elseStatement && ts.isBlock(st.thenStatement) && st.thenStatement.statements.length === 1 && ts.isThrowStatement(st.thenStatement.statements[0]); };
const nil = (y) => y.kind === ts.SyntaxKind.NullKeyword || (ts.isIdentifier(y) && y.text === 'undefined');
const pred = (rel) => rel.startsWith('ts/src/prediction/') || rel === 'ts/src/base/PredictionExchange.ts';
for (const [rel, sf] of sfs) methods(sf, (m) => {
    const r = res.get(m.name.text); if (!r) return;
    if (pred(rel) && !process.env.PRED) return;
    ...
    (function w (x) {
        if (ts.isBinaryExpression(x)) {
            ...
            if ([ts.SyntaxKind.EqualsEqualsEqualsToken, ts.SyntaxKind.ExclamationEqualsEqualsToken, ts.SyntaxKind.EqualsEqualsToken, ts.SyntaxKind.ExclamationEqualsToken].includes(op)
                && ((isId(x.left) && nil(x.right)) || (isId(x.right) && nil(x.left)))) r.bad.push(`${at} ${guard(x)?'guard':'nilcmp'} ${x.parent.getText(sf).slice(0, 60).replace(/\s+/g, ' ')}`);
...
for (const [m, r] of out) {
    const hard = r.bad.filter(b => !/ guard /.test(b)); console.log(`${hard.length ? 'BAD ' : (r.bad.length ? 'GRD ' : 'OK  ')} ${m}[${r.idx}] decls=${r.decls} bad=${r.bad.length} calls=${JSON.stringify(r.calls || {})}`);
}
