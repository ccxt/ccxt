...
public partial class testMainClass : BaseTest
{
    public partial class SharedMethods
    {
        ...
        public void assertStructure(BaseExchange exchange, object skippedProperties, object method, object entry, object format, object emptyAllowedFor = null, object deep = null)
        {
            ...
            if (((format is IList<object>) || (format.GetType().IsGenericType && format.GetType().GetGenericTypeDefinition().IsAssignableFrom(typeof(List<>)))))
            {
                ...
                for (int i = 0; i < getArrayLength(format); i++)
                {
                    ...
                    assert((value != null), add((((object)i).ToString() + " index is expected to have a value"), logText));
                    // because of other langs, this is needed for arrays
                    object typeAssertion = assertType(exchange, new Dictionary<string, object>() {}, entry, i, format);
                    assert(isEqual(typeAssertion, true), add((((object)i).ToString() + " index does not have an expected type "), logText));
                }
            } else
            {
                ...
                for (int i = 0; i < keys.Count; i++)
                {
                    ...
                    assert((value != null), add((add("\"", stringValue(key)) + "\" key is expected to have a value"), logText));
                    // add exclusion for info key, as it can be any type
                    if ((key != "info"))
                    {
                        object typeAssertion = assertType(exchange, new Dictionary<string, object>() {}, entry, key, format);
                        assert(isEqual(typeAssertion, true), add((add("\"", stringValue(key)) + "\" key is neither undefined, neither of expected type"), logText));
                        if (isTrue(deep))
                        {
        ...
        public void assertTimestamp(BaseExchange exchange, object skippedProperties, object method, object entry, object nowToCheck = null, object keyNameOrIndex = null, object allowNull = null)
        {
            ...
            if (isDateTimeObject)
            {
                assert((inOp(entry, keyNameOrIndex)), add((add("timestamp key \"", keyNameOrIndex) + "\" is missing from structure"), logText));
            } else
            {
                // if index was provided (mostly from fetchOHLCV) then we check if it exists, as mandatory
                assert(!(isEqual(getValue(entry, keyNameOrIndex), null)), add((add("timestamp index ", stringValue(keyNameOrIndex)) + " is undefined"), logText));
            }
            object ts = getValue(entry, keyNameOrIndex);
            assert((ts != null) || isTrue(allowNull), add("timestamp is null", logText));
            if ((ts != null))
            {
        ...
        public void assertTimestampAndDatetime(BaseExchange exchange, object skippedProperties, object method, object entry, object nowToCheck = null, object keyNameOrIndex = null, object allowNull = null)
        {
            ...
            if (isDateTimeObject)
            {
                ...
                assert((dt != null) || isTrue(allowNull), add("timestamp is null", logText));
                if ((dt != null))
                {
                    ...
                    if (isGreaterThanOrEqual(diff, 500))
                    {
                        ...
                        assert(false, add((((("datetime is not iso8601 of timestamp:" + dtParsedString) + "(string) != ") + dtEntryString) + "(from ts)"), logText));
                    }
                }
            }
        }
        ...
        public void assertValidCurrencyIdAndCode(BaseExchange exchange, object skippedProperties, object method, object entry, object currencyId, object currencyCode, object allowNull = null)
        {
            ...
            if (definedValues)
            {
                ...
                assert(isEqual(getValue(currencyByCode, "id"), currencyId), add((add((add("currencyId \"", stringValue(currencyId)) + "\" does not match currency id from instance: \""), stringValue(getValue(currencyByCode, "id"))) + "\""), logText));
                // check by id
                Dictionary<string, object> currencyById = exchange.safeCurrency(currencyId);
                assert(isEqual(getValue(currencyById, "code"), currencyCode), add(add((add("currencyCode ", stringValue(currencyCode)) + " does not match currency of id: "), stringValue(currencyId)), logText));
            }
        }
        ...
        public void assertSymbolInMarkets(BaseExchange exchange, object skippedProperties, object method, object symbol)
        {
            ...
            assert((!isEqual(exchange.markets, null)) && (inOp(exchange.markets, symbol)), add("symbol should be present in exchange.symbols", logText));
        }
        ...
        public void assertInArray(BaseExchange exchange, object skippedProperties, object method, object entry, object key, object expectedArray, object allowNull = null)
        {
            ...
            if ((value != null))
            {
                ...
                assert(exchange.inArray(value, expectedArray), add((((add((add("\"", stringValue(key)) + "\" key (value \""), stringValue(value)) + "\") is not from the expected list : [") + stingifiedArrayValue) + "]"), logText));
            }
        }
        public void assertFeeStructure(BaseExchange exchange, object skippedProperties, object method, object entry, object key, object allowNull = null)
        {
            ...
            if (((key is int) || (key is long) || (key is Int32) || (key is Int64)))
            {
                ...
                assert(isLessThan(key, getArrayLength(entry)), add((add("fee key ", keyString) + " was expected to be present in entry"), logText));
            } else
            {
                assert(exchange.isDictionary(entry), add("fee container is expected to be a dict", logText));
                assert(inOp(entry, key), add((add("fee key \"", key) + "\" was expected to be present in entry"), logText));
            }
            object feeObject = exchange.safeValue(entry, key);
            assert((feeObject != null) || isTrue(allowNull), add("fee object is null", logText));
            ...
            if ((feeObject != null))
            {
                ...
                assert(inOp(feeObject, "currency"), add((add("\"", keyString) + "\" fee object should contain \"currency\" key"), logText));
        ...
        public void assertInteger(BaseExchange exchange, object skippedProperties, object method, object entry, object key, object allowNull = null)
        {
            ...
            if ((entry != null))
            {
                ...
                if ((value != null))
                {
                    ...
                    assert(isInteger, add((add((add("\"", stringValue(key)) + "\" key (value \""), stringValue(value)) + "\") is not an integer"), logText));
                }
            }
        }
        ...
        public void assertOrderState(BaseExchange exchange, object skippedProperties, object method, object order, object assertedStatus, object strictCheck)
        {
            ...
            string msg = add((add("order should be ", assertedStatus) + ", but it was not asserted"), logText);
        ...
        public void assertNonEmtpyArray(BaseExchange exchange, object skippedProperties, object method, object entry, object hint = null)
        {
            ...
            assert(getArrayLength(entry) > 0, (add("response is expected to be a non-empty array", logText) + " (add \"emptyResponse\" in skip-tests.json to skip this check)"));
        }
        ...
        {
            object logText = logTemplate(exchange, method, new Dictionary<string, object>() {});
            assert(deepEqual(exchange, a, b), add(add((add("two dicts do not match: ", json(a)) + " != "), json(b)), logText));
        }
