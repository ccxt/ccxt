    ...
    public static void testTicker(BaseExchange exchange, object skippedProperties, object method, object entry, object symbol)
    {
        ...
        if (!(inOp(skippedProperties, "maxIncrease")) && !isUnrecognizedSymbol)
        {
            ...
            if ((percentage != null))
            {
                ...
                {
                    assert(Precise.stringLe(percentage, Precise.stringMul("+100", maxIncrease)), add((("percentage should be below " + maxIncrease) + "00% "), logText));
                }
            }
            //
            // change
            ...
            if ((change != null))
            {
                ...
                if (!isEqual(isOptionMarket, true))
                {
                    assert(Precise.stringLe(change, Precise.stringMul(approxValue, maxIncrease)), add((("change should be below " + maxIncrease) + "x price "), logText));
                }
            }
        }
        //
