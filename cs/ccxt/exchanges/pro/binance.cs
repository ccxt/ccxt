using ccxt.sbe.binance_spot_3_2;
namespace ccxt.pro;
...
public partial class binance : ccxt.binance
{
     ...
     */
    public virtual object getSbeWebSocketUrl(object url)
    {
        object useSbe = this.safeBool(this.options, "useSbe", false);
        if (isTrue(useSbe))
        {
    ...
    public async override Task<object> fetchOrderBookWs(object symbol, object limit = null, object parameters = null)
    {
        ...
        object baseUrl = getValue(getValue(getValue(getValue(this.urls, "api"), "ws"), "ws-api"), marketType);
        object url = this.getSbeWebSocketUrl(baseUrl);
        object requestId = this.requestId(url);
    ...
    public virtual void handleFetchOrderBook(WebSocketClient client, object message)
    {
        ...
        object priceExponent = this.safeInteger(result, "priceExponent");
        if (isTrue(!isEqual(priceExponent, null)))
        {
            // SBE format - normalize to JSON format
            object qtyExponent = this.safeInteger(result, "qtyExponent", 0);
            object normalized = new Dictionary<string, object>() {
            ...
            // Convert bids
            object bidsArray = this.safeList(result, "bids", new List<object>() {});
            for (object i = 0; isLessThan(i, getArrayLength(bidsArray)); postFixIncrement(ref i))
            {
                object bid = getValue(bidsArray, i);
                object price = this.applyExponent(this.safeInteger(bid, "price", 0), priceExponent);
                object qty = this.applyExponent(this.safeInteger(bid, "qty", 0), qtyExponent);
                ((IList<object>)getValue(normalized, "bids")).Add(new List<object> {((price)).ToString(), ((qty)).ToString()});
            }
            // Convert asks
            object asksArray = this.safeList(result, "asks", new List<object>() {});
            for (object i = 0; isLessThan(i, getArrayLength(asksArray)); postFixIncrement(ref i))
            {
                object ask = getValue(asksArray, i);
                object price = this.applyExponent(this.safeInteger(ask, "price", 0), priceExponent);
                object qty = this.applyExponent(this.safeInteger(ask, "qty", 0), qtyExponent);
                ((IList<object>)getValue(normalized, "asks")).Add(new List<object> {((price)).ToString(), ((qty)).ToString()});
            }
            result = normalized;
        }
    ...
    public async override Task<object> fetchTickerWs(object symbol, object parameters = null)
    {
        ...
        object baseUrl = getValue(getValue(getValue(getValue(this.urls, "api"), "ws"), "ws-api"), type);
        object url = this.getSbeWebSocketUrl(baseUrl);
        object requestId = this.requestId(url);
        object messageHash = ((object)requestId).ToString();
        object subscription = new Dictionary<string, object>() {
    ...
    public virtual void handleFetchOHLCV(WebSocketClient client, object message)
    {
        ...
        if (isTrue(!isEqual(result, null)))
        {
            ...
            } else if (isTrue((result is IDictionary<string, object>)))
            {
                // SBE format - result is an object with klines array and exponents
                object klinesArray = this.safeList(result, "klines", new List<object>() {});
                if (isTrue(isGreaterThan(getArrayLength(klinesArray), 0)))
                {
                    // Get exponents for converting mantissa values
                    object priceExponent = this.safeInteger(result, "priceExponent", 0);
                    object qtyExponent = this.safeInteger(result, "qtyExponent", 0);
                    // Convert mantissa values to standard OHLCV format
                    object normalizedKlines = new List<object>() {};
                    for (object i = 0; isLessThan(i, getArrayLength(klinesArray)); postFixIncrement(ref i))
                    {
                        object kline = getValue(klinesArray, i);
                        // Convert mantissa128 byte arrays to numbers
                        object volume = this.mantissa128ToNumber(getValue(kline, "volume"));
                        object openTime = (Math.Floor(Double.Parse((divide(getValue(kline, "openTime"), 1000)).ToString()))); // microseconds to milliseconds
                        object openPrice = this.applyExponent(getValue(kline, "openPrice"), priceExponent);
                        object highPrice = this.applyExponent(getValue(kline, "highPrice"), priceExponent);
                        object lowPrice = this.applyExponent(getValue(kline, "lowPrice"), priceExponent);
                        object closePrice = this.applyExponent(getValue(kline, "closePrice"), priceExponent);
                        object vol = this.applyExponent(volume, qtyExponent);
                        object closeTime = (Math.Floor(Double.Parse((divide(getValue(kline, "closeTime"), 1000)).ToString())));
                        object quoteVolume = this.mantissa128ToNumber(getValue(kline, "quoteVolume"));
                        object quoteVol = this.applyExponent(quoteVolume, priceExponent);
                        object numTrades = getValue(kline, "numTrades");
                        object takerBuyBaseVolume = this.mantissa128ToNumber(getValue(kline, "takerBuyBaseVolume"));
                        object takerBuyBase = this.applyExponent(takerBuyBaseVolume, qtyExponent);
                        object takerBuyQuoteVolume = this.mantissa128ToNumber(getValue(kline, "takerBuyQuoteVolume"));
                        object takerBuyQuote = this.applyExponent(takerBuyQuoteVolume, priceExponent);
                        ((IList<object>)normalizedKlines).Add(new List<object>() {openTime, ((openPrice)).ToString(), ((highPrice)).ToString(), ((lowPrice)).ToString(), ((closePrice)).ToString(), ((vol)).ToString(), closeTime, ((quoteVol)).ToString(), numTrades, ((takerBuyBase)).ToString(), ((takerBuyQuote)).ToString(), "0"});
                    }
    ...
    public async virtual Task<object> watchMultiTickerHelper(object methodName, object channelName, object symbols = null, object parameters = null, object isUnsubscribe = null)
    {
        ...
        if (isTrue(isTrue(isMarkPrice) && !isTrue(this.inArray(marketType, new List<object>() {"swap", "future"}))))
        {
        ...
        object result = await this.watchMultiple(url, hashes, this.deepExtend(request, parameters), hashes, subscription);
        if (isTrue(isUnsubscribe))
        {
            return result;
        }
    ...
    public virtual void handleTickerWs(WebSocketClient client, object message)
    {
        ...
        if (isTrue(!isEqual(priceExponent, null)))
        {
            ...
            object priceMantissa = this.safeInteger(result, "price");
            if (isTrue(!isEqual(priceMantissa, null)))
            {
                ((IDictionary<string,object>)normalized)["price"] = ((this.applyExponent(priceMantissa, priceExponent)).ToString());
            }
            // Handle bid/ask fields (for ticker.book)
            object bidPriceMantissa = this.safeInteger(result, "bidPrice");
            if (isTrue(!isEqual(bidPriceMantissa, null)))
            {
                ((IDictionary<string,object>)normalized)["bidPrice"] = ((this.applyExponent(bidPriceMantissa, priceExponent)).ToString());
            }
            object bidQtyMantissa = this.safeInteger(result, "bidQty");
            if (isTrue(!isEqual(bidQtyMantissa, null)))
            {
                ((IDictionary<string,object>)normalized)["bidQty"] = ((this.applyExponent(bidQtyMantissa, qtyExponent)).ToString());
            }
            object askPriceMantissa = this.safeInteger(result, "askPrice");
            if (isTrue(!isEqual(askPriceMantissa, null)))
            {
                ((IDictionary<string,object>)normalized)["askPrice"] = ((this.applyExponent(askPriceMantissa, priceExponent)).ToString());
            }
            object askQtyMantissa = this.safeInteger(result, "askQty");
            if (isTrue(!isEqual(askQtyMantissa, null)))
            {
                ((IDictionary<string,object>)normalized)["askQty"] = ((this.applyExponent(askQtyMantissa, qtyExponent)).ToString());
            }
            // Copy other fields
            object lastUpdateId = this.safeInteger(result, "lastUpdateId");
            if (isTrue(!isEqual(lastUpdateId, null)))
            {
                ((IDictionary<string,object>)normalized)["lastUpdateId"] = lastUpdateId;
            }
            result = normalized;
        }
    ...
    public virtual void handleAccountStatusWs(WebSocketClient client, object message)
    {
        ...
        object commissionExponent = this.safeInteger(result, "commissionExponent");
        if (isTrue(!isEqual(commissionExponent, null)))
        {
            // SBE format - normalize to JSON format
            object normalized = new Dictionary<string, object>() {};
            // Convert commission rates from mantissa
            object makerMantissa = this.safeInteger(result, "commissionRateMaker");
            object takerMantissa = this.safeInteger(result, "commissionRateTaker");
            object buyerMantissa = this.safeInteger(result, "commissionRateBuyer");
            object sellerMantissa = this.safeInteger(result, "commissionRateSeller");
            ((IDictionary<string,object>)normalized)["commissionRates"] = new Dictionary<string, object>() {
                { "maker", ((object)(this.applyExponent(makerMantissa, commissionExponent))).ToString() },
                { "taker", ((object)(this.applyExponent(takerMantissa, commissionExponent))).ToString() },
                { "buyer", ((object)(this.applyExponent(buyerMantissa, commissionExponent))).ToString() },
                { "seller", ((object)(this.applyExponent(sellerMantissa, commissionExponent))).ToString() },
            };
            ...
            // Copy boolean fields
            ((IDictionary<string,object>)normalized)["canTrade"] = this.safeValue(result, "canTrade");
            ((IDictionary<string,object>)normalized)["canWithdraw"] = this.safeValue(result, "canWithdraw");
            ((IDictionary<string,object>)normalized)["canDeposit"] = this.safeValue(result, "canDeposit");
            ((IDictionary<string,object>)normalized)["brokered"] = this.safeValue(result, "brokered");
            ((IDictionary<string,object>)normalized)["requireSelfTradePrevention"] = this.safeValue(result, "requireSelfTradePrevention");
            ((IDictionary<string,object>)normalized)["preventSor"] = this.safeValue(result, "preventSor");
            // Convert updateTime from microseconds to milliseconds
            object updateTime = this.safeInteger(result, "updateTime");
            if (isTrue(!isEqual(updateTime, null)))
            {
                ((IDictionary<string,object>)normalized)["updateTime"] = (Math.Floor(Double.Parse((divide(updateTime, 1000)).ToString())));
            }
            // Copy other fields
            ((IDictionary<string,object>)normalized)["accountType"] = this.safeValue(result, "accountType");
            ((IDictionary<string,object>)normalized)["tradeGroupId"] = this.safeInteger(result, "tradeGroupId");
            ((IDictionary<string,object>)normalized)["uid"] = this.safeInteger(result, "uid");
            // Convert balances array
            object balances = this.safeList(result, "balances", new List<object>() {});
            ((IDictionary<string,object>)normalized)["balances"] = new List<object>() {};
            for (object i = 0; isLessThan(i, getArrayLength(balances)); postFixIncrement(ref i))
            {
                object balance = getValue(balances, i);
                object exponent = this.safeInteger(balance, "exponent", 0);
                object freeMantissa = this.safeInteger(balance, "free", 0);
                object lockedMantissa = this.safeInteger(balance, "locked", 0);
                ((IList<object>)getValue(normalized, "balances")).Add(new Dictionary<string, object>() {
                    { "asset", this.safeString(balance, "asset") },
                    { "free", ((object)(this.applyExponent(freeMantissa, exponent))).ToString() },
                    { "locked", ((object)(this.applyExponent(lockedMantissa, exponent))).ToString() },
                });
            }
            // Copy permissions and reduceOnlyAssets arrays as-is
            ((IDictionary<string,object>)normalized)["permissions"] = this.safeList(result, "permissions", new List<object>() {});
            ((IDictionary<string,object>)normalized)["reduceOnlyAssets"] = this.safeList(result, "reduceOnlyAssets", new List<object>() {});
            result = normalized;
        }
    ...
    public override object normalizeSbeOrder(object order)
    {
        // Check if this is SBE format (has exponent fields)
        object priceExponent = this.safeInteger(order, "priceExponent");
        if (isTrue(isEqual(priceExponent, null)))
        {
        ...
        // SBE format - normalize to JSON format
        object qtyExponent = this.safeInteger(order, "qtyExponent", 0);
        object normalized = new Dictionary<string, object>() {};
        // Copy non-price/qty fields directly
        ((IDictionary<string,object>)normalized)["orderId"] = this.safeInteger(order, "orderId");
        ((IDictionary<string,object>)normalized)["orderListId"] = this.safeInteger(order, "orderListId");
        ((IDictionary<string,object>)normalized)["symbol"] = this.safeString(order, "symbol");
        ((IDictionary<string,object>)normalized)["clientOrderId"] = this.safeString(order, "clientOrderId");
        ((IDictionary<string,object>)normalized)["origClientOrderId"] = this.safeString(order, "origClientOrderId");
        // Convert timestamps from microseconds to milliseconds
        object transactTime = this.safeInteger(order, "transactTime");
        if (isTrue(!isEqual(transactTime, null)))
        {
            ((IDictionary<string,object>)normalized)["transactTime"] = (Math.Floor(Double.Parse((divide(transactTime, 1000)).ToString())));
        }
        object workingTime = this.safeInteger(order, "workingTime");
        if (isTrue(!isEqual(workingTime, null)))
        {
            ((IDictionary<string,object>)normalized)["workingTime"] = (Math.Floor(Double.Parse((divide(workingTime, 1000)).ToString())));
        }
        object time = this.safeInteger(order, "time");
        if (isTrue(!isEqual(time, null)))
        {
            ((IDictionary<string,object>)normalized)["time"] = (Math.Floor(Double.Parse((divide(time, 1000)).ToString())));
        }
        ...
        object trailingTime = this.safeInteger(order, "trailingTime");
        if (isTrue(!isEqual(trailingTime, null)))
        {
            ((IDictionary<string,object>)normalized)["trailingTime"] = (Math.Floor(Double.Parse((divide(trailingTime, 1000)).ToString())));
        }
        // Convert mantissa values to decimal strings
        object priceMantissa = this.safeInteger(order, "price");
        if (isTrue(!isEqual(priceMantissa, null)))
        {
            ((IDictionary<string,object>)normalized)["price"] = ((object)(this.applyExponent(priceMantissa, priceExponent))).ToString();
        }
        object origQtyMantissa = this.safeInteger(order, "origQty");
        if (isTrue(!isEqual(origQtyMantissa, null)))
        {
            ((IDictionary<string,object>)normalized)["origQty"] = ((object)(this.applyExponent(origQtyMantissa, qtyExponent))).ToString();
        }
        object executedQtyMantissa = this.safeInteger(order, "executedQty");
        if (isTrue(!isEqual(executedQtyMantissa, null)))
        {
            ((IDictionary<string,object>)normalized)["executedQty"] = ((object)(this.applyExponent(executedQtyMantissa, qtyExponent))).ToString();
        }
        object cummulativeQuoteQtyMantissa = this.safeInteger(order, "cummulativeQuoteQty");
        if (isTrue(!isEqual(cummulativeQuoteQtyMantissa, null)))
        {
            ((IDictionary<string,object>)normalized)["cummulativeQuoteQty"] = ((object)(this.applyExponent(cummulativeQuoteQtyMantissa, priceExponent))).ToString();
        }
        object stopPriceMantissa = this.safeInteger(order, "stopPrice");
        if (isTrue(!isEqual(stopPriceMantissa, null)))
        {
            ((IDictionary<string,object>)normalized)["stopPrice"] = ((object)(this.applyExponent(stopPriceMantissa, priceExponent))).ToString();
        }
        object icebergQtyMantissa = this.safeInteger(order, "icebergQty");
        if (isTrue(!isEqual(icebergQtyMantissa, null)))
        {
            ((IDictionary<string,object>)normalized)["icebergQty"] = ((object)(this.applyExponent(icebergQtyMantissa, qtyExponent))).ToString();
        }
        object preventedQuantityMantissa = this.safeInteger(order, "preventedQuantity");
        if (isTrue(!isEqual(preventedQuantityMantissa, null)))
        {
            ((IDictionary<string,object>)normalized)["preventedQuantity"] = ((object)(this.applyExponent(preventedQuantityMantissa, qtyExponent))).ToString();
        }
        object origQuoteOrderQtyMantissa = this.safeInteger(order, "origQuoteOrderQty");
        if (isTrue(!isEqual(origQuoteOrderQtyMantissa, null)))
        {
            ((IDictionary<string,object>)normalized)["origQuoteOrderQty"] = ((object)(this.applyExponent(origQuoteOrderQtyMantissa, priceExponent))).ToString();
        }
        object peggedPriceMantissa = this.safeInteger(order, "peggedPrice");
        if (isTrue(!isEqual(peggedPriceMantissa, null)))
        {
            ((IDictionary<string,object>)normalized)["peggedPrice"] = ((object)(this.applyExponent(peggedPriceMantissa, priceExponent))).ToString();
        }
        // Copy enum fields - they need to be mapped to their string equivalents
        // The parseOrder method handles this mapping
        ((IDictionary<string,object>)normalized)["status"] = this.safeValue(order, "status");
        ((IDictionary<string,object>)normalized)["timeInForce"] = this.safeValue(order, "timeInForce");
        ((IDictionary<string,object>)normalized)["type"] = this.safeValue(order, "orderType"); // Note: orderType maps to 'type' in JSON
        ((IDictionary<string,object>)normalized)["side"] = this.safeValue(order, "side");
        ((IDictionary<string,object>)normalized)["orderCapacity"] = this.safeValue(order, "orderCapacity");
        ((IDictionary<string,object>)normalized)["workingFloor"] = this.safeValue(order, "workingFloor");
        ((IDictionary<string,object>)normalized)["selfTradePreventionMode"] = this.safeValue(order, "selfTradePreventionMode");
        ((IDictionary<string,object>)normalized)["usedSor"] = this.safeValue(order, "usedSor");
        ((IDictionary<string,object>)normalized)["isWorking"] = this.safeValue(order, "isWorking");
        ((IDictionary<string,object>)normalized)["pegPriceType"] = this.safeValue(order, "pegPriceType");
        ((IDictionary<string,object>)normalized)["pegOffsetType"] = this.safeValue(order, "pegOffsetType");
        ((IDictionary<string,object>)normalized)["pegOffsetValue"] = this.safeValue(order, "pegOffsetValue");
        // Copy other fields
        ((IDictionary<string,object>)normalized)["trailingDelta"] = this.safeInteger(order, "trailingDelta");
        ((IDictionary<string,object>)normalized)["strategyId"] = this.safeInteger(order, "strategyId");
        ((IDictionary<string,object>)normalized)["strategyType"] = this.safeInteger(order, "strategyType");
        ...
        if (isTrue(isGreaterThan(getArrayLength(preventedMatches), 0)))
        {
            ((IDictionary<string,object>)normalized)["preventedMatches"] = new List<object>() {};
            for (object i = 0; isLessThan(i, getArrayLength(preventedMatches)); postFixIncrement(ref i))
            {
                object match = getValue(preventedMatches, i);
    ...
    public virtual void handleOrdersWs(WebSocketClient client, object message)
    {
        ...
        if (isTrue(((result is IList<object>) || (result.GetType().IsGenericType && result.GetType().GetGenericTypeDefinition().IsAssignableFrom(typeof(List<>))))))
        {
            ...
            for (object i = 0; isLessThan(i, getArrayLength(result)); postFixIncrement(ref i))
            {
                ((IList<object>)orders).Add(this.normalizeSbeOrder(getValue(result, i)));
            }
        } else if (isTrue((result is IDictionary<string, object>)))
        {
            // SBE format - result has an orders array
            object ordersArray = this.safeList(result, "orders", new List<object>() {});
            orders = new List<object>() {};
            for (object i = 0; isLessThan(i, getArrayLength(ordersArray)); postFixIncrement(ref i))
            {
                ((IList<object>)orders).Add(this.normalizeSbeOrder(getValue(ordersArray, i)));
            }
        }
    ...
    public virtual void handleTradesWs(WebSocketClient client, object message)
    {
        ...
        if (isTrue(!isEqual(result, null)))
        {
            ...
            } else
            {
                ...
                object tradesArray = this.safeList(result, "trades", new List<object>() {});
                if (isTrue(isGreaterThan(getArrayLength(tradesArray), 0)))
                {
                    // Convert mantissa values to decimal strings
                    // Check if exponents are at parent level (TradesResponse) or per-trade (AccountTradesResponse)
                    object parentPriceExponent = this.safeInteger(result, "priceExponent");
                    object parentQtyExponent = this.safeInteger(result, "qtyExponent");
                    object parentCommissionExponent = this.safeInteger(result, "commissionExponent");
                    object normalizedTrades = new List<object>() {};
                    for (object i = 0; isLessThan(i, getArrayLength(tradesArray)); postFixIncrement(ref i))
                    {
                        object trade = getValue(tradesArray, i);
                        ...
                        object priceExponent = this.safeInteger(trade, "priceExponent", parentPriceExponent);
                        object qtyExponent = this.safeInteger(trade, "qtyExponent", parentQtyExponent);
                        object commissionExponent = this.safeInteger(trade, "commissionExponent", parentCommissionExponent);
                        object price = this.applyExponent(getValue(trade, "price"), priceExponent);
                        object qty = this.applyExponent(getValue(trade, "qty"), qtyExponent);
                        object quoteQty = this.applyExponent(getValue(trade, "quoteQty"), priceExponent);
                        object timestamp = (Math.Floor(Double.Parse((divide(getValue(trade, "time"), 1000)).ToString()))); // microseconds to milliseconds
                        object normalized = new Dictionary<string, object>() {
                            { "id", ((object)(getValue(trade, "id"))).ToString() },
                            { "price", ((object)(price)).ToString() },
                            { "qty", ((object)(qty)).ToString() },
                            { "quoteQty", ((object)(quoteQty)).ToString() },
                            { "time", timestamp },
                            { "isBuyerMaker", isEqual(getValue(trade, "isBuyerMaker"), 1) },
                            { "isBestMatch", isEqual(getValue(trade, "isBestMatch"), 1) },
                        };
                        // Handle AccountTradesResponse specific fields
                        object orderId = this.safeInteger(trade, "orderId");
                        if (isTrue(!isEqual(orderId, null)))
                        {
                            ((IDictionary<string,object>)normalized)["orderId"] = orderId;
                            ((IDictionary<string,object>)normalized)["orderListId"] = this.safeInteger(trade, "orderListId");
                            ((IDictionary<string,object>)normalized)["symbol"] = this.safeString(trade, "symbol");
                            // Convert commission from mantissa
                            object commissionMantissa = this.safeInteger(trade, "commission");
                            if (isTrue(!isEqual(commissionMantissa, null)))
                            {
                                ((IDictionary<string,object>)normalized)["commission"] = ((this.applyExponent(commissionMantissa, commissionExponent)).ToString());
                            }
                            ((IDictionary<string,object>)normalized)["commissionAsset"] = this.safeString(trade, "commissionAsset");
                            ((IDictionary<string,object>)normalized)["isBuyer"] = this.safeBool(trade, "isBuyer", false);
                            ((IDictionary<string,object>)normalized)["isMaker"] = this.safeBool(trade, "isMaker", false);
                        }
                        ((IList<object>)normalizedTrades).Add(normalized);
                    }
