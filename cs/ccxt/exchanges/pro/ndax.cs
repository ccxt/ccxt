    ...
    public virtual void handleOHLCV(WebSocketClient client, object message)
    {
        ...
        for (int i = 0; isLessThan(i, payload.Count); postFixIncrement(ref i))
        {
            ...
            for (int j = 0; isLessThan(j, keys.Count); postFixIncrement(ref j))
            {
                ...
                if ((isGreaterThan(length, 0)) && (isEqual(((List<object>)parsed)[0], getValue(getValue(stored, subtract(length, 1)), 0))))
                {
                    ...
                    ((List<object>)stored)[Convert.ToInt32(subtract(length, 1))] = new List<object>() {((List<object>)parsed)[0], getValue(previous, 1), high, low, ((List<object>)parsed)[4], this.sum(((List<object>)parsed)[5], getValue(previous, 5))};
                ...
                {
                    if ((isGreaterThan(length, 0)) && (isLessThan(this.parseToInt(((List<object>)parsed)[0]), this.parseToInt(getValue(getValue(stored, subtract(length, 1)), 0)))))
                    {
                        continue;
                    } else
                    {
