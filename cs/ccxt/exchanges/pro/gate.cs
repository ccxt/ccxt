...
public partial class gate : ccxt.gate
{
    ...
    public async override Task<List<ccxt.Order>> CancelAllOrdersWs(string symbol = null, object parameters = null)
    {
        ...
        var requestrequestParamsVariable = ((bool) (isEqual(type, "spot"))) ? this.multiOrderSpotPrepareRequest(market, trigger, query) : this.prepareRequest(market, type, query);
    ...
    public async override Task<ccxt.Order> CancelOrderWs(string id, string symbol = null, object parameters = null)
    {
        ...
        var type = ((IList<object>) typequeryVariable)[0];
        var query = ((IList<object>) typequeryVariable)[1];
        var requestrequestParamsVariable = ((bool) (isEqual(type, "spot") || isEqual(type, "margin"))) ? this.spotOrderPrepareRequest(market, trigger, query) : this.prepareRequest(market, type, query);
        var request = ((IList<object>) requestrequestParamsVariable)[0];
        var requestParams = ((IList<object>) requestrequestParamsVariable)[1];
        object messageType = this.getTypeByMarket(market);
    ...
    public async override Task<object> unWatchOrderBook(object symbol, object parameters = null)
    {
        ...
        {
            limit = ((bool) ((((market.ContainsKey("spot") ? market["spot"] : null) as bool?) == true))) ? 50 : 100; // max 100 atm
            if (isEqual(messageType, "options"))
            {
    ...
    public virtual void handleOrderBook(WebSocketClient client, object message)
    {
        ...
        if (isEqual(nonce, null))
        {
            ...
            object waitAmount = ((bool) isSpot) ? snapshotDelay : 0;
    ...
    public async virtual Task<object> subscribeWatchTickersAndBidsAsks(object symbols = null, object callerMethodName = null, object parameters = null)
    {
        ...
        object result = ((bool) isWatchTickers) ? this.tickers : this.bidsasks;
    ...
    public virtual void handleTickerAndBidAsk(object objectName, object client, object message)
    {
        ...
        string marketType = ((bool) ((rawMarketType == "futures"))) ? "contract" : "spot";
    ...
    public virtual void handleOHLCV(WebSocketClient client, object message)
    {
        ...
        List<object> channelParts = ((string)channel).Split(new [] {((string)".")}, StringSplitOptions.None).ToList<object>();
        string? rawMarketType = this.safeString(channelParts, 0);
        string marketType = ((bool) ((rawMarketType == "spot"))) ? "spot" : "contract";
        object result = this.safeValue(message, "result");
        if (!((result is IList<object>) || (result.GetType().IsGenericType && result.GetType().GetGenericTypeDefinition().IsAssignableFrom(typeof(List<>)))))
        {
            result = new List<object>() {result};
    ...
    public async override Task<List<ccxt.Order>> WatchOrders(string symbol = null, Int64? since = null, Int64? limit = null, object parameters = null)
    {
        ...
        {
            suffix = ((bool) (isEqual(typeId, "spot"))) ? ".priceorders" : ".autoorders";
        }
        object channel = add(typeId, suffix);
        string messageHash = ((bool) (isEqual(isTrigger, true))) ? "triggerOrders" : "orders";
        List<object> payload = new List<object>() {("!" + "all")};
        if ((market != null))
        {
        ...
        for (int i = 0; i < (parsedOrders?.Count ?? 0); i++)
        {
            ...
            } else if ((eventVar == "finish"))
            {
                ...
                if ((status == null))
                {
                    ...
                    ((IDictionary<string,object>)parsed)["status"] = ((bool) ((left == 0))) ? "closed" : "canceled";
                }
            }
            callDynamically(stored, "append", new object[] {parsed});
