...
public partial class binance : Exchange
{
    ...
    public override object parseTrade(object trade, object market = null)
    {
        ...
        object isSpotTrade = isTrue(isTrue((inOp(trade, "isIsolated"))) || isTrue((inOp(trade, "M")))) || isTrue((inOp(trade, "orderListId")));
    ...
    public async override Task<object> fetchTradingFee(object symbol, object parameters = null)
    {
        ...
        */
        ...
        {
            data = this.safeValue(data, 0, new Dictionary<string, object>() {});
        }
    ...
    public override object sign(object path, object api = null, object method = null, object parameters = null, object headers = null, object body = null)
    {
        ...
        } else if (isTrue(isTrue(isTrue(isTrue(isTrue(isTrue(isTrue(isTrue(isTrue(isTrue(isTrue((isEqual(api, "private"))) || isTrue((isEqual(api, "eapiPrivate")))) || isTrue((isTrue(isEqual(api, "sapi")) && isTrue(!isEqual(path, "system/status"))))) || isTrue((isEqual(api, "sapiV2")))) || isTrue((isEqual(api, "sapiV3")))) || isTrue((isEqual(api, "sapiV4")))) || isTrue((isEqual(api, "dapiPrivate")))) || isTrue((isEqual(api, "dapiPrivateV2")))) || isTrue((isEqual(api, "fapiPrivate")))) || isTrue((isEqual(api, "fapiPrivateV2")))) || isTrue((isTrue(isEqual(api, "papi")) && isTrue(!isEqual(path, "ping"))))))
        {
            ...
            } else if (isTrue(isTrue(isTrue(isTrue((isEqual(path, "batchOrders"))) || isTrue((isGreaterThanOrEqual(getIndexOf(path, "sub-account"), 0)))) || isTrue((isEqual(path, "capital/withdraw/apply")))) || isTrue((isGreaterThanOrEqual(getIndexOf(path, "staking"), 0)))))
            {
                ...
                {
                    object orderidlist = this.safeValue(extendedParams, "orderidlist", new List<object>() {});
                    object origclientorderidlist = this.safeValue(extendedParams, "origclientorderidlist", new List<object>() {});
    ...
    public virtual object getExceptionsByUrl(object url, object exactOrBroad)
    {
        ...
        if (isTrue(((string)url).StartsWith(add(add("https://api.", hostname), "/"))))
        {
            marketType = "spot";
        } else if (isTrue(((string)url).StartsWith(add(add("https://dapi.", hostname), "/"))))
        {
            marketType = "inverse";
        } else if (isTrue(((string)url).StartsWith(add(add("https://fapi.", hostname), "/"))))
        {
            marketType = "linear";
        } else if (isTrue(((string)url).StartsWith(add(add("https://eapi.", hostname), "/"))))
        {
            marketType = "option";
        } else if (isTrue(((string)url).StartsWith(add(add("https://papi.", hostname), "/"))))
        {
            marketType = "portfoliomargin";
        }
        if (isTrue(!isEqual(marketType, null)))
        {
            object exceptionsForMarketType = this.safeValue(this.exceptions, marketType, new Dictionary<string, object>() {});
            return this.safeValue(exceptionsForMarketType, exactOrBroad, new Dictionary<string, object>() {});
        }
        return new Dictionary<string, object>() {};
    }
