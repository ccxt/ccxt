...
// Signature shared by the trait declaration and each wrapper's impl:
//   `<name>(&mut self, p: T, …) -> crate::Result<R>`
function typedMethodSignature(m: MethodInfo): string {
    const paramDecls = m.params.map(p => `${p.name}: ${p.rustType}`).join(', ');
    const sep = paramDecls.length ? ', ' : '';
    return `async fn ${m.rustName}(&mut self${sep}${paramDecls}) -> crate::Result<${m.rustReturn}>`;
}

// The `TypedExchange` trait: the unified interface every typed wrapper
// implements. The ONLY required method is `call_raw` (object-safe dynamic
// dispatch); every typed method is a DEFAULT that routes through it and decodes
// the result. This keeps the lib lean — each wrapper's impl is a single method,
// and the ~120 typed bodies are monomorphized at the caller's use sites rather
// than 120×N in the lib (a per-wrapper override set OOM-kills the build).
// `#[async_trait]` boxes the futures so the trait is object-safe
// (`Box<dyn TypedExchange>` — unlike raw `ExchangeBase`, which is `+ Sized`).
function genTypedExchangeTrait(methods: MethodInfo[]): string {
    const decl = (m: MethodInfo) => {
        ...
        return [
            `    /// Typed \`${m.tsName}\`.`,
            `    ${typedMethodSignature(m)} {`,
            `        let v = self.call_raw("${m.coreCall}", vec![${args}]).await?;`,
            `        Ok(${m.decodeExpr('v')})`,
            `    }`,
        ].join('\n');
    };
