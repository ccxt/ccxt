...
// java/tests (javaTranspiler.ts, generateJavaWrappers.ts, generateImplicitAPI.ts).
//
...
// `java.util.concurrent.CompletableFuture<Object>`, ...), and the hand-rolled emitters copy
// that habit. The helpers below rewrite those references to their simple names and add the
// matching single-type `import` declarations once at the top of the compilation unit. They
// run at WRITE time, after every upstream regex pass (which still match on the qualified
// spelling), so no pass and no pinned dependency needs patching.
//
...
// generated or hand-written type under java/ is named like any entry below
// (`grep -rE '\b(class|interface|enum|record)\s+(List|Map|...)\b' java/`).
...
// ConcurrentHashMap, Callable, Collections, Locale, Base64, zip.*, io.github.ccxt.types.*.
//
...
// Groups 2-4: the simple name of an allow-listed fully-qualified reference.
