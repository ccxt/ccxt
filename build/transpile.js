...
class Transpiler {
    ...
    createGeneratorPHPClassHeader (className, baseClass, bodyAsString) {
        return [
            "<?php",
            "",
            "namespace ccxt\\async\\generator;",
            "",
            "// PLEASE DO NOT EDIT THIS FILE, IT IS GENERATED AND WILL BE OVERWRITTEN:",
            "// https://github.com/ccxt/ccxt/blob/master/CONTRIBUTING.md#how-to-contribute-code",
            "",
    ...
    createAsyncPHPClassHeader (className, baseClass, bodyAsString) {
        return [
            "<?php",
            "",
    ...
    createPHPClass (className, baseClass, body, methods, async = '') {

        let bodyAsString = body.join ("\n")

        let header
        if (async === 'generator') {
            header = this.createGeneratorPHPClassHeader (className, baseClass, bodyAsString)
        } else if (async === 'async') {
            header = this.createAsyncPHPClassHeader (className, baseClass, bodyAsString)
        } else {
            header = this.createPHPClassHeader (className, baseClass, bodyAsString)
        }
    ...
    transpileGeneratorPHPToSyncPHP (php) {
    ...
    transpileJavaScriptToPythonAndPHP (args) {
        ...
        let generatorPhpBody = this.transpileJavaScriptToPHP (args)

        // transpile async PHP -> sync PHP
        let phpBody = this.transpileGeneratorPHPToSyncPHP (generatorPhpBody)

        return { python3Body, python2Body, phpBody, generatorPhpBody }
    }
    ...
    transpileDerivedExchangeClass (contents, methodNames = undefined) {
        ...
        let asyncBaseClass = 'generator\\' + className
        ...
        for (let i = 0; i < methods.length; i++) {
            ...
            let { python3Body, python2Body, phpBody, generatorPhpBody } = this.transpileJavaScriptToPythonAndPHP ({ js, className, variables, removeEmptyLines: true })
            ...
            generatorPhp.push ('');
            generatorPhp.push ('    public function ' + method + '(' + phpArgs + ') {');
            generatorPhp.push (generatorPhpBody);
            generatorPhp.push ('    }')

            // wrap the generators as react promises
            const rawsArgsRegex = / = .+?($|(?=,))/g
            if (generatorPhpBody.indexOf ('yield') > -1) {
                const rawArgs = phpArgs.replace (rawsArgsRegex, '')
                asyncPhp.push ('')
                asyncPhp.push ('    public function ' + method + '(' + phpArgs + ') {');
                asyncPhp.push ('        return $this->wrap_promise(parent::' + method + '(' + rawArgs + '));')
                asyncPhp.push ('    }')
            }
        }
        ...
        return {
            ...
            generatorPhp: this.createPHPClass    (className, baseClass, generatorPhp, methodNames, 'generator'),
            asyncPhp:     this.createPHPClass    (className, asyncBaseClass, asyncPhp, methodNames,'async'),
    ...
    transpileDerivedExchangeFile (jsFolder, filename, options) {
        ...
        try {
            ...
            const { python2Folder, python3Folder, phpFolder, generatorPhpFolder, asyncPhpFolder } = options
            ...
            const { python2, python3, php, generatorPhp, asyncPhp, className, baseClass } =
            ...
            ;[
                ...
                [ generatorPhpFolder, filename.replace ('.js', '.php'), generatorPhp ],
    ...
    transpileEverything () {
        ...
        const [ /* node */, /* script */, pattern ] = process.argv
            ...
            , generatorPhpFolder = './php/async/generator/'
            , options = { python2Folder, python3Folder, phpFolder, asyncPhpFolder, generatorPhpFolder }

        createFolderRecursively (python2Folder)
        createFolderRecursively (python3Folder)
        createFolderRecursively (phpFolder)
        createFolderRecursively (asyncPhpFolder)
        createFolderRecursively (generatorPhpFolder)
