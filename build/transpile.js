...
// insert common regexes in the middle (critical)
    ...
    [ /([^a-zA-Z0-9_])(?:let|const|var)\s/g, '$1' ],
    [ /\((!?)Object\.keys\s*\((.*)\)\.length\)/g, '$1($2.size > 0)' ], // looking for a boolean, not the length
    ...
    [ /parseFloat\s*/g, 'parse_float'],
    [ /parseInt\s*/g, 'parse_int'],
...
function createRubyClass (className, baseClass, body, methods) {
    ...
    const requireClass = (baseClass == 'Exchange') ?
        null : ( "require_relative '" + baseClass + "'\n")
