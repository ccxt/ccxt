...
class Transpiler {
    ...
    createPHPClassHeader (className, baseClass, bodyAsString, namespace, useException = true) {
        ...
        if (useException) {
            header.push( "use Exception; // a common import")
        }
        return header
    }
    ...
    createPHPTrait (className, body, methods, async = false) {
        ...
        let header = this.createPHPClassHeader (className, undefined, bodyAsString, async ? 'ccxt\\async' : 'ccxt', false)
    ...
    transpileExchangeCommonFiles(jsFolder, filename, className, options ) {
        ...
        let python3File = python3.join('\n')
        // comply with flake8 E302
        // two blank lines between functions
        python3File = python3File.replace(/^\s*\n/gm, '\n\n')
        // add W292 no newline at end of file
        python3File+= '\n'
        let python2File = python2.join('\n')
