...
`;
    ...
    writeModFile(folder: string, names: string[]) {
        // Each exchange gets its main `<id>.rs` plus an optional sibling
        // `<id>_api.rs` (auto-generated by `build/generateImplicitAPI.ts`
        // — typed wrappers per implicit endpoint, e.g.
        // `binance.sapi_get_account_info(...)`). Include the _api file
        // when present so the wrapper methods compile into the crate.
        // Filter out `_api`-suffixed entries from `names` so the existence
        // check below is the sole source of those `pub mod` lines (no dups).
        const baseNames = names.filter(n => !n.endsWith('_api'));
        const lines: string[] = [];
