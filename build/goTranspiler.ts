...
${constStatements.join('\n')}
    ...
    transpileBaseMethods(baseExchangeFile: string, isWs = false, force = true) {
        ...
        if (parts.length > 1) {
            ...
            const file = goPointerLocalNativeNilCompares (goParamNativeNilCompares (coerceGoBoolMethodReturns (fileHeader + baseMethods + "\n", CCXT_GO_BOOL_METHOD_NAMES), 'IsEqual('), 'IsEqual(');
    ...
    transpilePredictionBaseMethods (predictionBaseFile = './ts/src/base/PredictionExchange.ts', registerOnly = false, force = true) {
        ...
        if (parts.length > 1) {
            ...
            const file = goPointerLocalNativeNilCompares (goParamNativeNilCompares (fileHeader + '\n' + structDef + methods + shims, 'IsEqual('), 'IsEqual(');
...
async function runMain () {
    ...
    if (process.argv.includes ('--self-test')) {
        const problems = goDerefWrapSelfTest ().concat (goParamNilSelfTest ()).concat (goPointerLocalNilSelfTest ());
        if (problems.length) {
            console.error ('SELF-TEST FAILED:\n  - ' + problems.join ('\n  - '));
            process.exit (3);
        }
