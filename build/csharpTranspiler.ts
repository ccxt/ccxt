...
async function runMain () {
    ...
    // sign is async only in JS; in C# it is synchronous. Because the AST transpiler
    // resolves the base `Exchange.ts` (via `extends Exchange`) and base/functions/rsa.ts
    // (rsa/jwt) to infer return types / async-ness, those files must be
    // sign-synchronous ON DISK while exchanges + wrappers transpile. The top-level
    // process strips them before forking workers and restores them after; child
    // processes inherit the already-stripped files.
