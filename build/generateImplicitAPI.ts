...
//
// Two layers feed the decision, highest priority first:
//
//   1. RETURN_TYPE_OVERRIDES / PREDICTION_RETURN_TYPE_OVERRIDES below —
//      hand-written, for endpoints whose shape cannot be read off the call
//      sites (no caller, contradictory callers, or a genuinely multi-shape
//      endpoint). Each entry carries a comment recording the evidence.
//
//   2. build/implicitReturnTypes.json — generated by
//      build/classifyImplicitReturns.ts, which walks every
//      `await this.<endpoint> (...)` call site in ts/src/** and infers the
//      shape from how the value is used (`res['key']` → Dict, `res[0]` /
//      for-of → List, `.replace` / `=== 'OK'` → string). Regenerate with:
//
//          npx tsx build/classifyImplicitReturns.ts
//
// Endpoints that neither layer covers are the ones the library itself never
// calls, so there is no evidence to classify them by; they keep
// `implicitReturnType` (= Dict | List), which stays permissive for external
// callers without asserting a shape we cannot prove.
//
...
// resolved types (see typescriptImportedTypes).
//
...
// the classifier's per-endpoint verdicts, keyed by exchange id (prediction
// exchanges are namespaced 'prediction/<id>' because their ids collide)
const CLASSIFIED_RETURN_TYPES: { [exchangeId: string]: { [method: string]: string } } =
    JSON.parse (fs.readFileSync ('./build/implicitReturnTypes.json', 'utf8'));
...
function classifiedReturnTypes (exchange: string): { [method: string]: string } {
    const key = isPrediction ? ('prediction/' + exchange) : exchange;
    const classified = CLASSIFIED_RETURN_TYPES[key];
...
function typescriptReturnType (exchange: string, method: string): string {
    const override = returnTypeOverrides (exchange)[method];
    if (override !== undefined) {
        return override;
    }
    const classified = classifiedReturnTypes (exchange)[method];
...
function typescriptImportedTypes (exchange: string): string[] {
    ...
    for (const method of methods) {
        for (const name of typescriptReturnType (exchange, method).split (/[^A-Za-z]+/)) {
            const isKnown = (name === 'Dict') || (name === 'List') || (name === 'implicitReturnType');
            if (isKnown && !imported.includes (name)) {
                imported.push (name);
            }
        }
    }
