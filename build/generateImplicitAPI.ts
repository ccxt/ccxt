...
//
// The verdicts come from build/implicitReturnTypes.json, generated by
// build/classifyImplicitReturns.ts, which walks every `await this.<endpoint>
// (...)` call site in ts/src/** and infers the shape from how the value is used
// (`res['key']` → Dict, `res[0]` / for-of → List, `.replace` / `=== 'OK'` →
// string). The few endpoints whose shape no call site can prove — because they
// are genuinely multiplexed over the request body — are recorded in that
// script's own OVERRIDES table, next to the evidence for each one, so there is
// a single place where a return type is decided. Regenerate with:
//
//     npx tsx build/classifyImplicitReturns.ts
//
// Endpoints the file does not cover are the ones the library itself never
...
// ids, so they are keyed separately under 'prediction/<id>'.
