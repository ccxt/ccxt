...
/**
 * Ensure the house-style generated banner is present. Idempotent when the marker is already
 * in the file. Placement mirrors other generated ports:
 *   file-top        — Python (after optional coding cookie)
 *   after-namespace — C# (`namespace ccxt;` then banner)
 *   after-package   — Go (`package ccxt` then banner, before imports)
 *   after-imports   — Java (package + imports, then banner, then class)
 */
export function ensureGeneratedBanner (
    text: string,
    commentPrefix: '//' | '#',
    placement: 'file-top' | 'after-namespace' | 'after-package' | 'after-imports',
): string {
    if (text.indexOf (GENERATED_BANNER_MARKER) >= 0) {
        return text;
    }
    const banner = formatGeneratedBanner (commentPrefix);
    if (placement === 'file-top') {
        const coding = text.match (/^# -\*- coding: [^*]+-\*-\n/);
        if (coding !== null) {
            let rest = text.slice (coding[0].length);
            if (rest.startsWith ('\n')) {
                rest = rest.slice (1);
            }
            return coding[0] + '\n' + banner + '\n' + rest;
    ...
    if (placement === 'after-namespace') {
        const match = text.match (/^namespace [^\n]*\n/);
        if (match === null) {
            return banner + '\n' + text;
        }
    ...
    let i = 0;
    if (i < lines.length && lines[i].startsWith ('package ')) {
        i += 1;
    }
    ...
    out.push (commentPrefix + ' ' + GENERATED_BANNER_MARKER);
    out.push (commentPrefix + ' ' + GENERATED_BANNER_URL);
    out.push ('');
