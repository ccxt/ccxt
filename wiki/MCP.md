...
- **live (WebSocket)** — `watch_subscribe` opens a background ccxt.pro stream; `watch_read` returns fresh data (for **state** streams — the single‑symbol `watchTicker`/`watchOrderBook`, which return the complete current object each read — the snapshot in `latest`; for **event** streams — everything else, e.g. `watchTrades`/`watchOHLCV`/`watchOrders` — new/changed items in `events` with a cursor; event streams carry only what changed, so use the `get_*` tools for a full multi‑symbol snapshot; private streams need an account); `watch_unsubscribe` stops it; `watch_list` shows active streams. The long‑lived server holds the socket so the agent always has fresh data; idle streams auto‑stop after 10 minutes.
