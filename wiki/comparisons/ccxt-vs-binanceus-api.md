...
<!-- summary: A regional entity of Binance, shipped as its own CCXT class. Inherits 102 unified capabilities and 844 raw endpoints from `binance`. -->
<!-- weight: 400 -->
...
| | `binanceus` |
| --- | --- |
| Unified capabilities | 102 (inherited from `binance` plus its own overrides) |
| `fetch*` methods | 50 |
| WebSocket `watch*` methods | 23 |
| Raw endpoints as implicit methods | 844 |
...
It inherits 102 unified capabilities from `binance`. Where the venue does not offer a feature, the corresponding `has` flag is turned off, and the method raises `NotSupported` rather than failing quietly. Check `exchange.has` at runtime.

**Can I reach Binance.US-specific endpoints?**
Yes — all 844 endpoints in this class's `api` block are generated as implicit methods, with signing, rate limiting and error mapping applied. The mechanism is documented on the [`binance` implicit API page](/docs/exchanges/binance/implicit-api); this venue's own endpoint list differs slightly from the parent's.
