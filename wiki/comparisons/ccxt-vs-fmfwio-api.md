...
<!-- summary: A white-label venue of HitBTC, shipped as its own CCXT class. Inherits 53 unified capabilities and 111 raw endpoints from `hitbtc`. -->
<!-- weight: 400 -->
...
| | `fmfwio` |
| --- | --- |
| Unified capabilities | 53 (inherited from `hitbtc` plus its own overrides) |
| `fetch*` methods | 34 |
...
It inherits 53 unified capabilities from `hitbtc`. Where the venue does not offer a feature, the corresponding `has` flag is turned off, and the method raises `NotSupported` rather than failing quietly. Check `exchange.has` at runtime.
