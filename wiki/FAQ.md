# Frequently Asked Questions
  ...
  Some exchanges support `createOrder` with the additional "embedded" `stopLoss` & `takeProfit` sub-orders - check [StopLoss And TakeProfit Orders Attached To A Position](Manual.md#stoploss-and-takeprofit-orders-attached-to-a-position). 
  However, some exchanges might not support that feature and you will need to run separate `createOrder` methods to add conditional order (e.g. ***trigger order | stoploss order | takeprofit order**) to an already open, existing position - check [StopLoss And TakeProfit Orders Attached To A Position](Manual.md#Conditional Orders)
