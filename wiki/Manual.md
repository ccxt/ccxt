...
All data that you are getting with CCXT is received directly from the exchanges' API engine. Typically, exchanges construct "secondary data" (OHLCV bars, tickers, etc) from the realtime "first-hand" data" (trades, orderbook ...). However, some exchanges might have imperfect engine, and their constructed OHLCV bars might have some imperfections too, for example, values not being calculated at :00 offset on universal clock, but few milli/seconds later, because it takes some time for the exchange hardware to collect the first-hand data and generate the secondary statistical data from it. So, there is always some latency between the moment when a trade happens and the moment when a corresponding OHLCV candle is updated & published by the exchange API. Some exchanges might do it in regular intervals like once a second or once in a few seconds, but some slow exchanges might take minutes to update the secondary statistical information.

If there are users who are not willing to tolerate such minor discrepancy, then they still have a way to manually construct OHLCV data. For that, you have to aggregate the public "trades" (either historical or realtime data). Please take a look into "build-ohlcv-bars" file inside [examples folder](https://github.com/ccxt/ccxt/tree/master/examples) to find out how it is being done using `buildOHLCVC/build_ohlcvc` helper methods. The latency remains exchange-specific, cause the exchange engine still needs time to calculate and distribute data, however using WebSockets the latency will be reduced as much as possible. But note, that approach is a bit tricky also and you will need to handle redundancy, "data holes" in the history, exchange downtimes, and/or other aspects of provided data from exchange, and unfortunately it's not possible that we covered all those nuances in this manual.
