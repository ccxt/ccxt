...
If there are users who are not willing to tolerate such discrepancy or your trading strategy is very time-critical, then you don't have to rely on secondary data from the exchanges and you can manually construct OHLCV data - please take a look into "build-ohlcv-bars" file inside [examples folder](https://github.com/ccxt/ccxt/tree/master/examples). As you see have to aggregate the public "trades" using REST polling `fetchTrades` or WebSocket `watchTrades`, and then generate OHLCV bars from them using `build_ohlcvc` helper method. WebSocket is preffered approach because of the lower latency and performancebut it will not make a slow exchange engine to work faster.
