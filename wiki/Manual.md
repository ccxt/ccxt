...
If there are users who are not willing to tolerate such discrepancy, then they can manually construct OHLCV data - please take a look into "build-ohlcv-bars" file inside [examples folder](https://github.com/ccxt/ccxt/tree/master/examples). As you see have to aggregate the public "trades" using REST polling `fetchTrades` or WebSocket `watchTrades` (WebSocket is preffered approach because of the lower latency and performance) and then generate OHLCV bars from them using `build_ohlcvc` helper method.
