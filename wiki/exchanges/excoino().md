...
<a name="excoino" id="excoino"></a>

## excoino{docsify-ignore}
**Kind**: global class  
...
**Kind**: instance method of [<code>excoino</code>](#excoino)  
...
```javascript
excoino.fetchTickers (symbols[, params])
```
...
```javascript
excoino.fetchTicker (symbol[, params])
```
...
```javascript
excoino.fetchOHLCV (symbol, timeframe[, since, limit, params])
```
...
```javascript
excoino.fetchOrderBooks (symbols[, limit, params])
```
