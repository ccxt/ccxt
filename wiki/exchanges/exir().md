...
**Kind**: instance method of [<code>exir</code>](#exir)  
...
```javascript
exir.fetchTickers (symbols[, params])
```
...
```javascript
exir.fetchTicker (symbol[, params])
```
...
```javascript
exir.fetchOHLCV (symbol, timeframe[, since, limit, params])
```
...
```javascript
exir.fetchOrderBooks (symbols[, limit, params])
```
