...
<a name="ramzinex" id="ramzinex"></a>

## ramzinex{docsify-ignore}
**Kind**: global class  
...
**Kind**: instance method of [<code>ramzinex</code>](#ramzinex)  
...
```javascript
ramzinex.fetchTickers (symbols[, params])
```
...
```javascript
ramzinex.fetchTicker (symbol[, params])
```
...
```javascript
ramzinex.fetchOHLCV (symbol, timeframe[, since, limit, params])
```
...
```javascript
ramzinex.fetchOrderBooks (symbols[, limit, params])
```
