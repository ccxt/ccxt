...
<a name="sarrafex" id="sarrafex"></a>

## sarrafex{docsify-ignore}
**Kind**: global class  
...
**Kind**: instance method of [<code>sarrafex</code>](#sarrafex)  
...
```javascript
sarrafex.fetchTickers (symbols[, params])
```
...
```javascript
sarrafex.fetchTicker (symbol[, params])
```
...
```javascript
sarrafex.fetchOHLCV (symbol, timeframe[, since, limit, params])
```
...
```javascript
sarrafex.fetchOrderBooks (symbols[, limit, params])
```
