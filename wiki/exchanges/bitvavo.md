...
| params.clientOrderId | <code>string</code> | No | An ID supplied by the client that must be unique among all open orders for the same market |
...
| params.selfTradePrevention | <code>string</code> | No | "decrementAndCancel", "cancelOldest", "cancelNewest", "cancelBoth" |
| params.responseRequired | <code>bool</code> | No | Set this to 'false' when only an acknowledgement of success or failure is required, this is faster. |
| params.clientOrderId | <code>string</code> | No | An ID supplied by the client |


```javascript
bitvavo.editOrder (symbol, type, side, amount[, price, params])
```
