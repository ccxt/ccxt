Every endpoint in `kucoineu`'s `api` definition is exposed as an **implicit method** — a thin, generated wrapper around the raw exchange endpoint. Use these for exchange-specific functionality the [CCXT API](/docs/exchanges/kucoineu) does not cover.
...
```javascript
const kucoineu = new ccxt.kucoineu ();
const response = await kucoineu.publicGetCurrencies (params);
```
...
```python
import ccxt
kucoineu = ccxt.kucoineu()
response = kucoineu.public_get_currencies(params)
```

#### **PHP**

```php
$kucoineu = new \ccxt\kucoineu();
$response = $kucoineu->public_get_currencies($params);
```
...
```csharp
using ccxt;
var kucoineu = new Kucoineu();
var response = await kucoineu.publicGetCurrencies(parameters);
```

#### **Go**

```go
kucoineu := ccxt.NewKucoineu(nil)
response := <-kucoineu.PublicGetCurrencies(params)
```
...
📚 **Official kucoineu API documentation:** [kucoin.com](https://www.kucoin.com/en-eu/docs-new)

> 351 implicit endpoints across 9 access groups.

## public

**Base URL**: `https://api.kucoin.eu`
