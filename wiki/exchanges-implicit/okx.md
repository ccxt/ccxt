Every endpoint in `okx`'s `api` definition is exposed as an **implicit method** — a thin, generated wrapper around the raw exchange endpoint. Use these for exchange-specific functionality the [CCXT API](/docs/exchanges/okx) does not cover.
...
const response = await okx.publicGetMarketTickers (params);
```
...
import ccxt
okx = ccxt.okx()
response = okx.public_get_market_tickers(params)
```
...
using ccxt;
var okx = new Okx();
var response = await okx.publicGetMarketTickers(parameters);
```
...
```go
okx := ccxt.NewOkx(nil)
response := <-okx.PublicGetMarketTickers(params)
```
