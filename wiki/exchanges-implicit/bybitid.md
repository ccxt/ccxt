Every endpoint in `bybitid`'s `api` definition is exposed as an **implicit method** — a thin, generated wrapper around the raw exchange endpoint. Use these for exchange-specific functionality the [CCXT API](/docs/exchanges/bybitid) does not cover.
...
```javascript
const bybitid = new ccxt.bybitid ();
const response = await bybitid.publicGetSpotV3PublicSymbols (params);
```
...
```python
import ccxt
bybitid = ccxt.bybitid()
response = bybitid.public_get_spot_v3_public_symbols(params)
```

#### **PHP**

```php
$bybitid = new \ccxt\bybitid();
$response = $bybitid->public_get_spot_v3_public_symbols($params);
```
...
```csharp
using ccxt;
var bybitid = new Bybitid();
var response = await bybitid.publicGetSpotV3PublicSymbols(parameters);
```

#### **Go**

```go
bybitid := ccxt.NewBybitid(nil)
response := <-bybitid.PublicGetSpotV3PublicSymbols(params)
```
...
📚 **Official bybitid API documentation:** [bybit-exchange.github.io](https://bybit-exchange.github.io/docs/v5/intro) · [github.com](https://github.com/bybit-exchange)
