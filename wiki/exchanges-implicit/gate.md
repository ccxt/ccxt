Every endpoint in `gate`'s `api` definition is exposed as an **implicit method** — a thin, generated wrapper around the raw exchange endpoint. Use these for exchange-specific functionality the [CCXT API](/docs/exchanges/gate) does not cover.
...
import ccxt
gate = ccxt.gate()
response = gate.public_wallet_get_currency_chains(params)
```
...
using ccxt;
var gate = new Gate();
var response = await gate.publicWalletGetCurrencyChains(parameters);
```

#### **Go**

```go
gate := ccxt.NewGate(nil)
response := <-gate.PublicWalletGetCurrencyChains(params)
```
