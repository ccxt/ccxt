...
function injectMissingUndefined(fn, args) {
    ...
    const argsContainsParams = args.find((arg) => arg
        && typeof arg === 'object'
        && !Array.isArray(arg)
...
function createRequestTemplate(cliOptions, exchange, methodName, args, result) {
    ...
    log('Report: (paste inside static/request/'
        + exchange.id
        + '.json ->'
        + methodName
        + ')');
...
function createResponseTemplate(cliOptions, exchange, methodName, args, result) {
    ...
    log('Report: (paste inside static/response/'
        + exchange.id
        + '.json ->'
        + methodName
        + ')');
...
function printUsage(commandToShow) {
    ...
    log('Usage:', commandToShow, 'id'.green, 'method'.yellow, '"param1" param2 "param3" param4 ...'.blue);
    log('Examples:');
...
const printHumanReadable = (exchange, result, cliOptions, useTable = false) => {
    ...
    if (cliOptions.table && Array.isArray(result) || useTable) {
        ...
        if (arrayOfObjects || (cliOptions.table && Array.isArray(result))) {
            ...
            log(result.length > 0
                ? configuredAsTable(result.map((rawElement) => {
                    const element = { ...rawElement };
                    const keys = Object.keys(element);
                    delete element['info'];
                    keys.forEach((key) => {
                        if (!cliOptions.iso8601)
                            return element[key];
                        try {
 ...
 * @param forceCache
 */
async function handleMarketsLoading(exchange, forceRefresh = false) {
    const cachePath = getCacheDirectory();
 ...
 * @param cliOptions
 */
function parseMethodArgs(exchange, params, methodName, cliOptions, inject = true) {
    let args = params
        .map((s) => (s.match(/^[0-9]{4}[-][0-9]{2}[-][0-9]{2}[T\s]?[0-9]{2}[:][0-9]{2}[:][0-9]{2}/g)
        ...
        if (cliOptions.keys) {
            ...
            for (const [credential, isRequired] of Object.entries(requiredCredentials)) {
                if (isRequired && exchange[credential] === undefined) {
                    const credentialEnvName = (exchangeId
                        + '_'
                        + credential).toUpperCase(); // example: KRAKEN_APIKEY
...
function createCCXTExchange(exchangeId) {
...
function printExchangeMethods(exchangeId) {
...
function printMethodUsage(methodName) {
