...
module.exports = class vega extends Exchange {
    ...
    async fetchTicker (symbol, params = {}) {
        if (symbol === undefined) {
            throw new ArgumentsRequired (
                this.id + ' fetchTicker() requires a symbol argument'
            );
        }
    ...
    async fetchPositions (symbols = undefined, params = {}) {
        // if (symbol === undefined) {
        //   throw new ArgumentsRequired(
        //     this.id + ' fetchPositions() requires a symbol argument'
        //   );
        // }
        // await this.loadMarkets();
        // const market = this.market(symbol);
    ...
    // }
        ...
        const market = this.safeString (marketData, 'market');
        return {
            'market': market,
        };
    }
    ...
    // }
        ...
        return keys.reduce ((ret, k) => {
            ret[k] = this.safeString (order, k, null);
            return ret;
        }, {});
    }
    ...
    // }
        ...
        return keys.reduce ((ret, k) => {
            ret[k] = this.safeString (position, k, null);
            return ret;
        }, {});
    }

    sign (path, api, method = 'GET', params = {}, headers = {}, body = undefined) {
        ...
        if (isPrivate) {
            ...
            url = this.implodeHostname (
                this.urls['test'][isPrivateWallet ? 'privateWallet' : 'privateNode']
            ) + url;
        } else {
