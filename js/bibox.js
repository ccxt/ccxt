...
module.exports = class bibox extends Exchange {
    ...
    async fetchCurrenciesPrivate (params = {}) {
        ...
        for (let i = 0; i < currencies.length; i++) {
            ...
            const code = this.safeCurrencyCode (id);
            const backupPrecision = this.numberToString (this.precisionFromString (this.safeString (currency, 'total_amount')));
            const precision = this.parseNumber (this.parsePrecision (this.safeString ('currency', 'original_decimals', backupPrecision)));
            const deposit = this.safeValue (currency, 'enable_deposit');
            const withdraw = this.safeValue (currency, 'enable_withdraw');
            const active = (deposit && withdraw);
            result[code] = {
