...
module.exports = class bitget extends Exchange {
    ...
    async cancelOrder (id, symbol = undefined, params = {}) {
        ...
        let result = ('data' in response) ? response : this.safeValue (response, market['id'], {});
        //
        //
        // futures, swap
        //
    ...
    sign_sha (signParam) {
        const secretkeySHA = encodeURIComponent (this.hash (this.secret, 'sha1'));
        const keyInput = new ArrayBuffer (64);
        const keyOutput = new ArrayBuffer (64);
        for (let i = 0; i < 64; i++) {
        ...
        const key1 = new ArrayBuffer (secretkeySHA.length);
        const data1 = new ArrayBuffer (signParam.length);
        ...
        let keyInputCopy = '';
        let data1Copy = '';
        for (let xx = 0; xx < keyOutput.byteLength; xx++) {
            keyOutputCopy += String.fromCharCode (keyOutput[xx]);
        }
        for (let xx = 0; xx < keyInput.byteLength; xx++) {
            keyInputCopy += String.fromCharCode (keyInput[xx]);
        }
        for (let xx = 0; xx < data1.byteLength; xx++) {
