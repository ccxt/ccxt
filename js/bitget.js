...
module.exports = class bitget extends Exchange {
    ...
    async fetchOpenOrders (symbol = undefined, since = undefined, limit = undefined, params = {}) {
        ...
        //  '6': incomplete（open+partially filled),
        //  '7': complete（cancelled+fully filled),
        return await this.fetchOrdersByState ('0', symbol, since, limit, params);
    }
    ...
    fromCharCode (x) {
        return this.FROM_CHAR_CODE (x);
    }

    sign_sha (signParam) {
        const secretkeySHA = this.encodeURIComponent (this.hash (this.secret, 'sha1'));
        const keyInput = this.ArrayBuffer (64);
        const keyOutput = this.ArrayBuffer (64);
        for (let i = 0; i < 64; i++) {
        ...
        for (let j = 0; j < secretkeySHA.length; j++) {
            keyInput[j] = (secretkeySHA[j].charCodeAt ()) ^ 0x36;
            keyOutput[j] = (secretkeySHA[j].charCodeAt ()) ^ 0x5c;
        }
        const key1 = this.ArrayBuffer (secretkeySHA.length);
        const data1 = this.ArrayBuffer (signParam.length);
        for (let i = 0; i < secretkeySHA.length; i++) {
            key1[i] = secretkeySHA[i].charCodeAt ();
        }
        for (let i = 0; i < signParam.length; i++) {
            data1[i] = signParam[i].charCodeAt ();
        }
        let keyOutputCopy = '';
        let keyInputCopy = '';
        let data1Copy = '';
        for (let xx = 0; xx < keyOutput.length; xx++) {
            keyOutputCopy += this.fromCharCode (keyOutput[xx]);
        }
        for (let xx = 0; xx < keyInput.length; xx++) {
            keyInputCopy += this.fromCharCode (keyInput[xx]);
        }
        for (let xx = 0; xx < data1.length; xx++) {
            data1Copy += this.fromCharCode (data1[xx]);
        }
        let md5 = this.algo.MD5.create ();
        md5.update (keyInputCopy);
        md5.update (data1Copy);
        let md = md5.finalize ();
        md5 = this.algo.MD5.create ();
        md5.update (keyOutputCopy);
        md5.update (md, 0, 16);
        md = md5.finalize ().toString (this.enc.Hex);
        return md;
    }
    ...
    sign (path, api = 'public', method = 'GET', params = {}, headers = undefined, body = undefined) {
        ...
        if (type === 'public') {
            ...
            query['method'] = path;
            if (path === 'orders/{instrument_id}/{order_id}') {
                query['method'] = 'getOrderDetail';
            }
            if (path === 'orders/{instrument_id}') {
                query['method'] = 'orders';
            }
            if (path === 'cancel_order/{instrument_id}/{order_id}') {
            ...
            if (method === 'GET') {
                ...
                // }
                body = queryparam;
                url += '?accesskey=' + this.apiKey + '&sign=' + sign + '&req_time=' + timestamp;
