...
module.exports = class coingecko extends Exchange {
    ...
    async fetchMarkets () {
        ...
        for (let i = 0; i < coins.length; i++) {
            const coin = coins[i];
            for (let j = 0; j < this.vsCurrencies.length; j++) {
                markets.push ({
                    'id': `${coin['id']}/${this.vsCurrencies[j].toLowerCase ()}`,
                    'symbol': `${coin['symbol'].toUpperCase ()}/${this.vsCurrencies[j]}`,
                    'base': coin['symbol'].toUpperCase (),
                    'quote': this.vsCurrencies[j],
                    'baseId': coin['id'],
                    'quoteId': this.vsCurrencies[j].toLowerCase (),
                    'info': coin,
                });
            }
        }
