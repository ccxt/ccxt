...
module.exports = class bybit extends Exchange {
    ...
    async createUnifiedAccountOrder (symbol, type, side, amount, price = undefined, params = {}) {
        ...
        if (triggerPrice) {
        ...
        } else if (stopLossTriggerPrice || takeProfitTriggerPrice) {
            ...
            if (isStopLoss) {
                const stopLossPrice = this.safeNumber (isStopLoss, 'price');
                params = this.omit (params, 'stopLoss');
                request['stopLoss'] = this.priceToPrecision (symbol, stopLossPrice);
            }
            if (isTakeProfit) {
                const takeProfitPrice = this.safeNumber (isTakeProfit, 'price');
