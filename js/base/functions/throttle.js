...
function throttle (config) {
    function inner (cost = undefined) {
        ...
        if (this.queue.length > this.config['maxCapacity']) {
            throw new Error ('throttle queue is over maxCapacity (' + this.config['maxCapacity'] + '). For recommendation, see: https://github.com/ccxt/ccxt/issues/11645#issuecomment-1195695526');
        }
