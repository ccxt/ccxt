...
class ZeroExExchange extends Exchange {

    constructor () {
        super ();
        if (this.constructor === ZeroExExchange) {
    ...
    static async tokenRegistry () {
        const tokens = await ZeroExExchange.provider ().tokenRegistry.getTokensAsync ();
        const result = {};
    ...
    async tokenPairs () {
        ...
        for (let i = 0; i < marketsResponse.length; i++) {
            ...
            const base = ZeroExExchange.addressLookup (market.tokenA.address);
            const quote = ZeroExExchange.addressLookup (market.tokenB.address);
