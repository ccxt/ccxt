...
const { ArgumentsRequired, PermissionDenied, InsufficientFunds, InvalidOrder, ExchangeError, OrderNotFound, ExchangeNotAvailable, BadRequest } = require ('./base/errors');
...
module.exports = class timebit extends Exchange {
    ...
    sign (path, api = 'open', method = 'GET', params = {}, headers = undefined, body = undefined) {
        ...
        if (api === 'private') {
            ...
            let query = undefined;
             ...
             let _timestamp = Math.floor(+new Date() / 1000).toString();
             // 
            delete params.orderId;
            delete params.currency;
            query = this.urlencodeWithArrayRepeat (params);
            let _data = _timestamp.concat(method).concat(uri);
    ...
    convertSymbol (market) {
        ...
        let maker = parseFloat (0.1) / 100;;
        let taker = parseFloat (0.1) / 100;;
    ...
    preOptiomizeOrderBookData(orderBook) {
        ...
        let _tranferList = [];
        let _sellTranferList = [];
        for(let i = 0; i < sellList.length; i++) {
            let _sellOrder = sellList[i];
            let _sellTranferObj = [];
    ...
    async fetchTicker (symbol, params = {}) {
        ...
        let method = 'publicGetTickerPair';
        const response = await this[method] (this.extend (request, params));
    ...
    async fetchCurrencies (params = {}) {
        let method = 'publicGetCurrencies';
        const response = await this[method] (params);
        // {
        //     "result":true,
        //     "code":1000,
    ...
    parseTrade (trade, market = undefined) {
        ...
        let _type = this.safeString (trade, 'type');
        ...
        if ('fee' in trade) {
            ...
            if (market !== undefined) {
                ...
                let label = symbol.substring(3, symbol.length).toString();
    ...
    parseOrder (order, market = undefined) {
        ...
        let _type = this.safeString (order, 'type');
        let side = undefined;
        let type = undefined;
