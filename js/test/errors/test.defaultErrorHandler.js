...
function testDefaultErrorHandler (errorClass, httpStatusCode, httpStatusText, body, url, method) {
    ...
    try {
        ...
        if (e instanceof ccxt.ArgumentsRequired) {
            throw e;
        } else if (!(e instanceof errorClass)) {
            throw new ccxt.BaseError (errorClass.name +  'raised instead of ' + e.name);
        }
    }
}
...
testDefaultErrorHandler (ccxt.ExchangeNotAvailable, 200, 'OK', 'maintenance', 'http://lol.com', 'GET');
...
testDefaultErrorHandler (ccxt.DDoSProtection, 200, 'OK', 'maintenance ddos', 'http://lol.com', 'GET');
testDefaultErrorHandler (ccxt.DDoSProtection, 69, '6ix9ine', 'ddos', 'http://lol.com', 'GET');

testDefaultErrorHandler (ccxt.ExchangeNotAvailable, 666, 'evil', 'maintenance', 'http://lol.com', 'GET');
...
testDefaultErrorHandler (ccxt.ExchangeNotAvailable, 520, 'Bad Gateway', 'maintenance', 'http://lol.com', 'GET');
testDefaultErrorHandler (ccxt.RequestTimeout, 408, '', '', 'http://lol.com', 'GET');
