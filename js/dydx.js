...
module.exports = class dydx extends Exchange {
    describe () {
        return this.deepExtend (super.describe (), {
            ...
            'has': {
                ...
                'privateAPI': true,
                'publicAPI': true,
                'withdraw': true, // TODO, needs stark sign
            },
            'timeframes': {
                '1m': '1MIN',
                '5m': '5MINS',
    ...
    async starkSignature (hashHex) {
        ...
        const secret = this.normalizeHex32 (this.starkPrivateKey); // request can be hex, doesnt need to be BN
    ...
    numToHex (dec) {
        return this.binaryToBase16 (this.numberToBE (dec));
    }
    ...
    async starkHashHex (signObject, type) { 
        const assetIdCollateralHex = this.networkId === 1 ? '02893294412a4c8f915f75892b395ebbf6859ec246ec365c3b1f56f47c3a0a5d' : '03bda2b4764039f2df44a00a9cf1d1569a83f95406a983ce4beb95791c376008';
        const positionIdHex = this.numToHex (await this.getPositionId ()); 
        const nonceHash = this.hash (signObject.clientId, 'SHA256');
        const nonceHex = this.hexMod (this.remove0xPrefix (nonceHash), '100000000');
        ...
        if (type === 'order') {
            ...
            const quantumsAmountSyntheticHex = this.numToHex (quantumsAmountSynthetic);
            const quantumsAmountCollateralHex = this.numToHex (quantumsAmountCollateral);
            const quantumsAmountFeeHex = this.numToHex (quantumsAmountFee);
            const expirationEpochHoursHex = this.numToHex (expirationEpochHoursInt);
            const [ assetIdSellHex, assetIdBuyHex ] = isBuyingSynthetic
                ? [ assetIdCollateralHex, assetIdSyntheticHex ]
                : [ assetIdSyntheticHex, assetIdCollateralHex ];
            const [ quantumsAmountSellHex, quantumsAmountBuyHex ] = isBuyingSynthetic
                ? [ quantumsAmountCollateralHex, quantumsAmountSyntheticHex ]
                : [ quantumsAmountSyntheticHex, quantumsAmountCollateralHex ];
            const numbersArray1 = [quantumsAmountSellHex, quantumsAmountBuyHex, quantumsAmountFeeHex, nonceHex];
            const bitShiftArray1 = [64, 64, 32];
            const orderPart1 = this.packStarkData (numbersArray1, bitShiftArray1);
            const numbersArray2 = ['3', positionIdHex, positionIdHex, positionIdHex, expirationEpochHoursHex];
            const bitShiftArray2 = [64, 64, 64, 32, 17];
            const orderPart2 = this.packStarkData (numbersArray2, bitShiftArray2);
            const assetsHex = this.getCacheablePedersenHash (this.getCacheablePedersenHash (assetIdSellHex, assetIdBuyHex), assetIdFeeHex);
            const hashHex = this.pedersenHash (this.constantPoints (), this.pedersenHash (this.constantPoints (), assetsHex, orderPart1), orderPart2);
            return hashHex;
        }
        if (type === 'withdrawl') {
            const quantumsAmount = this.toQuantumsHelper (signObject.debitAmount, quantumSizeColl, 'ROUND_DOWN', true);
            const quantumsAmountHex = this.numToHex (quantumsAmount);
            const expirationEpochHoursInt = Math.ceil (new Date (signObject.expiration).getTime () / 3600000);
            const expirationEpochHoursHex = this.numToHex (expirationEpochHoursInt);
            const numbersArray = ['6', positionIdHex, nonceHex, quantumsAmountHex, expirationEpochHoursHex];
            const bitShiftArray = [64, 32, 64, 32, 49];
            const packedWithdrawalHex = this.packStarkData (numbersArray, bitShiftArray);
            const hashHex = this.pedersenHash (this.constantPoints (), assetIdCollateralHex, packedWithdrawalHex);
            return hashHex;
        }
    }
    ...
    getCacheablePedersenHash (leftString, rightString) {
        ...
        this.pedersenHashCache[leftString][rightString] = this.safeValue (this.pedersenHashCache[leftString], rightString, 
            this.pedersenHash (this.constantPoints (), leftString, rightString));
    ...
    constantPoints () {
        ...
        if (constantPoints.length === 0) {
            ...
            for (let i = 0; i < constantPointsHex.length; i++) {
                constantPoints[i] = this.starkEc.curve.point (constantPointsHex[i][0], constantPointsHex[i][1]);
            }
    ...
    deriveStarkPrivateKey () {
        ...
        const hashBN = this.newBN (this.remove0xPrefix (hashedData), 16);
        const privateKey = hashBN['iushrn'] (5).toString ('hex'); // TODO move this BN 
        this.starkPrivateKey = this.normalizeHex32 (privateKey);
    }
    ...
    async getPositionId () {
        ...
        for (let i = 0; i < accounts.length; i++) {
            ...
            const accountNumber = this.safeString (this, 'accountNumber', '0');
    ...
    async withdraw (code, amount, address, tag = undefined, params = {}) {
        ...
        this.checkStarkKeyCredentials ();
        await this.loadMarkets ();
        const amountString = this.numberToString (this.currencyToPrecision (code, amount));        // const walletBytes = this.remove0xPrefix (this.walletAddress);
        const lpPositionId = this.safeString (params, 'lpPositionId', '');
        const debitAmount = this.safeString (params, 'debitAmount', '');
        const expiration = this.safeString (params, 'expiration', '');
        const clientId = this.safeString (params, 'clientId', this.milliseconds ().toString ());
        const withdrawlToSign = {
            'creditAsset': code,
        ...
        const request = withdrawlToSign;
        const hashHex = await this.starkHashHex (withdrawlToSign, 'withdrawl');
        const signature = await this.starkSignature (hashHex);
        request['signature'] = signature;
        const response = await this.privatePostFastWithdrawals (this.extend (request, params));
