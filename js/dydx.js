...
const { ArgumentsRequired, BadRequest, ExchangeNotAvailable, AuthenticationError, BadSymbol, ExchangeError, InvalidOrder, InsufficientFunds, NotSupported } = require ('./base/errors');
const { TICK_SIZE } = require ('./base/functions/number');
const Precise = require ('./base/Precise');
const { ROUND_DOWN, ROUND_UP } = require ('./base/functions/misc');
...
module.exports = class dydx extends Exchange {
    ...
    async fetchTradingFees (params = {}) {
        ...
        const result = {};
        const maker = this.safeNumber (response, 'defaultMakerFee');
        const taker = this.safeNumber (response, 'defaultTakerFee');
    ...
    async fetchBalance (params = {}) {
        ...
        const allAccounts = this.account ();
        allAccounts['free'] = '0';
        allAccounts['total'] = '0';
        for (let i = 0; i < accounts.length; i++) {
            ...
            if (account['free']) {
                allAccounts['free'] = Precise.stringAdd (allAccounts['free'], account['free']);
            }
            if (account['total']) {
                allAccounts['total'] = Precise.stringAdd (allAccounts['total'], account['total']);
            }
        }
    ...
    async fetchPositions (symbols = undefined, params = {}) {
        ...
        return this.parsePositions (data, symbols, params);
    }
    ...
    async createOrder (symbol, type, side, amount, price = undefined, params = {}) {
        const passedPrice = price ? price : this.safeNumber (params, 'price');
        if (!passedPrice) {
        ...
        request['size'] = this.amountToPrecision (market['symbol'], amount);
        request['price'] = this.priceToPrecision (market['symbol'], passedPrice);
        // [Required] limitFee
        ...
        request['timeInForce'] = this.safeString (params, 'timeInForce', request['timeInForce']);
        // [Required] expiration
        request['expiration'] = this.safeString (params, 'expiration', this.options['gtcDate']);
        // [Required] postOnly
        ...
        const hashHex = await this.starkHashHex (orderToSign, 'order');
        const signature = await this.starkSignature (hashHex);
        request['signature'] = signature;
        params = this.omit (params, [ 'clientOrderId', 'price' ]);
    ...
    sign (path, api = 'private', method = 'GET', params = {}, headers = undefined, body = undefined) {
        ...
        if (method === 'POST' && path === 'onboarding') {
            ...
            const secret = this.base64ToBinary (this.urldecodeBase64 (this.secret));
    ...
    toQuantumsHelper (humanAmount, quantumSize, roundMode, assertIntegerResult) {
        ...
        const zeroPrecise = new Precise ('0');
        ...
        const quotient = amountPrecise.div (quantumSizePrecise);
        return quotient.toString (); // this.decimalToPrecision (quotientStr, roundMode, 0);
    }

    async starkSignature (hashHex) {
        const request = this.fixStarkHashLength (hashHex);
        const secret = this.normalizeHex32 (this.starkPrivateKey);
        const signature = this.ecdsa (request, secret, 'stark'); // could extract curve specifics to this file, if pre defined curves in elliptic/lib shall not be extended
        return this.normalizeHex32 (signature['r']) + this.normalizeHex32 (signature['s']);
    }

    fixStarkHashLength (messageHash) {
        const hashLength = messageHash.length;
        if (hashLength <= 62) {
            return messageHash;
        }
        if (hashLength !== 63) {
            throw new BadRequest (this.id + ' fixStarkHashLength: Invalid hash length: ' + hashLength.toString () + ' !== 63');
        }
        return this.hexShiftBitsN (messageHash, 4, true);
    }

    numToHex (dec, size) {
        return this.binaryToBase16 (this.numberToBE (dec, size));
    }

    async starkHashHex (signObject, type) {
        const assetIdCollateralHex = this.urls['api']['data']['assetIdCollateralHex'];
        const positionId = await this.getPositionId ();
        const positionIdHex = this.numToHex (positionId, 8);
        const nonceHash = this.hash (this.encode (signObject['clientId']), 'SHA256');
        const nonceHex = this.remove0xPrefix (this.hexMod (this.remove0xPrefix (nonceHash), '100000000'));
        const quantumSizeColl = '1e-6'; // for USDC collateral only, as all markets are
        if (type === 'order') {
            const isBuyingSynthetic = signObject['side'] === 'BUY';
            const assetIdSyntheticHex = this.remove0xPrefix (signObject['assetIdSynthetic']);
            const quantumSizeSyn = Precise.stringDiv ('1', signObject['assetResolution']);
            const quantumsAmountSynthetic = this.toQuantumsHelper (signObject['humanSize'], quantumSizeSyn, ROUND_DOWN, true); // TODO check if roundings are even needed
            ...
            const quantumsAmountFee = Precise.stringMul (signObject['limitFee'], quantumsAmountCollateral);
            const expirationEpochHoursInt1 = this.parse8601 (signObject['expirationIsoTimestamp']) / 3600000;
            const expirationEpochHoursInt = Math.ceil (expirationEpochHoursInt1) + 168;
            const assetIdFeeHex = assetIdCollateralHex;
            const quantumsAmountSyntheticHex = this.numToHex (quantumsAmountSynthetic, 8);
            const quantumsAmountCollateralHex = this.numToHex (quantumsAmountCollateral, 8);
            const quantumsAmountFeeHex = this.numToHex (quantumsAmountFee, 8);
            const expirationEpochHoursHex = this.numToHex (expirationEpochHoursInt, 4);
            const [ assetIdSellHex, assetIdBuyHex ] = isBuyingSynthetic ? [ assetIdCollateralHex, assetIdSyntheticHex ] : [ assetIdSyntheticHex, assetIdCollateralHex ];
            const [ quantumsAmountSellHex, quantumsAmountBuyHex ] = isBuyingSynthetic ? [ quantumsAmountCollateralHex, quantumsAmountSyntheticHex ] : [ quantumsAmountSyntheticHex, quantumsAmountCollateralHex ];
            const orderPart1Hex = quantumsAmountSellHex + quantumsAmountBuyHex + quantumsAmountFeeHex + nonceHex.padStart (8, '0');
            const orderPart2HexUnshifted = '3' + positionIdHex + positionIdHex + positionIdHex + expirationEpochHoursHex;
            const orderPart2Hex = this.hexShiftBitsN (orderPart2HexUnshifted, 17, true);
            const assetsHex = this.getCacheablePedersenHash (this.getCacheablePedersenHash (assetIdSellHex, assetIdBuyHex), assetIdFeeHex);
            const hashHex = this.pedersenHash (this.pedersenHash (assetsHex, orderPart1Hex), orderPart2Hex);
            return hashHex;
        }
    }

    getCacheablePedersenHash (leftString, rightString) {
        this.pedersenHashCache[leftString] = this.safeValue (this.pedersenHashCache, leftString, {});
        this.pedersenHashCache[leftString][rightString] = this.safeValue (
            this.pedersenHashCache[leftString],
            rightString,
            this.pedersenHash (leftString, rightString)
        );
    ...
    deriveStarkPrivateKey (privateKey) {
        const encodedMessageString = this.urls['api']['data']['encodedMessageStrings']['starkKey'];
        const signatureObject = this.signHash (encodedMessageString, privateKey);
        const signature = this.remove0xPrefix (signatureObject['r']) + this.remove0xPrefix (signatureObject['s']) + this.numToHex (signatureObject['v'], 1) + '00';
        const binary = this.base16ToBinary (signature);
        const hashedData = this.hash (binary, 'keccak');
        const starkPrivateKey = this.hexShiftBitsN (this.remove0xPrefix (hashedData), 5, false);
        this.starkPrivateKey = this.normalizeHex32 (starkPrivateKey);
    }
    ...
    async getPositionId () {
        const positionId = this.safeString (this.options, 'positionId');
        if (positionId) {
            return positionId;
        }
        this.checkStarkKeyCredentials ();
        ...
        for (let i = 0; i < accounts.length; i++) {
            const currentAcc = accounts[i];
            const currentAccountNumber = this.safeString (currentAcc, 'accountNumber');
            const accountNumber = this.safeString (this.options, 'accountNumber');
    ...
    recoverDefaultApiCredentials (privateKey) {
        const encodedMessageString = this.urls['api']['data']['encodedMessageStrings']['onboarding'];
        const signatureObject = this.signHash (encodedMessageString, privateKey);
        const signature = this.remove0xPrefix (signatureObject['r']) + this.remove0xPrefix (signatureObject['s']) + this.numToHex (signatureObject['v'], 1) + '00';
        // secret
        ...
        this.secret = this.base16ToUrlencodeBase64 (secret);
        this.password = this.base16ToUrlencodeBase64 (passphrase);
    }

    base16ToUrlencodeBase64 (string) {
        return this.urlencodeBase64 (this.binaryToBase64 (this.base16ToBinary (string)));
    }
    ...
    checkApiKeyCredentials (error = true) {
        ...
        if (!apiKeysProvided && this.privateKey) {
            this.recoverDefaultApiCredentials (this.privateKey);
        }
        return this.checkCredentials (keys, error);
    }
