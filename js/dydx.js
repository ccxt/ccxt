...
module.exports = class dydx extends Exchange {
        ...
        return this.deepExtend (super.describe (), {
            ...
            'parseColletion': {
                ...
                'transaction': {
                    ...
                    'type': {
                        'deposit': 'DEPOSIT',
                        'withdraw': [ 'WITHDRAWAL', 'FAST_WITHDRAWAL' ],
                    },
                },
            },
        });
    ...
    parseTrade (trade, market) {
        ...
        const takerOrMaker = this.parseOrderTakerMaker (this.safeString (trade, 'liquidity'));
        let fee = undefined;
    ...
    async fetchPosition (symbol, params = {}) {
        ...
        request['status'] = this.toPositionStatus ('open');
    ...
    async fetchPositions (symbols = undefined, params = {}) {
        ...
        //     ],
        //   }
        const arrayPositions = this.safeValue (response, 'positions');
        return this.parsePositions (arrayPositions, symbols, params);
    }

    parsePositions (positions, symbols = undefined, params = undefined) {
        const result = [];
        for (let i = 0; i < positions.length; i++) {
            const position = this.parsePosition (positions[i]);
            if (symbols === undefined || symbols.indexOf (position['symbol']) >= 0) {
