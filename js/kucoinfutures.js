...
module.exports = class kucoinfutures extends kucoin {
    ...
    async fetchTransactionFee (code, params = {}) {
        ...
        const response = await this.publicGetCurrenciesCurrency (this.extend (request, params));
        //
        //     v1
        //     {
        //         "code":"200000",
        ...
        const apiVersions = this.safeValue (versions, 'public', {});
        const methodVersions = this.safeValue (apiVersions, 'GET', {});
        const defaultVersion = this.safeString (methodVersions, 'currencies/{currency}', this.options['version']);
        const version = this.safeString (params, 'version', defaultVersion);
        let withdrawFees = {};
        if (version === 'v1') {
            withdrawFees[code] = this.safeNumber (data, 'withdrawalMinFee');
        } else if (version === 'v2') {
            const networkList = this.safeValue (data, 'chains', []);
            withdrawFees[code] = {};
            for (let j = 0; j < networkList.length; j++) {
                const networkEntry = networkList[j];
                const networkId = this.safeString (networkEntry, 'chainName');
                const networkCode = this.safeCurrencyCode (networkId);
                const fee = this.safeNumber (networkEntry, 'withdrawalMinFee');
