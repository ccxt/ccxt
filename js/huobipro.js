...
module.exports = class huobipro extends Exchange {
    ...
    async fetchTradingLimits(symbols = undefined, params = {}) {
        ...
        let info = {};
        let limits = {};
        if (typeof symbols === 'undefined') symbols = this.symbols;
        for (let i = 0; i < symbols.length; i++) {
            let symbol = symbols[i];
            let market = this.market(symbol);
            let response = await this.publicGetCommonExchange(
                this.extend({
                    symbol: market['id'],
                })
            );
    ...
    parseTicker(ticker, market = undefined) {
        ...
        let vwap = undefined;
        if (
            typeof baseVolume !== 'undefined' &&
            typeof quoteVolume !== 'undefined' &&
            baseVolume > 0
        )
    ...
    async fetchOrderBook(symbol, limit = undefined, params = {}) {
        await this.loadMarkets();
        let market = this.market(symbol);
        let response = await this.marketGetDepth(
            this.extend(
                {
                    symbol: market['id'],
                    type: 'step0',
    ...
    async fetchTicker(symbol, params = {}) {
        await this.loadMarkets();
        let market = this.market(symbol);
        let response = await this.marketGetDetailMerged(
            this.extend(
                {
                    symbol: market['id'],
                },
    ...
    async fetchTrades(symbol, since = undefined, limit = 1000, params = {}) {
        ...
        let response = await this.marketGetHistoryTrade(
    ...
    async fetchOHLCV(
        ...
        let response = await this.marketGetHistoryKline(
            this.extend(request, params)
        );
    ...
    async fetchCurrencies(params = {}) {
        let response = await this.publicGetSettingsCurrencys(
            this.extend(
                {
        ...
        for (let i = 0; i < currencies.length; i++) {
            ...
            let active =
                currency['visible'] &&
                currency['deposit-enabled'] &&
                currency['withdraw-enabled'];
            result[code] = {
                id: id,
                code: code,
                type: 'crypto',
    ...
    async fetchBalance(params = {}) {
        ...
        for (let i = 0; i < balances.length; i++) {
            ...
            if (currency in result) account = result[currency];
            else account = this.account();
    ...
    async fetchOrdersByStates(
        ...
        if (!symbol)
            throw new ExchangeError(
                this.id + ' fetchOrders() requires a symbol parameter'
            );
        await this.loadMarkets();
        let market = this.market(symbol);
        let response = await this.privateGetOrderOrders(
            this.extend(
                {
                    symbol: market['id'],
                    states: states,
    ...
    async fetchOrders(
        ...
        return await this.fetchOrdersByStates(
    ...
    async fetchOrder(id, symbol = undefined, params = {}) {
        await this.loadMarkets();
        let response = await this.privateGetOrderOrdersId(
            this.extend(
                {
                    id: id,
                },
        ...
        if (this.options['createMarketBuyOrderRequiresPrice']) {
            if (type === 'market' && side === 'buy') {
                if (typeof price === 'undefined') {
                    ...
                    order['amount'] = this.priceToPrecision(
                        symbol,
    ...
    async fetchDepositAddress(code, params = {}) {
        ...
        let response = await this.privateGetDwDepositVirtualAddresses(
            this.extend(
                {
    ...
    async withdraw(code, amount, address, tag = undefined, params = {}) {
        ...
        if (typeof tag !== 'undefined') request['addr-tag'] = tag; // only for XRP?
        let response = await this.privatePostDwWithdrawApiCreate(
            this.extend(request, params)
        );
        let id = undefined;
        if ('data' in response) {
    ...
    sign(
        ...
        let url = '/';
        if (api === 'market') url += api;
        else url += this.version;
        url += '/' + this.implodeParams(path, params);
        let query = this.omit(params, this.extractParams(path));
        if (api === 'private') {
            this.checkRequiredCredentials();
            let timestamp = this.ymdhms(this.milliseconds(), 'T');
            let request = this.keysort(
                this.extend(
                    {
    ...
    _websocketDispatch(contextId,data) {
        ...
        const [someother,rawsymbol,channel]= data.ch.split('.');
        //:symbol
