...
function testOrderBook(exchange, skippedProperties, method, entry, symbol) {
    ...
    for (let i = 0; i < bidsLength; i++) {
        ...
        if (bidsLength > nextI) {
            ...
            assert(Precise.stringGt(currentBidString, nextBidString), 'Position:' + i.toString() + ' current bid should be > than the next one: ' + currentBidString + '>' + nextBidString + logText);
        }
    ...
    for (let i = 0; i < asksLength; i++) {
        ...
        if (asksLength > nextI) {
            ...
            assert(Precise.stringLt(currentAskString, nextAskString), 'Position:' + i.toString() + ' current ask should be < than the next one: ' + currentAskString + '<' + nextAskString + logText);
        }
