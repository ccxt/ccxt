...
properties
    ...
    .filter((property) => fs.existsSync(__dirname + '/Exchange/test.' + property))
    .forEach(async (property) => {
    // eslint-disable-next-line global-require, import/no-dynamic-require, no-path-concat
    testFiles[property] = await import(__dirname + '/Exchange/test.' + property);
});

import errors from '../base/errors.js';
Object.keys(errors)
    // eslint-disable-next-line no-path-concat
    .filter((error) => fs.existsSync(__dirname + '/errors/test.' + error))
    .forEach(async (error) => {
    // eslint-disable-next-line global-require, import/no-dynamic-require, no-path-concat
    testFiles[error] = await import(__dirname + '/errors/test.' + error);
});
