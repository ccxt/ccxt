...
test('the balances behind a route are read once and cached, so a quote is one call', async () => {
    ...
    const reads = venue.calls.filter((call) => call === 'fetchBalance');
...
test('loadBalances refreshes on demand, and a live execute invalidates the cache', async () => {
    ...
    const countReads = () => venue.calls.filter((call) => call === 'fetchBalance').length;
    ...
    await held.execute(route, undefined, { 'dryRun': true, 'usdRates': { 'USDT': 1 } });
    ...
    //  a live run drops it
    await held.execute(route, undefined, { 'usdRates': { 'USDT': 1 }, 'idempotencyKey': 'invalidates' });
...
test('venues given to the constructor filter the route and fund it, and execute defaults to them', async () => {
    ...
    const report = await held.execute(route, undefined, { 'usdRates': { 'USDT': 1 }, 'idempotencyKey': 'held-venues' });
...
test('a resting order on an immediate path is cancelled, and a cancel that fails halts the route', async () => {
    ...
    for (let i = 0; i < strategies.length; i++) {
        ...
        const options = { 'strategy': strategy, 'usdRates': { 'USDT': 1 } };
...
test('a limit buy reserves more than the plan says it costs, by exactly the slippage', async () => {
    ...
    assert.ok(Math.abs(reserved - 100.25) < 1e-9, 'a venue reserving at the limit price holds 100.25 for a 100 order; got ' + reserved.toString());
    ...
    assert.ok(Math.abs(widerStep['amount'] * widerStep['limitPrice'] - 101) < 1e-9, '100 bps reserves 101 against a declared 100');
...
test('a split hop pools its venues: reconciliation scales on the HOP total, not per wallet', async () => {
    ...
    router.applyResize(steps, second);
    assert.ok(Math.abs(steps[2]['amount'] - 99) < 1e-9, 'today the downstream order is sized at the hop mean of 99, not venueA\'s own 98; '
...
test('a live plan with no requestId is refused: without an identity there is no idempotency', async () => {
    ...
    const plan = router.buildExecutionPlan(route, {});
    const venue = new StubVenue('stub');
    await assert.rejects(async () => await router.execute(plan, { 'stub': venue }, { 'strategy': 'sequential', 'usdRates': { 'USDT': 1 } }), /carries no requestId/);
    ...
    await assert.rejects(async () => await router.execute(plan, { 'stub': new StubVenue('stub') }, { 'strategy': 'sequential', 'usdRates': { 'USDT': 1 }, 'idempotencyKey': 'hand-built-1' }), /already executed/);
...
test('a dry run never consumes a plan, and a halted live run always does', async () => {
    ...
    await assert.rejects(async () => await router.execute(failedPlan, { 'stub': retry }, { 'strategy': 'sequential', 'usdRates': { 'USDT': 1 } }), /already executed/);
...
test('an ambiguous venue error is never retried, whatever its ccxt class', async () => {
    ...
    for (let i = 0; i < ambiguous.length; i++) {
        ...
        assert.strictEqual(venue.calls.length, 1, name + ' may already be a live position: it must be placed exactly once, never retried');
        assert.strictEqual(report['steps'][0]['status'], 'outcome_unknown', name + ' leaves the outcome unknown, not definitively failed');
        assert.strictEqual(report['haltReason'], 'outcome_unknown', name + ' halts the route naming the ambiguity');
        assert.ok(report['openOrders'].length >= 1, name + ' must leave an openOrders entry for an operator to reconcile');
    }
});
