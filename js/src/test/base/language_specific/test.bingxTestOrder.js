...
async function testBingxTestOrder() {
    ...
    exchange.parseOrder = () => ({});
    await assert.rejects(exchange.createOrder('BTC/USD:BTC', 'limit', 'buy', 1, 100, { 'test': true }), (error) => (error instanceof ccxt.NotSupported) && error.message.includes('only supports test orders for linear swap markets'));
    assert.deepStrictEqual(calls, []);
    await assert.rejects(exchange.createOrder('BTC/USDT', 'limit', 'buy', 1, 100, { 'test': true }), (error) => (error instanceof ccxt.NotSupported) && error.message.includes('only supports test orders for linear swap markets'));
    assert.deepStrictEqual(calls, []);
