...
function makeStubbedWhitebit(state) {
    ...
    exchange.v4PrivatePostProfileWebsocketToken = async () => {
    ...
    exchange.watch = async (url, messageHash, message = undefined, subscribeHash = undefined, subscription = undefined) => {
...
async function testWhitebitAuthenticateRestFailureRetries() {
    ...
    for (let i = 0; i < outcomes.length; i++) {
        ...
        assert(outcomes[i].reason instanceof AuthenticationError, 'every caller must observe the typed AuthenticationError');
    }
...
async function testWhitebitAuthenticateEmptyTokenRejection() {
    ...
    for (let i = 0; i < outcomes.length; i++) {
        ...
        assert(outcomes[i].reason instanceof AuthenticationError, 'an empty websocket_token must reject with AuthenticationError');
    }
    await sleep(20);
