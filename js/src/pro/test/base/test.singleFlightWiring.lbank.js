...
function assertSettled(exchange, context) {
...
function assertNotDialed(exchange, context) {
...
function makeStubbedLbank(state) {
    ...
    exchange.spotPrivatePostSubscribeGetKey = async () => {
    ...
    exchange.spotPrivatePostSubscribeRefreshKey = async () => {
...
function expireSubscription(exchange) {
...
async function testLbankAuthenticateFailedGetKeyRejection() {
    ...
    assert(outcomes[0].reason instanceof ExchangeError, 'the lbank leader must reject with ExchangeError');
    assert(outcomes[1].reason instanceof ExchangeError, 'the lbank waiter must observe the same ExchangeError');
    const client = wsClient(exchange);
    ...
    const fetchesBeforeRetry = state.getKeyFetches;
    exchange.spotPrivatePostSubscribeGetKey = async () => {
        state.getKeyFetches = state.getKeyFetches + 1;
...
async function testLbankAuthenticateFailedRefreshRejection() {
    ...
    assert(outcomes[0].reason instanceof ExchangeError, 'the lbank refresh leader must reject with ExchangeError');
    assert(outcomes[1].reason instanceof ExchangeError, 'the lbank refresh waiter must observe the same ExchangeError');
    const client = wsClient(exchange);
    ...
    const refreshesBeforeRetry = state.refreshFetches;
    exchange.spotPrivatePostSubscribeRefreshKey = async () => {
        state.refreshFetches = state.refreshFetches + 1;
        return { 'result': 'true' };
    };
...
async function testLbankAuthenticateMissingCredentials() {
    ...
    let credentialsError = undefined;
    try {
        await exchange.authenticate();
    }
    catch (e) {
