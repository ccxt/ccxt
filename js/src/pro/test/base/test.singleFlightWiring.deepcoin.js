...
function makeStubbedDeepcoin(state, expireTime) {
    ...
    exchange.privateGetDeepcoinListenkeyAcquire = async () => {
    ...
    exchange.privateGetDeepcoinListenkeyExtend = async () => {
...
async function testDeepcoinAuthenticateEmptyKeyRejection() {
    ...
    assert(outcomes[0].reason instanceof AuthenticationError, 'the deepcoin leader must reject with AuthenticationError');
    assert(outcomes[1].reason instanceof AuthenticationError, 'the deepcoin waiter must observe the same AuthenticationError');
