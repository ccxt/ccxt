...
async function setupExchange(customWsUrl) {
...
function calculateAndDisplayMetrics(received, dropped, latencySum, minLatency, maxLatency, latencies, startTime, benchmarkName, startMemory, endMemory, startCpu, endCpu) {
...
function processTickerMessage(ticker, metrics, RESERVOIR_SIZE) {
    metrics.received++;
    const now = Date.now();
...
function storeMetrics(received, dropped, latencySum, minLatency, maxLatency, latencies, startTime, benchmarkName, startMemory, endMemory, startCpu, endCpu) {
    const avgLatency = received > 0 ? latencySum / received : 0;
    const medianLatency = median(latencies);
    const p50Latency = percentile(latencies, 50);
    const p90Latency = percentile(latencies, 90);
...
function displayResultsTable(results) {
    ...
    const columns = {
        'Test Name': ((r) => r.name),
        'Msgs Recv': ((r) => r.received.toString()),
        'Dropped': ((r) => r.dropped.toString()),
        'Thrput(msg/s)': ((r) => r.throughput.toFixed(2)),
        'Avg Lat(ms)': ((r) => r.avgLatency.toFixed(2)),
        'Min Lat(ms)': ((r) => r.minLatency.toFixed(2)),
        'Max Lat(ms)': ((r) => r.maxLatency.toFixed(2)),
        'Med Lat(ms)': ((r) => r.medianLatency.toFixed(2)),
        'P50 Lat(ms)': ((r) => r.p50Latency.toFixed(2)),
        'P90 Lat(ms)': ((r) => r.p90Latency.toFixed(2)),
        'P99 Lat(ms)': ((r) => r.p99Latency.toFixed(2)),
        'RSS(MB)': ((r) => r.rssDiff.toFixed(2)),
        'Heap Used(MB)': ((r) => r.heapUsedDiff.toFixed(2)),
        'Heap Tot(MB)': ((r) => r.heapTotalDiff.toFixed(2)),
        'CPU User(ms)': ((r) => r.userCpuDiff.toFixed(2)),
        'CPU Sys(ms)': ((r) => r.systemCpuDiff.toFixed(2)),
        'Actual Duration(ms)': ((r) => r.actualDuration.toFixed(2)),
    };
    // Calculate column widths
    const columnWidths = Object.keys(columns).map((header) => Math.max(header.length, ...results.map((r) => columns[header](r).length)));
    // Create header row
    const headers = Object.keys(columns);
    const headerRow = headers.map((h, i) => h.padEnd(columnWidths[i])).join(' | ');
    const separator = columnWidths.map((w) => '-'.repeat(w)).join(' | ');
    // Create data rows
    const dataRows = results.map((result) => headers.map((h, i) => columns[h](result).padEnd(columnWidths[i])).join(' | '));
    // Print the table
...
(async () => {
    ...
    benchmarkResults.push(storeMetrics(metrics1.received, metrics1.dropped, metrics1.latencySum, metrics1.minLatency, metrics1.maxLatency, metrics1.latencies, metrics1.startTime, 'watchTicker (original)', metrics1.startMemory, endMemory1, metrics1.startCpu, endCpu1));
    ...
    benchmarkResults.push(storeMetrics(metrics2.received, metrics2.dropped, metrics2.latencySum, metrics2.minLatency, metrics2.maxLatency, metrics2.latencies, metrics2.startTime, 'subscribeTicker (sync=true)', metrics2.startMemory, endMemory2, metrics2.startCpu, endCpu2));
    ...
    benchmarkResults.push(storeMetrics(metrics3.received, metrics3.dropped, metrics3.latencySum, metrics3.minLatency, metrics3.maxLatency, metrics3.latencies, metrics3.startTime, 'subscribeTicker (sync=false)', metrics3.startMemory, endMemory3, metrics3.startCpu, endCpu3));
    await binance.close();
    ...
    benchmarkResults.push(storeMetrics(avgMetrics.received, avgMetrics.dropped, avgMetrics.latencySum, avgMetrics.minLatency, avgMetrics.maxLatency, avgMetrics.latencies, avgMetrics.startTime, NUM_CONSUMERS + ' subscribeTicker (same topic, avg per consumer)', avgMetrics.startMemory, endMemory4, avgMetrics.startCpu, endCpu4));
    await binance.close();
    ...
    benchmarkResults.push(storeMetrics(totalMetrics.received, totalMetrics.dropped, totalMetrics.latencySum, totalMetrics.minLatency, totalMetrics.maxLatency, totalMetrics.latencies, totalMetrics.startTime, NUM_CONSUMERS + ' subscribeTicker (different topics, sync=true, total)', totalMetrics.startMemory, endMemory5, totalMetrics.startCpu, endCpu5));
    ...
    benchmarkResults.push(storeMetrics(avgMetricsSyncFalse.received, avgMetricsSyncFalse.dropped, avgMetricsSyncFalse.latencySum, avgMetricsSyncFalse.minLatency, avgMetricsSyncFalse.maxLatency, avgMetricsSyncFalse.latencies, avgMetricsSyncFalse.startTime, NUM_CONSUMERS + ' subscribeTicker (same topic, sync=false, avg per consumer)', avgMetricsSyncFalse.startMemory, endMemory5SyncFalse, avgMetricsSyncFalse.startCpu, endCpu5SyncFalse));
    await binance.close();
    ...
    benchmarkResults.push(storeMetrics(totalMetricsSyncFalse.received, totalMetricsSyncFalse.dropped, totalMetricsSyncFalse.latencySum, totalMetricsSyncFalse.minLatency, totalMetricsSyncFalse.maxLatency, totalMetricsSyncFalse.latencies, totalMetricsSyncFalse.startTime, NUM_CONSUMERS + ' subscribeTicker (different topics, sync=false, total)', totalMetricsSyncFalse.startMemory, endMemory5cSyncFalse, totalMetricsSyncFalse.startCpu, endCpu5cSyncFalse));
    ...
    benchmarkResults.push(storeMetrics(totalParallelMetrics.received, totalParallelMetrics.dropped, totalParallelMetrics.latencySum, totalParallelMetrics.minLatency, totalParallelMetrics.maxLatency, totalParallelMetrics.latencies, totalParallelMetrics.startTime, NUM_CONSUMERS + ' parallel watchTicker (different symbols, total)', totalParallelMetrics.startMemory, endMemory6, totalParallelMetrics.startCpu, endCpu6));
...
function reservoirSamplePush(reservoir, value, count, size) {
