...
export const signToken = (payload, private_key) => {
 ...
 */
export const signAuthenticationToken = (methodRaw, uri, params, requestID, keystore) => {
    if (!keystore.session_id || !validate(keystore.session_id))
        return '';
    let method = 'GET';
    if (methodRaw)
        method = methodRaw.toLocaleUpperCase();
    let data = '';
