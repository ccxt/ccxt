 ...
 */
export const buildMixinOneSafePaymentUri = (params) => {
    let address = '';
    if (params.uuid && validate(params.uuid))
        address = params.uuid;
    else if (params.mainnetAddress && getPublicFromMainnetAddress(params.mainnetAddress))
        address = params.mainnetAddress;
    else if (params.mixAddress && parseMixAddress(params.mixAddress))
        address = params.mixAddress;
        ...
        throw new Error('fail to get payment destination address');
...
export const signSafeRegistration = (user_id, tipPin, privateKey) => {
...
export const deriveGhostPublicKey = (r, A, B, index) => {
...
export const getMainnetAddressGhostKey = (recipient, hexSeed = '') => {
    if (recipient.receivers.length === 0)
        return undefined;
    if (hexSeed && hexSeed.length !== 128)
        return undefined;
    const publics = recipient.receivers.map(d => getPublicFromMainnetAddress(d));
    if (!publics.every(p => !!p))
        return undefined;
...
export const buildSafeTransactionRecipient = (members, threshold, amount) => ({
    members,
    threshold,
    amount,
    mixAddress: buildMixAddress({ members, threshold }),
});
export const getUnspentOutputsForRecipients = (outputs, rs) => {
    ...
    for (let i = 0; i < outputs.length; i++) {
        ...
        if (totalInput.minus(totalOutput).isNegative())
            continue;
        return {
...
export const encodeSafeTransaction = (tx, sigs = []) => {
...
export const decodeSafeTransaction = (raw) => {
...
export const buildSafeTransaction = (utxos, rs, gs, extra, references = []) => {
    if (utxos.length === 0)
    ...
    utxos.forEach(o => {
        if (!asset)
            asset = o.asset;
        if (o.asset !== asset)
            throw new Error('inconsistent asset in outputs');
...
export const signSafeTransaction = (tx, views, privateKey, index = 0) => {
