...
const scalarBaseMult = (x) => {
    const res = base.multiply(x);
    // @ts-ignore
    return Buffer.from(res.toRawBytes());
};
const scalarBaseMultToPoint = (x) => base.multiply(x);
const publicFromPrivate = (priv) => {
...
const newPoint = (x) => ed25519.ExtendedPoint.fromHex(x.toString('hex'));
const keyMultPubPriv = (pub, priv) => {
...
export const getRandomBytes = (len) => Buffer.from(randomBytes(len ?? ed25519.CURVE.Fp.BYTES));
