...
export const getPublicFromMainnetAddress = (address) => {
    try {
        if (!address.startsWith(MainAddressPrefix))
            return undefined;
        ...
        if (!checksum.subarray(0, 4).equals(data.subarray(64)))
            return undefined;
...
export const getMainnetAddressFromPublic = (pubKey) => {
...
export const parseMixAddress = (address) => {
    try {
        if (!address.startsWith(MixAddressPrefix))
            return undefined;
        const data = bs58.decode(address.slice(3));
        ...
        const checksum = newHash(msg);
        if (!checksum.subarray(0, 4).equals(Buffer.from(data.subarray(data.length - 4))))
            return undefined;
        ...
        if (version !== 2)
            return undefined;
        if (!threshold || !total || threshold === 0 || threshold > total || total > 64)
            return undefined;
...
export const buildMixAddress = (ma) => {
