...
export default class polymarket extends Exchange {
    ...
    async fetchTradingFee(symbol, params = {}) {
        ...
        return {
            'info': response,
            'symbol': this.safeOutcomeSymbol(undefined, outcomeObj),
            'maker': rate,
            'taker': rate,
            'percentage': true,
            'tierBased': false,
     ...
     */
    async buildClobOrderBody(symbol, type, side, amount, price = undefined, params = {}) {
        const outcome = symbol;
    ...
    signClobOrder(message, exchangeAddress, domainVersion, signatureType) {
        ...
        const orderDomain = { 'name': domainName, 'version': domainVersion, 'chainId': chainId, 'verifyingContract': exchangeAddress };
        if (signatureType !== 3) {
        ...
        const appDomainData = this.ethAbiEncode(['bytes32', 'bytes32', 'bytes32', 'uint256', 'address'], [domainTypeHash, nameHash, versionHash, this.convertToBigInt(this.numberToString(chainId)), exchangeAddress]);
    ...
    sign(path, api = 'gamma', method = 'GET', params = {}, headers = undefined, body = undefined) {
        ...
        if (access === 'private') {
            ...
            if (isL1Auth) {
                ...
                const secretBytes = this.base64ToBinary(secret.replaceAll('-', '+').replaceAll('_', '/'));
    ...
    tokenIdToSymbol(tokenId) {
        ...
        return market ? market['symbol'] : undefined;
    }
