...
export interface DepthDiffStreamEvent {
  eventTime: bigint;
  firstBookUpdateId: bigint;
...
export class DepthDiffStreamEventDecoder {
  ...
  private static readonly TEMPLATE_ID = 10003;
  ...
  decode(buffer: ArrayBuffer, offset: number = 0): DepthDiffStreamEvent {
    ...
    const firstBookUpdateId = view.getBigInt64(pos, this.littleEndian);
    pos += 8;

    const lastBookUpdateId = view.getBigInt64(pos, this.littleEndian);
    pos += 8;
    ...
    pos = offset + DepthDiffStreamEventDecoder.BLOCK_LENGTH;
    ...
    return {
      eventTime: eventTime,
      firstBookUpdateId: firstBookUpdateId,
      lastBookUpdateId: lastBookUpdateId,
  ...
  static getBlockLength(): number {
    return DepthDiffStreamEventDecoder.BLOCK_LENGTH;
  }

  static getTemplateId(): number {
    return DepthDiffStreamEventDecoder.TEMPLATE_ID;
  }

  static getSchemaId(): number {
    return DepthDiffStreamEventDecoder.SCHEMA_ID;
  }

  static getSchemaVersion(): number {
    return DepthDiffStreamEventDecoder.SCHEMA_VERSION;
  }
}
