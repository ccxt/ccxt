...
class Allocations
{
    ...
    public int|float|null $sourceTradeId = null;
    public int|float|null $sourceAllocationId = null;
...
class AccountAllocationsResponse
{
    ...
    private function decodeAllocationsGroup(string $data, int &$offset): array
    {
        ...
        for ($i = 0; $i < $numInGroup; $i++) {
            ...
            $offset += 1;
            $item->allocationId = unpack('q', substr($data, $offset, 8))[1];
            $offset += 8;
            $item->orderId = unpack('q', substr($data, $offset, 8))[1];
            $offset += 8;
            $item->orderListId = unpack('q', substr($data, $offset, 8))[1];
            $offset += 8;
            $item->sourceTradeId = unpack('q', substr($data, $offset, 8))[1];
            $offset += 8;
            $item->sourceAllocationId = unpack('q', substr($data, $offset, 8))[1];
            $offset += 8;
    ...
    public function decode(string $data): void
    {
        ...
        $this->allocations = $this->decodeAllocationsGroup($data, $offset);
    }
}
