...
export function registerWatchTools (server: McpServer, ctx: ServerContext): void {
    ...
    server.registerTool ('watch_read', {
        ...
        'description': 'Get the latest data from a stream. For a STATE stream (streamKind "state": ticker(s), order book(s), ohlcv, balance, positions, ...) it returns the COMPLETE current snapshot in "latest" — the full merged set, so call it whenever you need the current value; "updatesSinceRead" tells you how much changed since you last read. For an EVENT stream (streamKind "events": trades, orders, my trades, liquidations) it returns new items oldest-first in "events" plus a "nextCursor" — thread that back as "cursor" on each poll so you do not re-receive (double-count) old items; "moreBuffered": true means call again with nextCursor for more, and "missedBeforeBuffer" > 0 means older updates were evicted before you read them. Set waitForChange:true to BLOCK until the next update arrives (or timeout) instead of returning immediately — the efficient way to wait for a slow event like an order fill or a position change ("timedOut": true means the wait elapsed with nothing new).',
        'inputSchema': {
            ...
            'timeoutMs': z.number ().int ().positive ().optional ().describe ('max time to block when waitForChange is set (default 25000, capped at 55000 to stay under host tool-call timeouts)'),
        },
