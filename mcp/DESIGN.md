...
- `watch_read` drains updates since a cursor (capped at 50, projected); `watch_unsubscribe` stops + releases (closing the public socket on last ref, best-effort `unWatch*` on shared account instances); `watch_list` shows active streams. An **idle TTL (10 min, unref'd timer)** auto-stops abandoned streams; a hard cap of 25 concurrent subscriptions; loops give up after 5 consecutive errors and surface the error on the next `watch_read`. Verified live against Binance (concurrent `watchOHLCV`/`watchTicker`/`watchTrades`).
