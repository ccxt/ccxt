...
MCP hosts surface no model-visible push today, but the stdio server is long-lived — so streaming is **subscribe + poll**, implemented in `subscriptions.ts` + `tools/watch.ts`:
