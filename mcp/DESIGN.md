...
MCP hosts surface no model-visible push today, but the stdio server is long-lived — so streaming is **subscribe + poll**, implemented in `subscriptions.ts` + `tools/watch.ts`. Two access patterns fall out of ccxt.pro's per-channel cache and `newUpdates` mode: **state** streams — only the single-symbol `watchTicker`/`watchOrderBook`, which return the *complete* current object on every `watch*` call, so `watch_read` returns that snapshot in `latest` (no history buffer, plus `updatesSinceRead` for freshness); **event** streams — everything else (trades/ohlcv/orders/my trades/balance/positions and the multi-symbol `watchTickers`/`watchOrderBookForSymbols`/…). With `newUpdates=true` those return only the *changed subset* each cycle, so treating them as "state" would let a partial delta masquerade as the full set (e.g. `watchPositions` after one position moved would look like the only open position); instead they accumulate in a ring buffer that `watch_read` drains oldest-first via a cursor, and the agent uses the `get_*` snapshot tools for a full current set. `streamKind` on subscribe/read tells the agent which it is.
