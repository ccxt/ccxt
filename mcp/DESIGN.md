...
MCP hosts surface no model-visible push today, but the stdio server is long-lived — so streaming is **subscribe + poll**, implemented in `subscriptions.ts` + `tools/watch.ts`. Two access patterns fall out of ccxt.pro's per-channel cache: **state** streams (ticker/order book/ohlcv/balance/positions) — each `watch*` returns the full current snapshot ccxt has built up, so `watch_read` returns that snapshot in `latest` (no history buffer, plus `updatesSinceRead` for freshness); **event** streams (trades/my trades/orders) — each `watch*` yields new items, so they accumulate in a ring buffer that `watch_read` drains oldest-first via a cursor. `streamKind` on subscribe/read tells the agent which it is.
