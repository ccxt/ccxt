...
**live (WebSocket)** — `watch_subscribe` opens a background ccxt.pro stream; `watch_read` returns fresh data (for **state** streams — the single-symbol `watchTicker`/`watchOrderBook`, which return the complete current object each read — the snapshot in `latest`; for **event** streams — everything else: `watchTrades`/`watchOHLCV`, plus the private `watchMyTrades`/`watchOrders`/`watchBalance`/`watchPositions` needing an account — new/changed items in `events` with a cursor. Event streams carry only what changed, so use the `get_*` tools for a full multi-symbol snapshot); `watch_unsubscribe` stops it; `watch_list` shows active streams. The long-lived server holds the socket so the agent always has fresh data; idle streams auto-stop after 10 minutes.
