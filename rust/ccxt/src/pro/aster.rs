...
impl std::ops::Deref for AsterCore {
    type Target = crate::exchanges::aster::AsterCore;
 ...
 */
    ...
    m
        ...
        if is_greater_than(&subtract(&time, &lastAuthenticatedTime), &listenKeyRefreshRate) {
            ...
            if is_equal(&type_var, &Value::Str("spot".to_string())) {
                ...
                response = self.fapi_private_post_v3_listen_key(&[params.clone()]).await;
            }
    ...
    m
        ...
        let _try_result = futures::FutureExt::catch_unwind(std::panic::AssertUnwindSafe(async {
            if is_equal(&type_var, &Value::Str("spot".to_string())) {
                self.sapi_private_put_v3_listen_key(&[]).await; // extend the expiry
            }  else {
                self.fapi_private_put_v3_listen_key(&[]).await; // extend the expiry
            }
 ...
 */
    ...
    let mut m = indexmap::IndexMap::new();
        ...
        m.insert("type".to_string(), self.parse_order_type(self.safe_string_lower(order.clone(), Value::Str("o".to_string()), &[])));
