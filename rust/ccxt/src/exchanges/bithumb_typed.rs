...
//
// Typed wrapper for bithumb.
//
// Owns a `Box<BithumbCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::bithumb::BithumbCore;
use crate::types::*;
...
pub struct Bithumb {
    ...
    core: Box<BithumbCore>,
}

impl Bithumb {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BithumbCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BithumbCore::new(config.into_option())) }
    }

    pub fn from_core(core: BithumbCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BithumbCore {
        &mut self.core
    }
...
impl std::ops::Deref for Bithumb {
    type Target = BithumbCore;
...
impl crate::typed::TypedExchange for Bithumb {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Bithumb::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Bithumb::markets(self) }
}
