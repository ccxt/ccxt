...
//
// Typed wrapper for bigone.
//
// Owns a `Box<BigoneCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::bigone::BigoneCore;
use crate::types::*;
...
impl Bigone {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BigoneCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BigoneCore::new(config.into_option())) }
    }

    pub fn from_core(core: BigoneCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BigoneCore {
        &mut self.core
    }
...
impl std::ops::Deref for Bigone {
    type Target = BigoneCore;
...
impl crate::typed::TypedExchange for Bigone {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Bigone::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Bigone::markets(self) }
}
