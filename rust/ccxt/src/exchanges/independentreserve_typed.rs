...
//
// Typed wrapper for independentreserve.
//
// Owns a `Box<IndependentreserveCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::independentreserve::IndependentreserveCore;
use crate::types::*;
...
pub struct Independentreserve {
    ...
    core: Box<IndependentreserveCore>,
}

impl Independentreserve {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(IndependentreserveCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(IndependentreserveCore::new(config.into_option())) }
    }

    pub fn from_core(core: IndependentreserveCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut IndependentreserveCore {
        &mut self.core
    }
...
impl std::ops::Deref for Independentreserve {
    type Target = IndependentreserveCore;
...
impl crate::typed::TypedExchange for Independentreserve {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Independentreserve::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Independentreserve::markets(self) }
}
