...
//
// Typed wrapper for nado.
//
// Owns a `Box<NadoCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::nado::NadoCore;
use crate::types::*;
...
impl Nado {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(NadoCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(NadoCore::new(config.into_option())) }
    }

    pub fn from_core(core: NadoCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut NadoCore {
        &mut self.core
    }
...
impl std::ops::Deref for Nado {
    type Target = NadoCore;
...
impl crate::typed::TypedExchange for Nado {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Nado::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Nado::markets(self) }
}
