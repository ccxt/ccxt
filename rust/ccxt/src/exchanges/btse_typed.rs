...
//
// Typed wrapper for btse.
//
// Owns a `Box<BtseCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::btse::BtseCore;
use crate::types::*;
...
impl Btse {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BtseCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BtseCore::new(config.into_option())) }
    }

    pub fn from_core(core: BtseCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BtseCore {
        &mut self.core
    }
...
impl std::ops::Deref for Btse {
    type Target = BtseCore;
...
impl crate::typed::TypedExchange for Btse {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Btse::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Btse::markets(self) }
}
