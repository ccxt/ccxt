...
//
// Typed wrapper for krakenfutures.
//
// Owns a `Box<KrakenfuturesCore>` and exposes the unified CCXT API
...
pub struct Krakenfutures {
    ...
    core: Box<KrakenfuturesCore>,
}

impl Krakenfutures {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(KrakenfuturesCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(KrakenfuturesCore::new(config.into_option())) }
    }

    pub fn from_core(core: KrakenfuturesCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut KrakenfuturesCore {
        &mut self.core
    }
...
impl std::ops::Deref for Krakenfutures {
    type Target = KrakenfuturesCore;
...
impl crate::typed::TypedExchange for Krakenfutures {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Krakenfutures::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Krakenfutures::markets(self) }
}
