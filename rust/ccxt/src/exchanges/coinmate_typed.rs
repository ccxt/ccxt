...
//
// Typed wrapper for coinmate.
//
// Owns a `Box<CoinmateCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::coinmate::CoinmateCore;
use crate::types::*;
...
pub struct Coinmate {
    ...
    core: Box<CoinmateCore>,
}

impl Coinmate {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(CoinmateCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(CoinmateCore::new(config.into_option())) }
    }
...
impl crate::typed::TypedExchange for Coinmate {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Coinmate::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Coinmate::markets(self) }
}
