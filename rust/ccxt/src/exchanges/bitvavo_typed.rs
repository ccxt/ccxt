...
//
// Typed wrapper for bitvavo.
//
// Owns a `Box<BitvavoCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::bitvavo::BitvavoCore;
use crate::types::*;
...
pub struct Bitvavo {
    ...
    core: Box<BitvavoCore>,
}

impl Bitvavo {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BitvavoCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BitvavoCore::new(config.into_option())) }
    }

    pub fn from_core(core: BitvavoCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BitvavoCore {
        &mut self.core
    }
...
impl std::ops::Deref for Bitvavo {
    type Target = BitvavoCore;
...
impl crate::typed::TypedExchange for Bitvavo {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Bitvavo::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Bitvavo::markets(self) }
}
