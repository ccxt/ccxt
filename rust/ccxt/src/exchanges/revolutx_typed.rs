...
//
// Typed wrapper for revolutx.
//
// Owns a `Box<RevolutxCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::revolutx::RevolutxCore;
use crate::types::*;
...
pub struct Revolutx {
    ...
    core: Box<RevolutxCore>,
}

impl Revolutx {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(RevolutxCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(RevolutxCore::new(config.into_option())) }
    }

    pub fn from_core(core: RevolutxCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut RevolutxCore {
        &mut self.core
    }
...
impl std::ops::Deref for Revolutx {
    type Target = RevolutxCore;
...
impl crate::typed::TypedExchange for Revolutx {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Revolutx::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Revolutx::markets(self) }
}
