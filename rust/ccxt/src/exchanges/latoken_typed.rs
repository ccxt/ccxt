...
//
// Typed wrapper for latoken.
//
// Owns a `Box<LatokenCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::latoken::LatokenCore;
use crate::types::*;
...
pub struct Latoken {
    ...
    core: Box<LatokenCore>,
}

impl Latoken {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(LatokenCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(LatokenCore::new(config.into_option())) }
    }

    pub fn from_core(core: LatokenCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut LatokenCore {
        &mut self.core
    }
...
impl std::ops::Deref for Latoken {
    type Target = LatokenCore;
...
impl crate::typed::TypedExchange for Latoken {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Latoken::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Latoken::markets(self) }
}
