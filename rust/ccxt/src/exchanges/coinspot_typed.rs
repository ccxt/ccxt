...
//
// Typed wrapper for coinspot.
//
// Owns a `Box<CoinspotCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::coinspot::CoinspotCore;
use crate::types::*;
...
pub struct Coinspot {
    ...
    core: Box<CoinspotCore>,
}

impl Coinspot {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(CoinspotCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(CoinspotCore::new(config.into_option())) }
    }

    pub fn from_core(core: CoinspotCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut CoinspotCore {
        &mut self.core
    }
...
impl std::ops::Deref for Coinspot {
    type Target = CoinspotCore;
...
impl crate::typed::TypedExchange for Coinspot {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Coinspot::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Coinspot::markets(self) }
}
