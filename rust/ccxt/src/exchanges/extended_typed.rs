...
//
// Typed wrapper for extended.
//
// Owns a `Box<ExtendedCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::extended::ExtendedCore;
use crate::types::*;
...
pub struct Extended {
    ...
    core: Box<ExtendedCore>,
}

impl Extended {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(ExtendedCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(ExtendedCore::new(config.into_option())) }
    }

    pub fn from_core(core: ExtendedCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut ExtendedCore {
        &mut self.core
    }
...
impl crate::typed::TypedExchange for Extended {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Extended::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Extended::markets(self) }
}
