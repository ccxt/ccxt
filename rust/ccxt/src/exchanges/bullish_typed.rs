...
//
// Typed wrapper for bullish.
//
// Owns a `Box<BullishCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::bullish::BullishCore;
use crate::types::*;
...
pub struct Bullish {
    ...
    core: Box<BullishCore>,
}

impl Bullish {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BullishCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BullishCore::new(config.into_option())) }
    }

    pub fn from_core(core: BullishCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BullishCore {
        &mut self.core
    }
...
impl std::ops::Deref for Bullish {
    type Target = BullishCore;
...
impl crate::typed::TypedExchange for Bullish {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Bullish::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Bullish::markets(self) }
}
