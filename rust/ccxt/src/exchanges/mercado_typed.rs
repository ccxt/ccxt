...
//
// Typed wrapper for mercado.
//
// Owns a `Box<MercadoCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::mercado::MercadoCore;
use crate::types::*;
...
pub struct Mercado {
    ...
    core: Box<MercadoCore>,
}

impl Mercado {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(MercadoCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(MercadoCore::new(config.into_option())) }
    }

    pub fn from_core(core: MercadoCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut MercadoCore {
        &mut self.core
    }
...
impl std::ops::Deref for Mercado {
    type Target = MercadoCore;
...
impl crate::typed::TypedExchange for Mercado {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Mercado::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Mercado::markets(self) }
}
