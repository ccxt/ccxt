...
//
// Typed wrapper for digifinex.
//
// Owns a `Box<DigifinexCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::digifinex::DigifinexCore;
use crate::types::*;
...
pub struct Digifinex {
    ...
    core: Box<DigifinexCore>,
}

impl Digifinex {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(DigifinexCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(DigifinexCore::new(config.into_option())) }
    }

    pub fn from_core(core: DigifinexCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut DigifinexCore {
        &mut self.core
    }
...
impl std::ops::Deref for Digifinex {
    type Target = DigifinexCore;
...
impl crate::typed::TypedExchange for Digifinex {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Digifinex::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Digifinex::markets(self) }
}
