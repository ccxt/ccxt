...
//
// Typed wrapper for luno.
//
// Owns a `Box<LunoCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::luno::LunoCore;
use crate::types::*;
...
impl Luno {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(LunoCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(LunoCore::new(config.into_option())) }
    }

    pub fn from_core(core: LunoCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut LunoCore {
        &mut self.core
    }
...
impl std::ops::Deref for Luno {
    type Target = LunoCore;
...
impl crate::typed::TypedExchange for Luno {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Luno::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Luno::markets(self) }
}
