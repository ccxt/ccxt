...
//
// Typed wrapper for deepcoin.
//
// Owns a `Box<DeepcoinCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::deepcoin::DeepcoinCore;
use crate::types::*;
...
pub struct Deepcoin {
    ...
    core: Box<DeepcoinCore>,
}

impl Deepcoin {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(DeepcoinCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(DeepcoinCore::new(config.into_option())) }
    }

    pub fn from_core(core: DeepcoinCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut DeepcoinCore {
        &mut self.core
    }
...
impl std::ops::Deref for Deepcoin {
    type Target = DeepcoinCore;
...
impl crate::typed::TypedExchange for Deepcoin {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Deepcoin::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Deepcoin::markets(self) }
}
