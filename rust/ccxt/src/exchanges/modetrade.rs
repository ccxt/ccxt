 ...
 */
    ...
    pub fn parse_currency(&self, mut rawCurrency: Value) -> Value {
        ...
        });
        {
            ...
            while { if !__for_first_912 { j = add(&j, &Value::Int(1)); } __for_first_912 = false; is_less_than(&j, &get_array_length(&networks)) } {
        ...
        if !is_equal(&symbols, &Value::Null) {
            {
                ...
                while { if !__for_first_914 { i = add(&i, &Value::Int(1)); } __for_first_914 = false; is_less_than(&i, &get_array_length(&symbols)) } {
 ...
 */
    ...
    m
        ...
        let mut ordersRequests: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_915 { i = add(&i, &Value::Int(1)); } __for_first_915 = false; is_less_than(&i, &get_array_length(&orders)) } {
                ...
                if !is_true(&isSandboxMode) {
                    ...
                    if is_equal(&path, &Value::Str("batch-order".to_string())) {
                        let mut ordersList: Value = self.safe_list_k(params.clone(), "orders", &[Value::List(vec![])]);
                        {
                            ...
                            while { if !__for_first_917 { i = add(&i, &Value::Int(1)); } __for_first_917 = false; is_less_than(&i, &get_array_length(&ordersList)) } {
