...
//
// Typed wrapper for apex.
//
// Owns a `Box<ApexCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::apex::ApexCore;
use crate::types::*;
...
impl Apex {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(ApexCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(ApexCore::new(config.into_option())) }
    }

    pub fn from_core(core: ApexCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut ApexCore {
        &mut self.core
    }
...
impl std::ops::Deref for Apex {
    type Target = ApexCore;
...
impl crate::typed::TypedExchange for Apex {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Apex::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Apex::markets(self) }
}
