...
//
// Typed wrapper for lbank.
//
// Owns a `Box<LbankCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::lbank::LbankCore;
use crate::types::*;
...
impl Lbank {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(LbankCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(LbankCore::new(config.into_option())) }
    }

    pub fn from_core(core: LbankCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut LbankCore {
        &mut self.core
    }
...
impl std::ops::Deref for Lbank {
    type Target = LbankCore;
...
impl crate::typed::TypedExchange for Lbank {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Lbank::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Lbank::markets(self) }
}
