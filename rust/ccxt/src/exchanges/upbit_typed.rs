...
//
// Typed wrapper for upbit.
//
// Owns a `Box<UpbitCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::upbit::UpbitCore;
use crate::types::*;
...
impl Upbit {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(UpbitCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(UpbitCore::new(config.into_option())) }
    }

    pub fn from_core(core: UpbitCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut UpbitCore {
        &mut self.core
    }
...
impl std::ops::Deref for Upbit {
    type Target = UpbitCore;
...
impl crate::typed::TypedExchange for Upbit {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Upbit::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Upbit::markets(self) }
}
