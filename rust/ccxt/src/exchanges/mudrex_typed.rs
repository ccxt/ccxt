...
//
// Typed wrapper for mudrex.
//
// Owns a `Box<MudrexCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::mudrex::MudrexCore;
use crate::types::*;
...
impl Mudrex {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(MudrexCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(MudrexCore::new(config.into_option())) }
    }

    pub fn from_core(core: MudrexCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut MudrexCore {
        &mut self.core
    }
...
impl std::ops::Deref for Mudrex {
    type Target = MudrexCore;
...
impl crate::typed::TypedExchange for Mudrex {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Mudrex::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Mudrex::markets(self) }
}
