...
//
// Typed wrapper for pacifica.
//
// Owns a `Box<PacificaCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::pacifica::PacificaCore;
use crate::types::*;
...
pub struct Pacifica {
    ...
    core: Box<PacificaCore>,
}

impl Pacifica {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(PacificaCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(PacificaCore::new(config.into_option())) }
    }

    pub fn from_core(core: PacificaCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut PacificaCore {
        &mut self.core
    }
...
impl std::ops::Deref for Pacifica {
    type Target = PacificaCore;
...
impl crate::typed::TypedExchange for Pacifica {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Pacifica::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Pacifica::markets(self) }
}
