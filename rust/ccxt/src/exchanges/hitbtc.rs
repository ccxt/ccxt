 ...
 */
    ...
    m
        ...
        let mut ids: Value = object_keys(&response);
        {
            ...
            while { if !__for_first_723 { i = add(&i, &Value::Int(1)); } __for_first_723 = false; is_less_than(&i, &get_array_length(&ids)) } {
 ...
 */
    ...
    pub fn parse_currency(&self, mut currency: Value) -> Value {
        ...
        });
        {
            ...
            while { if !__for_first_724 { j = add(&j, &Value::Int(1)); } __for_first_724 = false; is_less_than(&j, &get_array_length(&rawNetworks)) } {
 ...
 */
    ...
    m
        ...
        let mut keys: Value = object_keys(&response);
        {
            ...
            while { if !__for_first_726 { i = add(&i, &Value::Int(1)); } __for_first_726 = false; is_less_than(&i, &get_array_length(&keys)) } {
            let mut marketId: Value = get_value(&keys, &i);
            let mut marketId: Value = get_value(&keys, &i);
            let mut market: Value = self.safe_market(&[marketId.clone()]);
            let mut symbol: Value = get_value(&market, &Value::Str("symbol".to_string()));
            let mut entry: Value = self.safe_dict(response.clone(), marketId.clone(), &[Value::Map({
    let mut m = indexmap::IndexMap::new();
    m
})]);
 ...
 */
    ...
    m
        ...
        let mut marketIds: Value = object_keys(&response);
        {
            ...
            while { if !__for_first_727 { i = add(&i, &Value::Int(1)); } __for_first_727 = false; is_less_than(&i, &get_array_length(&marketIds)) } {
            let mut marketId: Value = get_value(&marketIds, &i);
            let mut marketId: Value = get_value(&marketIds, &i);
            let mut marketInner: Value = self.market(marketId.clone());
            let mut rawTrades: Value = self.safe_list(response.clone(), marketId.clone(), &[Value::List(vec![])]);
 ...
 */
    ...
    m
        ...
        let mut marketIds: Value = object_keys(&response);
        {
            ...
            while { if !__for_first_728 { i = add(&i, &Value::Int(1)); } __for_first_728 = false; is_less_than(&i, &get_array_length(&marketIds)) } {
            let mut marketId: Value = get_value(&marketIds, &i);
            let mut marketId: Value = get_value(&marketIds, &i);
    ...
    m
            ...
            add_element_to_object(&mut result, &symbol, self.parse_order_book(orderbook.clone(), symbol.clone(), &[timestamp.clone(), Value::Str("bid".to_string()), Value::Str("ask".to_string())]));
        }
        }
        return result;
 ...
 */
    ...
    m
        ...
        });
        {
            ...
            while { if !__for_first_729 { i = add(&i, &Value::Int(1)); } __for_first_729 = false; is_less_than(&i, &get_array_length(&response)) } {
 ...
 */
    ...
    m
        ...
        });
        {
            ...
            while { if !__for_first_730 { i = add(&i, &Value::Int(1)); } __for_first_730 = false; is_less_than(&i, &get_array_length(&marketIds)) } {
 ...
 */
    ...
    m
        ...
        let mut rates: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_732 { i = add(&i, &Value::Int(1)); } __for_first_732 = false; is_less_than(&i, &get_array_length(&contracts)) } {
            ...
            let mut fundingRateData: Value = self.safe_list(response.clone(), marketId.clone(), &[Value::List(vec![])]);
            {
                ...
                while { if !__for_first_731 { j = add(&j, &Value::Int(1)); } __for_first_731 = false; is_less_than(&j, &get_array_length(&fundingRateData)) } {
 ...
 */
    ...
    m
        ...
        let mut result: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_733 { i = add(&i, &Value::Int(1)); } __for_first_733 = false; is_less_than(&i, &get_array_length(&response)) } {
 ...
 */
    ...
    pub fn parse_position(&self, mut position: Value, optional_args: &[Value]) -> Value {
        ...
        let mut contracts: Value = Value::Null;
        {
            ...
            while { if !__for_first_734 { i = add(&i, &Value::Int(1)); } __for_first_734 = false; is_less_than(&i, &get_array_length(&positions)) } {
        ...
        let mut collateral: Value = Value::Null;
        {
            ...
            while { if !__for_first_735 { i = add(&i, &Value::Int(1)); } __for_first_735 = false; is_less_than(&i, &get_array_length(&currencies)) } {
 ...
 */
    ...
    m
        ...
        let mut markets: Value = object_keys(&response);
        {
            ...
            while { if !__for_first_736 { i = add(&i, &Value::Int(1)); } __for_first_736 = false; is_less_than(&i, &get_array_length(&markets)) } {
            let mut marketId: Value = get_value(&markets, &i);
            let mut marketId: Value = get_value(&markets, &i);
            let mut marketInner: Value = self.safe_market(&[marketId.clone()]);
            let mut openInterest: Value = self.safe_dict(response.clone(), marketId.clone(), &[Value::Map({
    let mut m = indexmap::IndexMap::new();
    m
})]);
            append_to_array(&mut results, self.parse_open_interest(openInterest.clone(), &[marketInner.clone()]));
        }
        }
 ...
 */
    ...
    pub fn parse_deposit_withdraw_fee(&self, mut fee: Value, optional_args: &[Value]) -> Value {
        ...
        let mut result: Value = self.deposit_withdraw_fee(fee.clone());
        {
            ...
            while { if !__for_first_737 { j = add(&j, &Value::Int(1)); } __for_first_737 = false; is_less_than(&j, &get_array_length(&networks)) } {
