...
//
// Typed wrapper for coinbaseinternational.
//
// Owns a `Box<CoinbaseinternationalCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::coinbaseinternational::CoinbaseinternationalCore;
use crate::types::*;
...
pub struct Coinbaseinternational {
    ...
    core: Box<CoinbaseinternationalCore>,
}

impl Coinbaseinternational {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(CoinbaseinternationalCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(CoinbaseinternationalCore::new(config.into_option())) }
    }

    pub fn from_core(core: CoinbaseinternationalCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut CoinbaseinternationalCore {
        &mut self.core
    }
...
impl std::ops::Deref for Coinbaseinternational {
    type Target = CoinbaseinternationalCore;
...
impl crate::typed::TypedExchange for Coinbaseinternational {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Coinbaseinternational::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Coinbaseinternational::markets(self) }
}
