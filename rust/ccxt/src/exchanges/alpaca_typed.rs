...
//
// Typed wrapper for alpaca.
//
// Owns a `Box<AlpacaCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::alpaca::AlpacaCore;
use crate::types::*;
...
impl Alpaca {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(AlpacaCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(AlpacaCore::new(config.into_option())) }
    }

    pub fn from_core(core: AlpacaCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut AlpacaCore {
        &mut self.core
    }
...
impl std::ops::Deref for Alpaca {
    type Target = AlpacaCore;
...
impl crate::typed::TypedExchange for Alpaca {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Alpaca::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Alpaca::markets(self) }
}
