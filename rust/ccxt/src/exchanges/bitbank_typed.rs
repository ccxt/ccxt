...
//
// Typed wrapper for bitbank.
//
// Owns a `Box<BitbankCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::bitbank::BitbankCore;
use crate::types::*;
...
pub struct Bitbank {
    ...
    core: Box<BitbankCore>,
}

impl Bitbank {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BitbankCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BitbankCore::new(config.into_option())) }
    }

    pub fn from_core(core: BitbankCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BitbankCore {
        &mut self.core
    }
...
impl std::ops::Deref for Bitbank {
    type Target = BitbankCore;
...
impl crate::typed::TypedExchange for Bitbank {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Bitbank::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Bitbank::markets(self) }
}
