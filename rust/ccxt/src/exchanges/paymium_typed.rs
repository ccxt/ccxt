...
//
// Typed wrapper for paymium.
//
// Owns a `Box<PaymiumCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::paymium::PaymiumCore;
use crate::types::*;
...
pub struct Paymium {
    ...
    core: Box<PaymiumCore>,
}

impl Paymium {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(PaymiumCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(PaymiumCore::new(config.into_option())) }
    }

    pub fn from_core(core: PaymiumCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut PaymiumCore {
        &mut self.core
    }
...
impl std::ops::Deref for Paymium {
    type Target = PaymiumCore;
...
impl crate::typed::TypedExchange for Paymium {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Paymium::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Paymium::markets(self) }
}
