 ...
 */
    ...
    m
        ...
        let mut coins: Value = self.to_array(response.clone());
        {
            ...
            while { if !__for_first_894 { i = add(&i, &Value::Int(1)); } __for_first_894 = false; is_less_than(&i, &get_array_length(&coins)) } {
 ...
 */
    ...
    pub fn parse_balance(&self, mut response: Value) -> Value {
        ...
        let mut currencyIds: Value = object_keys(&balances);
        {
            ...
            while { if !__for_first_895 { i = add(&i, &Value::Int(1)); } __for_first_895 = false; is_less_than(&i, &get_array_length(&currencyIds)) } {
 ...
 */
    ...
    pub fn orders_to_trades(&self, mut orders: Value) -> Value {
        let mut result: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_897 { i = add(&i, &Value::Int(1)); } __for_first_897 = false; is_less_than(&i, &get_array_length(&orders)) } {
            let mut trades: Value = self.safe_value_k(get_value(&orders, &i), "trades", &[Value::List(vec![])]);
            {
                ...
                while { if !__for_first_896 { y = add(&y, &Value::Int(1)); } __for_first_896 = false; is_less_than(&y, &get_array_length(&trades)) } {
