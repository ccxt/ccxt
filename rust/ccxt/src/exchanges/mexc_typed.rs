...
//
// Typed wrapper for mexc.
//
// Owns a `Box<MexcCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::mexc::MexcCore;
use crate::types::*;
...
impl Mexc {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(MexcCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(MexcCore::new(config.into_option())) }
    }

    pub fn from_core(core: MexcCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut MexcCore {
        &mut self.core
    }
...
impl std::ops::Deref for Mexc {
    type Target = MexcCore;
...
impl crate::typed::TypedExchange for Mexc {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Mexc::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Mexc::markets(self) }
}
