...
//
// Typed wrapper for fmfwio.
//
// Owns a `Box<FmfwioCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::fmfwio::FmfwioCore;
use crate::types::*;
...
impl Fmfwio {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(FmfwioCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(FmfwioCore::new(config.into_option())) }
    }

    pub fn from_core(core: FmfwioCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut FmfwioCore {
        &mut self.core
    }
...
impl std::ops::Deref for Fmfwio {
    type Target = FmfwioCore;
...
impl crate::typed::TypedExchange for Fmfwio {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Fmfwio::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Fmfwio::markets(self) }
}
