...
//
// Typed wrapper for bequant.
//
// Owns a `Box<BequantCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::bequant::BequantCore;
use crate::types::*;
...
pub struct Bequant {
    ...
    core: Box<BequantCore>,
}

impl Bequant {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BequantCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BequantCore::new(config.into_option())) }
    }

    pub fn from_core(core: BequantCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BequantCore {
        &mut self.core
    }
...
impl std::ops::Deref for Bequant {
    type Target = BequantCore;
...
impl crate::typed::TypedExchange for Bequant {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Bequant::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Bequant::markets(self) }
}
