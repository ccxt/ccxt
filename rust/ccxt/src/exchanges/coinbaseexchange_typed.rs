...
//
// Typed wrapper for coinbaseexchange.
//
// Owns a `Box<CoinbaseexchangeCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::coinbaseexchange::CoinbaseexchangeCore;
use crate::types::*;
...
pub struct Coinbaseexchange {
    ...
    core: Box<CoinbaseexchangeCore>,
}

impl Coinbaseexchange {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(CoinbaseexchangeCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(CoinbaseexchangeCore::new(config.into_option())) }
    }

    pub fn from_core(core: CoinbaseexchangeCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut CoinbaseexchangeCore {
        &mut self.core
    }
...
impl std::ops::Deref for Coinbaseexchange {
    type Target = CoinbaseexchangeCore;
...
impl crate::typed::TypedExchange for Coinbaseexchange {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Coinbaseexchange::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Coinbaseexchange::markets(self) }
}
