...
//
// Typed wrapper for phemex.
//
// Owns a `Box<PhemexCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::phemex::PhemexCore;
use crate::types::*;
...
impl Phemex {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(PhemexCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(PhemexCore::new(config.into_option())) }
    }

    pub fn from_core(core: PhemexCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut PhemexCore {
        &mut self.core
    }
...
impl std::ops::Deref for Phemex {
    type Target = PhemexCore;
...
impl crate::typed::TypedExchange for Phemex {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Phemex::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Phemex::markets(self) }
}
