...
//
// Typed wrapper for bit2c.
//
// Owns a `Box<Bit2cCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::bit2c::Bit2cCore;
use crate::types::*;
...
impl Bit2c {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(Bit2cCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(Bit2cCore::new(config.into_option())) }
    }

    pub fn from_core(core: Bit2cCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut Bit2cCore {
        &mut self.core
    }
...
impl std::ops::Deref for Bit2c {
    type Target = Bit2cCore;
...
impl crate::typed::TypedExchange for Bit2c {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Bit2c::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Bit2c::markets(self) }
}
