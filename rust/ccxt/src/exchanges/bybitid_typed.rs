...
//
// Typed wrapper for bybitid.
//
// Owns a `Box<BybitidCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::bybitid::BybitidCore;
use crate::types::*;
...
pub struct Bybitid {
    ...
    core: Box<BybitidCore>,
}

impl Bybitid {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BybitidCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BybitidCore::new(config.into_option())) }
    }

    pub fn from_core(core: BybitidCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BybitidCore {
        &mut self.core
    }
...
impl std::ops::Deref for Bybitid {
    type Target = BybitidCore;
...
impl crate::typed::TypedExchange for Bybitid {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Bybitid::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Bybitid::markets(self) }
}
