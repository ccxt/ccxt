...
//
// Typed wrapper for bitbns.
//
// Owns a `Box<BitbnsCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::bitbns::BitbnsCore;
use crate::types::*;
...
impl Bitbns {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BitbnsCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BitbnsCore::new(config.into_option())) }
    }

    pub fn from_core(core: BitbnsCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BitbnsCore {
        &mut self.core
    }
...
impl std::ops::Deref for Bitbns {
    type Target = BitbnsCore;
...
impl crate::typed::TypedExchange for Bitbns {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Bitbns::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Bitbns::markets(self) }
}
