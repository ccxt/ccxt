...
//
// Typed wrapper for p2b.
//
// Owns a `Box<P2bCore>` and exposes the unified CCXT API
...
impl P2b {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(P2bCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(P2bCore::new(config.into_option())) }
    }

    pub fn from_core(core: P2bCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut P2bCore {
        &mut self.core
    }
...
impl crate::typed::TypedExchange for P2b {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { P2b::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { P2b::markets(self) }
}
