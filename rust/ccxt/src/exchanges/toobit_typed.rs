...
//
// Typed wrapper for toobit.
//
// Owns a `Box<ToobitCore>` and exposes the unified CCXT API
...
impl Toobit {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(ToobitCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(ToobitCore::new(config.into_option())) }
    }

    pub fn from_core(core: ToobitCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut ToobitCore {
        &mut self.core
    }
...
impl std::ops::Deref for Toobit {
    type Target = ToobitCore;
...
impl crate::typed::TypedExchange for Toobit {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Toobit::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Toobit::markets(self) }
}
