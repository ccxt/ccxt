...
//
// Typed wrapper for hibachi.
//
// Owns a `Box<HibachiCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::hibachi::HibachiCore;
use crate::types::*;
...
pub struct Hibachi {
    ...
    core: Box<HibachiCore>,
}

impl Hibachi {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(HibachiCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(HibachiCore::new(config.into_option())) }
    }

    pub fn from_core(core: HibachiCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut HibachiCore {
        &mut self.core
    }
...
impl std::ops::Deref for Hibachi {
    type Target = HibachiCore;
...
impl crate::typed::TypedExchange for Hibachi {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Hibachi::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Hibachi::markets(self) }
}
