...
//
// Typed wrapper for blofin.
//
// Owns a `Box<BlofinCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::blofin::BlofinCore;
use crate::types::*;
...
impl Blofin {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BlofinCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BlofinCore::new(config.into_option())) }
    }

    pub fn from_core(core: BlofinCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BlofinCore {
        &mut self.core
    }
...
impl std::ops::Deref for Blofin {
    type Target = BlofinCore;
...
impl crate::typed::TypedExchange for Blofin {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Blofin::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Blofin::markets(self) }
}
