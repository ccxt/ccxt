...
//
// Typed wrapper for woo.
//
// Owns a `Box<WooCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::woo::WooCore;
use crate::types::*;
...
impl Woo {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(WooCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(WooCore::new(config.into_option())) }
    }

    pub fn from_core(core: WooCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut WooCore {
        &mut self.core
    }
...
impl crate::typed::TypedExchange for Woo {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Woo::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Woo::markets(self) }
}
