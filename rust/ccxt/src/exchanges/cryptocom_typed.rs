...
//
// Typed wrapper for cryptocom.
//
// Owns a `Box<CryptocomCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::cryptocom::CryptocomCore;
use crate::types::*;
...
pub struct Cryptocom {
    ...
    core: Box<CryptocomCore>,
}

impl Cryptocom {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(CryptocomCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(CryptocomCore::new(config.into_option())) }
    }

    pub fn from_core(core: CryptocomCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut CryptocomCore {
        &mut self.core
    }
...
impl std::ops::Deref for Cryptocom {
    type Target = CryptocomCore;
...
impl crate::typed::TypedExchange for Cryptocom {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Cryptocom::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Cryptocom::markets(self) }
}
