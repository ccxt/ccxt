...
//
// Typed wrapper for bittrade.
//
// Owns a `Box<BittradeCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::bittrade::BittradeCore;
use crate::types::*;
...
pub struct Bittrade {
    ...
    core: Box<BittradeCore>,
}

impl Bittrade {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BittradeCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BittradeCore::new(config.into_option())) }
    }

    pub fn from_core(core: BittradeCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BittradeCore {
        &mut self.core
    }
...
impl std::ops::Deref for Bittrade {
    type Target = BittradeCore;
...
impl crate::typed::TypedExchange for Bittrade {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Bittrade::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Bittrade::markets(self) }
}
