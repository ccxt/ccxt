...
//
// Typed wrapper for indodax.
//
// Owns a `Box<IndodaxCore>` and exposes the unified CCXT API
...
pub struct Indodax {
    ...
    core: Box<IndodaxCore>,
}

impl Indodax {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(IndodaxCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(IndodaxCore::new(config.into_option())) }
    }

    pub fn from_core(core: IndodaxCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut IndodaxCore {
        &mut self.core
    }
...
impl std::ops::Deref for Indodax {
    type Target = IndodaxCore;
...
impl crate::typed::TypedExchange for Indodax {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Indodax::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Indodax::markets(self) }
}
