...
//
// Typed wrapper for btcbox.
//
// Owns a `Box<BtcboxCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::btcbox::BtcboxCore;
use crate::types::*;
...
impl Btcbox {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BtcboxCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BtcboxCore::new(config.into_option())) }
    }

    pub fn from_core(core: BtcboxCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BtcboxCore {
        &mut self.core
    }
...
impl std::ops::Deref for Btcbox {
    type Target = BtcboxCore;
...
impl crate::typed::TypedExchange for Btcbox {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Btcbox::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Btcbox::markets(self) }
}
