...
//
// Typed wrapper for hashkey.
//
// Owns a `Box<HashkeyCore>` and exposes the unified CCXT API
...
pub struct Hashkey {
    ...
    core: Box<HashkeyCore>,
}

impl Hashkey {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(HashkeyCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(HashkeyCore::new(config.into_option())) }
    }

    pub fn from_core(core: HashkeyCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut HashkeyCore {
        &mut self.core
    }
...
impl crate::typed::TypedExchange for Hashkey {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Hashkey::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Hashkey::markets(self) }
}
