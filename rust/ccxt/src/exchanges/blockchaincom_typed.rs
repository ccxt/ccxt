...
//
// Typed wrapper for blockchaincom.
//
// Owns a `Box<BlockchaincomCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::exchanges::blockchaincom::BlockchaincomCore;
use crate::types::*;
...
pub struct Blockchaincom {
    ...
    core: Box<BlockchaincomCore>,
}

impl Blockchaincom {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BlockchaincomCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BlockchaincomCore::new(config.into_option())) }
    }

    pub fn from_core(core: BlockchaincomCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BlockchaincomCore {
        &mut self.core
    }
...
impl std::ops::Deref for Blockchaincom {
    type Target = BlockchaincomCore;
...
impl crate::typed::TypedExchange for Blockchaincom {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Blockchaincom::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Blockchaincom::markets(self) }
}
