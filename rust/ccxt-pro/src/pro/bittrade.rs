 ...
 */
    ...
    m
        ...
        let mut url: Value = self.implode_params(crate::value::get_value_k(&get_value(&self.urls.as_map().and_then(|__m| __m.get("api")).cloned().unwrap_or(Value::Null).as_map().and_then(|__m| __m.get("ws")).cloned().unwrap_or(Value::Null), &api), "public"), hostname.clone());
 ...
 */
    ...
    pub fn handle_order_book_snapshot(&mut self, mut client: Value, mut message: Value, mut subscription: Value) {
        ...
        add_element_to_object(&mut snapshot, &Value::Str("nonce".into()), self.safe_integer_k(data.clone(), "seqNum", &[]));
        add_element_to_object(&mut snapshot, &Value::Str("timestamp".into()), timestamp.clone());
        add_element_to_object(&mut snapshot, &Value::Str("datetime".into()), self.iso8601(timestamp.clone()));
...
match _try_result { Ok(__try_ok) => { if !matches!(__try_ok, Value::Null) { return __try_ok; } return Value::Null; } Err(_try_err) => { let e: Value = panic_to_value(_try_err); 
    ...
    pub fn handle_order_book_message(&self, mut client: Value, mut message: Value, mut orderbook: Value) -> Value {
        ...
        if (is_less_than_or_equal(&prevSeqNum, &crate::value::get_value_k(&orderbook, "nonce"))) && is_true(&(seqNum.as_f64().unwrap_or(f64::NAN) > crate::value::get_value_k(&orderbook, "nonce").as_f64().unwrap_or(f64::NAN))) {
            ...
            add_element_to_object(&mut orderbook, &Value::Str("nonce".into()), seqNum.clone());
            let mut timestamp: Value = self.safe_integer_k(message.clone(), "ts", &[]);
