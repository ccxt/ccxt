 ...
 */
    ...
    pub fn handle_ticker(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        add_element_to_object(&mut ticker, &Value::Str("datetime".to_string()), self.iso8601(timestamp));
 ...
 */
    ...
    pub fn handle_order_book_snapshot(&mut self, mut client: Value, mut message: Value, mut subscription: Value) {
        ...
        add_element_to_object(&mut snapshot, &Value::Str("nonce".to_string()), self.safe_integer_k(data, "seqNum", &[]));
        add_element_to_object(&mut snapshot, &Value::Str("timestamp".to_string()), timestamp.clone());
        add_element_to_object(&mut snapshot, &Value::Str("datetime".to_string()), self.iso8601(timestamp));
...
match _try_result { Ok(__try_ok) => { if !matches!(__try_ok, Value::Null) { return __try_ok; } return Value::Null; } Err(_try_err) => { let e: Value = panic_to_value(_try_err); 
    ...
    pub fn handle_order_book_message(&self, mut client: Value, mut message: Value, mut orderbook: Value) -> Value {
        ...
        if (is_less_than_or_equal(&prevSeqNum, &crate::value::get_value_k(&orderbook, "nonce"))) && is_true(&(seqNum.as_f64().unwrap_or(f64::NAN) > crate::value::get_value_k(&orderbook, "nonce").as_f64().unwrap_or(f64::NAN))) {
            ...
            add_element_to_object(&mut orderbook, &Value::Str("nonce".to_string()), seqNum);
    ...
    m
        ...
        self.spawn(&[Value::Str("watch_order_book_snapshot".to_string()).clone(), client, message, subscription]);
}
    ...
    pub fn handle_ping(&mut self, mut client: Value, mut message: Value) {
        self.spawn(&[Value::Str("pong".to_string()).clone(), client, message]);
}
