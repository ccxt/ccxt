        ...
        if !is_equal(&symbols, &Value::Null) {
            {
                ...
                while { if !__for_first_594 { i = add(&i, &Value::Int(1)); } __for_first_594 = false; is_less_than(&i, &get_array_length(&symbols)) } {
 ...
 */
    ...
    pub fn handle_trade(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut data: Value = self.safe_value_k(message.clone(), "data", &[Value::List(vec![])]);
        {
            ...
            while { if !__for_first_595 { i = add(&i, &Value::Int(1)); } __for_first_595 = false; is_less_than(&i, &get_array_length(&data)) } {
            ...
            if !is_equal(&marketId, &Value::Null) {
                ...
                if is_equal(&eventType, &Value::Str("place".to_string())) || is_equal(&eventType, &Value::Str("canceled".to_string())) {
                    ...
                    let mut previousOrderTrades: Value = get_value(&previousOrder, &Value::Str("trades".to_string()));
                    {
                        ...
                        while { if !__for_first_596 { j = add(&j, &Value::Int(1)); } __for_first_596 = false; is_less_than(&j, &get_array_length(&previousOrderTrades)) } {
    ...
    m
        ...
        }
        {
            ...
            while { if !__for_first_598 { i = add(&i, &Value::Int(1)); } __for_first_598 = false; is_less_than(&i, &get_array_length(&marketIds)) } {
    ...
    pub fn handle_ticker(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut messageHashes: Value = self.find_message_hashes(client.clone(), Value::Str("ticker::".to_string()));
        {
            ...
            while { if !__for_first_600 { i = add(&i, &Value::Int(1)); } __for_first_600 = false; is_less_than(&i, &get_array_length(&messageHashes)) } {
                ...
                if !is_equal(&bids, &Value::Null) {
                    {
                        ...
                        while { if !__for_first_601 { j = add(&j, &Value::Int(1)); } __for_first_601 = false; is_less_than(&j, &get_array_length(&bids)) } {
                ...
                if !is_equal(&asks, &Value::Null) {
                    {
                        ...
                        while { if !__for_first_602 { j = add(&j, &Value::Int(1)); } __for_first_602 = false; is_less_than(&j, &get_array_length(&asks)) } {
    ...
    pub fn parse_ws_balance(&self, mut response: Value) -> Value {
        ...
        });
        {
            ...
            while { if !__for_first_604 { i = add(&i, &Value::Int(1)); } __for_first_604 = false; is_less_than(&i, &get_array_length(&response)) } {
