 ...
 */
    ...
    pub fn parse_ws_ticker(&self, mut message: Value, optional_args: &[Value]) -> Value {
        ...
        let mut marketType: Value = (if is_true(&(Value::Bool(instType.as_str() == Some("spot")))) { Value::Str("spot".to_string()) } else { Value::Str("contract".to_string()) });
 ...
 */
    ...
    m
        ...
        let mut data: Value = self.safe_list_k(message.clone(), "data", &[Value::List(vec![])]);
        {
            ...
            while { if !__for_first_2 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_2 = false; i.as_f64().unwrap_or(f64::NAN) < Value::Int(data.len() as i64).as_f64().unwrap_or(f64::NAN) } {
 ...
 */
    ...
    m
        ...
        if is_true(&(Value::Bool(limit.as_f64() == Some(1.0)))) || is_true(&(Value::Bool(limit.as_f64() == Some(5.0)))) || is_true(&(Value::Bool(limit.as_f64() == Some(15.0)))) || is_true(&(Value::Bool(limit.as_f64() == Some(50.0)))) {
 ...
 */
    ...
    pub fn handle_order_book(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut incrementalBook: bool = channel.as_str() == Some("books");
        if incrementalBook {
            ...
            if !isSnapshot && (is_equal(&checksum, &Value::Bool(true))) && is_true(&(Value::Bool(responseChecksum != Value::Null))) {
                ...
                let mut payloadArray: Value = Value::List(vec![]);
                {
                    ...
                    while { if !__for_first_4 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_4 = false; i.as_f64().unwrap_or(f64::NAN) < Value::Int(25).as_f64().unwrap_or(f64::NAN) } {
 ...
 */
    ...
    pub fn parse_ws_trade(&self, mut trade: Value, optional_args: &[Value]) -> Value {
        ...
        if (category != Value::Null) {
            defaultType = (if is_true(&(Value::Bool(category.as_str() != Some("SPOT")))) { Value::Str("contract".to_string()) } else { Value::Str("spot".to_string()) });
        }  else {
            defaultType = (if is_true(&(Value::Bool(posMode != Value::Null))) { Value::Str("contract".to_string()) } else { Value::Str("spot".to_string()) });
        }
        if (market == Value::Null) {
 ...
 */
    ...
    pub fn handle_positions(&mut self, mut client: Value, mut message: Value) {
        ...
        if !is_true(&(Value::Bool(in_op(&self.positions, &instType)))) || is_true(&(Value::Bool(action.as_str() == Some("snapshot")))) {
        ...
        let mut newPositions: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_8 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_8 = false; i.as_f64().unwrap_or(f64::NAN) < Value::Int(rawPositions.len() as i64).as_f64().unwrap_or(f64::NAN) } {
        ...
        let mut messageHashes: Value = self.find_message_hashes(client.clone(), Value::Str(format!("{}{}", instType, Value::Str(":positions::".to_string()))));
        {
            ...
            while { if !__for_first_9 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_9 = false; i.as_f64().unwrap_or(f64::NAN) < Value::Int(messageHashes.len() as i64).as_f64().unwrap_or(f64::NAN) } {
 ...
 */
    ...
    m
        ...
        let mut messageHash: Value = (if is_true(&(Value::Bool(isTrigger.as_bool() == Some(true)))) { Value::Str("triggerOrder".to_string()) } else { Value::Str("order".to_string()) });
        ...
        if is_true(&(Value::Bool((type_var.as_str() == Some("spot")) || (type_var.as_str() == Some("margin"))))) && is_true(&(Value::Bool(symbol == Value::Null))) {
        ...
        if (type_var.as_str() == Some("spot")) && is_true(&(Value::Bool(symbol != Value::Null))) {
        ...
        let mut instId: Value = (if is_true(&(Value::Bool((type_var.as_str() == Some("spot")) || (type_var.as_str() == Some("margin"))))) { marketId.clone() } else { Value::Str("default".to_string()) }); // different from other streams here the 'rest' id is required for spot markets, contract markets require default here
    ...
    pub fn handle_order(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut isLinearSwap: bool = category.as_str() == Some("usdt-futures");
        let mut isInverseSwap: bool = category.as_str() == Some("coin-futures");
        let mut isUSDCFutures: bool = category.as_str() == Some("usdc-futures");
        if (instType.as_str() == Some("uta")) {
            ...
            if is_true(&(Value::Bool(category.as_str() == Some("spot")))) || is_true(&(Value::Bool(category.as_str() == Some("margin")))) {
                marketType = Value::Str("spot".to_string());
            }  else {
                marketType = Value::Str("contract".to_string());
            }
        ...
        let mut isTrigger: bool = is_true(&(Value::Bool(channel.as_str() == Some("orders-algo")))) || is_true(&(Value::Bool(channel.as_str() == Some("ordersAlgo"))));
        ...
        });
        {
            ...
            while { if !__for_first_10 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_10 = false; i.as_f64().unwrap_or(f64::NAN) < Value::Int(data.len() as i64).as_f64().unwrap_or(f64::NAN) } {
        ...
        let mut keys: Value = object_keys(&marketSymbols);
        {
            ...
            if (channel.as_str() == Some("orders-crossed")) {
                innerMessageHash = Value::Str(format!("{}{}", innerMessageHash, Value::Str(":cross".to_string())));
            }  else if (channel.as_str() == Some("orders-isolated")) {
    ...
    pub fn parse_ws_order(&self, mut order: Value, optional_args: &[Value]) -> Value {
        ...
        if (category.as_str() == Some("spot")) {
            isSpot = true;
        }
        if (category.as_str() == Some("margin")) {
            isMargin = true;
        }
 ...
 */
    ...
    pub fn handle_my_trades(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut instType: Value = self.safe_string_lower(arg.clone(), Value::Str("instType".to_string()), &[]);
        {
            ...
            while { if !__for_first_12 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_12 = false; i.as_f64().unwrap_or(f64::NAN) < length.as_f64().unwrap_or(f64::NAN) } {
 ...
 */
    ...
    m
        ...
        let mut instTypeLower: Value = (if is_true(&(Value::Bool(instType == Value::Null))) { Value::Str("".to_string()) } else { to_lower(&instType) });
    ...
    pub fn handle_balance(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut data: Value = self.safe_list_k(message.clone(), "data", &[Value::List(vec![])]);
        {
            ...
            while { if !__for_first_14 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_14 = false; i.as_f64().unwrap_or(f64::NAN) < Value::Int(data.len() as i64).as_f64().unwrap_or(f64::NAN) } {
            ...
            if (instType.as_str() == Some("uta")) {
                let mut coins: Value = self.safe_list_k(rawBalance.clone(), "coin", &[Value::List(vec![])]);
                {
                    ...
                    while { if !__for_first_13 { j = (match (&(j), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_13 = false; j.as_f64().unwrap_or(f64::NAN) < Value::Int(coins.len() as i64).as_f64().unwrap_or(f64::NAN) } {
                    ...
                    if is_true(&(Value::Bool(borrow != Value::Null))) || is_true(&(Value::Bool(debts != Value::Null))) {
                ...
                let mut freeQuery: Value = (if is_true(&(Value::Bool(in_op(&rawBalance, &Value::Str("maxTransferOut".to_string()))))) { Value::Str("maxTransferOut".to_string()) } else { Value::Str("available".to_string()) });
    ...
    m
        ...
        if (sandboxMode.as_bool() == Some(true)) {
            ...
            if is_true(&(Value::Bool(instType.as_str() != Some("SCOIN-FUTURES")))) && is_true(&(Value::Bool(instType.as_str() != Some("SUSDT-FUTURES")))) && is_true(&(Value::Bool(instType.as_str() != Some("SUSDC-FUTURES")))) {
...
match _try_result { Ok(__try_ok) => { if !matches!(__try_ok, Value::Null) { return __try_ok; } return Value::Null; } Err(_try_err) => { let e: Value = panic_to_value(_try_err); 
    ...
    pub fn handle_order_book_un_subscription(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut instType: Value = self.safe_string_lower(arg.clone(), Value::Str("instType".to_string()), &[]);
        let mut type_var: Value = (if is_true(&(Value::Bool(instType.as_str() == Some("spot")))) { Value::Str("spot".to_string()) } else { Value::Str("contract".to_string()) });
    ...
    m
        ...
        }
        {
            ...
            while { if !__for_first_15 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_15 = false; i.as_f64().unwrap_or(f64::NAN) < Value::Int(argsList.len() as i64).as_f64().unwrap_or(f64::NAN) } {
