 ...
 */
    ...
    pub fn handle_tickers(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut result: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_674 { i = add(&i, &Value::Int(1)); } __for_first_674 = false; is_less_than(&i, &get_array_length(&data)) } {
 ...
 */
    ...
    pub fn handle_bid_ask(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut result: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_675 { i = add(&i, &Value::Int(1)); } __for_first_675 = false; is_less_than(&i, &get_array_length(&data)) } {
    ...
    m
            ...
            if !is_equal(&get_value(&ticker, &Value::Str("symbol".to_string())), &Value::Null) {
                add_element_to_object(&mut self.tickers, &get_value(&ticker, &Value::Str("symbol".to_string())), ticker.clone());
            }
            append_to_array(&mut result, ticker.clone());
        }
        }
 ...
 */
    ...
    m
        ...
        let mut topic: Value = ternary(is_true(&(is_equal(&trigger, &Value::Bool(true)))), Value::Str("algoexecutionreport".to_string()), Value::Str("executionreport".to_string()));
        ...
        if is_true(&Value::Bool(is_array(&data))) {
            {
                ...
                while { if !__for_first_676 { i = add(&i, &Value::Int(1)); } __for_first_676 = false; is_less_than(&i, &get_array_length(&data)) } {
 ...
 */
    ...
    m
        ...
        if !is_true(&self.is_empty(symbols.clone())) {
            ...
            }
            {
                ...
                while { if !__for_first_677 { i = add(&i, &Value::Int(1)); } __for_first_677 = false; is_less_than(&i, &get_array_length(&symbols)) } {
                if is_equal(&symbols, &Value::Null) {
                    panic!("{}", crate::exchange_errors::arguments_required(add(&self.id, &Value::Str(" watchPositions() symbols is required".to_string()))));
                }
    ...
    pub async fn load_positions_snapshot(&mut self, mut client: Value, mut messageHash: Value) -> Value {
        ...
        let mut cache: Value = self.positions.clone();
        {
            ...
            while { if !__for_first_678 { i = add(&i, &Value::Int(1)); } __for_first_678 = false; is_less_than(&i, &get_array_length(&positions)) } {
    ...
    pub fn handle_positions(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut newPositions: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_679 { i = add(&i, &Value::Int(1)); } __for_first_679 = false; is_less_than(&i, &get_array_length(&rawPositions)) } {
 ...
 */
    ...
    pub fn handle_balance(&mut self, mut client: Value, mut message: Value) {
        ...
        { let __be_tmp = self.iso8601(ts.clone()); add_element_to_object(&mut self.balance, &Value::Str("datetime".to_string()), __be_tmp); };
        {
            ...
            while { if !__for_first_680 { i = add(&i, &Value::Int(1)); } __for_first_680 = false; is_less_than(&i, &get_array_length(&keys)) } {
