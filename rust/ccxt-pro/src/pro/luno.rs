 ...
 */
    ...
    pub fn handle_trades(&mut self, mut client: Value, mut message: Value, mut subscription: Value) {
        ...
        }
        {
            ...
            while { if !__for_first_480 { i = add(&i, &Value::Int(1)); } __for_first_480 = false; is_less_than(&i, &get_array_length(&rawTrades)) } {
 ...
 */
    ...
    pub fn parse_order_book_bids_asks(&self, mut bidasks: Value, optional_args: &[Value]) -> Value {
        ...
        let mut result: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_481 { i = add(&i, &Value::Int(1)); } __for_first_481 = false; is_less_than(&i, &get_array_length(&bidasks)) } {
    ...
    pub fn handle_message(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut handlers: Value = Value::List(vec![Value::Str("handle_order_book".to_string()).clone(), Value::Str("handle_trades".to_string()).clone()]);
        {
            ...
            while { if !__for_first_482 { j = add(&j, &Value::Int(1)); } __for_first_482 = false; is_less_than(&j, &get_array_length(&handlers)) } {
