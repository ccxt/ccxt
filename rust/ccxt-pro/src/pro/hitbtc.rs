 ...
 */
    ...
    pub fn handle_order_book(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut type_var: Value = ternary(is_true(&(!is_equal(&snapshot, &Value::Null) && !is_equal(&snapshot, &Value::Null))), Value::Str("snapshot".to_string()), Value::Str("update".to_string()));
    ...
    pub fn handle_deltas(&self, mut bookside: Value, mut deltas: Value) {
        {
            ...
            while { if !__for_first_381 { i = add(&i, &Value::Int(1)); } __for_first_381 = false; is_less_than(&i, &get_array_length(&deltas)) } {
    ...
    m
        ...
        if is_equal(&symbols, &Value::Null) {
        ...
        }  else {
            {
                ...
                while { if !__for_first_382 { i = add(&i, &Value::Int(1)); } __for_first_382 = false; is_less_than(&i, &get_array_length(&symbols)) } {
 ...
 */
    ...
    pub fn handle_bid_ask(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut topic: Value = Value::Str("bidask".to_string());
        {
            ...
            while { if !__for_first_384 { i = add(&i, &Value::Int(1)); } __for_first_384 = false; is_less_than(&i, &get_array_length(&marketIds)) } {
            let mut marketId: Value = get_value(&marketIds, &i);
            let mut marketId: Value = get_value(&marketIds, &i);
            let mut market: Value = self.safe_market(&[marketId.clone()]);
            let mut symbol: Value = get_value(&market, &Value::Str("symbol".to_string()));
 ...
 */
    ...
    pub fn handle_trades(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut marketIds: Value = object_keys(&data);
        {
            ...
            let mut trades: Value = self.parse_ws_trades(get_value(&data, &marketId), &[market.clone()]);
            {
                ...
                while { if !__for_first_385 { j = add(&j, &Value::Int(1)); } __for_first_385 = false; is_less_than(&j, &get_array_length(&trades)) } {
    ...
    m
        ...
        let mut result: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_387 { i = add(&i, &Value::Int(1)); } __for_first_387 = false; is_less_than(&i, &get_array_length(&tradesArray)) } {
 ...
 */
    ...
    pub fn handle_ohlcv(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        }
        {
            ...
            while { if !__for_first_389 { i = add(&i, &Value::Int(1)); } __for_first_389 = false; is_less_than(&i, &get_array_length(&marketIds)) } {
            let mut marketId: Value = get_value(&marketIds, &i);
            let mut marketId: Value = get_value(&marketIds, &i);
            let mut market: Value = self.safe_market(&[marketId.clone()]);
            let mut symbol: Value = get_value(&market, &Value::Str("symbol".to_string()));
    ...
    m
            ...
            let mut ohlcvs: Value = self.parse_ws_ohlc_vs(get_value(&data, &marketId), &[market.clone()]);
            {
                ...
                while { if !__for_first_388 { j = add(&j, &Value::Int(1)); } __for_first_388 = false; is_less_than(&j, &get_array_length(&ohlcvs)) } {
        ...
        if is_true(&Value::Bool(is_array(&data))) {
            {
                ...
                while { if !__for_first_390 { i = add(&i, &Value::Int(1)); } __for_first_390 = false; is_less_than(&i, &get_array_length(&data)) } {
    ...
    pub fn handle_order_request(&self, mut client: Value, mut message: Value) -> Value {
        ...
        if is_true(&Value::Bool(is_array(&result))) {
            let mut parsedOrders: Value = Value::List(vec![]);
            {
                ...
                while { if !__for_first_391 { i = add(&i, &Value::Int(1)); } __for_first_391 = false; is_less_than(&i, &get_array_length(&result)) } {
