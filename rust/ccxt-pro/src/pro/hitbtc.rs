 ...
 */
    ...
    pub fn handle_order_book(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut marketIds: Value = object_keys(&data);
        {
            ...
            while { if !__for_first_371 { i = add(&i, &Value::Int(1)); } __for_first_371 = false; is_less_than(&i, &get_array_length(&marketIds)) } {
    ...
    pub fn handle_deltas(&self, mut bookside: Value, mut deltas: Value) {
        {
            ...
            while { if !__for_first_372 { i = add(&i, &Value::Int(1)); } __for_first_372 = false; is_less_than(&i, &get_array_length(&deltas)) } {
 ...
 */
    ...
    pub fn handle_trades(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut marketIds: Value = object_keys(&data);
        {
            ...
            let mut trades: Value = self.parse_ws_trades(get_value(&data, &marketId), &[market.clone()]);
            {
                ...
                while { if !__for_first_376 { j = add(&j, &Value::Int(1)); } __for_first_376 = false; is_less_than(&j, &get_array_length(&trades)) } {
    ...
    m
        ...
        let mut result: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_378 { i = add(&i, &Value::Int(1)); } __for_first_378 = false; is_less_than(&i, &get_array_length(&tradesArray)) } {
 ...
 */
    ...
    m
            ...
            let mut ohlcvs: Value = self.parse_ws_ohlc_vs(get_value(&data, &marketId), &[market.clone()]);
            {
                ...
                while { if !__for_first_379 { j = add(&j, &Value::Int(1)); } __for_first_379 = false; is_less_than(&j, &get_array_length(&ohlcvs)) } {
    ...
    pub fn handle_order_request(&self, mut client: Value, mut message: Value) -> Value {
        ...
        if is_true(&Value::Bool(is_array(&result))) {
            let mut parsedOrders: Value = Value::List(vec![]);
            {
                ...
                while { if !__for_first_382 { i = add(&i, &Value::Int(1)); } __for_first_382 = false; is_less_than(&i, &get_array_length(&result)) } {
