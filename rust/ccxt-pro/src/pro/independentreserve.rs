    ...
    m
        ...
        if is_true(&(is_equal(&checksum, &Value::Bool(true)))) && is_true(&(is_equal(&receivedSnapshot, &Value::Bool(true)))) {
            ...
            let mut payload: Value = Value::Str("".to_string());
            {
                ...
                while { if !__for_first_416 { i = add(&i, &Value::Int(1)); } __for_first_416 = false; is_less_than(&i, &Value::Int(10)) } {
    ...
    pub fn handle_deltas(&self, mut bookside: Value, mut deltas: Value) {
        {
            ...
            while { if !__for_first_418 { i = add(&i, &Value::Int(1)); } __for_first_418 = false; is_less_than(&i, &get_array_length(&deltas)) } {
