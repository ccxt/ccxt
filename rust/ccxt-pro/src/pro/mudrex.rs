 ...
 */
    pub fn set_broker_headers(&mut self) {
        ...
        add_element_to_object(&mut headers, &Value::Str("Partner-Id".into()), brokerId.clone());
        add_element_to_object(&mut innerOptions, &Value::Str("headers".into()), headers.clone());
        add_element_to_object(&mut wsOptions, &Value::Str("options".into()), innerOptions.clone());
    ...
    m
        ...
        let mut baseIdString: Value = (if is_true(&(market.as_map().and_then(|__m| __m.get("baseId")).cloned().unwrap_or(Value::Null) != Value::Null)) { market.as_map().and_then(|__m| __m.get("baseId")).cloned().unwrap_or(Value::Null) } else { Value::Str("".into()) });
        let mut quoteIdString: Value = (if is_true(&(market.as_map().and_then(|__m| __m.get("quoteId")).cloned().unwrap_or(Value::Null) != Value::Null)) { market.as_map().and_then(|__m| __m.get("quoteId")).cloned().unwrap_or(Value::Null) } else { Value::Str("".into()) });
    ...
    m
        ...
        return self.filter_by_array_tickers(self.tickers.clone(), Value::Str("symbol".into()), &[symbols.clone()]);
    ...
    m
        ...
        let mut streamBaseId: Value = (if is_true(&(market.as_map().and_then(|__m| __m.get("baseId")).cloned().unwrap_or(Value::Null) != Value::Null)) { market.as_map().and_then(|__m| __m.get("baseId")).cloned().unwrap_or(Value::Null) } else { Value::Str("".into()) });
        let mut streamQuoteId: Value = (if is_true(&(market.as_map().and_then(|__m| __m.get("quoteId")).cloned().unwrap_or(Value::Null) != Value::Null)) { market.as_map().and_then(|__m| __m.get("quoteId")).cloned().unwrap_or(Value::Null) } else { Value::Str("".into()) });
    ...
    pub fn handle_ohlcv(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut parsed: Value = Value::from(vec![self.safe_timestamp(data.clone(), Value::Str("t".into()), &[]), self.safe_number_k(data.clone(), "o", &[]), self.safe_number_k(data.clone(), "h", &[]), self.safe_number_k(data.clone(), "l", &[]), self.safe_number_k(data.clone(), "c", &[]), self.safe_number_k(data, "v", &[])]);
    ...
    pub fn handle_ticker(&mut self, mut client: Value, mut message: Value) {
        let mut data: Value = self.safe_list_k(message.clone(), "data", &[Value::from(vec![])]);
        {
            ...
            client.resolve(&[result.clone(), Value::Str("tickers".into())]);
        }
        }
}
}
