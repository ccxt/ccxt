 ...
 */
                ...
                if (previousSequence == Value::Null) || (sequence.as_f64().unwrap_or(f64::NAN) > previousSequence.as_f64().unwrap_or(f64::NAN)) {
                    if (type_var.as_deref() == Some("match")) {
                        ...
                        let mut trades: Value = previousOrder.as_map().and_then(|__m| __m.get("trades")).cloned().unwrap_or(Value::Null);
                        {
                            ...
                            totalAmount = self.safe_string_k(tradeEntry.clone(), "amount", &[Value::Str("0".into())]);
                        }
                        }
                        ...
                        add_element_to_object(&mut previousOrder, &Value::Str("cost".into()), self.parse_number(totalCost.clone(), &[]));
    ...
    m
                        ...
                        if (crate::value::get_value_k(&previousOrder.as_map().and_then(|__m| __m.get("fee")).cloned().unwrap_or(Value::Null), "cost") != Value::Null) && (self.safe_number(trade.as_map().and_then(|__m| __m.get("fee")).cloned().unwrap_or(Value::Null), Value::Str("cost".into()), &[]) != Value::Null) {
                            ...
                            add_element_to_object(get_value_mut(&mut previousOrder, &Value::Str("fee".into())), &Value::Str("cost".into()), self.parse_number(crate::precise::Precise::stringAdd(&self.safe_string_k(previousOrderFee.clone(), "cost", &[]), &self.safe_string_k(tradeFee.clone(), "cost", &[])), &[]));
                        }
    ...
    m
            ...
            self.handle_deltas(get_value(&orderbook, &Value::Str("asks".into())), (match __pro_message.get("asks").cloned() { Some(__v) if matches!(__v, Value::Arr(_)) => __v, _ => Value::from(vec![]) }));
            self.handle_deltas(get_value(&orderbook, &Value::Str("bids".into())), (match __pro_message.get("bids").cloned() { Some(__v) if matches!(__v, Value::Arr(_)) => __v, _ => Value::from(vec![]) }));
            ...
            let mut orderbook: Value = get_value(&self.orderbooks, &symbol);
            let mut timestamp: Value = self.parse8601((match __pro_message.get("time").cloned() { Some(Value::Str(__s)) if !__s.is_empty() => Value::Str(__s), Some(Value::Int(__n)) => Value::Str(__n.to_string().into()), Some(Value::Float(__f)) => Value::Str(__f.to_string().into()), _ => Value::Null }));
    ...
    pub fn handle_error_message(&self, mut client: Value, mut message: Value) -> Value {
        ...
        //     }
        //
        let mut errMsg: Option<String> = (match __pro_message.get("message").cloned() { Some(Value::Str(__s)) if !__s.is_empty() => Value::Str(__s), Some(Value::Int(__n)) => Value::Str(__n.to_string().into()), Some(Value::Float(__f)) => Value::Str(__f.to_string().into()), _ => Value::Null }).as_str().map(str::to_owned);
