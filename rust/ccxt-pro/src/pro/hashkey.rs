 ...
 */
    ...
    m
        ...
        let mut snapshot: Value = self.parse_order_book(dataEntry.clone(), symbol.clone(), &[timestamp.clone(), Value::Str("b".into()), Value::Str("a".into())]);
        orderbook.reset(snapshot.clone());
        add_element_to_object(&mut orderbook, &Value::Str("nonce".into()), self.safe_integer_k(message.clone(), "id", &[]));
    ...
    pub fn parse_ws_trade(&self, mut trade: Value, optional_args: &[Value]) -> Value {
        ...
        if (isBuyerMaker != Value::Null) {
            if isPublicTrade {
                ...
                side = (if is_true(&isBuyerMaker) { Value::Str("sell".into()) } else { Value::Str("buy".into()) });
            }  else {
                takerOrMaker = (if is_true(&isBuyerMaker) { Value::Str("maker".into()) } else { Value::Str("taker".into()) });
 ...
 */
    ...
    pub fn handle_position(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut symbol: Value = parsed.as_map().and_then(|__m| __m.get("symbol")).cloned().unwrap_or(Value::Null);
        client.resolve(&[parsed.clone(), Value::Str(format!("{}{}", Value::Str(format!("{}{}", messageHash, Value::Str(":".into())).into()), symbol).into())]);
}
 ...
 */
    ...
    m
        ...
        add_element_to_object(get_value_mut(&mut self.balance, &type_var), &Value::Str("info".into()), message.clone());
    ...
    m
        ...
        let _try_result = futures::FutureExt::catch_unwind(std::panic::AssertUnwindSafe(async {
            ...
            self.delay(listenKeyRefreshRate.clone(), &[Value::Str("keep_alive_listen_key".into()).clone(), listenKey.clone(), params.clone()]).await;
