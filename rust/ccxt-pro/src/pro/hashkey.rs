 ...
 */
    ...
    m
        ...
        let mut stored: Value = get_value(&get_value(&self.ohlcvs, &symbol), &timeframe);
        {
            ...
            while { if !__for_first_367 { i = add(&i, &Value::Int(1)); } __for_first_367 = false; is_less_than(&i, &get_array_length(&data)) } {
    ...
    pub fn handle_trades(&mut self, mut client: Value, mut message: Value) {
        ...
        if !is_equal(&data, &Value::Null) {
            data = self.sort_by(data.clone(), Value::Str("t".to_string()), &[]);
            {
                ...
                while { if !__for_first_368 { i = add(&i, &Value::Int(1)); } __for_first_368 = false; is_less_than(&i, &get_array_length(&data)) } {
