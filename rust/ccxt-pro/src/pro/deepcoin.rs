 ...
 */
    ...
    pub fn handle_order_book_snapshot(&self, mut client: Value, mut message: Value) {
        ...
        });
        {
            ...
            while { if !__for_first_299 { i = add(&i, &Value::Int(1)); } __for_first_299 = false; is_less_than(&i, &get_array_length(&entries)) } {
        ...
        let mut cachedMessages: Value = get_value(&orderbook, &Value::Str("cache".to_string()));
        {
            ...
            while { if !__for_first_300 { j = add(&j, &Value::Int(1)); } __for_first_300 = false; is_less_than(&j, &get_array_length(&cachedMessages)) } {
