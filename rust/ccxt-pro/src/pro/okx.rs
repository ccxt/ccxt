 ...
 */
    ...
    pub fn parse_ws_bid_ask(&self, mut ticker: Value, optional_args: &[Value]) -> Value {
        ...
        if (ask == Value::Null) {
            ...
            let mut firstAsk: Value = self.safe_list(asks.clone(), Value::Int(0), &[Value::List(vec![])]);
        ...
        if (bid == Value::Null) {
            ...
            let mut firstBid: Value = self.safe_list(bids.clone(), Value::Int(0), &[Value::List(vec![])]);
 ...
 */
    ...
    pub fn handle_liquidation(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut rawLiquidations: Value = self.safe_list_k(message, "data", &[Value::List(vec![])]);
        {
            ...
            client.resolve(&[Value::List(vec![liquidation.clone()]), Value::Str(format!("{}{}", Value::Str("liquidations::".to_string()), symbol))]);
        }
        }
}
 ...
 */
    ...
    pub fn handle_my_liquidation(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut rawLiquidations: Value = self.safe_list_k(message, "data", &[Value::List(vec![])]);
        {
            ...
            client.resolve(&[Value::List(vec![liquidation.clone()]), Value::Str(format!("{}{}", Value::Str("myLiquidations::".to_string()), symbol))]);
        }
        }
}
 ...
 */
    ...
    m
        ...
        if is_true(&self.newUpdates) {
            return (if is_true(&(Value::Bool(newPositions == Value::Null))) { Value::List(vec![]) } else { newPositions.clone() });
        }
