 ...
 */
    ...
    pub fn handle_ticker(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut channel: Value = self.safe_string_k(arg.clone(), "channel", &[]);
        if is_equal(&channel, &Value::Str("tickers".to_string())) {
 ...
 */
    ...
    pub fn handle_liquidation(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut rawLiquidations: Value = self.safe_list_k(message.clone(), "data", &[Value::List(vec![])]);
        {
            ...
            while { if !__for_first_541 { i = add(&i, &Value::Int(1)); } __for_first_541 = false; is_less_than(&i, &get_array_length(&rawLiquidations)) } {
 ...
 */
    ...
    pub fn handle_my_liquidation(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut rawLiquidations: Value = self.safe_list_k(message.clone(), "data", &[Value::List(vec![])]);
        {
            ...
            while { if !__for_first_543 { i = add(&i, &Value::Int(1)); } __for_first_543 = false; is_less_than(&i, &get_array_length(&rawLiquidations)) } {
            let mut rawLiquidation: Value = get_value(&rawLiquidations, &i);
            let mut rawLiquidation: Value = get_value(&rawLiquidations, &i);
 ...
 */
    ...
    m
        ...
        let mut messageHashes: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_544 { i = add(&i, &Value::Int(1)); } __for_first_544 = false; is_less_than(&i, &get_array_length(&symbolsAndTimeframes)) } {
 ...
 */
    ...
    m
        ...
        let mut messageHashes: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_545 { i = add(&i, &Value::Int(1)); } __for_first_545 = false; is_less_than(&i, &get_array_length(&symbolsAndTimeframes)) } {
            let mut symbolAndTimeframe: Value = get_value(&symbolsAndTimeframes, &i);
            let mut symbolAndTimeframe: Value = get_value(&symbolsAndTimeframes, &i);
            let mut sym: Value = get_value(&symbolAndTimeframe, &Value::Int(0));
            let mut tf: Value = get_value(&symbolAndTimeframe, &Value::Int(1));
    ...
    pub fn handle_deltas(&self, mut bookside: Value, mut deltas: Value) {
        {
            ...
            while { if !__for_first_549 { i = add(&i, &Value::Int(1)); } __for_first_549 = false; is_less_than(&i, &get_array_length(&deltas)) } {
 ...
 */
    ...
    pub fn handle_orders(&mut self, mut client: Value, mut message: Value) {
        ...
        if is_greater_than(&ordersLength, &Value::Int(0)) {
            ...
            let mut parsed: Value = self.parse_orders(orders.clone(), &[]);
            {
                ...
                while { if !__for_first_554 { i = add(&i, &Value::Int(1)); } __for_first_554 = false; is_less_than(&i, &get_array_length(&parsed)) } {
            ...
            client.resolve(&[stored.clone(), channel.clone()]);
            {
                ...
                while { if !__for_first_555 { i = add(&i, &Value::Int(1)); } __for_first_555 = false; is_less_than(&i, &get_array_length(&marketIds)) } {
    ...
    pub fn handle_my_trades(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut filteredOrders: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_556 { i = add(&i, &Value::Int(1)); } __for_first_556 = false; is_less_than(&i, &get_array_length(&rawOrders)) } {
        ...
        });
        {
            ...
            while { if !__for_first_557 { i = add(&i, &Value::Int(1)); } __for_first_557 = false; is_less_than(&i, &get_array_length(&filteredOrders)) } {
        ...
        let mut tradeSymbols: Value = object_keys(&symbols);
        {
            ...
            while { if !__for_first_558 { i = add(&i, &Value::Int(1)); } __for_first_558 = false; is_less_than(&i, &get_array_length(&tradeSymbols)) } {
 ...
 */
    ...
    m
        ...
        });
        {
            ...
            while { if !__for_first_559 { i = add(&i, &Value::Int(1)); } __for_first_559 = false; is_less_than(&i, &idsLength) } {
