 ...
 */
    ...
    m
            ...
            client.resolve(&[fullParsedTicker.clone(), Value::Str("alltickers".into())]);
        }
        }
        return message;
 ...
 */
    ...
    pub fn handle_liquidation(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut liquidationsBySymbol: Value = self.index_by(newLiquidations.clone(), Value::Str("symbol".into()));
 ...
 */
    ...
    pub fn handle_trades(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut dataByMarketIds: Value = self.group_by(data.clone(), Value::Str("symbol".into()), &[]);
        ...
        if (authenticated == Value::Null) {
            ...
            let mut request: Value = Value::Map({
                let mut m = indexmap::IndexMap::new();
                    ...
                    m.insert("args".to_string(), Value::from(vec![self.apiKey.clone(), timestamp.clone(), signature.clone()]));
                m
            });
 ...
 */
    ...
    pub fn handle_my_trades(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut messageHash: Value = self.safe_string_k(message.clone(), "table", &[]);
        let mut data: Value = self.safe_list_k(message, "data", &[Value::from(vec![])]);
        let mut dataByExecType: Value = self.group_by(data.clone(), Value::Str("execType".into()), &[]);
        ...
        let mut keys: Value = object_keys(&symbols);
        {
            ...
            client.resolve(&[stored.clone(), Value::Str(format!("{}{}", Value::Str(format!("{}{}", messageHash, Value::Str(":".into())).into()), get_value(&keys, &i)).into())]);
        }
        }
}
 ...
 */
    ...
    m
            ...
            add_element_to_object(&mut orderbook, &Value::Str("symbol".into()), symbol.clone());
            {
                ...
                side = (if is_true(&(side.as_str() == Some("Buy"))) { Value::Str("bids".into()) } else { Value::Str("asks".into()) });
            ...
            });
            {
                ...
                let mut size: Value = (if is_true(&(action.as_deref() == Some("delete"))) { Value::Int(0) } else { self.parent.convert_from_raw_quantity(symbol.clone(), self.safe_string_k(get_value(&data, &i), "size", &[Value::Str("0".into())]), &[]) });
                let mut id: Value = self.safe_string_k(get_value(&data, &i), "id", &[]);
                let mut side: Value = self.safe_string_k(get_value(&data, &i), "side", &[]);
                side = (if is_true(&(side.as_str() == Some("Buy"))) { Value::Str("bids".into()) } else { Value::Str("asks".into()) });
                let mut bookside: Value = get_value(&orderbook, &side);
