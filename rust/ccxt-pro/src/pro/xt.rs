...
if let Err(_try_err) = _try_result { let e: Value = panic_to_value(_try_err);
    ...
    pub fn get_cache_index(&self, mut orderbook: Value, mut cache: Value) -> Value {
        ...
        }
        {
            ...
            while { if !__for_first_662 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_662 = false; i.as_f64().unwrap_or(f64::NAN) < get_array_length(&cache).as_f64().unwrap_or(f64::NAN) } {
    ...
    pub fn handle_delta(&self, mut orderbook: Value, mut delta: Value) {
        ...
        let mut asks: Value = crate::value::get_value_k(&orderbook, "asks");
        {
            ...
            while { if !__for_first_663 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_663 = false; i.as_f64().unwrap_or(f64::NAN) < ((obBids.len() as i64) as f64) } {
            let mut bid: Value = self.safe_list(obBids.clone(), i.clone(), &[]);
        ...
        }
        {
            ...
            while { if !__for_first_664 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_664 = false; i.as_f64().unwrap_or(f64::NAN) < ((obAsks.len() as i64) as f64) } {
            let mut ask: Value = self.safe_list(obAsks.clone(), i.clone(), &[]);
 ...
 */
    ...
    m
        ...
        let mut type_varparamsMarketTypeVariable = self.handle_market_type_and_params(methodName, &[market, params]);
        ...
        let mut request: Value = self.extend(subscribe, &[paramsMarketType]);
 ...
 */
    ...
    m
        ...
        let mut request: Value = self.extend(unsubscribe, &[paramsMarketType.clone()]);
        let mut tail: Value = access;
        if matches!(&isContract, Value::Bool(true)) {
            tail = (if privateAccess { Value::Str("user".into()) } else { Value::Str("market".into()) });
        }
        ...
        let mut subscriptionParamsOmitted: Value = self.omit(subscriptionParams, Value::Str("symbolsAndTimeframes".into()), &[]);
        let __ws_arg_0 = self.extend(request, &[paramsMarketType]);
        let __ws_arg_1 = self.extend(subscription, &[subscriptionParamsOmitted]);
 ...
 */
    ...
    m
        ...
        let mut paramsOmitted: Value = self.omit(params, Value::Str("levels".into()), &[]);
        ...
        let mut orderbook: Value = self.subscribe(name, Value::Str("public".into()), Value::Str("watchOrderBook".into()), &[market, Value::Null, paramsOmitted]).await;
 ...
 */
    ...
    m
        ...
        let mut messageHash: Value = Value::Str(format!("{}{}", Value::Str("unsubscribe::".into()), name).into());
        return self.un_subscribe(messageHash, name, Value::Str("public".into()), Value::Str("unWatchOrderBook".into()), Value::Str("orderbook".into()), &[market, Value::from(vec![symbol]), paramsOmitted]).await;
 ...
 */
    ...
    pub async fn load_positions_snapshot(&mut self, mut client: Value, mut messageHash: Value) -> Value {
        ...
        let mut cache: Value = self.positions.clone();
        {
            ...
            while { if !__for_first_665 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_665 = false; i.as_f64().unwrap_or(f64::NAN) < ((positions.len() as i64) as f64) } {
    ...
    m
        ...
        if (marketId.is_some()) {
            ...
            let mut event: Value = (match __pro_message.get("event").cloned() { Some(Value::Str(__s)) if !__s.is_empty() => Value::Str(__s), Some(Value::Int(__n)) => Value::Str(__n.to_string().into()), Some(Value::Float(__f)) => Value::Str(__f.to_string().into()), _ => Value::Null });
            let mut messageHashTail: Value = Value::Str("contract".into());
            if isSpot {
    ...
    pub fn handle_tickers(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut tradeType: Value = Value::Str("contract".into());
        if (spotTest.is_some()) {
        ...
        let mut messageHashes: Value = self.find_message_hashes(client.clone(), Value::Str(format!("{}{}", messageHashStart, Value::Str("::".into())).into()));
        {
            ...
            while { if !__for_first_668 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_668 = false; i.as_f64().unwrap_or(f64::NAN) < ((messageHashes.len() as i64) as f64) } {
            let mut messageHash: Value = messageHashes.as_array().and_then(|__arr| match &i { Value::Int(__n) => __arr.get(*__n as usize), Value::Str(__s) => __s.parse::<usize>().ok().and_then(|__n| __arr.get(__n)), _ => None }).cloned().unwrap_or(Value::Null);
            let mut parts: Value = split(&messageHash, &Value::Str("::".into()));
    ...
    m
        ...
        if (marketId != Value::Null) {
            ...
            let mut tradeType: Value = Value::Str("contract".into());
            if (matches!(&data, Value::Dict(__d) if __d.contains_key("q"))) {
                tradeType = Value::Str("spot".into());
            }
    ...
    m
            ...
            if (obBids != Value::Null) {
                let mut bids: Value = get_value(&orderbook, &Value::Str("bids".into()));
                {
                    ...
                    while { if !__for_first_670 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_670 = false; i.as_f64().unwrap_or(f64::NAN) < ((obBids.len() as i64) as f64) } {
                    let mut bid: Value = self.safe_list(obBids.clone(), i.clone(), &[]);
                    let mut price: Value = self.safe_number(bid.clone(), Value::Int(0), &[]);
                    let mut quantity: Value = self.safe_number(bid, Value::Int(1), &[]);
    ...
    pub fn parse_ws_order_trade(&self, mut trade: Value, optional_args: &[Value]) -> Value {
        ...
        let mut tradeType: Value = Value::Str("spot".into());
        if (matches!(&trade, Value::Dict(__d) if __d.contains_key("symbol"))) {
            tradeType = Value::Str("contract".into());
        }
        let mut marketResolved: Value = self.safe_market(&[marketId, market, Value::Null, tradeType]);
    ...
    pub fn parse_ws_order(&self, mut order: Value, optional_args: &[Value]) -> Value {
        ...
        let mut tradeType: Value = Value::Str("spot".into());
        if (matches!(&order, Value::Dict(__d) if __d.contains_key("symbol"))) {
            tradeType = Value::Str("contract".into());
        }
        let mut marketResolved: Value = self.safe_market(&[marketId, market, Value::Null, tradeType]);
    ...
    let mut m = indexmap::IndexMap::new();
        ...
        m.insert("symbol".to_string(), marketResolved.as_map().and_then(|__m| __m.get("symbol")).cloned().unwrap_or(Value::Null));
        m.insert("type".to_string(), marketResolved.as_map().and_then(|__m| __m.get("type")).cloned().unwrap_or(Value::Null));
    ...
    m
        ...
        let mut tradeType: Value = Value::Str("spot".into());
        if (matches!(&data, Value::Dict(__d) if __d.contains_key("coin"))) {
            tradeType = Value::Str("contract".into());
        }
    ...
    pub fn handle_un_subscription(&mut self, mut client: Value, mut subscription: Value) {
        ...
        let mut subMessageHashes: Value = (match __pro_message.get("subMessageHashes").cloned() { Some(__v) if matches!(__v, Value::Arr(_)) => __v, _ => Value::from(vec![]) });
        {
            ...
            while { if !__for_first_671 { j = (match (&(j), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_671 = false; j.as_f64().unwrap_or(f64::NAN) < ((messageHashes.len() as i64) as f64) } {
