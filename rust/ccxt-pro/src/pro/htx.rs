 ...
 */
    ...
    pub fn handle_trades(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        }
        {
            ...
            while { if !__for_first_392 { i = add(&i, &Value::Int(1)); } __for_first_392 = false; is_less_than(&i, &get_array_length(&data)) } {
        ...
        let _try_result = futures::FutureExt::catch_unwind(std::panic::AssertUnwindSafe(async {
            ...
            if is_true(&(is_equal(&sequence, &Value::Null))) || is_true(&(is_less_than(&nonce, &sequence))) {
                ...
                orderbook.reset(snapshot.clone());
                {
                    ...
                    while { if !__for_first_393 { i = add(&i, &Value::Int(1)); } __for_first_393 = false; is_less_than(&i, &get_array_length(&messages)) } {
    ...
    pub fn handle_deltas(&self, mut bookside: Value, mut deltas: Value) {
        {
            ...
            while { if !__for_first_394 { i = add(&i, &Value::Int(1)); } __for_first_394 = false; is_less_than(&i, &get_array_length(&deltas)) } {
    ...
    m
        ...
        if is_true(&self.is_empty(rawPositions.clone())) {
            let mut prefixes: Value = Value::List(vec![Value::Str("cross:positions".to_string()), Value::Str("isolated:positions".to_string())]);
            {
                ...
                while { if !__for_first_396 { i = add(&i, &Value::Int(1)); } __for_first_396 = false; is_less_than(&i, &get_array_length(&prefixes)) } {
        ...
        let mut timestamp: Value = self.safe_integer_k(message.clone(), "ts", &[]);
        {
            ...
            while { if !__for_first_397 { i = add(&i, &Value::Int(1)); } __for_first_397 = false; is_less_than(&i, &get_array_length(&rawPositions)) } {
        ...
        let mut marginModes: Value = object_keys(&positionsByMarginMode);
        {
            ...
            while { if !__for_first_399 { i = add(&i, &Value::Int(1)); } __for_first_399 = false; is_less_than(&i, &get_array_length(&marginModes)) } {
        ...
        if !is_equal(&channel, &Value::Null) {
            ...
            if is_equal(&topic, &Value::Str("account".to_string())) {
                ...
                let mut detailsLength: Value = get_array_length(&details);
                {
                    ...
                    while { if !__for_first_400 { i = add(&i, &Value::Int(1)); } __for_first_400 = false; is_less_than(&i, &detailsLength) } {
