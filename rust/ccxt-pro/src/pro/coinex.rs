...
impl CoinexCore {
    ...
    pub fn handle_ticker(&mut self, mut client: Value, mut message: Value) {
        ...
        });
        {
            ...
            while { if !__for_first_269 { i = add(&i, &Value::Int(1)); } __for_first_269 = false; is_less_than(&i, &get_array_length(&rawTickers)) } {
        ...
        let mut messageHashes: Value = self.find_message_hashes(client.clone(), Value::Str("tickers::".to_string()));
        {
            ...
            while { if !__for_first_270 { i = add(&i, &Value::Int(1)); } __for_first_270 = false; is_less_than(&i, &get_array_length(&messageHashes)) } {
    ...
    pub fn handle_balance(&mut self, mut client: Value, mut message: Value) {
        ...
        if is_true(&isSwap) {
            account = Value::Str("swap".to_string());
            {
                ...
                while { if !__for_first_272 { i = add(&i, &Value::Int(1)); } __for_first_272 = false; is_less_than(&i, &get_array_length(&balances)) } {
                rawBalances = self.array_concat(rawBalances.clone(), balances.clone());
            }
            }
            info = rawBalances.clone();
        }
        {
            ...
            while { if !__for_first_273 { i = add(&i, &Value::Int(1)); } __for_first_273 = false; is_less_than(&i, &get_array_length(&rawBalances)) } {
 ...
 */
    ...
    pub fn handle_trades(&mut self, mut client: Value, mut message: Value) {
        ...
        }
        {
            ...
            while { if !__for_first_274 { i = add(&i, &Value::Int(1)); } __for_first_274 = false; is_less_than(&i, &get_array_length(&trades)) } {
    ...
    pub fn handle_deltas(&self, mut bookside: Value, mut deltas: Value) {
        {
            ...
            while { if !__for_first_278 { i = add(&i, &Value::Int(1)); } __for_first_278 = false; is_less_than(&i, &get_array_length(&deltas)) } {
