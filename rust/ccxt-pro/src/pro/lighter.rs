    ...
    pub fn handle_deltas(&self, mut bookside: Value, mut deltas: Value) {
        {
            ...
            while { if !__for_first_469 { i = add(&i, &Value::Int(1)); } __for_first_469 = false; is_less_than(&i, &get_array_length(&deltas)) } {
    ...
    pub fn handle_ticker(&mut self, mut client: Value, mut message: Value) {
        ...
        if is_equal(&channel, &Value::Str("market_stats:all".to_string())) {
            let mut marketIds: Value = object_keys(&data);
            {
                ...
                while { if !__for_first_470 { i = add(&i, &Value::Int(1)); } __for_first_470 = false; is_less_than(&i, &get_array_length(&marketIds)) } {
 ...
 */
    ...
    pub fn handle_trades(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut dataLength: Value = get_array_length(&data);
        {
            ...
            while { if !__for_first_472 { i = add(&i, &Value::Int(1)); } __for_first_472 = false; is_less_than(&i, &dataLength) } {
 ...
 */
    ...
    pub fn handle_my_trades(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut messageHash: Value = self.get_message_hash(Value::Str("myTrades".to_string()), &[]);
        {
            ...
            while { if !__for_first_474 { i = add(&i, &Value::Int(1)); } __for_first_474 = false; is_less_than(&i, &get_array_length(&marketIds)) } {
            let mut marketId: Value = get_value(&marketIds, &i);
            let mut marketId: Value = get_value(&marketIds, &i);
            let mut market: Value = self.safe_market(&[marketId.clone()]);
            ...
            let mut tradesLength: Value = get_array_length(&trades);
            {
                ...
                while { if !__for_first_473 { j = add(&j, &Value::Int(1)); } __for_first_473 = false; is_less_than(&j, &tradesLength) } {
 ...
 */
    ...
    pub fn handle_liquidation(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut dataLength: Value = get_array_length(&data);
        {
            ...
            while { if !__for_first_475 { i = add(&i, &Value::Int(1)); } __for_first_475 = false; is_less_than(&i, &dataLength) } {
            let mut iReversed: Value = subtract(&subtract(&dataLength, &Value::Int(1)), &i);
    ...
    pub fn handle_balance(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        if is_equal(&type_var, &Value::Str("spot".to_string())) {
            ...
            let mut assetIds: Value = object_keys(&assets);
            {
                ...
                while { if !__for_first_476 { i = add(&i, &Value::Int(1)); } __for_first_476 = false; is_less_than(&i, &get_array_length(&assetIds)) } {
 ...
 */
    ...
    pub fn handle_orders(&mut self, mut client: Value, mut message: Value) -> Value {
        ...
        let mut messageHash: Value = self.get_message_hash(Value::Str("orders".to_string()), &[]);
        {
            ...
            while { if !__for_first_478 { i = add(&i, &Value::Int(1)); } __for_first_478 = false; is_less_than(&i, &get_array_length(&marketIds)) } {
            let mut marketId: Value = get_value(&marketIds, &i);
            let mut marketId: Value = get_value(&marketIds, &i);
            let mut market: Value = self.safe_market(&[marketId.clone()]);
            let mut orders: Value = self.safe_list(data.clone(), marketId.clone(), &[Value::List(vec![])]);
            {
                ...
                while { if !__for_first_477 { j = add(&j, &Value::Int(1)); } __for_first_477 = false; is_less_than(&j, &get_array_length(&orders)) } {
...
if let Err(_try_err) = _try_result { let e: Value = panic_to_value(_try_err);
            ...
            if !is_equal(&id, &Value::Null) {
                let mut subscriptionKeys: Value = object_keys(&get_value(&client, &Value::Str("subscriptions".to_string())));
                {
                    ...
                    while { if !__for_first_479 { i = add(&i, &Value::Int(1)); } __for_first_479 = false; is_less_than(&i, &get_array_length(&subscriptionKeys)) } {
