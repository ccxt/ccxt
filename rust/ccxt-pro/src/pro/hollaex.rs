 ...
 */
    ...
    pub fn handle_trades(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut parsedTrades: Value = self.parse_trades(data.clone(), &[market.clone()]);
        {
            ...
            while { if !__for_first_383 { j = add(&j, &Value::Int(1)); } __for_first_383 = false; is_less_than(&j, &get_array_length(&parsedTrades)) } {
 ...
 */
    ...
    pub fn handle_my_trades(&mut self, mut client: Value, mut message: Value, optional_args: &[Value]) {
        ...
        });
        {
            ...
            while { if !__for_first_384 { i = add(&i, &Value::Int(1)); } __for_first_384 = false; is_less_than(&i, &get_array_length(&rawTrades)) } {
        ...
        let mut keys: Value = object_keys(&marketIds);
        {
            ...
            while { if !__for_first_385 { i = add(&i, &Value::Int(1)); } __for_first_385 = false; is_less_than(&i, &get_array_length(&keys)) } {
 ...
 */
    ...
    pub fn handle_order(&mut self, mut client: Value, mut message: Value, optional_args: &[Value]) {
        ...
        });
        {
            ...
            while { if !__for_first_386 { i = add(&i, &Value::Int(1)); } __for_first_386 = false; is_less_than(&i, &get_array_length(&rawOrders)) } {
