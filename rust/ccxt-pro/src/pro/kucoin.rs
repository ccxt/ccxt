 ...
 */
        ...
        if is_greater_than_or_equal(&get_index_of(&topic, &Value::Str("Depth".to_string())), &Value::Int(0)) {
            ...
            if is_equal(&nonce, &Value::Null) {
                ...
                let mut subscription: Value = Value::Null;
                {
                    ...
                    while { if !__for_first_464 { i = add(&i, &Value::Int(1)); } __for_first_464 = false; is_less_than(&i, &get_array_length(&subscriptions)) } {
    ...
    pub fn get_cache_index(&self, mut orderbook: Value, mut cache: Value) -> Value {
        ...
        if is_true(&(is_equal(&nonce, &Value::Null))) || is_true(&(is_equal(&firstDeltaStart, &Value::Null))) {
        ...
        }
        {
            ...
            while { if !__for_first_465 { i = add(&i, &Value::Int(1)); } __for_first_465 = false; is_less_than(&i, &get_array_length(&cache)) } {
    ...
    pub fn handle_bid_asks(&self, mut bookSide: Value, mut bidAsks: Value) {
        {
            ...
            while { if !__for_first_466 { i = add(&i, &Value::Int(1)); } __for_first_466 = false; is_less_than(&i, &get_array_length(&bidAsks)) } {
    ...
    m
        ...
        }  else {
            {
                ...
                while { if !__for_first_467 { i = add(&i, &Value::Int(1)); } __for_first_467 = false; is_less_than(&i, &get_array_length(&symbols)) } {
                let mut symbol: Value = get_value(&symbols, &i);
                let mut symbol: Value = get_value(&symbols, &i);
                { let __be_tmp = self.order_book(&[Value::Map({
    let mut m = indexmap::IndexMap::new();
    ...
    pub fn handle_subscription_status(&mut self, mut client: Value, mut message: Value) {
        ...
        if is_equal(&isUnSub, &Value::Bool(true)) {
            ...
            let mut subMessageHashes: Value = self.safe_list_k(subscription.clone(), "subMessageHashes", &[Value::List(vec![])]);
            {
                ...
                while { if !__for_first_468 { i = add(&i, &Value::Int(1)); } __for_first_468 = false; is_less_than(&i, &get_array_length(&messageHashes)) } {
 ...
 */
    ...
    pub async fn load_positions_snapshot(&mut self, mut client: Value, mut messageHash: Value, mut uta: Value) -> Value {
        ...
        let mut cache: Value = self.positions.clone();
        {
            ...
            while { if !__for_first_471 { i = add(&i, &Value::Int(1)); } __for_first_471 = false; is_less_than(&i, &get_array_length(&positions)) } {
