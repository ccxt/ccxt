 ...
 */
    ...
    pub fn handle_order_book(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut nonce: Value = Value::Null;
        {
            ...
            if (timestamp == Value::Null) {
                ...
                let mut newTimestampValue: Value = (if (newTimestamp == Value::Null) { Value::Int(0) } else { newTimestamp.clone() });
            ...
            if (nonce == Value::Null) {
                ...
                let mut newNonceValue: Value = (if (newNonce == Value::Null) { Value::Int(0) } else { newNonce.clone() });
    ...
    pub fn handle_subscription_status(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut subscription: Value = (if (id == Value::Null) { Value::Null } else { self.safe_dict(subscriptionsById.clone(), id.clone(), &[]) });
