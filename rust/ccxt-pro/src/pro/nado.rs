 ...
 */
    ...
    m
        ...
        let mut messageHashes: Value = Value::from(vec![]);
        {
            ...
            while { if !__for_first_489 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_489 = false; i.as_f64().unwrap_or(f64::NAN) < ((symbols.len() as i64) as f64) } {
            let mut market: Value = self.market(symbols.as_array().and_then(|__arr| match &i { Value::Int(__n) => __arr.get(*__n as usize), Value::Str(__s) => __s.parse::<usize>().ok().and_then(|__n| __arr.get(__n)), _ => None }).cloned().unwrap_or(Value::Null));
            append_to_array(&mut markets, market.clone());
            append_to_array(&mut messageHashes, Value::Str(format!("{}{}", Value::Str("trade:".into()), market.as_map().and_then(|__m| __m.get("symbol")).cloned().unwrap_or(Value::Null)).into()));
        }
 ...
 */
    ...
    m
        ...
        if is_true(&self.newUpdates) {
            limit = stored.get_limit(market.as_map().and_then(|__m| __m.get("symbol")).cloned().unwrap_or(Value::Null), limit.clone());
        }
        return self.filter_by_since_limit(stored, &[since, limit, Value::Int(0), Value::Bool(true)]);
 ...
 */
    ...
    m
        ...
        let mut subscriptionParams: Value = Value::from(vec![]);
        {
            ...
            while { if !__for_first_492 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_492 = false; i.as_f64().unwrap_or(f64::NAN) < ((symbolsAndTimeframes.len() as i64) as f64) } {
    ...
    m
        ...
        if is_true(&self.newUpdates) {
            if (messageHash.as_str() == Some("ticker")) {
                return self.filter_by_array(ticker.clone(), Value::Str("symbol".into()), &[symbols.clone()]);
            }
 ...
 */
    ...
    m
        ...
        let mut orders: Value = self.watch_private(Value::Str("order_update".into()), stream, messageHash, &[params]).await;
        if is_true(&self.newUpdates) {
 ...
 */
    ...
    m
        ...
        return self.un_watch_private(stream, messageHash, &[params]).await;
 ...
 */
    ...
    m
        ...
        let mut trades: Value = self.watch_private(Value::Str("fill".into()), stream, messageHash, &[params]).await;
        if is_true(&self.newUpdates) {
 ...
 */
    ...
    m
        ...
        let mut positions: Value = self.watch_private(Value::Str("position_change".into()), stream, messageHash, &[params]).await;
        if is_true(&self.newUpdates) {
            return positions;
        }
 ...
 */
    ...
    m
        ...
        let mut request: Value = self.parent.create_order_request(symbol, type_var, side, amount, &[price, params]).await;
 ...
 */
    ...
    m
        ...
        let mut request: Value = self.parent.edit_order_request(id, symbol, type_var, side, &[amount, price, params]).await;
 ...
 */
    ...
    m
        ...
        let mut request: Value = self.parent.cancel_orders_request(ids, &[symbol, params]).await;
        let mut response: Value = self.watch_execute_request(requestIdString, request).await;
        //
        //     {
        //         "status": "success",
        ...
        let mut result: Value = Value::from(vec![]);
        {
            ...
            while { if !__for_first_494 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_494 = false; i.as_f64().unwrap_or(f64::NAN) < ((cancelledOrders.len() as i64) as f64) } {
 ...
 */
    ...
    m
        ...
        let mut request: Value = self.parent.cancel_all_orders_request(&[symbol, params]).await;
        let mut response: Value = self.watch_execute_request(requestIdString, request).await;
        let mut data: Value = self.safe_dict_k(response, "data", &[Value::Map({
            let mut m = indexmap::IndexMap::new();
            m
    ...
    m
        ...
        let mut client: Value = self.client(&[url.clone()]);
        {
            ...
            while { if !__for_first_496 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_496 = false; i.as_f64().unwrap_or(f64::NAN) < ((messageHashes.len() as i64) as f64) } {
    ...
    m
        ...
        });
        {
            ...
            while { if !__for_first_498 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_498 = false; i.as_f64().unwrap_or(f64::NAN) < ((marketIds.len() as i64) as f64) } {
    ...
    pub fn handle_all_bids_asks(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut symbols: Value = object_keys(&tickers);
        {
            ...
            while { if !__for_first_499 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_499 = false; i.as_f64().unwrap_or(f64::NAN) < ((symbols.len() as i64) as f64) } {
            let mut symbol: Value = symbols.as_array().and_then(|__arr| match &i { Value::Int(__n) => __arr.get(*__n as usize), Value::Str(__s) => __s.parse::<usize>().ok().and_then(|__n| __arr.get(__n)), _ => None }).cloned().unwrap_or(Value::Null);
    ...
    pub fn handle_order_book(&mut self, mut client: Value, mut message: Value) {
        ...
        if (maxTimestamp != Value::Null) && (lastMaxTimestamp != Value::Null) && (maxTimestamp.as_str() != lastMaxTimestamp.as_str()) {
            let mut subscriptions: Value = object_keys(&get_value(&client, &Value::Str("subscriptions".into())));
            {
                ...
                while { if !__for_first_500 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_500 = false; i.as_f64().unwrap_or(f64::NAN) < ((subscriptions.len() as i64) as f64) } {
