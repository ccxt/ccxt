    ...
    pub fn handle_message(&mut self, mut client: Value, mut message: Value) {
        ...
        if (type_var.as_deref() == Some("DATA")) {
            ...
            let mut keys: Value = object_keys(&methods);
            {
                ...
                while { if !__for_first_289 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_289 = false; i.as_f64().unwrap_or(f64::NAN) < ((keys.len() as i64) as f64) } {
