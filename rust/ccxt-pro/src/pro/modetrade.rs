 ...
 */
        ...
        if is_true(&Value::Bool(matches!(&data, Value::Arr(_)))) {
            {
                ...
                let mut tradeId: Value = (if is_true(&(Value::Bool(tradeIdStr == Value::Null))) { Value::Null } else { self.omit_zero(tradeIdStr.clone()) });
    ...
    pub fn handle_order(&mut self, mut client: Value, mut message: Value, mut topic: Value) {
        ...
        if (symbol != Value::Null) {
            ...
            let mut order: Value = (if is_true(&(Value::Bool(orderId == Value::Null))) { Value::Null } else { self.safe_dict(orders.clone(), orderId.clone(), &[]) });
            if (order != Value::Null) {
