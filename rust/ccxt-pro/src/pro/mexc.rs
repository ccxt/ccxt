 ...
 */
    ...
    pub fn handle_tickers(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut result: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_483 { i = add(&i, &Value::Int(1)); } __for_first_483 = false; is_less_than(&i, &get_array_length(&data)) } {
 ...
 */
    ...
    m
        ...
        let mut topics: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_484 { i = add(&i, &Value::Int(1)); } __for_first_484 = false; is_less_than(&i, &get_array_length(&symbols)) } {
            if is_true(&isSpot) {
 ...
 */
    ...
    pub fn get_cache_index(&self, mut orderbook: Value, mut cache: Value) -> Value {
        ...
        }
        {
            ...
            while { if !__for_first_485 { i = add(&i, &Value::Int(1)); } __for_first_485 = false; is_less_than(&i, &get_array_length(&cache)) } {
 ...
 */
    ...
    pub fn handle_trades(&mut self, mut client: Value, mut message: Value) {
        ...
        }
        {
            ...
            while { if !__for_first_487 { j = add(&j, &Value::Int(1)); } __for_first_487 = false; is_less_than(&j, &get_array_length(&trades)) } {
 ...
 */
    ...
    m
        ...
        let mut topics: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_488 { i = add(&i, &Value::Int(1)); } __for_first_488 = false; is_less_than(&i, &get_array_length(&symbols)) } {
            if is_true(&isSpot) {
                let mut market: Value = self.market(get_value(&symbols, &i));
                append_to_array(&mut topics, add(&Value::Str("spot@public.aggre.bookTicker.v3.api.pb@100ms@".to_string()), &get_value(&market, &Value::Str("id".to_string()))));
            }
    ...
    pub fn handle_unsubscriptions(&mut self, mut client: Value, mut messageHashes: Value) {
        {
            ...
            while { if !__for_first_490 { i = add(&i, &Value::Int(1)); } __for_first_490 = false; is_less_than(&i, &get_array_length(&messageHashes)) } {
            ...
            if is_greater_than_or_equal(&get_index_of(&messageHash, &Value::Str("ticker".to_string())), &Value::Int(0)) {
                ...
                if is_greater_than_or_equal(&get_index_of(&symbol, &Value::Str("unsubscribe".to_string())), &Value::Int(0)) {
                    ...
                    let mut symbols: Value = object_keys(&self.tickers);
                    {
                        ...
                        while { if !__for_first_489 { j = add(&j, &Value::Int(1)); } __for_first_489 = false; is_less_than(&j, &get_array_length(&symbols)) } {
