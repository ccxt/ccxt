    ...
    pub fn handle_message(&mut self, mut client: Value, mut message: Value) {
        ...
        if !is_equal(&method, &Value::Null) {
        ...
        }  else {
            {
                ...
                while { if !__for_first_607 { i = add(&i, &Value::Int(1)); } __for_first_607 = false; is_less_than(&i, &get_array_length(&message)) } {
 ...
 */
    ...
    pub fn handle_trades(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut parsed: Value = self.parse_ws_trades(data.clone(), &[market.clone()]);
        {
            ...
            while { if !__for_first_609 { i = add(&i, &Value::Int(1)); } __for_first_609 = false; is_less_than(&i, &get_array_length(&parsed)) } {
 ...
 */
    ...
    m
        ...
        let mut selectedTimeframe: Value = Value::Null;
        {
            ...
            while { if !__for_first_610 { i = add(&i, &Value::Int(1)); } __for_first_610 = false; is_less_than(&i, &get_array_length(&symbolsAndTimeframes)) } {
 ...
 */
    ...
    m
        ...
        { let __be_tmp = self.iso8601(timestamp.clone()); add_element_to_object(get_value_mut(unsafe { crate::runtime::coerce_value_to_mut(&self.balance) }, &type_var), &Value::Str("datetime".to_string()), __be_tmp); };
        {
            ...
            while { if !__for_first_617 { i = add(&i, &Value::Int(1)); } __for_first_617 = false; is_less_than(&i, &get_array_length(&data)) } {
 ...
 */
    ...
    pub async fn load_positions_snapshot(&mut self, mut client: Value, mut messageHash: Value, mut type_var: Value) -> Value {
        ...
        let mut cache: Value = get_value(&self.positions, &type_var);
        {
            ...
            while { if !__for_first_618 { i = add(&i, &Value::Int(1)); } __for_first_618 = false; is_less_than(&i, &get_array_length(&positions)) } {
    ...
    pub fn handle_positions(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut newPositions: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_619 { i = add(&i, &Value::Int(1)); } __for_first_619 = false; is_less_than(&i, &get_array_length(&rawPositions)) } {
...
if let Err(_try_err) = _try_result { let error: Value = panic_to_value(_try_err);
            ...
            let mut messageHashes: Value = object_keys(&get_value(&client, &Value::Str("futures".to_string())));
            {
                ...
                while { if !__for_first_621 { i = add(&i, &Value::Int(1)); } __for_first_621 = false; is_less_than(&i, &get_array_length(&messageHashes)) } {
                let mut messageHash: Value = get_value(&messageHashes, &i);
                let mut messageHash: Value = get_value(&messageHashes, &i);
