    ...
    pub fn handle_message(&mut self, mut client: Value, mut message: Value) {
        ...
        if !is_equal(&method, &Value::Null) {
        ...
        }  else {
            {
                ...
                while { if !__for_first_613 { i = add(&i, &Value::Int(1)); } __for_first_613 = false; is_less_than(&i, &get_array_length(&message)) } {
 ...
 */
    ...
    pub fn handle_trades(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut parsed: Value = self.parse_ws_trades(data.clone(), &[market.clone()]);
        {
            ...
            while { if !__for_first_615 { i = add(&i, &Value::Int(1)); } __for_first_615 = false; is_less_than(&i, &get_array_length(&parsed)) } {
 ...
 */
    ...
    m
        ...
        let mut selectedTimeframe: Value = Value::Null;
        {
            ...
            while { if !__for_first_616 { i = add(&i, &Value::Int(1)); } __for_first_616 = false; is_less_than(&i, &get_array_length(&symbolsAndTimeframes)) } {
 ...
 */
    ...
    pub fn handle_tickers(&mut self, mut client: Value, mut message: Value) {
        ...
        });
        {
            ...
            while { if !__for_first_619 { i = add(&i, &Value::Int(1)); } __for_first_619 = false; is_less_than(&i, &get_array_length(&data)) } {
 ...
 */
    ...
    pub fn set_order_book_snapshot(&mut self, mut client: Value, mut message: Value, mut channel: Value) {
        ...
        }
        {
            ...
            while { if !__for_first_622 { i = add(&i, &Value::Int(1)); } __for_first_622 = false; is_less_than(&i, &length) } {
            let mut entry: Value = get_value(&data, &i);
            let mut entry: Value = get_value(&data, &i);
 ...
 */
    ...
    pub fn handle_positions(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut newPositions: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_625 { i = add(&i, &Value::Int(1)); } __for_first_625 = false; is_less_than(&i, &get_array_length(&rawPositions)) } {
...
if let Err(_try_err) = _try_result { let error: Value = panic_to_value(_try_err);
            ...
            let mut messageHashes: Value = object_keys(&get_value(&client, &Value::Str("futures".to_string())));
            {
                ...
                while { if !__for_first_627 { i = add(&i, &Value::Int(1)); } __for_first_627 = false; is_less_than(&i, &get_array_length(&messageHashes)) } {
                let mut messageHash: Value = get_value(&messageHashes, &i);
                let mut messageHash: Value = get_value(&messageHashes, &i);
