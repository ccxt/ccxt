...
impl ToobitCore {
    ...
    pub fn handle_incoming_pong(&self, mut client: Value, mut pongTimestamp: Value) {
        crate::set_value(&mut client, &Value::Str("lastPong".into()), pongTimestamp.clone());
}
 ...
 */
    ...
    m
        ...
        let mut selectedTimeframe: Value = Value::Null;
        {
            ...
            let mut unfiedTimeframe: Value = self.safe_string(data.clone(), Value::Int(1), &[Value::Str("1m".into())]);
 ...
 */
    ...
    pub fn handle_order_book_partial_snapshot(&mut self, mut client: Value, mut message: Value) {
        ...
        //     }
        //
        self.set_order_book_snapshot(client.clone(), message.clone(), Value::Str("depth".into()));
}
    ...
    m
            ...
            let mut orderbook: Value = get_value(&self.orderbooks, &symbol);
            let mut timestamp: Value = self.safe_integer_k(entry.clone(), "t", &[]);
            let mut snapshot: Value = self.parse_order_book(entry.clone(), symbol.clone(), &[timestamp.clone(), Value::Str("b".into()), Value::Str("a".into())]);
 ...
 */
    ...
    m
        ...
        if is_true(&(subscriptionHash == Value::Null)) || (in_op(&get_value(&client, &Value::Str("subscriptions".into())), &subscriptionHash)) {
            return;
        }
        let mut type_var: Value = (if is_true(&(marketType.as_str() == Some("spot"))) { Value::Str("spot".into()) } else { Value::Str("contract".into()) });
        ...
        if !(in_op(&get_value(&client, &Value::Str("futures".into())), &messageHash)) {
            ...
            self.spawn(&[Value::Str("load_balance_snapshot".into()).clone(), client.clone(), messageHash.clone(), marketType.clone()]);
        }
}
    ...
    pub fn handle_balance(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut type_var: Value = (if is_true(&(channel.as_deref() == Some("outboundContractAccountInfo"))) { Value::Str("contract".into()) } else { Value::Str("spot".into()) });
 ...
 */
    ...
    pub fn handle_order(&mut self, mut client: Value, mut message: Value) {
        ...
        messageHash = Value::Str(format!("{}{}", Value::Str("orders:".into()), self.safe_string_k(order.clone(), "symbol", &[])).into());
        client.resolve(&[orders.clone(), messageHash.clone()]);
}
 ...
 */
    ...
    pub fn set_positions_cache(&mut self, mut client: Value, mut type_var: Value, optional_args: &[Value]) {
        ...
        if is_equal(&fetchPositionsSnapshot, &Value::Bool(true)) {
            ...
            if !(in_op(&get_value(&client, &Value::Str("futures".into())), &messageHash)) {
                client.future(&[messageHash.clone()]);
                self.spawn(&[Value::Str("load_positions_snapshot".into()).clone(), client.clone(), messageHash.clone(), type_var.clone(), isPortfolioMargin.clone()]);
            }
        }  else {
    ...
    pub async fn load_positions_snapshot(&mut self, mut client: Value, mut messageHash: Value, mut type_var: Value) -> Value {
        ...
        if (in_op(&get_value(&client, &Value::Str("futures".into())), &messageHash)) {
            ...
            client.resolve(&[cache.clone(), add(&type_var, &Value::Str(":positions".into()))]);
        }
    ...
    pub fn handle_positions(&mut self, mut client: Value, mut message: Value) {
        ...
        client.resolve(&[newPositions.clone(), Value::Str(format!("{}{}", accountType, Value::Str(":positions".into())).into())]);
}
    ...
    m
        ...
        if (match (&(time), &(lastAuthenticatedTime)) { (Value::Int(x), Value::Int(y)) => Value::Int(x - y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 - *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x - *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x - y), _ => Value::Null }).as_f64().unwrap_or(f64::NAN) > delay.as_f64().unwrap_or(f64::NAN) {
            ...
            let _try_result = futures::FutureExt::catch_unwind(std::panic::AssertUnwindSafe(async {
                ...
                add_element_to_object(get_value_mut(&mut self.options, &Value::Str("ws".into())), &Value::Str("lastAuthenticatedTime".into()), time.clone());
...
if let Err(_try_err) = _try_result { let e: Value = panic_to_value(_try_err);
                ...
                let mut err = Value::from(crate::exchange_errors::authentication_error(format!("{}{}", Value::Str(format!("{}{}", self.id.clone(), Value::Str(" ".into())).into()), self.exception_message(e.clone(), &[]))));
    ...
    m
        ...
        let _try_result = futures::FutureExt::catch_unwind(std::panic::AssertUnwindSafe(async {
            let mut response: Value = self.parent.private_post_api_v1_user_data_stream(&[params.clone()]).await;
            { let __be_tmp = self.safe_string_k(response.clone(), "listenKey", &[]); add_element_to_object(get_value_mut(&mut self.options, &Value::Str("ws".into())), &Value::Str("listenKey".into()), __be_tmp); };
