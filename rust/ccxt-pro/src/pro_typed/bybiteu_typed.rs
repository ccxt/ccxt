...
//
// Typed wrapper for bybiteu.
//
// Owns a `Box<BybiteuCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::pro::bybiteu::BybiteuCore;
use crate::types::*;
...
pub struct Bybiteu {
    ...
    core: Box<BybiteuCore>,
}

impl Bybiteu {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BybiteuCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BybiteuCore::new(config.into_option())) }
    }

    pub fn from_core(core: BybiteuCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BybiteuCore {
        &mut self.core
    }
...
impl std::ops::Deref for Bybiteu {
    type Target = BybiteuCore;
...
impl crate::typed::TypedExchange for Bybiteu {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Bybiteu::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Bybiteu::markets(self) }
}
