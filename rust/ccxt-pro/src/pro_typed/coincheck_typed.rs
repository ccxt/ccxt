...
//
// Typed wrapper for coincheck.
//
// Owns a `Box<CoincheckCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::pro::coincheck::CoincheckCore;
use crate::types::*;
...
pub struct Coincheck {
    ...
    core: Box<CoincheckCore>,
}

impl Coincheck {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(CoincheckCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(CoincheckCore::new(config.into_option())) }
    }

    pub fn from_core(core: CoincheckCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut CoincheckCore {
        &mut self.core
    }
...
impl std::ops::Deref for Coincheck {
    type Target = CoincheckCore;
...
impl crate::typed::TypedExchange for Coincheck {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Coincheck::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Coincheck::markets(self) }
}
