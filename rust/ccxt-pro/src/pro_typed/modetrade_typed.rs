...
//
// Typed wrapper for modetrade.
//
// Owns a `Box<ModetradeCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::pro::modetrade::ModetradeCore;
use crate::types::*;
use crate::exchange_generated::ExchangeBase;
use crate::exchange::ExchangeRuntime;

pub struct Modetrade {
    ...
    core: Box<ModetradeCore>,
}

impl Modetrade {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(ModetradeCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(ModetradeCore::new(config.into_option())) }
    }

    pub fn from_core(core: ModetradeCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut ModetradeCore {
        &mut self.core
    }
...
impl std::ops::Deref for Modetrade {
    type Target = ModetradeCore;
...
impl crate::typed::TypedExchange for Modetrade {
