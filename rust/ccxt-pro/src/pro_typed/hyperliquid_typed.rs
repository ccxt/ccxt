...
//
// Typed wrapper for hyperliquid.
//
// Owns a `Box<HyperliquidCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::pro::hyperliquid::HyperliquidCore;
use crate::types::*;
...
pub struct Hyperliquid {
    ...
    core: Box<HyperliquidCore>,
}

impl Hyperliquid {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(HyperliquidCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(HyperliquidCore::new(config.into_option())) }
    }

    pub fn from_core(core: HyperliquidCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut HyperliquidCore {
        &mut self.core
    }
...
impl std::ops::Deref for Hyperliquid {
    type Target = HyperliquidCore;
...
impl crate::typed::TypedExchange for Hyperliquid {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Hyperliquid::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Hyperliquid::markets(self) }
}
