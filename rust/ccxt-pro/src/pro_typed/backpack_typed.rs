...
//
// Typed wrapper for backpack.
//
// Owns a `Box<BackpackCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::pro::backpack::BackpackCore;
use crate::types::*;
...
pub struct Backpack {
    ...
    core: Box<BackpackCore>,
}

impl Backpack {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BackpackCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BackpackCore::new(config.into_option())) }
    }

    pub fn from_core(core: BackpackCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BackpackCore {
        &mut self.core
    }
...
impl std::ops::Deref for Backpack {
    type Target = BackpackCore;
...
impl crate::typed::TypedExchange for Backpack {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Backpack::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Backpack::markets(self) }
}
