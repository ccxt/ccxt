...
//
// Typed wrapper for okx.
//
// Owns a `Box<OkxCore>` and exposes the unified CCXT API
...
impl Okx {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(OkxCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(OkxCore::new(config.into_option())) }
    }

    pub fn from_core(core: OkxCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut OkxCore {
        &mut self.core
    }
...
impl crate::typed::TypedExchange for Okx {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Okx::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Okx::markets(self) }
}
