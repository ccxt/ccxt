...
//
// Typed wrapper for hollaex.
//
// Owns a `Box<HollaexCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::pro::hollaex::HollaexCore;
use crate::types::*;
...
pub struct Hollaex {
    ...
    core: Box<HollaexCore>,
}

impl Hollaex {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(HollaexCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(HollaexCore::new(config.into_option())) }
    }

    pub fn from_core(core: HollaexCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut HollaexCore {
        &mut self.core
    }
...
impl std::ops::Deref for Hollaex {
    type Target = HollaexCore;
...
impl crate::typed::TypedExchange for Hollaex {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Hollaex::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Hollaex::markets(self) }
}
