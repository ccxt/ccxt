...
//
// Typed wrapper for bybit.
//
// Owns a `Box<BybitCore>` and exposes the unified CCXT API
...
impl Bybit {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BybitCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BybitCore::new(config.into_option())) }
    }

    pub fn from_core(core: BybitCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BybitCore {
        &mut self.core
    }
...
impl crate::typed::TypedExchange for Bybit {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Bybit::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Bybit::markets(self) }
}
