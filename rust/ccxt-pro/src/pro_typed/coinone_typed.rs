...
use crate::Params;
use crate::Config;
use crate::pro::coinone::CoinoneCore;
use crate::types::*;
...
pub struct Coinone {
    ...
    core: Box<CoinoneCore>,
}

impl Coinone {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(CoinoneCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(CoinoneCore::new(config.into_option())) }
    }
...
impl crate::typed::TypedExchange for Coinone {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Coinone::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Coinone::markets(self) }
}
