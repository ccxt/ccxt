...
//
// Typed wrapper for bitopro.
//
// Owns a `Box<BitoproCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::pro::bitopro::BitoproCore;
use crate::types::*;
...
pub struct Bitopro {
    ...
    core: Box<BitoproCore>,
}

impl Bitopro {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BitoproCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BitoproCore::new(config.into_option())) }
    }

    pub fn from_core(core: BitoproCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BitoproCore {
        &mut self.core
    }
...
impl std::ops::Deref for Bitopro {
    type Target = BitoproCore;
...
impl crate::typed::TypedExchange for Bitopro {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Bitopro::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Bitopro::markets(self) }
}
