...
//
// Typed wrapper for gemini.
//
// Owns a `Box<GeminiCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::pro::gemini::GeminiCore;
use crate::types::*;
...
impl Gemini {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(GeminiCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(GeminiCore::new(config.into_option())) }
    }

    pub fn from_core(core: GeminiCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut GeminiCore {
        &mut self.core
    }
...
impl std::ops::Deref for Gemini {
    type Target = GeminiCore;
...
impl crate::typed::TypedExchange for Gemini {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Gemini::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Gemini::markets(self) }
}
