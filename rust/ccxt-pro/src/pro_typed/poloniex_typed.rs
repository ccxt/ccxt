...
//
// Typed wrapper for poloniex.
//
// Owns a `Box<PoloniexCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::pro::poloniex::PoloniexCore;
use crate::types::*;
...
pub struct Poloniex {
    ...
    core: Box<PoloniexCore>,
}

impl Poloniex {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(PoloniexCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(PoloniexCore::new(config.into_option())) }
    }

    pub fn from_core(core: PoloniexCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut PoloniexCore {
        &mut self.core
    }
...
impl std::ops::Deref for Poloniex {
    type Target = PoloniexCore;
...
impl crate::typed::TypedExchange for Poloniex {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Poloniex::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Poloniex::markets(self) }
}
