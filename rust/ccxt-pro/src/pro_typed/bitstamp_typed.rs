...
//
// Typed wrapper for bitstamp.
//
// Owns a `Box<BitstampCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::pro::bitstamp::BitstampCore;
use crate::types::*;
...
pub struct Bitstamp {
    ...
    core: Box<BitstampCore>,
}

impl Bitstamp {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BitstampCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BitstampCore::new(config.into_option())) }
    }

    pub fn from_core(core: BitstampCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BitstampCore {
        &mut self.core
    }
...
impl std::ops::Deref for Bitstamp {
    type Target = BitstampCore;
...
impl crate::typed::TypedExchange for Bitstamp {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Bitstamp::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Bitstamp::markets(self) }
}
