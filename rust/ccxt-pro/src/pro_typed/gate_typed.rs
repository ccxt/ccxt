...
//
// Typed wrapper for gate.
//
// Owns a `Box<GateCore>` and exposes the unified CCXT API
...
impl Gate {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(GateCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(GateCore::new(config.into_option())) }
    }

    pub fn from_core(core: GateCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut GateCore {
        &mut self.core
    }
...
impl crate::typed::TypedExchange for Gate {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Gate::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Gate::markets(self) }
}
