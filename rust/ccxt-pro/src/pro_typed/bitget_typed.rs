...
//
// Typed wrapper for bitget.
//
// Owns a `Box<BitgetCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::pro::bitget::BitgetCore;
use crate::types::*;
...
impl Bitget {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BitgetCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BitgetCore::new(config.into_option())) }
    }

    pub fn from_core(core: BitgetCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BitgetCore {
        &mut self.core
    }
...
impl std::ops::Deref for Bitget {
    type Target = BitgetCore;
...
impl crate::typed::TypedExchange for Bitget {
