...
//
// Typed wrapper for bydfi.
//
// Owns a `Box<BydfiCore>` and exposes the unified CCXT API
...
impl Bydfi {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BydfiCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BydfiCore::new(config.into_option())) }
    }

    pub fn from_core(core: BydfiCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BydfiCore {
        &mut self.core
    }
...
impl std::ops::Deref for Bydfi {
    type Target = BydfiCore;
...
impl crate::typed::TypedExchange for Bydfi {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Bydfi::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Bydfi::markets(self) }
}
