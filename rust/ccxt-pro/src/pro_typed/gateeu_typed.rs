...
//
// Typed wrapper for gateeu.
//
// Owns a `Box<GateeuCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::pro::gateeu::GateeuCore;
use crate::types::*;
...
impl Gateeu {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(GateeuCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(GateeuCore::new(config.into_option())) }
    }

    pub fn from_core(core: GateeuCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut GateeuCore {
        &mut self.core
    }
...
impl std::ops::Deref for Gateeu {
    type Target = GateeuCore;
...
impl crate::typed::TypedExchange for Gateeu {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Gateeu::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Gateeu::markets(self) }
}
