...
//
// Typed wrapper for bingx.
//
// Owns a `Box<BingxCore>` and exposes the unified CCXT API
...
impl Bingx {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BingxCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BingxCore::new(config.into_option())) }
    }

    pub fn from_core(core: BingxCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BingxCore {
        &mut self.core
    }
...
impl std::ops::Deref for Bingx {
    type Target = BingxCore;
...
impl crate::typed::TypedExchange for Bingx {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Bingx::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Bingx::markets(self) }
}
