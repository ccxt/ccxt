...
//
// Typed wrapper for dydx.
//
// Owns a `Box<DydxCore>` and exposes the unified CCXT API
...
impl Dydx {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(DydxCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(DydxCore::new(config.into_option())) }
    }

    pub fn from_core(core: DydxCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut DydxCore {
        &mut self.core
    }
...
impl std::ops::Deref for Dydx {
    type Target = DydxCore;
...
impl crate::typed::TypedExchange for Dydx {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Dydx::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Dydx::markets(self) }
}
