...
//
// Typed wrapper for htx.
//
// Owns a `Box<HtxCore>` and exposes the unified CCXT API
...
impl Htx {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(HtxCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(HtxCore::new(config.into_option())) }
    }

    pub fn from_core(core: HtxCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut HtxCore {
        &mut self.core
    }
...
impl crate::typed::TypedExchange for Htx {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Htx::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Htx::markets(self) }
}
