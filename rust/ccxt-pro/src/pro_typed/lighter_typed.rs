...
//
// Typed wrapper for lighter.
//
// Owns a `Box<LighterCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::pro::lighter::LighterCore;
use crate::types::*;
...
pub struct Lighter {
    ...
    core: Box<LighterCore>,
}

impl Lighter {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(LighterCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(LighterCore::new(config.into_option())) }
    }

    pub fn from_core(core: LighterCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut LighterCore {
        &mut self.core
    }
...
impl std::ops::Deref for Lighter {
    type Target = LighterCore;
...
impl crate::typed::TypedExchange for Lighter {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Lighter::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Lighter::markets(self) }
}
