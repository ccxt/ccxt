...
//
// Typed wrapper for deribit.
//
// Owns a `Box<DeribitCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::pro::deribit::DeribitCore;
use crate::types::*;
...
pub struct Deribit {
    ...
    core: Box<DeribitCore>,
}

impl Deribit {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(DeribitCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(DeribitCore::new(config.into_option())) }
    }

    pub fn from_core(core: DeribitCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut DeribitCore {
        &mut self.core
    }
...
impl std::ops::Deref for Deribit {
    type Target = DeribitCore;
...
impl crate::typed::TypedExchange for Deribit {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Deribit::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Deribit::markets(self) }
}
