...
//
// Typed wrapper for bitfinex.
//
// Owns a `Box<BitfinexCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::pro::bitfinex::BitfinexCore;
use crate::types::*;
...
pub struct Bitfinex {
    ...
    core: Box<BitfinexCore>,
}

impl Bitfinex {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BitfinexCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BitfinexCore::new(config.into_option())) }
    }

    pub fn from_core(core: BitfinexCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BitfinexCore {
        &mut self.core
    }
...
impl std::ops::Deref for Bitfinex {
    type Target = BitfinexCore;
...
impl crate::typed::TypedExchange for Bitfinex {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Bitfinex::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Bitfinex::markets(self) }
}
