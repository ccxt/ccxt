...
//
// Typed wrapper for ndax.
//
// Owns a `Box<NdaxCore>` and exposes the unified CCXT API
...
impl Ndax {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(NdaxCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(NdaxCore::new(config.into_option())) }
    }

    pub fn from_core(core: NdaxCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut NdaxCore {
        &mut self.core
    }
...
impl std::ops::Deref for Ndax {
    type Target = NdaxCore;
...
impl crate::typed::TypedExchange for Ndax {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Ndax::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Ndax::markets(self) }
}
