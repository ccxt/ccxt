...
//
// Typed wrapper for bitmex.
//
// Owns a `Box<BitmexCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::pro::bitmex::BitmexCore;
use crate::types::*;
...
impl Bitmex {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BitmexCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BitmexCore::new(config.into_option())) }
    }

    pub fn from_core(core: BitmexCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BitmexCore {
        &mut self.core
    }
...
impl std::ops::Deref for Bitmex {
    type Target = BitmexCore;
...
impl crate::typed::TypedExchange for Bitmex {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Bitmex::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Bitmex::markets(self) }
}
