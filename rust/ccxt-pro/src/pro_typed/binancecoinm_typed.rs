...
//
// Typed wrapper for binancecoinm.
//
// Owns a `Box<BinancecoinmCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::pro::binancecoinm::BinancecoinmCore;
use crate::types::*;
...
pub struct Binancecoinm {
    ...
    core: Box<BinancecoinmCore>,
}

impl Binancecoinm {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(BinancecoinmCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(BinancecoinmCore::new(config.into_option())) }
    }

    pub fn from_core(core: BinancecoinmCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut BinancecoinmCore {
        &mut self.core
    }
...
impl std::ops::Deref for Binancecoinm {
    type Target = BinancecoinmCore;
...
impl crate::typed::TypedExchange for Binancecoinm {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Binancecoinm::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Binancecoinm::markets(self) }
}
