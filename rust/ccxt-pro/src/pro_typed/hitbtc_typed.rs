...
//
// Typed wrapper for hitbtc.
//
// Owns a `Box<HitbtcCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::pro::hitbtc::HitbtcCore;
use crate::types::*;
...
impl Hitbtc {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(HitbtcCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(HitbtcCore::new(config.into_option())) }
    }

    pub fn from_core(core: HitbtcCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut HitbtcCore {
        &mut self.core
    }
...
impl std::ops::Deref for Hitbtc {
    type Target = HitbtcCore;
...
impl crate::typed::TypedExchange for Hitbtc {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Hitbtc::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Hitbtc::markets(self) }
}
