...
pub trait PredictionBase: crate::exchange_generated::ExchangeBase {
    ...
    fn events_list(&self) -> Value {
        ...
        let mut keys: Value = object_keys(&self.pred().events);
        {
            ...
            let mut identity: Value = self.safe_string2(event.clone(), Value::Str("id".to_string()), Value::Str("event".to_string()), &[keys.as_array().and_then(|__arr| match &i { Value::Int(__n) => __arr.get(*__n as usize), Value::Str(__s) => __s.parse::<usize>().ok().and_then(|__n| __arr.get(__n)), _ => None }).cloned().unwrap_or(Value::Null)]);
    ...
    fn slug_to_outcome_symbol(&self, mut eventSlug: Value, mut marketSlug: Value, mut outcome: Value) -> Value {
        ...
        return Value::Str(format!("{}{}", Value::Str(format!("{}{}", self.slug_to_market_symbol(eventSlug.clone(), marketSlug.clone()), Value::Str(":".to_string()))), label));
...
// sendEvmTransaction dispatches to the exchange's signEvmTransaction override
    ...
    fn rlp_encode_list(&self, mut items: Value) -> Value {
        let mut concatenated: Value = Value::Str("".to_string());
        {
            ...
            concatenated = Value::Str(format!("{}{}", concatenated, items.as_array().and_then(|__arr| match &i { Value::Int(__n) => __arr.get(*__n as usize), Value::Str(__s) => __s.parse::<usize>().ok().and_then(|__n| __arr.get(__n)), _ => None }).cloned().unwrap_or(Value::Null)));
        }
        }
