    ...
    + Sized
{
    ...
    fn safe_list2(&self, mut dictionaryOrList: Value, mut key1: Value, mut key2: Value, optional_args: &[Value]) -> Value {
         ...
         */
        let mut value: Value = self.safe_value(dictionaryOrList.clone(), key1.clone(), &[]);
        if is_true(&(value != Value::Null)) && (is_array(&value)) {
            return value;
        }
        let mut value2: Value = self.safe_value(dictionaryOrList.clone(), key2.clone(), &[]);
        if is_true(&(value2 != Value::Null)) && (is_array(&value2)) {
            return value2;
        }
        return defaultValue;
 ...
 */
    ...
    m
                ...
                if (in_op(&market, &Value::Str("base".to_string()))) {
                ...
                if (in_op(&market, &Value::Str("quote".to_string()))) {
                    let mut currency: Value = self.safe_currency_structure(Value::Map({
                        let mut m = indexmap::IndexMap::new();
    ...
    fn safe_order(&self, mut order: Value, optional_args: &[Value]) -> Value {
        ...
        if shouldParseFees {
            ...
            if !parseFee && is_true(&(reducedLength.as_f64() == Some(0.0))) {
            ...
            if parseFee && is_true(&(reducedLength.as_f64() == Some(1.0))) {
...
// allowEmpty: false always returns string[] (throws on empty/undefined at runtime)
    ...
    m
        ...
        if (self.has.as_map().and_then(|__m| __m.get("fetchTradingLimits")).cloned().unwrap_or(Value::Null) != Value::Null) && !is_equal(&self.has.as_map().and_then(|__m| __m.get("fetchTradingLimits")).cloned().unwrap_or(Value::Null), &Value::Bool(false)) {
            if is_true(&reload) || !(in_op(&self.options, &Value::Str("limitsLoaded".to_string()))) {
...
match _try_result { Ok(__try_ok) => { if !matches!(__try_ok, Value::Null) { return __try_ok; } return Value::Null; } Err(_try_err) => { let e: Value = panic_to_value(_try_err); 
    ...
    m
        ...
        let mut skipZeroPrices: Value = self.safe_bool_k(options.clone(), "skipZeroPrices", &[Value::Bool(true)]);
        {
            ...
            if is_true(&(skipZeroPrices.as_bool() == Some(true))) && !is_true(&(price.as_f64().unwrap_or(f64::NAN) > Value::Int(0).as_f64().unwrap_or(f64::NAN))) && !is_true(&(price.as_f64().unwrap_or(f64::NAN) < Value::Int(0).as_f64().unwrap_or(f64::NAN))) {
                continue;
            }
 ...
 */
    ...
    fn check_required_argument(&self, mut methodName: Value, mut argument: Value, mut argumentName: Value, optional_args: &[Value]) {
        ...
        if is_true(&(argument == Value::Null)) || (is_true(&(optionsLength.as_f64().unwrap_or(f64::NAN) > Value::Int(0).as_f64().unwrap_or(f64::NAN))) && (!is_true(&(self.in_array(argument.clone(), options.clone()))))) {
...
// undefined here, so fetchPositions/fetchPositionsHistory legitimately pass a symbol list
    ...
    fn clean_cache(&mut self, mut subscription: Value) {
        ...
        if (topic.as_str() == Some("ohlcv")) {
            ...
            if (topic.as_str() == Some("myTrades")) && (!is_equal(&self.myTrades, &Value::Null)) {
                ...
                self.orders = Value::Null;
            }  else if (topic.as_str() == Some("positions")) && is_true(&(self.positions.clone() != Value::Null)) {
                self.positions = Value::Null;
                let mut clients: Value = object_values(&self.clients);
                {
                    ...
                    if is_true(&(futures != Value::Null)) && (in_op(&futures, &Value::Str("fetchPositionsSnapshot".to_string()))) {
                        remove(&mut futures, &Value::Str("fetchPositionsSnapshot".to_string()));
                    }
                }
                }
            }  else if is_true(&((topic.as_str() == Some("ticker")) || (topic.as_str() == Some("markPrice")))) && is_true(&(self.tickers.clone() != Value::Null)) {
            ...
            }  else if (topic.as_str() == Some("bidsasks")) && is_true(&(self.bidsasks.clone() != Value::Null)) {
    ...
    m
         ...
         */
        if is_true(&((self.has.as_map().and_then(|__m| __m.get("createMarketOrderWithCostWs")).cloned().unwrap_or(Value::Null) != Value::Null) && !is_equal(&self.has.as_map().and_then(|__m| __m.get("createMarketOrderWithCostWs")).cloned().unwrap_or(Value::Null), &Value::Bool(false)))) || (is_true(&((self.has.as_map().and_then(|__m| __m.get("createMarketBuyOrderWithCostWs")).cloned().unwrap_or(Value::Null) != Value::Null) && !is_equal(&self.has.as_map().and_then(|__m| __m.get("createMarketBuyOrderWithCostWs")).cloned().unwrap_or(Value::Null), &Value::Bool(false)))) && is_true(&((self.has.as_map().and_then(|__m| __m.get("createMarketSellOrderWithCostWs")).cloned().unwrap_or(Value::Null) != Value::Null) && !is_equal(&self.has.as_map().and_then(|__m| __m.get("createMarketSellOrderWithCostWs")).cloned().unwrap_or(Value::Null), &Value::Bool(false))))) {
 ...
 */
    ...
    m
         ...
         * @returns {object} an [order structure]{@link https://docs.ccxt.com/?id=order-structure}
         */
        if is_true(&((self.has.as_map().and_then(|__m| __m.get("createMarketOrderWithCost")).cloned().unwrap_or(Value::Null) != Value::Null) && !is_equal(&self.has.as_map().and_then(|__m| __m.get("createMarketOrderWithCost")).cloned().unwrap_or(Value::Null), &Value::Bool(false)))) || (is_true(&((self.has.as_map().and_then(|__m| __m.get("createMarketBuyOrderWithCost")).cloned().unwrap_or(Value::Null) != Value::Null) && !is_equal(&self.has.as_map().and_then(|__m| __m.get("createMarketBuyOrderWithCost")).cloned().unwrap_or(Value::Null), &Value::Bool(false)))) && is_true(&((self.has.as_map().and_then(|__m| __m.get("createMarketSellOrderWithCost")).cloned().unwrap_or(Value::Null) != Value::Null) && !is_equal(&self.has.as_map().and_then(|__m| __m.get("createMarketSellOrderWithCost")).cloned().unwrap_or(Value::Null), &Value::Bool(false))))) {
    ...
    m
         ...
         * @returns {object} an [order structure]{@link https://docs.ccxt.com/?id=order-structure}
         */
        if (is_equal(&self.options.as_map().and_then(|__m| __m.get("createMarketSellOrderRequiresPrice")).cloned().unwrap_or(Value::Null), &Value::Bool(true))) || is_true(&((self.has.as_map().and_then(|__m| __m.get("createMarketSellOrderWithCost")).cloned().unwrap_or(Value::Null) != Value::Null) && !is_equal(&self.has.as_map().and_then(|__m| __m.get("createMarketSellOrderWithCost")).cloned().unwrap_or(Value::Null), &Value::Bool(false)))) {
 ...
 */
    ...
    m
        ...
        if is_true(&((self.has.as_map().and_then(|__m| __m.get("fetchOpenOrders")).cloned().unwrap_or(Value::Null) != Value::Null) && !is_equal(&self.has.as_map().and_then(|__m| __m.get("fetchOpenOrders")).cloned().unwrap_or(Value::Null), &Value::Bool(false)))) && is_true(&((self.has.as_map().and_then(|__m| __m.get("fetchClosedOrders")).cloned().unwrap_or(Value::Null) != Value::Null) && !is_equal(&self.has.as_map().and_then(|__m| __m.get("fetchClosedOrders")).cloned().unwrap_or(Value::Null), &Value::Bool(false)))) {
