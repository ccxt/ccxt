 ...
 */
    ...
    m
            ...
            if is_true(&(rawMarketsLength.as_f64().unwrap_or(f64::NAN) < pageLimit.as_f64().unwrap_or(f64::NAN))) || is_true(&(page.as_f64().unwrap_or(f64::NAN) >= maxPages.as_f64().unwrap_or(f64::NAN))) || (is_true(&(total != Value::Null)) && is_true(&(fetchedRawCount.as_f64().unwrap_or(f64::NAN) >= total.as_f64().unwrap_or(f64::NAN)))) || (is_true(&(userLimit != Value::Null)) && is_true(&(collectedLength.as_f64().unwrap_or(f64::NAN) >= userLimit.as_f64().unwrap_or(f64::NAN)))) {
                break;
            }
        ...
        if is_true(&(userLimit != Value::Null)) && is_true(&(flatMarketsLength.as_f64().unwrap_or(f64::NAN) > userLimit.as_f64().unwrap_or(f64::NAN))) {
 ...
 */
    ...
    m
        ...
        let mut tradesLength: Value = Value::Int(trades.len() as i64);
        {
            ...
            if is_true(&(tokenId == Value::Null)) && is_true(&(marketId != Value::Null)) {
 ...
 */
    ...
    m
        ...
        let mut outcomesList: Value = (if is_true(&(outcomes == Value::Null)) { Value::List(vec![]) } else { outcomes.clone() });
        {
                        let mut i: Value = Value::Int(0);
 ...
 */
    ...
    pub fn handle_order_book(&self, mut client: Value, mut message: Value) {
        ...
        let mut bookSide: Value = (if is_true(&(sideStr.as_str() == Some("bids"))) { get_value(&orderbook, &Value::Str("bids".to_string())) } else { get_value(&orderbook, &Value::Str("asks".to_string())) });
 ...
 */
    ...
    pub fn handle_order(&mut self, mut client: Value, mut message: Value) {
        ...
        let mut side: Value = (if is_true(&(sideInt.as_f64() == Some(1.0))) { Value::Str("buy".to_string()) } else { Value::Str("sell".to_string()) });
        let mut tradingMethod: Value = self.safe_integer_k(message.clone(), "tradingMethod", &[]);
        let mut type_var: Value = (if is_true(&(tradingMethod.as_f64() == Some(1.0))) { Value::Str("market".to_string()) } else { Value::Str("limit".to_string()) });
