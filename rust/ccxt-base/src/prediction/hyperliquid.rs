 ...
 */
    ...
    m
        ...
        });
        {
            ...
            while { if !__for_first_1153 { i = add(&i, &Value::Int(1)); } __for_first_1153 = false; is_less_than(&i, &get_array_length(&parts)) } {
            ...
            if is_equal(&questionClass, &Value::Str("pricebucket".to_string())) {
                ...
                if is_true(&questionUnderlying) && is_true(&thresholdsRaw) && !is_equal(&indexStr, &Value::Null) {
                    ...
                    let mut thresholds: Value = Value::List(vec![]);
                    {
                        ...
                        while { if !__for_first_1154 { i = add(&i, &Value::Int(1)); } __for_first_1154 = false; is_less_than(&i, &get_array_length(&thresholdParts)) } {
 ...
 */
    ...
    m
        ...
        });
        {
            ...
            while { if !__for_first_1156 { qi = add(&qi, &Value::Int(1)); } __for_first_1156 = false; is_less_than(&qi, &get_array_length(&questionsList)) } {
    ...
    m
            ...
            let mut namedOutcomes: Value = self.safe_list_k(question.clone(), "namedOutcomes", &[Value::List(vec![])]);
            {
                ...
                while { if !__for_first_1155 { ni = add(&ni, &Value::Int(1)); } __for_first_1155 = false; is_less_than(&ni, &get_array_length(&namedOutcomes)) } {
        ...
        }
        {
            ...
            while { if !__for_first_1158 { i = add(&i, &Value::Int(1)); } __for_first_1158 = false; is_less_than(&i, &get_array_length(&outcomesList)) } {
    ...
    m
            ...
            let mut marketOutcomes: Value = self.safe_list_k(market.clone(), "outcomes", &[Value::List(vec![])]);
            {
                ...
                while { if !__for_first_1157 { oi = add(&oi, &Value::Int(1)); } __for_first_1157 = false; is_less_than(&oi, &get_array_length(&marketOutcomes)) } {
 ...
 */
    pub fn calculate_price_precision(&self, mut midPx: Value, mut szDecimals: Value) -> Value {
        ...
        let mut zeroCount: Value = subtract(&pricePrecisionDecimals, &Value::Int(1));
        {
            ...
            while { if !__for_first_1159 { zi = add(&zi, &Value::Int(1)); } __for_first_1159 = false; is_less_than(&zi, &zeroCount) } {
 ...
 */
    ...
    m
        ...
        if !is_equal(&outcomes, &Value::Null) {
            ...
            self.load_outcomes(&[outcomes.clone()]).await;
            {
                ...
                while { if !__for_first_1160 { i = add(&i, &Value::Int(1)); } __for_first_1160 = false; is_less_than(&i, &get_array_length(&outcomes)) } {
    ...
    m
        ...
        let mut outcomeHandles: Value = object_keys(&outcomesMap);
        {
            ...
            while { if !__for_first_1161 { i = add(&i, &Value::Int(1)); } __for_first_1161 = false; is_less_than(&i, &get_array_length(&outcomeHandles)) } {
 ...
 */
    ...
    m
        ...
        let mut asks: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_1162 { i = add(&i, &Value::Int(1)); } __for_first_1162 = false; is_less_than(&i, &get_array_length(&rawBids)) } {
        ...
        }
        {
            ...
            while { if !__for_first_1163 { i = add(&i, &Value::Int(1)); } __for_first_1163 = false; is_less_than(&i, &get_array_length(&rawAsks)) } {
 ...
 */
    ...
    m
        ...
        if !is_equal(&outcomes, &Value::Null) {
            ...
            self.load_outcomes(&[outcomes.clone()]).await;
            {
                ...
                while { if !__for_first_1165 { i = add(&i, &Value::Int(1)); } __for_first_1165 = false; is_less_than(&i, &get_array_length(&outcomes)) } {
                let mut requested: Value = get_value(&outcomes, &i);
                let mut requested: Value = get_value(&outcomes, &i);
                let mut requestedOutcomeObj: Value = self.safe_outcome(requested.clone(), &[]);
                let mut requestedOutcome: Value = self.safe_string_k(requestedOutcomeObj.clone(), "outcome", &[requested.clone()]);
    ...
    m
        ...
        let mut positions: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_1166 { i = add(&i, &Value::Int(1)); } __for_first_1166 = false; is_less_than(&i, &get_array_length(&balances)) } {
 ...
 */
    ...
    m
        ...
        }
        {
            ...
            while { if !__for_first_1168 { i = add(&i, &Value::Int(1)); } __for_first_1168 = false; is_less_than(&i, &get_array_length(&outcomesList)) } {
            let mut oc: Value = self.safe_dict(outcomesList.clone(), i.clone(), &[Value::Map({
    let mut m = indexmap::IndexMap::new();
    m
})]);
    ...
    pub fn resolve_outcome_input(&self, mut outcomeInput: Value) -> Value {
        ...
        let mut isNumericInput: Value = Value::Bool(is_greater_than(&inputCharsLength, &Value::Int(0)));
        {
            ...
            while { if !__for_first_1169 { di = add(&di, &Value::Int(1)); } __for_first_1169 = false; is_less_than(&di, &get_array_length(&inputChars)) } {
        ...
        }
        {
            ...
            while { if !__for_first_1170 { i = add(&i, &Value::Int(1)); } __for_first_1170 = false; is_less_than(&i, &get_array_length(&candidates)) } {
    ...
    m
        ...
        if !is_equal(&clientOrderId, &Value::Null) {
            ...
            add_element_to_object(&mut cancelAction, &Value::Str("type".to_string()), Value::Str("cancelByCloid".to_string()));
            {
                ...
                while { if !__for_first_1171 { i = add(&i, &Value::Int(1)); } __for_first_1171 = false; is_less_than(&i, &get_array_length(&cloids)) } {
            ...
            add_element_to_object(&mut cancelAction, &Value::Str("type".to_string()), Value::Str("cancel".to_string()));
            {
                ...
                while { if !__for_first_1172 { i = add(&i, &Value::Int(1)); } __for_first_1172 = false; is_less_than(&i, &get_array_length(&ids)) } {
                append_to_array(&mut cancelReq, Value::Map({
                    let mut m = indexmap::IndexMap::new();
                        m.insert("a".to_string(), assetId.clone());
        ...
        let mut orders: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_1173 { i = add(&i, &Value::Int(1)); } __for_first_1173 = false; is_less_than(&i, &get_array_length(&statuses)) } {
 ...
 */
    ...
    m
        ...
        }
        {
            ...
            while { if !__for_first_1174 { i = add(&i, &Value::Int(1)); } __for_first_1174 = false; is_less_than(&i, &get_array_length(&rawOrders)) } {
 ...
 */
    ...
    m
        ...
        }
        {
            ...
            while { if !__for_first_1175 { i = add(&i, &Value::Int(1)); } __for_first_1175 = false; is_less_than(&i, &get_array_length(&historicalOrders)) } {
 ...
 */
    ...
    m
        ...
        let mut lowerQueriesLength: Value = get_array_length(&lowerQueries);
        {
            ...
            while { if !__for_first_1179 { i = add(&i, &Value::Int(1)); } __for_first_1179 = false; is_less_than(&i, &get_array_length(&marketValues)) } {
    ...
    m
            ...
            if is_greater_than(&lowerQueriesLength, &Value::Int(0)) {
                ...
                let mut matches: Value = Value::Bool(false);
                {
                    ...
                    while { if !__for_first_1178 { qi = add(&qi, &Value::Int(1)); } __for_first_1178 = false; is_less_than(&qi, &get_array_length(&lowerQueries)) } {
                    ...
                    let mut allWords: Value = Value::Bool(true);
                    {
                        ...
                        while { if !__for_first_1177 { wi = add(&wi, &Value::Int(1)); } __for_first_1177 = false; is_less_than(&wi, &wordsLength) } {
        ...
        let mut groupKeys: Value = object_keys(&groupMap);
        {
            ...
            while { if !__for_first_1180 { gi = add(&gi, &Value::Int(1)); } __for_first_1180 = false; is_less_than(&gi, &get_array_length(&groupKeys)) } {
...
if let Err(_try_err) = _try_result { let e: Value = panic_to_value(_try_err);
    ...
    m
        ...
        let mut statuses: Value = self.safe_list_k(data.clone(), "statuses", &[Value::List(vec![])]);
        {
            ...
            while { if !__for_first_1181 { i = add(&i, &Value::Int(1)); } __for_first_1181 = false; is_less_than(&i, &get_array_length(&statuses)) } {
