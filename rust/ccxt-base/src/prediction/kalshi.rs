 ...
 */
    ...
    m
            ...
            if is_true(&((cursor == Value::Null) || (cursor.as_str() == Some("")))) || rawMarketsLength.as_f64().unwrap_or(f64::NAN) < limit.as_f64().unwrap_or(f64::NAN) || collectedLength.as_f64().unwrap_or(f64::NAN) >= maxMarkets.as_f64().unwrap_or(f64::NAN) {
                break;
            }
        }
 ...
 */
    ...
    pub fn parse_market(&self, mut raw: Value) -> Value {
        ...
        let mut active: Value = Value::Bool(is_true(&(status.as_str() == Some("active"))) || is_true(&(status.as_str() == Some("open"))));
        ...
        let mut resolved: Value = Value::Bool(is_true(&(status.as_str() == Some("settled"))) || (is_true(&(result != Value::Null)) && is_true(&(result.as_str() != Some("")))));
 ...
 */
    ...
    m
            ...
            if is_true(&(openPrice != Value::Null)) || is_true(&(previousPrice != Value::Null)) {
 ...
 */
    pub fn parse_my_trade(&self, mut fill: Value, optional_args: &[Value]) -> Value {
        ...
        if is_true(&(sideLeg.as_str() == Some("no"))) && is_true(&(ticker != Value::Null)) {
        ...
        let mut side: Value = (if is_true(&(action.as_str() == Some("sell"))) { Value::Str("sell".to_string()) } else { Value::Str("buy".to_string()) });
 ...
 */
    ...
    m
        ...
        let mut result: Value = Value::List(vec![]);
        {
            ...
            if is_true(&(status.as_str() == Some("closed"))) || is_true(&(status.as_str() == Some("canceled"))) {
 ...
 */
    ...
    m
        ...
        if is_true(&(requestedStatus.as_str() == Some("active"))) || is_true(&(requestedStatus.as_str() == Some("open"))) {
            status = Value::Str("open".to_string());
        }  else if is_true(&(requestedStatus.as_str() == Some("closed"))) || is_true(&(requestedStatus.as_str() == Some("inactive"))) {
            status = Value::Str("closed".to_string());
        }  else if is_true(&(requestedStatus.as_str() == Some("settled"))) || is_true(&(requestedStatus.as_str() == Some("resolved"))) {
        ...
        let mut result: Value = Value::List(vec![]);
        {
            ...
            let mut parsedMarkets: Value = (if is_true(&(parsedMarketsRaw != Value::Null)) { parsedMarketsRaw.clone() } else { Value::List(vec![]) });
 ...
 */
    ...
    m
        ...
        let mut collectedLength: Value = Value::Int(collected.len() as i64);
        {
            ...
            if is_true(&(st != Value::Null)) && is_true(&(st.as_str() != Some(""))) && is_true(&(already == Value::Null)) {
    ...
    m
        ...
        let mut maxPages: Value = self.safe_integer_k(self.options.clone(), "maxEventPagesPerSeries", &[Value::Int(20)]);
        {
            ...
            let mut cursor: Value = Value::Null;
            {
                ...
                if is_true(&(limit != Value::Null)) && is_true(&(collectedAfterPage.as_f64().unwrap_or(f64::NAN) >= limit.as_f64().unwrap_or(f64::NAN))) {
                    break;
                }
                if is_true(&(cursor == Value::Null)) || is_true(&(cursor.as_str() == Some(""))) || is_true(&(pageEventsLength.as_f64().unwrap_or(f64::NAN) < reqLimit.as_f64().unwrap_or(f64::NAN))) {
                    break;
                }
            }
            }
 ...
 */
    pub fn parse_event(&self, mut rawEvent: Value) -> Value {
        ...
        let mut latestClose: Value = Value::Null;
        {
            ...
            let mut marketResolved: bool = is_true(&(marketStatus.as_str() == Some("settled"))) || (is_true(&(marketResult != Value::Null)) && is_true(&(marketResult.as_str() != Some(""))));
            if !marketResolved {
        ...
        if is_true(&(resolved == Value::Null)) && is_true(&(marketsCount.as_f64().unwrap_or(f64::NAN) > Value::Int(0).as_f64().unwrap_or(f64::NAN))) {
