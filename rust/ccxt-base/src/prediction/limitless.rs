 ...
 */
    ...
    m
        ...
        if is_greater_than(&queriesLength, &Value::Int(0)) {
            ...
            });
            {
                ...
                while { if !__for_first_1255 { i = add(&i, &Value::Int(1)); } __for_first_1255 = false; is_less_than(&i, &get_array_length(&queries)) } {
                ...
                let mut found: Value = self.safe_list_k(response.clone(), "markets", &[Value::List(vec![])]);
                {
                    ...
                    while { if !__for_first_1254 { j = add(&j, &Value::Int(1)); } __for_first_1254 = false; is_less_than(&j, &get_array_length(&found)) } {
                    let mut raw: Value = get_value(&found, &j);
                    let mut raw: Value = get_value(&found, &j);
                    let mut slug: Value = self.safe_string_k(raw.clone(), "slug", &[]);
                    if is_true(&(!is_equal(&slug, &Value::Null) && !is_equal(&slug, &Value::Str("".to_string())))) && !is_true(&(Value::Bool(in_op(&seen, &slug)))) {
            ...
            let mut totalPages: Value = crate::runtime::Math::min(&allPages, &cappedPages);
            {
                ...
                while { if !__for_first_1256 { i = add(&i, &Value::Int(1)); } __for_first_1256 = false; is_less_than_or_equal(&i, &totalPages) } {
                page = i.clone();
            ...
            let mut length: Value = get_array_length(&responses);
            {
                ...
                while { if !__for_first_1257 { j = add(&j, &Value::Int(1)); } __for_first_1257 = false; is_less_than(&j, &length) } {
            ...
            if is_greater_than_or_equal(&lastPageLength, &pageSize) && is_less_than(&allRawLength, &maxMarkets) {
                while is_true(&Value::Bool(true)) {
                    ...
                    if is_equal(&pageMarketsLength, &Value::Int(0)) {
                        break;
                    }
                    {
                                                let mut i: Value = Value::Int(0);
                        let mut __for_first_1258: bool = true;
                        while { if !__for_first_1258 { i = add(&i, &Value::Int(1)); } __for_first_1258 = false; is_less_than(&i, &get_array_length(&page_markets)) } {
        ...
        let mut expandedRaw: Value = self.expand_group_rows(allRaw.clone());
        {
            ...
            while { if !__for_first_1259 { i = add(&i, &Value::Int(1)); } __for_first_1259 = false; is_less_than(&i, &get_array_length(&expandedRaw)) } {
            ...
            let mut eventKey: Value = ternary(is_true(&(!is_equal(&groupId, &Value::Null) && !is_equal(&groupId, &Value::Str("".to_string())))), self.shorten_slug(groupId.clone()), Value::Null);
    ...
    m
        ...
        let mut eventKeys: Value = object_keys(&eventGroups);
        {
            ...
            while { if !__for_first_1260 { i = add(&i, &Value::Int(1)); } __for_first_1260 = false; is_less_than(&i, &get_array_length(&eventKeys)) } {
    ...
    pub fn parse_market(&self, mut raw: Value) -> Value {
        ...
        let mut active: Value = Value::Bool(is_true(&(!is_equal(&isExpired, &Value::Bool(true)))) && is_true(&(is_equal(&marketStatus, &Value::Str("FUNDED".to_string())))));
    ...
    m
        ...
        let mut tokenEntries: Value = object_keys(&tokens);
        {
            ...
            while { if !__for_first_1261 { i = add(&i, &Value::Int(1)); } __for_first_1261 = false; is_less_than(&i, &get_array_length(&tokenEntries)) } {
 ...
 */
    pub fn expand_group_rows(&self, mut rawRows: Value) -> Value {
        let mut result: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_1263 { i = add(&i, &Value::Int(1)); } __for_first_1263 = false; is_less_than(&i, &get_array_length(&rawRows)) } {
            ...
            if is_true(&(is_equal(&rowType, &Value::Str("group".to_string())))) && is_true(&(!is_equal(&nestedMarkets, &Value::Null))) {
                ...
                let mut nestedMarketsLength: Value = get_array_length(&nestedMarkets);
                {
                    ...
                    while { if !__for_first_1262 { j = add(&j, &Value::Int(1)); } __for_first_1262 = false; is_less_than(&j, &nestedMarketsLength) } {
 ...
 */
    ...
    m
        ...
        let mut slugs: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_1265 { i = add(&i, &Value::Int(1)); } __for_first_1265 = false; is_less_than(&i, &get_array_length(&outcomes)) } {
            let mut outcomeObj: Value = self.outcome(get_value(&outcomes, &i));
            let mut slug: Value = self.safe_string(get_value(&outcomeObj, &Value::Str("info".to_string())), Value::Str("slug".to_string()), &[]);
            if is_equal(&slug, &Value::Null) {
                panic!("{}", crate::exchange_errors::exchange_error(add(&self.id, &Value::Str(" fetchTickers() missing slug".to_string()))));
            }
        ...
        let mut promises: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_1266 { i = add(&i, &Value::Int(1)); } __for_first_1266 = false; is_less_than(&i, &get_array_length(&slugs)) } {
        ...
        let mut responses: Value = promise_all(&promises).await;
        {
            ...
            while { if !__for_first_1268 { i = add(&i, &Value::Int(1)); } __for_first_1268 = false; is_less_than(&i, &get_array_length(&slugs)) } {
            let mut slug: Value = get_value(&slugs, &i);
            let mut slug: Value = get_value(&slugs, &i);
            ...
            let mut grouped: Value = get_value(&outcomesBySlug, &slug);
            {
                ...
                while { if !__for_first_1267 { j = add(&j, &Value::Int(1)); } __for_first_1267 = false; is_less_than(&j, &get_array_length(&grouped)) } {
 ...
 */
    ...
    m
        ...
        let mut filtered: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_1269 { i = add(&i, &Value::Int(1)); } __for_first_1269 = false; is_less_than(&i, &get_array_length(&rows)) } {
 ...
 */
    ...
    m
        ...
        let mut asks: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_1270 { bi = add(&bi, &Value::Int(1)); } __for_first_1270 = false; is_less_than(&bi, &get_array_length(&bidsSource)) } {
        ...
        }
        {
            ...
            while { if !__for_first_1271 { ai = add(&ai, &Value::Int(1)); } __for_first_1271 = false; is_less_than(&ai, &get_array_length(&asksSource)) } {
 ...
 */
    ...
    m
            ...
            if !is_equal(&firstPrices, &Value::Null) {
                let mut selectedSeries: Value = first.clone();
                {
                    ...
                    while { if !__for_first_1272 { i = add(&i, &Value::Int(1)); } __for_first_1272 = false; is_less_than(&i, &get_array_length(&rawHistory)) } {
    ...
    m
                    ...
                    if is_equal(&title, &Value::Null) {
                        panic!("{}", crate::exchange_errors::exchange_error(add(&self.id, &Value::Str(" fetchOHLCV() missing title".to_string()))));
                    }
        ...
        let mut pseudoTrades: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_1273 { i = add(&i, &Value::Int(1)); } __for_first_1273 = false; is_less_than(&i, &get_array_length(&history)) } {
            ...
            if is_equal(&pointTs, &Value::Null) {
                ...
                pointTs = ternary(is_true(&(!is_equal(&tsString, &Value::Null) && !is_equal(&tsString, &Value::Str("".to_string())))), self.parse8601(tsString.clone()), Value::Null);
        ...
        let mut bucketOrder: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_1274 { i = add(&i, &Value::Int(1)); } __for_first_1274 = false; is_less_than(&i, &get_array_length(&sorted)) } {
            ...
            if is_equal(&pTs, &Value::Null) {
                panic!("{}", crate::exchange_errors::exchange_error(add(&self.id, &Value::Str(" method() missing pTs".to_string()))));
            }
            ...
            if !is_true(&(Value::Bool(in_op(&candles, &key)))) {
                ...
                let mut candle: Value = get_value(&candles, &key);
                let mut pPriceOrZero: Value = ternary(is_true(&(is_equal(&pPrice, &Value::Null))), Value::Int(0), pPrice.clone());
                ...
                let mut candleLow: Value = ternary(is_true(&(is_equal(&get_value(&candle, &Value::Int(3)), &Value::Null))), pPrice.clone(), get_value(&candle, &Value::Int(3)));
                let mut pPriceOrCandleLow: Value = ternary(is_true(&(is_equal(&pPrice, &Value::Null))), get_value(&candle, &Value::Int(3)), pPrice.clone());
        ...
        let mut result: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_1275 { i = add(&i, &Value::Int(1)); } __for_first_1275 = false; is_less_than(&i, &get_array_length(&bucketOrder)) } {
 ...
 */
    ...
    m
        ...
        let mut items: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_1276 { i = add(&i, &Value::Int(1)); } __for_first_1276 = false; is_less_than(&i, &length) } {
        ...
        let mut found: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_1277 { i = add(&i, &Value::Int(1)); } __for_first_1277 = false; is_less_than(&i, &get_array_length(&results)) } {
 ...
 */
    ...
    m
        ...
        let mut maker: Value = ternary(is_true(&(!is_equal(&self.walletAddress, &Value::Str("".to_string())))), self.walletAddress.clone(), walletFromAccount.clone());
...
if let Err(_try_err) = _try_result { let e: Value = panic_to_value(_try_err);
    ...
    pub fn sign_evm_transaction(&self, mut tx: Value, mut privateKey: Value) -> Value {
        ...
        let mut signedFields: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_1278 { i = add(&i, &Value::Int(1)); } __for_first_1278 = false; is_less_than(&i, &get_array_length(&fields)) } {
 ...
 */
    ...
    m
        ...
        let mut trades: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_1279 { i = add(&i, &Value::Int(1)); } __for_first_1279 = false; is_less_than(&i, &get_array_length(&data)) } {
 ...
 */
    ...
    m
        ...
        if is_equal(&rawSide, &Value::Null) {
            panic!("{}", crate::exchange_errors::exchange_error(add(&self.id, &Value::Str(" parsePredictionTrade() missing rawSide".to_string()))));
        }
        ...
        if is_greater_than_or_equal(&get_index_of(&rawSide, &Value::Str("limit".to_string())), &Value::Int(0)) {
            ...
            if is_equal(&rawSide, &Value::Null) {
                panic!("{}", crate::exchange_errors::exchange_error(add(&self.id, &Value::Str(" method() missing rawSide".to_string()))));
            }
    ...
    pub fn get_outcome_by_slug_and_label(&self, mut slug: Value, mut label: Value, optional_args: &[Value]) -> Value {
        ...
        let mut outcomes: Value = self.safe_list_k(mkt.clone(), "outcomes", &[Value::List(vec![])]);
        {
            ...
            while { if !__for_first_1280 { i = add(&i, &Value::Int(1)); } __for_first_1280 = false; is_less_than(&i, &get_array_length(&outcomes)) } {
 ...
 */
    ...
    m
        ...
        let mut labels: Value = Value::List(vec![Value::Str("yes".to_string()), Value::Str("no".to_string())]);
        {
            ...
            while { if !__for_first_1282 { i = add(&i, &Value::Int(1)); } __for_first_1282 = false; is_less_than(&i, &get_array_length(&clob)) } {
            let mut entry: Value = self.safe_dict(clob.clone(), i.clone(), &[]);
            {
                                let mut j: Value = Value::Int(0);
                let mut __for_first_1281: bool = true;
                while { if !__for_first_1281 { j = add(&j, &Value::Int(1)); } __for_first_1281 = false; is_less_than(&j, &get_array_length(&labels)) } {
 ...
 */
    ...
    m
        ...
        if is_greater_than(&queriesLength, &Value::Int(0)) {
            ...
            });
            {
                ...
                while { if !__for_first_1284 { i = add(&i, &Value::Int(1)); } __for_first_1284 = false; is_less_than(&i, &get_array_length(&queries)) } {
                if is_equal(&queries, &Value::Null) {
                    panic!("{}", crate::exchange_errors::exchange_error(add(&self.id, &Value::Str(" fetchEvents() missing queries".to_string()))));
                }
                let mut q: Value = get_value(&queries, &i);
                ...
                let mut found: Value = self.safe_list_k(response.clone(), "markets", &[Value::List(vec![])]);
                {
                    ...
                    while { if !__for_first_1283 { j = add(&j, &Value::Int(1)); } __for_first_1283 = false; is_less_than(&j, &get_array_length(&found)) } {
                    let mut raw: Value = get_value(&found, &j);
                    let mut raw: Value = get_value(&found, &j);
                    let mut rawSlug: Value = self.safe_string_k(raw.clone(), "slug", &[]);
                    if is_true(&(!is_equal(&rawSlug, &Value::Null) && !is_equal(&rawSlug, &Value::Str("".to_string())))) && !is_true(&(Value::Bool(in_op(&seen, &rawSlug)))) {
            ...
            let mut listRawLength: Value = get_array_length(&listRaw);
            {
                ...
                while { if !__for_first_1285 { i = add(&i, &Value::Int(1)); } __for_first_1285 = false; is_less_than(&i, &listRawLength) } {
        ...
        let mut rawMarketsLength: Value = get_array_length(&expandedMarkets);
        {
            ...
            while { if !__for_first_1286 { i = add(&i, &Value::Int(1)); } __for_first_1286 = false; is_less_than(&i, &rawMarketsLength) } {
    ...
    m
        ...
        let mut eventKeysLength: Value = get_array_length(&eventKeys);
        {
            ...
            while { if !__for_first_1287 { i = add(&i, &Value::Int(1)); } __for_first_1287 = false; is_less_than(&i, &eventKeysLength) } {
    ...
    m
        ...
        while is_true(&Value::Bool(true)) {
            ...
            }
            {
                ...
                while { if !__for_first_1288 { i = add(&i, &Value::Int(1)); } __for_first_1288 = false; is_less_than(&i, &dataLength) } {
 ...
 */
    ...
    m
        ...
        let mut wanted: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_1289 { i = add(&i, &Value::Int(1)); } __for_first_1289 = false; is_less_than(&i, &get_array_length(&tags)) } {
        ...
        let mut categoriesLength: Value = get_array_length(&categories);
        {
            ...
            while { if !__for_first_1291 { i = add(&i, &Value::Int(1)); } __for_first_1291 = false; is_less_than(&i, &categoriesLength) } {
            ...
            let mut matched: bool = false;
            {
                ...
                while { if !__for_first_1290 { wi = add(&wi, &Value::Int(1)); } __for_first_1290 = false; is_less_than(&wi, &get_array_length(&wanted)) } {
        ...
        let mut allRaw: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_1293 { ci = add(&ci, &Value::Int(1)); } __for_first_1293 = false; is_less_than(&ci, &categoryIdsLength) } {
            ...
            let mut categoryMarketsLength: Value = get_array_length(&categoryMarkets);
            {
                ...
                while { if !__for_first_1292 { mi = add(&mi, &Value::Int(1)); } __for_first_1292 = false; is_less_than(&mi, &categoryMarketsLength) } {
 ...
 */
    ...
    m
        ...
        if is_equal(&access, &Value::Str("private".to_string())) {
            ...
            if is_equal(&method, &Value::Str("POST".to_string())) && is_true(&(!is_equal(&querystring, &Value::Str("".to_string())))) {
