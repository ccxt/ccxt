 ...
 */
    ...
    pub fn parse_cancel_orders(&self, mut data: Value) -> Value {
        ...
        let mut orders: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_378 { i = add(&i, &Value::Int(1)); } __for_first_378 = false; is_less_than(&i, &get_array_length(&dataKeys)) } {
            ...
            let mut orderIds: Value = get_value(&data, &marketId);
            {
                ...
                while { if !__for_first_377 { j = add(&j, &Value::Int(1)); } __for_first_377 = false; is_less_than(&j, &get_array_length(&orderIds)) } {
