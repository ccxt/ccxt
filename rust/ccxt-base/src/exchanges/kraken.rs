 ...
 */
    ...
    m
        ...
        return Value::Map({
    let mut m = indexmap::IndexMap::new();
        m.insert("status".to_string(), (if is_true(&(statusRaw.as_str() == Some("online"))) { Value::Str("ok".to_string()) } else { Value::Str("maintenance".to_string()) }));
 ...
 */
    ...
    pub fn parse_trade(&self, mut trade: Value, optional_args: &[Value]) -> Value {
        ...
        if (is_array(&trade)) {
            ...
            id = trade.clone();
        }  else if (in_op(&trade, &Value::Str("ordertxid".to_string()))) {
            ...
            if (in_op(&trade, &Value::Str("fee".to_string()))) {
                let mut currency: Value = Value::Null;
 ...
 */
    ...
    m
        ...
        if (market != Value::Null) {
            symbol = market.as_map().and_then(|__m| __m.get("symbol")).cloned().unwrap_or(Value::Null);
            if (in_op(&order, &Value::Str("fee".to_string()))) {
    ...
    m
        ...
        let mut isViqcOrder: bool = is_true(&(flags != Value::Null)) && is_true(&(get_index_of(&flags, &Value::Str("viqc".to_string())).as_f64().unwrap_or(f64::NAN) > Value::Int(-1).as_f64().unwrap_or(f64::NAN))); // volume in quote currency
    ...
    m
        ...
        if is_true(&(flags != Value::Null)) && !(in_op(&request, &Value::Str("oflags".to_string()))) {
 ...
 */
    ...
    pub fn parse_transactions_by_type(&self, mut type_var: Value, mut transactions: Value, optional_args: &[Value]) -> Value {
        ...
        let mut result: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_856 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_856 = false; is_less_than(&i, &get_array_length(&transactions)) } {
 ...
 */
    ...
    pub fn parse_position(&self, mut position: Value, optional_args: &[Value]) -> Value {
        ...
        let mut side: Value = (if is_true(&(rawSide.as_str() == Some("buy"))) { Value::Str("long".to_string()) } else { Value::Str("short".to_string()) });
 ...
 */
                    ...
                    if is_greater_than(&numErrors, &Value::Int(0)) {
                        {
                            ...
                            while { if !__for_first_858 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_858 = false; is_less_than(&i, &get_array_length(&get_value(&response, &Value::Str("error".to_string())))) } {
    ...
    m
                    ...
                    if (in_op(&result, &Value::Str("orders".to_string()))) {
