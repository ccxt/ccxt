...
impl CoinspotCore {
    ...
    pub fn parse_balance(&self, mut response: Value) -> Value {
        ...
        let mut balances: Value = self.safe_value2(response.clone(), Value::Str("balance".into()), Value::Str("balances".into()), &[]);
        if is_true(&(matches!(&balances, Value::Arr(_)))) {
            ...
            let mut currencyIds: Value = object_keys(&balances);
            {
                ...
                let mut code: Value = self.safe_currency_code(currencyId.clone(), &[]);
                let mut account: Value = self.account();
                add_element_to_object(&mut account, &Value::Str("total".into()), self.safe_string(balances.clone(), currencyId.clone(), &[]));
                if (code != Value::Null) {
                    add_element_to_object(&mut result, &code, account.clone());
                }
            }
 ...
 */
    ...
    m
        ...
        return self.parse_order_book(orderbook.clone(), market.as_map().and_then(|__m| __m.get("symbol")).cloned().unwrap_or(Value::Null), &[Value::Null, Value::Str("buyorders".into()), Value::Str("sellorders".into()), Value::Str("rate".into()), Value::Str("amount".into())]);
 ...
 */
    ...
    m
        ...
        let mut fullPath: Value = (if is_true(&(version != Value::Null)) { Value::Str(format!("{}{}", add(&Value::Str("/".into()), &version), endpoint).into()) } else { endpoint.clone() });
