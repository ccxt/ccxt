 ...
 */
    ...
    m
        ...
        let mut result: Value = Value::List(vec![]);
        {
            ...
            if is_true(&(status.as_str() == Some("1"))) && (is_equal(&isSpotTradingAllowed, &Value::Bool(true))) {
 ...
 */
    ...
    pub fn parse_trade(&self, mut trade: Value, optional_args: &[Value]) -> Value {
        ...
        // if swap
        if (in_op(&trade, &Value::Str("v".to_string()))) {
            //
            // swap: fetchTrades
            //
            //     {
            ...
            // if swap
            if (in_op(&trade, &Value::Str("positionMode".to_string()))) {
 ...
 */
    ...
    pub fn parse_ticker(&self, mut ticker: Value, optional_args: &[Value]) -> Value {
        ...
        // if swap
        if (is_equal(&isSwap, &Value::Bool(true))) || (in_op(&ticker, &Value::Str("timestamp".to_string()))) {
            //
            //     {
            //         "symbol": "ETH_USDT",
            //         "lastPrice": 3581.3,
 ...
 */
    ...
    m
        ...
        let mut response: Value = Value::Null;
        if is_true(&(marginMode != Value::Null)) || is_true(&(isMargin.as_bool() == Some(true))) || is_true(&(marketType.as_str() == Some("margin"))) {
 ...
 */
    ...
    pub fn parse_position(&self, mut position: Value, optional_args: &[Value]) -> Value {
        ...
        let mut side: Value = (if is_true(&(rawSide.as_str() == Some("1"))) { Value::Str("long".to_string()) } else { Value::Str("short".to_string()) });
        let mut openType: Value = self.safe_string_k(position.clone(), "margin_mode", &[]);
        let mut marginType: Value = (if is_true(&(openType.as_str() == Some("1"))) { Value::Str("isolated".to_string()) } else { Value::Str("cross".to_string()) });
 ...
 */
    ...
    pub fn parse_transfer(&self, mut transfer: Value, optional_args: &[Value]) -> Value {
        ...
        if is_true(&(fromAccountType != Value::Null)) && is_true(&(toAccountType != Value::Null)) {
            ...
            accountFrom = (if is_true(&(direction.as_str() == Some("IN"))) { Value::Str("MAIN".to_string()) } else { Value::Str("CONTRACT".to_string()) });
            accountTo = (if is_true(&(direction.as_str() == Some("IN"))) { Value::Str("CONTRACT".to_string()) } else { Value::Str("MAIN".to_string()) });
        }  else {
 ...
 */
    ...
    pub fn parse_leverage(&self, mut leverage: Value, optional_args: &[Value]) -> Value {
        ...
        let mut shortLeverage: Value = Value::Null;
        {
            ...
            marginMode = (if is_true(&(openType.as_f64() == Some(1.0))) { Value::Str("isolated".to_string()) } else { Value::Str("cross".to_string()) });
        }
        }
        return Value::Map({
    let mut m = indexmap::IndexMap::new();
 ...
 */
    ...
    m
        ...
        if (direction != Value::Null) {
            add_element_to_object(&mut request, &Value::Str("positionType".to_string()), (if is_true(&(direction.as_str() == Some("short"))) { Value::Int(2) } else { Value::Int(1) }));
        }
