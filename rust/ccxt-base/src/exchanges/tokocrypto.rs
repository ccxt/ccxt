 ...
 */
    ...
    m
        ...
        let mut result: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_1084 { i = add(&i, &Value::Int(1)); } __for_first_1084 = false; is_less_than(&i, &get_array_length(&list)) } {
            ...
            let mut permissions: Value = self.safe_list_k(market.clone(), "permissions", &[Value::List(vec![])]);
            {
                ...
                while { if !__for_first_1083 { j = add(&j, &Value::Int(1)); } __for_first_1083 = false; is_less_than(&j, &get_array_length(&permissions)) } {
    ...
    m
        ...
        if is_true(&(Value::Bool(in_op(&config, &Value::Str("noCoin".to_string()))))) && !is_true(&(Value::Bool(in_op(&params, &Value::Str("coin".to_string()))))) {
            ...
            let mut byLimit: Value = self.safe_list_k(config.clone(), "byLimit", &[Value::List(vec![])]);
            {
                ...
                while { if !__for_first_1086 { i = add(&i, &Value::Int(1)); } __for_first_1086 = false; is_less_than(&i, &get_array_length(&byLimit)) } {
