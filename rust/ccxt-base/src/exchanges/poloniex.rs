 ...
 */
    ...
    m
        ...
        let mut withdrawals: Value = self.safe_value_k(response.clone(), "withdrawals", &[Value::from(vec![])]);
        let mut deposits: Value = self.safe_value_k(response.clone(), "deposits", &[Value::from(vec![])]);
