 ...
 */
    ...
    m
        ...
        let mut keyStart: Value = (if (market.as_map().and_then(|__m| __m.get("spot")).cloned().unwrap_or(Value::Null).as_bool() == Some(true)) { Value::Str("startTime".to_string()) } else { Value::Str("sTime".to_string()) });
        let mut keyEnd: Value = (if (market.as_map().and_then(|__m| __m.get("spot")).cloned().unwrap_or(Value::Null).as_bool() == Some(true)) { Value::Str("endTime".to_string()) } else { Value::Str("eTime".to_string()) });
        if (since != Value::Null) {
 ...
 */
    ...
    pub fn parse_swap_market(&self, mut market: Value) -> Value {
        ...
        let mut marketType: Value = (if (type_var.as_str() == Some("future")) { Value::Str("future".to_string()) } else { Value::Str("swap".to_string()) });
        return self.safe_market_structure(&[Value::Map({
    let mut m = indexmap::IndexMap::new();
        m.insert("id".to_string(), id.clone());
 ...
 */
    ...
    m
        ...
        if (triggerPrice != Value::Null) {
            ...
            upperCaseType = (if (price == Value::Null) { Value::Str("STOP".to_string()) } else { Value::Str("STOP_LIMIT".to_string()) });
        ...
        if matches!(&isMarket, Value::Bool(true)) {
            if (side.as_str() == Some("buy")) {
                ...
                let mut amountKey: Value = (if (market.as_map().and_then(|__m| __m.get("spot")).cloned().unwrap_or(Value::Null).as_bool() == Some(true)) { Value::Str("amount".to_string()) } else { Value::Str("sz".to_string()) });
                add_element_to_object(&mut request, &amountKey, quoteAmount.clone());
            }  else {
                let mut amountKey: Value = (if (market.as_map().and_then(|__m| __m.get("spot")).cloned().unwrap_or(Value::Null).as_bool() == Some(true)) { Value::Str("quantity".to_string()) } else { Value::Str("sz".to_string()) });
                add_element_to_object(&mut request, &amountKey, self.amount_to_precision(symbol.clone(), amount.clone()));
            }
        }  else {
            let mut amountKey: Value = (if (market.as_map().and_then(|__m| __m.get("spot")).cloned().unwrap_or(Value::Null).as_bool() == Some(true)) { Value::Str("quantity".to_string()) } else { Value::Str("sz".to_string()) });
            add_element_to_object(&mut request, &amountKey, self.amount_to_precision(symbol.clone(), amount.clone()));
            let mut priceKey: Value = (if (market.as_map().and_then(|__m| __m.get("spot")).cloned().unwrap_or(Value::Null).as_bool() == Some(true)) { Value::Str("price".to_string()) } else { Value::Str("px".to_string()) });
        ...
        if (clientOrderId != Value::Null) {
            ...
            let mut clientOrderIdKey: Value = (if (market.as_map().and_then(|__m| __m.get("spot")).cloned().unwrap_or(Value::Null).as_bool() == Some(true)) { Value::Str("clientOrderId".to_string()) } else { Value::Str("clOrdId".to_string()) });
 ...
 */
    ...
    pub fn parse_transaction(&self, mut transaction: Value, optional_args: &[Value]) -> Value {
        ...
        let mut type_var: Value = (if (matches!(&transaction, Value::Dict(__d) if __d.contains_key("withdrawalRequestsId"))) { Value::Str("withdrawal".to_string()) } else { Value::Str("deposit".to_string()) });
 ...
 */
    ...
    pub fn parse_margin_modification(&self, mut data: Value, optional_args: &[Value]) -> Value {
        ...
        let mut type_var: Value = (if (rawType.as_deref() == Some("ADD")) { Value::Str("add".to_string()) } else { Value::Str("reduce".to_string()) });
        return Value::Map({
    let mut m = indexmap::IndexMap::new();
