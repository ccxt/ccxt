 ...
 */
    ...
    m
        ...
        let mut details: Value = self.safe_list_k(data.clone(), "details", &[Value::List(vec![])]);
        {
            ...
            while { if !__for_first_427 { i = add(&i, &Value::Int(1)); } __for_first_427 = false; is_less_than(&i, &get_array_length(&details)) } {
    ...
    m
        ...
        if is_equal(&clientOrderIds, &Value::Null) {
            ...
            if !is_equal(&tpslIds, &Value::Null) {
                {
                    ...
                    while { if !__for_first_430 { i = add(&i, &Value::Int(1)); } __for_first_430 = false; is_less_than(&i, &get_array_length(&tpslIds)) } {
        ...
        }  else {
            {
                ...
                while { if !__for_first_432 { i = add(&i, &Value::Int(1)); } __for_first_432 = false; is_less_than(&i, &get_array_length(&clientOrderIds)) } {
                append_to_array(&mut request, Value::Map({
                    let mut m = indexmap::IndexMap::new();
                        m.insert("instId".to_string(), get_value(&market, &Value::Str("id".to_string())));
 ...
 */
    ...
    m
        ...
        let mut instIds: Value = Value::Str("".to_string());
        {
            ...
            while { if !__for_first_433 { i = add(&i, &Value::Int(1)); } __for_first_433 = false; is_less_than(&i, &get_array_length(&symbolsList)) } {
