 ...
 */
    ...
    m
        ...
        if (marketType.as_str() == Some("spot")) {
            ...
            if (clientOrderId == Value::Null) {
                if (matches!(&isAlgo, Value::Bool(true))) {
 ...
 */
    ...
    pub fn parse_order(&self, mut order: Value, optional_args: &[Value]) -> Value {
        ...
        let mut type_var: Value = Value::Null;
        if (matches!(&isLinearOrder, Value::Bool(true))) {
        ...
        if is_true(&(Value::Bool(type_var != Value::Null))) && is_true(&(get_index_of(&type_var, &Value::Str("market".to_string())).as_f64().unwrap_or(f64::NAN) >= Value::Int(0).as_f64().unwrap_or(f64::NAN))) && is_true(&(Value::Bool(!matches!(&isLinearOrder, Value::Bool(true))))) {
