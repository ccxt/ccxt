 ...
 */
    ...
    m
        ...
        return self.parse_order_book(response.clone(), market.as_map().and_then(|__m| __m.get("symbol")).cloned().unwrap_or(Value::Null), &[Value::Null, Value::Str("bids".into()), Value::Str("asks".into()), Value::Str("price".into()), Value::Str("amount".into())]);
 ...
 */
    ...
    m
        ...
        return self.parse_deposit_addresses(response.clone(), &[codes.clone(), Value::Bool(false)]);
 ...
 */
    ...
    let mut m = indexmap::IndexMap::new();
        m.insert("info".to_string(), response.clone());
        m.insert("id".to_string(), self.safe_string_k(response.clone(), "uuid", &[]));
    m
}), &[market.clone()]);
 ...
 */
    ...
    m
        ...
        if (api.as_str() == Some("public")) {
            ...
            add_element_to_object(&mut headers, &Value::Str("Api-Signature".into()), self.hmac(self.encode(auth.clone()), self.encode(self.secret.clone()), Value::Str("sha256".into()), &[]));
        }
        return Value::Map({
    let mut m = indexmap::IndexMap::new();
