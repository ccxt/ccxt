...
impl OkxCore {
    ...
    m
        ...
        if is_true(&(type_var.is_none())) && is_true(&(instType != Value::Null)) {
            add_element_to_object(&mut params, &Value::Str("type".into()), instType.clone());
        }
    ...
    pub fn create_expired_option_market(&self, mut symbol: Value) -> Value {
        ...
        let mut datetime: Value = (if is_true(&(expiry == Value::Null)) { Value::Null } else { self.convert_expire_date(expiry.clone()) });
    ...
    pub fn safe_market(&self, optional_args: &[Value]) -> Value {
        ...
        if isOption && is_true(&(marketId != Value::Null)) && (is_true(&(self.markets_by_id.clone() == Value::Null)) || !(in_op(&self.markets_by_id, &marketId))) {
    ...
    m
        ...
        let mut update: Value = Value::Map({
            let mut m = indexmap::IndexMap::new();
                ...
                m.insert("status".to_string(), (if is_true(&(dataLength == 0.0)) { Value::Str("ok".into()) } else { Value::Str("maintenance".into()) }));
        ...
        });
        {
            ...
            add_element_to_object(&mut update, &Value::Str("eta".into()), self.safe_integer_k(event.clone(), "end", &[]));
            add_element_to_object(&mut update, &Value::Str("url".into()), self.safe_string_k(event.clone(), "href", &[]));
            if (state.as_deref() == Some("ongoing")) {
                add_element_to_object(&mut update, &Value::Str("status".into()), Value::Str("maintenance".into()));
            }  else if (state.as_deref() == Some("scheduled")) {
                add_element_to_object(&mut update, &Value::Str("status".into()), Value::Str("ok".into()));
        ...
        if is_true(&contract) {
            ...
            if is_true(&future) {
                ...
                if (expiry != Value::Null) {
                    ...
                    optionType = (if is_true(&(optionType.as_str() == Some("P"))) { Value::Str("put".into()) } else { Value::Str("call".into()) });
                }
            }
        }
        let mut feesType: Value = (if is_true(&(type_var == Value::Null)) { Value::Str("".into()) } else { type_var.clone() });
        let mut fees: Value = self.safe_dict2(self.fees.clone(), feesType.clone(), Value::Str("trading".into()), &[Value::Map({
    let mut m = indexmap::IndexMap::new();
    m
})]);
 ...
 */
    ...
    m
        ...
        if !is_true(&self.check_required_credentials(&[Value::Bool(false)])) || is_true(&(isSandboxMode.as_bool() == Some(true))) {
            return Value::Map({
    let mut m = indexmap::IndexMap::new();
    m
});
        ...
        //
        let mut data: Value = self.safe_list_k(response, "data", &[Value::from(vec![])]);
        let mut dataByCurrencyId: Value = self.group_by(data.clone(), Value::Str("ccy".into()), &[]);
 ...
 */
    ...
    m
        ...
        if is_true(&rpi) && is_true(&(limit.as_f64().unwrap_or(f64::NAN) > ((400i64) as f64))) {
        ...
        if (limit != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("sz".to_string(), limit.clone()); } // max 400
        }
        let mut response: Value = Value::Null;
        if is_true(&rpi) {
            let __ws_arg_12 = self.extend(request.clone(), &[params.clone()]);
            response = self.public_get_market_books_rpi(&[__ws_arg_12]).await;
        }  else if is_true(&(method.as_str() == Some("publicGetMarketBooksFull"))) || is_true(&(limit.as_f64().unwrap_or(f64::NAN) > ((400i64) as f64))) {
    ...
    pub fn parse_ticker(&self, mut ticker: Value, optional_args: &[Value]) -> Value {
        ...
        if (instType.is_some()) {
            marketType = (if is_true(&(instType.as_deref() == Some("SPOT"))) { Value::Str("spot".into()) } else { Value::Str("swap".into()) });
        }
        let mut timestamp: Value = self.safe_integer_k(ticker.clone(), "ts", &[]);
        let mut marketId: Value = self.safe_string_k(ticker.clone(), "instId", &[]);
        market = self.safe_market(&[marketId.clone(), market.clone(), Value::Str("-".into()), marketType.clone()]);
        ...
        let mut quoteVolume: Value = (if is_true(&(spot.as_bool() == Some(true))) { self.safe_string_k(ticker.clone(), "volCcy24h", &[]) } else { Value::Null });
 ...
 */
    ...
    m
        ...
        if (marketType.as_str() == Some("option")) {
            ...
            let mut currencyId: Value = self.safe_string2(params.clone(), Value::Str("uly".into()), Value::Str("marketId".into()), &[defaultUnderlying.clone()]);
            if (currencyId == Value::Null) {
                ...
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("uly".to_string(), currencyId.clone()); }
            }
        }
 ...
 */
    ...
    m
        ...
        let mut market: Value = self.get_market_from_symbols(&[symbols.clone()]);
        let mut marketType: Value = Value::Null;
        { let __destr_tmp = self.handle_market_type_and_params(Value::Str("fetchMarkPrices".into()), &[market.clone(), params.clone(), Value::Str("swap".into())]); marketType = get_value(&__destr_tmp, &Value::Int(0)); params = get_value(&__destr_tmp, &Value::Int(1)); }
        let mut request: Value = Value::Map({
            let mut m = indexmap::IndexMap::new();
                m.insert("instType".to_string(), self.convert_to_instrument_type(marketType.clone()));
            m
 ...
 */
    ...
    pub fn parse_ohlcv(&self, mut ohlcv: Value, optional_args: &[Value]) -> Value {
        ...
        let mut volumeIndex: Value = (if is_true(&(type_var.as_str() == Some("spot"))) { Value::Int(5) } else { Value::Int(6) });
        return Value::from(vec![self.safe_integer(ohlcv.clone(), Value::Int(0), &[]), self.safe_number(ohlcv.clone(), Value::Int(1), &[]), self.safe_number(ohlcv.clone(), Value::Int(2), &[]), self.safe_number(ohlcv.clone(), Value::Int(3), &[]), self.safe_number(ohlcv.clone(), Value::Int(4), &[]), self.safe_number(ohlcv.clone(), volumeIndex.clone(), &[])]);
 ...
 */
    ...
    m
        ...
        if is_true(&(timezone.as_str() == Some("UTC"))) && is_true(&(duration.as_f64().unwrap_or(f64::NAN) >= ((21600i64) as f64))) {
        ...
        if (since != Value::Null) {
            ...
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("before".to_string(), startTime.clone()); }
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("after".to_string(), self.sum(&[since.clone(), (match (&(durationInMilliseconds), &(limit)) { (Value::Int(x), Value::Int(y)) => Value::Int(x * y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 * *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x * *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x * y), _ => Value::Null })])); }
        }
        ...
        if (priceType.as_str() == Some("mark")) {
            ...
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("instId".to_string(), crate::value::get_value_k(&market.as_map().and_then(|__m| __m.get("info")).cloned().unwrap_or(Value::Null), "instFamily")); } // okx index candles require instFamily instead of instId
            if isHistoryCandles {
            ...
            if isHistoryCandles {
                if limitIsUndefined && is_true(&(limit.as_f64() == Some(100.0))) {
                    limit = Value::Int(300);
                    if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("limit".to_string(), Value::Int(300)); } // reassign to 300, but this whole logic needs to be simplified...
                }
 ...
 */
    ...
    m
        ...
        }  else if is_true(&(market.as_map().and_then(|__m| __m.get("swap")).cloned().unwrap_or(Value::Null).as_bool() == Some(true))) || is_true(&(market.as_map().and_then(|__m| __m.get("future")).cloned().unwrap_or(Value::Null).as_bool() == Some(true))) || is_true(&(market.as_map().and_then(|__m| __m.get("option")).cloned().unwrap_or(Value::Null).as_bool() == Some(true))) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("uly".to_string(), Value::Str(format!("{}{}", Value::Str(format!("{}{}", market.as_map().and_then(|__m| __m.get("baseId")).cloned().unwrap_or(Value::Null), Value::Str("-".into())).into()), market.as_map().and_then(|__m| __m.get("quoteId")).cloned().unwrap_or(Value::Null)).into())); }
        }  else {
 ...
 */
    ...
    m
        ...
        if (marketType.as_str() == Some("funding")) {
            ...
            let __ws_arg_31 = self.extend(request, &[query.clone()]);
 ...
 */
    ...
    m
        ...
        return self.create_order(symbol.clone(), Value::Str("market".into()), Value::Str("buy".into()), cost.clone(), &[Value::Null, __ws_arg_32]).await;
 ...
 */
    ...
    m
        ...
        return self.create_order(symbol.clone(), Value::Str("market".into()), Value::Str("sell".into()), cost.clone(), &[Value::Null, __ws_arg_33]).await;
    ...
    m
        ...
        let mut conditional: bool = is_true(&(stopLossPrice != Value::Null)) || is_true(&(takeProfitPrice != Value::Null)) || is_true(&(type_var.as_str() == Some("conditional")));
        let mut request: Value = Value::Map({
            let mut m = indexmap::IndexMap::new();
                m.insert("instId".to_string(), market.as_map().and_then(|__m| __m.get("id")).cloned().unwrap_or(Value::Null));
                m.insert("side".to_string(), side.clone());
        ...
        let mut isConditionalOrOCO: bool = conditional || is_true(&(type_var.as_str() == Some("oco")));
        ...
        let mut trigger: bool = is_true(&(triggerPrice != Value::Null)) || is_true(&(type_var.as_str() == Some("trigger")));
        let mut isReduceOnly: bool = is_true(&(self.safe_bool_k(params.clone(), "reduceOnly", &[Value::Bool(false)]).as_bool() == Some(true))) || is_true(&(closeFraction.is_some()));
        ...
        if is_true(&(marginMode != Value::Null)) && is_true(&(marginMode.as_str() != Some("cash"))) {
        ...
        if (spot.as_bool() == Some(true)) {
            if (margin.as_bool() == Some(true)) {
                let mut defaultCurrency: Value = (if is_true(&(side.as_str() == Some("buy"))) { market.as_map().and_then(|__m| __m.get("quote")).cloned().unwrap_or(Value::Null) } else { market.as_map().and_then(|__m| __m.get("base")).cloned().unwrap_or(Value::Null) });
                let mut currency: Value = self.safe_string_k(params.clone(), "ccy", &[defaultCurrency.clone()]);
                add_element_to_object(&mut request, &Value::Str("ccy".into()), self.safe_currency_code(currency.clone(), &[]));
            }
            let mut tradeMode: Value = (if is_true(&(margin.as_bool() == Some(true))) { marginMode.clone() } else { Value::Str("cash".into()) });
            add_element_to_object(&mut request, &Value::Str("tdMode".into()), tradeMode.clone());
            ...
            if is_true(&(market.as_map().and_then(|__m| __m.get("swap")).cloned().unwrap_or(Value::Null).as_bool() == Some(true))) || is_true(&(market.as_map().and_then(|__m| __m.get("future")).cloned().unwrap_or(Value::Null).as_bool() == Some(true))) {
                ...
                if (positionSide != Value::Null) {
                    ...
                    if (hedged.as_bool() == Some(true)) {
                        ...
                        let mut isProtective: bool = is_true(&(takeProfitPrice != Value::Null)) || is_true(&(stopLossPrice != Value::Null)) || isReduceOnly;
                        if isProtective {
            ...
            if is_true(&(spot.as_bool() == Some(true))) && is_true(&(side.as_str() == Some("buy"))) {
                ...
                if (tgtCcy.as_str() == Some("quote_ccy")) {
                    ...
                    if is_true(&createMarketBuyOrderRequiresPrice) {
                        ...
                        notional = (if is_true(&(notional == Value::Null)) { amount.clone() } else { notional.clone() });
                    }
                    add_element_to_object(&mut request, &Value::Str("sz".into()), self.cost_to_precision(symbol.clone(), notional.clone()));
                }
            }
            if marketIOC && is_true(&(contract.as_bool() == Some(true))) {
        ...
        if isTrailingPercentOrder {
            ...
            add_element_to_object(&mut request, &Value::Str("callbackRatio".into()), convertedTrailingPercent.clone());
            add_element_to_object(&mut request, &Value::Str("ordType".into()), Value::Str("move_order_stop".into()));
        }  else if isTrailingPriceOrder {
            add_element_to_object(&mut request, &Value::Str("callbackSpread".into()), trailingPrice.clone());
            add_element_to_object(&mut request, &Value::Str("ordType".into()), Value::Str("move_order_stop".into()));
            ...
            if hasStopLoss {
                ...
                add_element_to_object(&mut slOrder, &Value::Str("slTriggerPx".into()), slTriggerPx.clone());
                ...
                if (stopLossOrderType.is_some()) {
                    ...
                    if (!stopLossLimitOrderType) && (!stopLossMarketOrderType) {
                        ...
                        if (stopLossLimitPrice == Value::Null) {
                            ...
                            add_element_to_object(&mut slOrder, &Value::Str("slOrdPx".into()), self.price_to_precision(symbol.clone(), stopLossLimitPrice.clone()));
                        }
                    }  else if (stopLossOrderType.as_deref() == Some("market")) {
                        add_element_to_object(&mut slOrder, &Value::Str("slOrdPx".into()), Value::Str("-1".into()));
                    }
                }  else if (stopLossLimitPrice != Value::Null) {
                    add_element_to_object(&mut slOrder, &Value::Str("slOrdPx".into()), self.price_to_precision(symbol.clone(), stopLossLimitPrice.clone())); // limit sl order
                }  else {
                    add_element_to_object(&mut slOrder, &Value::Str("slOrdPx".into()), Value::Str("-1".into())); // market sl order
                }
                ...
                if (stopLossTriggerPriceType != Value::Null) {
                    if is_true(&(stopLossTriggerPriceType.as_str() != Some("last"))) && is_true(&(stopLossTriggerPriceType.as_str() != Some("index"))) && is_true(&(stopLossTriggerPriceType.as_str() != Some("mark"))) {
                    ...
                    add_element_to_object(&mut slOrder, &Value::Str("slTriggerPxType".into()), stopLossTriggerPriceType.clone());
                }
            ...
            if hasTakeProfit {
                ...
                add_element_to_object(&mut tpOrder, &Value::Str("tpTriggerPx".into()), self.price_to_precision(symbol.clone(), takeProfitTriggerPrice.clone()));
                ...
                if (takeProfitOrderType != Value::Null) {
                    ...
                    if (!takeProfitLimitOrderType) && (!takeProfitMarketOrderType) {
                        ...
                        if (takeProfitLimitPrice == Value::Null) {
                            ...
                            add_element_to_object(&mut tpOrder, &Value::Str("tpOrdKind".into()), takeProfitOrderType.clone());
                            add_element_to_object(&mut tpOrder, &Value::Str("tpOrdPx".into()), self.price_to_precision(symbol.clone(), takeProfitLimitPrice.clone()));
                        }
                    }  else if (takeProfitOrderType.as_str() == Some("market")) {
                        add_element_to_object(&mut tpOrder, &Value::Str("tpOrdPx".into()), Value::Str("-1".into()));
                    }
                }  else if (takeProfitLimitPrice != Value::Null) {
                    add_element_to_object(&mut tpOrder, &Value::Str("tpOrdKind".into()), Value::Str("limit".into()));
                    add_element_to_object(&mut tpOrder, &Value::Str("tpOrdPx".into()), self.price_to_precision(symbol.clone(), takeProfitLimitPrice.clone())); // limit tp order
                }  else {
                    add_element_to_object(&mut tpOrder, &Value::Str("tpOrdPx".into()), Value::Str("-1".into())); // market tp order
                }
                ...
                if (takeProfitTriggerPriceType != Value::Null) {
                    if is_true(&(takeProfitTriggerPriceType.as_str() != Some("last"))) && is_true(&(takeProfitTriggerPriceType.as_str() != Some("index"))) && is_true(&(takeProfitTriggerPriceType.as_str() != Some("mark"))) {
                    ...
                    add_element_to_object(&mut tpOrder, &Value::Str("tpTriggerPxType".into()), takeProfitTriggerPriceType.clone());
                }
        ...
        if trigger {
            ...
            add_element_to_object(&mut request, &Value::Str("triggerPx".into()), self.price_to_precision(symbol.clone(), triggerPrice.clone()));
            add_element_to_object(&mut request, &Value::Str("orderPx".into()), (if is_true(&isMarketOrder) { Value::Str("-1".into()) } else { self.price_to_precision(symbol.clone(), price.clone()) }));
        }  else if conditional {
            ...
            if (takeProfitPrice != Value::Null) {
                ...
                add_element_to_object(&mut request, &Value::Str("tpOrdPx".into()), tpOrdPxReq.clone());
                add_element_to_object(&mut request, &Value::Str("tpTriggerPxType".into()), tpTriggerPxType.clone());
            }
            if (stopLossPrice != Value::Null) {
                ...
                add_element_to_object(&mut request, &Value::Str("slOrdPx".into()), slOrdPxReq.clone());
                add_element_to_object(&mut request, &Value::Str("slTriggerPxType".into()), slTriggerPxType.clone());
            }
        }
        if (clientOrderId == Value::Null) {
            ...
            if (brokerId != Value::Null) {
                ...
                add_element_to_object(&mut request, &Value::Str("tag".into()), brokerId.clone());
            }
        }  else {
            add_element_to_object(&mut request, &Value::Str("clOrdId".into()), clientOrderId.clone());
 ...
 */
    ...
    m
        ...
        if is_true(&(requestOrdType.as_deref() == Some("trigger"))) || is_true(&(requestOrdType.as_deref() == Some("conditional"))) || is_true(&(requestOrdType.as_deref() == Some("move_order_stop"))) || is_true(&(type_var.as_str() == Some("move_order_stop"))) || is_true(&(type_var.as_str() == Some("oco"))) || is_true(&(type_var.as_str() == Some("iceberg"))) || is_true(&(type_var.as_str() == Some("twap"))) {
            method = Value::Str("privatePostTradeOrderAlgo".into());
        }
        if is_true(&(method.as_str() != Some("privatePostTradeOrder"))) && is_true(&(method.as_str() != Some("privatePostTradeOrderAlgo"))) && is_true(&(method.as_str() != Some("privatePostTradeBatchOrders"))) {
 ...
 */
    ...
    m
        ...
        if is_true(&(type_var.as_str() == Some("trigger"))) || is_true(&(type_var.as_str() == Some("conditional"))) || is_true(&(type_var.as_str() == Some("move_order_stop"))) || is_true(&(type_var.as_str() == Some("oco"))) || is_true(&(type_var.as_str() == Some("iceberg"))) || is_true(&(type_var.as_str() == Some("twap"))) {
        ...
        if is_true(&isAlgoOrder) {
            ...
            if (stopLossTriggerPrice != Value::Null) {
                ...
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("newSlTriggerPx".to_string(), self.price_to_precision(symbol.clone(), stopLossTriggerPrice.clone())); }
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("newSlOrdPx".to_string(), (if is_true(&(type_var.as_str() == Some("market"))) { Value::Str("-1".into()) } else { self.price_to_precision(symbol.clone(), stopLossPrice.clone()) })); }
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("newSlTriggerPxType".to_string(), stopLossTriggerPriceType.clone()); }
            }
            if (takeProfitTriggerPrice != Value::Null) {
                if (takeProfitPrice == Value::Null) {
                ...
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("newTpTriggerPx".to_string(), self.price_to_precision(symbol.clone(), takeProfitTriggerPrice.clone())); }
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("newTpOrdPx".to_string(), (if is_true(&(type_var.as_str() == Some("market"))) { Value::Str("-1".into()) } else { self.price_to_precision(symbol.clone(), takeProfitPrice.clone()) })); }
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("newTpTriggerPxType".to_string(), takeProfitTriggerPriceType.clone()); }
            }
        }  else {
            if (stopLossTriggerPrice != Value::Null) {
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("newSlTriggerPx".to_string(), self.price_to_precision(symbol.clone(), stopLossTriggerPrice.clone())); }
            ...
            if hasStopLoss {
                ...
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("newSlTriggerPx".to_string(), self.price_to_precision(symbol.clone(), stopLossTriggerPrice.clone())); }
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("newSlOrdPx".to_string(), (if is_true(&(stopLossType.as_deref() == Some("market"))) { Value::Str("-1".into()) } else { self.price_to_precision(symbol.clone(), stopLossPrice.clone()) })); }
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("newSlTriggerPxType".to_string(), stopLossTriggerPriceType.clone()); }
            }
            if hasTakeProfit {
                ...
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("newTpOrdKind".to_string(), (if is_true(&(takeProfitType.as_str() == Some("limit"))) { takeProfitType.clone() } else { Value::Str("condition".into()) })); }
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("newTpTriggerPx".to_string(), self.price_to_precision(symbol.clone(), takeProfitTriggerPrice.clone())); }
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("newTpOrdPx".to_string(), (if is_true(&(takeProfitType.as_str() == Some("market"))) { Value::Str("-1".into()) } else { self.price_to_precision(symbol.clone(), takeProfitPrice.clone()) })); }
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("newTpTriggerPxType".to_string(), takeProfitTriggerPriceType.clone()); }
            }
        }
 ...
 */
    ...
    m
        ...
        let mut isTrigger: bool = is_true(&(trigger != Value::Null)) && (!is_equal(&trigger, &Value::Bool(false)));
        if isTrigger || is_true(&(trailing.as_bool() == Some(true))) {
        ...
        let __ws_arg_36 = self.extend(request, &[query.clone()]);
    ...
    pub fn parse_ids(&self, mut ids: Value) -> Value {
         ...
         */
        if is_true(&(ids != Value::Null)) && matches!(&ids, Value::Str(_)) {
 ...
 */
    ...
    m
        ...
        let mut defaultMethod: Value = self.safe_string_k(options.clone(), "method", &[Value::Str("privatePostTradeCancelBatchOrders".into())]);
        ...
        return self.parse_orders(ordersData.clone(), &[market.clone(), Value::Null, Value::Null, orderParams.clone()]);
 ...
 */
    ...
    m
        ...
        let mut trailing: Value = self.safe_bool_k(params.clone(), "trailing", &[Value::Bool(false)]);
        let mut isStopOrTrailing: bool = is_true(&(trigger.as_bool() == Some(true))) || is_true(&(trailing.as_bool() == Some(true)));
        if (isStopOrTrailing) {
            method = Value::Str("privatePostTradeCancelAlgos".into());
        }
        {
            ...
            add_element_to_object(&mut requestItem, &idKey, (if is_true(&(clientOrderId != Value::Null)) { clientOrderId.clone() } else { id.clone() }));
 ...
 */
    ...
    m
        ...
        if is_true(&(timeout != Value::Null)) && is_true(&(timeout.as_f64().unwrap_or(f64::NAN) > ((0i64) as f64))) {
    ...
    pub fn parse_order(&self, mut order: Value, optional_args: &[Value]) -> Value {
        ...
        if is_true(&(scode.is_some())) && is_true(&(scode.as_deref() != Some("0"))) {
    ...
    m
        ...
        if is_true(&(side.as_str() == Some("buy"))) && is_true(&(type_var.as_str() == Some("market"))) && is_true(&(instType.as_deref() == Some("SPOT"))) && is_true(&(tgtCcy.as_deref() == Some("quote_ccy"))) {
    ...
    let mut m = indexmap::IndexMap::new();
        ...
        m.insert("triggerPrice".to_string(), self.safe_number_n(order.clone(), Value::from(vec![Value::Str("triggerPx".into()), Value::Str("moveTriggerPx".into())]), &[]));
 ...
 */
    ...
    m
        ...
        let mut defaultMethod: Value = self.safe_string_k(options.clone(), "method", &[Value::Str("privateGetTradeOrder".into())]);
        let mut method: Value = self.safe_string_k(params.clone(), "method", &[defaultMethod.clone()]);
        let mut trigger: Value = self.safe_value2(params.clone(), Value::Str("stop".into()), Value::Str("trigger".into()), &[]);
        let mut isTrigger: bool = is_true(&(trigger != Value::Null)) && (!is_equal(&trigger, &Value::Bool(false)));
        if isTrigger {
        ...
        if (method.as_str() == Some("privateGetTradeOrderAlgo")) {
            ...
            let __ws_arg_39 = self.extend(request, &[query.clone()]);
 ...
 */
    ...
    m
        ...
        if (limit != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("limit".to_string(), crate::runtime::Math::min(&limit, &maxLimit)); } // default 100, max 100
        }
    ...
    m
        ...
        let mut defaultMethod: Value = self.safe_string_k(options.clone(), "method", &[Value::Str("privateGetTradeOrdersPending".into())]);
        let mut method: Value = self.safe_string_k(params.clone(), "method", &[defaultMethod.clone()]);
        let mut ordType: Value = self.safe_string_k(params.clone(), "ordType", &[]);
        let mut trigger: Value = self.safe_value2(params.clone(), Value::Str("stop".into()), Value::Str("trigger".into()), &[]);
        let mut trailing: Value = self.safe_bool_k(params.clone(), "trailing", &[Value::Bool(false)]);
        let mut isTrigger: bool = is_true(&(trigger != Value::Null)) && (!is_equal(&trigger, &Value::Bool(false)));
        if is_true(&(trailing.as_bool() == Some(true))) || isTrigger || (is_true(&(ordType != Value::Null)) && (in_op(&algoOrderTypes, &ordType))) {
            method = Value::Str("privateGetTradeOrdersAlgoPending".into());
        }
        if (trailing.as_bool() == Some(true)) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("ordType".to_string(), Value::Str("move_order_stop".into())); }
        }  else if is_true(&(trigger != Value::Null)) && is_true(&(ordType == Value::Null)) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("ordType".to_string(), Value::Str("trigger".into())); }
        }
 ...
 */
    ...
    m
        ...
        { let __destr_tmp = self.handle_market_type_and_params(Value::Str("fetchCanceledOrders".into()), &[market.clone(), params.clone()]); type_var = get_value(&__destr_tmp, &Value::Int(0)); query = get_value(&__destr_tmp, &Value::Int(1)); }
        if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("instType".to_string(), self.convert_to_instrument_type(type_var.clone())); }
        if (limit != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("limit".to_string(), limit.clone()); } // default 100, max 100
        }
        if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("state".to_string(), Value::Str("canceled".into())); }
    ...
    m
        ...
        let mut defaultMethod: Value = self.safe_string_k(options.clone(), "method", &[Value::Str("privateGetTradeOrdersHistory".into())]);
        let mut method: Value = self.safe_string_k(params.clone(), "method", &[defaultMethod.clone()]);
        let mut ordType: Value = self.safe_string_k(params.clone(), "ordType", &[]);
        let mut trigger: Value = self.safe_value2(params.clone(), Value::Str("stop".into()), Value::Str("trigger".into()), &[]);
        let mut trailing: Value = self.safe_bool_k(params.clone(), "trailing", &[Value::Bool(false)]);
        ...
        if (trailing.as_bool() == Some(true)) {
            ...
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("ordType".to_string(), Value::Str("move_order_stop".into())); }
        }  else if isTrigger || (is_true(&(ordType != Value::Null)) && (in_op(&algoOrderTypes, &ordType))) {
            method = Value::Str("privateGetTradeOrdersAlgoHistory".into());
            let mut algoId: Value = self.safe_string_k(params.clone(), "algoId", &[]);
            if (algoId != Value::Null) {
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("algoId".to_string(), algoId.clone()); }
        ...
        if (method.as_str() == Some("privateGetTradeOrdersAlgoHistory")) {
            ...
            let __ws_arg_43 = self.extend(request, &[send.clone()]);
 ...
 */
    ...
    m
        ...
        let mut type_var: Value = Value::Null;
        let mut query: Value = Value::Null;
        { let __destr_tmp = self.handle_market_type_and_params(Value::Str("fetchClosedOrders".into()), &[market.clone(), params.clone()]); type_var = get_value(&__destr_tmp, &Value::Int(0)); query = get_value(&__destr_tmp, &Value::Int(1)); }
        if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("instType".to_string(), self.convert_to_instrument_type(type_var.clone())); }
        if (limit != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("limit".to_string(), crate::runtime::Math::min(&limit, &maxLimit)); } // default 100, max 100
        }
    ...
    m
        ...
        let mut trigger: Value = self.safe_bool2(params.clone(), Value::Str("stop".into()), Value::Str("trigger".into()), &[]);
        let mut trailing: Value = self.safe_bool_k(params.clone(), "trailing", &[Value::Bool(false)]);
        if is_true(&(trailing.as_bool() == Some(true))) || is_true(&(trigger.as_bool() == Some(true))) || (is_true(&(ordType != Value::Null)) && (in_op(&algoOrderTypes, &ordType))) {
            method = Value::Str("privateGetTradeOrdersAlgoHistory".into());
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("state".to_string(), Value::Str("effective".into())); }
        }
        if (trailing.as_bool() == Some(true)) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("ordType".to_string(), Value::Str("move_order_stop".into())); }
            ...
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("state".to_string(), Value::Str("filled".into())); }
        }
        let mut send: Value = self.omit(query, Value::from(vec![Value::Str("method".into()), Value::Str("stop".into()), Value::Str("trigger".into()), Value::Str("trailing".into())]), &[]);
        let mut response: Value = Value::Null;
        if (method.as_str() == Some("privateGetTradeOrdersAlgoHistory")) {
            ...
            let __ws_arg_46 = self.extend(request, &[send.clone()]);
 ...
 */
    ...
    m
        ...
        add_element_to_object(&mut request, &Value::Str("instType".into()), self.convert_to_instrument_type(type_var.clone()));
        if is_true(&(limit != Value::Null)) && is_true(&(since == Value::Null)) {
 ...
 */
    ...
    m
        ...
        return self.fetch_my_trades(&[symbol.clone(), since.clone(), limit.clone(), __ws_arg_48]).await;
 ...
 */
    ...
    m
        ...
        if (method.as_str() != Some("privateGetAssetBills")) {
            if (marginMode != Value::Null) {
                add_element_to_object(&mut request, &Value::Str("mgnMode".into()), marginMode.clone());
            }
        }
        ...
        if (code != Value::Null) {
            ...
            add_element_to_object(&mut request, &Value::Str("ccy".into()), currency.as_map().and_then(|__m| __m.get("id")).cloned().unwrap_or(Value::Null));
        }
        { let __destr_tmp = self.handle_until_option(Value::Str("end".into()), request.clone(), params.clone(), &[]); request = __destr_tmp.as_array().and_then(|__arr| __arr.get(0)).cloned().unwrap_or(Value::Null); params = __destr_tmp.as_array().and_then(|__arr| __arr.get(1)).cloned().unwrap_or(Value::Null); }
        let mut response: Value = Value::Null;
        if (method.as_str() == Some("privateGetAccountBillsArchive")) {
            ...
            let __ws_arg_51 = self.extend(request, &[query.clone()]);
    ...
    m
        ...
        let mut networksById: Value = self.index_by(networks.clone(), Value::Str("id".into()));
        let mut networkData: Value = (if is_true(&(chain == Value::Null)) { Value::Null } else { self.safe_dict(networksById.clone(), chain.clone(), &[]) });
        ...
        if (chain.as_str() == Some("USDT-Polygon")) {
            networkData = self.safe_dict2(networksById.clone(), Value::Str("USDT-Polygon-Bridge".into()), Value::Str("USDT-Polygon".into()), &[]);
        }
 ...
 */
    ...
    m
        ...
        //
        let mut data: Value = self.safe_list_k(response.clone(), "data", &[Value::from(vec![])]);
        let mut filtered: Value = self.filter_by(data.clone(), Value::Str("selected".into()), Value::Bool(true), &[]);
        let mut parsed: Value = self.parse_deposit_addresses(filtered.clone(), &[Value::from(vec![currency.as_map().and_then(|__m| __m.get("code")).cloned().unwrap_or(Value::Null)]), Value::Bool(false)]);
 ...
 */
    ...
    m
        ...
        if is_true(&(codeNetwork != Value::Null)) && (in_op(&response, &codeNetwork)) {
            return get_value(&response, &codeNetwork);
        }
        // if the network is not specified, return the first address
        let mut keys: Value = object_keys(&response);
        let mut first: Value = self.safe_string(keys.clone(), Value::Int(0), &[Value::Str("".into())]);
 ...
 */
    ...
    m
        ...
        let mut currency: Value = self.currency(code.clone());
        if is_true(&(tag != Value::Null)) && is_true(&(((tag.len() as i64) as f64) > ((0i64) as f64))) {
    ...
    m
            ...
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("chain".to_string(), Value::Str(format!("{}{}", Value::Str(format!("{}{}", currency.as_map().and_then(|__m| __m.get("id")).cloned().unwrap_or(Value::Null), Value::Str("-".into())).into()), network).into())); }
            params = self.omit(params.clone(), Value::Str("network".into()), &[]);
        }
        ...
        if (fee == Value::Null) {
            ...
            let mut networkCodeResolved: Value = self.network_id_to_code(&[network.clone(), currency.as_map().and_then(|__m| __m.get("code")).cloned().unwrap_or(Value::Null)]);
            let mut targetNetwork: Value = (if is_true(&(networkCodeResolved == Value::Null)) { Value::Map({
    let mut m = indexmap::IndexMap::new();
    m
}) } else { self.safe_dict(currency.as_map().and_then(|__m| __m.get("networks")).cloned().unwrap_or(Value::Null), networkCodeResolved.clone(), &[Value::Map({
    let mut m = indexmap::IndexMap::new();
    m
})]) });
        ...
        if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("fee".to_string(), self.number_to_string(fee.clone())); } // withdrawals to OKCoin or OKX are fee-free, please set 0
        let mut query: Value = self.omit(params, Value::from(vec![Value::Str("fee".into())]), &[]);
        let __ws_arg_53 = self.extend(request, &[query.clone()]);
 ...
 */
    ...
    pub fn parse_transaction(&self, mut transaction: Value, optional_args: &[Value]) -> Value {
        ...
        tagTo = (if is_true(&(tagTo == Value::Null)) { self.safe_string_k(transaction.clone(), "pmtId", &[]) } else { self.safe_string2(transaction.clone(), Value::Str("pmtId".into()), tagTo.clone(), &[]) });
 ...
 */
    ...
    m
        ...
        let __ws_arg_59 = self.extend(request, &[query.clone()]);
 ...
 */
    ...
    m
        ...
        if (symbols != Value::Null) {
            ...
            if marketIdsLength > ((0i64) as f64) {
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("instId".to_string(), join(&marketIds, &Value::Str(",".into()))); }
            }
        }
    ...
    m
        ...
        let mut method: Option<String> = self.safe_string_k(fetchPositionsOptions.clone(), "method", &[Value::Str("privateGetAccountPositions".into())]).as_str().map(str::to_owned);
        let mut response: Value = Value::Null;
        ...
        return self.filter_by_array_positions(result.clone(), Value::Str("symbol".into()), &[self.market_symbols(&[symbols.clone()]), Value::Bool(false)]);
        ...
        if (market.as_map().and_then(|__m| __m.get("margin")).cloned().unwrap_or(Value::Null).as_bool() == Some(true)) {
            ...
            if (side.as_str() == Some("net")) {
                ...
                if (parsedCurrency != Value::Null) {
                    side = (if is_true(&(market.as_map().and_then(|__m| __m.get("base")).cloned().unwrap_or(Value::Null).as_str() == parsedCurrency.as_str())) { Value::Str("long".into()) } else { Value::Str("short".into()) });
                }
            }
            if (side == Value::Null) {
 ...
 */
    ...
    m
        ...
        if (fromId.as_str() == Some("master")) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("type".to_string(), Value::Str("1".into())); }
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("subAcct".to_string(), toId.clone()); }
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("from".to_string(), self.safe_string_k(params.clone(), "from", &[Value::Str("6".into())])); }
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("to".to_string(), self.safe_string_k(params.clone(), "to", &[Value::Str("6".into())])); }
        }  else if (toId.as_str() == Some("master")) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("type".to_string(), Value::Str("2".into())); }
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("subAcct".to_string(), fromId.clone()); }
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("from".to_string(), self.safe_string_k(params.clone(), "from", &[Value::Str("6".into())])); }
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("to".to_string(), self.safe_string_k(params.clone(), "to", &[Value::Str("6".into())])); }
        }
    ...
    let mut m = indexmap::IndexMap::new();
        ...
        m.insert("currency".to_string(), code.clone());
        m.insert("amount".to_string(), amount.clone());
        m.insert("fromAccount".to_string(), (if is_true(&(fromAccountId == Value::Null)) { Value::Null } else { self.safe_string(accountsById.clone(), fromAccountId.clone(), &[]) }));
        m.insert("toAccount".to_string(), (if is_true(&(toAccountId == Value::Null)) { Value::Null } else { self.safe_string(accountsById.clone(), toAccountId.clone(), &[]) }));
 ...
 */
    ...
    m
        ...
        if (api.as_str() == Some("public")) {
            ...
            if (method.as_str() == Some("POST")) && is_true(&((path.as_str() == Some("trade/batch-orders")) || (path.as_str() == Some("trade/order-algo")) || (path.as_str() == Some("trade/order")))) {
            ...
            if (method.as_str() == Some("GET")) {
                ...
                if isArray || is_true(&(((object_keys(&query).len() as i64) as f64) > ((0i64) as f64))) {
            ...
            add_element_to_object(&mut headers, &Value::Str("OK-ACCESS-SIGN".into()), signature.clone());
        }
        return Value::Map({
    let mut m = indexmap::IndexMap::new();
 ...
 */
    ...
    m
                ...
                let mut isExtendedPerpetual: bool = ruleType.as_deref() == Some("xperp"); // long-dated futures that still pay funding, e.g. ETH-USD_UM_XPERP-310404
                if is_true(&(market.as_map().and_then(|__m| __m.get("swap")).cloned().unwrap_or(Value::Null).as_bool() != Some(true))) && !isExtendedPerpetual {
                    panic!("{}", crate::exchange_errors::bad_request(format!("{}{}", Value::Str(format!("{}{}", Value::Str(format!("{}{}", self.id.clone(), Value::Str(" fetchFundingRates() symbols must be swap markets or XPERP futures, ".into())).into()), get_value(&symbols, &i)).into()), Value::Str(" is not".into()))));
                }
            }
            }
        }
 ...
 */
    ...
    m
        ...
        if (limit != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("limit".to_string(), to_string_val(&limit)); } // default 100, max 100
        }
        let mut market: Value = Value::Null;
        if (symbol != Value::Null) {
            ...
            if (market.as_map().and_then(|__m| __m.get("contract")).cloned().unwrap_or(Value::Null).as_bool() == Some(true)) {
                if (market.as_map().and_then(|__m| __m.get("linear")).cloned().unwrap_or(Value::Null).as_bool() == Some(true)) {
                    if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("ctType".to_string(), Value::Str("linear".into())); }
                    if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("ccy".to_string(), market.as_map().and_then(|__m| __m.get("quoteId")).cloned().unwrap_or(Value::Null)); }
                }  else {
                    if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("ctType".to_string(), Value::Str("inverse".into())); }
                    if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("ccy".to_string(), market.as_map().and_then(|__m| __m.get("baseId")).cloned().unwrap_or(Value::Null)); }
                }
            }
        }
        ...
        let mut result: Value = Value::from(vec![]);
        {
            ...
            let mut amount: Value = Value::Null;
            if is_true(&(balanceChange != Value::Null)) && (!is_true(&crate::precise::Precise::stringEq(&balanceChange, &Value::Str("0".into())))) {
 ...
 */
    ...
    m
        ...
        if is_true(&(lever == Value::Null)) || is_true(&(lever.as_f64().unwrap_or(f64::NAN) < ((1i64) as f64))) || is_true(&(lever.as_f64().unwrap_or(f64::NAN) > ((125i64) as f64))) {
 ...
 */
    ...
    m
        ...
        if (since != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("before".to_string(), since.clone()); }
        }
        if (limit != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("limit".to_string(), limit.clone()); }
        }
 ...
 */
    ...
    let mut m = indexmap::IndexMap::new();
        m.insert("status".to_string(), (if is_true(&(errorCode.as_deref() == Some("0"))) { Value::Str("ok".into()) } else { Value::Str("failed".into()) }));
    m
})]);
    ...
    pub fn parse_margin_modification(&self, mut data: Value, optional_args: &[Value]) -> Value {
        ...
        let mut code: Value = (if is_true(&(responseMarket.as_map().and_then(|__m| __m.get("inverse")).cloned().unwrap_or(Value::Null).as_bool() == Some(true))) { responseMarket.as_map().and_then(|__m| __m.get("base")).cloned().unwrap_or(Value::Null) } else { responseMarket.as_map().and_then(|__m| __m.get("quote")).cloned().unwrap_or(Value::Null) });
 ...
 */
    ...
    m
        ...
        let mut market: Value = self.market(symbol.clone());
        let mut type_var: Value = (if is_true(&(market.as_map().and_then(|__m| __m.get("spot")).cloned().unwrap_or(Value::Null).as_bool() == Some(true))) { Value::Str("MARGIN".into()) } else { self.convert_to_instrument_type(market.as_map().and_then(|__m| __m.get("type")).cloned().unwrap_or(Value::Null)) });
        ...
        if is_true(&(uly == Value::Null)) || is_true(&(uly.as_str() == Some(""))) {
 ...
 */
    ...
    m
        ...
        if (since != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("before".to_string(), (match (&(since), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x - y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 - *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x - *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x - y), _ => Value::Null })); }
        }
        if (limit != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("limit".to_string(), limit.clone()); }
        }
 ...
 */
    ...
    m
        ...
        if (uly != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("uly".to_string(), uly.clone()); }
        }
        let mut instFamily: Value = self.safe_string_k(params.clone(), "instFamily", &[]);
        if (instFamily != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("instFamily".to_string(), instFamily.clone()); }
        }
 ...
 */
    ...
    m
        ...
        let mut market: Value = Value::Null;
        if (is_true(&(self.markets.clone() != Value::Null)) && (in_op(&self.markets, &symbol))) || (is_true(&(self.markets_by_id.clone() != Value::Null)) && (in_op(&self.markets_by_id, &symbol))) {
            market = self.market(symbol.clone());
        ...
        let mut type_var: Value = Value::Null;
        let mut response: Value = Value::Null;
        { let __destr_tmp = self.handle_market_type_and_params(Value::Str("fetchOpenInterestHistory".into()), &[market.clone(), params.clone()]); type_var = get_value(&__destr_tmp, &Value::Int(0)); params = get_value(&__destr_tmp, &Value::Int(1)); }
        if (type_var.as_str() == Some("option")) {
    ...
    pub fn parse_open_interest(&self, mut interest: Value, optional_args: &[Value]) -> Value {
        ...
        if is_true(&(matches!(&interest, Value::Arr(_)))) {
    ...
    pub fn set_sandbox_mode(&mut self, mut enable: Value) {
        ...
        }  else if is_true(&(matches!(&self.headers, Value::Dict(__d) if __d.contains_key("x-simulated-trading")))) {
 ...
 */
    ...
    m
        ...
        if (codes != Value::Null) {
            ...
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("ccy".to_string(), join(&ids, &Value::Str(",".into()))); }
        }
 ...
 */
    ...
    m
        ...
        let mut market: Value = self.market(symbol.clone());
        let mut type_var: Value = Value::Null;
        { let __destr_tmp = self.handle_market_type_and_params(Value::Str("fetchSettlementHistory".into()), &[market.clone(), params.clone()]); type_var = get_value(&__destr_tmp, &Value::Int(0)); params = get_value(&__destr_tmp, &Value::Int(1)); }
 ...
 */
    ...
    m
        ...
        let mut marketType: Value = Value::Null;
        { let __destr_tmp = self.handle_market_type_and_params(Value::Str("fetchUnderlyingAssets".into()), &[Value::Null, params.clone()]); marketType = get_value(&__destr_tmp, &Value::Int(0)); params = get_value(&__destr_tmp, &Value::Int(1)); }
        if is_true(&(marketType == Value::Null)) || is_true(&(marketType.as_str() == Some("spot"))) {
            marketType = Value::Str("option".into());
        }
        if is_true(&(marketType.as_str() != Some("option"))) && is_true(&(marketType.as_str() != Some("swap"))) && is_true(&(marketType.as_str() != Some("future"))) {
 ...
 */
    ...
    m
        ...
        if is_true(&(symbols == Value::Null)) || is_true(&(symbolsLength.as_f64() != Some(1.0))) {
            let mut uly: Value = self.safe_string_k(params.clone(), "uly", &[]);
            if (uly != Value::Null) {
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("uly".to_string(), uly.clone()); }
            }
            ...
            if is_true(&(uly == Value::Null)) && is_true(&(instFamily == Value::Null)) {
        ...
        if (symbols != Value::Null) {
            if (symbolsLength.as_f64() == Some(1.0)) {
                ...
                let mut marketId: Value = self.safe_string_k(market.clone(), "id", &[Value::Str("".into())]);
                let mut optionParts: Value = split(&marketId, &Value::Str("-".into()));
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("uly".to_string(), crate::value::get_value_k(&market.as_map().and_then(|__m| __m.get("info")).cloned().unwrap_or(Value::Null), "uly")); }
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("instFamily".to_string(), crate::value::get_value_k(&market.as_map().and_then(|__m| __m.get("info")).cloned().unwrap_or(Value::Null), "instFamily")); }
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("expTime".to_string(), self.safe_string(optionParts.clone(), Value::Int(2), &[])); }
            }
        }
 ...
 */
    ...
    m
        ...
        if (side != Value::Null) {
            if is_true(&(side.as_str() == Some("buy"))) {
                ...
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("posSide".to_string(), side.clone()); }
            }
        }
        if (clientOrderId != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("clOrdId".to_string(), clientOrderId.clone()); }
 ...
 */
    ...
    m
        ...
        //
        let mut result: Value = self.safe_list_k(response.clone(), "data", &[Value::from(vec![])]);
        return self.parse_option_chain(result.clone(), &[Value::Null, Value::Str("instId".into())]);
 ...
 */
    ...
    m
        ...
        { let __destr_tmp = self.handle_until_option(Value::Str("after".into()), request.clone(), params.clone(), &[]); request = __destr_tmp.as_array().and_then(|__arr| __arr.get(0)).cloned().unwrap_or(Value::Null); params = __destr_tmp.as_array().and_then(|__arr| __arr.get(1)).cloned().unwrap_or(Value::Null); }
        if (since != Value::Null) {
            add_element_to_object(&mut request, &Value::Str("before".into()), since.clone());
        }
        if (limit != Value::Null) {
            add_element_to_object(&mut request, &Value::Str("limit".into()), limit.clone());
        }
        ...
        return self.parse_conversions(rows.clone(), &[code.clone(), Value::Str("baseCcy".into()), Value::Str("quoteCcy".into()), since.clone(), limit.clone()]);
    ...
    let mut m = indexmap::IndexMap::new();
        ...
        m.insert("price".to_string(), self.safe_number2(conversion.clone(), Value::Str("cnvtPx".into()), Value::Str("fillPx".into()), &[]));
        m.insert("fee".to_string(), Value::Null);
    m
});
 ...
 */
    ...
    pub fn handle_errors(&self, mut httpCode: Value, mut reason: Value, mut url: Value, mut method: Value, mut headers: Value, mut body: Value, mut response: Value, mut requestHeaders: Value, mut requestBody: Value) -> Value {
        ...
        if is_true(&(code.as_str() != Some("0"))) && is_true(&(code.as_str() != Some("2"))) {
 ...
 */
    ...
    m
        ...
        if is_true(&(since == Value::Null)) || is_true(&(since.as_f64().unwrap_or(f64::NAN) > oneWeekAgo.as_f64().unwrap_or(f64::NAN))) {
 ...
 */
    ...
    m
        ...
        if (marginMode != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("mgnMode".to_string(), marginMode.clone()); }
        }
        if (instType != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("instType".to_string(), instType.clone()); }
        }
