 ...
 */
    ...
    m
        ...
        if (reduceOnly.as_bool() == Some(true)) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("flags".to_string(), Value::List(vec![Value::Str("REDUCE_ONLY".to_string())])); }
        }
 ...
 */
    ...
    pub fn parse_balance(&self, mut response: Value) -> Value {
        ...
        });
        {
            ...
            while { if !__for_first_1048 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_1048 = false; is_less_than(&i, &get_array_length(&response)) } {
