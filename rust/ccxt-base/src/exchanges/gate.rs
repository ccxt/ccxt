 ...
 */
    ...
    let mut m = indexmap::IndexMap::new();
        ...
        m.insert("optionType".to_string(), (if is_true(&(optionType.as_str() == Some("C"))) { Value::Str("call".into()) } else { Value::Str("put".into()) }));
    ...
    pub fn safe_market(&self, optional_args: &[Value]) -> Value {
        ...
        let mut isOption: bool = is_true(&(marketId != Value::Null)) && (is_true(&(Value::Int(marketId.as_str().and_then(|__s| __s.find("-C")).map(|__i| __i as i64).unwrap_or(-1)).as_f64().unwrap_or(f64::NAN) > ((-1i64) as f64))) || is_true(&(Value::Int(marketId.as_str().and_then(|__s| __s.find("-P")).map(|__i| __i as i64).unwrap_or(-1)).as_f64().unwrap_or(f64::NAN) > ((-1i64) as f64))));
 ...
 */
    ...
    m
        ...
        let mut marginResponsespotMarketsResponseVariable = promise_all(&Value::from(vec![marginPromise.clone(), spotMarketsPromise.clone()])).await;
        ...
        let mut marginMarkets: Value = self.index_by(marginResponse.clone(), Value::Str("id".into()));
        //
        //  Spot
        //
        //     [
    ...
    m
                ...
                let mut parsedMarket: Value = self.parse_contract_market(contract.clone(), settleId.clone());
    ...
    pub fn parse_contract_market(&self, mut market: Value, mut settleId: Value) -> Value {
        ...
        let mut date: Option<String> = self.safe_string(parts.clone(), Value::Int(2), &[]).as_str().map(str::to_owned);
        let mut base: Value = self.safe_currency_code(baseId.clone(), &[]);
        let mut quote: Value = self.safe_currency_code(quoteId.clone(), &[]);
    ...
    m
                ...
                let mut optionLetter: Value = (if is_true(&(isCall.as_bool() == Some(true))) { Value::Str("C".into()) } else { Value::Str("P".into()) });
                let mut optionType: Value = (if is_true(&(isCall.as_bool() == Some(true))) { Value::Str("call".into()) } else { Value::Str("put".into()) });
    ...
    m
        ...
        return Value::from(vec![request.clone(), query.clone()]);
    ...
    pub fn get_margin_mode(&self, mut trigger: Value, mut params: Value) -> Value {
        ...
        let mut marginMode: Value = self.safe_string_lower2(params.clone(), Value::Str("marginMode".into()), Value::Str("account".into()), &[defaultMarginMode.clone()]);
    ...
    m
        ...
        let mut fetchMarketsContractOptions: Value = self.safe_dict(options.clone(), method.clone(), &[Value::Map({
    let mut m = indexmap::IndexMap::new();
    m
})]);
        let mut defaultSettle: Value = (if is_true(&(type_var.as_str() == Some("swap"))) { Value::from(vec![Value::Str("usdt".into())]) } else { Value::from(vec![Value::Str("btc".into())]) });
 ...
 */
    ...
    m
        ...
        let __ws_arg_2 = self.extend(request, &[query.clone()]);
 ...
 */
    ...
    m
        ...
        let mut request: Value = requestqueryVariable.as_array().and_then(|__arr| __arr.get(0)).cloned().unwrap_or(Value::Null);
        let mut query: Value = requestqueryVariable.as_array().and_then(|__arr| __arr.get(1)).cloned().unwrap_or(Value::Null);
        let __ws_arg_3 = self.extend(request, &[query.clone()]);
    ...
    m
        ...
        });
        {
            ...
            if is_true(&(obtainFailed.is_some())) && is_true(&(obtainFailed != Some(0))) {
                continue;
            }
 ...
 */
    ...
    m
        ...
        return self.index_by(parsed.clone(), Value::Str("network".into()));
 ...
 */
    ...
    m
        ...
        let mut chainsIndexedById: Value = chainsIndexedByIdRaw.clone();
 ...
 */
    ...
    pub fn parse_trading_fee(&self, mut info: Value, optional_args: &[Value]) -> Value {
        ...
        let mut taker: Value = (if is_true(&(gtDiscount.as_bool() == Some(true))) { Value::Str("gt_taker_fee".into()) } else { Value::Str("taker_fee".into()) });
        let mut maker: Value = (if is_true(&(gtDiscount.as_bool() == Some(true))) { Value::Str("gt_maker_fee".into()) } else { Value::Str("maker_fee".into()) });
        let mut contract: Value = self.safe_bool_k(market.clone(), "contract", &[]);
        let mut takerKey: Value = (if is_true(&(contract.as_bool() == Some(true))) { Value::Str("futures_taker_fee".into()) } else { taker.clone() });
        let mut makerKey: Value = (if is_true(&(contract.as_bool() == Some(true))) { Value::Str("futures_maker_fee".into()) } else { maker.clone() });
        return Value::Map({
    let mut m = indexmap::IndexMap::new();
 ...
 */
    ...
    m
        ...
        let mut response: Value = self.private_wallet_get_withdraw_status(&[params.clone()]).await;
        return self.parse_deposit_withdraw_fees(response.clone(), &[codes.clone(), Value::Str("currency".into())]);
 ...
 */
    ...
    m
        ...
        if (type_var.as_str() == Some("swap")) {
            ...
            let __ws_arg_8 = self.extend(request, &[requestParams.clone()]);
    ...
    pub fn parse_funding_histories(&self, mut response: Value, mut symbol: Value, mut since: Value, mut limit: Value) -> Value {
        ...
        let mut sorted: Value = self.sort_by(result.clone(), Value::Str("timestamp".into()), &[]);
 ...
 */
    ...
    m
        ...
        let mut response: Value = Value::Null;
        if is_true(&(market.as_map().and_then(|__m| __m.get("spot")).cloned().unwrap_or(Value::Null).as_bool() == Some(true))) || is_true(&(market.as_map().and_then(|__m| __m.get("margin")).cloned().unwrap_or(Value::Null).as_bool() == Some(true))) {
        ...
        let mut priceKey: Value = (if is_true(&(market.as_map().and_then(|__m| __m.get("spot")).cloned().unwrap_or(Value::Null).as_bool() == Some(true))) { Value::Int(0) } else { Value::Str("p".into()) });
        let mut amountKey: Value = (if is_true(&(market.as_map().and_then(|__m| __m.get("spot")).cloned().unwrap_or(Value::Null).as_bool() == Some(true))) { Value::Int(1) } else { Value::Str("s".into()) });
        let mut nonce: Value = self.safe_integer_k(response.clone(), "id", &[]);
        let mut result: Value = self.parse_order_book(response.clone(), symbol.clone(), &[timestamp.clone(), Value::Str("bids".into()), Value::Str("asks".into()), priceKey.clone(), amountKey.clone()]);
        add_element_to_object(&mut result, &Value::Str("nonce".into()), nonce.clone());
        return result;
 ...
 */
    ...
    pub fn parse_ticker(&self, mut ticker: Value, optional_args: &[Value]) -> Value {
        ...
        let mut marketType: Value = (if is_true(&(matches!(&ticker, Value::Dict(__d) if __d.contains_key("mark_price")))) { Value::Str("contract".into()) } else { Value::Str("spot".into()) });
        let mut symbol: Value = self.safe_symbol(marketId.clone(), &[market.clone(), Value::Str("_".into()), marketType.clone()]);
 ...
 */
    ...
    pub fn parse_balance_helper(&self, mut entry: Value) -> Value {
        ...
        if is_true(&(matches!(&entry, Value::Dict(__d) if __d.contains_key("borrowed")))) {
 ...
 */
    ...
    m
        ...
        if is_true(&isUnifiedAccount) {
            ...
            let __ws_arg_28 = self.extend(request, &[requestQuery.clone()]);
        ...
        let mut contract: bool = is_true(&(type_var.as_str() == Some("swap"))) || is_true(&(type_var.as_str() == Some("future"))) || is_true(&(type_var.as_str() == Some("option")));
        if contract {
        ...
        if is_true(&(matches!(&data, Value::Dict(__d) if __d.contains_key("balances")))) {
 ...
 */
    ...
    m
        ...
        let mut maxLimit: Value = (if is_true(&(market.as_map().and_then(|__m| __m.get("contract")).cloned().unwrap_or(Value::Null).as_bool() == Some(true))) { Value::Int(1999) } else { Value::Int(1000) });
        limit = (if is_true(&(limit == Value::Null)) { maxLimit.clone() } else { crate::runtime::Math::min(&limit, &maxLimit) });
        ...
        if (since != Value::Null) {
            ...
            let mut toTimestamp: Value = self.sum(&[request.as_map().and_then(|__m| __m.get("from")).cloned().unwrap_or(Value::Null), distance.clone()]);
            ...
            if (until != Value::Null) {
                ...
                add_element_to_object(&mut request, &Value::Str("to".into()), to.clone());
            }
        }  else {
            if (until != Value::Null) {
                add_element_to_object(&mut request, &Value::Str("to".into()), until.clone());
            }
            add_element_to_object(&mut request, &Value::Str("limit".into()), limit.clone());
        }
 ...
 */
    ...
    m
        ...
        if (since != Value::Null) && is_true(&(market.as_map().and_then(|__m| __m.get("contract")).cloned().unwrap_or(Value::Null).as_bool() == Some(true))) {
            add_element_to_object(&mut request, &Value::Str("from".into()), self.parse_to_int((match ((since).as_f64(), (Value::Int(1000)).as_f64()) { (Some(x), Some(y)) if y != 0.0 => Value::Float(x / y), _ => Value::Null })));
        }
        let mut response: Value = Value::Null;
 ...
 */
    ...
    m
        ...
        let mut market: Value = (if is_true(&(symbol != Value::Null)) { self.market(symbol) } else { Value::Null });
        let mut until: Value = self.safe_integer_k(params.clone(), "until", &[]);
        ...
        if contract {
            ...
            add_element_to_object(&mut request, &Value::Str("account".into()), marginMode.clone());
        }
        if (limit != Value::Null) {
    ...
    pub fn parse_trade(&self, mut trade: Value, optional_args: &[Value]) -> Value {
        ...
        let mut marketType: Value = (if is_true(&(matches!(&trade, Value::Dict(__d) if __d.contains_key("contract")))) { Value::Str("contract".into()) } else { Value::Str("spot".into()) });
        market = self.safe_market(&[marketId.clone(), market.clone(), Value::Str("_".into()), marketType.clone()]);
        ...
        let mut side: Value = self.safe_string2(trade.clone(), Value::Str("side".into()), Value::Str("type".into()), &[contractSide.clone()]);
 ...
 */
    ...
    m
        ...
        if (code != Value::Null) {
            ...
            add_element_to_object(&mut request, &Value::Str("currency".into()), currency.as_map().and_then(|__m| __m.get("id")).cloned().unwrap_or(Value::Null)); // todo: currencies have network-junctions
        }
        if (limit != Value::Null) {
            add_element_to_object(&mut request, &Value::Str("limit".into()), limit.clone());
        }
 ...
 */
    ...
    m
        ...
        let mut nonTriggerOrder: bool = !isTpsl && is_true(&(trigger == Value::Null));
    ...
    m
            ...
            let mut triggerValue: Value = self.safe_value_n(orderParams.clone(), Value::from(vec![Value::Str("triggerPrice".into()), Value::Str("stopPrice".into()), Value::Str("takeProfitPrice".into()), Value::Str("stopLossPrice".into())]), &[]);
        ...
        if is_true(&(market.as_map().and_then(|__m| __m.get("future")).cloned().unwrap_or(Value::Null).as_bool() == Some(true))) || is_true(&(market.as_map().and_then(|__m| __m.get("option")).cloned().unwrap_or(Value::Null).as_bool() == Some(true))) {
 ...
 */
    ...
    m
        ...
        if isMarketOrder {
            if is_true(&(timeInForce.as_str() == Some("poc"))) || is_true(&(timeInForce.as_str() == Some("gtc"))) {
                ...
                if (timeInForce == Value::Null) {
                    ...
                    let mut exchangeSpecificTif: Value = self.safe_string(self.options.as_map().and_then(|__m| __m.get("timeInForce")).cloned().unwrap_or(Value::Null), defaultTif.clone(), &[Value::Str("ioc".into())]);
        ...
        if (contract.as_bool() == Some(true)) {
            ...
            if is_equal(&isClose, &Value::Bool(true)) {
                ...
                let mut signedAmount: Value = (if is_true(&(side.as_str() == Some("sell"))) { crate::precise::Precise::stringNeg(&amountToPrecision) } else { amountToPrecision.clone() });
        ...
        if nonTriggerOrder {
            if (contract.as_bool() == Some(true)) {
                ...
                if isMarketOrder {
                    ...
                    add_element_to_object(&mut request, &Value::Str("price".into()), (if is_true(&(price.as_f64() == Some(0.0))) { Value::Str("0".into()) } else { self.price_to_precision(symbol.clone(), price.clone()) }));
                }
                ...
                if isMarketOrder && is_true(&(side.as_str() == Some("buy"))) {
                    ...
                    add_element_to_object(&mut request, &Value::Str("amount".into()), quoteAmount.clone());
                }  else {
    ...
    m
                ...
                if (type_var.as_str() == Some("market")) {
                    ...
                    add_element_to_object(get_value_mut(&mut request, &Value::Str("initial".into())), &Value::Str("price".into()), (if is_true(&(price.as_f64() == Some(0.0))) { Value::Str("0".into()) } else { self.price_to_precision(symbol.clone(), price.clone()) }));
                }
                if (trigger == Value::Null) {
                    ...
                    if isStopLossOrder {
                        ...
                        rule = (if is_true(&(side.as_str() == Some("buy"))) { Value::Int(1) } else { Value::Int(2) });
                        triggerOrderPrice = self.price_to_precision(symbol.clone(), stopLossPrice.clone());
                    }  else if isTakeProfitOrder {
                        rule = (if is_true(&(side.as_str() == Some("buy"))) { Value::Int(2) } else { Value::Int(1) });
    ...
    m
                ...
                if (reduceOnly != Value::Null) {
                    add_element_to_object(get_value_mut(&mut request, &Value::Str("initial".into())), &Value::Str("reduce_only".into()), reduceOnly.clone());
                }
                if (timeInForce != Value::Null) {
    ...
    m
                ...
                if (trigger == Value::Null) {
                    ...
                    if isStopLossOrder {
                        // we let trigger orders be aliases for stopLoss orders because
                        // gateio doesn't accept conventional trigger orders for spot markets
                        rule = (if is_true(&(side.as_str() == Some("buy"))) { Value::Str(">=".into()) } else { Value::Str("<=".into()) });
                        triggerOrderPrice = self.price_to_precision(symbol.clone(), stopLossPrice.clone());
                    }  else if isTakeProfitOrder {
                        rule = (if is_true(&(side.as_str() == Some("buy"))) { Value::Str("<=".into()) } else { Value::Str(">=".into()) });
                        triggerOrderPrice = self.price_to_precision(symbol.clone(), takeProfitPrice.clone());
    ...
    m
                    ...
                    if (clientOrderId != Value::Null) {
                        add_element_to_object(get_value_mut(&mut request, &Value::Str("trigger".into())), &Value::Str("text".into()), clientOrderId.clone());
                    }
                }
            }
        }
 ...
 */
    ...
    m
        ...
        return self.create_order(symbol.clone(), Value::Str("market".into()), Value::Str("buy".into()), cost.clone(), &[Value::Null, params.clone()]).await;
 ...
 */
    ...
    m
        ...
        if is_true(&(triggerPrice != Value::Null)) && is_true(&(triggerPrice.as_f64() != Some(0.0))) {
            remainingString = amount.clone();
            cost = Value::Str("0".into());
        }
        if is_true(&(contract != Value::Null)) && is_true(&(contract.as_str() != Some(""))) {
            let mut isMarketOrder: bool = is_true(&crate::precise::Precise::stringEquals(&price, &Value::Str("0".into()))) && is_true(&(timeInForce.as_str() == Some("IOC")));
        ...
        if is_true(&(matches!(&order, Value::Dict(__d) if __d.contains_key("currency_pair")))) || is_true(&(matches!(&order, Value::Dict(__d) if __d.contains_key("market")))) {
            marketType = Value::Str("spot".into());
        }
        let mut exchangeSymbol: Value = self.safe_string2(order.clone(), Value::Str("currency_pair".into()), Value::Str("market".into()), &[contract.clone()]);
        let mut symbol: Value = self.safe_symbol(exchangeSymbol.clone(), &[market.clone(), Value::Str("_".into()), marketType.clone()]);
        let mut fees: Value = Value::from(vec![]);
        ...
        if is_true(&(account.as_deref() == Some("spot"))) || is_true(&(account.as_deref() == Some("unified"))) {
    ...
    m
        ...
        if (clientOrderId == Value::Null) {
            if is_true(&(matches!(&order, Value::Dict(__d) if __d.contains_key("initial")))) {
                clientOrderId = self.safe_string(order.as_map().and_then(|__m| __m.get("initial")).cloned().unwrap_or(Value::Null), Value::Str("text".into()), &[]);
            }  else if is_true(&(matches!(&order, Value::Dict(__d) if __d.contains_key("trigger")))) {
    ...
    let mut m = indexmap::IndexMap::new();
        ...
        m.insert("fees".to_string(), (if multipleFeeCurrencies { fees.clone() } else { Value::from(vec![]) }));
    ...
    m
        ...
        let mut market: Value = (if is_true(&(symbol == Value::Null)) { Value::Null } else { self.market(symbol) });
        ...
        let mut query: Value = type_varqueryVariable.as_array().and_then(|__arr| __arr.get(1)).cloned().unwrap_or(Value::Null);
        let mut contract: bool = is_true(&(type_var.as_str() == Some("swap"))) || is_true(&(type_var.as_str() == Some("future"))) || is_true(&(type_var.as_str() == Some("option")));
        let mut requestrequestParamsVariable = (if contract { self.prepare_request(&[market.clone(), type_var.clone(), query.clone()]) } else { self.spot_order_prepare_request(&[market.clone(), trigger.clone(), query.clone()]) });
        let mut request: Value = requestrequestParamsVariable.as_array().and_then(|__arr| __arr.get(0)).cloned().unwrap_or(Value::Null);
        let mut requestParams: Value = requestrequestParamsVariable.as_array().and_then(|__arr| __arr.get(1)).cloned().unwrap_or(Value::Null);
 ...
 */
    ...
    m
        ...
        self.load_unified_status(&[]).await;
        let mut market: Value = (if is_true(&(symbol == Value::Null)) { Value::Null } else { self.market(symbol.clone()) });
        let mut result: Value = self.handle_market_type_and_params(Value::Str("fetchOrder".into()), &[market.clone(), params.clone()]);
        let mut type_var: Option<String> = self.safe_string(result.clone(), Value::Int(0), &[]).as_str().map(str::to_owned);
        let mut trigger: Value = self.safe_bool_n(params.clone(), Value::from(vec![Value::Str("trigger".into()), Value::Str("is_stop_order".into()), Value::Str("stop".into())]), &[Value::Bool(false)]);
 ...
 */
    ...
    m
        ...
        if !is_true(&useHistorical) && (is_true(&((since == Value::Null) && (until == Value::Null))) || is_true(&(type_var.as_str() != Some("swap")))) {
    ...
    m
        ...
        let mut spot: bool = is_true(&(type_var.as_str() == Some("spot"))) || is_true(&(type_var.as_str() == Some("margin")));
        let mut request: Value = Value::Map({
            let mut m = indexmap::IndexMap::new();
            m
        });
        ...
        if (lastId != Value::Null) {
            add_element_to_object(&mut request, &Value::Str("last_id".into()), lastId.clone());
        }
        return Value::from(vec![request.clone(), finalParams.clone()]);
    ...
    m
        ...
        let mut type_var: Option<String> = self.safe_string(res.clone(), Value::Int(0), &[]).as_str().map(str::to_owned);
        ...
        let mut request: Value = requestrequestParamsVariable.as_array().and_then(|__arr| __arr.get(0)).cloned().unwrap_or(Value::Null);
        let mut requestParams: Value = requestrequestParamsVariable.as_array().and_then(|__arr| __arr.get(1)).cloned().unwrap_or(Value::Null);
        let mut spot: bool = is_true(&(type_var.as_deref() == Some("spot"))) || is_true(&(type_var.as_deref() == Some("margin")));
        let mut openStatus: bool = status.as_str() == Some("open");
        let mut openSpotOrders: bool = spot && openStatus && is_true(&(trigger.as_bool() != Some(true)));
        let mut response: Value = Value::Null;
        if spot {
            if (trigger.as_bool() != Some(true)) {
                if openStatus {
 ...
 */
    ...
    m
        ...
        let mut type_var: Value = type_varqueryVariable.as_array().and_then(|__arr| __arr.get(0)).cloned().unwrap_or(Value::Null);
        let mut query: Value = type_varqueryVariable.as_array().and_then(|__arr| __arr.get(1)).cloned().unwrap_or(Value::Null);
        let mut requestrequestParamsVariable = (if is_true(&((type_var.as_str() == Some("spot")) || (type_var.as_str() == Some("margin")))) { self.spot_order_prepare_request(&[market.clone(), trigger.clone(), query.clone()]) } else { self.prepare_request(&[market.clone(), type_var.clone(), query.clone()]) });
        let mut request: Value = requestrequestParamsVariable.as_array().and_then(|__arr| __arr.get(0)).cloned().unwrap_or(Value::Null);
        let mut requestParams: Value = requestrequestParamsVariable.as_array().and_then(|__arr| __arr.get(1)).cloned().unwrap_or(Value::Null);
 ...
 */
    ...
    m
        ...
        let mut type_var: Value = Value::Null;
        let mut defaultSettle: Value = (if is_true(&(market == Value::Null)) { Value::Str("usdt".into()) } else { market.as_map().and_then(|__m| __m.get("settle")).cloned().unwrap_or(Value::Null) });
        let mut settle: Value = self.safe_string_lower(params.clone(), Value::Str("settle".into()), &[defaultSettle.clone()]);
        ...
        if isSpot && is_true(&(symbol == Value::Null)) {
 ...
 */
    ...
    m
        ...
        let mut type_var: Value = type_varqueryVariable.as_array().and_then(|__arr| __arr.get(0)).cloned().unwrap_or(Value::Null);
        let mut query: Value = type_varqueryVariable.as_array().and_then(|__arr| __arr.get(1)).cloned().unwrap_or(Value::Null);
        let mut requestrequestParamsVariable = (if is_true(&(type_var.as_str() == Some("spot"))) { self.multi_order_spot_prepare_request(&[market.clone(), trigger.clone(), query.clone()]) } else { self.prepare_request(&[market.clone(), type_var.clone(), query.clone()]) });
        let mut request: Value = requestrequestParamsVariable.as_array().and_then(|__arr| __arr.get(0)).cloned().unwrap_or(Value::Null);
        let mut requestParams: Value = requestrequestParamsVariable.as_array().and_then(|__arr| __arr.get(1)).cloned().unwrap_or(Value::Null);
        let mut response: Value = Value::Null;
        if (type_var.as_str() == Some("spot")) || (type_var.as_str() == Some("margin")) {
 ...
 */
    ...
    m
        ...
        if !(in_op(&self.options.as_map().and_then(|__m| __m.get("accountsByType")).cloned().unwrap_or(Value::Null), &fromId)) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("from".to_string(), Value::Str("margin".into())); }
        ...
        if !(in_op(&self.options.as_map().and_then(|__m| __m.get("accountsByType")).cloned().unwrap_or(Value::Null), &toId)) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("to".to_string(), Value::Str("margin".into())); }
        ...
        if is_true(&(toId.as_str() == Some("futures"))) || is_true(&(toId.as_str() == Some("delivery"))) || is_true(&(fromId.as_str() == Some("futures"))) || is_true(&(fromId.as_str() == Some("delivery"))) {
 ...
 */
    ...
    m
        ...
        if is_true(&(leverage.as_f64().unwrap_or(f64::NAN) < ((0i64) as f64))) || is_true(&(leverage.as_f64().unwrap_or(f64::NAN) > ((100i64) as f64))) {
    ...
    pub fn parse_position(&self, mut position: Value, optional_args: &[Value]) -> Value {
        ...
        market = self.safe_market(&[contract.clone(), market.clone(), Value::Str("_".into()), Value::Str("contract".into())]);
        ...
        if is_true(&(marginBalance != Value::Null)) && is_true(&(unrealisedPnl != Value::Null)) {
 ...
 */
    ...
    m
        ...
        if is_true(&(type_var == Value::Null)) || is_true(&(type_var.as_str() == Some("spot"))) {
        ...
        return self.parse_positions(responseList.clone(), &[symbols.clone()]);
 ...
 */
    ...
    m
        ...
        return self.parse_leverage_tiers(response.clone(), &[symbols.clone(), Value::Str("name".into())]);
 ...
 */
    ...
    pub fn parse_market_leverage_tiers(&self, mut info: Value, optional_args: &[Value]) -> Value {
        ...
        //     ]
        //
        if !is_true(&(matches!(&info, Value::Arr(_)))) {
 ...
 */
    ...
    m
        ...
        let mut market: Value = self.market(symbol.clone());
        if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("currency_pair".to_string(), market.as_map().and_then(|__m| __m.get("id")).cloned().unwrap_or(Value::Null)); }
        if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("type".to_string(), Value::Str("repay".into())); }
 ...
 */
    ...
    m
        ...
        let mut market: Value = self.market(symbol.clone());
        if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("currency_pair".to_string(), market.as_map().and_then(|__m| __m.get("id")).cloned().unwrap_or(Value::Null)); }
        if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("type".to_string(), Value::Str("borrow".into())); }
 ...
 */
    ...
    pub fn parse_borrow_interest(&self, mut info: Value, optional_args: &[Value]) -> Value {
        ...
        let mut marginMode: Value = (if is_true(&(marketId != Value::Null)) { Value::Str("isolated".into()) } else { Value::Str("cross".into()) });
        let mut timestamp: Value = self.safe_integer_k(info.clone(), "create_time", &[]);
        return Value::Map({
    let mut m = indexmap::IndexMap::new();
        m.insert("info".to_string(), info.clone());
    ...
    m
        ...
        }  else if is_true(&(matches!(&params, Value::Arr(_)))) {
    ...
    m
        ...
        let mut endPart: Value = (if is_true(&(path.as_str() == Some(""))) { Value::Str("".into()) } else { (add(&Value::Str("/".into()), &path)) });
        let mut entirePath: Value = Value::Str(format!("{}{}", add(&Value::Str("/".into()), &type_var), endPart).into());
        if is_true(&(type_var.as_str() == Some("subAccounts"))) || is_true(&(type_var.as_str() == Some("withdrawals"))) {
        ...
        if (authentication.as_str() == Some("public")) {
            ...
            if (is_true(&(type_var.as_str() == Some("futures"))) || is_true(&(type_var.as_str() == Some("delivery")))) && (method.as_str() == Some("POST")) {
            ...
            if is_true(&(method.as_str() == Some("GET"))) || is_true(&(method.as_str() == Some("DELETE"))) || requiresURLEncoding || is_true(&(method.as_str() == Some("PATCH"))) {
                if ((object_keys(&query).len() as i64) as f64) > ((0i64) as f64) {
    ...
    m
            ...
            let mut bodyPayload: Value = (if is_true(&(body == Value::Null)) { Value::Str("".into()) } else { body.clone() });
            let mut bodySignature: Value = self.hash(self.encode(bodyPayload.clone()), Value::Str("sha512".into()), &[]);
            ...
            let mut payloadArray: Value = Value::from(vec![to_upper(&method), signaturePath.clone(), rawQueryString.clone(), bodySignature.clone(), timestampString.clone()]);
 ...
 */
    ...
    m
        ...
        let mut sorted: Value = self.sort_by(settlements.clone(), Value::Str("timestamp".into()), &[]);
        return self.filter_by_symbol_since_limit(sorted.clone(), &[symbol.clone(), since.clone(), limit.clone()]);
 ...
 */
    ...
    m
        ...
        if is_true(&(type_var.as_str() == Some("spot"))) || is_true(&(type_var.as_str() == Some("margin"))) {
            if (code != Value::Null) {
                currency = self.currency(code.clone());
                add_element_to_object(&mut request, &Value::Str("currency".into()), currency.as_map().and_then(|__m| __m.get("id")).cloned().unwrap_or(Value::Null)); // todo: currencies have network-junctions
            }
        }
        if is_true(&(type_var.as_str() == Some("swap"))) || is_true(&(type_var.as_str() == Some("future"))) {
            let mut defaultSettle: Value = (if is_true(&(type_var.as_str() == Some("swap"))) { Value::Str("usdt".into()) } else { Value::Str("btc".into()) });
            let mut settle: Value = self.safe_string_lower(params.clone(), Value::Str("settle".into()), &[defaultSettle.clone()]);
            params = self.omit(params.clone(), Value::Str("settle".into()), &[]);
            add_element_to_object(&mut request, &Value::Str("settle".into()), settle.clone());
        }
        if (since != Value::Null) {
            add_element_to_object(&mut request, &Value::Str("from".into()), since.clone());
        }
 ...
 */
    ...
    m
        ...
        let mut request: Value = requestqueryVariable.as_array().and_then(|__arr| __arr.get(0)).cloned().unwrap_or(Value::Null);
        let mut query: Value = requestqueryVariable.as_array().and_then(|__arr| __arr.get(1)).cloned().unwrap_or(Value::Null);
        add_element_to_object(&mut request, &Value::Str("dual_mode".into()), hedged.clone());
 ...
 */
    ...
    m
        ...
        if is_true(&(marketType == Value::Null)) || is_true(&(marketType.as_str() == Some("spot"))) {
 ...
 */
    ...
    m
        ...
        return self.parse_leverages(self.to_array(response.clone()), &[symbols.clone(), marketIdRequest.clone(), Value::Str("spot".into())]);
 ...
 */
    ...
    m
        ...
        return self.parse_positions(responseList.clone(), &[symbols.clone(), params.clone()]);
    ...
    pub fn handle_errors(&self, mut code: Value, mut reason: Value, mut url: Value, mut method: Value, mut headers: Value, mut body: Value, mut response: Value, mut requestHeaders: Value, mut requestBody: Value) -> Value {
        ...
        if (label != Value::Null) {
            ...
            self.throw_exactly_matched_exception(self.exceptions.as_map().and_then(|__m| __m.get("exact")).cloned().unwrap_or(Value::Null), label.clone(), feedback.clone());
