 ...
 */
    ...
    m
            ...
            let mut createdTs: Value = (if (buyStart.as_f64() != Some(0.0)) { buyStart.clone() } else { Value::Null });
            let mut active: Value = Value::Bool((tradeStatus.as_deref() == Some("tradable")) || (is_true(&margin) && (marginStatus == Some(1))));
    ...
    m
                ...
                let mut optionLetter: Value = (if (isCall.as_bool() == Some(true)) { Value::Str("C".to_string()) } else { Value::Str("P".to_string()) });
                let mut optionType: Value = (if (isCall.as_bool() == Some(true)) { Value::Str("call".to_string()) } else { Value::Str("put".to_string()) });
    ...
    m
        ...
        let mut defaultSettle: Value = (if (type_var.as_str() == Some("swap")) { Value::from(vec![Value::Str("usdt".to_string())]) } else { Value::from(vec![Value::Str("btc".to_string())]) });
 ...
 */
    ...
    pub fn parse_trading_fee(&self, mut info: Value, optional_args: &[Value]) -> Value {
        ...
        let mut taker: Value = (if (gtDiscount.as_bool() == Some(true)) { Value::Str("gt_taker_fee".to_string()) } else { Value::Str("taker_fee".to_string()) });
        let mut maker: Value = (if (gtDiscount.as_bool() == Some(true)) { Value::Str("gt_maker_fee".to_string()) } else { Value::Str("maker_fee".to_string()) });
        let mut contract: Value = self.safe_bool_k(market.clone(), "contract", &[]);
        let mut takerKey: Value = (if (contract.as_bool() == Some(true)) { Value::Str("futures_taker_fee".to_string()) } else { taker.clone() });
        let mut makerKey: Value = (if (contract.as_bool() == Some(true)) { Value::Str("futures_maker_fee".to_string()) } else { maker.clone() });
        return Value::Map({
    let mut m = indexmap::IndexMap::new();
 ...
 */
    ...
    m
        ...
        let mut priceKey: Value = (if (market.as_map().and_then(|__m| __m.get("spot")).cloned().unwrap_or(Value::Null).as_bool() == Some(true)) { Value::Int(0) } else { Value::Str("p".to_string()) });
        let mut amountKey: Value = (if (market.as_map().and_then(|__m| __m.get("spot")).cloned().unwrap_or(Value::Null).as_bool() == Some(true)) { Value::Int(1) } else { Value::Str("s".to_string()) });
 ...
 */
    ...
    pub fn parse_ticker(&self, mut ticker: Value, optional_args: &[Value]) -> Value {
        ...
        let mut marketType: Value = (if (matches!(&ticker, Value::Dict(__d) if __d.contains_key("mark_price"))) { Value::Str("contract".to_string()) } else { Value::Str("spot".to_string()) });
 ...
 */
    ...
    pub fn parse_trade(&self, mut trade: Value, optional_args: &[Value]) -> Value {
        ...
        let mut marketType: Value = (if (matches!(&trade, Value::Dict(__d) if __d.contains_key("contract"))) { Value::Str("contract".to_string()) } else { Value::Str("spot".to_string()) });
 ...
 */
        ...
        if nonTriggerOrder {
            if (contract.as_bool() == Some(true)) {
                ...
                if isMarketOrder {
                    ...
                    add_element_to_object(&mut request, &Value::Str("price".to_string()), (if (price.as_f64() == Some(0.0)) { Value::Str("0".to_string()) } else { self.price_to_precision(symbol.clone(), price.clone()) }));
                }
    ...
    m
                ...
                if (type_var.as_str() == Some("market")) {
                    ...
                    add_element_to_object(get_value_mut(&mut request, &Value::Str("initial".to_string())), &Value::Str("price".to_string()), (if (price.as_f64() == Some(0.0)) { Value::Str("0".to_string()) } else { self.price_to_precision(symbol.clone(), price.clone()) }));
                }
    ...
    m
                ...
                if (trigger == Value::Null) {
                    ...
                    if isStopLossOrder {
                        // we let trigger orders be aliases for stopLoss orders because
                        // gateio doesn't accept conventional trigger orders for spot markets
                        rule = (if (side.as_str() == Some("buy")) { Value::Str(">=".to_string()) } else { Value::Str("<=".to_string()) });
                        triggerOrderPrice = self.price_to_precision(symbol.clone(), stopLossPrice.clone());
                    }  else if isTakeProfitOrder {
                        rule = (if (side.as_str() == Some("buy")) { Value::Str("<=".to_string()) } else { Value::Str(">=".to_string()) });
                        triggerOrderPrice = self.price_to_precision(symbol.clone(), takeProfitPrice.clone());
 ...
 */
    ...
    m
        ...
        if (contract != Value::Null) && (contract.as_str() != Some("")) {
            let mut isMarketOrder: bool = is_true(&crate::precise::Precise::stringEquals(&price, &Value::Str("0".to_string()))) && (timeInForce.as_str() == Some("IOC"));
 ...
 */
    ...
    m
        ...
        let mut type_var: Value = Value::Null;
        let mut defaultSettle: Value = (if (market == Value::Null) { Value::Str("usdt".to_string()) } else { market.as_map().and_then(|__m| __m.get("settle")).cloned().unwrap_or(Value::Null) });
        let mut settle: Value = self.safe_string_lower(params.clone(), Value::Str("settle".to_string()), &[defaultSettle.clone()]);
 ...
 */
    ...
    pub fn parse_borrow_interest(&self, mut info: Value, optional_args: &[Value]) -> Value {
        ...
        let mut marginMode: Value = (if (marketId != Value::Null) { Value::Str("isolated".to_string()) } else { Value::Str("cross".to_string()) });
        let mut timestamp: Value = self.safe_integer_k(info.clone(), "create_time", &[]);
        return Value::Map({
    let mut m = indexmap::IndexMap::new();
        m.insert("info".to_string(), info.clone());
    ...
    m
        ...
        let mut endPart: Value = (if (path.as_str() == Some("")) { Value::Str("".to_string()) } else { (add(&Value::Str("/".to_string()), &path)) });
    ...
    m
            ...
            let mut bodyPayload: Value = (if (body == Value::Null) { Value::Str("".to_string()) } else { body.clone() });
 ...
 */
    ...
    m
        ...
        if (type_var.as_str() == Some("swap")) || (type_var.as_str() == Some("future")) {
            let mut defaultSettle: Value = (if (type_var.as_str() == Some("swap")) { Value::Str("usdt".to_string()) } else { Value::Str("btc".to_string()) });
            let mut settle: Value = self.safe_string_lower(params.clone(), Value::Str("settle".to_string()), &[defaultSettle.clone()]);
            params = self.omit(params.clone(), Value::Str("settle".to_string()), &[]);
