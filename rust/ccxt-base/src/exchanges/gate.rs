 ...
 */
    ...
    m
        ...
        if !is_true(&trigger) {
            ...
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("account".to_string(), marginMode.clone()); }
 ...
 */
    ...
    pub fn parse_trading_fee(&self, mut info: Value, optional_args: &[Value]) -> Value {
        ...
        let mut takerKey: Value = (if is_true(&(contract.as_bool() == Some(true))) { Value::Str("futures_taker_fee".to_string()) } else { taker.clone() });
        let mut makerKey: Value = (if is_true(&(contract.as_bool() == Some(true))) { Value::Str("futures_maker_fee".to_string()) } else { maker.clone() });
        return Value::Map({
    let mut m = indexmap::IndexMap::new();
 ...
 */
    ...
    m
        ...
        if (tag != Value::Null) {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("memo".to_string(), tag.clone()); }
        }
        let mut networkCode: Value = Value::Null;
        { let __destr_tmp = self.handle_network_code_and_params(params.clone()); networkCode = __destr_tmp.as_array().and_then(|__arr| __arr.get(0)).cloned().unwrap_or(Value::Null); params = __destr_tmp.as_array().and_then(|__arr| __arr.get(1)).cloned().unwrap_or(Value::Null); }
        if (networkCode != Value::Null) {
 ...
 */
    ...
    m
            ...
            let mut triggerValue: Value = self.safe_value_n(orderParams.clone(), Value::from(vec![Value::Str("triggerPrice".to_string()), Value::Str("stopPrice".to_string()), Value::Str("takeProfitPrice".to_string()), Value::Str("stopLossPrice".to_string())]), &[]);
        ...
        if isMarketOrder {
            if is_true(&(timeInForce.as_str() == Some("poc"))) || is_true(&(timeInForce.as_str() == Some("gtc"))) {
                ...
                if (timeInForce == Value::Null) {
                    ...
                    let mut exchangeSpecificTif: Value = self.safe_string(self.options.as_map().and_then(|__m| __m.get("timeInForce")).cloned().unwrap_or(Value::Null), defaultTif.clone(), &[Value::Str("ioc".to_string())]);
 ...
 */
    ...
    m
        ...
        if !(in_op(&self.options.as_map().and_then(|__m| __m.get("accountsByType")).cloned().unwrap_or(Value::Null), &fromId)) {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("from".to_string(), Value::Str("margin".to_string())); }
        ...
        if !(in_op(&self.options.as_map().and_then(|__m| __m.get("accountsByType")).cloned().unwrap_or(Value::Null), &toId)) {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("to".to_string(), Value::Str("margin".to_string())); }
 ...
 */
    ...
    m
        ...
        let mut market: Value = self.market(symbol.clone());
        if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("currency_pair".to_string(), market.as_map().and_then(|__m| __m.get("id")).cloned().unwrap_or(Value::Null)); }
        if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("type".to_string(), Value::Str("repay".to_string())); }
 ...
 */
    ...
    m
        ...
        let mut market: Value = self.market(symbol.clone());
        if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("currency_pair".to_string(), market.as_map().and_then(|__m| __m.get("id")).cloned().unwrap_or(Value::Null)); }
        if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("type".to_string(), Value::Str("borrow".to_string())); }
 ...
 */
    ...
    m
        ...
        return self.parse_leverages(self.to_array(response.clone()), &[symbols.clone(), marketIdRequest.clone(), Value::Str("spot".to_string())]);
