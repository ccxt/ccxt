 ...
 */
    ...
    m
        ...
        //            ...
        //
        let mut promises: Value = Value::from(vec![marketsPromise.clone()]);
 ...
 */
    ...
    m
        ...
        if limit.as_f64().unwrap_or(f64::NAN) <= ((500i64) as f64) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("depth".to_string(), self.find_nearest_ceiling(Value::from(vec![Value::Int(10), Value::Int(50), Value::Int(100), Value::Int(500)]), limit.clone())); }
        }
    ...
    m
        ...
        return self.parse_order_book(result.clone(), self.safe_symbol(marketId.clone(), &[]), &[timestamp.clone(), Value::Str("bids".into()), Value::Str("asks".into()), Value::Str("price".into()), Value::Str("size".into())]);
 ...
 */
    ...
    pub fn parse_trade(&self, mut trade: Value, optional_args: &[Value]) -> Value {
        ...
        if (isTakerBuyer != Value::Null) {
            side = (if is_true(&isTakerBuyer) { Value::Str("buy".into()) } else { Value::Str("sell".into()) });
 ...
 */
    ...
    m
        ...
        add_element_to_object(&mut request, &Value::Str("type".into()), self.safe_string(priceTypeMap.clone(), selectedPriceType.clone(), &[]));
        if (limit != Value::Null) {
            add_element_to_object(&mut request, &Value::Str("limit".into()), crate::runtime::Math::min(&limit, &Value::Int(1000)));
        }
        { let __destr_tmp = self.handle_until_option_string(Value::Str("end_time".into()), request.clone(), params.clone(), &[Value::Int(1000000)]); request = __destr_tmp.as_array().and_then(|__arr| __arr.get(0)).cloned().unwrap_or(Value::Null); params = __destr_tmp.as_array().and_then(|__arr| __arr.get(1)).cloned().unwrap_or(Value::Null); }
 ...
 */
    ...
    m
        ...
        if (code != Value::Null) {
            ...
            add_element_to_object(&mut request, &Value::Str("currency".into()), Value::from(vec![currency.as_map().and_then(|__m| __m.get("code")).cloned().unwrap_or(Value::Null)]));
        }
        if (limit != Value::Null) {
            add_element_to_object(&mut request, &Value::Str("limit".into()), crate::runtime::Math::min(&limit, &Value::Int(1000)));
        }
        ...
        if (useTransfersEndpoint.as_bool() == Some(true)) {
            ...
            let mut filteredResults: Value = self.filter_transfers_by_type(transfers.clone(), Value::Str("deposit".into()), &[Value::Bool(true)]);
 ...
 */
    ...
    m
        ...
        let mut currency: Value = Value::Null;
        if (code == Value::Null) {
            add_element_to_object(&mut request, &Value::Str("currency".into()), Value::Null);
        }  else {
            currency = self.currency(code.clone());
            add_element_to_object(&mut request, &Value::Str("currency".into()), Value::from(vec![currency.as_map().and_then(|__m| __m.get("code")).cloned().unwrap_or(Value::Null)]));
        }
        ...
        if (useTransfersEndpoint.as_bool() == Some(true)) {
            ...
            let mut filteredResults: Value = self.filter_transfers_by_type(transfers.clone(), Value::Str("withdrawal".into()), &[Value::Bool(true)]);
            let mut transactions: Value = self.get_list_from_object_values(get_value(&filteredResults, &Value::Int(0)), Value::Str("info".into()));
            return self.parse_transactions(transactions.clone(), &[currency.clone(), since.clone(), limit.clone()]);
        }  else {
 ...
 */
    ...
    m
        ...
        if is_true(&paginate) {
            return self.fetch_paginated_call_dynamic(Value::Str("fetchTransfers".into()), &[Value::Null, since.clone(), limit.clone(), params.clone(), maxLimit.clone()]).await;
        }
        if (limit != Value::Null) {
            add_element_to_object(&mut request, &Value::Str("limit".into()), crate::runtime::Math::min(&limit, &Value::Int(1000)));
        }
        ...
        let mut rows: Value = self.safe_list_k(response, "result", &[Value::from(vec![])]);
        let mut transfers: Value = self.parse_transfers(rows.clone(), &[currency.clone(), since.clone(), limit.clone()]);
        let mut filteredResults: Value = self.filter_transfers_by_type(transfers.clone(), Value::Str("internal".into()), &[Value::Bool(false)]);
 ...
 */
    ...
    m
        ...
        add_element_to_object(get_value_mut(&mut request, &Value::Str("signature".into())), &Value::Str("chain_id".into()), networkId.clone());
        request = self.create_signed_request(request.clone(), Value::Str("EIP712_WITHDRAWAL_TYPE".into()), &[currency.clone()]);
        let __ws_arg_15 = self.extend(request, &[query.clone()]);
 ...
 */
    ...
    m
        ...
        if (price != Value::Null) {
            add_element_to_object(&mut orderLeg, &Value::Str("limit_price".into()), self.price_to_precision(symbol.clone(), price.clone()));
        }  else {
            add_element_to_object(&mut orderLeg, &Value::Str("limit_price".into()), Value::Null);
        }
        if (side.as_str() == Some("sell")) {
            add_element_to_object(&mut orderLeg, &Value::Str("is_buying_asset".into()), Value::Bool(false));
        }  else if (side.as_str() == Some("buy")) {
            add_element_to_object(&mut orderLeg, &Value::Str("is_buying_asset".into()), Value::Bool(true));
        }  else {
        ...
        let mut orderRequest: Value = Value::Map({
            let mut m = indexmap::IndexMap::new();
                ...
                m.insert("legs".to_string(), Value::from(vec![orderLeg.clone()]));
                m.insert("signature".to_string(), self.default_signature());
    ...
    m
        ...
        if is_true(&postOnly) {
            add_element_to_object(&mut orderRequest, &Value::Str("post_only".into()), Value::Bool(true));
        }
        ...
        add_element_to_object(&mut orderRequest, &Value::Str("time_in_force".into()), timeInForce.clone());
    ...
    m
        ...
        if (builderFee.as_bool() == Some(true)) {
            ...
            add_element_to_object(&mut orderRequest, &Value::Str("builder".into()), self.safe_string_k(self.options.clone(), "builder", &[]));
            add_element_to_object(&mut orderRequest, &Value::Str("builder_fee".into()), self.safe_string_k(self.options.clone(), "builderRate", &[]));
        }
    ...
    pub fn eip_message_for_order(&self, mut order: Value, mut structureType: Value) -> Value {
        ...
        let mut legs: Value = Value::from(vec![]);
        {
            ...
            let mut sizeDec: Value = self.safe_string(sizeParts.clone(), Value::Int(1), &[Value::Str("".into())]);
            ...
            if (self.omit_zero(limitPrice.clone()) != Value::Null) {
                let mut price: Value = crate::value::get_value_k(&leg, "limit_price");
                let mut limitParts: Value = split(&price, &Value::Str(".".into()));
                let mut limitDec: Value = self.safe_string(limitParts.clone(), Value::Int(1), &[Value::Str("".into())]);
                ...
                add_element_to_object(&mut legOrder, &Value::Str("limitPrice".into()), self.parse_to_int(priceInteger.clone()));
            }  else {
                add_element_to_object(&mut legOrder, &Value::Str("limitPrice".into()), Value::Int(0)); // should be zero to validate type-check
            }
        ...
        if (structureType.as_str() == Some("EIP712_ORDER_WITH_BUILDER_TYPE")) && matches!(self.safe_bool_k(self.options.clone(), "builderFee", &[Value::Bool(true)]), Value::Bool(true)) {
            add_element_to_object(&mut returnValue, &Value::Str("builder".into()), crate::value::get_value_k(&order, "builder"));
            add_element_to_object(&mut returnValue, &Value::Str("builderFee".into()), self.parse_to_int((match (&(self.convert_to_big_int_custom(self.fee_amount_multiplier())), &(crate::runtime::parse_float(&crate::value::get_value_k(&order, "builder_fee")))) { (Value::Int(x), Value::Int(y)) => Value::Int(x * y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 * *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x * *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x * y), _ => Value::Null }))); // the order is matter for Multiply in go, b must be float64 otherwise the value would be 0
        }
        return returnValue;
 ...
 */
    ...
    m
        ...
        if (symbol != Value::Null) {
            ...
            add_element_to_object(&mut request, &Value::Str("base".into()), Value::from(vec![]));
            ...
            add_element_to_object(&mut request, &Value::Str("quote".into()), Value::from(vec![]));
 ...
 */
    ...
    m
        ...
        if (symbols != Value::Null) {
            ...
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("base".to_string(), Value::from(vec![])); }
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("quote".to_string(), Value::from(vec![])); }
            {
                                let mut i: Value = Value::Int(0);
 ...
 */
    ...
    let mut m = indexmap::IndexMap::new();
        ...
        m.insert("symbol".to_string(), self.safe_symbol(marketId.clone(), &[market.clone()]));
        m.insert("marginMode".to_string(), self.safe_string_lower(marginMode.clone(), Value::Str("margin_type".into()), &[]));
    m
});
 ...
 */
    ...
    m
        ...
        let mut orderType: Value = (if is_true(&(isMarket.as_bool() == Some(true))) { Value::Str("market".into()) } else { Value::Str("limit".into()) });
        ...
        let mut timeInForce: Value = (if is_true(&(isPostOnly.as_bool() == Some(true))) { Value::Str("PO".into()) } else { self.parse_time_in_force(timeInForceRaw.clone()) });
 ...
 */
    ...
    m
        ...
        return self.parse_orders(Value::from(vec![result.clone()]), &[]);
 ...
 */
    ...
    pub fn create_signed_request(&self, mut request: Value, mut structureType: Value, optional_args: &[Value]) -> Value {
        ...
        let mut ethEncodedMessageHashed: Value = add(&Value::Str("0x".into()), &self.hash(ethEncodedMessage.clone(), Value::Str("keccak".into()), &[Value::Str("hex".into())]));
        ...
        let mut signature: Value = ecdsa(self.remove0x_prefix(ethEncodedMessageHashed.clone()), privateKeyWithoutZero.clone(), Value::Str("secp256k1".into()), Value::Null);
        ...
        add_element_to_object(get_value_mut(&mut request, &Value::Str("signature".into())), &Value::Str("signer".into()), (if is_true(&(signerAddress == Value::Null)) { self.eth_get_address_from_private_key(add(&Value::Str("0x".into()), &privateKeyWithoutZero), &[]) } else { signerAddress.clone() }));
        return request;
    ...
    m
        ...
        if is_equal(&isPrivate, &Value::Bool(true)) {
            ...
            if (is_equal(&Value::Bool(ends_with(&path, &Value::Str("auth/api_key/login".into()))), &Value::Bool(true))) || (is_equal(&Value::Bool(ends_with(&path, &Value::Str("auth/wallet/login".into()))), &Value::Bool(true))) {
                ...
                add_element_to_object(&mut headers, &Value::Str("Cookie".into()), cookieValue.clone());
                add_element_to_object(&mut headers, &Value::Str("X-Grvt-Account-Id".into()), accountId.clone());
            }
        }
        return Value::Map({
    let mut m = indexmap::IndexMap::new();
