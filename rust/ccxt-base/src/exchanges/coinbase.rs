 ...
 */
    ...
    m
        ...
        if is_true(&(accountsLength.as_f64().unwrap_or(f64::NAN) > Value::Int(0).as_f64().unwrap_or(f64::NAN))) && is_true(&(cursor != Value::Null)) && is_true(&(cursor.as_str() != Some(""))) {
 ...
 */
    ...
    m
        ...
        });
        {
            ...
            let mut type_var: Value = (if is_true(&(assetId != Value::Null)) { Value::Str("crypto".to_string()) } else { Value::Str("fiat".to_string()) });
            if (code != Value::Null) {
 ...
 */
    ...
    m
        ...
        if (marketType != Value::Null) && (marketType.as_str() != Some("default")) {
            add_element_to_object(&mut request, &Value::Str("product_type".to_string()), (if is_true(&(marketType.as_str() == Some("swap"))) { Value::Str("FUTURE".to_string()) } else { Value::Str("SPOT".to_string()) }));
        }
        let mut response: Value = Value::Null;
        let mut usePrivate: Value = Value::Bool(false);
        { let __destr_tmp = self.handle_option_and_params(params.clone(), Value::Str("fetchTickers".to_string()), Value::Str("usePrivate".to_string()), &[Value::Bool(false)]); usePrivate = __destr_tmp.as_array().and_then(|__arr| __arr.get(0)).cloned().unwrap_or(Value::Null); params = __destr_tmp.as_array().and_then(|__arr| __arr.get(1)).cloned().unwrap_or(Value::Null); }
 ...
 */
    ...
    pub fn parse_ticker(&self, mut ticker: Value, optional_args: &[Value]) -> Value {
        ...
        if (in_op(&ticker, &Value::Str("bids".to_string()))) {
 ...
 */
    ...
    m
        ...
        }  else if is_true(&(isV3.as_bool() == Some(true))) || is_true(&(method.as_str() == Some("v3PrivateGetBrokerageAccounts"))) {
 ...
 */
    ...
    m
        ...
        let mut postOnly: Value = (if is_true(&(timeInForce.as_str() == Some("PO"))) { Value::Bool(true) } else { self.safe_bool2(params.clone(), Value::Str("postOnly".to_string()), Value::Str("post_only".to_string()), &[Value::Bool(false)]) });
        ...
        if (type_var.as_str() == Some("limit")) {
            if isStop {
                ...
                if is_true(&(timeInForce.as_str() == Some("GTD"))) || is_true(&(endTime != Value::Null)) {
    ...
    m
        ...
        if is_true(&(totalFees != Value::Null)) && is_true(&(market != Value::Null)) {
 ...
 */
    ...
    pub fn parse_position(&self, mut position: Value, optional_args: &[Value]) -> Value {
        ...
        if (rawMargin != Value::Null) {
            marginMode = (if is_true(&(rawMargin.as_str() == Some("MARGIN_TYPE_CROSS"))) { Value::Str("cross".to_string()) } else { Value::Str("isolated".to_string()) });
        }
    ...
    m
        ...
        let mut side: Value = (if is_true(&(positionSide.as_str() == Some("POSITION_SIDE_LONG"))) { Value::Str("long".to_string()) } else { Value::Str("short".to_string()) });
