...
impl crate::exchange::DerivedExchange for IndodaxCore {
    ...
    fn parse_ticker(&self, ticker: crate::Value, market: crate::Value) -> crate::Value {
        // Forward to the inherent method on IndodaxCore.
        IndodaxCore::parse_ticker(self, ticker, &[market])
    }
    fn parse_trade(&self, trade: crate::Value, market: crate::Value) -> crate::Value {
        // Forward to the inherent method on IndodaxCore.
        IndodaxCore::parse_trade(self, trade, &[market])
    }
    fn parse_order(&self, order: crate::Value, market: crate::Value) -> crate::Value {
        // Forward to the inherent method on IndodaxCore.
        IndodaxCore::parse_order(self, order, &[market])
    }
    fn parse_ohlcv(&self, ohlcv: crate::Value, market: crate::Value) -> crate::Value {
        // Forward to the inherent method on IndodaxCore.
        IndodaxCore::parse_ohlcv(self, ohlcv, &[market])
    }
    ...
    fn parse_transaction(&self, transaction: crate::Value, currency: crate::Value) -> crate::Value {
        // Forward to the inherent method on IndodaxCore.
        IndodaxCore::parse_transaction(self, transaction, &[currency])
    }
    fn sign(&self, path: crate::Value, api: crate::Value, method: crate::Value, params: crate::Value, headers: crate::Value, body: crate::Value) -> crate::Value {
        // Forward to the inherent method on IndodaxCore.
        IndodaxCore::sign(self, path, &[api, method, params, headers, body])
    }
 ...
 */
    ...
    m
        ...
        let mut response: Value = self.public_get_api_server_time(&[params]).await;
 ...
 */
    ...
    m
        ...
        let mut rawMarkets: Value = self.to_array(response);
        {
            ...
            while { if !__for_first_825 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_825 = false; i.as_f64().unwrap_or(f64::NAN) < ((rawMarkets.len() as i64) as f64) } {
 ...
 */
    ...
    m
        ...
        return self.parse_order_book(orderbook, market.as_map().and_then(|__m| __m.get("symbol")).cloned().unwrap_or(Value::Null), &[Value::Null, Value::Str("buy".into()), Value::Str("sell".into())]);
    ...
    let mut m = indexmap::IndexMap::new();
        ...
        m.insert("baseVolume".to_string(), self.safe_string(ticker.clone(), baseVolume, &[]));
        m.insert("quoteVolume".to_string(), self.safe_string(ticker.clone(), quoteVolume, &[]));
 ...
 */
    ...
    m
        ...
        // }
        //
        let mut response: Value = self.public_get_api_ticker_all(&[params]).await;
    ...
    m
        ...
        });
        {
            ...
            while { if !__for_first_827 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_827 = false; i.as_f64().unwrap_or(f64::NAN) < ((keys.len() as i64) as f64) } {
 ...
 */
    ...
    m
        ...
        let mut request: Value = Value::Map({
            let mut m = indexmap::IndexMap::new();
                m.insert("to".to_string(), until);
                m.insert("tf".to_string(), selectedTimeframe);
        ...
        if (since != Value::Null) {
            ...
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("from".into(), (match (&((match (&(now), &((match (&(limitResolved), &(duration)) { (Value::Int(x), Value::Int(y)) => Value::Int(x * y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 * *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x * *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x * y), _ => Value::Null }))) { (Value::Int(x), Value::Int(y)) => Value::Int(x - y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 - *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x - *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x - y), _ => Value::Null })), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x - y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 - *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x - *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x - y), _ => Value::Null })); }
        }
    ...
    pub fn parse_order(&self, mut order: Value, optional_args: &[Value]) -> Value {
        ...
        if (marketResolved != Value::Null) {
            ...
            if (marketResolved.as_map().and_then(|__m| __m.get("quoteId")).cloned().unwrap_or(Value::Null).as_str() == Some("idr")) && (matches!(&order, Value::Dict(__d) if __d.contains_key("order_rp"))) {
                quoteId = Value::Str("rp".into());
            }
            if (marketResolved.as_map().and_then(|__m| __m.get("baseId")).cloned().unwrap_or(Value::Null).as_str() == Some("idr")) && (matches!(&order, Value::Dict(__d) if __d.contains_key("remain_rp"))) {
 ...
 */
    ...
    m
        ...
        let mut orders: Value = self.parse_orders(historyResult.as_map().and_then(|__m| __m.get("orders")).cloned().unwrap_or(Value::Null), &[market]);
 ...
 */
    ...
    m
        ...
        if isMarketBuy {
            paramsOmitted = self.omit(params.clone(), Value::Str("cost".into()), &[]);
        }
        if (type_var.as_str() == Some("market")) {
            if (side.as_str() == Some("buy")) {
                let mut quoteAmount: Value = Value::Null;
                let mut cost: Value = self.safe_number_k(params, "cost", &[]);
                if (cost != Value::Null) {
                ...
                add_element_to_object(&mut request, &market.as_map().and_then(|__m| __m.get("quoteId")).cloned().unwrap_or(Value::Null), quoteAmount);
            }  else {
    ...
    m
        ...
        let mut id: Value = self.safe_string_k(data, "order_id", &[]);
        return self.safe_order(Value::Map({
    let mut m = indexmap::IndexMap::new();
 ...
 */
    ...
    m
        ...
        let mut result: Value = self.deposit_withdraw_fee(response);
 ...
 */
    ...
    m
        ...
        if (code == Value::Null) {
            ...
            let mut withdraws: Value = self.safe_list(withdraw, currency.as_map().and_then(|__m| __m.get("id")).cloned().unwrap_or(Value::Null), &[Value::from(vec![])]);
            let mut deposits: Value = self.safe_list(deposit, currency.as_map().and_then(|__m| __m.get("id")).cloned().unwrap_or(Value::Null), &[Value::from(vec![])]);
 ...
 */
    ...
    m
        ...
        let mut request: Value = Value::Map({
            let mut m = indexmap::IndexMap::new();
                m.insert("currency".to_string(), currency.as_map().and_then(|__m| __m.get("id")).cloned().unwrap_or(Value::Null));
                m.insert("withdraw_amount".to_string(), amount);
                m.insert("withdraw_address".to_string(), address);
                m.insert("request_id".to_string(), to_string_val(&requestId));
            m
        });
        if (tagWithdrawTag != Value::Null) && (tagWithdrawTag.as_str() != Some("")) {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("withdraw_memo".into(), tagWithdrawTag); }
        }
        let __ws_arg_13 = self.extend(request, &[paramsWithdrawTag]);
    ...
    let mut m = indexmap::IndexMap::new();
        ...
        m.insert("type".to_string(), (if (depositId.is_none()) { Value::Str("withdraw".into()) } else { Value::Str("deposit".into()) }));
 ...
 */
    ...
    m
        ...
        });
        {
            ...
            while { if !__for_first_832 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_832 = false; i.as_f64().unwrap_or(f64::NAN) < ((addressKeys.len() as i64) as f64) } {
                ...
                if (in_op(&networks, &marketId)) {
                    ...
                    if Value::Int(networkId.as_str().and_then(|__s| __s.find(",")).map(|__i| __i as i64).unwrap_or(-1)).as_f64().unwrap_or(f64::NAN) >= ((0i64) as f64) {
                        ...
                        let mut networkIds: Value = split(&networkId, &Value::Str(",".into()));
                        {
                            ...
                            while { if !__for_first_831 { j = (match (&(j), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_831 = false; j.as_f64().unwrap_or(f64::NAN) < ((networkIds.len() as i64) as f64) } {
    ...
    m
        ...
        if isPublic {
            ...
            privateBody = self.urlencode(__ws_arg_14, &[]);
            privateHeaders = Value::Map({
                let mut m = indexmap::IndexMap::new();
                    ...
                    m.insert("Sign".to_string(), self.hmac(self.encode(privateBody.clone()), self.encode(self.secret.clone()), Value::Str("sha512".into()), &[]));
                m
            });
        }
        let mut requestBody: Value = privateBody;
        if isPublic {
            requestBody = body;
        }
        let mut requestHeaders: Value = privateHeaders;
        if isPublic {
