 ...
 */
    ...
    m
        ...
        let mut rawMarkets: Value = self.to_array(response.clone());
        {
            ...
            while { if !__for_first_827 { i = add(&i, &Value::Int(1)); } __for_first_827 = false; is_less_than(&i, &get_array_length(&rawMarkets)) } {
 ...
 */
    ...
    m
        ...
        let mut exchangeOrders: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_830 { i = add(&i, &Value::Int(1)); } __for_first_830 = false; is_less_than(&i, &get_array_length(&marketIds)) } {
 ...
 */
    ...
    m
        ...
        });
        {
            ...
            while { if !__for_first_834 { i = add(&i, &Value::Int(1)); } __for_first_834 = false; is_less_than(&i, &get_array_length(&addressKeys)) } {
                ...
                if is_true(&Value::Bool(in_op(&networks, &marketId))) {
                    ...
                    if is_greater_than_or_equal(&get_index_of(&networkId, &Value::Str(",".to_string())), &Value::Int(0)) {
                        ...
                        let mut networkIds: Value = split(&networkId, &Value::Str(",".to_string()));
                        {
                            ...
                            while { if !__for_first_833 { j = add(&j, &Value::Int(1)); } __for_first_833 = false; is_less_than(&j, &get_array_length(&networkIds)) } {
