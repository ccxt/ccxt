 ...
 */
    ...
    m
        ...
        let mut rawMarkets: Value = self.to_array(response.clone());
        {
            ...
            while { if !__for_first_832 { i = add(&i, &Value::Int(1)); } __for_first_832 = false; is_less_than(&i, &get_array_length(&rawMarkets)) } {
    ...
    m
        ...
        let mut currencyIds: Value = object_keys(&free);
        {
            ...
            while { if !__for_first_833 { i = add(&i, &Value::Int(1)); } __for_first_833 = false; is_less_than(&i, &get_array_length(&currencyIds)) } {
 ...
 */
    ...
    m
        ...
        let mut exchangeOrders: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_835 { i = add(&i, &Value::Int(1)); } __for_first_835 = false; is_less_than(&i, &get_array_length(&marketIds)) } {
    ...
    m
        ...
        if is_equal(&code, &Value::Null) {
            let mut keys: Value = object_keys(&withdraw);
            {
                ...
                while { if !__for_first_836 { i = add(&i, &Value::Int(1)); } __for_first_836 = false; is_less_than(&i, &get_array_length(&keys)) } {
                let mut key: Value = get_value(&keys, &i);
                let mut key: Value = get_value(&keys, &i);
 ...
 */
    ...
    m
        ...
        });
        {
            ...
            while { if !__for_first_839 { i = add(&i, &Value::Int(1)); } __for_first_839 = false; is_less_than(&i, &get_array_length(&addressKeys)) } {
                ...
                if is_true(&Value::Bool(in_op(&networks, &marketId))) {
                    ...
                    if is_greater_than_or_equal(&get_index_of(&networkId, &Value::Str(",".to_string())), &Value::Int(0)) {
                        ...
                        let mut networkIds: Value = split(&networkId, &Value::Str(",".to_string()));
                        {
                            ...
                            while { if !__for_first_838 { j = add(&j, &Value::Int(1)); } __for_first_838 = false; is_less_than(&j, &get_array_length(&networkIds)) } {
