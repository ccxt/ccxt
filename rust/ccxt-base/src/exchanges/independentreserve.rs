 ...
 */
    ...
    m
        ...
        let mut quoteCurrencyIds: Value = self.to_array(quoteCurrencies.clone());
        {
            ...
            while { if !__for_first_809 { i = add(&i, &Value::Int(1)); } __for_first_809 = false; is_less_than(&i, &get_array_length(&baseCurrencyIds)) } {
            ...
            let mut minAmount: Value = self.safe_number(limits.clone(), baseId.clone(), &[]);
            {
                ...
                while { if !__for_first_808 { j = add(&j, &Value::Int(1)); } __for_first_808 = false; is_less_than(&j, &get_array_length(&quoteCurrencyIds)) } {
    ...
    pub fn parse_balance(&self, mut response: Value) -> Value {
        ...
        });
        {
            ...
            while { if !__for_first_810 { i = add(&i, &Value::Int(1)); } __for_first_810 = false; is_less_than(&i, &get_array_length(&response)) } {
 ...
 */
    ...
    m
        ...
        let mut rows: Value = self.to_array(response.clone());
        {
            ...
            while { if !__for_first_811 { i = add(&i, &Value::Int(1)); } __for_first_811 = false; is_less_than(&i, &get_array_length(&rows)) } {
    ...
    m
        ...
        let mut symbols: Value = self.symbols.clone();
        {
            ...
            while { if !__for_first_812 { i = add(&i, &Value::Int(1)); } __for_first_812 = false; is_less_than(&i, &get_array_length(&symbols)) } {
    ...
    m
        ...
        if is_equal(&api, &Value::Str("public".to_string())) {
            ...
            let mut keys: Value = object_keys(&params);
            {
                ...
                while { if !__for_first_813 { i = add(&i, &Value::Int(1)); } __for_first_813 = false; is_less_than(&i, &get_array_length(&keys)) } {
            ...
            add_element_to_object(&mut query, &Value::Str("signature".to_string()), to_upper(&signature));
            {
                ...
                while { if !__for_first_814 { i = add(&i, &Value::Int(1)); } __for_first_814 = false; is_less_than(&i, &get_array_length(&keys)) } {
                let mut key: Value = get_value(&keys, &i);
                let mut key: Value = get_value(&keys, &i);
