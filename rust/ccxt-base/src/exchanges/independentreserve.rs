 ...
 */
    ...
    m
        ...
        let mut quoteCurrencyIds: Value = self.to_array(quoteCurrencies.clone());
        {
            ...
            while { if !__for_first_826 { i = add(&i, &Value::Int(1)); } __for_first_826 = false; is_less_than(&i, &get_array_length(&baseCurrencyIds)) } {
            ...
            let mut minAmount: Value = self.safe_number(limits.clone(), baseId.clone(), &[]);
            {
                ...
                while { if !__for_first_825 { j = add(&j, &Value::Int(1)); } __for_first_825 = false; is_less_than(&j, &get_array_length(&quoteCurrencyIds)) } {
    ...
    pub fn parse_balance(&self, mut response: Value) -> Value {
        ...
        });
        {
            ...
            while { if !__for_first_827 { i = add(&i, &Value::Int(1)); } __for_first_827 = false; is_less_than(&i, &get_array_length(&response)) } {
 ...
 */
    ...
    m
        ...
        let mut rows: Value = self.to_array(response.clone());
        {
            ...
            while { if !__for_first_828 { i = add(&i, &Value::Int(1)); } __for_first_828 = false; is_less_than(&i, &get_array_length(&rows)) } {
    ...
    m
        ...
        let mut symbols: Value = self.symbols.clone();
        {
            ...
            while { if !__for_first_829 { i = add(&i, &Value::Int(1)); } __for_first_829 = false; is_less_than(&i, &get_array_length(&symbols)) } {
    ...
    m
        ...
        if is_equal(&api, &Value::Str("public".to_string())) {
            ...
            let mut keys: Value = object_keys(&params);
            {
                ...
                while { if !__for_first_830 { i = add(&i, &Value::Int(1)); } __for_first_830 = false; is_less_than(&i, &get_array_length(&keys)) } {
            ...
            add_element_to_object(&mut query, &Value::Str("signature".to_string()), to_upper(&signature));
            {
                ...
                while { if !__for_first_831 { i = add(&i, &Value::Int(1)); } __for_first_831 = false; is_less_than(&i, &get_array_length(&keys)) } {
                let mut key: Value = get_value(&keys, &i);
                let mut key: Value = get_value(&keys, &i);
