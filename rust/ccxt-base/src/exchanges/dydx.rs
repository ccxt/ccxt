 ...
 */
    ...
    m
        ...
        let mut rows: Vec<Value> = self.safe_list_k(response.clone(), "historicalFunding", &[Value::List(vec![])]).as_array().cloned().unwrap_or_default();
        {
 ...
 */
    ...
    m
        ...
        // }
        //
        let mut rows: Vec<Value> = self.safe_list_k(response.clone(), "subaccounts", &[Value::List(vec![])]).as_array().cloned().unwrap_or_default();
        let mut result: Value = Value::List(vec![]);
        {
            ...
            let mut account: Value = match &i { Value::Int(__n) => rows.get(*__n as usize), Value::Str(__s) => __s.parse::<usize>().ok().and_then(|__n| rows.get(__n)), _ => None }.cloned().unwrap_or(Value::Null);
            let mut account: Value = match &i { Value::Int(__n) => rows.get(*__n as usize), Value::Str(__s) => __s.parse::<usize>().ok().and_then(|__n| rows.get(__n)), _ => None }.cloned().unwrap_or(Value::Null);
