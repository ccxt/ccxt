 ...
 */
    ...
    m
        ...
        let mut rawPromises: Value = Value::from(vec![self.public_get_market_data_prices(&[__ws_arg_2]).await, self.public_get_market_data_stats(&[self.extend(request, &[params.clone()])]).await]);
 ...
 */
    ...
    pub fn order_message(&self, mut market: Value, mut nonce: Value, mut feeRate: Value, mut type_var: Value, mut side: Value, mut amount: Value, optional_args: &[Value]) -> Value {
        ...
        let mut settlement: Value = Value::Str(format!("{}{}", Value::Str("1e".into()), self.safe_string_k(info.clone(), "settlementDecimals", &[])).into());
    ...
    m
        ...
        if is_true(&postOnly) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("orderFlags".to_string(), Value::Str("POST_ONLY".into())); }
        }  else if (timeInForce.as_deref() == Some("ioc")) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("orderFlags".to_string(), Value::Str("IOC".into())); }
        }  else if (reduceOnly.as_bool() == Some(true)) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("orderFlags".to_string(), Value::Str("REDUCE_ONLY".into())); }
        }
        if (triggerPrice != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("triggerPrice".to_string(), triggerPrice.clone()); }
        }
 ...
 */
    ...
    m
            ...
            let mut orderRequest: Value = self.create_order_request((match (&(nonce), &(i)) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }), symbol.clone(), type_var.clone(), side.clone(), amount.clone(), &[price.clone(), orderParams.clone()]);
 ...
 */
    ...
    m
            ...
            let mut orderRequest: Value = self.edit_order_request((match (&(nonce), &(i)) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }), id.clone(), symbol.clone(), type_var.clone(), side.clone(), &[amount.clone(), price.clone(), orderParams.clone()]);
 ...
 */
    ...
    m
        ...
        if (symbol != Value::Null) {
            ...
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("contractId".to_string(), self.safe_integer_k(market.clone(), "numericId", &[])); }
        }
 ...
 */
    ...
    pub fn sign_message(&self, mut message: Value, mut privateKey: Value) -> Value {
        if is_equal(&get_array_length(&privateKey), &Value::Int(44)) {
            ...
            let mut hash: Value = self.hash(message.clone(), Value::Str("sha256".into()), &[Value::Str("hex".into())]);
 ...
 */
    ...
    m
        ...
        add_element_to_object(&mut formattedResponse, &Value::Str("ask".into()), self.safe_list(self.safe_dict_k(response.clone(), "ask", &[]), Value::Str("levels".into()), &[]));
        add_element_to_object(&mut formattedResponse, &Value::Str("bid".into()), self.safe_list(self.safe_dict_k(response, "bid", &[]), Value::Str("levels".into()), &[]));
        return self.parse_order_book(formattedResponse.clone(), symbol.clone(), &[self.milliseconds(), Value::Str("bid".into()), Value::Str("ask".into()), Value::Str("price".into()), Value::Str("quantity".into())]);
 ...
 */
    ...
    m
        ...
        if (status != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("status".to_string(), status.clone()); }
        }
        if (since != Value::Null) {
 ...
 */
    ...
    m
        ...
        let mut orders: Value = self.fetch_orders_by_status(Value::Str("filled".into()), &[symbol.clone(), since.clone(), limit.clone(), params.clone()]).await;
        let mut filtered: Value = self.filter_by(orders.clone(), Value::Str("status".into()), Value::Str("closed".into()), &[]);
 ...
 */
    ...
    m
        ...
        let mut filtered: Value = self.filter_by(orders.clone(), Value::Str("status".into()), Value::Str("canceled".into()), &[]);
        return self.filter_by_since_limit(filtered.clone(), &[since.clone(), limit.clone()]);
 ...
 */
    ...
    m
        ...
        if (until != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("toMs".to_string(), until.clone()); }
        }
 ...
 */
    ...
    pub fn parse_ledger_entry(&self, mut item: Value, optional_args: &[Value]) -> Value {
        ...
        if (transactionType == Value::Null) {
            ...
            direction = (if is_true(&((transactionType.as_str() == Some("deposit")) || (transactionType.as_str() == Some("transfer-in")))) { Value::Str("in".into()) } else { Value::Str("out".into()) });
 ...
 */
    ...
    m
        ...
        let mut deposits: Value = self.filter_by(transactions.clone(), Value::Str("type".into()), Value::Str("deposit".into()), &[]);
        return self.filter_by_since_limit(deposits.clone(), &[since.clone(), limit.clone(), Value::Str("timestamp".into())]);
 ...
 */
    ...
    m
        ...
        let mut transactions: Value = self.fetch_deposits_withdrawals(&[code.clone(), since.clone(), Value::Null, params.clone()]).await;
        let mut withdrawals: Value = self.filter_by(transactions.clone(), Value::Str("type".into()), Value::Str("withdrawal".into()), &[]);
        return self.filter_by_since_limit(withdrawals.clone(), &[since.clone(), limit.clone(), Value::Str("timestamp".into())]);
