 ...
 */
    ...
    pub fn parse_currency(&self, mut rawCurrency: Value) -> Value {
        ...
        });
        {
            ...
            while { if !__for_first_719 { j = add(&j, &Value::Int(1)); } __for_first_719 = false; is_less_than(&j, &get_array_length(&networks)) } {
 ...
 */
    ...
    pub fn parse_ticker(&self, mut ticker: Value, optional_args: &[Value]) -> Value {
        ...
        if is_true(&(is_equal(&get_value(&market, &Value::Str("contract".to_string())), &Value::Bool(true)))) && is_true(&(!is_equal(&get_value(&market, &Value::Str("contractSize".to_string())), &Value::Null))) {
            // 'v' counts contracts, and a ticker reports base volume
            baseVolume = crate::precise::Precise::stringMul(&baseVolume, &self.number_to_string(get_value(&market, &Value::Str("contractSize".to_string()))));
        }
 ...
 */
    ...
    pub fn parse_balance(&self, mut balance: Value) -> Value {
        ...
        let mut balances: Value = self.safe_list_k(balance.clone(), "balances", &[Value::List(vec![])]);
        {
            ...
            while { if !__for_first_720 { i = add(&i, &Value::Int(1)); } __for_first_720 = false; is_less_than(&i, &get_array_length(&balances)) } {
 ...
 */
    ...
    m
        ...
        let mut ordersRequests: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_721 { i = add(&i, &Value::Int(1)); } __for_first_721 = false; is_less_than(&i, &get_array_length(&orders)) } {
    ...
    m
        ...
        let mut responseOrders: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_722 { i = add(&i, &Value::Int(1)); } __for_first_722 = false; is_less_than(&i, &get_array_length(&result)) } {
 ...
 */
    ...
    m
        ...
        let mut rows: Value = self.to_array(response.clone());
        {
            ...
            while { if !__for_first_723 { i = add(&i, &Value::Int(1)); } __for_first_723 = false; is_less_than(&i, &get_array_length(&rows)) } {
 ...
 */
    ...
    pub fn parse_market_leverage_tiers(&self, mut info: Value, optional_args: &[Value]) -> Value {
        ...
        let mut tiers: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_724 { i = add(&i, &Value::Int(1)); } __for_first_724 = false; is_less_than(&i, &get_array_length(&riskLimits)) } {
 ...
 */
    ...
    m
        ...
        });
        {
            ...
            while { if !__for_first_725 { i = add(&i, &Value::Int(1)); } __for_first_725 = false; is_less_than(&i, &get_array_length(&data)) } {
    ...
    pub fn handle_errors(&self, mut code: Value, mut reason: Value, mut url: Value, mut method: Value, mut headers: Value, mut body: Value, mut response: Value, mut requestHeaders: Value, mut requestBody: Value) -> Value {
        ...
        if is_equal(&responseCodeInteger, &Value::Int(0)) {
            let mut result: Value = self.safe_list_k(response.clone(), "result", &[Value::List(vec![])]); // for batch methods
            {
                ...
                while { if !__for_first_726 { i = add(&i, &Value::Int(1)); } __for_first_726 = false; is_less_than(&i, &get_array_length(&result)) } {
