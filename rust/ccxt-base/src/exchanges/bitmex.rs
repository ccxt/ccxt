 ...
 */
    ...
    pub fn convert_from_raw_cost(&self, mut symbol: Value, mut rawQuantity: Value) -> Value {
        return self.convert_from_raw_quantity(symbol.clone(), rawQuantity.clone(), &[Value::Str("quote".into())]);
 ...
 */
    ...
    let mut m = indexmap::IndexMap::new();
        m.insert("min".to_string(), (if is_true(&contract) { self.parse_number(Value::Str("1".into()), &[]) } else { Value::Null }));
    ...
    let mut m = indexmap::IndexMap::new();
        m.insert("min".to_string(), Value::Null);
        m.insert("max".to_string(), (if positionIsQuote { maxOrderQty.clone() } else { Value::Null }));
    m
}));
    m
}));
 ...
 */
    ...
    m
        ...
        let mut orders: Value = self.to_array(response.clone());
        {
            ...
            let mut order: Value = get_value(&orders, &i);
            let mut side: Value = (if is_true(&(crate::value::get_value_k(&order, "side").as_str() == Some("Sell"))) { Value::Str("asks".into()) } else { Value::Str("bids".into()) });
 ...
 */
    ...
    m
        ...
        if (until != Value::Null) {
            ...
            add_element_to_object(&mut request, &Value::Str("endTime".into()), self.iso8601(until.clone()));
        }
 ...
 */
    ...
    m
        ...
        return self.filter_by_array(orders.clone(), Value::Str("status".into()), &[Value::from(vec![Value::Str("closed".into()), Value::Str("canceled".into())]), Value::Bool(false)]);
 ...
 */
    ...
    m
        ...
        let mut transactions: Value = self.filter_by_array(response.clone(), Value::Str("transactType".into()), &[Value::from(vec![Value::Str("Withdrawal".into()), Value::Str("Deposit".into())]), Value::Bool(false)]);
    ...
    pub fn parse_transaction(&self, mut transaction: Value, optional_args: &[Value]) -> Value {
        ...
        let mut amount: Value = self.convert_to_real_amount(currency.as_map().and_then(|__m| __m.get("code")).cloned().unwrap_or(Value::Null), amountStringAbs.clone());
        let mut feeCostString: Value = self.safe_string_k(transaction.clone(), "fee", &[]);
        let mut feeCost: Value = self.convert_to_real_amount(currency.as_map().and_then(|__m| __m.get("code")).cloned().unwrap_or(Value::Null), feeCostString.clone());
 ...
 */
    ...
    pub fn parse_ohlcv(&self, mut ohlcv: Value, optional_args: &[Value]) -> Value {
        ...
        return Value::from(vec![self.parse8601(self.safe_string_k(ohlcv.clone(), "timestamp", &[])), self.safe_number_k(ohlcv.clone(), "open", &[]), self.safe_number_k(ohlcv.clone(), "high", &[]), self.safe_number_k(ohlcv.clone(), "low", &[]), self.safe_number_k(ohlcv, "close", &[]), volume.clone()]);
 ...
 */
    ...
    m
        ...
        if (limit != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("count".to_string(), limit.clone()); } // default 100, max 500
        }
        ...
        if (since != Value::Null) {
            ...
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("startTime".to_string(), startTime.clone()); } // starting date filter for results
        }  else {
 ...
 */
    ...
    m
        ...
        if (since != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("startTime".to_string(), self.iso8601(since.clone())); }
        }  else {
            // by default reverse=false, i.e. trades are fetched since the time of market inception (year 2015 for XBTUSD)
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("reverse".to_string(), Value::Bool(true)); }
        }
        if (limit != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("count".to_string(), crate::runtime::Math::min(&limit, &Value::Int(1000))); } // api maximum 1000
        }
        let mut until: Value = self.safe_integer2(params.clone(), Value::Str("until".into()), Value::Str("endTime".into()), &[]);
        if (until != Value::Null) {
            params = self.omit(params.clone(), Value::from(vec![Value::Str("until".into())]), &[]);
 ...
 */
    ...
    m
        ...
        if execInstLength > ((0i64) as f64) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("execInst".to_string(), join(&execInstructions, &Value::Str(",".into()))); }
        }
        ...
        if isTriggerOrder || isTrailingAmountOrder {
            ...
            if isTrailingAmountOrder {
                ...
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("pegOffsetValue".to_string(), self.parse_to_numeric(trailingAmount.clone())); }
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("pegPriceType".to_string(), Value::Str("TrailingStopPeg".into())); }
            }  else {
                ...
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("stopPx".to_string(), self.parse_to_numeric(self.price_to_precision(symbol.clone(), triggerPrice.clone()))); }
            }
    ...
    m
        ...
        if (origClOrdID != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("origClOrdID".to_string(), origClOrdID.clone()); }
        ...
        if (amount != Value::Null) {
            let mut qty: Value = self.parse_to_int(self.amount_to_precision(symbol.clone(), amount.clone()));
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("orderQty".to_string(), qty.clone()); }
        }
        if (price != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("price".to_string(), price.clone()); }
        }
        let mut brokerId: Value = self.safe_string_k(self.options.clone(), "brokerId", &[Value::Str("CCXT".into())]);
        if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("text".to_string(), brokerId.clone()); }
 ...
 */
    ...
    m
        ...
        if (clientOrderId == Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("orderID".to_string(), ids.clone()); }
        }  else {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("clOrdID".to_string(), clientOrderId.clone()); }
            params = self.omit(params.clone(), Value::from(vec![Value::Str("clOrdID".into()), Value::Str("clientOrderId".into())]), &[]);
        }
 ...
 */
    ...
    m
        ...
        return self.parse_leverages(leverages.clone(), &[symbols.clone(), Value::Str("symbol".into())]);
 ...
 */
    ...
    pub fn parse_position(&self, mut position: Value, optional_args: &[Value]) -> Value {
        ...
        let mut marginMode: Value = (if is_true(&(crossMargin.as_bool() == Some(true))) { Value::Str("cross".into()) } else { Value::Str("isolated".into()) });
 ...
 */
    ...
    m
        ...
        if (self.twofa.clone() != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("otpToken".to_string(), totp(self.twofa.clone())); }
        }
 ...
 */
    ...
    m
                ...
                if (networksLength == 1.0) {
                    add_element_to_object(get_value_mut(&mut result, &Value::Str("withdraw".into())), &Value::Str("fee".into()), withdrawalFee.clone());
 ...
 */
    ...
    m
        ...
        return self.parse_deposit_withdraw_fees(assets.clone(), &[codes.clone(), Value::Str("asset".into())]);
 ...
 */
    ...
    let mut m = indexmap::IndexMap::new();
        ...
        m.insert("side".to_string(), self.safe_string_lower(liquidation.clone(), Value::Str("side".into()), &[]));
 ...
 */
    ...
    m
        ...
        if (until.is_some()) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("endTime".to_string(), self.iso8601(since.clone())); }
