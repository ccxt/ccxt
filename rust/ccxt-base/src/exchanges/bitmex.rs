 ...
 */
    ...
    m
        ...
        if (since != Value::Null) {
            ...
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("startTime".to_string(), startTime.clone()); }; // starting date filter for results
        }  else {
 ...
 */
    ...
    m
        ...
        if execInstLength > ((0i64) as f64) {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("execInst".to_string(), join(&execInstructions, &Value::Str(",".to_string()))); }
        }
        ...
        if isTriggerOrder || isTrailingAmountOrder {
            ...
            if isTrailingAmountOrder {
                ...
                if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("pegPriceType".to_string(), Value::Str("TrailingStopPeg".to_string())); }
            }  else {
    ...
    m
        ...
        if (origClOrdID != Value::Null) {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("origClOrdID".to_string(), origClOrdID.clone()); }
        ...
        if (amount != Value::Null) {
            let mut qty: Value = self.parse_to_int(self.amount_to_precision(symbol.clone(), amount.clone()));
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("orderQty".to_string(), qty.clone()); }
        }
        if (price != Value::Null) {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("price".to_string(), price.clone()); }
        }
        let mut brokerId: Value = self.safe_string_k(self.options.clone(), "brokerId", &[Value::Str("CCXT".to_string())]);
        if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("text".to_string(), brokerId.clone()); }
 ...
 */
    ...
    m
        ...
        if (clientOrderId == Value::Null) {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("orderID".to_string(), ids.clone()); }
        }  else {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("clOrdID".to_string(), clientOrderId.clone()); }
            params = self.omit(params.clone(), Value::from(vec![Value::Str("clOrdID".to_string()), Value::Str("clientOrderId".to_string())]), &[]);
        }
