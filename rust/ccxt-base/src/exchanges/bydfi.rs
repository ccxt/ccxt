 ...
 */
    ...
    pub fn get_closest_limit(&self, mut limit: Value) -> Value {
        ...
        let mut result: Value = Value::Int(1000);
        {
            ...
            while { if !__for_first_491 { i = add(&i, &Value::Int(1)); } __for_first_491 = false; is_less_than(&i, &get_array_length(&limits)) } {
            if is_equal(&limit, &Value::Null) {
 ...
 */
    ...
    m
        ...
        let mut ordersRequests: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_493 { i = add(&i, &Value::Int(1)); } __for_first_493 = false; is_less_than(&i, &get_array_length(&orders)) } {
            let mut rawOrder: Value = get_value(&orders, &i);
            let mut rawOrder: Value = get_value(&orders, &i);
 ...
 */
    ...
    pub fn parse_balance(&self, mut response: Value) -> Value {
        ...
        });
        {
            ...
            while { if !__for_first_494 { i = add(&i, &Value::Int(1)); } __for_first_494 = false; is_less_than(&i, &get_array_length(&response)) } {
