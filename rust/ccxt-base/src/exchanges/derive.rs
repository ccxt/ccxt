 ...
 */
    ...
    m
        ...
        let mut result: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_631 { i = add(&i, &Value::Int(1)); } __for_first_631 = false; is_less_than(&i, &get_array_length(&tradesArray)) } {
    ...
    pub fn parse_balance(&self, mut response: Value) -> Value {
        ...
        });
        {
            ...
            let mut collaterals: Value = self.safe_list_k(subaccount.clone(), "collaterals", &[Value::List(vec![])]);
            {
                ...
                while { if !__for_first_633 { j = add(&j, &Value::Int(1)); } __for_first_633 = false; is_less_than(&j, &get_array_length(&collaterals)) } {
