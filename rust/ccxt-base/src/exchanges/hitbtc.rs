 ...
 */
    ...
    pub fn parse_currency(&self, mut currency: Value) -> Value {
        ...
        });
        {
            ...
            while { if !__for_first_737 { j = add(&j, &Value::Int(1)); } __for_first_737 = false; is_less_than(&j, &get_array_length(&rawNetworks)) } {
 ...
 */
    ...
    m
        ...
        let mut rates: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_745 { i = add(&i, &Value::Int(1)); } __for_first_745 = false; is_less_than(&i, &get_array_length(&contracts)) } {
            ...
            let mut fundingRateData: Value = self.safe_list(response.clone(), marketId.clone(), &[Value::List(vec![])]);
            {
                ...
                while { if !__for_first_744 { j = add(&j, &Value::Int(1)); } __for_first_744 = false; is_less_than(&j, &get_array_length(&fundingRateData)) } {
 ...
 */
    ...
    pub fn parse_position(&self, mut position: Value, optional_args: &[Value]) -> Value {
        ...
        let mut contracts: Value = Value::Null;
        {
            ...
            while { if !__for_first_747 { i = add(&i, &Value::Int(1)); } __for_first_747 = false; is_less_than(&i, &get_array_length(&positions)) } {
        ...
        let mut collateral: Value = Value::Null;
        {
            ...
            while { if !__for_first_748 { i = add(&i, &Value::Int(1)); } __for_first_748 = false; is_less_than(&i, &get_array_length(&currencies)) } {
 ...
 */
    ...
    pub fn parse_deposit_withdraw_fee(&self, mut fee: Value, optional_args: &[Value]) -> Value {
        ...
        let mut result: Value = self.deposit_withdraw_fee(fee.clone());
        {
            ...
            while { if !__for_first_750 { j = add(&j, &Value::Int(1)); } __for_first_750 = false; is_less_than(&j, &get_array_length(&networks)) } {
