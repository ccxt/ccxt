        ...
        if (type_var.as_str() == Some("spot")) {
            ...
            if (type_var.as_str() == Some("swap")) {
                ...
                if (type_var.as_str() == Some("future")) {
                    ...
                    symbol = Value::Str(format!("{}{}", symbol, Value::Str(format!("{}{}", Value::Str(format!("{}{}", Value::Str(format!("{}{}", Value::Str("-".into()), self.number_to_string(strike.clone())).into()), Value::Str("-".into())).into()), self.safe_string(idParts.clone(), Value::Int(4), &[])).into())).into());
                }
            }
        }
 ...
 */
    ...
    m
        ...
        return self.parse_order_book(response.clone(), symbol.clone(), &[timestamp.clone(), Value::Str("bids".into()), Value::Str("asks".into()), Value::Str("price".into()), Value::Str("priceLevelQuantity".into())]);
 ...
 */
    ...
    m
        ...
        if is_true(&paginate) {
            ...
            return self.fetch_paginated_call_dynamic(Value::Str("fetchTrades".into()), &[symbol.clone(), since.clone(), limit.clone(), params.clone(), maxLimit.clone()]).await;
        }
        let mut market: Value = self.market(symbol.clone());
        let mut request: Value = Value::Map({
            let mut m = indexmap::IndexMap::new();
        ...
        if (limit != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("_pageSize".to_string(), self.get_closest_limit(limit.clone())); }
        }
 ...
 */
    ...
    m
        ...
        add_element_to_object(&mut request, &Value::Str("createdAtDatetime[gte]".into()), self.iso8601(startTime.clone()));
        add_element_to_object(&mut request, &Value::Str("createdAtDatetime[lte]".into()), self.iso8601(until.clone()));
 ...
 */
    ...
    m
        ...
        if is_true(&paginate) {
            ...
            return self.fetch_paginated_call_dynamic(Value::Str("fetchFundingRateHistory".into()), &[symbol.clone(), since.clone(), limit.clone(), params.clone(), maxLimit.clone()]).await;
        }
        let mut market: Value = self.market(symbol.clone());
 ...
 */
    ...
    m
        ...
        if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("type".to_string(), to_upper(&type_var)); }
 ...
 */
    ...
    m
        ...
        if (symbol != Value::Null) {
            market = self.market(symbol.clone());
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("symbol".to_string(), market.as_map().and_then(|__m| __m.get("id")).cloned().unwrap_or(Value::Null)); }
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("commandType".to_string(), Value::Str("V1CancelAllOrdersByMarket".into())); }
        }  else {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("commandType".to_string(), Value::Str("V1CancelAllOrders".into())); }
        }
 ...
 */
    ...
    m
        ...
        if (since != Value::Null) {
            add_element_to_object(&mut request, &Value::Str("createdAtDatetime[gte]".into()), self.iso8601(since.clone()));
        }
 ...
 */
    ...
    pub fn parse_balance_for_single_currency(&self, mut response: Value, mut code: Value) -> Value {
        ...
        if let Value::Dict(__d) = &mut account { std::sync::Arc::make_mut(__d).insert("used".to_string(), self.safe_string_k(response.clone(), "lockedQuantity", &[])); }
    ...
    pub fn parse_balance(&self, mut response: Value) -> Value {
        ...
        });
        {
            ...
            if let Value::Dict(__d) = &mut account { std::sync::Arc::make_mut(__d).insert("used".to_string(), self.safe_string_k(balance.clone(), "lockedQuantity", &[])); }
            if (code != Value::Null) {
                add_element_to_object(&mut result, &code, account.clone());
            }
        }
 ...
 */
    ...
    m
        ...
        add_element_to_object(&mut request, &Value::Str("createdAtDatetime[gte]".into()), self.iso8601(startTimestamp.clone()));
        add_element_to_object(&mut request, &Value::Str("createdAtDatetime[lte]".into()), self.iso8601(until.clone()));
    ...
    m
        ...
        if (api.as_str() == Some("private")) {
            ...
            if (method.as_str() == Some("GET")) {
                ...
                let mut digest: Value = self.hash(self.encode(payload.clone()), Value::Str("sha256".into()), &[Value::Str("hex".into())]);
                let mut signature: Value = self.hmac(self.encode(digest.clone()), self.encode(self.secret.clone()), Value::Str("sha256".into()), &[Value::Str("hex".into())]);
                headers = Value::Map({
                    let mut m = indexmap::IndexMap::new();
                        m.insert("BX-TIMESTAMP".to_string(), timestamp.clone());
                        m.insert("BX-NONCE".to_string(), nonce.clone());
                ...
                if (rateLimitToken != Value::Null) {
                    add_element_to_object(&mut headers, &Value::Str("BX-RATE-LIMIT-TOKEN".into()), rateLimitToken.clone());
                }
            }
 ...
 */
    ...
    m
        ...
        if let Value::Dict(__d) = &mut self.options { std::sync::Arc::make_mut(__d).insert("authorizer".to_string(), authorizer.clone()); }
