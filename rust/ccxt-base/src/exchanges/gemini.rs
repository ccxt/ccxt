 ...
 */
    ...
    m
        ...
        let mut type_var: Value = (if is_true(&swap) { Value::Str("swap".into()) } else { Value::Str("spot".into()) });
 ...
 */
    ...
    m
        ...
        if (limit != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("limit_bids".to_string(), limit.clone()); }
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("limit_asks".to_string(), limit.clone()); }
        }
    ...
    m
        ...
        let mut tickerAtickerBVariable = promise_all(&Value::from(vec![tickerPromiseA.clone(), tickerPromiseB.clone()])).await;
 ...
 */
    ...
    m
        ...
        let mut last: Value = self.safe_string2(ticker.clone(), Value::Str("last".into()), Value::Str("close".into()), &[price.clone()]);
 ...
 */
    ...
    m
        ...
        if (limit != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("limit_trades".to_string(), crate::runtime::Math::min(&limit, &Value::Int(500))); }
        }
        if (since != Value::Null) {
 ...
 */
    ...
    m
        ...
        if (triggerPrice != Value::Null) {
            ...
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("type".to_string(), Value::Str("exchange stop limit".into())); }
        }  else {
            ...
            if (timeInForce.is_some()) {
                if is_true(&(timeInForce.as_deref() == Some("IOC"))) || is_true(&(timeInForce.as_deref() == Some("immediate-or-cancel"))) {
                    if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("options".to_string(), Value::from(vec![Value::Str("immediate-or-cancel".into())])); }
                }  else if is_true(&(timeInForce.as_deref() == Some("FOK"))) || is_true(&(timeInForce.as_deref() == Some("fill-or-kill"))) {
                    if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("options".to_string(), Value::from(vec![Value::Str("fill-or-kill".into())])); }
                }  else if (timeInForce.as_deref() == Some("PO")) {
                    if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("options".to_string(), Value::from(vec![Value::Str("maker-or-cancel".into())])); }
                }
            }
            ...
            if (options != Value::Null) {
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("options".to_string(), Value::from(vec![options.clone()])); }
            }
        }
 ...
 */
    ...
    m
        ...
        if (limit != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("limit_trades".to_string(), limit.clone()); }
        }
        if (since != Value::Null) {
 ...
 */
    ...
    m
        ...
        if (limit != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("limit_transfers".to_string(), limit.clone()); }
        }
        if (since != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("timestamp".to_string(), since.clone()); }
        }
 ...
 */
    ...
    m
        ...
        let mut results: Value = self.parse_deposit_addresses(response.clone(), &[Value::from(vec![code.clone()]), Value::Bool(false), Value::Map({
    let mut m = indexmap::IndexMap::new();
        m.insert("network".to_string(), networkCode.clone());
        m.insert("currency".to_string(), code.clone());
    m
})]);
        return self.index_by(results.clone(), Value::Str("network".into()));
