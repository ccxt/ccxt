...
impl LighterCore {
    ...
    pub fn init_auth_object(&self, mut strAccountIndex: Value, mut strApiKeyIndex: Value) {
        if !(in_op(&self.options, &Value::Str("auths".to_string()))) {
 ...
 */
    ...
    pub fn handle_api_key_index(&self, mut params: Value, mut methodName1: Value, mut optionName1: Value, mut optionName2: Value, optional_args: &[Value]) -> Value {
        ...
        if is_true(&(apiKeyIndex == Value::Null)) || is_true(&(apiKeyIndex.as_f64().unwrap_or(f64::NAN) < Value::Int(4).as_f64().unwrap_or(f64::NAN))) || is_true(&(apiKeyIndex.as_f64().unwrap_or(f64::NAN) > Value::Int(254).as_f64().unwrap_or(f64::NAN))) {
...
if let Err(_try_err) = _try_result { let e: Value = panic_to_value(_try_err);
    ...
    m
        ...
        add_element_to_object(&mut request, &Value::Str("reduce_only".to_string()), (if is_true(&(reduceOnly.as_bool() == Some(true))) { Value::Int(1) } else { Value::Int(0) }));
 ...
 */
    ...
    m
        ...
        return Value::Map({
    let mut m = indexmap::IndexMap::new();
        m.insert("status".to_string(), (if is_true(&(status.as_str() == Some("200"))) { Value::Str("ok".to_string()) } else { Value::Str("error".to_string()) }));
 ...
 */
    ...
    pub fn parse_position(&self, mut position: Value, optional_args: &[Value]) -> Value {
        ...
        if (sign != Value::Null) {
            side = (if is_true(&(sign.as_f64() == Some(1.0))) { Value::Str("long".to_string()) } else { Value::Str("short".to_string()) });
        }
        let mut marginModeId: Value = self.safe_integer_k(position.clone(), "margin_mode", &[]);
        let mut marginMode: Value = Value::Null;
        if (marginModeId != Value::Null) {
            marginMode = (if is_true(&(marginModeId.as_f64() == Some(0.0))) { Value::Str("cross".to_string()) } else { Value::Str("isolated".to_string()) });
        }
 ...
 */
    ...
    let mut m = indexmap::IndexMap::new();
        ...
        m.insert("type".to_string(), (if is_true(&(accountType.as_str() == Some("0"))) { Value::Str("main".to_string()) } else { Value::Str("subaccount".to_string()) }));
