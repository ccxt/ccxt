 ...
 */
    ...
    pub fn parse_ticker(&self, mut ticker: Value, optional_args: &[Value]) -> Value {
        ...
        if (in_op(&ticker, &Value::Str("bid".to_string()))) {
            if (is_array(&ticker.as_map().and_then(|__m| __m.get("bid")).cloned().unwrap_or(Value::Null))) {
        ...
        if (in_op(&ticker, &Value::Str("ask".to_string()))) {
            if (is_array(&ticker.as_map().and_then(|__m| __m.get("ask")).cloned().unwrap_or(Value::Null))) {
 ...
 */
    ...
    pub fn parse_currency(&self, mut currency: Value) -> Value {
        ...
        let mut active: Value = Value::Bool(is_true(&(visible.as_bool() == Some(true))) && (is_equal(&depositEnabled, &Value::Bool(true))) && (is_equal(&withdrawEnabled, &Value::Bool(true))) && is_true(&(state.as_str() == Some("online"))) && (!is_equal(&countryDisabled, &Value::Bool(true))));
