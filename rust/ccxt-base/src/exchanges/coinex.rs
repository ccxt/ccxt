 ...
 */
    ...
    pub async fn fetch_contract_markets(&mut self, mut params: Value) -> Value {
        ...
        let mut result: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_538 { i = add(&i, &Value::Int(1)); } __for_first_538 = false; is_less_than(&i, &get_array_length(&markets)) } {
 ...
 */
    ...
    m
        ...
        let mut balances: Value = self.safe_list_k(response.clone(), "data", &[Value::List(vec![])]);
        {
            ...
            while { if !__for_first_543 { i = add(&i, &Value::Int(1)); } __for_first_543 = false; is_less_than(&i, &get_array_length(&balances)) } {
            let mut entry: Value = get_value(&balances, &i);
            let mut entry: Value = get_value(&balances, &i);
            let mut currencyId: Value = self.safe_string_k(entry.clone(), "ccy", &[]);
            let mut code: Value = self.safe_currency_code(currencyId.clone(), &[]);
 ...
 */
    ...
    m
        ...
        let mut isStopLossOrTakeProfitTrigger: Value = Value::Bool(false);
        {
            ...
            while { if !__for_first_544 { i = add(&i, &Value::Int(1)); } __for_first_544 = false; is_less_than(&i, &get_array_length(&orders)) } {
 ...
 */
    ...
    m
        ...
        let mut requestIds: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_546 { i = add(&i, &Value::Int(1)); } __for_first_546 = false; is_less_than(&i, &get_array_length(&ids)) } {
 ...
 */
    ...
    m
        ...
        let mut orderSymbols: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_548 { i = add(&i, &Value::Int(1)); } __for_first_548 = false; is_less_than(&i, &get_array_length(&orders)) } {
            let mut rawOrder: Value = get_value(&orders, &i);
            let mut rawOrder: Value = get_value(&orders, &i);
            let mut marketId: Value = self.safe_string_k(rawOrder.clone(), "symbol", &[]);
 ...
 */
    ...
    m
        ...
        let mut openOrders: Value = self.fetch_orders_by_status(Value::Str("pending".to_string()), &[symbol.clone(), since.clone(), limit.clone(), params.clone()]).await;
        {
            ...
            while { if !__for_first_550 { i = add(&i, &Value::Int(1)); } __for_first_550 = false; is_less_than(&i, &get_array_length(&openOrders)) } {
 ...
 */
    ...
    m
        ...
        let mut result: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_551 { i = add(&i, &Value::Int(1)); } __for_first_551 = false; is_less_than(&i, &get_array_length(&position)) } {
 ...
 */
    ...
    pub fn parse_market_leverage_tiers(&self, mut info: Value, optional_args: &[Value]) -> Value {
        ...
        let mut minNotional: Value = Value::Int(0);
        {
            ...
            while { if !__for_first_552 { i = add(&i, &Value::Int(1)); } __for_first_552 = false; is_less_than(&i, &get_array_length(&brackets)) } {
 ...
 */
    ...
    m
})]);
        {
            ...
            while { if !__for_first_556 { i = add(&i, &Value::Int(1)); } __for_first_556 = false; is_less_than(&i, &get_array_length(&chains)) } {
