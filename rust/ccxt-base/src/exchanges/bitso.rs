 ...
 */
    ...
    let mut m = indexmap::IndexMap::new();
        ...
        m.insert("address".to_string(), (if (withdrawalAddress != Value::Null) { withdrawalAddress.clone() } else { receivingAddress.clone() }));
        ...
        m.insert("type".to_string(), (if (withdrawId.is_none()) { Value::Str("deposit".to_string()) } else { Value::Str("withdrawal".to_string()) }));
    ...
    pub fn handle_errors(&self, mut httpCode: Value, mut reason: Value, mut url: Value, mut method: Value, mut headers: Value, mut body: Value, mut response: Value, mut requestHeaders: Value, mut requestBody: Value) -> Value {
        ...
        if (in_op(&response, &Value::Str("success".to_string()))) {
            ...
            if matches!(&success, Value::Str(_)) {
                if (success.as_str() == Some("true")) || (is_equal(&success, &Value::Str("1".to_string()))) {
