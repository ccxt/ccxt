 ...
 */
    ...
    m
        ...
        return self.parse_order_book(response.clone(), market.as_map().and_then(|__m| __m.get("symbol")).cloned().unwrap_or(Value::Null), &[Value::Null, Value::Str("bids".into()), Value::Str("asks".into()), Value::Str("px".into()), Value::Str("qty".into())]);
 ...
 */
    ...
    pub fn parse_transaction(&self, mut transaction: Value, optional_args: &[Value]) -> Value {
        ...
        }  else if is_true(&(matches!(&transaction, Value::Dict(__d) if __d.contains_key("withdrawalId")))) {
