...
impl ExtendedCore {
    ...
    pub fn index_by_stringified_numeric_id(&self, mut input: Value) -> Value {
        ...
        let mut keys: Value = object_keys(&input);
        {
            ...
            while { if !__for_first_658 { i = add(&i, &Value::Int(1)); } __for_first_658 = false; is_less_than(&i, &get_array_length(&keys)) } {
    ...
    m
        ...
        if !is_equal(&symbols, &Value::Null) {
            let mut marketIds: Value = Value::List(vec![]);
            {
                ...
                while { if !__for_first_659 { i = add(&i, &Value::Int(1)); } __for_first_659 = false; is_less_than(&i, &get_array_length(&symbols)) } {
 ...
 */
    ...
    pub fn parse_funding_histories(&self, mut histories: Value, optional_args: &[Value]) -> Value {
        ...
        let mut result: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_663 { i = add(&i, &Value::Int(1)); } __for_first_663 = false; is_less_than(&i, &get_array_length(&histories)) } {
 ...
 */
    ...
    m
        ...
        if is_true(&(is_equal(&get_value(&market, &Value::Str("spot".to_string())), &Value::Bool(true)))) && !is_equal(&uppercaseType, &Value::Str("LIMIT".to_string())) {
 ...
 */
    ...
    m
        ...
        let mut dataLength: Value = get_array_length(&data);
        {
            ...
            while { if !__for_first_671 { i = add(&i, &Value::Int(1)); } __for_first_671 = false; is_less_than(&i, &dataLength) } {
            let mut entry: Value = get_value(&data, &i);
            let mut entry: Value = get_value(&data, &i);
            if is_true(&(!is_equal(&cursor, &Value::Null))) && is_true(&(is_equal(&i, &subtract(&dataLength, &Value::Int(1))))) {
                entry = self.extend(entry.clone(), &[Value::Map({
 ...
 */
    ...
    m
        ...
        if is_true(&(is_equal(&method, &Value::Str("GET".to_string())) || is_equal(&method, &Value::Str("DELETE".to_string())) || is_true(&queryPost))) && is_true(&(is_greater_than(&get_array_length(&object_keys(&query)), &Value::Int(0)))) {
