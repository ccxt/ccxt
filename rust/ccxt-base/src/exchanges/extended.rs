 ...
 */
    ...
    pub fn parse_currency(&self, mut currency: Value) -> Value {
        ...
        if is_true(&(currencyId != Value::Null)) && is_true(&(get_index_of(&currencyId, &Value::Str("SPOT".to_string())).as_f64().unwrap_or(f64::NAN) >= Value::Int(0).as_f64().unwrap_or(f64::NAN))) {
 ...
 */
    ...
    pub fn parse_account(&self, mut account: Value) -> Value {
        ...
        if (accountIndex != Value::Null) {
            type_var = (if is_true(&(accountIndex.as_f64() == Some(0.0))) { Value::Str("main".to_string()) } else { Value::Str("subaccount".to_string()) });
        }
        return Value::Map({
    let mut m = indexmap::IndexMap::new();
 ...
 */
    ...
    m
        ...
        if is_true(&((method.as_str() == Some("GET")) || (method.as_str() == Some("DELETE")) || queryPost)) && is_true(&(Value::Int(object_keys(&query).len() as i64).as_f64().unwrap_or(f64::NAN) > Value::Int(0).as_f64().unwrap_or(f64::NAN))) {
