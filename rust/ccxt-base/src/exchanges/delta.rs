 ...
 */
    ...
    m
        ...
        if (reduceOnly.as_bool() == Some(true)) {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("reduce_only".to_string(), reduceOnly.clone()); }
 ...
 */
    ...
    m
        ...
        if (clientOrderId != Value::Null) {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("client_oid".to_string(), clientOrderId.clone()); }
 ...
 */
    ...
    m
        ...
        if (since != Value::Null) {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("start_time".to_string(), Value::Str(format!("{}{}", to_string_val(&since), Value::Str("000".to_string())))); }
        }
        if (limit != Value::Null) {
 ...
 */
    ...
    m
        ...
        if (networkCode != Value::Null) {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("network".to_string(), self.network_code_to_id(networkCode.clone(), &[code.clone()])); }
