 ...
 */
    ...
    m
        ...
        let mut result: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_576 { i = add(&i, &Value::Int(1)); } __for_first_576 = false; is_less_than(&i, &get_array_length(&tickers)) } {
    ...
    pub fn parse_balance(&self, mut response: Value) -> Value {
        ...
        let mut currencyIds: Value = object_keys(&balances);
        {
            ...
            while { if !__for_first_577 { i = add(&i, &Value::Int(1)); } __for_first_577 = false; is_less_than(&i, &get_array_length(&currencyIds)) } {
 ...
 */
    ...
    m
        ...
        });
        {
            ...
            while { if !__for_first_578 { i = add(&i, &Value::Int(1)); } __for_first_578 = false; is_less_than(&i, &get_array_length(&keys)) } {
            let mut key: Value = get_value(&keys, &i);
            let mut key: Value = get_value(&keys, &i);
            let mut value: Value = get_value(&walletAddress, &key);
            let mut value: Value = get_value(&walletAddress, &key);
            if is_true(&(is_equal(&value, &Value::Null))) || is_true(&(is_equal(&value, &Value::Null))) || is_true(&(is_equal(&value, &Value::Str("".to_string())))) || is_true(&(is_equal(&value, &Value::Str("-1".to_string())))) {
                continue;
            }
