 ...
 */
    ...
    pub fn parse_balance(&self, mut response: Value) -> Value {
        ...
        });
        {
            ...
            while { if !__for_first_339 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_339 = false; is_less_than(&i, &get_array_length(&response)) } {
 ...
 */
    ...
    m
        ...
        return self.parse_order_book(orderbook.clone(), market.as_map().and_then(|__m| __m.get("symbol")).cloned().unwrap_or(Value::Null), &[Value::Null, Value::Str("bids".to_string()), Value::Str("asks".to_string()), Value::Str("price".to_string()), Value::Str("size".to_string())]);
 ...
 */
    ...
    pub fn parse_transaction(&self, mut transaction: Value, optional_args: &[Value]) -> Value {
        ...
        let mut fee: Value = Value::Null;
        if is_true(&Value::Bool(matches!(&transaction, Value::Dict(__d) if __d.contains_key("fee")))) {
