 ...
 */
    ...
    pub fn parse_market(&self, mut market: Value) -> Value {
        ...
        let mut swap: Value = Value::Bool(is_true(&(typeId.as_str() == Some("futures"))) || is_true(&(typeId.as_str() == Some("tradfiFutures"))));
 ...
 */
    ...
    m
        ...
        let mut marketIds: Value = object_keys(&markets);
        {
            ...
            if is_true(&(market == Value::Null)) || is_true(&(market == Value::Null)) || is_true(&(marketSymbol == Value::Null)) || is_true(&(marketSymbol.as_str() == Some(""))) {
                continue;
            }
            ...
            let mut hasAmountLimits: Value = Value::Bool(is_true(&(amountLimits != Value::Null)) && is_true(&(amountLimits != Value::Null)) && (self.safe_number_k(amountLimits.clone(), "min", &[]) != Value::Null) && (self.safe_number_k(amountLimits.clone(), "max", &[]) != Value::Null));
            let mut hasPriceLimits: Value = Value::Bool(is_true(&(priceLimits != Value::Null)) && is_true(&(priceLimits != Value::Null)) && (self.safe_number_k(priceLimits.clone(), "min", &[]) != Value::Null) && (self.safe_number_k(priceLimits.clone(), "max", &[]) != Value::Null));
            let mut hasCostLimits: Value = Value::Bool(is_true(&(costLimits != Value::Null)) && is_true(&(costLimits != Value::Null)) && (self.safe_number_k(costLimits.clone(), "min", &[]) != Value::Null) && (self.safe_number_k(costLimits.clone(), "max", &[]) != Value::Null));
            if is_true(&(hasAmountLimits.as_bool() == Some(true))) && is_true(&(hasPriceLimits.as_bool() == Some(true))) && is_true(&(hasCostLimits.as_bool() == Some(true))) {
                add_element_to_object(&mut result, &symbol, Value::Map({
    let mut m = indexmap::IndexMap::new();
        m.insert("info".to_string(), market.clone());
        m.insert("limits".to_string(), Value::Map({
 ...
 */
    ...
    pub fn parse_trade(&self, mut trade: Value, optional_args: &[Value]) -> Value {
        ...
        if (role != Value::Null) {
            takerOrMaker = (if is_true(&(role.as_f64() == Some(1.0))) { Value::Str("maker".to_string()) } else { Value::Str("taker".to_string()) });
        }
 ...
 */
    ...
    let mut m = indexmap::IndexMap::new();
        ...
        m.insert("addressFrom".to_string(), (if is_true(&(method.as_str() == Some("1"))) { address.clone() } else { Value::Null }));
        m.insert("address".to_string(), address.clone());
        m.insert("addressTo".to_string(), (if is_true(&(method.as_str() == Some("2"))) { address.clone() } else { Value::Null }));
        m.insert("amount".to_string(), self.safe_number_k(transaction.clone(), "amount", &[]));
        m.insert("type".to_string(), (if is_true(&(method.as_str() == Some("1"))) { Value::Str("deposit".to_string()) } else { Value::Str("withdrawal".to_string()) }));
