 ...
 */
    ...
    m
        ...
        let mut allNetworks: Value = self.array_concat(depositsNetworks.clone(), withdrawsNetworks.clone());
        {
            ...
            while { if !__for_first_1071 { j = add(&j, &Value::Int(1)); } __for_first_1071 = false; is_less_than(&j, &get_array_length(&allNetworks)) } {
 ...
 */
    ...
    m
        ...
        });
        {
            ...
            while { if !__for_first_1072 { i = add(&i, &Value::Int(1)); } __for_first_1072 = false; is_less_than(&i, &get_array_length(&currenciesIds)) } {
 ...
 */
    ...
    pub fn parse_deposit_withdraw_fees(&self, mut response: Value, optional_args: &[Value]) -> Value {
        ...
        let mut currencyIds: Value = object_keys(&response);
        {
            ...
            while { if !__for_first_1073 { i = add(&i, &Value::Int(1)); } __for_first_1073 = false; is_less_than(&i, &get_array_length(&currencyIds)) } {
    ...
    m
        ...
        let mut depositWithdrawCodes: Value = object_keys(&depositWithdrawFees);
        {
            ...
            while { if !__for_first_1074 { i = add(&i, &Value::Int(1)); } __for_first_1074 = false; is_less_than(&i, &get_array_length(&depositWithdrawCodes)) } {
 ...
 */
    ...
    m
        ...
        let mut marketIds: Value = object_keys(&markets);
        {
            ...
            while { if !__for_first_1077 { i = add(&i, &Value::Int(1)); } __for_first_1077 = false; is_less_than(&i, &get_array_length(&marketIds)) } {
            ...
            if is_true(&symbols) {
                let mut symbolFound: Value = Value::Bool(false);
                {
                    ...
                    while { if !__for_first_1076 { j = add(&j, &Value::Int(1)); } __for_first_1076 = false; is_less_than(&j, &get_array_length(&symbols)) } {
 ...
 */
    ...
    m
        ...
        let mut currencyKeys: Value = object_keys(&currenciesData);
        {
            ...
            while { if !__for_first_1079 { i = add(&i, &Value::Int(1)); } __for_first_1079 = false; is_less_than(&i, &get_array_length(&currencyKeys)) } {
            ...
            let mut feeKeys: Value = object_keys(&feesData);
            {
                ...
                while { if !__for_first_1078 { j = add(&j, &Value::Int(1)); } __for_first_1078 = false; is_less_than(&j, &get_array_length(&feeKeys)) } {
        ...
        if is_true(&checkActive) {
            let _try_result = futures::FutureExt::catch_unwind(std::panic::AssertUnwindSafe(async {
                ...
                let mut orders: Value = self.to_array(response.clone());
                {
                    ...
                    while { if !__for_first_1080 { i = add(&i, &Value::Int(1)); } __for_first_1080 = false; is_less_than(&i, &get_array_length(&orders)) } {
        ...
        if is_true(&checkExecuted) {
            let _try_result = futures::FutureExt::catch_unwind(std::panic::AssertUnwindSafe(async {
                ...
                let mut marketIds: Value = object_keys(&response);
                {
                    ...
                    while { if !__for_first_1082 { i = add(&i, &Value::Int(1)); } __for_first_1082 = false; is_less_than(&i, &get_array_length(&marketIds)) } {
                    let mut marketId: Value = get_value(&marketIds, &i);
                    let mut marketId: Value = get_value(&marketIds, &i);
                    let mut marketNew: Value = self.safe_market(&[marketId.clone(), Value::Null, Value::Str("_".to_string())]);
                    let mut marketOrders: Value = self.safe_list(response.clone(), marketId.clone(), &[Value::List(vec![])]);
                    {
                        ...
                        while { if !__for_first_1081 { j = add(&j, &Value::Int(1)); } __for_first_1081 = false; is_less_than(&j, &get_array_length(&marketOrders)) } {
 ...
 */
    ...
    m
        ...
        if !is_equal(&symbols, &Value::Null) {
            {
                ...
                while { if !__for_first_1083 { i = add(&i, &Value::Int(1)); } __for_first_1083 = false; is_less_than(&i, &get_array_length(&symbols)) } {
                let mut symbol: Value = get_value(&symbols, &i);
                let mut symbol: Value = get_value(&symbols, &i);
                let mut market: Value = self.market(symbol.clone());
        ...
        });
        {
            ...
            while { if !__for_first_1084 { i = add(&i, &Value::Int(1)); } __for_first_1084 = false; is_less_than(&i, &get_array_length(&marketIds)) } {
            let mut marketId: Value = get_value(&marketIds, &i);
            let mut marketId: Value = get_value(&marketIds, &i);
    ...
    m
        ...
        if is_true(&Value::Bool(is_array(&response))) {
            ...
            let mut keys: Value = object_keys(&response);
            {
                ...
                while { if !__for_first_1085 { i = add(&i, &Value::Int(1)); } __for_first_1085 = false; is_less_than(&i, &get_array_length(&keys)) } {
 ...
 */
    ...
    pub fn parse_balance(&self, mut response: Value) -> Value {
        ...
        });
        {
            ...
            while { if !__for_first_1086 { i = add(&i, &Value::Int(1)); } __for_first_1086 = false; is_less_than(&i, &get_array_length(&balanceKeys)) } {
 ...
 */
    ...
    m
        ...
        let mut results: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_1088 { i = add(&i, &Value::Int(1)); } __for_first_1088 = false; is_less_than(&i, &get_array_length(&marketIds)) } {
            let mut marketId: Value = get_value(&marketIds, &i);
            let mut marketId: Value = get_value(&marketIds, &i);
            let mut marketNew: Value = self.safe_market(&[marketId.clone(), Value::Null, Value::Str("_".to_string())]);
            let mut orders: Value = self.safe_list(response.clone(), marketId.clone(), &[Value::List(vec![])]);
            {
                ...
                while { if !__for_first_1087 { j = add(&j, &Value::Int(1)); } __for_first_1087 = false; is_less_than(&j, &get_array_length(&orders)) } {
 ...
 */
    ...
    m
        ...
        let mut subAccounts: Value = self.safe_list_k(response.clone(), "data", &[Value::List(vec![])]);
        {
            ...
            while { if !__for_first_1089 { i = add(&i, &Value::Int(1)); } __for_first_1089 = false; is_less_than(&i, &get_array_length(&subAccounts)) } {
 ...
 */
    ...
    pub fn parse_funding_histories(&self, mut contracts: Value, optional_args: &[Value]) -> Value {
        ...
        let mut result: Value = Value::List(vec![]);
        {
            ...
            while { if !__for_first_1090 { i = add(&i, &Value::Int(1)); } __for_first_1090 = false; is_less_than(&i, &get_array_length(&contracts)) } {
