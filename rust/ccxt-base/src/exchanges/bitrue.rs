...
impl crate::exchange::DerivedExchange for BitrueCore {
    ...
    fn parse_ticker(&self, ticker: crate::Value, market: crate::Value) -> crate::Value {
        // Forward to the inherent method on BitrueCore.
        BitrueCore::parse_ticker(self, ticker, &[market.clone()])
    }
    fn parse_trade(&self, trade: crate::Value, market: crate::Value) -> crate::Value {
        // Forward to the inherent method on BitrueCore.
        BitrueCore::parse_trade(self, trade, &[market.clone()])
    }
    fn parse_order(&self, order: crate::Value, market: crate::Value) -> crate::Value {
        // Forward to the inherent method on BitrueCore.
        BitrueCore::parse_order(self, order, &[market.clone()])
    }
    ...
    fn parse_ohlcv(&self, ohlcv: crate::Value, market: crate::Value) -> crate::Value {
        // Forward to the inherent method on BitrueCore.
        BitrueCore::parse_ohlcv(self, ohlcv, &[market.clone()])
    }
    ...
    fn parse_transfer(&self, transfer: crate::Value, currency: crate::Value) -> crate::Value {
        // Forward to the inherent method on BitrueCore.
        BitrueCore::parse_transfer(self, transfer, &[currency.clone()])
    }
    ...
    fn parse_margin_modification(&self, data: crate::Value, market: crate::Value) -> crate::Value {
        // Forward to the inherent method on BitrueCore.
        BitrueCore::parse_margin_modification(self, data, &[market.clone()])
    }
    fn parse_transaction(&self, transaction: crate::Value, currency: crate::Value) -> crate::Value {
        // Forward to the inherent method on BitrueCore.
        BitrueCore::parse_transaction(self, transaction, &[currency.clone()])
    }
    fn parse_deposit_withdraw_fee(&self, fee: crate::Value, currency: crate::Value) -> crate::Value {
        // Forward to the inherent method on BitrueCore.
        BitrueCore::parse_deposit_withdraw_fee(self, fee, &[currency.clone()])
    }
    fn sign(&self, path: crate::Value, api: crate::Value, method: crate::Value, params: crate::Value, headers: crate::Value, body: crate::Value) -> crate::Value {
        // Forward to the inherent method on BitrueCore.
        BitrueCore::sign(self, path, &[api.clone(), method.clone(), params.clone(), headers.clone(), body.clone()])
    }
 ...
 */
    ...
    m
        ...
        let mut formattedStatus: Value = (if is_true(&(keysLength > ((0i64) as f64))) { Value::Str("maintenance".into()) } else { Value::Str("ok".into()) });
        return Value::Map({
    let mut m = indexmap::IndexMap::new();
 ...
 */
    ...
    pub fn parse_balance(&self, mut response: Value) -> Value {
        ...
        let mut balances: Value = self.safe_list2(response.clone(), Value::Str("balances".into()), Value::Str("account".into()), &[Value::from(vec![])]);
        {
                        let mut i: Value = Value::Int(0);
            ...
            if let Value::Dict(__d) = &mut account { std::sync::Arc::make_mut(__d).insert("used".to_string(), self.safe_string2(balance.clone(), Value::Str("locked".into()), Value::Str("accountLock".into()), &[])); }
            if (code != Value::Null) {
 ...
 */
    ...
    m
            ...
            add_element_to_object(&mut tickers, &market.as_map().and_then(|__m| __m.get("id")).cloned().unwrap_or(Value::Null), ticker.clone());
        }
        }
    ...
    pub fn parse_trade(&self, mut trade: Value, optional_args: &[Value]) -> Value {
        ...
        if (buyerMaker != Value::Null) {
            side = (if is_true(&buyerMaker) { Value::Str("sell".into()) } else { Value::Str("buy".into()) });
        }
        if (isBuyer != Value::Null) {
            side = (if is_true(&isBuyer) { Value::Str("buy".into()) } else { Value::Str("sell".into()) }); // this is a true side
        }
    ...
    m
        ...
        if (market.as_map().and_then(|__m| __m.get("swap")).cloned().unwrap_or(Value::Null).as_bool() == Some(true)) {
            ...
            if is_true(&postOnly) {
                ...
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("type".to_string(), Value::Str("FOK".into())); }
            }  else if (timeInForce.as_deref() == Some("ioc")) {
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("type".to_string(), Value::Str("IOC".into())); }
            }
            ...
            if is_true(&isMarket) && is_true(&(side.as_str() == Some("buy"))) && is_true(&createMarketBuyOrderRequiresPrice) {
                ...
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("amount".to_string(), self.parse_to_numeric(amount.clone())); }
            ...
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("open".to_string(), (if is_true(&(reduceOnly.as_bool() == Some(true))) { Value::Str("CLOSE".into()) } else { Value::Str("OPEN".into()) })); }
 ...
 */
    ...
    m
        ...
        if (origClientOrderId == Value::Null) {
            ...
            if (market.as_map().and_then(|__m| __m.get("swap")).cloned().unwrap_or(Value::Null).as_bool() == Some(true)) {
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("clientOrderId".to_string(), origClientOrderId.clone()); }
            }  else {
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("origClientOrderId".to_string(), origClientOrderId.clone()); }
            }
        }
        if (market.as_map().and_then(|__m| __m.get("swap")).cloned().unwrap_or(Value::Null).as_bool() == Some(true)) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("contractName".to_string(), market.as_map().and_then(|__m| __m.get("id")).cloned().unwrap_or(Value::Null)); }
    ...
    m
            ...
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("orderId".to_string(), id.clone()); } // spot market id is mandatory
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("symbol".to_string(), market.as_map().and_then(|__m| __m.get("id")).cloned().unwrap_or(Value::Null)); }
 ...
 */
    ...
    m
        ...
        if (networkCode != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("chainName".to_string(), self.network_code_to_id(networkCode.clone(), &[currency.as_map().and_then(|__m| __m.get("code")).cloned().unwrap_or(Value::Null)])); }
        }
        if (tag != Value::Null) {
 ...
 */
    ...
    m
        ...
        return self.parse_deposit_withdraw_fees(coins.clone(), &[codes.clone(), Value::Str("coin".into())]);
 ...
 */
    ...
    m
        ...
        if (code != Value::Null) {
            ...
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("coinSymbol".to_string(), currency.as_map().and_then(|__m| __m.get("id")).cloned().unwrap_or(Value::Null)); }
        }
        if (since != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("beginTime".to_string(), since.clone()); }
        }
        if (limit != Value::Null) {
 ...
 */
    ...
    m
        ...
        if is_true(&((type_var.as_str() == Some("api")) && (version.as_str() == Some("kline")))) || is_true(&((type_var.as_str() == Some("open")) && get_index_of(&path, &Value::Str("listenKey".into())).as_f64().unwrap_or(f64::NAN) >= ((0i64) as f64))) {
...
if let Err(_try_err) = _try_result { let e: Value = panic_to_value(_try_err);
        ...
        if (error != Value::Null) {
            ...
            if is_true(&(error.as_str() == Some("200"))) || is_true(&crate::precise::Precise::stringEquals(&error, &Value::Str("0".into()))) {
                return Value::Null;
            }
    ...
    m
        ...
        if is_true(&(matches!(&config, Value::Dict(__d) if __d.contains_key("noSymbol")))) && !(in_op(&params, &Value::Str("symbol".into()))) {
            return config.as_map().and_then(|__m| __m.get("noSymbol")).cloned().unwrap_or(Value::Null);
        }  else if is_true(&(matches!(&config, Value::Dict(__d) if __d.contains_key("byLimit")))) && (in_op(&params, &Value::Str("limit".into()))) {
