 ...
 */
    ...
    m
            ...
            timestamp = self.safe_timestamp(response, Value::Str("date".into()), &[]);
        }
 ...
 */
    ...
    m
        ...
        return self.safe_timestamp(response, Value::Str("server_time".into()), &[]);
                ...
                if (limit != Value::Null) || (until != Value::Null) {
                    if (until != Value::Null) {
                        ...
                        if (limit != Value::Null) {
                            ...
                            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("end_time".to_string(), endByUntil); }
                        }
                    }  else {
                        if (limit == Value::Null) {
                            panic!("{}", crate::exchange_errors::arguments_required(format!("{}{}", self.id.clone(), Value::Str(" fetchOHLCV() requires a limit argument".into()))));
                        }
                        if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("end_time".to_string(), self.sum(&[startTime, (match (&(limit), &(duration)) { (Value::Int(x), Value::Int(y)) => Value::Int(x * y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 * *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x * *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x * y), _ => Value::Null })])); }
                    }
                }
            }
 ...
 */
    ...
    m
        ...
        if swap {
            ...
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("market".to_string(), marketType); }
 ...
 */
    ...
    m
        ...
        if (marketType.as_str() == Some("swap")) {
            ...
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("instrument_id".to_string(), self.safe_string_k(market, "id", &[])); }
        }  else {
    ...
    pub fn parse_cancel_orders(&self, mut response: Value) -> Value {
        ...
        let mut error: Value = self.safe_list_k(response, "error", &[Value::from(vec![])]);
        let mut result: Value = Value::from(vec![]);
        {
                        let mut i: Value = Value::Int(0);
 ...
 */
    ...
    m
        ...
        if fromSwap || toSwap {
            ...
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("currency_mark".to_string(), currencyId); }
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("num".to_string(), amountString); }
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("from".to_string(), fromId); }; // 1 = SPOT, 2 = MARGIN, 3 = OTC
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("to".to_string(), toId); }; // 1 = SPOT, 2 = MARGIN, 3 = OTC
            //
            //     {
            //         "code": 0
            //     }
