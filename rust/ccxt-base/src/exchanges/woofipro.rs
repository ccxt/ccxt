 ...
 */
    ...
    m
        ...
        let mut indexedChains: Value = self.index_by(chainRows.clone(), Value::Str("chain_id".into()));
        {
 ...
 */
    ...
    m
        ...
        add_element_to_object(&mut data, &Value::Str("timestamp".into()), self.safe_integer_k(response.clone(), "timestamp", &[]));
 ...
 */
    ...
    m
        ...
        return self.filter_by_array(result.clone(), Value::Str("symbol".into()), &[symbols.clone()]);
 ...
 */
    ...
    m
        ...
        if (since != Value::Null) {
            add_element_to_object(&mut request, &Value::Str("start_t".into()), since.clone());
        }
 ...
 */
    ...
    pub fn parse_order(&self, mut order: Value, optional_args: &[Value]) -> Value {
        ...
        if (success != Value::Null) {
            status = (if is_true(&(success)) { Value::Str("NEW".into()) } else { Value::Str("REJECTED".into()) });
        }
    ...
    m
        ...
        if !isConditional {
            if is_true(&postOnly) {
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("order_type".to_string(), Value::Str("POST_ONLY".into())); }
            }  else if (timeInForce.as_deref() == Some("fok")) {
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("order_type".to_string(), Value::Str("FOK".into())); }
            }  else if (timeInForce.as_deref() == Some("ioc")) {
                if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("order_type".to_string(), Value::Str("IOC".into())); }
            }
        }
        ...
        if (triggerPrice != Value::Null) {
            ...
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("algo_type".to_string(), Value::Str("STOP".into())); }
        }  else if hasStopLoss || hasTakeProfit {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("algo_type".to_string(), Value::Str("TP_SL".into())); }
            let mut childOrders: Value = Value::from(vec![]);
            let mut closeSide: Value = (if is_true(&(orderSide.as_str() == Some("BUY"))) { Value::Str("SELL".into()) } else { Value::Str("BUY".into()) });
            if hasStopLoss {
 ...
 */
    ...
    m
        ...
        if isByClientOrder {
            add_element_to_object(&mut extendParams, &Value::Str("client_order_id".into()), clientOrderIdExchangeSpecific.clone());
        }  else {
            add_element_to_object(&mut extendParams, &Value::Str("id".into()), id.clone());
        }
        if (trigger.as_bool() == Some(true)) {
 ...
 */
    ...
    m
        ...
        if (clientOrderIds != Value::Null) {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("client_order_ids".to_string(), join(&clientOrderIds, &Value::Str(",".into()))); }
            let __ws_arg_17 = self.extend(request.clone(), &[params.clone()]);
            response = self.v1_private_delete_client_batch_order(&[__ws_arg_17]).await;
        }  else {
            if let Value::Dict(__d12) = &mut request { std::sync::Arc::make_mut(__d12).insert("order_ids".to_string(), join(&ids, &Value::Str(",".into()))); }
 ...
 */
    ...
    m
        ...
        if (limit != Value::Null) {
            ...
            add_element_to_object(&mut request, &Value::Str("size".into()), maxLimit.clone());
        }
        if (isTrigger.as_bool() == Some(true)) {
            add_element_to_object(&mut request, &Value::Str("algo_type".into()), Value::Str("STOP".into()));
        }
 ...
 */
    ...
    m
        ...
        if (limit != Value::Null) {
            ...
            add_element_to_object(&mut request, &Value::Str("size".into()), Value::Int(500));
        }
        { let __destr_tmp = self.handle_until_option(Value::Str("end_t".into()), request.clone(), params.clone(), &[]); request = __destr_tmp.as_array().and_then(|__arr| __arr.get(0)).cloned().unwrap_or(Value::Null); params = __destr_tmp.as_array().and_then(|__arr| __arr.get(1)).cloned().unwrap_or(Value::Null); }
 ...
 */
    ...
    pub fn parse_ledger_entry(&self, mut item: Value, optional_args: &[Value]) -> Value {
        ...
        let mut direction: Value = (if is_true(&(side.as_deref() == Some("DEPOSIT"))) { Value::Str("in".into()) } else { Value::Str("out".into()) });
 ...
 */
    ...
    let mut m = indexmap::IndexMap::new();
        ...
        m.insert("symbol".to_string(), market.as_map().and_then(|__m| __m.get("symbol")).cloned().unwrap_or(Value::Null));
        m.insert("marginMode".to_string(), self.safe_string_lower(marginMode.clone(), Value::Str("default_margin_mode".into()), &[]));
    m
});
 ...
 */
    ...
    m
        ...
        let mut rows: Value = self.safe_list_k(data, "rows", &[Value::from(vec![])]);
        return self.parse_margin_modes(rows.clone(), &[symbols.clone(), Value::Str("symbol".into())]);
 ...
 */
    ...
    let mut m = indexmap::IndexMap::new();
        ...
        m.insert("status".to_string(), (if is_true(&(success.as_bool() == Some(true))) { Value::Str("ok".into()) } else { Value::Str("failed".into()) }));
        m.insert("timestamp".to_string(), timestamp.clone());
        m.insert("datetime".to_string(), self.iso8601(timestamp.clone()));
    m
});
 ...
 */
    ...
    m
        ...
        add_element_to_object(&mut modification, &Value::Str("type".into()), (if is_true(&(type_var.as_str() == Some("ADD"))) { Value::Str("add".into()) } else { Value::Str("reduce".into()) }));
        add_element_to_object(&mut modification, &Value::Str("amount".into()), self.parse_number(self.number_to_string(amount.clone()), &[]));
        return modification;
 ...
 */
    ...
    m
        ...
        if (access.as_str() == Some("public")) {
            ...
            add_element_to_object(&mut headers, &Value::Str("orderly-signature".into()), self.urlencode_base64(self.base64_to_binary(signature.clone(), &[]), &[]));
        }
        return Value::Map({
    let mut m = indexmap::IndexMap::new();
