 ...
 */
    ...
    m
        ...
        { let __destr_tmp = self.handle_option_and_params(params.clone(), Value::Str("fetchCurrencies".into()), Value::Str("skipFetchCurrencies".into()), &[Value::Bool(false)]); skipFetchCurrencies = __destr_tmp.as_array().and_then(|__arr| __arr.get(0)).cloned().unwrap_or(Value::Null); params = __destr_tmp.as_array().and_then(|__arr| __arr.get(1)).cloned().unwrap_or(Value::Null); }
    ...
    m
        ...
        let _try_result = futures::FutureExt::catch_unwind(std::panic::AssertUnwindSafe(async {
            response = self.v1_private_post_private_get_currency_networks(&[params]).await;
...
if let Err(_try_err) = _try_result { let e: Value = panic_to_value(_try_err);
    ...
    pub fn parse_currency(&self, mut currency: Value) -> Value {
        ...
        let mut chains: Value = self.safe_list_k(currency.clone(), "network_list", &[Value::from(vec![])]);
        {
            ...
            while { if !__for_first_593 { j = (match (&(j), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_593 = false; j.as_f64().unwrap_or(f64::NAN) < ((chains.len() as i64) as f64) } {
 ...
 */
    ...
    m
        ...
        let mut result: Value = Value::from(vec![]);
        {
            ...
            while { if !__for_first_594 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_594 = false; i.as_f64().unwrap_or(f64::NAN) < ((data.len() as i64) as f64) } {
 ...
 */
    ...
    m
        ...
        if (symbols != Value::Null) {
            ...
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("instrument_name".to_string(), market.as_map().and_then(|__m| __m.get("id")).cloned().unwrap_or(Value::Null)); }
        }
 ...
 */
    ...
    m
        ...
        if (since != Value::Null) {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("start_ts".to_string(), since.clone()); }
        }
        if (limit != Value::Null) {
        ...
        params = self.omit(params.clone(), Value::from(vec![Value::Str("until".into())]), &[]);
        if (until != Value::Null) {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("end_ts".to_string(), until); }
        }
 ...
 */
    ...
    m
        ...
        if is_true(&paginate) {
            return self.fetch_paginated_call_deterministic(Value::Str("fetchOHLCV".into()), &[symbol.clone(), since.clone(), limit.clone(), timeframe.clone(), params.clone(), Value::Int(300)]).await;
        }
        let mut market: Value = self.market(symbol);
        let mut request: Value = Value::Map({
            let mut m = indexmap::IndexMap::new();
        ...
        if (limit != Value::Null) {
            if limit.as_f64().unwrap_or(f64::NAN) > ((300i64) as f64) {
                limit = Value::Int(300);
            }
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("count".to_string(), limit.clone()); }
        }
        ...
        let mut until: Value = self.safe_integer_k(params.clone(), "until", &[now]);
        params = self.omit(params.clone(), Value::from(vec![Value::Str("until".into())]), &[]);
        if (since != Value::Null) {
            ...
            if (limit != Value::Null) {
                if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("end_ts".to_string(), self.sum(&[since.clone(), (match (&((match (&(duration), &(limit)) { (Value::Int(x), Value::Int(y)) => Value::Int(x * y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 * *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x * *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x * y), _ => Value::Null })), &(Value::Int(1000))) { (Value::Int(x), Value::Int(y)) => Value::Int(x * y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 * *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x * *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x * y), _ => Value::Null })])); }
            }  else {
                if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("end_ts".to_string(), until.clone()); }
            }
        }  else {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("end_ts".to_string(), until); }
        }
 ...
 */
    ...
    m
        ...
        if (limit != Value::Null) && (limit.as_f64() != Some(0.0)) {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("depth".to_string(), crate::runtime::Math::min(&limit, &Value::Int(50))); }; // max 50
        }
    ...
    m
        ...
        });
        {
            ...
            while { if !__for_first_595 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_595 = false; i.as_f64().unwrap_or(f64::NAN) < ((positionBalances.len() as i64) as f64) } {
            ...
            if let Value::Dict(__d) = &mut account { std::sync::Arc::make_mut(__d).insert("used".to_string(), self.safe_string_k(balance, "reserved_qty", &[])); }
            if (code != Value::Null) {
 ...
 */
    ...
    m
        ...
        if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("broker_id".to_string(), broker); }
        let mut marketType: Value = Value::Null;
        let mut marginMode: Value = Value::Null;
        { let __destr_tmp = self.handle_market_type_and_params(Value::Str("createOrder".into()), &[market, params.clone()]); marketType = __destr_tmp.as_array().and_then(|__arr| __arr.get(0)).cloned().unwrap_or(Value::Null); params = __destr_tmp.as_array().and_then(|__arr| __arr.get(1)).cloned().unwrap_or(Value::Null); }
        { let __destr_tmp = self.custom_handle_margin_mode_and_params(Value::Str("createOrder".into()), &[params.clone()]); marginMode = __destr_tmp.as_array().and_then(|__arr| __arr.get(0)).cloned().unwrap_or(Value::Null); params = __destr_tmp.as_array().and_then(|__arr| __arr.get(1)).cloned().unwrap_or(Value::Null); }
        if (marketType.as_str() == Some("margin")) || (marginMode != Value::Null) {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("spot_margin".to_string(), Value::Str("MARGIN".into())); }
        }  else if (marketType.as_str() == Some("spot")) {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("spot_margin".to_string(), Value::Str("SPOT".into())); }
        }
        ...
        if (timeInForce != Value::Null) {
            if (timeInForce.as_str() == Some("GTC")) {
                if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("time_in_force".to_string(), Value::Str("GOOD_TILL_CANCEL".into())); }
            }  else if (timeInForce.as_str() == Some("IOC")) {
                if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("time_in_force".to_string(), Value::Str("IMMEDIATE_OR_CANCEL".into())); }
            }  else if (timeInForce.as_str() == Some("FOK")) {
                if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("time_in_force".to_string(), Value::Str("FILL_OR_KILL".into())); }
            }  else {
        ...
        if (postOnly.as_bool() == Some(true)) || (timeInForce.as_str() == Some("PO")) {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("exec_inst".to_string(), Value::from(vec![Value::Str("POST_ONLY".into())])); }
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("time_in_force".to_string(), Value::Str("GOOD_TILL_CANCEL".into())); }
        }
        ...
        if isTrigger {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("ref_price".to_string(), self.price_to_precision(symbol.clone(), triggerPrice.clone())); }
            ...
            if (uppercaseType.as_str() == Some("LIMIT")) || (uppercaseType.as_str() == Some("STOP_LIMIT")) || (uppercaseType.as_str() == Some("TAKE_PROFIT_LIMIT")) {
                ...
                if (side.as_str() == Some("buy")) {
                    if is_true(&crate::precise::Precise::stringLt(&priceString, &triggerPrice)) {
                        if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("type".to_string(), Value::Str("TAKE_PROFIT".into())); }
                    }  else {
                        if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("type".to_string(), Value::Str("STOP_LOSS".into())); }
                    }
                }  else {
                    if is_true(&crate::precise::Precise::stringLt(&priceString, &triggerPrice)) {
                        if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("type".to_string(), Value::Str("STOP_LOSS".into())); }
            ...
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("ref_price".to_string(), self.price_to_precision(symbol.clone(), stopLossPrice)); }
            ...
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("ref_price".to_string(), self.price_to_precision(symbol, takeProfitPrice)); }
        }  else {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("type".to_string(), uppercaseType); }
        }
        params = self.omit(params.clone(), Value::from(vec![Value::Str("postOnly".into()), Value::Str("clientOrderId".into()), Value::Str("timeInForce".into()), Value::Str("stopPrice".into()), Value::Str("triggerPrice".into()), Value::Str("stopLossPrice".into()), Value::Str("takeProfitPrice".into())]), &[]);
 ...
 */
    ...
    m
        ...
        if (side.as_str() == Some("buy")) && ((uppercaseType.as_str() == Some("MARKET")) || (uppercaseType.as_str() == Some("STOP_LOSS")) || (uppercaseType.as_str() == Some("TAKE_PROFIT"))) {
            ...
            let mut cost: Value = self.safe_number2(params.clone(), Value::Str("cost".into()), Value::Str("notional".into()), &[]);
            ...
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("notional".to_string(), quoteAmount); }
        }  else {
    ...
    m
        ...
        if (id != Value::Null) {
            ...
            if (originalClientOrderId == Value::Null) {
                ...
                if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("orig_client_oid".to_string(), originalClientOrderId); }
                params = self.omit(params.clone(), Value::from(vec![Value::Str("orig_client_oid".into()), Value::Str("clientOrderId".into())]), &[]);
            }
        }
        ...
        if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("new_quantity".to_string(), self.amount_to_precision(symbol.clone(), amount)); }
        if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("new_price".to_string(), self.price_to_precision(symbol, price)); }
        return self.extend(request, &[params]);
 ...
 */
    ...
    m
        ...
        let mut orderRequests: Value = Value::from(vec![]);
        {
            ...
            while { if !__for_first_597 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_597 = false; i.as_f64().unwrap_or(f64::NAN) < ((ids.len() as i64) as f64) } {
 ...
 */
    ...
    m
        ...
        let mut orderRequests: Value = Value::from(vec![]);
        {
            ...
            while { if !__for_first_598 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_598 = false; i.as_f64().unwrap_or(f64::NAN) < ((orders.len() as i64) as f64) } {
 ...
 */
    ...
    m
        ...
        if (tag != Value::Null) {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("address_tag".to_string(), tag); }
        }
        ...
        if (networkId != Value::Null) {
            if let Value::Dict(__d) = &mut request { std::sync::Arc::make_mut(__d).insert("network_id".to_string(), networkId); }
        }
 ...
 */
    ...
    m
        ...
        let mut depositAddressesRaw: Value = self.fetch_deposit_addresses_by_network(code, &[params]).await;
 ...
 */
    ...
    let mut m = indexmap::IndexMap::new();
        ...
        m.insert("quoteVolume".to_string(), (if (market.as_map().and_then(|__m| __m.get("quote")).cloned().unwrap_or(Value::Null).as_str() == Some("USD")) { self.safe_string_k(ticker.clone(), "vv", &[]) } else { Value::Null }));
    ...
    m
        ...
        if (execInst != Value::Null) {
            postOnly = Value::Bool(false);
            {
                ...
                let mut inst: Value = execInst.as_array().and_then(|__arr| match &i { Value::Int(__n) => __arr.get(*__n as usize), Value::Str(__s) => __s.parse::<usize>().ok().and_then(|__n| __arr.get(__n)), _ => None }).cloned().unwrap_or(Value::Null);
    ...
    m
        ...
        let mut marginMode: Value = Value::Null;
        { let __destr_tmp = self.handle_margin_mode_and_params(methodName, &[params.clone()]); marginMode = __destr_tmp.as_array().and_then(|__arr| __arr.get(0)).cloned().unwrap_or(Value::Null); params = __destr_tmp.as_array().and_then(|__arr| __arr.get(1)).cloned().unwrap_or(Value::Null); }
 ...
 */
    ...
    let mut m = indexmap::IndexMap::new();
        ...
        m.insert("type".to_string(), self.parse_ledger_entry_type(self.safe_string_k(item, "journal_type", &[])));
        m.insert("currency".to_string(), code);
 ...
 */
    ...
    pub fn parse_settlements(&self, mut settlements: Value, optional_args: &[Value]) -> Value {
        ...
        let mut result: Value = Value::from(vec![]);
        {
            ...
            while { if !__for_first_602 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_602 = false; i.as_f64().unwrap_or(f64::NAN) < ((settlements.len() as i64) as f64) } {
 ...
 */
    ...
    m
        ...
        let mut result: Value = Value::from(vec![]);
        {
            ...
            while { if !__for_first_604 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_604 = false; i.as_f64().unwrap_or(f64::NAN) < ((positions.len() as i64) as f64) } {
    ...
    pub fn params_to_string(&self, mut object: Value, mut level: Value) -> Value {
        ...
        }
        {
            ...
            while { if !__for_first_606 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_606 = false; i.as_f64().unwrap_or(f64::NAN) < ((paramsKeys.len() as i64) as f64) } {
            ...
            if (value.as_str() == Some("undefined")) {
            ...
            }  else if (matches!(&value, Value::Arr(_))) {
                {
                    ...
                    while { if !__for_first_605 { j = (match (&(j), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_605 = false; j.as_f64().unwrap_or(f64::NAN) < ((value.len() as i64) as f64) } {
 ...
 */
    ...
    pub fn parse_trading_fees(&self, mut response: Value) -> Value {
        ...
        if let Value::Dict(__d) = &mut result { std::sync::Arc::make_mut(__d).insert("info".to_string(), response.clone()); }
        {
                        let mut i: Value = Value::Int(0);
            let mut __for_first_607: bool = true;
            while { if !__for_first_607 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_607 = false; i.as_f64().unwrap_or(f64::NAN) < ((self.symbols.len() as i64) as f64) } {
            ...
            let mut takerFeeKey: Value = (if (isSwap.as_bool() == Some(true)) { Value::Str("effective_deriv_taker_rate_bps".into()) } else { Value::Str("effective_spot_taker_rate_bps".into()) });
            let mut makerFeeKey: Value = (if (isSwap.as_bool() == Some(true)) { Value::Str("effective_deriv_maker_rate_bps".into()) } else { Value::Str("effective_spot_maker_rate_bps".into()) });
            ...
            add_element_to_object(&mut result, &symbol, tradingFee);
        }
        }
        return result;
    ...
    m
        ...
        let mut url: Value = add(&add(&get_value(&self.urls.as_map().and_then(|__m| __m.get("api")).cloned().unwrap_or(Value::Null), &type_var), &Value::Str("/".into())), &path);
