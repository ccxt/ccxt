...
match _try_result { Ok(__try_ok) => { if !matches!(__try_ok, Value::Null) { return __try_ok; } return Value::Null; } Err(_try_err) => { let e: Value = panic_to_value(_try_err); 
    ...
    pub fn check_constructor(&self, mut exchange: Value) {
        ...
        if (get_value(&exchange, &Value::Str("id".to_string())).as_str() == Some("binance")) {
            ...
            assert(Value::Bool(get_value(&exchange, &Value::Str("hostname".to_string())).as_str() == Some("binance.us")), &[Value::Str(format!("{}{}", Value::Str("binance.us hostname does not match ".to_string()), get_value(&exchange, &Value::Str("hostname".to_string()))))]);
