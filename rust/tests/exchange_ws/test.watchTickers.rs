...
if let Err(_try_err) = _try_result { let e: Value = panic_to_value(_try_err);
            ...
            if (matches!(&e, Value::Str(__s) if __s.contains("[ArgumentsRequired]"))) && is_true(&(Value::Bool((argSymbols == Value::Null) || (Value::Int(argSymbols.len() as i64).as_f64() == Some(0.0))))) {
...
if let Err(_try_err) = _try_result { let ex: Value = panic_to_value(_try_err);
                    ...
                    if is_true(&(Value::Bool(tickerSymbol != Value::Null))) && crate::tests_support::shared::ticker_exception_needs_ohlcv(ex.clone(), exchange.clone(), ticker.clone()) {
