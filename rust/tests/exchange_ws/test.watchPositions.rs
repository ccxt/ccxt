...
if let Err(_try_err) = _try_result { let e: Value = panic_to_value(_try_err);
        ...
        if is_equal(&success, &Value::Bool(true)) {
            ...
            now = exchange.milliseconds();
            {
                ...
                while { if !__for_first_69 { i = add(&i, &Value::Int(1)); } __for_first_69 = false; is_less_than(&i, &get_array_length(&response)) } {
...
if let Err(_try_err) = _try_result { let e: Value = panic_to_value(_try_err);
        ...
        if is_equal(&success2, &Value::Bool(true)) {
            ...
            now = exchange.milliseconds();
            {
                ...
                while { if !__for_first_70 { i = add(&i, &Value::Int(1)); } __for_first_70 = false; is_less_than(&i, &get_array_length(&positionsForSymbols)) } {
