...
pub async fn testFetchMyLiquidations(mut exchange: Value, mut skippedProperties: Value, mut code: Value) -> Value {
    ...
    if !is_true(&get_value(&get_value(&exchange, &Value::Str("has".to_string())), &Value::Str("fetchMyLiquidations".to_string()))) {
    ...
    assert!(ccxt::runtime::is_true(&(Value::Bool(is_array(&items)))));
    {
        ...
        while { if !__for_first_1364 { i = add(&i, &Value::Int(1)); } __for_first_1364 = false; is_less_than(&i, &get_array_length(&items)) } {
