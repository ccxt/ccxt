...
pub fn testCurrency(mut exchange: Value, mut skippedProperties: Value, mut method: Value, mut entry: Value) {
    ...
    let mut isNative: Value = Value::Bool((get_value(&exchange, &Value::Str("has".to_string())).as_map().and_then(|__m| __m.get("fetchCurrencies")).cloned().unwrap_or(Value::Null) != Value::Null) && (get_value(&exchange, &Value::Str("has".to_string())).as_map().and_then(|__m| __m.get("fetchCurrencies")).cloned().unwrap_or(Value::Null).as_bool() != Some(false)) && (get_value(&exchange, &Value::Str("has".to_string())).as_map().and_then(|__m| __m.get("fetchCurrencies")).cloned().unwrap_or(Value::Null).as_str() != Some("emulated")));
