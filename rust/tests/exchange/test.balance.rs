...
pub fn testBalance(mut exchange: Value, mut skippedProperties: Value, mut method: Value, mut entry: Value) {
    ...
    assert!(ccxt::runtime::is_true(&(((codesLength.as_f64() == freeLength.as_f64()) || (codesLength.as_f64() == usedLength.as_f64())))));
    {
        ...
        while { if !__for_first_1467 { i = (match (&(i), &(Value::Int(1))) { (Value::Int(x), Value::Int(y)) => Value::Int(x + y), (Value::Int(x), Value::Float(y)) => Value::Float(*x as f64 + *y), (Value::Float(x), Value::Int(y)) => Value::Float(*x + *y as f64), (Value::Float(x), Value::Float(y)) => Value::Float(x + y), _ => Value::Null }); } __for_first_1467 = false; i.as_f64().unwrap_or(f64::NAN) < Value::Int(allCodes.len() as i64).as_f64().unwrap_or(f64::NAN) } {
