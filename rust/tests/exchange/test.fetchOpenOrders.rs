...
pub async fn testFetchOpenOrders(mut exchange: Value, mut skippedProperties: Value, mut symbol: Value) -> Value {
    ...
    let mut now: Value = exchange.milliseconds();
    {
        ...
        crate::tests_support::shared::assert_in_array(exchange.clone(), &[skippedProperties.clone(), method.clone(), order.clone(), Value::Str("status".to_string()).clone(), Value::from(vec![Value::Str("open".to_string())]).clone()]);
    }
    }
