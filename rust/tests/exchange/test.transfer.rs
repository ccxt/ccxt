...
pub fn testTransfer(mut exchange: Value, mut skippedProperties: Value, mut method: Value, mut entry: Value, mut requestedCode: Value) {
    ...
    let mut emptyAllowedFor: Value = Value::from(vec![Value::Str("fromAccount".to_string()), Value::Str("toAccount".to_string())]);
    ...
    //
    crate::tests_support::shared::assert_in_array(exchange.clone(), &[skippedProperties.clone(), method.clone(), entry.clone(), Value::Str("status".to_string()).clone(), Value::from(vec![Value::Str("ok".to_string()), Value::Str("pending".to_string()), Value::Str("failed".to_string())]).clone()]);
