...
pub async fn testLoadMarkets(mut exchange: Value, mut skippedProperties: Value) -> Value {
    ...
    let mut marketValues: Value = object_values(&markets);
    {
        ...
        while { if !__for_first_1477 { i = add(&i, &Value::Int(1)); } __for_first_1477 = false; is_less_than(&i, &get_array_length(&marketValues)) } {
    ...
    let mut allMarkets: Value = object_values(&get_value(&exchange, &Value::Str("markets".to_string())));
    {
        ...
        while { if !__for_first_1478 { i = add(&i, &Value::Int(1)); } __for_first_1478 = false; is_less_than(&i, &get_array_length(&allMarkets)) } {
    ...
    }
    {
        ...
        while { if !__for_first_1479 { i = add(&i, &Value::Int(1)); } __for_first_1479 = false; is_less_than(&i, &get_array_length(&marketTypes)) } {
