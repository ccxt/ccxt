...
fn helperTestHandleMarketTypeAndParams() {
    ...
    assert!(ccxt::runtime::is_true(&(Value::Bool(!is_true(&(Value::Bool(matches!(&params1, Value::Dict(__d) if __d.contains_key("defaultType")))))))));
...
fn helperTestHandleNetworkRequest() {
    ...
    assert!(ccxt::runtime::is_true(&(Value::Bool(!is_true(&(Value::Bool(matches!(&params1, Value::Dict(__d) if __d.contains_key("network")))))))));
