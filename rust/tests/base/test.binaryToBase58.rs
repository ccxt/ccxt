...
pub fn testBinaryToBase58() {
    ...
    assert!(ccxt::runtime::is_true(&(is_equal(&exchange.binary_to_base58(binary1.clone()), &Value::Str("Cn8eVZg".to_string())))));
    // Test 2: String with space
    let mut binary2: Value = helperStrToBinary5(exchange.clone_self(), Value::Str("hello world".to_string()));
    assert!(ccxt::runtime::is_true(&(is_equal(&exchange.binary_to_base58(binary2.clone()), &Value::Str("StV1DL6CwTryKyV".to_string())))));
    // Test 3: Short string
    let mut binary3: Value = helperStrToBinary5(exchange.clone_self(), Value::Str("test".to_string()));
    assert!(ccxt::runtime::is_true(&(is_equal(&exchange.binary_to_base58(binary3.clone()), &Value::Str("3yZe7d".to_string())))));
    // Test 4: Single byte
    let mut binary4: Value = helperStrToBinary5(exchange.clone_self(), Value::Str("a".to_string()));
    assert!(ccxt::runtime::is_true(&(is_equal(&exchange.binary_to_base58(binary4.clone()), &Value::Str("2g".to_string())))));
    // Test 5: Two bytes
    let mut binary5: Value = helperStrToBinary5(exchange.clone_self(), Value::Str("ab".to_string()));
    assert!(ccxt::runtime::is_true(&(is_equal(&exchange.binary_to_base58(binary5.clone()), &Value::Str("8Qq".to_string())))));
    // Test 6: Three bytes
    let mut binary6: Value = helperStrToBinary5(exchange.clone_self(), Value::Str("abc".to_string()));
    assert!(ccxt::runtime::is_true(&(is_equal(&exchange.binary_to_base58(binary6.clone()), &Value::Str("ZiCa".to_string())))));
    // Test 7: JSON-like binary
    let mut binary7: Value = helperStrToBinary5(exchange.clone_self(), Value::Str("{\"key\":\"value\"}".to_string()));
    assert!(ccxt::runtime::is_true(&(is_equal(&exchange.binary_to_base58(binary7.clone()), &Value::Str("4SoiMiEYtTt5tPdi81Fik".to_string())))));
}