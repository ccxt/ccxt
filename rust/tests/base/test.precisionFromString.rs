// PLEASE DO NOT EDIT THIS FILE, IT IS GENERATED AND WILL BE OVERWRITTEN:
// https://github.com/ccxt/ccxt/blob/master/CONTRIBUTING.md#how-to-contribute-code

#![allow(unused, non_snake_case, dead_code, clippy::all)]
use ccxt::Value;
use ccxt::get_value;
use ccxt::runtime::*;
use crate::tests_support::{ArrayCache, ArrayCacheByTimestamp, ArrayCacheBySymbolById, ArrayCacheBySymbolBySide};
use ccxt::exchange_generated::ExchangeBase;

pub fn testPrecisionFromString() {
    let mut exchange = crate::tests_support::make_exchange(Value::Map({
        let mut m = indexmap::IndexMap::new();
            m.insert("id".to_string(), Value::Str("sampleexchange".into()));
        m
    }));
    // Test 1: Scientific notation negative exponent
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("1e-4".into())).as_f64() == Some(4.0)))));
    // Test 2: Scientific notation uppercase E
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("1E-4".into())).as_f64() == Some(4.0)))));
    // Test 3: Scientific notation larger negative exponent
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("1e-8".into())).as_f64() == Some(8.0)))));
    // Test 4: Scientific notation with decimal
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("2.5e-6".into())).as_f64() == Some(6.0)))));
    // Test 5: Scientific notation positive exponent
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("1e4".into())).as_f64() == Value::Int(-4).as_f64()))));
    // Test 6: Scientific notation explicit positive
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("1e+4".into())).as_f64() == Value::Int(-4).as_f64()))));
    // Test 7: Decimal string 4 places
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("0.0001".into())).as_f64() == Some(4.0)))));
    // Test 8: Decimal string 5 places
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("0.00001".into())).as_f64() == Some(5.0)))));
    // Test 9: Decimal string 1 place
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("0.1".into())).as_f64() == Some(1.0)))));
    // Test 10: Decimal string 2 places
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("0.01".into())).as_f64() == Some(2.0)))));
    // Test 11: Decimal string 8 places
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("0.00000001".into())).as_f64() == Some(8.0)))));
    // Test 12: Trailing zeros should be stripped
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("0.0100".into())).as_f64() == Some(2.0)))));
    // Test 13: Trailing zeros stripped
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("0.00100".into())).as_f64() == Some(3.0)))));
    // Test 14: All trailing zeros after decimal
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("1.0000".into())).as_f64() == Some(0.0)))));
    // Test 15: Integer string (no decimal)
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("1".into())).as_f64() == Some(0.0)))));
    // Test 16: Integer string larger
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("10".into())).as_f64() == Some(0.0)))));
    // Test 17: Integer string even larger
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("100".into())).as_f64() == Some(0.0)))));
    // Test 18: Zero with decimal
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("0.0".into())).as_f64() == Some(0.0)))));
    // Test 19: One with decimal
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("1.0".into())).as_f64() == Some(0.0)))));
    // Test 20: Mixed precision
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("0.12345".into())).as_f64() == Some(5.0)))));
    // Test 21: Negative mantissa with negative exponent
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("-8e-8".into())).as_f64() == Some(8.0)))));
    // Test 22: Negative mantissa uppercase E with zero-padded exponent
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("-8E-08".into())).as_f64() == Some(8.0)))));
    // Test 23: Negative decimal mantissa
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("-2.5e-6".into())).as_f64() == Some(6.0)))));
    // Test 24: Negative mantissa with positive exponent
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("-1e4".into())).as_f64() == Value::Int(-4).as_f64()))));
    // Test 25: Explicitly positive mantissa
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("+1e-4".into())).as_f64() == Some(4.0)))));
    // Test 26: Negative mantissa with explicitly signed exponent
    assert!(ccxt::runtime::is_true(&(Value::Bool(exchange.precision_from_string(Value::Str("-1e+4".into())).as_f64() == Value::Int(-4).as_f64()))));
}