...
//
// Typed wrapper for limitless.
//
// Owns a `Box<LimitlessCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::prediction::limitless::LimitlessCore;
use crate::types::*;
...
pub struct Limitless {
    ...
    core: Box<LimitlessCore>,
}

impl Limitless {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(LimitlessCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(LimitlessCore::new(config.into_option())) }
    }

    pub fn from_core(core: LimitlessCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut LimitlessCore {
        &mut self.core
    }
...
impl std::ops::Deref for Limitless {
    type Target = LimitlessCore;
...
impl crate::typed::TypedExchange for Limitless {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Limitless::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Limitless::markets(self) }
}
