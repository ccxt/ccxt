...
//
// Typed wrapper for polymarket.
//
// Owns a `Box<PolymarketCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::prediction::polymarket::PolymarketCore;
use crate::types::*;
...
pub struct Polymarket {
    ...
    core: Box<PolymarketCore>,
}

impl Polymarket {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(PolymarketCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(PolymarketCore::new(config.into_option())) }
    }

    pub fn from_core(core: PolymarketCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut PolymarketCore {
        &mut self.core
    }
...
impl std::ops::Deref for Polymarket {
    type Target = PolymarketCore;
...
impl crate::typed::TypedExchange for Polymarket {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Polymarket::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Polymarket::markets(self) }
}
