...
//
// Typed wrapper for opinion.
//
// Owns a `Box<OpinionCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::prediction::opinion::OpinionCore;
use crate::types::*;
...
pub struct Opinion {
    ...
    core: Box<OpinionCore>,
}

impl Opinion {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(OpinionCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(OpinionCore::new(config.into_option())) }
    }

    pub fn from_core(core: OpinionCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut OpinionCore {
        &mut self.core
    }
...
impl std::ops::Deref for Opinion {
    type Target = OpinionCore;
...
impl crate::typed::TypedExchange for Opinion {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Opinion::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Opinion::markets(self) }
}
