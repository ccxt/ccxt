...
//
// Typed wrapper for predictfun.
//
// Owns a `Box<PredictfunCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::prediction::predictfun::PredictfunCore;
use crate::types::*;
...
pub struct Predictfun {
    ...
    core: Box<PredictfunCore>,
}

impl Predictfun {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(PredictfunCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(PredictfunCore::new(config.into_option())) }
    }

    pub fn from_core(core: PredictfunCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut PredictfunCore {
        &mut self.core
    }
...
impl std::ops::Deref for Predictfun {
    type Target = PredictfunCore;
...
impl crate::typed::TypedExchange for Predictfun {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Predictfun::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Predictfun::markets(self) }
}
