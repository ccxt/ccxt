...
//
// Typed wrapper for sxbet.
//
// Owns a `Box<SxbetCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::prediction::sxbet::SxbetCore;
use crate::types::*;
...
impl Sxbet {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(SxbetCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(SxbetCore::new(config.into_option())) }
    }

    pub fn from_core(core: SxbetCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut SxbetCore {
        &mut self.core
    }
...
impl std::ops::Deref for Sxbet {
    type Target = SxbetCore;
...
impl crate::typed::TypedExchange for Sxbet {
    ...
    fn market(&self, symbol: &str) -> crate::Result<Market> { Sxbet::market(self, symbol) }

    fn markets(&self) -> Vec<Market> { Sxbet::markets(self) }
}
