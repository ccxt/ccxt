...
//
// Typed wrapper for kalshi.
//
// Owns a `Box<KalshiCore>` and exposes the unified CCXT API
...
use crate::Params;
use crate::Config;
use crate::prediction::kalshi::KalshiCore;
use crate::types::*;
...
impl Kalshi {
    pub fn new(config: Option<Value>) -> Self {
        Self { core: Box::new(KalshiCore::new(config)) }
    }

    /// Construct from a typed [`Config`] builder — the `Value`-free path.
    /// `Config::none()` for defaults.
    pub fn with_config(config: Config) -> Self {
        Self { core: Box::new(KalshiCore::new(config.into_option())) }
    }

    pub fn from_core(core: KalshiCore) -> Self {
    ...
    #[inline]
    fn core_mut(&mut self) -> &mut KalshiCore {
        &mut self.core
    }
...
impl std::ops::Deref for Kalshi {
    type Target = KalshiCore;
...
impl crate::typed::TypedExchange for Kalshi {
