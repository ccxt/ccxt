...
function example() {
    ...
    $balance_margin = $exchange->fetch_balance(array(
    'defaultType' => 'margin',
    ...
    if ($amount_to_trade > $balance_margin[$symbol][$borrow_coin]['free']) {
        ...
        if ($balance_margin[$symbol][$collateral_coin]['free'] < $needed_collateral_amount) {
            ...
            $balance_spot = $exchange->fetch_balance(array(
    'type' => 'spot',
));
            if ($balance_spot[$collateral_coin]['free'] < $needed_collateral_amount) {
                ...
                $exchange->transfer($collateral_coin, $needed_collateral_amount, 'spot', $margin_mode, array(
    'symbol' => $symbol,
        ...
        $borrow_result = $exchange->borrow_margin($borrow_coin, $needed_amount_to_borrow, $symbol, array(
    'marginMode' => $margin_mode,
));
    }
    var_dump('Submitting order.');
    $order = $exchange->create_order($symbol, $order_type, $order_side, $amount_to_trade, $limit_price, array(
    'marginMode' => $margin_mode,
));
    ...
    if ($needed_amount_to_borrow !== null) {
        ...
        $order_back = $exchange->create_order($symbol, $order_type, ($order_side === 'buy' ? 'sell' : 'buy'), $amount_to_repay_back, $purchase_back_price, array(
    'marginMode' => $margin_mode,
));
        var_dump('Now, repaying the loan.');
        $repay_result = $exchange->repay_margin($borrow_coin, $amount_to_repay_back, $symbol, array(
    'marginMode' => $margin_mode,
));
        var_dump('finished.');
    }
