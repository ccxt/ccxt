...
async function primeDirectOutcomeForFastFetchTrades (exchange: any, outcomeId: string) {
...
async function example () {
    ...
    for (let si = 0; si < statuses.length; si++) {
        events = await exchange.fetchEvents ({ 'queries': [ query ], 'status': statuses[si] as any });
        if (events.length > 0) {
