...
// -----------------------------------------------------------------------------------------------
// 1) THE FUN PART — a pure, offline sentiment stub. Feed it a tweet, get a lean.
//    YES on this market means "Anthropic IPOs first", so filing/listing chatter leans YES.
// -----------------------------------------------------------------------------------------------

interface Signal {
    lean: string;        // 'yes' (Anthropic first) | 'no' (OpenAI first) | 'neutral'
    score: number;       // net keyword score, positive => leans YES
    hits: string[];      // which keywords fired (for the printout)
}
...
function darioSignal (tweet: string): Signal {
...
// -----------------------------------------------------------------------------------------------
// 2) THE ARB MATH — also pure and offline. A binary market has YES and NO; exactly one settles at
//    1.00 USDC. NO on venue B costs (1 - yesBid_B). If (yesAsk_A + noAsk_B) < 1.00, the pair is a
//    risk-free "dutch book" regardless of who actually IPOs first.
// -----------------------------------------------------------------------------------------------

interface Quote {
    venue: string;
    yesBid: number;      // best bid for the YES share (what you can sell YES at)
    yesAsk: number;      // best ask for the YES share (what you can buy YES at)
}
...
interface Arb {
    ...
    noAsk: number;
    cost: number;
    edge: number;        // 1.00 - cost, in probability points
}

function findCrossVenueArb (board: Quote[]): Arb[] {
    const arbs: Arb[] = [];
...
function pickYesOutcome (market: any): any {
...
function looksLikeIpoFirst (text: string): boolean {
    const t = (text || '').toLowerCase ();
    return (t.indexOf ('anthropic') !== -1) && (t.indexOf ('ipo') !== -1 || t.indexOf ('public') !== -1);
}

async function readVenue (venueId: string): Promise<any> {
...
async function main () {
    ...
    const board: Quote[] = [];
    let best: any = undefined;      // a venue row we could actually trade on
    for (const venueId of VENUES) {
        let row: any = undefined;
        try {
            row = await readVenue (venueId);
        } catch (e) {
            console.log (venueId.padEnd (12), 'error:', (e as Error).constructor.name);
            continue;
        }
        if (row === undefined) {
...
export { darioSignal, findCrossVenueArb, Quote, Signal, Arb };
