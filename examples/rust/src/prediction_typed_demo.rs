...
async fn demo() {
    ...
    match k.fetch_ticker("KXBTCD", Value::Null).await {
        Ok(t) => println!("kalshi {} last={:?}", t.symbol, t.last),
        Err(e) => println!("fetch_ticker unsupported: {e}"),
    }
    let _book = k.fetch_order_book("KXBTCD", Some(10), Value::Null).await; // -> Result<OrderBook>
    let _trades = k.fetch_trades("KXBTCD", None, Some(50), Value::Null).await; // -> Result<Vec<Trade>>
    ...
    for ex in &mut venues {
        ...
        let _ = ex.fetch_ticker("BTC", Value::Null).await; // via TypedExchangeExt
    }
}
