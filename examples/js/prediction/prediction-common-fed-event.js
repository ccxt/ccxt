...
//
// Usage:
//   npx tsx examples/ts/prediction/prediction-common-fed-event.ts
...
async function main() {
    ...
    for (const id of ids) {
        const match = (eventsByExchange[id] || []).find((ev) => tokenize(ev.title).indexOf(bestToken) !== -1);
        if (match === undefined) {
            continue;
        }
