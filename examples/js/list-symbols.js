...
let listSymbols = async (marketId) => {

    // check if the exchange is supported by ccxt
    let marketFound = ccxt.markets.indexOf (marketId) > -1
    if (!marketFound)
        throw new Error ('Market `' + marketId + '` not found')
    log ('Instantiating', marketId.green, 'exchange market')

    // instantiate the exchange
    let market = new ccxt[marketId] ()
    ...
    log (market.id.green, 'has', symbols.length, 'symbols:', symbols.join (', ').yellow)
...
(async () => {
    ...
    if (process.argv.length > 2) {
        ...
        let marketId = process.argv[2]
        await listSymbols (marketId);

    } else {

        log ("Usage: node", process.argv[1], 'marketId'.bright.green)
    }
