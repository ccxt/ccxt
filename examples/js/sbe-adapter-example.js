 ...
 */
function createMockOkxOrderbook() {
    const buffer = new ArrayBuffer(82);
    const view = new DataView(buffer);
    let offset = 0;
    // Message Header (8 bytes)
    view.setUint16(offset, 26, true);
    offset += 2; // blockLength
    view.setUint16(offset, 1006, true);
    offset += 2; // templateId (SnapshotDepthResponseEvent)
    view.setUint16(offset, 1, true);
    offset += 2; // schemaId
    view.setUint16(offset, 0, true);
    offset += 2; // version
    // Message Fields (26 bytes)
    view.setBigInt64(offset, BigInt(12345), true);
    offset += 8; // instIdCode
    view.setBigInt64(offset, BigInt(1700000000000000), true);
    offset += 8; // tsUs (microseconds)
    view.setBigInt64(offset, BigInt(100), true);
    offset += 8; // seqId
    view.setInt8(offset, -2);
    offset += 1; // pxExponent (price has 2 decimals)
    view.setInt8(offset, -3);
    offset += 1; // szExponent (size has 3 decimals)
    // Asks Group (1 level)
    view.setUint16(offset, 20, true);
    offset += 2; // blockLength
    view.setUint16(offset, 1, true);
    offset += 2; // numInGroup (1 ask)
    view.setBigInt64(offset, BigInt(5000000), true);
    offset += 8; // pxMantissa (50000.00 when converted)
    view.setBigInt64(offset, BigInt(1500000), true);
    offset += 8; // szMantissa (1500.000 when converted)
    ...
    offset += 2; // blockLength
    view.setUint16(offset, 1, true);
    offset += 2; // numInGroup (1 bid)
    view.setBigInt64(offset, BigInt(4999900), true);
    offset += 8; // pxMantissa (49999.00 when converted)
    view.setBigInt64(offset, BigInt(2000000), true);
    offset += 8; // szMantissa (2000.000 when converted)
    view.setInt32(offset, 10, true);
    offset += 4; // ordCount
    return buffer;
}
