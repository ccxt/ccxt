...
(async function test() {
    ...
    if (!currenciesOnly) {
        ...
        let markets = ccxt.flatten(exchanges
            .map(exchange => Object.values(exchange.markets).map(market => exchange.extend(market, {
    ...
    if (!marketsOnly) {
        ...
        let currencies = ccxt.flatten(exchanges
            .map(exchange => Object.values(exchange.currencies).map(currency => exchange.extend(currency, {
            exchange: exchange.id[(currency.active !== false) ? 'green' : 'yellow'],
        }))))
            .filter(currency => (checkAgainst(currency['code'], argument) ||
