...
class testMainClass extends baseMainTestClass {
    ...
    public function init($exchange_id, $symbol) {
        ...
        if ($this->response_tests) {
            $this->run_mock_response_tests($exchange_id, $symbol);
            return;
        }
        if ($this->static_tests) {
            $this->run_static_tests($exchange_id, $symbol); // symbol here is the testname
            return;
        }
        if ($this->id_tests) {
    ...
    public function test_method($method_name, $exchange, $args, $is_public) {
        ...
        call_method($this->test_files, $method_name_in_test, $exchange, $skipped_properties, $args);
    ...
    'orderId' => 'teste',
        ...
        $order = $mocked_exchange->fetch_orders('BTC/USDT');
        dump($order);
    }
    ...
    public function test_woo() {
        ...
        try {
            $woo->create_order('BTC/USDT:USDT', 'limit', 'buy', 1, 20000, array(
                'stopPrice' => 30000,
            ));
        } catch(Exception $e) {
