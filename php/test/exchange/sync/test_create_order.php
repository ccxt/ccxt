...
function test_create_order($exchange, $skipped_properties, $symbol) {
    ...
    [$best_bid, $best_ask] = fetch_best_bid_ask($exchange, 'createOrder', $symbol);
    // **************** [Scenario 1 - START] **************** //
...
function tco_create_unfillable_order($exchange, $market, $log_prefix, $skipped_properties, $best_bid, $best_ask, $limit_price_safety_multiplier_from_median, $buy_or_sell, $predefined_amount = null) {
    try {
        ...
        if ($buy_or_sell === 'buy') {
            ...
            $created_order = tco_create_order_safe($exchange, $symbol, 'limit', 'buy', $order_amount, $limit_buy_price_non_fillable, array(), $skipped_properties);
        } else {
            $order_amount = tco_get_minimum_amount_for_limit_price($exchange, $market, $limit_sell_price_non_fillable, $predefined_amount);
            $created_order = tco_create_order_safe($exchange, $symbol, 'limit', 'sell', $order_amount, $limit_sell_price_non_fillable, array(), $skipped_properties);
        }
        $fetched_order = fetch_order($exchange, $symbol, $created_order['id'], $skipped_properties);
...
function tco_create_fillable_order($exchange, $market, $log_prefix, $skipped_properties, $best_bid, $best_ask, $limit_price_safety_multiplier_from_median, $buy_or_sell_string, $predefined_amount = null) {
    try {
        ...
        $entryorder_filled = tco_create_order_safe($exchange, $symbol, 'limit', $entry_side, $entry_amount, $entryorder_price, array(), $skipped_properties);
        ...
        $entryorder_fetched = fetch_order($exchange, $symbol, $entryorder_filled['id'], $skipped_properties);
...
function tco_try_cancel_order($exchange, $symbol, $order, $skipped_properties) {
    $order_fetched = fetch_order($exchange, $symbol, $order['id'], $skipped_properties);
