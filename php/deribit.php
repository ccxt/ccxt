...
class deribit extends Exchange {
            ...
            'api' => array(
                'public' => array(
                    'get' => array(
                        // Authentication
                        'auth' => array( 'cost' => 1 ),
                        'exchange_token' => array( 'cost' => 1 ),
                        'fork_token' => array( 'cost' => 1 ),
                        // Session management
                        'set_heartbeat' => array( 'cost' => 1 ),
                        'disable_heartbeat' => array( 'cost' => 1 ),
                        // Supporting
                        ...
                        'subscribe' => array( 'cost' => 1 ),
                        'unsubscribe' => array( 'cost' => 1 ),
                        'unsubscribe_all' => array( 'cost' => 1 ),
                        // Account management
                        ...
                        'get_last_settlements_by_currency' => array( 'cost' => 1 ),
                        'get_last_settlements_by_instrument' => array( 'cost' => 1 ),
                        'get_last_trades_by_currency' => array( 'cost' => 1 ),
                        'get_last_trades_by_currency_and_time' => array( 'cost' => 1 ),
                        'get_last_trades_by_instrument' => array( 'cost' => 1 ),
                ...
                'private' => array(
                    'get' => array(
                        ...
                        'logout' => array( 'cost' => 1 ),
                        // Session management
                        'enable_cancel_on_disconnect' => array( 'cost' => 1 ),
                        'disable_cancel_on_disconnect' => array( 'cost' => 1 ),
                        'get_cancel_on_disconnect' => array( 'cost' => 1 ),
                        // Subscription management
                        'subscribe' => array( 'cost' => 1 ),
                        'unsubscribe' => array( 'cost' => 1 ),
                        'unsubscribe_all' => array( 'cost' => 1 ),
                        // Account management
                        'change_api_key_name' => array( 'cost' => 1 ),
                        'change_scope_in_api_key' => array( 'cost' => 1 ),
                        'change_subaccount_name' => array( 'cost' => 1 ),
                        'create_api_key' => array( 'cost' => 1 ),
                        'create_subaccount' => array( 'cost' => 1 ),
                        'disable_api_key' => array( 'cost' => 1 ),
                        'disable_tfa_for_subaccount' => array( 'cost' => 1 ),
                        'enable_affiliate_program' => array( 'cost' => 1 ),
                        'enable_api_key' => array( 'cost' => 1 ),
                        'get_access_log' => array( 'cost' => 1 ),
                        ...
                        'get_position' => array( 'cost' => 1 ),
                        'get_positions' => array( 'cost' => 1 ),
                        'get_subaccounts' => array( 'cost' => 1 ),
                        'get_subaccounts_details' => array( 'cost' => 1 ),
                        'get_transaction_log' => array( 'cost' => 1 ),
                        'list_api_keys' => array( 'cost' => 1 ),
                        'remove_api_key' => array( 'cost' => 1 ),
                        'remove_subaccount' => array( 'cost' => 1 ),
                        ...
                        'sell' => array( 'cost' => 4 ),
                        'edit' => array( 'cost' => 4 ),
                        'edit_by_label' => array( 'cost' => 4 ),
                        'cancel' => array( 'cost' => 4 ),
                        'cancel_all' => array( 'cost' => 4 ),
                        ...
                        'get_user_trades_by_currency_and_time' => array( 'cost' => 1 ),
    ...
    public function parse_currency(array $rawCurrency): array {
        ...
        return $this->safe_currency_structure(array(
            ...
            'code' => $code,
            'id' => $currencyId,
            'name' => $this->safe_string($rawCurrency, 'currency_long'),
            'active' => null,
            'deposit' => null,
            'withdraw' => null,
            'type' => 'crypto',
    ...
    public function fetch_markets($params = array()): array {
        ...
        if ($fetchAllMarkets) {
            $instrumentsResponse = $this->publicGetGetInstruments($paramsFetchAllMarkets);
            $instrumentsResponses[] = $instrumentsResponse;
        } else {
            $currenciesResponse = $this->publicGetGetCurrencies($paramsFetchAllMarkets);
            //
            //     {
            //         "jsonrpc": "2.0",
            //         "result": [
            ...
            for ($i = 0; $i < count($currenciesResult); $i++) {
                ...
                $instrumentsResponse = $this->publicGetGetInstruments($this->extend($request, $paramsFetchAllMarkets));
                //
                //     {
                //         "jsonrpc":"2.0",
                //         "result":[
    ...
    public function fetch_balance($params = array()): array {
        ...
        $response = null;
        if ($code === null) {
            $response = $this->privateGetGetAccountSummaries($paramsOmitted);
        } else {
            $response = $this->privateGetGetAccountSummary($this->extend($request, $paramsOmitted));
        }
        //
        //     {
        //         "jsonrpc": "2.0",
    ...
    public function fetch_tickers(?array $symbols = null, $params = array()): array {
        ...
        $response = $this->publicGetGetBookSummaryByCurrency($this->extend($request, $paramsOmitted));
        //
        //     {
        //         "jsonrpc": "2.0",
        //         "result": [
    ...
    public function fetch_ohlcv(string $symbol, string $timeframe = '1m', ?int $since = null, ?int $limit = null, $params = array()): array {
        ...
        if ($paginate) {
            return $this->fetch_paginated_call_deterministic('fetchOHLCV', $symbol, $since, $limit, $timeframe, $paramsPaginate, 5000);
        }
        $market = $this->market($symbol);
        $request = array(
            'instrument_name' => $market['id'],
        ...
        $response = $this->publicGetGetTradingviewChartData($this->extend($request, $paramsOmitted));
        //
        //     {
        //         "jsonrpc": "2.0",
        //         "result": {
    ...
    public function fetch_trades(string $symbol, ?int $since = null, ?int $limit = null, $params = array()): array {
        ...
        if (($since === null) && !(is_array($request) && array_key_exists('end_timestamp' ?? '', $request))) {
            $response = $this->publicGetGetLastTradesByInstrument($this->extend($request, $paramsOmitted));
        } else {
            $response = $this->publicGetGetLastTradesByInstrumentAndTime($this->extend($request, $paramsOmitted));
        }
        //
        //      {
        //          "jsonrpc":"2.0",
    ...
    public function create_order(string $symbol, string $type, string $side, float $amount, ?float $price = null, $params = array()): array {
        ...
        if ($this->capitalize($side) === 'Buy') {
            $response = $this->privateGetBuy($this->extend($request, $paramsOmitted));
        } else {
            $response = $this->privateGetSell($this->extend($request, $paramsOmitted));
        }
        //
        //     {
        //         "jsonrpc": "2.0",
    ...
    public function edit_order(string $id, string $symbol, string $type, string $side, ?float $amount = null, ?float $price = null, $params = array()): array {
        ...
        $response = $this->privateGetEdit($this->extend($request, $paramsOmitted));
        $result = $this->safe_dict($response, 'result', array());
        $order = $this->safe_value($result, 'order');
        $trades = $this->safe_list($result, 'trades', array());
        $order['trades'] = $trades;
    ...
    public function fetch_positions(?array $symbols = null, $params = array()): array {
        ...
        $response = $this->privateGetGetPositions($this->extend($request, $paramsOmitted));
        //
        //     {
        //         "jsonrpc": "2.0",
        //         "id": 2236,
    ...
    public function transfer(string $code, float $amount, string $fromAccount, string $toAccount, $params = array()): array {
        ...
        if ($method === 'privateGetSubmitTransferToUser') {
            $response = $this->privateGetSubmitTransferToUser($this->extend($request, $paramsOmitted));
        } else {
            $response = $this->privateGetSubmitTransferToSubaccount($this->extend($request, $paramsOmitted));
        }
        //
        //     {
        //         "jsonrpc": "2.0",
    ...
    public function withdraw(string $code, float $amount, string $address, ?string $tag = null, $params = array()): array {
        ...
        $response = $this->privateGetWithdraw($this->extend($request, $paramsWithdrawTag));
    ...
    public function fetch_funding_rate_history(?string $symbol = null, ?int $since = null, ?int $limit = null, $params = array()): array {
        ...
        $response = $this->publicGetGetFundingRateHistory($this->extend($request, $paramsOmitted));
        //
        //    {
        //        "jsonrpc": "2.0",
        //        "id": 7617,
    ...
    public function fetch_liquidations(string $symbol, ?int $since = null, ?int $limit = null, $params = array()): array {
        ...
        if ($paginate) {
            return $this->fetch_paginated_call_cursor('fetchLiquidations', $symbol, $since, $limit, $paramsPaginate, 'continuation', 'continuation', null);
        }
        $market = $this->market($symbol);
        if ($market['spot'] === true) {
        ...
        $response = $this->publicGetGetLastSettlementsByInstrument($this->extend($request, $paramsPaginate));
        //
        //     {
        //         "jsonrpc": "2.0",
        //         "result": {
