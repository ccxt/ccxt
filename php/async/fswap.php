...
use ccxt\async\abstract\fswap as Exchange;
use ccxt\Precise;
use React\Async;
...
class fswap extends Exchange {
    ...
    public function fetch_markets($params = array ()): PromiseInterface {
        return Async\async(function () use ($params) {
            ...
            //
            $response = Async\await($this->fswapPublicGetCmcPairs ($params));
    ...
    public function fetch_currencies($params = array ()): PromiseInterface {
        return Async\async(function () use ($params) {
            ...
            //
            $response = Async\await($this->fswapPublicGetAssets ($params));
            $data = $this->safe_value($response, 'data', array());
    ...
    public function fetch_balance($params = array ()): PromiseInterface {
        return Async\async(function () use ($params) {
            ...
            //
            $response = Async\await($this->mixinPrivateGetSafeSnapshots ($params));
    ...
    public function fetch_order(string $id, ?string $symbol = null, $params = array ()): PromiseInterface {
        return Async\async(function () use ($id, $symbol, $params) {
            ...
            //
            $response = Async\await($this->fswapPrivateGetOrdersFollowId (
                $this->extend(array(
                    'follow_id' => $id,
                ), $params)
            ));
    ...
    public function fetch_my_trades(?string $symbol = null, ?int $since = null, ?int $limit = null, $params = array ()): PromiseInterface {
        return Async\async(function () use ($symbol, $since, $limit, $params) {
            ...
            // Fetch the $trades
            $response = Async\await($this->fswapPrivateGetTransactionsBaseQuoteMine ($this->extend($request, $params)));
            $data = $this->safe_value($response, 'data', array());
    ...
    public function get_safe_tx(string $asset_id, string $amount, string $memo): PromiseInterface {
        return Async\async(function () use ($asset_id, $amount, $memo) {
            ...
            $resp = Async\await($this->mixinPrivateGetSafeSnapshots (array(
                'asset' => $asset_id,
                'state' => 'unspent',
            )));
            ...
            $ghosts = Async\await($this->mixinPrivatePostSafeKeys ($recipientDetails));
    ...
    public function get_safe_tx_raw(string $hexTx, array $signaturesMap = []) {
        return Async\async(function () use ($hexTx, $signaturesMap) {
            $encodeResp = Async\await($this->ccxtProxyPostMixinEncodetx (array(
                'tx' => $hexTx,
    ...
    public function verify_safe_tx(string $hexTx) {
        return Async\async(function () use ($hexTx) {
            ...
            $raw = Async\await($this->get_safe_tx_raw($hexTx));
            $request_id = $this->uuid();
            // Verify tx
            $verifyResp = Async\await($this->mixinPrivatePostSafeTransactionRequests (array( {
                $raw,
                $request_id,
            } )));
    ...
    public function sign_safe_tx(array $tx, array $views, string $privateKey): PromiseInterface {
        return Async\async(function () use ($tx, $views, $privateKey) {
            $index = 0;
    ...
    public function send_safe_tx(string $signedRaw, string $request_id) {
        return Async\async(function () use ($signedRaw, $request_id) {
            $resp = Async\await($this->mixinPrivatePostSafeTransactions (array(
                'raw' => $signedRaw,
    ...
    public function safe_transfer(string $asset_id, string $amount, string $memo) {
        return Async\async(function () use ($asset_id, $amount, $memo) {
            $hexTx = Async\await($this->get_safe_tx($asset_id, $amount, $memo));
            array( verifiedTx, request_id ) = Async\await($this->verify_safe_tx($hexTx));
            $views = $this->safe_value(verifiedTx[0], 'views');
            $signedTx = Async\await($this->sign_safe_tx(verifiedTx, $views, $this->privateKey));
    ...
    public function create_order(string $symbol, string $type, string $side, float $amount, ?float $price, ?array () $params): PromiseInterface {
        return Async\async(function () use ($symbol, $type, $side, $amount, $price, $params) {
            ...
            var_dump ('memo:', $memo);
            // 3. Init safe tx
            $resp = Async\await($this->safe_transfer($payAssetId, (string) $amount, $memo));
