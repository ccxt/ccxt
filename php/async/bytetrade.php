...
class bytetrade extends Exchange {
    ...
    public function fetch_balance($params = array ()) : Generator {
        ...
        $balances = yield $this->publicGetBalance (array_merge($request, $params));
    ...
    public function transfer($code, $amount, $address, $message = '', $params = array ()) : Generator {
        yield;
        $this->check_required_dependencies();
        if ($this->apiKey === null) {
        ...
        $response = yield $this->publicPostTransactionTransfer ($request);
        $timestamp = $this->milliseconds();
        $statusCode = $this->safe_string($response, 'code');
        $status = '';
        if ($statusCode === '0') {
    ...
    public function withdraw($code, $amount, $address, $tag = null, $params = array ()) : Generator {
        ...
        $addressResponse = yield $this->fetch_deposit_address($code);
        ...
        $response = yield $this->publicPostTransactionWithdraw ($request);
        return array(
            'info' => $response,
