...
class upbit extends Exchange {
    ...
    private function do_fetch_currency(string $code, $params = array()) {
    ...
    private function do_fetch_currency_by_id(string $id, $params = array()) {
        // this method is for retrieving funding fees and limits per currency
        // it requires private access and API keys properly set up
        $request = array(
            'currency' => $id,
    ...
    private function do_fetch_market(string $symbol, $params = array()) {
    ...
    private function do_fetch_market_by_id(?string $id, $params = array()) {
        // this method is for retrieving trading fees and limits per market
        // it requires private access and API keys properly set up
        $request = array(
            'market' => $id,
    ...
    private function do_create_order(string $symbol, string $type, string $side, float $amount, ?float $price = null, $params = array()) {
        ...
        if ($test === true) {
            $response = Async\await($this->privatePostOrdersTest($this->extend($request, $paramsRequest)));
        } else {
            $response = Async\await($this->privatePostOrders($this->extend($request, $paramsRequest)));
        }
        //
        //     {
    ...
    private function do_edit_order(string $id, string $symbol, string $type, string $side, ?float $amount = null, ?float $price = null, $params = array()) {
        ...
        $response = Async\await($this->privatePostOrdersCancelAndNew($this->extend($request, $paramsRequest)));
        //   {
    ...
    private function do_fetch_closed_orders(?string $symbol = null, ?int $since = null, ?int $limit = null, $params = array()) {
        ...
        $response = Async\await($this->privateGetOrdersClosed($this->extend($requestUntil, $paramsUntil)));
        //
        //     [
        //         {
        //             "uuid": "637fd66-d019-4d77-bee6-8e0cff28edd9",
    ...
    private function do_withdraw(string $code, float $amount, string $address, ?string $tag = null, $params = array()) {
        ...
        if ($code !== 'KRW') {
            ...
            $response = Async\await($this->privatePostWithdrawsCoin($this->extend($request, $paramsOmitted)));
        } else {
            $response = Async\await($this->privatePostWithdrawsKrw($this->extend($request, $paramsTag)));
        }
        //
        //     {
        //         "type": "withdraw",
