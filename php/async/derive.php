...
use ccxt\async\abstract\derive as Exchange;
...
class derive extends Exchange {
    ...
    public function fetch_markets($params = array ()): PromiseInterface {
        return Async\async(function () use ($params) {
            ...
            list($spotMarkets, $swapMarkets, $optionMarkets) = Async\await(Promise\all(array( $spotMarketsPromise, $swapMarketsPromise, $optionMarketsPromise )));
            //
            // {
            //     "result" => {
            //         "instruments" => array(
    ...
    public function sign_order($order, $privateKey) {
        return $this->sign_hash($this->hash_order_message(mb_substr($order), -64), mb_substr($privateKey, -64));
    }

    public function hash_message($message) {
        // takes a utf8 encoded $message
        $binaryMessage = $this->encode($message);
        $binaryMessageLength = count($binaryMessage);
        $prefix = $this->encode('\x19Ethereum Signed Message:\n' . $this->number_to_string($binaryMessageLength));
    ...
    public function sign_hash($hash, $privateKey) {
        ...
        return '0x' . $signature['r'].padStart (64, '0') . $signature['s'].padStart (64, '0') . $this->int_to_base16($this->sum(27, $signature['v']));
    }
