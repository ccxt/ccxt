...
class gate extends Exchange {
    public function describe(): mixed {
        return $this->deep_extend(parent::describe(), array(
            ...
            'exceptions' => array(
                'exact' => array(
                    'INVALID_PARAM_VALUE' => '\\ccxt\\BadRequest', // {"label":"INVALID_PARAM_VALUE","message":"Your order size 0.003749448 USDT is too small. The minimum is 3 USDT"} or {"label":"INVALID_PARAM_VALUE","message":"Candlestick too long ago. Maximum 10000 points ago are allowed"}
                    ...
                    'POC_FILL_IMMEDIATELY' => '\\ccxt\\OrderImmediatelyFillable', // {"label":"POC_FILL_IMMEDIATELY","message":"Order would match and take immediately so its cancelled"}
                    ...
                    'ORDER_POC_IMMEDIATE' => '\\ccxt\\OrderImmediatelyFillable', // {"label":"ORDER_POC_IMMEDIATE","detail":"order price 1700 while counter price 1793.55"}
                    ...
                    'RISK_LIMIT_TOO_LOW' => '\\ccxt\\BadRequest', // {"label":"RISK_LIMIT_TOO_LOW","detail":"limit 1000000"}
                    'AUTO_TRIGGER_PRICE_LESS_LAST' => '\\ccxt\\InvalidOrder',  // {"label":"AUTO_TRIGGER_PRICE_LESS_LAST","message":"invalid argument: Trigger.Price must < last_price"}
                    'AUTO_TRIGGER_PRICE_GREATE_LAST' => '\\ccxt\\InvalidOrder', // {"label":"AUTO_TRIGGER_PRICE_GREATE_LAST","message":"invalid argument: Trigger.Price must > last_price"}
                    'POSITION_HOLDING' => '\\ccxt\\BadRequest',
                    'USER_LOAN_EXCEEDED' => '\\ccxt\\BadRequest', // {"label":"USER_LOAN_EXCEEDED","message":"Max loan amount per user would be exceeded"}
                    'NO_CHANGE' => '\\ccxt\\InvalidOrder', // {"label":"NO_CHANGE","message":"No change is made"}
                    'PRICE_THRESHOLD_EXCEEDED' => '\\ccxt\\InvalidOrder', // {"label":"PRICE_THRESHOLD_EXCEEDED","message":": 0.45288"}
                ),
                'broad' => array(
                    'Your order size' => '\\ccxt\\InvalidOrder', // {"label":"INVALID_PARAM_VALUE","message":"Your order size 0.003749448 USDT is too small. The minimum is 3 USDT"}
                ),
            ),
    ...
    public function load_unified_status($params = array()): PromiseInterface {
        return Async\async(self::do_load_unified_status(...))($params);
    }

    private function do_load_unified_status($params = array()) {
        /**
    ...
    public function upgrade_unified_trade_account($params = array()): PromiseInterface {
    ...
    private function do_fetch_spot_markets($params = array()) {
        ...
        for ($i = 0; $i < count($spotMarketsResponse); $i++) {
            ...
            $marginMarket = $this->safe_dict($marginMarkets, $id);
    ...
    public function parse_contract_market(array $market, ?string $settleId): array {
        //
        //  Perpetual swap
        //
        //     {
    ...
    private function do_fetch_option_markets($params = array()) {
        $result = array();
    ...
    private function do_fetch_option_underlyings() {
    ...
    public function get_margin_mode(?bool $trigger, array $params): array {
        /**
         * @ignore
        ...
        list($isUnifiedAccount, $params) = $this->handle_option_bool_and_params($params, 'getMarginMode', 'unifiedAccount', false);
        if ($isUnifiedAccount) {
            $marginMode = 'unified';
        }
    ...
    public function get_settlement_currencies(?string $type, ?string $method): array {
        $options = $this->safe_dict($this->options, $type, array()); // [ 'BTC', 'USDT' ] unified codes
        $fetchMarketsContractOptions = $this->safe_dict($options, $method, array());
        $defaultSettle = ($type === 'swap') ? array( 'usdt' ) : array( 'btc' );
        return $this->safe_list($fetchMarketsContractOptions, 'settlementCurrencies', $defaultSettle);
    }
    ...
    private function do_fetch_currencies($params = array()) {
        ...
        $apiBackup = $this->safe_dict($this->urls, 'apiBackup');
        if ($apiBackup !== null) {
            return array();
        }
    ...
    public function fetch_network_deposit_address(string $code, $params = array()): PromiseInterface {
        return Async\async(self::do_fetch_network_deposit_address(...))($code, $params);
    }

    private function do_fetch_network_deposit_address(string $code, $params = array()) {
        if ($this->markets === null) {
            Async\await($this->load_markets());
        }
    ...
    private function do_fetch_deposit_addresses_by_network(string $code, $params = array()) {
        ...
        $response = Async\await($this->privateWalletGetDepositAddress($this->extend($request, $params)));
        $chains = $this->safe_list($response, 'multichain_addresses', array());
        $currencyId = $this->safe_string($response, 'currency');
    ...
    public function parse_trading_fee(array $info, ?array $market = null): array {
        //
        //    {
        //        "user_id": 1486602,
        //        "taker_fee": "0.002",
    ...
    private function do_fetch_transaction_fees(?array $codes = null, $params = array()) {
        /**
         * @deprecated
        ...
        for ($i = 0; $i < count($response); $i++) {
            ...
            $withdrawFixOnChains = $this->safe_dict($entry, 'withdraw_fix_on_chains');
    ...
    public function parse_deposit_withdraw_fee(mixed $fee, ?array $currency = null): mixed {
        ...
        //    }
        //
        $withdrawFixOnChains = $this->safe_dict($fee, 'withdraw_fix_on_chains');
        $result = array(
            'info' => $fee,
            'withdraw' => array(
    ...
    private function do_fetch_balance($params = array()) {
        ...
        $isUnifiedAccount = false;
        list($isUnifiedAccount, $params) = $this->handle_option_bool_and_params($params, 'fetchBalance', 'unifiedAccount', false);
    ...
    private function do_fetch_option_ohlcv(string $symbol, ?string $timeframe = '1m', ?int $since = null, ?int $limit = null, $params = array()) {
    ...
    public function parse_ohlcv(mixed $ohlcv, ?array $market = null): array {
        ...
        if ((gettype($ohlcv) === 'array' && array_keys($ohlcv) === array_keys(array_keys($ohlcv)))) {
            ...
            return array(
                ...
                $this->safe_number($ohlcv, 'v'),    // trading volume, undefined for mark or index price
            );
        }
    }
    ...
    public function create_order_request(?string $symbol, ?string $type, ?string $side, ?float $amount, ?float $price = null, $params = array()): array {
        ...
        if ($contract === true) {
            $isClose = $this->safe_bool($params, 'close');
            if ($isClose === true) {
                $amount = 0;
            } else {
    ...
    public function edit_order_request(string $id, ?string $symbol, string $type, string $side, ?float $amount = null, ?float $price = null, $params = array()): array {
        ...
        $isUnifiedAccount = false;
        list($isUnifiedAccount, $params) = $this->handle_option_bool_and_params($params, 'editOrder', 'unifiedAccount', false);
        if ($isUnifiedAccount) {
            $account = 'unified';
        }
        $isLimitOrder = ($type === 'limit');
    ...
    public function parse_order(array $order, ?array $market = null): array {
        ...
        $put = $this->safe_dict_2($order, 'put', 'initial', array());
        ...
        return $this->safe_order(array(
            ...
            'filled' => null,
            'remaining' => $remaining,
            'fee' => $multipleFeeCurrencies ? null : $this->safe_dict($fees, 0),
    ...
    public function prepare_orders_by_status_request(?string $status, ?string $symbol = null, ?int $since = null, ?int $limit = null, $params = array()): array {
        $market = null;
        if ($symbol !== null) {
            $market = $this->market($symbol);
            $symbol = $market['symbol'];
    ...
    public function repay_isolated_margin(string $symbol, string $code, float $amount, $params = array()): PromiseInterface {
        return Async\async(self::do_repay_isolated_margin(...))($symbol, $code, $amount, $params);
    }

    private function do_repay_isolated_margin(string $symbol, string $code, float $amount, $params = array()) {
        /**
    ...
    private function do_repay_cross_margin(string $code, float $amount, $params = array()) {
        ...
        $isUnifiedAccount = false;
        list($isUnifiedAccount, $params) = $this->handle_option_bool_and_params($params, 'repayCrossMargin', 'unifiedAccount', false);
        if ($isUnifiedAccount) {
            $request['type'] = 'repay';
    ...
    public function borrow_isolated_margin(string $symbol, string $code, float $amount, $params = array()): PromiseInterface {
        return Async\async(self::do_borrow_isolated_margin(...))($symbol, $code, $amount, $params);
    }

    private function do_borrow_isolated_margin(string $symbol, string $code, float $amount, $params = array()) {
        /**
    ...
    private function do_borrow_cross_margin(string $code, float $amount, $params = array()) {
        ...
        $isUnifiedAccount = false;
        list($isUnifiedAccount, $params) = $this->handle_option_bool_and_params($params, 'borrowCrossMargin', 'unifiedAccount', false);
        if ($isUnifiedAccount) {
            $request['type'] = 'borrow';
            $response = Async\await($this->privateUnifiedPostLoans($this->extend($request, $params)));
        } else {
    ...
    private function do_fetch_borrow_interest(?string $code = null, ?string $symbol = null, ?int $since = null, ?int $limit = null, $params = array()) {
        ...
        Async\await($this->load_unified_status());
        $isUnifiedAccount = false;
        list($isUnifiedAccount, $params) = $this->handle_option_bool_and_params($params, 'fetchBorrowInterest', 'unifiedAccount', false);
        $request = array();
    ...
    private function do_modify_margin_helper(string $symbol, ?float $amount, $params = array()) {
        if ($this->markets === null) {
            Async\await($this->load_markets());
        }
        $market = $this->market($symbol);
    ...
    public function fetch_underlying_assets($params = array()): PromiseInterface {
        return Async\async(self::do_fetch_underlying_assets(...))($params);
    }

    private function do_fetch_underlying_assets($params = array()) {
        /**
    ...
    private function do_fetch_liquidations(string $symbol, ?int $since = null, ?int $limit = null, $params = array()) {
        /**
