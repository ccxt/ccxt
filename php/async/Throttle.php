...
class Throttle {
    ...
    public function __invoke($cost = null) {
        ...
        if ($this->queue->count() > $this->config['maxCapacity']) {
            throw new \RuntimeException('throttle queue is over maxCapacity (' . $this->config['maxCapacity'] . '), see https://github.com/ccxt/ccxt/issues/11645#issuecomment-1195695526');
        }
