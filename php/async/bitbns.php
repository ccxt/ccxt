...
class bitbns extends Exchange {
    ...
    private function do_create_order(string $symbol, string $type, string $side, float $amount, ?float $price = null, $params = array()) {
        ...
        $response = null;
        if ($type === 'limit') {
            $response = Async\await($this->v2PostOrders($this->extend($request, $paramsOmitted)));
        } else {
            $response = Async\await($this->v1PostPlaceMarketOrderQntySymbol($this->extend($request, $paramsOmitted)));
        }
        //
        //     {
        //         "data":"Successfully placed bid to purchase currency",
    ...
    private function do_cancel_order(string $id, ?string $symbol = null, $params = array()) {
        ...
        $response = Async\await($this->v2PostCancel($this->extend($request, $paramsOmitted)));
        $parsed = ($response === null) ? array() : $response;
        return $this->parse_order($parsed, $market);
    }
    ...
    private function do_fetch_open_orders(?string $symbol = null, ?int $since = null, ?int $limit = null, $params = array()) {
        ...
        $response = Async\await($this->v2PostGetordersnew($this->extend($request, $paramsOmitted)));
        //
        //     {
        //         "data":[
        //             {
