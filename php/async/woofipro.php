...
class woofipro extends Exchange {
    ...
    public function parse_currency(array $rawCurrency): array {
        $token = $this->safe_dict($rawCurrency, '_token', array());
        $currencyId = $this->safe_string($token, 'token');
        $networks = $this->safe_list($token, 'chain_details', array());
        $code = $this->safe_currency_code($currencyId);
        $indexedChains = $this->safe_dict($rawCurrency, '_indexedChains', array());
        ...
        for ($j = 0; $j < count($networks); $j++) {
            ...
            if ($networkCode !== null) {
                $resultingNetworks[$networkCode] = array(
                    ...
                    'info' => array( 'network' => $networkEntry, 'networkRow' => $networkRow ),
                );
            }
        }
    ...
    private function do_edit_order(string $id, string $symbol, string $type, string $side, ?float $amount = null, ?float $price = null, $params = array()) {
        ...
        if ($isConditional) {
            ...
            // request['symbol'] = market['id'];
            $response = Async\await($this->v1PrivatePutOrder($this->extend($request, $paramsOmitted2)));
        }
        //
        // {
        //     "success": true,
    ...
    private function do_cancel_order(string $id, ?string $symbol = null, $params = array()) {
        ...
        if ($trigger === true) {
            if ($isByClientOrder) {
                ...
                $response = Async\await($this->v1PrivateDeleteAlgoClientOrder($this->extend($request, $paramsOmitted2)));
            } else {
                $request['order_id'] = $id;
            ...
            if ($isByClientOrder) {
                ...
                $response = Async\await($this->v1PrivateDeleteClientOrder($this->extend($request, $paramsOmitted3)));
            } else {
                $request['order_id'] = $id;
