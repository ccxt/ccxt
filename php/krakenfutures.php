...
class krakenfutures extends Exchange {
    ...
    public function fetch_markets($params = array ()) {
        ...
        for ($i = 0; $i < count($instruments); $i++) {
            ...
            $id = $market['symbol'];
            $type = null;
            $index = (mb_strpos($market['type'], ' index') !== false);
            $linear = null;
            $inverse = null;
            $expiry = null;
            if (!$index) {
                $linear = (mb_strpos($market['type'], '_vanilla') !== false);
                $inverse = !$linear;
            ...
            $symbol = $id;
            $split = explode('_', $id);
            $parsed = $this->parse_symbol_id_joined($split[1]);
    ...
    public function parse_ticker($ticker, $market = null) {
        ...
        $volume = $this->safe_float($ticker, 'vol24h');
    ...
    public function create_order($symbol, $type, $side, $amount, $price = null, $params = array ()) {
        ...
        $this->verify_order_action_success($status, 'placed', array( 'filled' ));
    ...
    public function edit_order($id, $symbol, $type, $side, $amount = null, $price = null, $params = array ()) {
        ...
        $this->verify_order_action_success($status, 'edited', array( 'filled' ));
    ...
    public function cancel_order($id, $symbol = null, $params = array ()) {
        ...
        $this->verify_order_action_success($status, 'canceled');
        $order = array();
    ...
    public function fetch_open_orders($symbol = null, $since = null, $limit = null, $params = array ()) {
        ...
        return $this->parse_orders($response['openOrders'], $market, $since, $limit);
    }
    ...
    public function verify_order_action_success($status, $action = 'placed/edited/canceled', $omit = []) {
        $errors = array(
    ...
    public function parse_order($order, $market = null) {
        ...
        return $this->safe_order(array(
            ...
            'price' => $this->parse_number($price),
            'stopPrice' => $this->safe_number($details, 'triggerPrice'),
            'amount' => $amount,
            'cost' => $this->parse_number($cost),
            'average' => $this->parse_number($average),
    ...
    public function parse_position($position, $market = null) {
        ...
        return array(
            'info' => $position,
            'symbol' => $this->safe_currency_code($this->safe_string($position, 'symbol')),
            'timestamp' => $this->parse8601($datetime),
            'datetime' => $datetime,
            'initialMargin' => null,
            ...
            'markPrice' => null,
            'collateral' => $this->safe_string($position, 'size'),
            'marginType' => 'cross',
    ...
    public function parse_symbol_id_joined($symbolId) {
        // Convert by detecting and converting currencies in symbol
        $symbolIdLower = strtolower($symbolId);
        $quoteIds = $this->options['symbol']['quoteIds'];
        $reversed = $this->options['symbol']['reversed'];
        $method = $reversed ? 'startsWith' : 'endsWith';
        $quoteId = null;
        $baseId = null;
        for ($i = 0; $i < count($quoteIds); $i++) {
            if ($this->$method ($symbolIdLower, strtolower($quoteIds[$i]))) {
                $quoteId = $quoteIds[$i];
                break;
            }
        }
        ...
        if (!$reversed) {
            $baseIdLength = strlen($symbolId) - count($quoteId);
            $baseId = $this->slice_string($symbolId, 0, $baseIdLength);
            $quoteId = $this->slice_string($symbolId, $baseIdLength);
        } else {
            $quoteId = $this->slice_string($symbolId, 0, strlen($quoteId));
            $baseId = $this->slice_string($symbolId, strlen($quoteId));
        }
        return array(
            'baseId' => $baseId,
            'quoteId' => $quoteId,
    ...
    public function starts_with($string, $x) {
        return $this->slice_string($string, 0, strlen($x)) === $x;
    }

    public function ends_with($string, $x) {
        $start = max (0, strlen($string) - strlen($x));
        return $this->slice_string($string, $start) === $x;
    }

    public function slice_string($string, $start = null, $end = null) {
        if ($start === null) {
            $start = 0;
        }
        if ($end === null) {
