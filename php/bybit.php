...
class bybit extends \ccxt\async\bybit {
    ...
    public function parse_ws_ticker($ticker, $market = null) {
        ...
        $last = $this->safe_string_n($ticker, ['l', 'last_price', 'lastPrice']);
        $open = $this->safe_string_n($ticker, ['prev_price_24h', 'o', 'prevPrice24h']);
        $quoteVolume = $this->safe_string_n($ticker, ['v', 'turnover24h']);
        if ($quoteVolume === null) {
            $quoteVolume = $this->safe_string_2($ticker, 'turnover_24h_e8', 'turnover24hE8');
            $quoteVolume = Precise::string_div($quoteVolume, '100000000');
        }
        $baseVolume = $this->safe_string_n($ticker, ['qv', 'volume24h', 'volume_24h']);
        if ($baseVolume === null) {
            $baseVolume = $this->safe_string_2($ticker, 'volume_24h_e8', 'volume24hE8');
            $baseVolume = Precise::string_div($baseVolume, '100000000');
        }
        $bid = $this->safe_string_n($ticker, ['bidPrice', 'bid1_price', 'bid1Price']);
        $ask = $this->safe_string_n($ticker, ['askPrice', 'ask1_price', 'ask1Price']);
        $high = $this->safe_string_n($ticker, ['high_price_24h', 'high24h', 'h', 'highPrice24h']);
        $low = $this->safe_string_n($ticker, ['low_price_24h', 'low24h', 'l', 'lowPrice24h']);
    ...
    public function parse_ws_trade($trade, $market = null) {
        ...
        //     }
        //
        $id = $this->safe_string_n($trade, ['trade_id', 'v', 'tradeId', 'T', 'exec_id']);
        $marketId = $this->safe_string_2($trade, 'symbol', 's');
        $market = $this->safe_market($marketId, $market);
        $symbol = $market['symbol'];
        $price = $this->safe_string_n($trade, ['p', 'price', 'execPrice']);
        $amount = $this->safe_string_n($trade, ['q', 'size', 'exec_qty', 'execQty']);
        $cost = $this->safe_string_2($trade, 'exec_value', 'execValue');
        $timestamp = $this->safe_integer_n($trade, ['trade_time_ms', 't', 'tradeTime', 'tradeTimeMs']);
        if ($timestamp === null) {
        ...
        $orderId = $this->safe_string_n($trade, ['o', 'order_id', 'tradeTime']);
        $fee = null;
