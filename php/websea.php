...
class websea extends Exchange {
    ...
    public function fetch_ticker(string $symbol, $params = array ()): array {
        ...
        if ($market['type'] === 'swap') {
            // 合约市场使用合约API
            $response = $this->publicGetOpenApiContract24kline ($this->extend($request, $params));
        } else {
            // 现货市场使用现货API
    ...
    public function create_order(string $symbol, string $type, string $side, float $amount, ?float $price = null, $params = array ()): array {
        ...
        if ($marketType === 'swap') {
            ...
            // 合约下单
            $response = $this->privatePostOpenApiContractAdd ($this->extend($request, $cleanParams));
        ...
        if ($orderId !== null) {
            return $this->fetch_order($orderId, $symbol, array( 'type' => $marketType ));
        }
        // 如果没有订单ID，返回解析后的结果
    ...
    public function cancel_order(string $id, ?string $symbol = null, $params = array ()) {
        ...
        if ($marketType === 'swap') {
            // 合约取消订单
            $response = $this->privatePostOpenApiContractCancel ($this->extend($request, $query));
        } else {
            // 现货取消订单
    ...
    public function fetch_order(string $id, ?string $symbol = null, $params = array ()): array {
        ...
        if ($marketType === 'swap') {
            ...
            $request['order_id'] = $id;
            $response = $this->privateGetOpenApiContractGetOrderDetail ($this->extend($request, $query));
        } else {
            ...
            $response = $this->privateGetOpenApiEntrustStatus ($this->extend($request, $query));
        }
        //
        // 合约返回示例：
        // {
    ...
    public function fetch_open_orders(?string $symbol = null, ?int $since = null, ?int $limit = null, $params = array ()): array {
        ...
        if ($marketType === 'swap') {
            // 期货当前订单列表
            $response = $this->privateGetOpenApiContractCurrentList ($this->extend($request, $query));
    ...
    public function fetch_closed_orders(?string $symbol = null, ?int $since = null, ?int $limit = null, $params = array ()): array {
        ...
        $_from = $this->safe_integer($params, 'from');
        if ($_from !== null) {
            $request['from'] = $_from;
        }
        $direct = $this->safe_string($params, 'direct', 'prev');
        $request['direct'] = $direct;
        $request['limit'] = 1000;
        // Handle margin mode for contracts
        $request['is_full'] = $this->safe_integer($params, 'is_full', 2);
        ...
        if ($marketType === 'swap') {
            ...
            $response = $this->privateGetOpenApiContractHistoryList ($this->extend($request, $query));
        } else {
            // 现货历史订单列表
        ...
        //
        $result = $this->safe_value($response, 'result', array());
        return $this->parse_orders($result, $market, $since, $limit, array( 'type' => $marketType ));
    }
