...
ccxt 库支持多种不同的语言（TypeScript、JavaScript、Python、PHP、C#、Go、Java 和 Rust）。我们鼓励开发者设计*可移植*的代码，使单一语言的用户能够阅读其他语言的代码并轻松理解。这有助于库的推广采用。主要目标是为尽可能多的现有加密货币交易所提供统一、一致且健壮的通用接口。
