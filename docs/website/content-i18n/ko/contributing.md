...
ccxt 라이브러리는 여러 언어(TypeScript, JavaScript, Python, PHP, C#, Go, Java, Rust)로 제공됩니다. 개발자들이 *이식 가능한* 코드를 설계하도록 권장하여, 단일 언어 사용자도 다른 언어의 코드를 읽고 쉽게 이해할 수 있도록 합니다. 이는 라이브러리의 채택을 돕습니다. 주요 목표는 가능한 한 많은 기존 암호화폐 거래소에 일반화되고, 통합되고, 일관되며 견고한 인터페이스를 제공하는 것입니다.
