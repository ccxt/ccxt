---
title: "Jede Börse, eine Zahl: Baue einen Cross-Exchange-Portfolio-Tracker"
description: "Dein Geld liegt auf drei Börsen, und keines ihrer Dashboards stimmt mit den anderen überein. Hol dir die Salden von allen mit einer API, bewerte alles in USDT und gib die Zahl aus, die zählt."
author: CCXT Team
date: 2026-07-08T10:00:00Z
tags: [tutorials, portfolio]
---

Wenn du auf mehr als einer Börse handelst, kennst du das Ritual: hier einloggen, dort
einloggen, Zahlen im Kopf addieren, den Staub auf dem dritten Konto vergessen. Der ganze Sinn
einer vereinheitlichten API ist, dass „Wie viel habe ich eigentlich?“ ein einziges Skript sein
sollte — Read-only-Keys, keine Order-Berechtigungen, nichts, was kaputtgehen kann.

Der Plan: Salden von jeder Börse holen, pro Währung zusammenführen, alles über die Ticker
einer Börse in USDT bewerten, eine Tabelle und eine Summe ausgeben.

```javascript tab="JavaScript"
import ccxt from 'ccxt';

const ids = ['binance', 'kraken', 'bybit'];
const totals = {};

for (const id of ids) {
    const exchange = new ccxt[id]({
        apiKey: process.env[`${id.toUpperCase()}_APIKEY`],
        secret: process.env[`${id.toUpperCase()}_SECRET`],
    });
    const balance = await exchange.fetchBalance();
    for (const [currency, amount] of Object.entries(balance.total)) {
        if (amount > 0) {
            totals[currency] = (totals[currency] ?? 0) + amount;
        }
    }
    await exchange.close();
}

// alles über die Ticker einer Börse in USDT bewerten
const pricing = new ccxt.binance();
const tickers = await pricing.fetchTickers();
let portfolio = 0;
for (const [currency, amount] of Object.entries(totals)) {
    const price = (currency === 'USDT') ? 1 : tickers[`${currency}/USDT`]?.last;
    if (!price) continue;                    // kein USDT-Markt — siehe Hinweise unten
    const value = amount * price;
    portfolio += value;
    console.log(`${currency.padEnd(8)} ${amount.toFixed(6).padStart(14)} ≈ ${value.toFixed(2).padStart(12)} USDT`);
}
console.log(`${'TOTAL'.padEnd(8)} ${' '.repeat(14)} ≈ ${portfolio.toFixed(2).padStart(12)} USDT`);
await pricing.close();
```

```python tab="Python"
import os
import ccxt

IDS = ['binance', 'kraken', 'bybit']
totals = {}

for exchange_id in IDS:
    exchange = getattr(ccxt, exchange_id)({
        'apiKey': os.environ[f'{exchange_id.upper()}_APIKEY'],
        'secret': os.environ[f'{exchange_id.upper()}_SECRET'],
    })
    balance = exchange.fetch_balance()
    for currency, amount in balance['total'].items():
        if amount and amount > 0:
            totals[currency] = totals.get(currency, 0) + amount

# alles über die Ticker einer Börse in USDT bewerten
pricing = ccxt.binance()
tickers = pricing.fetch_tickers()
portfolio = 0.0
for currency, amount in sorted(totals.items()):
    if currency == 'USDT':
        price = 1.0
    else:
        ticker = tickers.get(f'{currency}/USDT')
        price = ticker['last'] if ticker else None
    if not price:
        continue                              # kein USDT-Markt — siehe Hinweise unten
    value = amount * price
    portfolio += value
    print(f'{currency:8} {amount:14.6f} ≈ {value:12.2f} USDT')

print(f'{"TOTAL":8} {"":14} ≈ {portfolio:12.2f} USDT')
```

```php tab="PHP"
<?php
$ids = ['binance', 'kraken', 'bybit'];
$totals = [];

foreach ($ids as $id) {
    $class = "\\ccxt\\{$id}";
    $exchange = new $class([
        'apiKey' => getenv(strtoupper($id) . '_APIKEY'),
        'secret' => getenv(strtoupper($id) . '_SECRET'),
    ]);
    $balance = $exchange->fetch_balance();
    foreach ($balance['total'] as $currency => $amount) {
        if ($amount > 0) {
            $totals[$currency] = ($totals[$currency] ?? 0) + $amount;
        }
    }
}

// alles über die Ticker einer Börse in USDT bewerten
$pricing = new \ccxt\binance();
$tickers = $pricing->fetch_tickers();
$portfolio = 0.0;
ksort($totals);
foreach ($totals as $currency => $amount) {
    $price = ($currency === 'USDT') ? 1.0 : ($tickers["{$currency}/USDT"]['last'] ?? null);
    if (!$price) {
        continue;                             // kein USDT-Markt — siehe Hinweise unten
    }
    $value = $amount * $price;
    $portfolio += $value;
    printf("%-8s %14.6f ≈ %12.2f USDT\n", $currency, $amount, $value);
}
printf("%-8s %14s ≈ %12.2f USDT\n", 'TOTAL', '', $portfolio);
```

```csharp tab="C#"
using ccxt;

var ids = new[] { "binance", "kraken", "bybit" };
var totals = new Dictionary<string, double>();

foreach (var id in ids)
{
    var exchange = Exchange.DynamicallyCreateInstance(id);
    exchange.apiKey = Environment.GetEnvironmentVariable($"{id.ToUpper()}_APIKEY");
    exchange.secret = Environment.GetEnvironmentVariable($"{id.ToUpper()}_SECRET");
    var balance = await exchange.FetchBalance();
    foreach (var (currency, amount) in balance.total)
    {
        if (amount > 0)
        {
            totals[currency] = totals.GetValueOrDefault(currency) + amount;
        }
    }
}

// alles über die Ticker einer Börse in USDT bewerten
var pricing = new Binance();
var tickers = await pricing.FetchTickers();
var portfolio = 0.0;
foreach (var (currency, amount) in totals.OrderBy(kv => kv.Key))
{
    double? price = currency == "USDT" ? 1.0
        : tickers.tickers.TryGetValue($"{currency}/USDT", out var t) ? t.last : null;
    if (price is not double p) continue;      // kein USDT-Markt — siehe Hinweise unten
    var value = amount * p;
    portfolio += value;
    Console.WriteLine($"{currency,-8} {amount,14:F6} ≈ {value,12:F2} USDT");
}
Console.WriteLine($"{"TOTAL",-8} {"",14} ≈ {portfolio,12:F2} USDT");
```

```go tab="Go"
package main

import (
    "fmt"
    "os"
    "sort"
    "strings"

    ccxt "github.com/ccxt/ccxt/go/v4"
)

func main() {
    ids := []string{"binance", "kraken", "bybit"}
    totals := map[string]float64{}

    for _, id := range ids {
        exchange := ccxt.CreateExchange(id, map[string]any{
            "apiKey": os.Getenv(strings.ToUpper(id) + "_APIKEY"),
            "secret": os.Getenv(strings.ToUpper(id) + "_SECRET"),
        })
        balance, err := exchange.FetchBalance()
        if err != nil {
            panic(err)
        }
        for currency, amount := range balance.Total {
            if amount != nil && *amount > 0 {
                totals[currency] += *amount
            }
        }
    }

    // alles über die Ticker einer Börse in USDT bewerten
    pricing := ccxt.NewBinance(nil)
    tickers, err := pricing.FetchTickers()
    if err != nil {
        panic(err)
    }
    currencies := make([]string, 0, len(totals))
    for currency := range totals {
        currencies = append(currencies, currency)
    }
    sort.Strings(currencies)
    portfolio := 0.0
    for _, currency := range currencies {
        price := 0.0
        if currency == "USDT" {
            price = 1.0
        } else if ticker, ok := tickers.Tickers[currency+"/USDT"]; ok && ticker.Last != nil {
            price = *ticker.Last
        }
        if price == 0 {
            continue // kein USDT-Markt — siehe Hinweise unten
        }
        value := totals[currency] * price
        portfolio += value
        fmt.Printf("%-8s %14.6f ≈ %12.2f USDT\n", currency, totals[currency], value)
    }
    fmt.Printf("%-8s %14s ≈ %12.2f USDT\n", "TOTAL", "", portfolio)
}
```

```java tab="Java"
import io.github.ccxt.Exchange;

import java.util.*;

public class PortfolioTracker {

    @SuppressWarnings("unchecked")
    public static void main(String[] args) {
        var ids = List.of("binance", "kraken", "bybit");
        var totals = new TreeMap<String, Double>();

        // dynamische Instanzen liefern untypisierte Maps (ansonsten siehe die typisierte API pro Klasse)
        for (var id : ids) {
            var config = new HashMap<String, Object>();
            config.put("apiKey", System.getenv(id.toUpperCase() + "_APIKEY"));
            config.put("secret", System.getenv(id.toUpperCase() + "_SECRET"));
            var exchange = Exchange.dynamicallyCreateInstance(id, config);
            var balance = (Map<String, Object>) exchange.fetchBalance().join();
            var total = (Map<String, Object>) balance.get("total");
            for (var entry : total.entrySet()) {
                var amount = (Number) entry.getValue();
                if (amount != null && amount.doubleValue() > 0) {
                    totals.merge(entry.getKey(), amount.doubleValue(), Double::sum);
                }
            }
        }

        // alles über die Ticker einer Börse in USDT bewerten
        var pricing = new io.github.ccxt.exchanges.Binance();
        var tickers = pricing.fetchTickers();
        var portfolio = 0.0;
        for (var entry : totals.entrySet()) {
            Double price = null;
            if (entry.getKey().equals("USDT")) {
                price = 1.0;
            } else {
                var ticker = tickers.tickers.get(entry.getKey() + "/USDT");
                if (ticker != null) price = ticker.last;
            }
            if (price == null) continue;      // kein USDT-Markt — siehe Hinweise unten
            var value = entry.getValue() * price;
            portfolio += value;
            System.out.printf("%-8s %14.6f ≈ %12.2f USDT%n", entry.getKey(), entry.getValue(), value);
        }
        System.out.printf("%-8s %14s ≈ %12.2f USDT%n", "TOTAL", "", portfolio);
    }
}
```

Lass es mit **Read-only-Keys** laufen (siehe [die API-Key-Checkliste](/de/blog/api-key-security) —
ein Tracker braucht nie Handelsberechtigungen) und du bekommst so etwas wie:

```
BTC          0.084310 ≈      9021.17 USDT
ETH          1.250000 ≈      4187.50 USDT
SOL         12.400000 ≈      2145.20 USDT
USDT      1830.550000 ≈      1830.55 USDT
TOTAL                 ≈     17184.42 USDT
```

## Die ehrlichen Grenzen eines 40-Zeilen-Trackers

- **`balance.total` ist die Wahrheit des Spot-Wallets, nicht dein Nettovermögen.**
  Derivate-Margin, Staking-/Earn-Guthaben und Gelder in offenen Orders werden je nach Börse
  unterschiedlich gemeldet (`fetchBalance` nimmt Params für andere Kontotypen entgegen — z. B.
  ein `type` von `'swap'` oder `'funding'` auf Börsen, die Wallets aufteilen). Iteriere über
  die Kontotypen, wenn du sie nutzt.
- **Das Überspringen bei „kein USDT-Markt“.** Kleine Assets handeln auf deiner Preisbörse
  vielleicht nur gegen BTC oder gar nicht. Die robuste Version fällt auf `X/BTC` × `BTC/USDT`
  zurück oder bewertet jeden Saldo auf der Börse, auf der er liegt.
- **Stablecoins zu 1,0 ist eine Vereinfachung**, die wir nur für USDT selbst gemacht haben.
  Während eines Depegs wird dein Tracker dich fröhlich anlügen — bewerte USDC und Co. über
  ihre echten Märkte.
- **Cache die Ticker.** Ein `fetchTickers()` pro Durchlauf ist in Ordnung; `fetchTicker`-Aufrufe
  pro Währung sind der Weg, auf dem ein Tracker zum Rate-Limit-Problem wird.

Von hier aus ist es ein kleiner Schritt zu einem Cronjob, der pro Stunde eine CSV-Zeile
anhängt — und plötzlich hast du den Portfolio-Verlaufschart, den dir keine Börse je über
mehrere Venues hinweg geben wird. Wenn du es stattdessen live willst, pusht `watchBalance` aus
dem [WebSocket-Beitrag](/de/blog/realtime-market-data-with-websockets) jede Änderung, sobald
sie passiert.
