...
const LANGS = [
  ...
  { id: 'java', label: 'Java', cmd: 'git clone https://github.com/ccxt/ccxt --depth 1 && ./gradlew :lib:build' },
] as const;

const AI_CMD = 'curl -fsSL https://raw.githubusercontent.com/ccxt/ccxt/master/install-skills.sh | bash';
  ...
  return (
    <section className="mt-20 w-full max-w-2xl">
      ...
      <div className="mt-4 rounded-lg border bg-fd-card p-1.5">
        <div role="tablist" aria-label="Language" className="flex gap-1 overflow-x-auto px-1 pt-0.5 pb-1.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {LANGS.map((l, i) => (
            <button
              key={l.id}
              type="button"
