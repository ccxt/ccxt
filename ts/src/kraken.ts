...
export default class kraken extends Exchange {
    ...
    override async fetchDepositAddresses (codes: Strings = undefined, params = {}): Promise<DepositAddress[]> {
        ...
        const methodParams = this.omit (params, [ 'method', 'new' ]);
        params = this.omit (params, 'method');
        const result: DepositAddress[] = [];
        for (let i = 0; i < codes.length; i++) {
            const code = codes[i];
            const currency = this.currency (code);
            const requestedNetworkId = this.networkCodeToId (networkCode, currency['code']);
            const requestedNetworkCode = this.networkIdToCode (requestedNetworkId, currency['code']);
            ...
            for (let j = 0; j < methods.length; j++) {
                ...
                let unifiedNetworkCode = this.parseNetwork (method);
                ...
                for (let k = 0; k < networkIds.length; k++) {
                    ...
                    if (method.indexOf (networkId) !== -1) {
                        unifiedNetworkCode = this.safeString (depositMethodNetworks, networkId, unifiedNetworkCode);
                        break;
                    }
                }
                ...
                for (let k = 0; k < addresses.length; k++) {
                    ...
                    parsed['network'] = unifiedNetworkCode;
                    result.push (parsed);
                }
            }
        }
