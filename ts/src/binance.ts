...
export default class binance extends Exchange {
    ...
    handleErrors (code, reason, url, method, headers, body, response, requestHeaders, requestBody) {
        ...
        if (message !== undefined) {
            ...
            this.throwExactlyMatchedException (this.getExceptionsByUrl (url, 'exact'), message, userMessage);
            this.throwExactlyMatchedException (this.exceptions['exact'], message, userMessage);
            this.throwBroadlyMatchedException (this.getExceptionsByUrl (url, 'broad'), message, userMessage);
        ...
        if (error !== undefined) {
            ...
            this.throwExactlyMatchedException (this.getExceptionsByUrl (url, 'exact'), error, userMessage);
        ...
        if (Array.isArray (response)) {
            ...
            if (arrayLength === 1) { // when there's a single error we can throw, otherwise we have a partial success
                ...
                if (errorCode !== undefined) {
                    this.throwExactlyMatchedException (this.getExceptionsByUrl (url, 'exact'), errorCode, errorCode + ': ' + userMessage);
