...
// ---------------------------------------------------------------------------
//
// insightx CCXT Exchange adapter  (https://insightx.finance)
//
...
export default class insightx extends Exchange {
    override describe (): any {
        return this.deepExtend (super.describe (), {
            ...
            'options': {
                ...
                'obDomainName': 'insightxCTFExchange',
                'obDomainVersion': '1',
                'networks': {
                    '2741': 'Abstract',
                    '59144': 'Linea',
     ...
     * @description retrieves data on all markets for insightx, each prediction market becomes one market with its outcome tokens listed under the outcomes key
     * @see https://docs.insightx.markets/builders/insightx-api-reference
    ...
    override async fetchMarkets (params = {}): Promise<Market[]> {
        ...
        for (let i = 0; i < rawMarkets.length; i++) {
            ...
            const m = this.parseinsightxMarket (raw);
            flatMarkets.push (m);
     ...
     * @ignore
     * @method
     * @name insightx#fetchRawMarketsBySearch
     * @description fetches raw insightx market objects matching the given search terms via the markets keyword filter
     * @see https://docs.insightx.markets/builders/insightx-api-reference
     ...
     * @returns {object[]} an array of raw insightx market objects
     */
    async fetchRawMarketsBySearch (queries: any[], params = {}): Promise<any[]> {
        ...
        for (let i = 0; i < queries.length; i++) {
            const q = queries[i];
            const response = await this.insightxPublicGetMarkets (this.extend ({
                'keyword': q,
                'state': state,
                'limit': limit,
            }, rest));
     ...
     * @ignore
     * @method
     * @name insightx#fetchRawMarketsList
     * @description fetches raw insightx market objects from the paginated markets listing
     * @see https://docs.insightx.markets/builders/insightx-api-reference
     * @param {object} [params] extra exchange-specific parameters
     * @param {string} [params.state] 'open', 'closed' or 'resolved', defaults to options.defaultMarketStatus
     * @returns {object[]} an array of raw insightx market objects
    ...
    async fetchRawMarketsList (params = {}): Promise<any[]> {
        ...
        // every open insightx market. Cap the total number of markets collected.
    ...
    override async fetchEvent (id: string, params = {}): Promise<PredictionEvent> {
        ...
        const market = this.parseinsightxMarket (response);
     ...
     * @ignore
     * @method
     * @name insightx#fetchRawMarketById
     * @description fetches a single raw insightx market object by its unified event id (a composite networkId:marketId)
    ...
    async fetchRawMarketById (id: string, params = {}): Promise<any> {
        ...
        return await this.insightxPublicGetMarketsId (this.extend (request, params));
    }

    /**
     * @ignore
     * @method
     * @name insightx#fetchRawQuestionById
     * @description fetches a single raw insightx question object by question id; falls back to keyword search by id/slug/title when direct lookup is unavailable
    ...
    async fetchRawQuestionById (id: string, params = {}): Promise<any> {
        ...
        try {
            result = await this.insightxPublicGetQuestionsId (this.extend (request, params));
        } catch (e) {
            ...
            const response = await this.insightxPublicGetQuestions (this.extend (keywordRequest, params));
     ...
     * @ignore
     * @method
     * @name insightx#fetchRawQuestionsBySearch
     * @description fetches raw insightx question objects matching the given search terms via the questions keyword filter
     * @param {string[]} queries search terms
     * @param {object} [params] extra parameters specific to the exchange API endpoint
     * @returns {object[]} an array of raw insightx question objects
     */
    async fetchRawQuestionsBySearch (queries: string[], params = {}): Promise<any[]> {
        ...
        for (let i = 0; i < queries.length; i++) {
            const q = queries[i];
            const response = await this.insightxPublicGetQuestions (this.extend ({
                'keyword': q,
                'limit': limit,
            }, rest));
            const foundList = this.safeList (response, 'data', response);
     ...
     * @ignore
     * @method
     * @name insightx#fetchRawQuestionsList
     * @description fetches raw insightx question objects from the paginated questions listing
     * @param {object} [params] extra parameters specific to the exchange API endpoint
    ...
    async fetchRawQuestionsList (params = {}): Promise<any[]> {
        ...
        while (true) {
            ...
            const response = await this.insightxPublicGetQuestions (this.extend (request, rest));
    ...
    /**
     * @method
     * @name insightx#fetchPositions
     * @description fetch the open outcome-token positions held by a wallet (insightx settles trades on-chain, so only read-only portfolio data is exposed by the API)
     * @see https://docs.insightx.markets/builders/insightx-api-reference
    ...
    override async fetchPositions (outcomes: Strings = undefined, params = {}): Promise<PredictionPosition[]> {
        ...
        const response = await this.insightxPublicGetUsersAddressPortfolio (this.extend ({ 'address': address }, rest));
        //
        //     {
        //         "data": [
        //             {
     ...
     * @description parses a raw insightx portfolio entry into a unified position structure
     * @param {object} position the raw portfolio entry
     * @param {object} [market] not used by insightx
    ...
    /**
     * @method
     * @name insightx#fetchTradeQuote
     * @description fetches a trade quote — price, shares, fees and the on-chain calldata — for buying or selling an outcome. insightx settles trades on-chain, so this returns the calldata to submit to the prediction-market contract rather than placing an off-chain order
     * @see https://docs.insightx.markets/builders/insightx-api-reference
    ...
    async fetchTradeQuote (outcome: Str, side: Str, amount: Num, params = {}): Promise<Dict> {
        ...
        const response = await this.insightxPublicPostMarketsQuote (this.extend (request, rest));
        //
        //     {
        //         "value": 10,
     ...
     * @ignore
     * @method
     * @name insightx#parseTradeQuote
     * @description parses a raw insightx quote response into a unified-ish quote object
    ...
    override async ethRpc (rpcUrl: Str, method: string, rpcParams: any[]) {
    ...
    async ensureErc20Allowance (rpcUrl: Str, networkId: Str, token: Str, owner: Str, spender: Str): Promise<any> {
     ...
     * @description create a trade order. insightx has two trading models: a gasless order book (CLOB) where an EIP-712 signed order is posted off-chain and settled by the operator, and an on-chain AMM. Order-book markets are used by default; the model can be forced via params.tradingModel
     * @see https://docs.insightx.markets/builders/insightx-order-book/order-book-api#37dc9e49da8281e2bc49cf4914b07528
     * @param {string} outcome unified outcome or outcome id
     ...
     * @ignore
     * @method
     * @name insightx#createOrderbookOrder
    ...
    async createOrderbookOrder (outcome: Str, type: Str, side: Str, amount: Num, price: Num = undefined, params = {}): Promise<PredictionOrder> {
        ...
        const response = await this.insightxPublicPostOrders (request);
        //
        //     {
     ...
     * @ignore
     * @method
     * @name insightx#buildOrderbookOrder
    ...
    /**
     * @method
     * @name insightx#createOrders
     * @description places multiple order book orders. insightx's batch endpoint is not reliable, so the
    ...
    /**
     * @method
     * @name insightx#editOrder
     * @description edits an open order by cancelling it and placing a replacement (gasless). insightx's
     * batch-modify endpoint is not reliable, so the cancel and replace are submitted sequentially
     * @see https://docs.insightx.markets/builders/insightx-order-book/order-book-api#37dc9e49da8281b58c5adb2f5998eec8
    ...
    /**
     * @method
     * @name insightx#createMarketBuyOrderWithCost
    ...
    override async createMarketBuyOrderWithCost (outcome: string, cost: number, params = {}): Promise<PredictionOrder> {
        // insightx's AMM prices buys in COLLATERAL, so `cost` maps directly onto the AMM value input.
     ...
     * @ignore
     * @method
     * @name insightx#signOrderbookTypedData
    ...
    signOrderbookTypedData (types: Dict, message: Dict, networkId: string): string {
        ...
        const domainName = this.safeString (this.options, 'obDomainName', 'insightxCTFExchange');
     ...
     * @ignore
     * @method
     * @name insightx#clobOrderMessage
     ...
     * @ignore
     * @method
     * @name insightx#getOrderResponseFromParams
     ...
     * @ignore
     * @method
     * @name insightx#parseAmmEventToOrder
    ...
    async fetchAmmOrders (outcome: Str = undefined, since: Int = undefined, limit: Int = undefined, params = {}): Promise<PredictionOrder[]> {
        ...
        const response = await this.insightxPublicGetUsersAddressEvents (this.extend (request, params));
        //
        //     {
        //         "data": [
        //             {
    ...
    override async cancelOrder (id: string, outcome: Str = undefined, params = {}): Promise<PredictionOrder> {
        ...
        if (fetched === undefined) {
            fetched = await this.insightxPublicGetOrdersHash (this.extend ({ 'hash': id }, params));
        }
        ...
        const response = await this.insightxPublicDeleteOrdersHash (this.extend (request, params));
        //
        //     {
        //         "orderHash": "0x758a1763c59bbe61c314f3c0c9b5bae0ad942120500eb39e3e8349bbe13990e0",
        //         "status": "cancelled"
    ...
    /**
     * @method
     * @name insightx#cancelAllOrders
     * @description cancels all open order book orders for the wallet, optionally scoped to one market (gasless)
     * @see https://docs.insightx.markets/builders/insightx-order-book/order-book-api#37dc9e49da8281e7a14cd34e6a716761
    ...
    async cancelAllOrders (outcome: Str = undefined, params = {}): Promise<any> {
        ...
        return await this.insightxPublicPostOrdersCancelAll (request);
        //
        //     {
        //         "cancelled_count": 2,
    ...
    /**
     * @method
     * @name insightx#cancelOrders
     * @description cancels multiple open order book orders by hash in one request (gasless)
     * @see https://docs.insightx.markets/builders/insightx-order-book/order-book-api#37dc9e49da828177961fd94a6055966f
     * @param {string[]} ids the order hashes to cancel
     * @param {string} [outcome] not used by insightx cancelOrders
     * @param {object} [params] extra parameters specific to the exchange API endpoint
    ...
    override async cancelOrders (ids: string[], outcome: Str = undefined, params = {}): Promise<PredictionOrder[]> {
        ...
        for (let i = 0; i < idsLength; i++) {
            ...
            if (fetched === undefined) {
                fetched = await this.insightxPublicGetOrdersHash ({ 'hash': id });
            }
            const fetchedInfo = this.safeDict (fetched, 'info', {});
            let rawOrder = this.safeDict (fetched, 'order', {});
            const rawOrderKeys = Object.keys (rawOrder);
        ...
        await this.insightxPublicPostOrdersCancelBatch (this.extend (request, params));
        //
        //     {
        //         "cancelled": [
    ...
    /**
     * @method
     * @name insightx#fetchOrder
     * @description fetches a single order book order by its hash
     * @see https://docs.insightx.markets/builders/insightx-order-book/order-book-api#37dc9e49da828116b8a0d976baea1df0
    ...
    async fetchOrder (id: string, outcome: Str = undefined, params = {}): Promise<PredictionOrder> {
        const response = await this.insightxPublicGetOrdersHash (this.extend ({ 'hash': id }, params));
        //
        //     {
        //         "orderHash": "0x758a1763c59bbe61c314f3c0c9b5bae0ad942120500eb39e3e8349bbe13990e0",
        //         "clientOrderId": null,
     ...
     * @see https://docs.insightx.markets/builders/insightx-order-book/order-book-api#37dc9e49da828171a003cf996487d008
     * @param {string} [outcome] unified outcome to filter by
     * @param {int} [since] timestamp in ms of the earliest order
     * @param {int} [limit] the maximum number of orders to return
     * @param {object} [params] extra parameters specific to the exchange API endpoint
    ...
    override async fetchOrders (outcome: Str = undefined, since: Int = undefined, limit: Int = undefined, params = {}): Promise<PredictionOrder[]> {
        ...
        const response = await this.insightxPublicGetOrders (this.extend (request, params));
        //
        //     {
        //         "data": [
        //             {
    ...
    /**
     * @method
     * @name insightx#fetchMyTrades
     * @description fetches the wallet's filled order book orders as trades. Note: insightx's REST exposes the order's
     ...
     * @ignore
     * @method
     * @name insightx#parseMarketToEvent
     * @description wraps a parsed insightx market into a unified event structure
     * @param {object} raw the raw insightx market object
     ...
     * @ignore
     * @method
     * @name insightx#parseinsightxMarket
     * @description converts a single raw insightx market into one ccxt market with a list of outcome objects
     * @param {object} raw the raw insightx market object
     ...
     */
    parseinsightxMarket (raw: Dict, eventSlug: Str = undefined): Market {
        const networkId = this.safeString (raw, 'networkId');
        const marketId = this.safeString (raw, 'id');
        ...
        // qualify the handle only with a real event slug (when passed); insightx market slugs are
    ...
    override async fetchTicker (outcome: string, params = {}): Promise<PredictionTicker> {
        ...
        const response = await this.insightxPublicGetMarketsId (this.extend (request, params));
        //
        //     {
        //         "id": "756",
        //         "networkId": "2741",
     ...
     * @description parses a raw insightx market object into a unified ticker for the specified outcome
     * @param {object} raw the raw insightx market object
    ...
    override parsePredictionTicker (raw: Dict, market: Market = undefined): PredictionTicker {
        ...
        return this.safePredictionTicker ({
            ...
            'average': price,
            // insightx's `volume*` fields are collateral (USDC); `volumeNotional*` is the share count —
     ...
     * @see https://docs.insightx.markets/builders/insightx-order-book/order-book-api#37dc9e49da8281bba6aaf24dd61f2bb1
     * @param {string} outcome unified outcome like TRUMP_WIN:YES or an outcome id
     * @param {int} [limit] not used by insightx fetchOrderBook
     * @param {object} [params] extra parameters specific to the exchange API endpoint
    ...
    override async fetchOrderBook (outcome: Str, limit: Int = undefined, params = {}): Promise<PredictionOrderBook> {
        ...
        if (tradingModel === 'ob') {
            ...
            const obResponse = await this.insightxPublicGetMarketsIdOrderbook (this.extend (obRequest, params));
            //
            //     {
     ...
     * @ignore
     * @method
     * @name insightx#parseWeiOrderBook
    ...
    override async fetchOHLCV (outcome: string, timeframe = '1d', since: Int = undefined, limit: Int = undefined, params = {}): Promise<OHLCV[]> {
        ...
        const response = await this.insightxPublicGetMarketsId (this.extend ({
            'id': marketId,
            'network_id': networkId,
        }, params));
        //
        ...
        let chart: NullableDict = undefined;
     ...
     * @ignore
     * @method
     * @name insightx#parseOHLCV
     * @description parses a single insightx price chart data point into an ohlcv tuple
     ...
     * @see https://docs.insightx.markets/builders/insightx-api-reference
     * @param {string[]} outcomes unified outcomes — required: insightx has no endpoint returning all tickers at once, so an unscoped call is not supported
     * @param {object} [params] extra parameters specific to the exchange API endpoint
    ...
    override async fetchTickers (outcomes: Strings = undefined, params = {}): Promise<PredictionTickers> {
        ...
        for (let i = 0; i < marketKeys.length; i++) {
            ...
            promises.push (this.insightxPublicGetMarketsId (this.extend ({
                'id': this.safeString (info, 'marketId'),
                'network_id': this.safeString (info, 'networkId'),
            }, params)));
        }
    ...
    override async fetchTrades (outcome: string, since: Int = undefined, limit: Int = undefined, params = {}): Promise<PredictionTrade[]> {
        ...
        const response = await this.insightxPublicGetMarketsIdEvents (this.extend (request, params));
        //
        //     {
        //         "data": [
        //             {
     ...
     * @ignore
     * @method
     * @name insightx#parsePredictionTrade
     ...
     * @description parses a raw insightx question object into the unified event shape with a nested markets list
     * @param {object} rawEvent the raw insightx question object
     * @returns {object} an event structure
     */
    parseEvent (rawEvent: Dict): any {
        ...
        for (let i = 0; i < rawMarkets.length; i++) {
            ...
            marketsList.push (this.parseinsightxMarket (rawMarket, questionSlug));
        }
    ...
    async subscribeinsightxChannel (messageHash: string, channel: string, params = {}): Promise<any> {
     ...
     * @see https://docs.insightx.markets/builders/insightx-order-book/order-book-api#37dc9e49da82810581f8d2c8be2364fa
    ...
    override async watchTrades (outcome: string, since: Int = undefined, limit: Int = undefined, params = {}): Promise<PredictionTrade[]> {
        ...
        const trades = await this.subscribeinsightxChannel (messageHash, channel, params);
        return this.filterBySinceLimit (trades, since, limit, 'timestamp', true) as PredictionTrade[];
    }

    /**
     * @method
     * @name insightx#watchMyTrades
    ...
    override async watchTicker (outcome: string, params = {}): Promise<PredictionTicker> {
        ...
        return await this.subscribeinsightxChannel (messageHash, channel, params);
    }

    /**
     * @method
     * @name insightx#watchTickers
    ...
    override async watchOHLCV (outcome: string, timeframe = '1m', since: Int = undefined, limit: Int = undefined, params = {}): Promise<OHLCV[]> {
        // insightx has no OHLCV websocket channel, so build candles from the live trade stream
    ...
    override async watchOrders (outcome: Str = undefined, since: Int = undefined, limit: Int = undefined, params = {}): Promise<PredictionOrder[]> {
        ...
        const orders = await this.subscribeinsightxChannel (messageHash, channel, params);
    ...
    override handleErrors (code: int, reason: string, url: string, method: string, headers: Dict, body: string, response, requestHeaders, requestBody) {
        // insightx error responses are { "error": "<message>", "details": [...] } with a 4xx status
