...
export default class bigone extends Exchange {
    ...
    async fetchTickers (symbols: string[] = undefined, params = {}) {
        ...
        if (isSpot) {
            ...
            tickers = await this.contractPublicGetInstruments (params);
            //
            //    [
            //        {
            //            "usdtPrice": 1.00031998,
    ...
    async fetchOrderBook (symbol: string, limit: Int = undefined, params = {}) {
        ...
        if (market['contract']) {
            ...
            //    }
            //
            return this.parseOrderBook (response, market['symbol'], undefined, 'bids', 'asks', 'price', 'quantity'); // TODO
        } else {
            const request = {
                'asset_pair_name': market['id'],
            };
    ...
    async createOrder (symbol: string, type: OrderType, side: OrderSide, amount, price = undefined, params = {}) {
         ...
         * @param {string} [params.operator] *spot stop order only* GTE or LTE (default)
         * @param {string} [params.client_order_id] *spot only* must match ^[a-zA-Z0-9-_]{1,36}$ this regex. client_order_id is unique in 24 hours, If created 24 hours later and the order closed, it will be released and can be reused
         * @param {object} [params.priceType] *contract only* MARK_PRICE, INDEX_PRICE, MARKET_PRICE (default)
         *
    ...
    async cancelOrder (id: string, symbol: string = undefined, params = {}) {
        ...
        if ((type === 'swap') || (type === 'future')) {
            ...
            response = await this.privatePostOrdersIdCancel (this.extend (request, params));
            //
            //    {
            //        "id": 10,
            //        "asset_pair_name": "EOS-BTC",
    ...
    async fetchOrders (symbol: string = undefined, since: Int = undefined, limit: Int = undefined, params = {}) {
         ...
         * @param {object} [params] extra parameters specific to the bigone api endpoint
         * @param {int} [params.until] *contrat only* the latest time in ms to fetch orders for
         *
         * EXCHANGE SPECIFIC PARAMETERS
         * @param {string} [params.page_token] *spot only* request page after this page token
         * @param {string} [params.side] 'ASK' or 'BID' (*spot*), BUY or SELL (*contract*)
         * @param {string} [params.state] *spot only* 'CANCELLED', 'FILLED', 'PENDING'
         * @param {string} [params.id] *contract only* Order id of previous response for pagination.
         * @param {string} [params.type] *contract only* Order types, includes LIMIT, MARKET, IOC, FOK, POST_ONLY, ADL
         * @param {string} [params.status] *contract only* Order status, includes NEW, PARTIALLY_FILLED, PARTIALLY_CANCELED, CANCELED, REJECTED, FILLED, UNTRIGGERED, PENDING_CANCEL, TRIGGERED, separated by comma
         * @param {string} [params.opt] *contract only* TRIGGERED or LIQUIDATE
        ...
        if ((type === 'swap') || (type === 'future')) {
            ...
            params = this.omit (params, [ 'state', 'until' ]);
        } else {
            request['asset_pair_name'] = market['id'];
        }
        if (limit !== undefined) {
