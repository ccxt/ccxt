...
function ecdsa (request: Hex, secret: Hex, curve: CurveFn, prehash?: CHash, fixedLength = false) {
    if (prehash) {
        ...
        if (secret.startsWith ('-----BEGIN EC PRIVATE KEY-----')) {
            ...
            if (asn1.sub.length === 4) {
                ...
                if (typeof asn1.sub[2].sub !== null && asn1.sub[2].sub.length > 0) {
                    const oid = asn1.sub[2].sub[0].content (undefined);  // TODO: Argument of type 'undefined' is not assignable to parameter of type 'number'.ts(2345)
                    if (supportedCurve[oid] === undefined) throw new Error('Unsupported curve');  // TODO: Type 'null' cannot be used as an index type.ts(2538)
                    curve = supportedCurve[oid];  // TODO: Type 'null' cannot be used as an index type.ts(2538)
                }
...
function axolotl (request: Hex, secret: Hex, curve: CurveFnEDDSA) {
    ...
    if (curve.signModified === undefined) {
        throw new Error ('curve.signModified is undefined');
    } else {
...
function crc32 (str: string, signed = false) {
