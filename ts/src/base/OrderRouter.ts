...
class OrderRouter {
    ...
    //  stale. The second costs an authenticated call per venue, goes stale the moment anything
    //  moves, and made one bad key enough to kill a quote. So the filter is always on and the
    //  wallet reads are a mode you ask for. There is deliberately no middle setting: a router
    //  that half-knows your balances is worse than one that knows none of them.
     ...
     * @returns {undefined}
     */
    assertBalancesApplied (route: Dict, params: Dict) {
    ...
    async request (url: string, method: string = 'GET', requestBody: Dict = {}, requestId: string = ''): Promise<Dict> {
        ...
        const options: Dict = { 'method': method, 'headers': headers };
        if (method === 'POST') {
     ...
     */
    async fetchHealth (): Promise<Dict> {
     ...
     */
    async fetchReadiness (): Promise<Dict> {
     ...
     */
    async fetchVersion (): Promise<Dict> {
     ...
     */
    async fetchExchangesStatus (): Promise<any[]> {
     ...
     */
    async fetchCachedOrderBook (exchangeId: string, symbol: string): Promise<Dict> {
