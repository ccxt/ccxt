...
export default class bullish extends Exchange {
        ...
        return this.deepExtend (super.describe (), {
            ...
            'exceptions': {
                'exact': {
                    ...
                    '6023': InvalidOrder, // {"message":"Order size outside valid range","raw":null,"errorCode":6023,"errorCodeName":"ORDER_SIZE_OUTSIDE_VALID_RANGE"}
                    ...
                    '8421': AuthenticationError, // {"statusReason":"Signature is invalid","statusReasonCode":8421}
                },
                'broad': {
                    'Bad request': BadRequest, // {"message":"Bad request parameters","errorCode":99999,"errorCodeName":"Bad request parameters"}
    ...
    sign (path, api = 'public', method = 'GET', params = {}, headers = undefined, body = undefined) {
        ...
        if (api === 'private') {
            ...
            let nonce = this.numberToString (this.microseconds ());
            ...
            if (method === 'GET') {
                ...
                if (path === 'v1/wallets/withdrawal') {
                    params['timestamp'] = timestamp;
                    nonce = this.uuid ();
                    params['nonce'] = nonce; // handle with signature error for withdrawal endpoint
                    params['authorizer'] = this.options['authorizer'];
                }
                ...
                if (path !== 'v1/wallets/withdrawal') {
                    headers['BX-TIMESTAMP'] = timestamp;
                    headers['BX-NONCE'] = nonce;
                }
    ...
    async signIn (params = {}) {
        ...
        this.options['authorizer'] = this.safeString (response, 'authorizer');
        return token;
    }
    ...
    handleErrors (httpCode: int, reason: string, url: string, method: string, headers: Dict, body: string, response, requestHeaders, requestBody) {
        ...
        //         }
        //
        let code = this.safeString (response, 'errorCode');
        const statusReasonCode = this.safeString (response, 'statusReasonCode');
        if (code === undefined && statusReasonCode !== undefined) {
