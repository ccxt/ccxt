...
async function testWsObSyncingFirstLoop (currentInstance: Exchange, otherInstance: Exchange, skippedProperties: any, symbol: string, ends: number) {
    ...
    while (now < ends) {
        ...
        if (('tempObNonces' in otherInstance.options) && (nonceString in otherInstance.options['tempObNonces'])) {
            ...
            testSharedMethods.assertDeepEqual (currentInstance, skippedProperties, 'testWsOrderBookSynchronization', ob1, ob2);
        }
