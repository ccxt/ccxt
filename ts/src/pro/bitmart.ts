...
export default class bitmart extends bitmartRest {
    ...
    async watchTradesForSymbols (symbols: string[], since: Int = undefined, limit: Int = undefined, params = {}): Promise<Trade[]> {
        ...
        const trades = await this.subscribeMultiple ('trade', channelName, marketType as string, symbols, params);
        if (this.newUpdates) {
    ...
    parseWsTrade (trade: Dict, market: Market = undefined) {
        ...
        let takerOrMaker: Str; // true for public trades
    ...
    handleOHLCV (client: Client, message) {
        ...
        if (isSpot) {
            for (let i = 0; i < data.length; i++) {
                ...
                parsed[0] = this.parseToInt ((parsed[0] as number) / durationInMs) * durationInMs;
