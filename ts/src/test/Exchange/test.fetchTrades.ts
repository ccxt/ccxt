...
async function testFetchTradesResponse (exchange: Exchange, skippedProperties: object, symbol: string, method: string, trades: object[]) {
    ...
    if (!('requireBothSides' in skippedProperties) && !('side' in skippedProperties) && trades.length > 50) {
...
async function testFetchTradesStructure (exchange: Exchange, skippedProperties: object, symbol: string, method: string, trades: object[]) {
...
async function testFetchTradesSidesBuySell (exchange: Exchange, skippedProperties: object, symbol: string, method: string, trades: object[]) {
    //
...
async function testFetchTradesSideSequence (exchange: Exchange, skippedProperties: object, symbol: string, method: string, trades: object[]) {
    //
    ...
    //   otherwise (if such rare event happens ever, the test can be restarted and
