...
export default class digitra extends Exchange {
    ...
    parseTimestamp (data, key) {
        ...
        return this.safeTimestamp ([ timestampMicro / 1000 / 1000 ], 0);
    }
    ...
    async fetchTrades (symbol: string, since: Int = undefined, limit: Int = undefined, params = {}) {
        ...
        if (params['cursor'] !== undefined) {
            request['cursor'] = this.safeString (params, 'cursor', '');
        }
    ...
    sign (path, api = 'public', method = 'GET', params = {}, headers = undefined, body = undefined) {
        ...
        if (Object.keys (query).length) {
            if (method === 'GET') {
                ...
                // We need to do this, because Digitra backend deserialized the json body
                // and then serialized it again to calculate the signature. The problem is
                // when they reserialize it, their backend adds spaces between the keys and
                // values, which causes the signature to be invalid when compared to our json body.
                // Example:
                // {"key":"value","key2":"value2"} => {"key": "value", "key2": "value2"}
                // json.Stringify() does not add spaces between keys and values, so we need to
                // do it manually, to comply with their expectations when calculating the signature.
    ...
    handleErrors (httpCode, reason, url, method, headers, body, response, requestHeaders, requestBody) {
        ...
        if (httpCode < 200 || httpCode > 204) {
            throw new ExchangeError (this.id + ' http status code error ' + httpCode + ' response body ' + this.json (response));
        }
        return undefined;
    }
}
