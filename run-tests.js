...
const testExchange = async (exchange) => {
    const args = [exchange, ...symbol === 'all' ? [] : symbol],
        allTests       = [{ language: 'JavaScript', key: '--js', exec: ['node', 'js/test/test.js', ...args] }],
    ...
    log.bright (('[' + percentsDone + ']').dim, 'Testing', exchange.cyan, (
        failed ? 'FAIL'.red : (hasWarnings ? (warnings.length ? warnings.join (' ') : 'WARN').yellow : 'OK'.green))
    );
...
(async function () {
    ...
    const warnings  = tested.filter(t => !t.failed && t.hasWarnings);
    const failed    = tested.filter(t =>  t.failed);
    const succeeded = tested.filter(t => !t.failed && !t.hasWarnings);
