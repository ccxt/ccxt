...
- **Live data (WebSocket)**: `watch_subscribe` opens a background stream and returns a `subscriptionId` and a `streamKind`. For **state** streams (only the single-symbol `watchTicker`/`watchOrderBook`, which return the complete current object each call) `watch_read` returns the **current snapshot** in `latest` — just read it whenever you need the current value (e.g. the live order book). For **event** streams (everything else — `watchTrades`/`watchOHLCV`, plus the private `watchMyTrades`/`watchOrders`/`watchBalance`/`watchPositions` needing an account) `watch_read` returns new/changed items in `events` plus a `nextCursor` — thread it back as `cursor` on each poll so you don't re-receive old items. Event streams deliver only what changed since the last update, not a full set — for a full current snapshot (all balances/positions/tickers) call the matching `get_*` tool. `watch_unsubscribe` when done; `watch_list` shows active streams. Prefer one-shot `get_*` tools for a single snapshot; use `watch_*` for live/continuous data.
