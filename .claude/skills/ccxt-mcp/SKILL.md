...
- **Live data (WebSocket)**: `watch_subscribe` opens a background stream and returns a `subscriptionId` and a `streamKind`. For **state** streams (`watchTicker`/`watchOrderBook`/`watchOHLCV`/`watchBalance`/`watchPositions`) `watch_read` returns the **current snapshot** in `latest` — just read it whenever you need the current value (e.g. the live order book). For **event** streams (`watchTrades`/`watchMyTrades`/`watchOrders`, the last two/three needing an account) `watch_read` returns new items in `events` plus a `nextCursor` — thread it back as `cursor` on each poll so you don't re-receive old items. `watch_unsubscribe` when done; `watch_list` shows active streams. Prefer one-shot `get_*` tools for a single snapshot; use `watch_*` for live/continuous data.
