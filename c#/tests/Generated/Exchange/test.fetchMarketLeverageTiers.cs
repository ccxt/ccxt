...
{
    async static public Task testFetchMarketLeverageTiers(Exchange exchange, object symbol)
    {
        object method = "fetchMarketLeverageTiers";
        object tiers = await exchange.fetchMarketLeverageTiers(symbol);
        assert((tiers.GetType().IsGenericType && tiers.GetType().GetGenericTypeDefinition().IsAssignableFrom(typeof(List<>))), add(add(add(add(add(add(exchange.id, " "), method), " "), symbol), " must return an array. "), exchange.json(tiers)));
        ...
        {
            testLeverageTier(exchange, method, getValue(tiers, j));
        }
    }
