...
{
    async static public Task testFetchClosedOrders(Exchange exchange, object symbol)
    {
        object method = "fetchClosedOrders";
        object orders = await exchange.fetchClosedOrders(symbol);
        assert((orders.GetType().IsGenericType && orders.GetType().GetGenericTypeDefinition().IsAssignableFrom(typeof(List<>))), add(add(add(add(exchange.id, " "), method), " must return an array, returned "), exchange.json(orders)));
