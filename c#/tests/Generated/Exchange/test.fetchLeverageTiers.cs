...
{
    async static public Task testFetchLeverageTiers(Exchange exchange, object symbol)
    {
        ...
        //     ],
        // };
        assert(((tiers).GetType() == typeof(Dictionary<string, object>)), add(add(add(add(add(add(exchange.id, " "), method), " "), symbol), " must return an object. "), exchange.json(tiers)));
        object tierKeys = new List<object>(((Dictionary<string,object>)tiers).Keys);
        ...
        for (object i = 0; isLessThan(i, arrayLength); postFixIncrement(ref i))
        {
            ...
            {
                testLeverageTier(exchange, method, getValue(tiersForSymbol, j));
            }
        }
    }
