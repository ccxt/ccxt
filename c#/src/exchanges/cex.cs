...
partial class cex : Exchange
{
    ...
    public async override Task<object> fetchCurrencies(object parameters = null)
    {
        ...
        */
        ...
        ((Dictionary<string, object>)this.options)["currencies"] = new Dictionary<string, object>() {
    ...
    public virtual object parseBalance(object response)
    {
        ...
        for (object i = 0; isLessThan(i, getArrayLength(currencyIds)); postFixIncrement(ref i))
        {
            ...
            ((Dictionary<string, object>)account)["used"] = this.safeString(balance, "orders", "0");
    ...
    public async override Task<object> createOrder(object symbol, object type, object side, object amount, object price = null, object parameters = null)
    {
        ...
        */
        ...
        {
            ((Dictionary<string, object>)request)["order_type"] = type;
        }
        ...
        //     }
        //
        object placedAmount = this.safeNumber(response, "amount");
        object remaining = this.safeNumber(response, "pending");
        ...
        {
            filled = mathMax(subtract(placedAmount, remaining), 0);
        }
    ...
    public override object parseOrder(object order, object market = null)
    {
        ...
        if (isTrue(isTrue(((timestamp).GetType() == typeof(string))) && isTrue(isGreaterThanOrEqual(getIndexOf(timestamp, "T"), 0))))
        {
            // ISO8601 string
        ...
        object amount = this.safeNumber(order, "amount");
        ...
        object remaining = this.safeNumber2(order, "pending", "remains");
        object filled = subtract(amount, remaining);
        object fee = null;
        object cost = null;
        if (isTrue(!isEqual(market, null)))
        {
            symbol = getValue(market, "symbol");
            object taCost = this.safeNumber(order, add("ta:", getValue(market, "quote")));
            object ttaCost = this.safeNumber(order, add("tta:", getValue(market, "quote")));
            ...
            object feeRate = this.safeNumber(order, "tradingFeeMaker");
            if (!isTrue(feeRate))
            {
            ...
            if (isTrue(feeRate))
            {
                feeRate = divide(feeRate, 100); // convert to mathematically-correct percentage coefficients: 1.0 = 100%
            }
            if (isTrue(isTrue((((Dictionary<string,object>)order).ContainsKey(toStringOrNull(baseFee)))) || isTrue((((Dictionary<string,object>)order).ContainsKey(toStringOrNull(baseTakerFee))))))
            {
            ...
            } else if (isTrue(isTrue((((Dictionary<string,object>)order).ContainsKey(toStringOrNull(quoteFee)))) || isTrue((((Dictionary<string,object>)order).ContainsKey(toStringOrNull(quoteTakerFee))))))
            {
        ...
        if (!isTrue(cost))
        {
            cost = multiply(price, filled);
        }
        object side = getValue(order, "type");
        object trades = null;
        object orderId = getValue(order, "id");
        if (isTrue(((Dictionary<string,object>)order).ContainsKey(toStringOrNull("vtx"))))
        {
            ...
            for (object i = 0; isLessThan(i, getArrayLength(getValue(order, "vtx"))); postFixIncrement(ref i))
            {
                ...
                object tradePrice = this.safeNumber(item, "price");
                ...
                object tradeAmount = this.safeNumber(item, "amount");
                object feeCost = this.safeNumber(item, "fee_amount");
                object absTradeAmount = ((bool) isTrue((isLessThan(tradeAmount, 0)))) ? prefixUnaryNeg(ref tradeAmount) : tradeAmount;
                object tradeCost = null;
                if (isTrue(isEqual(tradeSide, "sell")))
                {
                    tradeCost = absTradeAmount;
                    absTradeAmount = divide(this.sum(feeCost, tradeCost), tradePrice);
                } else
                {
                    tradeCost = multiply(absTradeAmount, tradePrice);
                }
                ((List<object>)trades).Add(new Dictionary<string, object>() {
    ...
    public async override Task<object> fetchOpenOrders(object symbol = null, object since = null, object limit = null, object parameters = null)
    {
        ...
        */
        ...
        await this.loadMarkets();
        object request = new Dictionary<string, object>() {};
        object method = "privatePostOpenOrders";
        object market = null;
        ...
        object orders = await this.callAsync(method, this.extend(request, parameters));
        for (object i = 0; isLessThan(i, getArrayLength(orders)); postFixIncrement(ref i))
        {
            ((List<object>)orders)[(int)i] = this.extend(getValue(orders, i), new Dictionary<string, object>() {
                { "status", "open" },
            });
        }
    ...
    public async override Task<object> fetchClosedOrders(object symbol = null, object since = null, object limit = null, object parameters = null)
    {
        ...
        */
        parameters ??= new Dictionary<string, object>();
        await this.loadMarkets();
        object method = "privatePostArchivedOrdersPair";
        if (isTrue(isEqual(symbol, null)))
        {
    ...
    public async override Task<object> fetchOrders(object symbol = null, object since = null, object limit = null, object parameters = null)
    {
        ...
        */
        ...
        for (object i = 0; isLessThan(i, getArrayLength(response)); postFixIncrement(ref i))
        {
            ...
            object amount = this.safeNumber(order, "amount");
            object price = this.safeNumber(order, "price");
            object remaining = this.safeNumber(order, "remains");
            object filled = subtract(amount, remaining);
            ...
            if (!isTrue(price))
            {
                ...
                cost = quoteAmount;
                average = divide(orderAmount, cost);
            } else
            {
                object ta = this.safeNumber(order, add("ta:", quoteId), 0);
                object tta = this.safeNumber(order, add("tta:", quoteId), 0);
                object fa = this.safeNumber(order, add("fa:", quoteId), 0);
                object tfa = this.safeNumber(order, add("tfa:", quoteId), 0);
                if (isTrue(isEqual(side, "sell")))
                {
                    cost = this.sum(this.sum(ta, tta), this.sum(fa, tfa));
                } else
                {
                    cost = subtract(this.sum(ta, tta), this.sum(fa, tfa));
                }
                type = "limit";
                orderAmount = amount;
                average = divide(cost, filled);
            }
            ...
            ((List<object>)results).Add(new Dictionary<string, object>() {
    ...
    public override object handleErrors(object code, object reason, object url, object method, object headers, object body, object response, object requestHeaders, object requestBody)
    {
        ...
        if (isTrue(((Dictionary<string,object>)response).ContainsKey(toStringOrNull("e"))))
        {
            if (isTrue(((Dictionary<string,object>)response).ContainsKey(toStringOrNull("ok"))))
            {
