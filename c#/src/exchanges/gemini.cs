...
partial class gemini : Exchange
{
    public override object describe()
    {
        return this.deepExtend(base.describe(), new Dictionary<string, object>() {
            ...
            { "options", new Dictionary<string, object>() {
                ...
                { "networkIds", new Dictionary<string, object>() {
                    { "bitcoin", "BTC" },
                    { "ethereum", "ERC20" },
                    { "bitcoincash", "BCH" },
                    { "litecoin", "LTC" },
    ...
    public async virtual Task<object> fetchMarketsFromWeb(object parameters = null)
    {
        ...
        parameters ??= new Dictionary<string, object>();
        object maxRetries = this.safeInteger(this.options, "fetchMarketFromWebRetries", 10);
        ...
        while (isLessThan(retry, maxRetries))
        {
            try
            {
                response = await this.webGetRestApi(parameters);
                break;
            } catch(Exception e)
            {
                retry = add(retry, 1);
        ...
        object sections = ((string)response).Split((string)"<h1 id=\"symbols-and-minimums\">Symbols and minimums</h1>").ToList<object>();
        object numSections = getArrayLength(sections);
        object error = add(add(add(add(add(this.id, " fetchMarketsFromWeb() the "), this.name), " API doc HTML markup has changed, breaking the parser of order limits and precision info for "), this.name), " markets.");
        if (isTrue(!isEqual(numSections, 2)))
        {
            throw new NotSupported ((string)error) ;
        }
        object tables = ((string)getValue(sections, 1)).Split((string)"tbody>").ToList<object>();
        object numTables = getArrayLength(tables);
        if (isTrue(isLessThan(numTables, 2)))
        {
            throw new NotSupported ((string)error) ;
        }
        object rows = ((string)getValue(tables, 1)).Split((string)"\n<tr>\n").ToList<object>(); // eslint-disable-line quotes
        ...
        for (object i = 1; isLessThan(i, numRows); postFixIncrement(ref i))
        {
            ...
            object cells = ((string)row).Split((string)"</td>\n").ToList<object>(); // eslint-disable-line quotes
            ...
            object minAmountParts = ((string)minAmountString).Split((string)" ").ToList<object>();
            ...
            object amountPrecisionParts = ((string)amountPrecisionString).Split((string)" ").ToList<object>();
            ...
            object pricePrecisionParts = ((string)pricePrecisionString).Split((string)" ").ToList<object>();
            object quoteId = this.safeStringLower(pricePrecisionParts, 1, ((string)marketId).Substring((int)startingIndex, (int)idLength));
    ...
    public async virtual Task<object> fetchUSDTMarkets(object parameters = null)
    {
        ...
        parameters ??= new Dictionary<string, object>();
        if (isTrue(((Dictionary<string,object>)this.urls).ContainsKey(toStringOrNull("test"))))
        {
    ...
    public async virtual Task<object> fetchMarketsFromAPI(object parameters = null)
    {
        ...
        for (object i = 0; isLessThan(i, getArrayLength(response)); postFixIncrement(ref i))
        {
            ...
            ((Dictionary<string, object>)result)[(string)marketId] = this.parseMarket(market);
        }
        object options = this.safeValue(this.options, "fetchMarketsFromAPI", new Dictionary<string, object>() {});
        object fetchDetailsForAllSymbols = this.safeValue(options, "fetchDetailsForAllSymbols", false);
        ...
        for (object i = 0; isLessThan(i, getArrayLength(marketIds)); postFixIncrement(ref i))
        {
            ...
            object method = "publicGetV1SymbolsDetailsSymbol";
            object request = new Dictionary<string, object>() {
                { "symbol", marketId },
            };
            ((List<object>)promises).Add(this.callAsync(method, this.extend(request, parameters)));
        }
        ...
        for (object i = 0; isLessThan(i, getArrayLength(promises)); postFixIncrement(ref i))
        {
            ...
            ((Dictionary<string, object>)result)[(string)marketId] = this.parseMarket(responseInner);
        }
        return this.toArray(result);
    }

    public virtual object parseMarket(object response)
    {
        ...
        if (isTrue(isEqual(baseId, null)))
        {
            ...
            object isUSDT = !isEqual(getIndexOf(marketId, "usdt"), -1);
            object quoteSize = ((bool) isTrue(isUSDT)) ? 4 : 3;
            baseId = ((string)marketId).Substring((int)0, (int)subtract(idLength, quoteSize)); // Not true for all markets
            quoteId = ((string)marketId).Substring((int)subtract(idLength, quoteSize), (int)idLength);
        }
        object bs = this.safeCurrencyCode(baseId);
        object quote = this.safeCurrencyCode(quoteId);
    ...
    public async override Task<object> fetchOrderBook(object symbol, object limit = null, object parameters = null)
    {
        ...
        */
        ...
        {
            ((Dictionary<string, object>)request)["limit_bids"] = limit;
            ((Dictionary<string, object>)request)["limit_asks"] = limit;
        }
    ...
    public async override Task<object> fetchTicker(object symbol, object parameters = null)
    {
        ...
        */
        ...
        return await this.callAsync(method, symbol);
    }
    ...
    public override object parseTicker(object ticker, object market = null)
    {
        ...
        if (isTrue(isTrue((!isEqual(marketId, null))) && isTrue((isEqual(market, null)))))
        {
            ...
            {
                baseId = ((string)marketId).Substring((int)0, (int)4);
                quoteId = ((string)marketId).Substring((int)4, (int)7);
            } else
            {
                baseId = ((string)marketId).Substring((int)0, (int)3);
                quoteId = ((string)marketId).Substring((int)3, (int)6);
            }
    ...
    public async override Task<object> fetchTrades(object symbol, object since = null, object limit = null, object parameters = null)
    {
        ...
        */
        ...
        if (isTrue(!isEqual(limit, null)))
        {
            ((Dictionary<string, object>)request)["limit_trades"] = limit;
        }
        if (isTrue(!isEqual(since, null)))
        {
            ((Dictionary<string, object>)request)["timestamp"] = since;
        }
    ...
    public async override Task<object> createOrder(object symbol, object type, object side, object amount, object price = null, object parameters = null)
    {
        ...
        */
        ...
        {
            ((Dictionary<string, object>)request)["stop_price"] = this.priceToPrecision(symbol, rawStopPrice);
            ((Dictionary<string, object>)request)["type"] = "exchange stop limit";
        } else
        {
            ...
            if (isTrue(!isEqual(timeInForce, null)))
            {
                ...
                {
                    ((Dictionary<string, object>)request)["options"] = new List<object>() {"immediate-or-cancel"};
                } else if (isTrue(isTrue((isEqual(timeInForce, "FOK"))) || isTrue((isEqual(timeInForce, "fill-or-kill")))))
                {
                    ((Dictionary<string, object>)request)["options"] = new List<object>() {"fill-or-kill"};
                } else if (isTrue(isEqual(timeInForce, "PO")))
                {
                    ((Dictionary<string, object>)request)["options"] = new List<object>() {"maker-or-cancel"};
                }
            }
            ...
            {
                ((Dictionary<string, object>)request)["options"] = new List<object>() {options};
            }
        }
    ...
    public async override Task<object> fetchMyTrades(object symbol = null, object since = null, object limit = null, object parameters = null)
    {
        ...
        */
        ...
        if (isTrue(!isEqual(since, null)))
        {
            ((Dictionary<string, object>)request)["timestamp"] = this.parseToInt(divide(since, 1000));
        }
    ...
    public async override Task<object> fetchTransactions(object code = null, object since = null, object limit = null, object parameters = null)
    {
        ...
        */
        ...
        if (isTrue(!isEqual(limit, null)))
        {
            ((Dictionary<string, object>)request)["limit_transfers"] = limit;
        }
        if (isTrue(!isEqual(since, null)))
        {
            ((Dictionary<string, object>)request)["timestamp"] = since;
    ...
    public async virtual Task<object> fetchDepositAddressesByNetwork(object code, object parameters = null)
    {
        ...
        {
            throw new ArgumentsRequired ((string)add(this.id, " fetchDepositAddressesByNetwork() requires a network parameter")) ;
        }
        parameters = this.omit(parameters, "network");
        object networks = this.safeValue(this.options, "networks", new Dictionary<string, object>() {});
        object networkId = this.safeString(networks, network, network);
        object networkIds = this.safeValue(this.options, "networkIds", new Dictionary<string, object>() {});
        object networkCode = this.safeString(networkIds, networkId, network);
        object request = new Dictionary<string, object>() {
            { "network", networkId },
        };
        object response = await this.privatePostV1AddressesNetwork(this.extend(request, parameters));
        object results = this.parseDepositAddresses(response, new List<object>() {getValue(currency, "code")}, false, new Dictionary<string, object>() {
            { "network", networkCode },
            { "currency", code },
        });
