...
partial class luno : Exchange
{
    ...
    public virtual object parseBalance(object response)
    {
        ...
        for (object i = 0; isLessThan(i, getArrayLength(wallets)); postFixIncrement(ref i))
        {
            ...
            {
                ((Dictionary<string, object>)getValue(result, code))["used"] = Precise.stringAdd(getValue(getValue(result, code), "used"), reservedUnconfirmed);
                ((Dictionary<string, object>)getValue(result, code))["total"] = Precise.stringAdd(getValue(getValue(result, code), "total"), balanceUnconfirmed);
            } else
            {
                object account = this.account();
                ((Dictionary<string, object>)account)["used"] = reservedUnconfirmed;
                ((Dictionary<string, object>)account)["total"] = balanceUnconfirmed;
    ...
    public async override Task<object> fetchOrderBook(object symbol, object limit = null, object parameters = null)
    {
        ...
        */
        parameters ??= new Dictionary<string, object>();
        await this.loadMarkets();
        object method = "publicGetOrderbook";
        if (isTrue(!isEqual(limit, null)))
        {
            if (isTrue(isLessThanOrEqual(limit, 100)))
            {
                method = add(method, "Top"); // get just the top of the orderbook when limit is low
            }
        }
    ...
    public async virtual Task<object> fetchOrdersByState(object state = null, object symbol = null, object since = null, object limit = null, object parameters = null)
    {
        ...
        {
            ((Dictionary<string, object>)request)["state"] = state;
        }
    ...
    public async override Task<object> fetchTickers(object symbols = null, object parameters = null)
    {
        ...
        */
        ...
        object ids = new List<string>(((Dictionary<string,object>)tickers).Keys);
        object result = new Dictionary<string, object>() {};
    ...
    public async override Task<object> createOrder(object symbol, object type, object side, object amount, object price = null, object parameters = null)
    {
        ...
        */
        ...
        {
            method = add(method, "Marketorder");
            ((Dictionary<string, object>)request)["type"] = ((string)side).ToUpper();
            // todo add createMarketBuyOrderRequires price logic as it is implemented in the other exchanges
            if (isTrue(isEqual(side, "buy")))
            {
                ((Dictionary<string, object>)request)["counter_volume"] = this.amountToPrecision(getValue(market, "symbol"), amount);
            } else
            {
                ((Dictionary<string, object>)request)["base_volume"] = this.amountToPrecision(getValue(market, "symbol"), amount);
            }
        } else
        {
            method = add(method, "Postorder");
            ((Dictionary<string, object>)request)["volume"] = this.amountToPrecision(getValue(market, "symbol"), amount);
            ((Dictionary<string, object>)request)["price"] = this.priceToPrecision(getValue(market, "symbol"), price);
    ...
    {
        object words = ((string)comment).Split((string)" ").ToList<object>();
    ...
    public override object parseLedgerEntry(object entry, object currency = null)
    {
        ...
        object timestamp = this.safeValue(entry, "timestamp");
