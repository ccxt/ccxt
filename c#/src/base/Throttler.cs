...
public class Throttler
{
    ...
    private Queue<(Task, float)> queue = new Queue<(Task, float)>();
    ...
    private async Task loop()
    {
        ...
        while (this.running)
        {
            ...
            else
            {
                ...
                var tokens = (float)this.config["tokens"] + ((float)this.config["refillRate"] * elapsed);
    ...
    public Task throttle(object cost2)
    {
        ...
        var t = new Task(() => { });
        this.queue.Enqueue((t, (float)cost));
        if (!this.running)
        {
            this.running = true;
            this.loop();
