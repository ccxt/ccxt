...
function swiftTypeToGo(swiftType: string): string {
    const map: { [key: string]: string } = {
        ...
        'Bool?': 'bool',
        'Void': '',
        '[OrderRequest]': 'string',   // get parsed to an object later
        'OrderRequest': 'string',
        '[String]': 'string',
        '[String]?': 'string',
    };
...
function transformSwiftHeaderToGo(swiftHeader: string): [string, string[]] {
    ...
    const goParams = paramList
        ? paramList.split(/,(?![^{]*})(?![^\[]*\])/).map(param => {
            ...
            if (customTypes.includes(swiftType)) {
                paramDicts.push(name);
            }
            if (swiftType === '[String]?') {
    ...
    const returnValue = `func (e *CCXTGoExchange) ${capitalize(methodName)}(${goParams}) ([]byte, error) {` 
        + stringArrays.map(name => `\n\t\t${name}_arr := strings.Split(${name}, ",")`)
        + paramDicts.map(name => `\n\t\t${name}_arr := ParseJSON(${name})`); // TODO: rename to other than _arr

    return [returnValue, stringArrays.concat(paramDicts)];
}
...
```
*/
function swiftAddExtraParams(methodCall: string): string {
    return methodCall.replace(
        ...
        (_, method, args) => {
            ...
            const first = parts[0];
            const rest = parts.slice(1).map(name => `${name}: ${name}`);
...
function transformSwiftStringArrayParams(header: string): [string, string[]] {
    ...
    const lines: string[] = [];
    const arrayParamNames: string[] = [];
    const customParamNames: string[] = [];

    let match;
    while ((match = paramRegex.exec(header)) !== null) {
        const paramName = match[1];
        const paramTypeRaw = match[2];
        const paramType = paramTypeRaw.replace("?", "");

        if (paramType === "[String]" || paramType === "[String]?") {
            arrayParamNames.push(paramName);
        }
    ...
    for (const name of arrayParamNames) {
        lines.push(`        let ${name}_string = ${name}?.joined(separator: ",") ?? ""`);
    }

    for (const name of customParamNames) {
        lines.push(`        let ${name}_string = stringify(${name})`);
    }

    return [lines.join('\n'), arrayParamNames.concat(customParamNames)];
}
...
becomes 
```
FetchTradingLimits(symbols: symbols_string, limits: limits_string, params: params)
```
*/
...
const swiftMethodDeclaration = (header: string, methodCall: string, returnType: string, methodName: string) => {
    ...
    ${newHeader}
        do {
            let data = try exchange.${addSuffixToParams(swiftAddExtraParams(methodCall), names, '_string').replace(')', ' as! Data)')}
            ${guard}
