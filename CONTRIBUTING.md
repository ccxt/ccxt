...
- All derived exchange classes are transpiled by `tsc` from TypeScript to JavaScript and then by our custom transpiler to PHP and Python. The source files are language-agnostic, easily mapped line-to-line to any other language and written in a cross-language-compatible way. Any coder can read it (by design).
