...
func (this *Bitfinex) watchOHLCVBody(ch chan any, symbol any, optionalArgs ...any) any {
	...
	var key any = ccxt.Add("trade:"+*interval+":", market["id"])
	var messageHash any = ccxt.Add(channel+":"+*interval+":", market["id"])
	var request map[string]any = map[string]any{
		"event":   "subscribe",
		"channel": channel,
		"key":     key,
...
func (this *Bitfinex) HandleOHLCV(client any, message []any, subscription map[string]any) {
	...
	var keyParts []string = func() []string {
		if key == nil {
			return nil
		}
...
func (this *Bitfinex) ParseWsTrade(trade any, optionalArgs ...any) any {
	...
	if !ccxt.IsEqual(typeVar, nil) {
		if ccxt.GetIndexOf(typeVar, "LIMIT") > -1 {
			typeVar = "limit"
		} else if ccxt.GetIndexOf(typeVar, "MARKET") > -1 {
			typeVar = "market"
		}
	}
	...
	if !(ccxt.InOp(this.Orderbooks, symbol)) {
		...
		if isRaw {
			...
			var amount any = func() any {
				if ccxt.Precise.StringGt(price, "0") {
					return size
				}
				return "0"
			}()
			...
			var size any = func() any {
				if ccxt.Precise.StringLt(amount, "0") {
					return ccxt.Precise.StringNeg(amount)
				}
				return amount
			}()
			var side string = func() string {
...
func (this *Bitfinex) ParseWsOrder(order any, optionalArgs ...any) any {
	...
	var stateParts []string = func() []string {
		if rawState == nil {
			return nil
		}
		return strings.Split(*rawState, " ")
	}()
