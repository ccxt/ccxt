...
func (this *Bitfinex) watchOHLCVBody(ch chan any, symbol string, optionalArgs ...any) any {
	...
	var key *string = ccxt.SafeStringPtr(ccxt.Add("trade:"+*interval+":", market["id"]))
	var messageHash *string = ccxt.SafeStringPtr(ccxt.Add(channel+":"+*interval+":", market["id"]))
	var request map[string]any = map[string]any{
		"event":   "subscribe",
		"channel": channel,
		"key":     key,
...
func (this *Bitfinex) ParseWsTrade(trade any, optionalArgs ...any) any {
	...
	var typeVar *string = this.SafeString(trade, 6)
	if typeVar != nil {
		if func() int {
			if typeVar == nil {
				return -1
			}
			return strings.Index(*typeVar, "LIMIT")
		}() > -1 {
			...
			return strings.Index(*typeVar, "MARKET")
		}() > -1 {
...
func (this *Bitfinex) HandleBalance(client any, message []any, subscription map[string]any) {
	...
	//   ]
	//
	var updateType *string = this.SafeString(message, 1)
	var data any = []any{}
	if updateType != nil && *updateType == "ws" {
...
func (this *Bitfinex) HandleSubscriptionStatus(client any, message map[string]any) any {
	...
	if func() bool { _, ok := message["key"]; return ok }() {
		...
		if unifiedChannel != nil {
			var subId string = "unsubscribe:" + *unifiedChannel + ":" + *symbol
...
func (this *Bitfinex) ParseWsOrder(order any, optionalArgs ...any) any {
	...
	var symbol *string = this.SafeSymbol(marketId)
	var marketResolved map[string]any = this.SafeMarket(symbol)
	...
	var typeVar *string = this.SafeString(order, 8, "")
	if func() int {
		if typeVar == nil {
			return -1
		}
