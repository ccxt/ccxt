...
func (this *Binance) Stream(typeVar any, subscriptionHash any, optionalArgs ...any) any {
	...
	if stream == nil {
		var streamIndex any = ccxt.DerefScalar(this.SafeInteger(this.Options, "streamIndex", int64(-1)))
...
func (this *Binance) HandleLiquidation(client any, message map[string]any) {
	...
	//    }
	//
	var rawLiquidation map[string]any = this.SafeDictMap(message, "o", map[string]any{})
...
func (this *Binance) watchTickersBody(ch chan any, optionalArgs ...any) any {
	...
	if channelName != nil && *channelName == "bookTicker" {
...
func (this *Binance) watchMultiTickerHelperBody(ch chan any, methodName string, channelName any, optionalArgs ...any) any {
	...
	var symbolsDefined bool = ((symbolsNormalized != nil))
	if symbolsNormalized != nil {
		firstMarket = this.Market(func() any {
			if 0 >= 0 && 0 < len(symbolsNormalized) {
				return symbolsNormalized[0]
			}
			return nil
	...
	var isOptionMarkPrice bool = (isMarkPrice && (marketType != nil && *marketType == "option"))
	...
	var isOptionTicker bool = ((marketType != nil && *marketType == "option") && !isMarkPrice && !isBidAsk)
	...
	if symbolsNormalized != nil {
		...
		for i := 0; i < len(symbolsNormalized); i++ {
			var symbol string = symbolsNormalized[i]
			var market map[string]any = this.Market(symbol)
			messageHashes = append(messageHashes, ccxt.Add(ccxt.Add(ccxt.Add(unifiedPrefix+":", channelName), "@"), symbol))
			if isUnsubscribe == true {
				unsubscribeMessageHashes = append(unsubscribeMessageHashes, ccxt.Add(ccxt.Add(ccxt.Add("unsubscribe::"+unifiedPrefix+":", channelName), "@"), symbol))
			}
			if isOptionMarkPrice {
		...
		if marketType != nil && *marketType == "option" {
			...
			messageHashes = append(messageHashes, ccxt.Add(unifiedPrefix+"s:", channelName))
...
func (this *Binance) HandleTickersAndBidsAsks(client any, message any, methodType any) {
	...
	for i := 0; i < len(rawTickers); i++ {
		...
		var messageHash any = ccxt.Add(ccxt.Add(ccxt.Add(unifiedPrefix+":", channelName), "@"), symbol)
	...
	if length > 0 {
		var batchMessageHash any = ccxt.Add(unifiedPrefix+"s:", channelName)
...
func (this *Binance) keepAliveListenKeyBody(ch chan any, optionalArgs ...any) any {
	...
	for i := 0; i < len(clients); i++ {
		...
		if clientSubscriptions != nil {
			subscriptionKeys = make([]string, 0, len(clientSubscriptions))
			for objectKey := range clientSubscriptions {
				subscriptionKeys = append(subscriptionKeys, objectKey)
			}
		}
...
func (this *Binance) fetchPositionsWsBody(ch chan any, optionalArgs ...any) any {
	...
	if (symbolsNormalized == nil) && (ccxt.IsEqual(typeVar, "spot")) {
...
func (this *Binance) HandleStockPrice(client any, message map[string]any) {
	...
	for i := 0; i < len(rates); i++ {
		var rate map[string]any = this.SafeDictMap(rates, i, map[string]any{})
...
func (this *Binance) watchMyTradesBody(ch chan any, optionalArgs ...any) any {
	...
	if (!ccxt.IsEqual(symbolResolved, nil)) && ((market != nil)) {
		messageHash = ccxt.Add(messageHash, ccxt.Add(":", symbolResolved))
...
func (this *Binance) HandleWsError(client any, message any) {
	...
	var error map[string]any = this.SafeDictMap(message, "error", map[string]any{})
