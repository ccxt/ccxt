...
func (this *Binance) watchOHLCVForSymbolsBody(ch chan any, symbolsAndTimeframes any, optionalArgs ...any) any {
	...
	if ccxt.EvalTruthy(stock) {
		...
		for i := 0; i < ccxt.GetArrayLength(symbolsAndTimeframes); i++ {
			...
			var stockTickerString any = func() any {
				if stockTicker == nil {
					return ""
				}
			...
			stockStreams = append(stockStreams, ccxt.Add(ccxt.Add(stockTickerString, "@kline_"), stockInterval))
	...
	if !ccxt.IsEqual(symbols, nil) {
		...
		if ccxt.IsEqual(marketType, "option") {
			...
			if isOptionTicker {
				...
				subscriptionArgs = append(subscriptionArgs, ccxt.Add(ccxt.Add(underlying, "@optionTicker@"), expirationDate))
			} else {
...
func (this *Binance) HandleTickersAndBidsAsks(client any, message any, methodType any) {
	...
	for i := 0; i < ccxt.GetArrayLength(rawTickers); i++ {
		...
		var numTickerMarkets any = func() any {
		...
		var tickerMarketById any = func() any {
			if ccxt.IsEqual(numTickerMarkets, 1) {
				return this.SafeValue(tickerMarketsByIdList, 0)
			}
			return nil
		}()
		var isSpot any = this.IsSpotUrl(client)
		var tickerFallbackType any = func() any {
			if ccxt.EvalTruthy(isSpot) {
				return "spot"
			}
			return "contract"
...
func (this *Binance) authenticateBody(ch chan any, optionalArgs ...any) any {
	...
	var lastAuthenticatedTime *int64 = this.SafeInteger(options, "lastAuthenticatedTime", 0)
	var refreshRateKey any = func() any {
		if isStock {
