...
func (this *Weex) HandleTicker(client any, message any) {
	...
	var data map[string]any = this.SafeDictMap(tickers, 0, map[string]any{})
...
func (this *Weex) HandleTrade(client any, message any) {
	...
	for i := 0; i < len(data); i++ {
		var rawTrade map[string]any = this.SafeDictMap(data, i, map[string]any{})
...
func (this *Weex) HandleOHLCV(client any, message any) {
	...
	for i := 0; i < len(data); i++ {
		var entry map[string]any = this.SafeDictMap(data, i, map[string]any{})
...
func (this *Weex) watchOrderBookForSymbolsBody(ch chan any, symbols any, optionalArgs ...any) any {
	...
	for i := 0; i < len(symbolsNormalized); i++ {
		...
		var channel any = ccxt.Add(ccxt.Add(market["id"], "@depth"), depthOption)
		messageHashes = append(messageHashes, messageHash)
		channels = append(channels, channel)
	}
	var subscription map[string]any = map[string]any{
...
func (this *Weex) HandleMyTrades(client any, message any) {
	...
	var messageHash string = "myTrades"
	var symbolKeys []string = nil
	if symbols != nil {
		symbolKeys = make([]string, 0, len(symbols))
		for objectKey := range symbols {
			symbolKeys = append(symbolKeys, objectKey)
		}
	}
	...
	for j := 0; j < len(symbolKeys); j++ {
		var symbol string = symbolKeys[j]
...
func (this *Weex) HandleOrders(client any, message any) {
	...
	for i := 0; i < len(data); i++ {
		var rawOrder map[string]any = this.SafeDictMap(data, i, map[string]any{})
	...
	for i := 0; i < len(symbolKeys); i++ {
		var symbol string = symbolKeys[i]
		var symbolMessageHash string = messageHash + "::" + symbol
...
func (this *Weex) HandlePositions(client any, message any) {
	...
	var data []any = ccxt.SafeListTyped(message, "d")
	for i := 0; i < len(data); i++ {
		var rawPosition map[string]any = this.SafeDictMap(data, i, map[string]any{})
