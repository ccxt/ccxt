...
func (this *Krakenfutures) HandlePositions(client any, message map[string]any) {
	...
	var rawPositions []any = ccxt.SafeListTyped(message, "positions")
...
func (this *Krakenfutures) watchOrdersBody(ch chan any, optionalArgs ...any) any {
	...
	var verboseparamsVariable []any = this.HandleOptionBoolAndParams(params, "watchOrders", "verbose", false)
	verbose = ccxt.GetValueBool(verboseparamsVariable, 0, false)
	params = ccxt.MapTyped(ccxt.GetValue(verboseparamsVariable, 1))
	...
	var feed *string = this.SafeString(params, "feed")
	if feed != nil {
		name = feed
		messageHash = "orders"
		if feed != nil && *feed == "open_orders_verbose" {
			messageHash = "orders:verbose"
		}
	}
	if symbol != nil {
...
func (this *Krakenfutures) HandleOrder(client any, message map[string]any) any {
	...
	var order map[string]any = ccxt.SafeMapTyped(message, "order")
	if !ccxt.IsEqual(order, nil) {
		...
		if (ccxt.IsEqual(previousOrder, nil)) || (reason != nil && *reason == "edited_by_user") {
			...
			var filledString *string = this.NumberToString(trade["amount"])
...
func (this *Krakenfutures) HandleOrderSnapshot(client any, message map[string]any) {
	...
	var feed *string = this.SafeString(message, "feed")
	var messageHash string = "orders"
	if feed != nil && *feed == "open_orders_verbose_snapshot" {
		messageHash = "orders:verbose"
	}
...
func (this *Krakenfutures) HandleOrderBookSnapshot(client any, message map[string]any) {
	...
	var bids []any = ccxt.SafeListTyped(message, "bids")
...
func (this *Krakenfutures) HandleOrderBook(client any, message map[string]any) {
	...
	var qty *float64 = this.SafeNumber(message, "qty")
	var timestamp *int64 = this.SafeInteger(message, "timestamp")
...
func (this *Krakenfutures) HandleBalance(client any, message map[string]any) {
	...
	//    }
	//
	var holding map[string]any = ccxt.SafeMapTyped(message, "holding")
	var futures map[string]any = ccxt.SafeMapTyped(message, "futures")
	var flexFutures map[string]any = ccxt.SafeMapTyped(message, "flex_futures")
...
func (this *Krakenfutures) HandleErrorMessage(client any, message any) any {
	...
	if (errMsg != nil) && (func() int {
		if errMsg == nil {
			return -1
		}
		return strings.Index(*errMsg, "Already subscribed")
	}() >= 0) {
		return false
	}
...
func (this *Krakenfutures) HandleAuthenticate(client any, message any) any {
	...
	if event == nil || *event != "error" {
		...
		this.Options.Store("challenge", challenge)
		this.Options.Store("signedChallenge", signature)
