...
func (this *Phemex) fetchMarketsBody(ch chan any, optionalArgs ...any) any {
	...
	for i := 0; i < GetArrayLength(products); i++ {
		...
		if (typeVar != nil && *typeVar == "perpetual") || (typeVar != nil && *typeVar == "perpetualv2") || (typeVar != nil && *typeVar == "perpetualpilot") {
			var id *string = this.SafeString(market, "symbol")
			var riskLimitValues map[string]any = this.SafeDictMap(riskLimitsById, id, map[string]any{})
			market = this.Extend(market, riskLimitValues)
			var v1ProductsValues map[string]any = this.SafeDictMap(v1ProductsById, id, map[string]any{})
...
func (this *Phemex) fetchOHLCVBody(ch chan any, symbol string, optionalArgs ...any) any {
	...
	if (market["linear"] == true) || isStableSettled {
		if (until != nil) || (since != nil) {
			...
			if since != nil {
				sinceSeconds = math.Round(float64(*since) / 1000)
			} else {
			...
			if until != nil {
				request["to"] = math.Round(float64(*until) / 1000)
			} else {
...
func (this *Phemex) ParseConversion(conversion any, optionalArgs ...any) any {
	...
	_ = toCurrency
	var quoteArgs map[string]any = this.SafeDictMap(conversion, "quoteArgs", map[string]any{})
