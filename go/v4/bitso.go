...
func (this *Bitso) fetchTransactionFeesBody(ch chan any, optionalArgs ...any) any {
	...
	if withdrawalFees != nil {
		currencyIds = make([]string, 0, len(withdrawalFees))
		for objectKey := range withdrawalFees {
...
func (this *Bitso) ParseDepositWithdrawFees(response any, optionalArgs ...any) any {
	...
	var withdrawalKeys []string = nil
	if withdrawalResponse != nil {
		withdrawalKeys = make([]string, 0, len(withdrawalResponse))
		for objectKey := range withdrawalResponse {
			withdrawalKeys = append(withdrawalKeys, objectKey)
		}
	}
...
func (this *Bitso) withdrawBody(ch chan any, code string, amount any, address any, optionalArgs ...any) any {
	...
	var method any = func() any {
		if func() bool { _, ok := methods[code]; return ok }() {
			return methods[code]
		}
		return nil
	}()
	if IsEqual(method, nil) {
		panic(ExchangeError(this.Id + " not valid withdraw coin: " + code))
	}
	var request map[string]any = map[string]any{
		"amount":          amount,
		"address":         address,
