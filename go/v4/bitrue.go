...
func (this *Bitrue) fetchClosedOrdersBody(ch chan any, optionalArgs ...any) any {
	...
	listEp2575 := (<-this.SpotV1PrivateGetAllOrders(this.Extend(request, params)))
	PanicOnError(listEp2575.Raw)
	var response []any = listEp2575.Value
...
func (this *Bitrue) HandleErrors(code any, reason any, url any, method any, headers any, body string, response any, requestHeaders any, requestBody any) any {
	...
	if IsGreaterThanOrEqual(code, 400) {
		...
		if strings.Index(body, "PRICE_FILTER") >= 0 {
			panic(InvalidOrder(this.Id + " order price is invalid, i.e. exceeds allowed price precision, exceeds min price or max price limits or is invalid float value in general, use this.priceToPrecision (symbol, amount) " + body))
		}
	}
	if response == nil {
	...
	if error != nil {
		...
		if (error != nil && *error == "-2015") && (*this.SafeBool(this.Options, "hasAlreadyAuthenticatedSuccessfully", false)) {
			panic(DDoSProtection(this.Id + " temporary banned: " + body))
		}
