...
func (this *KucoinCore) fetchStatusBody(ch chan any, optionalArgs ...any) any {
	...
	if EvalTruthy(uta) {
		...
		var defaultTradeType any = Ternary((defaultType != nil && *defaultType == "spot"), "SPOT", "FUTURES")
	...
	ch <- map[string]any{
		"status":  Ternary((status != nil && *status == "open"), "ok", "maintenance"),
		"updated": nil,
		"eta":     nil,
		"url":     nil,
		"info":    response,
...
func (this *KucoinCore) fetchMarketsBody(ch chan any, optionalArgs ...any) any {
	...
	var requestMarginables bool = EvalTruthy(credentialsSet) && EvalTruthy(this.SafeBool(params, "marginables", true))
	...
	var symbolsData any = Ternary(fetchSpotMarkets, this.SafeList(GetValue(responses, 0), "data", []any{}), []any{})
	...
	var crossData any = Ternary((requestMarginables == true), this.SafeDict(GetValue(responses, crossIndex), "data", map[string]any{}), map[string]any{})
	var crossItems any = this.SafeList(crossData, "items", []any{})
	var crossById map[string]any = this.IndexBy(crossItems, "symbol")
	var isolatedData any = Ternary((requestMarginables == true), GetValue(responses, isolatedIndex), map[string]any{})
	...
	var tickersResponse any = Ternary(EvalTruthy(fetchTickersFees), this.SafeDict(responses, tickersIndex, map[string]any{}), map[string]any{})
...
func (this *KucoinCore) fetchContractMarketsBody(ch chan any, optionalArgs ...any) any {
	...
	for i := 0; IsLessThan(i, GetArrayLength(data)); i++ {
		...
		var tickSize any = DerefScalar(this.SafeNumber(market, "tickSize"))
		var lotSize any = DerefScalar(this.SafeNumber(market, "lotSize"))
		var limitAmountMin any = lotSize
		if IsEqual(limitAmountMin, nil) {
			limitAmountMin = DerefScalar(this.SafeNumber(market, "baseMinSize"))
		}
		var limitAmountMax any = DerefScalar(this.SafeNumber(market, "maxOrderQty"))
		if IsEqual(limitAmountMax, nil) {
			limitAmountMax = DerefScalar(this.SafeNumber(market, "baseMaxSize"))
		}
...
func (this *KucoinCore) fetchUTAMarketsBody(ch chan any, optionalArgs ...any) any {
	...
	for i := 0; IsLessThan(i, GetArrayLength(symbolsData)); i++ {
		...
		var isMarginable any = Ternary((hasMargin != nil && *hasMargin == "1"), true, false)
...
func (this *KucoinCore) loadMigrationStatusBody(ch chan any, optionalArgs ...any) any {
	...
	_ = force
	if !(InOp(this.Options, "hf")) || (IsEqual(GetValue(this.Options, "hf"), nil)) || EvalTruthy(force) {
...
func (this *KucoinCore) HandleHfAndParams(optionalArgs ...any) any {
	params := GetArg(optionalArgs, 0, map[string]any{})
	_ = params
	var migrated any = DerefScalar(this.SafeBool(this.Options, "hf", false))
	var loadedHf any = nil
	if !IsEqual(migrated, nil) {
		if EvalTruthy(migrated) {
			loadedHf = true
		} else {
			loadedHf = false
		}
	}
	var hf any = DerefScalar(this.SafeBool(params, "hf", loadedHf))
...
func (this *KucoinCore) ParseDepositWithdrawFee(fee any, optionalArgs ...any) any {
	...
	var minWithdrawFee any = DerefScalar(this.SafeNumber(fee, "withdrawMinFee"))
...
func (this *KucoinCore) ParseOHLCV(ohlcv any, optionalArgs ...any) any {
	...
	if (timestampString != nil) && IsLessThanOrEqual(GetLength(timestampString), 10) {
...
func (this *KucoinCore) fetchOrderBookBody(ch chan any, symbol any, optionalArgs ...any) any {
	...
	if EvalTruthy(uta) {
	...
	} else if !EvalTruthy(isAuthenticated) || !IsEqual(limit, nil) {
		if level != nil && *level == 2 {
			...
			AddElementToObject(request, "limit", Ternary((!IsEqual(limit, nil)), limit, 100))
		}
...
func (this *KucoinCore) createSpotOrderBody(ch chan any, symbol any, typeVar any, side any, amount any, optionalArgs ...any) any {
	...
	var market any = this.Market(symbol)
	var testOrder any = DerefScalar(this.SafeBool(params, "test", false))
...
func (this *KucoinCore) CreateSpotOrderRequest(symbol any, typeVar any, side any, amount any, optionalArgs ...any) any {
	...
	var quoteAmount any = DerefScalar(this.SafeNumber2(params, "cost", "funds"))
	...
	takeProfitPrice := GetValue(triggerPricestopLossPricetakeProfitPriceVariable, 2)
	var isTriggerOrder bool = (!IsEqual(triggerPrice, nil)) || (!IsEqual(stopLossPrice, nil)) || (!IsEqual(takeProfitPrice, nil))
	var isMarginOrder bool = (tradeType != nil && *tradeType == "MARGIN_TRADE") || !IsEqual(marginMode, nil)
	...
	if isTriggerOrder {
		if !IsEqual(triggerPrice, nil) {
			...
			if !IsEqual(stopLossPrice, nil) {
				AddElementToObject(request, "stop", Ternary((IsEqual(side, "buy")), "entry", "loss"))
				AddElementToObject(request, "stopPrice", this.PriceToPrecision(symbol, stopLossPrice))
			} else {
				AddElementToObject(request, "stop", Ternary((IsEqual(side, "buy")), "loss", "entry"))
...
func (this *KucoinCore) CreateContractOrderRequest(symbol any, typeVar any, side any, amount any, optionalArgs ...any) any {
	...
	if !IsEqual(triggerPrice, nil) {
		AddElementToObject(request, "stop", Ternary((IsEqual(side, "buy")), "up", "down"))
		AddElementToObject(request, "stopPrice", this.PriceToPrecision(symbol, triggerPrice))
		...
		if hasStopLoss {
			...
			priceType = DerefScalar(this.SafeString(stopLoss, "triggerPriceType", "mark"))
			priceType = DerefScalar(this.SafeString(triggerPriceTypes, priceType, priceType))
		}
		if hasTakeProfit {
			...
			priceType = DerefScalar(this.SafeString(takeProfit, "triggerPriceType", "mark"))
			priceType = DerefScalar(this.SafeString(triggerPriceTypes, priceType, priceType))
		}
		...
		if !IsEqual(stopLossPrice, nil) {
			...
			AddElementToObject(request, "stop", Ternary((IsEqual(side, "buy")), "down", "up"))
			AddElementToObject(request, "stopPrice", this.PriceToPrecision(symbol, takeProfitPrice))
		}
	...
	if (IsEqual(postOnly, true)) && (!IsEqual(hidden, nil)) {
	...
	if (!IsEqual(iceberg, nil)) && (!IsEqual(iceberg, false)) {
...
func (this *KucoinCore) CreateUtaOrderRequest(symbol any, typeVar any, side any, amount any, optionalArgs ...any) any {
	...
	if IsEqual(isContract, true) {
		if !isUnified {
			...
			params = GetValue(hedgedparamsVariable, 1)
			if IsEqual(hedged, true) {
				var positionSide any = Ternary((IsEqual(side, "buy")), "LONG", "SHORT")
				if reduceOnly == true {
					positionSide = Ternary((IsEqual(positionSide, "LONG")), "SHORT", "LONG")
				}
	...
	if !IsEqual(triggerPrice, nil) {
		...
		AddElementToObject(request, "triggerDirection", Ternary((triggerDirection != nil && *triggerDirection == "ascending"), "UP", "DOWN"))
		...
		if !IsEqual(stopLossPrice, nil) {
			AddElementToObject(request, "triggerDirection", Ternary((IsEqual(side, "buy")), "UP", "DOWN"))
			...
			AddElementToObject(request, "triggerDirection", Ternary((IsEqual(side, "buy")), "DOWN", "UP"))
...
func (this *KucoinCore) cancelSpotOrderBody(ch chan any, id any, optionalArgs ...any) any {
	...
	var tradeType *string = this.SafeString(params, "tradeType") // keep it for backward compatibility
	var isMarginOrder bool = (tradeType != nil && *tradeType == "MARGIN_TRADE") || !IsEqual(marginMode, nil)
	if (IsEqual(hf, true)) || EvalTruthy(useSync) || isMarginOrder {
		if trigger != true {
			if IsEqual(symbol, nil) {
...
func (this *KucoinCore) cancelAllSpotOrdersBody(ch chan any, optionalArgs ...any) any {
	...
	var request map[string]any = map[string]any{}
	var trigger any = DerefScalar(this.SafeBool2(params, "trigger", "stop", false))
	var hf any = nil
	hfparamsVariable := this.HandleHfAndParams(params)
	hf = GetValue(hfparamsVariable, 0)
	params = GetValue(hfparamsVariable, 1)
...
func (this *KucoinCore) cancelAllContractOrdersBody(ch chan any, optionalArgs ...any) any {
	...
	params = this.Omit(params, []any{"stop", "trigger"})
	var response any = nil
	if (!IsEqual(trigger, nil)) && (!IsEqual(trigger, false)) {
...
func (this *KucoinCore) cancelAllUtaOrdersBody(ch chan any, optionalArgs ...any) any {
	...
	var market any = this.Market(symbol)
	var isContract any = GetValue(market, "contract")
	var tradeType any = Ternary((IsEqual(isContract, true)), "FUTURES", "SPOT")
	...
	var orderFilter any = Ternary((IsEqual(trigger, true)), "ADVANCED", "NORMAL")
	var request map[string]any = map[string]any{
		"accountMode": "unified",
		"symbol":      GetValue(market, "id"),
		"tradeType":   tradeType,
...
func (this *KucoinCore) fetchSpotOrdersByStatusBody(ch chan any, status any, optionalArgs ...any) any {
	...
	hf = GetValue(hfparamsVariable, 0)
	params = GetValue(hfparamsVariable, 1)
	if (IsEqual(hf, true)) && (IsEqual(symbol, nil)) {
	...
	var response any = nil
	if isMarginOrder && IsEqual(lowercaseStatus, "active") && (trigger != true) {
...
func (this *KucoinCore) fetchUtaOrdersByStatusBody(ch chan any, status any, optionalArgs ...any) any {
	...
	if !isContract && (IsEqual(symbol, nil)) {
...
func (this *KucoinCore) ParseOrder(order any, optionalArgs ...any) any {
	...
	market = this.SafeMarket(marketId, market)
	if (!IsEqual(market, nil)) && (IsEqual(GetValue(market, "contract"), true)) {
...
func (this *KucoinCore) ParseContractOrder(order any, optionalArgs ...any) any {
	...
	var cancelExist any = DerefScalar(this.SafeBool(order, "cancelExist", false))
	var status any = nil
	if !IsEqual(isActive, nil) {
		status = Ternary((IsEqual(isActive, true)), "open", "closed")
	}
	status = Ternary((cancelExist == true), "canceled", status)
	var fee any = nil
...
func (this *KucoinCore) ParseSpotOrder(order any, optionalArgs ...any) any {
	...
	var stopTriggered any = DerefScalar(this.SafeBool(order, "stopTriggered", false))
	var isActive any = DerefScalar(this.SafeBool2(order, "isActive", "active"))
	...
	if trigger {
		if responseStatus != nil && *responseStatus == "NEW" {
			status = "open"
		} else if (!IsEqual(isActive, true)) && (stopTriggered != true) {
			status = "cancelled"
		}
	}
	if cancelExist == true {
...
func (this *KucoinCore) fetchMySpotTradesBody(ch chan any, optionalArgs ...any) any {
	...
	marginMode = GetValue(marginModeparamsVariable, 0)
	params = GetValue(marginModeparamsVariable, 1)
	var isMargin bool = !IsEqual(marginMode, nil)
	if isMargin {
		hf = true
		AddElementToObject(request, "tradeType", Ternary((IsEqual(marginMode, nil)), nil, this.SafeString(GetValue(this.Options, "marginModes"), marginMode, marginMode)))
	}
	if (IsEqual(hf, true)) && IsEqual(symbol, nil) {
...
func (this *KucoinCore) ParseSpotOrUtaTrade(trade any, optionalArgs ...any) any {
	...
	if feeCostString != nil {
		...
		if IsEqual(feeCurrency, nil) {
			feeCurrency = Ternary((side != nil && *side == "sell"), GetValue(market, "quote"), GetValue(market, "base"))
		}
		fee = map[string]any{
			"cost":     feeCostString,
			"currency": feeCurrency,
...
func (this *KucoinCore) ParseTransaction(transaction any, optionalArgs ...any) any {
	...
	if !IsEqual(txid, nil) {
		...
		if IsGreaterThan(numTxidParts, 1) {
			if IsEqual(address, nil) {
				if IsGreaterThan(GetLength(GetValue(txidParts, 1)), 1) {
	...
	var typeVar any = Ternary((IsEqual(txid, nil)), "withdrawal", "deposit")
	...
	if isV1 {
		typeVar = Ternary((InOp(transaction, "address")), "withdrawal", "deposit")
		if !IsEqual(timestamp, nil) {
			timestamp = Multiply(timestamp, 1000)
		}
	...
	var internal any = DerefScalar(this.SafeBool(transaction, "isInner"))
...
func (this *KucoinCore) fetchBalanceBody(ch chan any, optionalArgs ...any) any {
	...
	hf = GetValue(hfparamsVariable, 0)
	params = GetValue(hfparamsVariable, 1)
	if (IsEqual(hf, true)) && (!IsEqual(typeVar, "main")) {
		typeVar = "trade_hf"
	}
	var marginMode any = nil
...
func (this *KucoinCore) transferUtaBody(ch chan any, code any, amount any, fromAccount any, toAccount any, optionalArgs ...any) any {
	...
	var exchangeIds any = Ternary((IsEqual(this.Ids, nil)), []any{}, this.Ids)
...
func (this *KucoinCore) ParseTransfer(transfer any, optionalArgs ...any) any {
	...
	var accountFrom any = Ternary((IsEqual(accountFromRaw, nil)), nil, this.SafeString(accountsByType, accountFromRaw, accountFromRaw))
	var accountTo any = Ternary((IsEqual(accountToRaw, nil)), nil, this.SafeString(accountsByType, accountToRaw, accountToRaw))
	return map[string]any{
...
func (this *KucoinCore) ParseLedgerEntry(item any, optionalArgs ...any) any {
	...
	var feeCost any = Ternary((feeCostString == nil), nil, this.OmitZero(feeCostString))
...
func (this *KucoinCore) fetchLedgerBody(ch chan any, optionalArgs ...any) any {
	...
	if EvalTruthy(uta) && (IsEqual(requestedType, "margin")) {
		marginMode = Ternary((IsEqual(marginMode, nil)), "cross", marginMode) // default to cross margin for UTA if margin is requested but marginMode is not specified
		requestedType = marginMode
	}
	var accountsByType any = this.SafeDict(this.Options, "accountsByType")
	if EvalTruthy(uta) {
...
func (this *KucoinCore) fetchBorrowInterestBody(ch chan any, optionalArgs ...any) any {
	...
	//
	var data any = this.SafeDict(response, "data", map[string]any{})
	var assets any = Ternary((IsEqual(marginMode, "isolated")), this.SafeList(data, "assets", []any{}), this.SafeList(data, "accounts", []any{}))
...
func (this *KucoinCore) ParseBorrowInterest(info any, optionalArgs ...any) any {
	...
	if IsEqual(marginMode, "isolated") {
		amountBorrowed = DerefScalar(this.SafeNumber(isolatedBase, "liabilityPrincipal"))
		interest = DerefScalar(this.SafeNumber(isolatedBase, "liabilityInterest"))
		currencyId = DerefScalar(this.SafeString(isolatedBase, "currency"))
	} else {
		amountBorrowed = DerefScalar(this.SafeNumber(info, "liabilityPrincipal"))
		interest = DerefScalar(this.SafeNumber(info, "liabilityInterest"))
...
func (this *KucoinCore) setLeverageBody(ch chan any, leverage any, optionalArgs ...any) any {
	...
	marketType = GetValue(marketTypeparamsVariable, 0)
	params = GetValue(marketTypeparamsVariable, 1)
	if (!IsEqual(symbol, nil)) || ((!IsEqual(marketType, "spot")) && (!IsEqual(marketType, "margin"))) {
		if IsEqual(symbol, nil) {
...
func (this *KucoinCore) setContractLeverageBody(ch chan any, leverage any, optionalArgs ...any) any {
	...
	var data any = this.SafeDict(response, "data", map[string]any{})
	var leverageNum any = DerefScalar(this.SafeNumber(data, "leverage"))
...
func (this *KucoinCore) ParsePosition(position any, optionalArgs ...any) any {
	...
	if IsEqual(side, nil) {
		if size != nil {
			...
			if IsGreaterThan(GetIndexOf(typeVar, "long"), OpNeg(1)) {
				side = "long"
			} else {
				side = "short"
			}
	...
	if !IsEqual(crossMode, nil) {
		marginMode = Ternary((IsEqual(crossMode, true)), "cross", "isolated")
	}
...
func (this *KucoinCore) cancelOrdersBody(ch chan any, ids any, optionalArgs ...any) any {
	...
	if EvalTruthy(uta) {
		...
		var requestKey any = Ternary(useClientorderId, "clientOidsList", "orderIdsList")
...
func (this *KucoinCore) reduceMarginBody(ch chan any, symbol any, amount any, optionalArgs ...any) any {
	...
	ch <- map[string]any{
		...
		"status":     Ternary((responseCode != nil && *responseCode == "200000"), "ok", nil),
		"timestamp":  nil,
		"datetime":   nil,
	}
	return nil
...
func (this *KucoinCore) ParseMarginModification(info any, optionalArgs ...any) any {
	...
	var mode any = Ternary((IsEqual(crossMode, true)), "cross", "isolated")
...
func (this *KucoinCore) ParseMarginMode(marginMode any, optionalArgs ...any) any {
	...
	marginType = Ternary((IsEqual(marginType, "ISOLATED")), "isolated", "cross")
	return map[string]any{
		"info":       marginMode,
		"symbol":     this.SafeString(market, "symbol"),
...
func (this *KucoinCore) Sign(path any, optionalArgs ...any) any {
	...
	if isPrivate || isFuturePrivate || isBroker || isEarn || isUtaPrivate {
		...
		partner = Ternary(isFuturePartner, this.SafeValue(partner, "future", partner), this.SafeValue(partner, "spot", partner))
