...
func  (this *PredictionExchange) FilterEventsByStatus(events any, optionalArgs ...any) any  {
    ...
    _ = status
    if (IsEqual(status, nil)) || (IsEqual(status, "all")) {
        return events
    }
    ...
    for i := 0; IsLessThan(i, GetArrayLength(events)); i++ {
        ...
        if (IsEqual(isActive, nil)) || (IsEqual(isActive, wantActive)) {
...
func  (this *PredictionExchange) FilterEventsBySearchIn(events any, queries any, optionalArgs ...any) any  {
    ...
    if (IsEqual(searchIn, nil)) || (IsEqual(queries, nil)) || (IsEqual(queriesLength, 0)) {
        return events
    }
    var checkTitle bool = (IsEqual(searchIn, "title")) || (IsEqual(searchIn, "both"))
    var checkDescription bool = (IsEqual(searchIn, "description")) || (IsEqual(searchIn, "both"))
    var result any = []any{}
    for i := 0; IsLessThan(i, GetArrayLength(events)); i++ {
        var event any = GetValue(events, i)
...
func (this *PredictionExchange) loadEventsHelperBody(ch chan any, optionalArgs ...any) any {
    ...
    defer ReturnPanicError(ch)
        ...
        _ = params
        if !EvalTruthy(reload) && EvalTruthy(this.Events) {
...
func  (this *PredictionExchange) SlugToMarketSymbol(eventSlug any, marketSlug any) any  {
    ...
    if (IsEqual(eventPart, nil)) || (IsEqual(eventPart, "")) || (IsEqual(eventPart, marketPart)) {
        return marketPart
    }
...
func (this *PredictionExchange) loadOutcomesBody(ch chan any, optionalArgs ...any) any {
    ...
    defer ReturnPanicError(ch)
        ...
        if !IsEqual(outcomes, nil) {
            ...
            if (IsGreaterThan(missingLength, 0)) && EvalTruthy(loadAll) && !wasWarm && !EvalTruthy(reload) {
