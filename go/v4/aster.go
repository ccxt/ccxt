...
func (this *AsterCore) ParseMarket(market any) any {
	...
	if isContract {
		...
		contractSize = DerefScalar(this.SafeNumber2(market, "contractSize", "unit", this.ParseNumber("1")))
	} else {
		spot = true
		swap = false
...
func (this *AsterCore) ParseTrade(trade any, optionalArgs ...any) any {
	...
	var isMaker any = DerefScalar(this.SafeBool(trade, "maker"))
	var takerOrMaker any = nil
	if !IsEqual(isMaker, nil) {
		takerOrMaker = Ternary(EvalTruthy(isMaker), "maker", "taker")
		if IsEqual(side, nil) {
			var isBuyer any = DerefScalar(this.SafeBool(trade, "buyer"))
			if !IsEqual(isBuyer, nil) {
				side = Ternary(EvalTruthy(isBuyer), "buy", "sell")
			}
		}
	}
	var isBuyerMaker any = DerefScalar(this.SafeBool2(trade, "isBuyerMaker", "m"))
...
func (this *AsterCore) initializeClientBody(ch chan any, optionalArgs ...any) any {
	...
	params := GetArg(optionalArgs, 0, map[string]any{})
	_ = params
	var builderFee any = DerefScalar(this.SafeBool(params, "builderFee", this.SafeBool(this.Options, "builderFee", true))) // we shouldn't omit here
	if builderFee != true {
