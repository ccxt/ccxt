...
func (this *BybitCore) UpdateRateLimiterState(statusCode interface{}, statusText interface{}, url interface{}, method interface{}, responseHeaders interface{}) {
	var remaining interface{} = this.SafeInteger2(responseHeaders, "X-Bapi-Limit-Status", "x-bapi-limit-status")
	var limit interface{} = this.SafeInteger2(responseHeaders, "X-Bapi-Limit", "x-bapi-limit")
	if IsTrue(!IsEqual(remaining, nil)) && IsTrue(!IsEqual(limit, nil)) && ToFloat64(limit) > 0 {
		if this.Throttler != nil {
			used := ToFloat64(limit) - ToFloat64(remaining)
			maxWeight := ToFloat64(this.Throttler.Config["maxWeight"])
			scaledUsed := math.Round((used / ToFloat64(limit)) * maxWeight)
