...
func (this *Limitless) fetchMarketsBody(ch chan any, optionalArgs ...any) any {
	...
	if queriesLength > 0 {
		...
		var knownTotal any = func() any {
			if totalMarketsCount != nil {
				return totalMarketsCount
			}
			return 0
		}()
		...
		if ccxt.IsGreaterThanOrEqual(lastPageLength, pageSize) && ccxt.IsLessThan(allRawLength, maxMarkets) {
			for true {
				...
				var page_markets any = func() any {
					if !ccxt.IsEqual(rawPageMarkets, nil) {
						return rawPageMarkets
					}
					return []any{}
				}()
	...
	for i := 0; i < ccxt.GetArrayLength(expandedRaw); i++ {
		...
		var eventKey any = func() any {
			if (groupId != nil) && (groupId == nil || *groupId != "") {
				return this.ShortenSlug(groupId)
			}
			return nil
		}()
...
func (this *Limitless) ParsePredictionTicker(ticker any, optionalArgs ...any) any {
	...
	if (lastStr == nil) && (pricesLength > 0) {
		lastStr = func() any {
			if isYes {
				return this.SafeString(prices, 0)
			}
			return this.SafeString(prices, 1)
		}()
	}
...
func (this *Limitless) fetchOrderBookBody(ch chan any, outcome any, optionalArgs ...any) any {
	...
	var bidsSource any = func() any {
		if isYes {
			return rawBids
		}
		return rawAsks
	}()
	var asksSource any = func() any {
		if isYes {
			return rawAsks
		}
		return rawBids
...
func (this *Limitless) fetchOHLCVBody(ch chan any, outcome any, optionalArgs ...any) any {
	...
	var rawHistory any = func() any {
		if !ccxt.IsEqual(rawHistoryList, nil) {
	...
	for i := 0; i < ccxt.GetArrayLength(history); i++ {
		...
		if ccxt.IsEqual(pointTs, nil) {
			...
			pointTs = func() any {
				if (tsString != nil) && (tsString == nil || *tsString != "") {
					return this.Parse8601(tsString)
				}
				return nil
			}()
	...
	for i := 0; i < len(sorted); i++ {
		...
		if !(func() bool { _, ok := candles[key]; return ok }()) {
			...
			var candleLow any = func() any {
				if ccxt.IsEqual(ccxt.GetValue(candle, 3), nil) {
					return pPrice
				}
...
func (this *Limitless) createOrderBody(ch chan any, outcome any, typeVar any, side any, amount any, optionalArgs ...any) any {
	...
	var walletFromAccount any = func() any {
		if usesSmartWallet {
			return this.SafeString(accountInfo, "smartWallet")
		}
		return this.SafeString(accountInfo, "account")
	}()
	...
	signRequest["takerAmount"] = func() any {
		if isMarket {
			return 1
		}
		return this.ParseToInt(this.ApplyScale(takerAmount, true))
	}()
...
func (this *Limitless) fetchMyTradesBody(ch chan any, optionalArgs ...any) any {
	...
	for i := 0; i < ccxt.GetArrayLength(data); i++ {
		...
		if strategy != nil {
			var buyIndex int = func() int {
				if strategy == nil {
					return -1
				}
				return strings.Index(*strategy, "buy")
			}()
			var sellIndex int = func() int {
				if strategy == nil {
					return -1
				}
				return strings.Index(*strategy, "sell")
			}()
...
func (this *Limitless) ParsePredictionTrade(trade any, optionalArgs ...any) any {
	...
	if func() int {
		...
		return strings.Index(*rawSide, "limit")
	}() >= 0 {
		typeVar = "limit"
		takerOrMaker = "maker"
		if rawSide == nil {
		...
		return strings.Index(*rawSide, "market")
	}() >= 0 {
		typeVar = "market"
		takerOrMaker = "taker"
	}
