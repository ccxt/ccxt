...
func (this *Limitless) fetchMarketsBody(ch chan any, optionalArgs ...any) any {
	...
	var expandedRaw []any = ccxt.ArrayTyped(this.ExpandGroupRows(allRaw))
	for i := 0; i < len(expandedRaw); i++ {
		var raw any = func() any {
			if i >= 0 && i < len(expandedRaw) {
				return ccxt.DerefScalar(expandedRaw[i])
			}
			return nil
		}()
	...
	for i := 0; i < ccxt.GetArrayLength(rawRows); i++ {
		...
		if (rowType != nil && *rowType == "group") && ((nestedMarkets != nil)) {
			...
			for j := 0; j < nestedMarketsLength; j++ {
				...
				var tagged map[string]any = this.Extend(func() any {
					if j >= 0 && j < len(nestedMarkets) {
						return ccxt.DerefScalar(nestedMarkets[j])
					}
					return nil
				}(), map[string]any{
					"groupSlug":  groupSlug,
...
func (this *Limitless) ParsePredictionTicker(ticker any, optionalArgs ...any) any {
	...
	var prices []any = ccxt.SafeListTyped(raw, "prices")
...
func (this *Limitless) fetchOrderBookBody(ch chan any, outcome string, optionalArgs ...any) any {
	...
	var outcomeLabel *string = this.SafeStringLower(outcomeObj["info"], "outcomeLabel", "yes")
	...
	var bidsSource any = rawAsks
	if isYes {
		bidsSource = rawBids
	}
	var asksSource any = rawBids
	if isYes {
		asksSource = rawAsks
	}
	var bids []any = []any{}
...
func (this *Limitless) fetchOHLCVBody(ch chan any, outcome string, optionalArgs ...any) any {
	...
	for i := 0; i < len(sorted); i++ {
		var point map[string]any = ccxt.SafeMapTyped(sorted, i)
		...
		var key string = strconv.FormatInt(bucket, 10)
...
func (this *Limitless) ParsePredictionOrder(order any, optionalArgs ...any) any {
	...
	// todo check
	var amountKey string = "makerAmount"
	...
	var execution map[string]any = ccxt.SafeMapTyped(data, "execution")
...
func (this *Limitless) ApplyScale(amount any, optionalArgs ...any) any {
	var multiply bool = ccxt.GetArgBool(optionalArgs, 0, false)
	_ = multiply
	var decimals *int64 = this.SafeInteger(this.Options, "usdcDecimals", 6)
...
func (this *Limitless) createOrderBody(ch chan any, outcome string, typeVar string, side string, amount any, optionalArgs ...any) any {
	...
	var paramsValue any = params
	var makerparamsValueVariable []any = this.HandleOptionAndParams(paramsValue, "createOrder", "maker", maker)
	maker = ccxt.GetValue(makerparamsValueVariable, 0)
	paramsValue = ccxt.GetValue(makerparamsValueVariable, 1)
	...
	var signerparamsValueVariable []any = this.HandleOptionAndParams(paramsValue, "createOrder", "signer", signer)
	signer = ccxt.GetValue(signerparamsValueVariable, 0)
	paramsValue = ccxt.GetValue(signerparamsValueVariable, 1)
	...
	paramsValue = ccxt.GetValue(takerparamsValueVariable, 1)
	...
	var signatureTypeparamsValueVariable []any = this.HandleOptionAndParams(paramsValue, "createOrder", "signatureType", signatureType)
	signatureType = ccxt.GetValue(signatureTypeparamsValueVariable, 0)
	paramsValue = ccxt.GetValue(signatureTypeparamsValueVariable, 1)
	...
	var expirationInt *int64 = this.SafeInteger(paramsValue, "expiration")
	...
	var postOnlyparamsValueVariable []any = this.HandlePostOnly(isMarket, false, paramsValue)
	postOnly = ccxt.GetValue(postOnlyparamsValueVariable, 0)
	paramsValue = ccxt.GetValue(postOnlyparamsValueVariable, 1)
	var timeInForce *string = this.SafeString(paramsValue, "timeInForce")
...
func (this *Limitless) cancelAllOrdersBody(ch chan any, optionalArgs ...any) any {
	...
	if outcome != nil {
		var warn any = true
		var warnparamsValueVariable []any = this.HandleOptionAndParams(paramsValue, "cancelAllOrders", "warnOnCancelAllOrdersWithOutcome", warn)
		warn = ccxt.GetValue(warnparamsValueVariable, 0)
		paramsValue = ccxt.GetValue(warnparamsValueVariable, 1)
		if warn == true {
...
func (this *Limitless) fetchMyTradesBody(ch chan any, optionalArgs ...any) any {
	...
	var paramsValue any = params
	var paginateparamsValueVariable []any = this.HandleOptionAndParams(paramsValue, "fetchMyTrades", "paginate", paginate)
	paginate = ccxt.GetValue(paginateparamsValueVariable, 0)
	paramsValue = ccxt.GetValue(paginateparamsValueVariable, 1)
	if paginate == true {
...
func (this *Limitless) ParsePredictionTrade(trade any, optionalArgs ...any) any {
	...
	if matchedSize != nil {
		...
		var feedSide *string = nil
		if sideRaw != nil && *sideRaw == "0" {
			feedSide = ccxt.SafeStringPtr("buy")
		} else if sideRaw != nil && *sideRaw == "1" {
			feedSide = ccxt.SafeStringPtr("sell")
		}
...
func (this *Limitless) fetchPositionsBody(ch chan any, optionalArgs ...any) any {
	...
	//     }
	//
	var clob []any = ccxt.SafeListTyped(response, "clob")
	var result []any = []any{}
	var labels []any = []any{"yes", "no"}
	for i := 0; i < len(clob); i++ {
		var entry map[string]any = ccxt.SafeMapTyped(clob, i)
...
func (this *Limitless) fetchEventsBody(ch chan any, optionalArgs ...any) any {
	...
	// group rows carry their tradeable children in a nested `markets` list — expand them
	// into regular rows before parsing (a group row itself has no tokens)
	var expandedMarkets []any = ccxt.ArrayTyped(this.ExpandGroupRows(rawMarkets))
	var rawMarketsLength int = len(expandedMarkets)
	for i := 0; i < rawMarketsLength; i++ {
		var raw any = func() any {
			if i >= 0 && i < len(expandedMarkets) {
				return ccxt.DerefScalar(expandedMarkets[i])
			}
			return nil
		}()
		var groupId *string = this.SafeStringN(raw, []any{"groupSlug", "groupId"}, this.SafeString(raw, "slug"))
...
func (this *Limitless) fetchRawMarketsByTagsBody(ch chan any, tags any, optionalArgs ...any) any {
	...
	for i := 0; i < categoriesLength; i++ {
		var category map[string]any = ccxt.SafeMapTyped(categories, i)
...
func (this *Limitless) Sign(path string, optionalArgs ...any) any {
	...
	if ccxt.IsEqual(access, "private") {
		...
		if (method == "POST") && (querystring != "") {
			...
			headersValue = this.Extend(map[string]any{
		...
		var payload *string = ccxt.SafeStringPtr(ccxt.Add(ccxt.Add(ccxt.Add(ccxt.Add(ccxt.Add(ccxt.Add(timestamp, newline), method), newline), url), newline), bodyString))
...
func (this *Limitless) FetchOrderTrades(id string, params map[string]any, options ...FetchOrderTradesOptions) ([]ccxt.PredictionTrade, error) {
...
func (this *Limitless) WatchMyTrades(params map[string]any, options ...WatchMyTradesOptions) ([]ccxt.PredictionTrade, error) {
...
func (this *Limitless) WatchOrderBook(outcome string, params map[string]any, options ...ccxt.WatchOrderBookOptions) (ccxt.PredictionOrderBook, error) {
