...
func (this *LimitlessCore) FetchMarkets(optionalArgs ...any) <-chan any {
	...
	go func() any {
		...
		if ccxt.IsTrue(ccxt.IsTrue(queries) && ccxt.IsTrue(ccxt.IsGreaterThan(queriesLength, 0))) {
			...
			if ccxt.IsTrue(ccxt.IsTrue(ccxt.IsGreaterThanOrEqual(lastPageLength, pageSize)) && ccxt.IsTrue(ccxt.IsLessThan(ccxt.GetArrayLength(allRaw), maxMarkets))) {
				for true {
					...
					if ccxt.IsTrue(ccxt.IsTrue(ccxt.IsLessThan(pageMarketsLength, pageSize)) || ccxt.IsTrue(ccxt.IsGreaterThanOrEqual(ccxt.GetArrayLength(allRaw), maxMarkets))) {
						break
					}
				}
			}
		...
		for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(allRaw)); i++ {
			...
			if ccxt.IsTrue(eventKey) {
				...
				retRes30316 := ccxt.GetValue(eventGroup, "markets")
				ccxt.AppendToArray(&retRes30316, m)
			}
		}
...
func (this *LimitlessCore) FetchOpenOrders(optionalArgs ...any) <-chan any {
	...
	go func() any {
		...
		retRes144615 := (<-this.FetchOrders(outcome, since, limit, params))
		ccxt.PanicOnError(retRes144615)
		ch <- retRes144615
		return nil
...
func (this *LimitlessCore) FetchClosedOrders(optionalArgs ...any) <-chan any {
	...
	go func() any {
		...
		retRes146815 := (<-this.FetchOrders(outcome, since, limit, params))
...
func (this *LimitlessCore) SendEvmTransaction(rpcUrl any, chainId any, fromAddress any, to any, value any, data any, gasLimit any) <-chan any {
	...
	go func() any {
		...
		retRes224915 := (<-this.EthRpc(rpcUrl, "eth_sendRawTransaction", []any{signed}))
...
func (this *LimitlessCore) WaitForTransactionReceipt(rpcUrl any, txHash any, optionalArgs ...any) <-chan any {
	...
	go func() any {
		...
		for ccxt.IsLessThan((ccxt.Subtract(this.Milliseconds(), start)), timeout) {
			...
			retRes225912 := (<-this.Sleep(2000))
			ccxt.PanicOnError(retRes225912)
		}
...
func (this *LimitlessCore) Approve(optionalArgs ...any) <-chan any {
	...
	go func() any {
		...
		retRes230515 := (<-this.WaitForTransactionReceipt(rpcUrl, txHash))
...
func (this *LimitlessCore) FetchMyTrades(optionalArgs ...any) <-chan any {
	...
	go func() any {
		...
		if ccxt.IsTrue(paginate) {
			...
			retRes241619 := (<-this.FetchPaginatedCallCursor("fetchMyTrades", outcome, since, limit, params, "nextCursor", "cursor", nil, maxLimit))
			ccxt.PanicOnError(retRes241619)
			ch <- retRes241619
			return nil
		}
		var request any = map[string]any{}
