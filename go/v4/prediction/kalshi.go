			...
			for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(rawMarkets)); i++ {
				...
				for j := 0; ccxt.IsLessThan(j, ccxt.GetArrayLength(parsed)); j++ {
					...
					if ccxt.IsTrue(eventKey) {
						...
						retRes25624 := ccxt.GetValue(eventEntry, "markets")
						ccxt.AppendToArray(&retRes25624, m)
					}
				}
			}
	...
	go func() any {
		...
		for i := 0; ccxt.IsLessThan(i, restingOrdersLength); i++ {
			...
			if ccxt.IsTrue(!ccxt.IsEqual(orderId, nil)) {
				ccxt.AppendToArray(&ids, orderId)
			}
		}
		...
		var batchLimit any = 20 // kalshi caps the batched-cancel endpoint at 20 ids per call
		var remaining any = ids
		var remainingLength any = ccxt.GetArrayLength(remaining)
		for ccxt.IsGreaterThan(remainingLength, 0) {
			var batchIds any = this.ArraySlice(remaining, 0, batchLimit)
			remaining = this.ArraySlice(remaining, batchLimit)

			response := (<-this.KalshiPrivateDeletePortfolioOrdersBatched(this.Extend(map[string]any{
				"ids": batchIds,
			}, params)))
			ccxt.PanicOnError(response)
			var batchResult any = this.SafeList(response, "orders", []any{})
			var batchResultLength any = ccxt.GetArrayLength(batchResult)
			for j := 0; ccxt.IsLessThan(j, batchResultLength); j++ {
				ccxt.AppendToArray(&canceledOrders, ccxt.GetValue(batchResult, j))
			}
			remainingLength = ccxt.GetArrayLength(remaining)
		}
