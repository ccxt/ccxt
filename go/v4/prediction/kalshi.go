	...
	for true {
		...
		for i := 0; i < len(rawMarkets); i++ {
			...
			for j := 0; j < len(parsed); j++ {
				var m any = func() any {
					if j >= 0 && j < len(parsed) {
						return ccxt.DerefScalar(parsed[j])
					}
					return nil
				}()
...
func (this *Kalshi) fetchOutcomeBody(ch chan any, outcomeSymbol any) any {
	...
	if ccxt.GetIndexOf(outcomeSymbol, ":") < 0 {
		...
		var baseTicker any = outcomeSymbol
		if isNo {
			baseTicker = ccxt.Slice(outcomeSymbol, 0, symbolLength-3)
		}
		...
		var parts []string = strings.Split(*marketPart, "_")
...
func (this *Kalshi) fetchOutcomesBody(ch chan any, outcomeSymbols any) any {
	...
	for i := 0; i < ccxt.GetArrayLength(outcomeSymbols); i++ {
		...
		var symbolLength int64 = this.ParseToInt(ccxt.GetLength(outcomeSymbol))
		var suffix string = ccxt.Slice(outcomeSymbol, symbolLength-3, nil)
		var baseTicker *string = outcomeSymbol
		if suffix == "-NO" {
			baseTicker = ccxt.SafeStringPtr(ccxt.Slice(outcomeSymbol, 0, symbolLength-3))
		}
		if !(func() bool {
			if baseTicker == nil {
				return false
			}
			_, ok := seen[*baseTicker]
			return ok
		}()) {
...
func (this *Kalshi) CalculateFee(symbol any, typeVar any, side any, amount any, price any, optionalArgs ...any) any {
	...
	var takerOrMaker string = ccxt.GetArgString(optionalArgs, 0, "taker")
	_ = takerOrMaker
...
func (this *Kalshi) ParseMarket(raw any) any {
	...
	if (eventTicker != nil) && (eventTicker == nil || *eventTicker != "") {
		eventParts = strings.Split(*eventTicker, "-")
	}
	...
	var subtitleOrTicker *string = ticker
	if subtitle != nil {
	...
	var priceRanges []any = ccxt.SafeListTyped(raw, "price_ranges")
	...
	for oi := 0; oi < len(outcomeLabels); oi++ {
		...
		if resolved && (result != nil) && (result == nil || *result != "") {
			winnerRaw = (ccxt.IsEqual(strings.ToLower(*label), result))
...
func (this *Kalshi) fetchOrderBookBody(ch chan any, outcome any, optionalArgs ...any) any {
	...
	if isNo {
		...
		for bi := 0; bi < len(rawNo); bi++ {
			var price *float64 = this.SafeNumber(func() any {
				if bi >= 0 && bi < len(rawNo) {
					return ccxt.DerefScalar(rawNo[bi])
				}
				return nil
			}(), 0)
			bids = append(bids, []any{price, this.SafeNumber(func() any {
				if bi >= 0 && bi < len(rawNo) {
					return ccxt.DerefScalar(rawNo[bi])
				}
				return nil
		...
		for ai := 0; ai < len(rawYes); ai++ {
			var yesPrice *float64 = this.SafeNumber(func() any {
				if ai >= 0 && ai < len(rawYes) {
					return ccxt.DerefScalar(rawYes[ai])
				}
				return nil
			}(), 0)
			...
			asks = append(asks, []any{price, this.SafeNumber(func() any {
				if ai >= 0 && ai < len(rawYes) {
					return ccxt.DerefScalar(rawYes[ai])
				}
				return nil
		...
		for bi := 0; bi < len(rawYes); bi++ {
			var price *float64 = this.SafeNumber(func() any {
				if bi >= 0 && bi < len(rawYes) {
					return ccxt.DerefScalar(rawYes[bi])
				}
				return nil
			}(), 0)
			bids = append(bids, []any{price, this.SafeNumber(func() any {
		...
		for ai := 0; ai < len(rawNo); ai++ {
			var noPrice *float64 = this.SafeNumber(func() any {
				if ai >= 0 && ai < len(rawNo) {
					return ccxt.DerefScalar(rawNo[ai])
				}
				return nil
			}(), 0)
			var price *float64 = func() *float64 {
...
func (this *Kalshi) fetchOHLCVBody(ch chan any, outcome any, optionalArgs ...any) any {
	...
	//     }
	//
	var candles []any = ccxt.SafeListTyped(response, "candlesticks")
...
func (this *Kalshi) fetchMyTradesBody(ch chan any, optionalArgs ...any) any {
	...
	for i := 0; i < fillsLength; i++ {
		trades = append(trades, this.ParseMyTrade(func() any {
			if i >= 0 && i < len(fills) {
...
func (this *Kalshi) ParseMyTrade(fill any, optionalArgs ...any) any {
	...
	var mkt map[string]any = this.SafeOutcome(outcomeKey, market)
...
func (this *Kalshi) fetchSettlementsBody(ch chan any, optionalArgs ...any) any {
	...
	for i := 0; i < rawSettlementsLength; i++ {
		parsed = append(parsed, this.ParseSettlement(func() any {
			if i >= 0 && i < len(rawSettlements) {
				return ccxt.DerefScalar(rawSettlements[i])
			}
			return nil
		}()))
	}
...
func (this *Kalshi) ParseSettlement(settlement any, optionalArgs ...any) any {
	...
	var heldLabel string = "NO"
	if heldYes {
		heldLabel = "YES"
	}
	var tickerMissing bool = (ticker == nil)
	var useHeldYesTicker bool = (heldYes || tickerMissing)
	var heldTicker any = nil
	if useHeldYesTicker {
		heldTicker = ticker
	} else {
		heldTicker = (*ticker + "-NO")
	}
	var mkt map[string]any = this.SafeOutcome(heldTicker, market)
	...
	var costKey string = "no_total_cost"
	if heldYes {
		costKey = "yes_total_cost"
	}
	var costDollarsKey string = "no_total_cost_dollars"
	if heldYes {
...
func (this *Kalshi) ParsePredictionOrder(order any, optionalArgs ...any) any {
	...
	var dollarsKey string = "yes_price_dollars"
	if labelIsNo {
		dollarsKey = "no_price_dollars"
	}
	var centsKey string = "yes_price"
	if labelIsNo {
		centsKey = "no_price"
	}
...
func (this *Kalshi) cancelAllOrdersBody(ch chan any, optionalArgs ...any) any {
	...
	for i := 0; i < restingOrdersLength; i++ {
		var restingOrder any = func() any {
			if i >= 0 && i < len(restingOrders) {
				return ccxt.DerefScalar(restingOrders[i])
			}
			return nil
		}()
...
func (this *Kalshi) fetchEventsByQueryBody(ch chan any, queries any, limit any, optionalArgs ...any) any {
	...
	for qi := 0; qi < queriesLength; qi++ {
		...
		for pi := 0; pi < pageLength; pi++ {
			var et *string = this.SafeString(func() any {
				if pi >= 0 && pi < len(page) {
					return ccxt.DerefScalar(page[pi])
				}
				return nil
			}(), "event_ticker")
			if et != nil {
...
func (this *Kalshi) Sign(path any, optionalArgs ...any) any {
	...
	if ccxt.IsEqual(access, "private") {
		...
		var tradeApiIndex int = func() int {
			if baseUrl == nil {
				return -1
			}
...
func (this *Kalshi) FetchRawEventByTicker(ticker string, options ...FetchRawEventByTickerOptions) (map[string]any, error) {
	...
	var res ccxt.AsyncResult[map[string]any] = ccxt.AwaitResult(ccxt.AssertAs[map[string]any], this.FetchRawEventByTickerAsync(ticker, opts.Params))
	if res.Err != nil {
		return map[string]any{}, res.Err
	}
	return res.Value, nil
...
func (this *Kalshi) FetchSeriesEvents(seriesTickers []string, status string, limit int64, rest map[string]any) ([]map[string]any, error) {
	var res ccxt.AsyncResult[[]map[string]any] = ccxt.AwaitResult(ccxt.NewMapArray, this.FetchSeriesEventsAsync(seriesTickers, status, limit, rest))
	if res.Err != nil {
		return nil, res.Err
	}
	return res.Value, nil
...
func (this *Kalshi) CancelOrders(ids []string, params map[string]any, options ...CancelOrdersOptions) ([]ccxt.PredictionOrder, error) {
...
func (this *Kalshi) FetchOrderTrades(id string, params map[string]any, options ...FetchOrderTradesOptions) ([]ccxt.PredictionTrade, error) {
...
func (this *Kalshi) WatchMyTrades(params map[string]any, options ...WatchMyTradesOptions) ([]ccxt.PredictionTrade, error) {
...
func (this *Kalshi) WatchOrderBook(outcome string, params map[string]any, options ...ccxt.WatchOrderBookOptions) (ccxt.PredictionOrderBook, error) {
