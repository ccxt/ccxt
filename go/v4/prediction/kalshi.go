	...
	for true {
		...
		for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(rawMarkets)); i++ {
			...
			for j := 0; ccxt.IsLessThan(j, ccxt.GetArrayLength(parsed)); j++ {
				...
				if (!ccxt.IsEqual(eventKey, nil)) && (eventKey != "") {
		...
		if (ccxt.IsEqual(cursor, nil) || (cursor == "")) || ccxt.IsLessThan(rawMarketsLength, limit) || ccxt.IsGreaterThanOrEqual(collectedLength, maxMarkets) {
			break
		}
	}
...
func (this *KalshiCore) createOrderBody(ch chan any, outcome any, typeVar any, side any, amount any, optionalArgs ...any) any {
	...
	if ccxt.IsEqual(ccxt.GetValue(order, "status"), nil) {
		...
		if (!ccxt.IsEqual(remainingCount, nil)) && (remainingCount == 0) {
...
func (this *KalshiCore) fetchSeriesEventsBody(ch chan any, seriesTickers any, status any, limit any, optionalArgs ...any) any {
	...
	for si := 0; ccxt.IsLessThan(si, seriesTickersLength); si++ {
		...
		for page := 0; ccxt.IsLessThan(page, maxPages); page++ {
			...
			if (ccxt.IsEqual(cursor, nil)) || (cursor == "") || (ccxt.IsLessThan(pageEventsLength, reqLimit)) {
				break
			}
		}
	}
...
func (this *KalshiCore) ParseEvent(rawEvent any) any {
	...
	var resolved any = this.SafeBool(rawEvent, "resolved")
...
func (this *KalshiCore) Sign(path any, optionalArgs ...any) any {
	...
	if (method == "GET") && (!ccxt.IsEqual(querystring, "")) {
