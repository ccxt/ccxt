...
func  (this *HyperliquidCore) FetchMarkets(optionalArgs ...any) <- chan any {
            ...
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(outcomesList)); i++ {
                ...
                for oi := 0; ccxt.IsLessThan(oi, ccxt.GetArrayLength(marketOutcomes)); oi++ {
                    ...
                    var outcomeSymbol any = this.SafeString(outcome, "symbol")
                    var outcomeId_ any = this.SafeString(outcome, "id")
 ...
 */
func  (this *HyperliquidCore) FetchTicker(symbol any, optionalArgs ...any) <- chan any {
            ch := make(chan any)
            go func() any {
                defer close(ch)
                defer ccxt.ReturnPanicError(ch)
            ...
            var ticker any = this.ParseTicker(tickerData, market)
            ccxt.AddElementToObject(ticker, "symbol", this.SafeString(outcomeObj, "symbol", outcome))
...
func  (this *HyperliquidCore) FetchTickers(optionalArgs ...any) <- chan any {
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(outcomes, nil)) {
                for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(outcomes)); i++ {
                    ...
                    var requestedSymbol any = this.SafeString(requestedOutcomeObj, "symbol", requested)
                    ccxt.AddElementToObject(requestedOutcomeSymbols, requestedSymbol, true)
                }
            }
            ...
            var outcomeSymbols any = ccxt.ObjectKeys(outcomesMap)
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(outcomeSymbols)); i++ {
                var outcomeSymbol any = ccxt.GetValue(outcomeSymbols, i)
                if ccxt.IsTrue(ccxt.IsTrue(!ccxt.IsEqual(outcomes, nil)) && !ccxt.IsTrue((ccxt.InOp(requestedOutcomeSymbols, outcomeSymbol)))) {
                    continue
                }
                var outcomeObj any = this.SafeDict(outcomesMap, outcomeSymbol, map[string]any {})
                var info any = this.SafeDict(outcomeObj, "info", map[string]any {})
                var coin any = this.SafeString(info, "coinName")
                ...
                ccxt.AddElementToObject(ticker, "symbol", outcomeSymbol)
                ccxt.AddElementToObject(tickers, outcomeSymbol, ticker)
            }
...
func  (this *HyperliquidCore) ParseTicker(raw any, optionalArgs ...any) any  {
    ...
    var ctx any = ccxt.Ternary(ccxt.IsTrue(market), this.SafeDict(this.SafeDict(market, "info", map[string]any {}), "ctx", map[string]any {}), map[string]any {})
...
func  (this *HyperliquidCore) FetchOrderBook(symbol any, optionalArgs ...any) <- chan any {
            ...
            ch <- this.ParseOrderBook(map[string]any {
                "bids": bids,
                "asks": asks,
...
func  (this *HyperliquidCore) FetchPositions(optionalArgs ...any) <- chan any {
            ...
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(balances)); i++ {
                ...
                if ccxt.IsTrue(!ccxt.IsTrue(coin) || !ccxt.IsTrue(ccxt.StartsWith(coin, "+"))) {
                    continue
                }
                var totalStr any = this.SafeString(balance, "total")
                var total any = this.ParseNumber(totalStr)
                if ccxt.IsTrue(ccxt.IsTrue(ccxt.IsEqual(total, nil)) || ccxt.IsTrue(ccxt.IsEqual(total, 0))) {
                    continue
                }
                var outcomeId any = ccxt.Add("#", ccxt.Slice(coin, 1, nil)) // +10 -> #10
                var outcomeObj any = this.SafeOutcome(outcomeId)
                if ccxt.IsTrue(!ccxt.IsEqual(outcomes, nil)) {
                    var outcomeSymbol any = this.SafeString(outcomeObj, "symbol")
                    if ccxt.IsTrue(ccxt.IsTrue(ccxt.IsEqual(outcomeSymbol, nil)) || !ccxt.IsTrue((ccxt.InOp(requestedOutcomeSymbols, outcomeSymbol)))) {
                        continue
                    }
                }
                ccxt.AppendToArray(&positions, this.ParsePosition(balance, outcomeObj))
            }
...
func  (this *HyperliquidCore) ParsePosition(position any, optionalArgs ...any) any  {
    ...
    var total any = this.ParseNumber(totalStr)
    var holdStr any = this.SafeString(position, "hold")
    var hold any = this.ParseNumber(holdStr)
    var entryNtlStr any = this.SafeString(position, "entryNtl")
    var entryNotional any = this.ParseNumber(entryNtlStr)
    var entryPrice any = nil
    if ccxt.IsTrue(ccxt.IsTrue(ccxt.IsTrue(!ccxt.IsEqual(entryNotional, nil)) && ccxt.IsTrue(!ccxt.IsEqual(total, nil))) && ccxt.IsTrue(ccxt.IsGreaterThan(total, 0))) {
        entryPrice = ccxt.Divide(entryNotional, total)
    }
    return map[string]any {
        "id": nil,
...
func  (this *HyperliquidCore) CreateOrder(symbol any, typeVar any, side any, amount any, optionalArgs ...any) <- chan any {
            ...
            ccxt.PanicOnError(retRes12008)
        
            retRes12018 := (<-this.InitializeClient())
            ...
            var sz any = this.AmountToPrecision(this.SafeString(outcomeObj, "marketSymbol"), amount)
...
func  (this *HyperliquidCore) CancelOrders(ids any, optionalArgs ...any) <- chan any {
            ...
            retRes13438 := (<-this.InitializeClient())
            ...
            var statuses any = this.SafeList(data, "statuses", []any{})
            var outcomeSymbol any = this.SafeString(outcomeObj, "symbol", outcome)
            ...
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(statuses)); i++ {
                ...
                ccxt.AppendToArray(&orders, this.SafeOrder(order))
            }
...
func  (this *HyperliquidCore) FetchOpenOrders(optionalArgs ...any) <- chan any {
            ...
            ch <- this.FilterBySymbolSinceLimit(parsed, symbol, since, limit)
            return nil
...
func  (this *HyperliquidCore) FetchOrder(id any, optionalArgs ...any) <- chan any {
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(outcome, nil)) {
                this.CheckEventsAndMarkets(outcome)
                var outcomeObj any = this.Outcome(outcome)
                var expected any = this.SafeString(outcomeObj, "symbol")
                if ccxt.IsTrue(!ccxt.IsEqual(this.SafeString(parsed, "symbol"), expected)) {
...
func  (this *HyperliquidCore) FetchMyTrades(optionalArgs ...any) <- chan any {
            ...
            response:= (<-this.PublicPostInfo(this.Extend(request, params)))
            ccxt.PanicOnError(response)
            var parsed any = this.ParseTrades(response, nil, since, nil)
            symbol = nil
            if ccxt.IsTrue(!ccxt.IsEqual(outcome, nil)) {
                this.CheckEventsAndMarkets(outcome)
                var outcomeObj any = this.Outcome(outcome)
...
func  (this *HyperliquidCore) FetchEvents(optionalArgs ...any) <- chan any {
            ...
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(marketValues)); i++ {
                ...
                if ccxt.IsTrue(ccxt.IsGreaterThan(lowerQueriesLength, 0)) {
                    ...
                    for qi := 0; ccxt.IsLessThan(qi, ccxt.GetArrayLength(lowerQueries)); qi++ {
                        if ccxt.IsTrue(ccxt.IsTrue(ccxt.IsGreaterThan(ccxt.GetIndexOf(description, ccxt.GetValue(lowerQueries, qi)), ccxt.OpNeg(1))) || ccxt.IsTrue(ccxt.IsGreaterThan(ccxt.GetIndexOf(symLower, ccxt.GetValue(lowerQueries, qi)), ccxt.OpNeg(1)))) {
                            matches = true
                            break
                        }
                    }
                ...
                retRes181712 := ccxt.GetValue(groupMap, parentSymbol)
                ccxt.AppendToArray(&retRes181712, mkt)
            }
            var events any = []any{}
            ...
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(events)); i++ {
                ...
                ccxt.AddElementToObject(this.Events, ccxt.GetValue(ev, "symbol"), ev)
            }
...
func  (this *HyperliquidCore) SignHash(hash any, privateKey any) any  {
    ...
    var r any = ccxt.PadStart(ccxt.GetValue(signature, "r"), 64, "0")
    var s any = ccxt.PadStart(ccxt.GetValue(signature, "s"), 64, "0")
    return map[string]any {
        "r": ccxt.Add("0x", r),
        "s": ccxt.Add("0x", s),
...
func  (this *HyperliquidCore) SignL1Action(action any, nonce any, optionalArgs ...any) any  {
    ...
    var isTestnet any = this.SafeBool(this.Options, "sandboxMode", true)
...
func  (this *HyperliquidCore) Sign(path any, optionalArgs ...any) any  {
    ...
    var sandboxMode any = this.SafeBool(this.Options, "sandboxMode", true)
    var baseUrl any = nil
