...
func (this *Hyperliquid) ParseOutcomeDescription(description *string) map[string]any {
	...
	for i := 0; i < len(parts); i++ {
		...
		var colonIndex int = strings.Index(part, ":")
		if colonIndex > -1 {
...
func (this *Hyperliquid) BuildOutcomeSymbol(desc any, side any, outcomeId any) any {
	...
	if expiry == nil || *expiry != "" {
		expiryDate = ccxt.GetValue(strings.Split(*expiry, "-"), 0)
	}
	var label string = "NO"
	if ccxt.IsEqual(side, 0) {
		label = "YES"
	}
	var base any = strings.ToUpper(*underlying)
...
func (this *Hyperliquid) BuildOutcomeParentSymbol(desc any, outcomeId any, optionalArgs ...any) any {
	...
	_ = name
	var question map[string]any = ccxt.GetArgMap(optionalArgs, 1, map[string]any{})
	_ = question
	...
	if (questionDescription != nil) && (questionDescription == nil || *questionDescription != "") {
		...
		if questionClass != nil && *questionClass == "pricebucket" {
			...
			if questionExpiry == nil || *questionExpiry != "" {
				expiryDate = ccxt.GetValue(strings.Split(*questionExpiry, "-"), 0)
			}
			...
			if ((questionUnderlying != nil) && (questionUnderlying == nil || *questionUnderlying != "")) && (thresholdsRaw == nil || *thresholdsRaw != "") && (indexStr != nil) {
				...
				if thresholdsLength > 0 {
					...
					var base any = ccxt.Add(strings.ToUpper(*questionUnderlying)+"_", bucketLabel)
					if (expiryDate != nil) && (!ccxt.IsEqual(expiryDate, "")) {
						base = ccxt.Add(ccxt.Add(base, "_"), expiryDate)
					}
					return base
			...
			var isFallbackLike bool = (rawDescription != nil && *rawDescription == "other") || (strings.Index(nameLower, "fallback") >= 0) || (strings.Index(nameLower, "other") >= 0)
			if ((questionUnderlying != nil) && (questionUnderlying == nil || *questionUnderlying != "")) && isFallbackLike {
				var base any = strings.ToUpper(*questionUnderlying) + "_OTHER"
				if (expiryDate != nil) && (!ccxt.IsEqual(expiryDate, "")) {
					base = ccxt.Add(ccxt.Add(base, "_"), expiryDate)
				}
				return base
...
func (this *Hyperliquid) fetchMarketsBody(ch chan any, optionalArgs ...any) any {
	...
	for qi := 0; qi < len(questionsList); qi++ {
		...
		var namedOutcomes []any = ccxt.SafeListTyped(question, "namedOutcomes")
		for ni := 0; ni < len(namedOutcomes); ni++ {
	...
	for i := 0; i < len(outcomesList); i++ {
		var outcomeInfo map[string]any = this.SafeDictMap(outcomesList, i, map[string]any{})
		...
		var marketOutcomes []any = ccxt.SafeListTyped(market, "outcomes")
		for oi := 0; oi < len(marketOutcomes); oi++ {
 ...
 */
func (this *Hyperliquid) ParseOutcomeMarket(outcomeInfo map[string]any, outcomeId any, optionalArgs ...any) any {
	var question map[string]any = ccxt.GetArgMap(optionalArgs, 0, map[string]any{})
	_ = question
	var description *string = this.SafeString(outcomeInfo, "description", "")
	var name *string = this.SafeString(outcomeInfo, "name", "")
	var sideSpecs []any = ccxt.SafeListTyped(outcomeInfo, "sideSpecs")
	...
	if (expiry != nil) && (expiry == nil || *expiry != "") {
		...
		var expParts []string = strings.Split(*expiry, "-")
		var expPartsLength int = len(expParts)
		if (expPartsLength >= 1) && (ccxt.GetLength(func() any {
			if 0 >= 0 && 0 < len(expParts) {
				return expParts[0]
			}
			return nil
		}()) == 8) {
			var ymd *string = ccxt.SafeStringPtr(func() any {
				if 0 >= 0 && 0 < len(expParts) {
					return expParts[0]
				}
				return nil
	...
	var outcomePrecision map[string]any = map[string]any{
		"amount": this.ParseNumber(this.ParsePrecision(strconv.Itoa(szDecimals))),
		"price":  0.0001,
	}
...
func (this *Hyperliquid) CalculatePricePrecision(midPx any, szDecimals any) any {
	...
	var parts []string = strings.Split(*midStr, ".")
	var intPart *string = ccxt.SafeStringPtr(func() any {
		if 0 >= 0 && 0 < len(parts) {
			return parts[0]
		}
		return nil
...
func (this *Hyperliquid) fetchTickersBody(ch chan any, optionalArgs ...any) any {
	...
	for i := 0; i < len(outcomeHandles); i++ {
		var outcomeHandle string = outcomeHandles[i]
...
func (this *Hyperliquid) ParsePredictionTicker(raw any, optionalArgs ...any) any {
	...
	var levels []any = ccxt.SafeListTyped(raw, "levels")
...
func (this *Hyperliquid) fetchOrderBookBody(ch chan any, outcome string, optionalArgs ...any) any {
	...
	var levels []any = ccxt.SafeListTyped(response, "levels")
	var rawBids []any = ccxt.SafeListTyped(levels, 0)
	var rawAsks []any = ccxt.SafeListTyped(levels, 1)
	var bids []any = []any{}
	var asks []any = []any{}
	for i := 0; i < len(rawBids); i++ {
		var entry map[string]any = ccxt.SafeMapTyped(rawBids, i)
		bids = append(bids, []any{this.SafeNumber(entry, "px"), this.SafeNumber(entry, "sz")})
	}
	for i := 0; i < len(rawAsks); i++ {
		var entry map[string]any = ccxt.SafeMapTyped(rawAsks, i)
...
func (this *Hyperliquid) fetchOHLCVBody(ch chan any, outcome string, optionalArgs ...any) any {
	...
	if since == nil {
		...
		var startOffset any = ccxt.Multiply(ccxt.Multiply(tf, candleCount), int64(-1000))
...
func (this *Hyperliquid) FindOutcomeInMarket(market any, optionalArgs ...any) any {
	...
	if normalizedHint != nil {
		for i := 0; i < len(outcomesList); i++ {
			var oc map[string]any = this.SafeDictMap(outcomesList, i, map[string]any{})
...
func (this *Hyperliquid) ParseOutcomeInputSideHint(outcomeInput any) any {
	...
	if (colonIndex > -1) && ccxt.IsLessThan(colonIndex, ccxt.GetLength(outcomeInput)-1) {
		var side string = strings.ToUpper(ccxt.Slice(outcomeInput, colonIndex+1, nil))
		if (side == "YES") || (side == "NO") {
			return side
		}
	}
...
func (this *Hyperliquid) ResolveOutcomeInput(outcomeInput any) any {
	...
	if ccxt.StartsWith(outcomeInput, "+") {
		candidates = append(candidates, "#"+ccxt.Slice(outcomeInput, 1, nil))
	}
...
func (this *Hyperliquid) createOrderBody(ch chan any, outcome string, typeVar string, side string, amount any, optionalArgs ...any) any {
	...
	if isMarket {
		...
		px = func() any {
			if isBuy {
				return ccxt.Precise.StringMul(priceStr, ccxt.Precise.StringAdd("1", slippage))
			}
	...
	var vaultAddressOption *string = ccxt.SafeStringPtr(ccxt.GetValue(ccxt.TupleSlice(this.HandleOptionStringAndParams(params, "createOrder", "vaultAddress")), 0))
...
func (this *Hyperliquid) cancelOrdersBody(ch chan any, ids any, optionalArgs ...any) any {
	...
	if !ccxt.IsEqual(clientOrderId, nil) {
		var cloids any = func() any {
			if ccxt.IsArray(clientOrderId) {
				return clientOrderId
			}
			return []any{clientOrderId}
		}()
	...
	var vaultAddressOption *string = ccxt.SafeStringPtr(ccxt.GetValue(ccxt.TupleSlice(this.HandleOptionStringAndParams(paramsOmitted, "cancelOrders", "vaultAddress")), 0))
	var vaultAddress any = this.FormatVaultAddress(vaultAddressOption)
...
func (this *Hyperliquid) fetchOpenOrdersBody(ch chan any, optionalArgs ...any) any {
	...
	if ccxt.IsArray(response) {
		rawOrders = ccxt.ArrayTyped(response)
	}
	for i := 0; i < len(rawOrders); i++ {
		...
		ordersWithStatus = append(ordersWithStatus, this.Extend(order, map[string]any{
			"ccxtStatus": "open",
		}))
	}
...
func (this *Hyperliquid) fetchOrdersBody(ch chan any, optionalArgs ...any) any {
	...
	if ccxt.IsArray(response) {
		historicalOrders = ccxt.ArrayTyped(response)
	}
	for i := 0; i < len(historicalOrders); i++ {
		var raw any = func() any {
			if i >= 0 && i < len(historicalOrders) {
				return ccxt.DerefScalar(historicalOrders[i])
			}
			return nil
		}()
...
func (this *Hyperliquid) ParseOrderType(status *string) *string {
	...
	var statusLower *string = func() *string {
		if (status != nil) && (status == nil || *status != "") {
			return ccxt.SafeStringPtr(strings.ToLower(*status))
		}
		return nil
	}()
...
func (this *Hyperliquid) ParseTimeInForce(timeInForce *string) *string {
	...
	var tifLower *string = func() *string {
		if (timeInForce != nil) && (timeInForce == nil || *timeInForce != "") {
			return ccxt.SafeStringPtr(strings.ToLower(*timeInForce))
		}
		return nil
	}()
...
func (this *Hyperliquid) fetchEventsBody(ch chan any, optionalArgs ...any) any {
	...
	for i := 0; i < len(queries); i++ {
		...
		lowerQueries = append(lowerQueries, strings.ToLower(*queryString))
	}
	...
	for i := 0; i < len(marketValues); i++ {
		...
		if lowerQueriesLength > 0 {
			var description string = strings.ToLower(*this.SafeString(info, "description", ""))
			...
			var symLower string = strings.ToLower(parentSymbolOrEmpty)
			...
			for qi := 0; qi < len(lowerQueries); qi++ {
				...
				for wi := 0; wi < wordsLength; wi++ {
					var word *string = ccxt.SafeStringPtr(words[wi])
	...
	var events []any = []any{}
	var groupKeys []string = nil
	if groupMap != nil {
		groupKeys = make([]string, 0, len(groupMap))
		for objectKey := range groupMap {
			groupKeys = append(groupKeys, objectKey)
		}
	}
	for gi := 0; gi < len(groupKeys); gi++ {
		var key string = groupKeys[gi]
...
func (this *Hyperliquid) ParseEvent(raw map[string]any) any {
	...
	if (expiryRaw != nil) && (expiryRaw == nil || *expiryRaw != "") {
		var parts []string = strings.Split(*expiryRaw, "-")
		var partsLength int = len(parts)
		if (partsLength >= 1) && (ccxt.GetLength(func() any {
			if 0 >= 0 && 0 < len(parts) {
				return parts[0]
			}
			return nil
