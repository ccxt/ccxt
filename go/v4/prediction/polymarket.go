...
func (this *Polymarket) fetchMarketsBody(ch chan any, optionalArgs ...any) any {
	...
	for rei := 0; rei < ccxt.GetArrayLength(rawEvents); rei++ {
		...
		var ccxtMarkets []any = ccxt.ArrayTyped(this.ParseEventToMarkets(rawEvent))
		for mi := 0; mi < len(ccxtMarkets); mi++ {
			flatMarkets = append(flatMarkets, func() any {
				if mi >= 0 && mi < len(ccxtMarkets) {
					return ccxt.DerefScalar(ccxtMarkets[mi])
				}
				return nil
			}())
		}
...
func (this *Polymarket) fetchOutcomeBody(ch chan any, outcomeSymbol any) any {
	...
	if (ccxt.GetIndexOf(outcomeSymbol, ":") < 0) && (ccxt.IsEqual(this.OutcomeSearchQuery(outcomeSymbol), nil)) {
		...
		if rawMarketsLength > 0 {
			...
			var ccxtMarkets []any = ccxt.ArrayTyped(this.ParseEventToMarkets(map[string]any{
				"markets": rawMarkets,
			}))
			var ccxtMarketsLength int = len(ccxtMarkets)
			for i := 0; i < ccxtMarketsLength; i++ {
				var mkt any = func() any {
					if i >= 0 && i < len(ccxtMarkets) {
						return ccxt.DerefScalar(ccxtMarkets[i])
					}
					return nil
				}()
...
func (this *Polymarket) fetchTradesBody(ch chan any, outcome any, optionalArgs ...any) any {
	...
	var conditionId *string = this.SafeString(outcomeInfo, "conditionId")
	if conditionId == nil {
		panic(ccxt.BadRequest(this.Id + " fetchTrades() requires outcome.info.conditionId for an outcome " + *tokenId))
	}
	...
	for i := 0; i < ccxt.GetArrayLength(rawTrades); i++ {
		...
		if tradeAsset == tokenId || (tradeAsset != nil && tokenId != nil && *tradeAsset == *tokenId) {
...
func (this *Polymarket) SignClobOrder(message any, exchangeAddress any, domainVersion any, sigType any) any {
	...
	var contentsHash *string = ccxt.SafeStringPtr(ccxt.Add("0x", this.Hash(contentsData, ccxt.Keccak, "hex")))
	...
	var appDomainSep *string = ccxt.SafeStringPtr(ccxt.Add("0x", this.Hash(appDomainData, ccxt.Keccak, "hex")))
...
func (this *Polymarket) watchOrderBookBody(ch chan any, outcome any, optionalArgs ...any) any {
	...
	var messageHash *string = ccxt.SafeStringPtr(ccxt.Add("orderbook::", outcome))
	var subscribeHash *string = ccxt.SafeStringPtr(ccxt.Add("subscribe::", tokenId))
...
func (this *Polymarket) watchTradesBody(ch chan any, outcome any, optionalArgs ...any) any {
	...
	var tokenId *string = this.SafeString(outcomeObj, "outcomeId")
	outcome = ccxt.DerefScalar(this.SafeString(outcomeObj, "outcome"))
	var messageHash *string = ccxt.SafeStringPtr(ccxt.Add("trades::", outcome))
	var subscribeHash *string = ccxt.SafeStringPtr(ccxt.Add("subscribe::", tokenId))
	var subscribeMsg map[string]any = map[string]any{
		"assets_ids": []any{tokenId},
		"type":       "market",
...
func (this *Polymarket) watchTickerBody(ch chan any, outcome any, optionalArgs ...any) any {
	...
	var tokenId *string = this.SafeString(outcomeObj, "outcomeId")
	outcome = ccxt.DerefScalar(this.SafeString(outcomeObj, "outcome"))
	var messageHash *string = ccxt.SafeStringPtr(ccxt.Add("ticker::", outcome))
	var subscribeHash *string = ccxt.SafeStringPtr(ccxt.Add("subscribe::", tokenId))
	var subscribeMsg map[string]any = map[string]any{
		"assets_ids": []any{tokenId},
		"type":       "market",
...
func (this *Polymarket) FetchClosedOrders(params map[string]any, options ...FetchClosedOrdersOptions) ([]ccxt.PredictionOrder, error) {
...
func (this *Polymarket) FetchOrders(params map[string]any, options ...FetchOrdersOptions) ([]ccxt.PredictionOrder, error) {
...
func (this *Polymarket) WatchPositions(params map[string]any, options ...WatchPositionsOptions) ([]ccxt.PredictionPosition, error) {
