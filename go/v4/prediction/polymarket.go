...
func (this *Polymarket) fetchMarketsBody(ch chan any, optionalArgs ...any) any {
	...
	for rei := 0; rei < ccxt.GetArrayLength(rawEvents); rei++ {
...
func (this *Polymarket) fetchRawEventsListBody(ch chan any, optionalArgs ...any) any {
	...
	for fi := 0; fi < firstPageLength; fi++ {
		allRawEvents = append(allRawEvents, ccxt.GetValue(firstPage, fi))
	}
	if ccxt.IsGreaterThanOrEqual(firstPageLength, pageSize) {
		var offsets []any = []any{}
		for p := 1; ccxt.IsLessThan(p, maxPages); p++ {
			offsets = append(offsets, ccxt.Multiply(p, pageSize))
		}
		var restPromises []any = []any{}
		...
		for ri := 0; ri < ccxt.GetArrayLength(restPages); ri++ {
			...
			for pi := 0; pi < pageLength; pi++ {
				allRawEvents = append(allRawEvents, ccxt.GetValue(page, pi))
			}
		}
	}
		...
		if rawMarketsLength > 0 {
			...
			for i := 0; i < ccxtMarketsLength; i++ {
				...
				if ccxt.IsEqual(mkt, nil) {
					panic(ccxt.ExchangeError(this.Id + " fetchOutcome() could not resolve mkt"))
				}
...
func (this *Polymarket) fetchOutcomesBody(ch chan any, outcomeSymbols any) any {
	...
	var tokenIdsLength int = len(tokenIds)
	if tokenIdsLength > 0 {
		if this.Markets == nil {
			this.Markets = this.CreateSafeDictionary()
		}
		...
		for ccxt.IsLessThan(startIndex, tokenIdsLength) {
			...
			for i := int(ccxt.ParseInt(startIndex)); ccxt.IsLessThan(i, endIndex); i++ {
				chunk = append(chunk, ccxt.GetValue(tokenIds, i))
			}
			...
			for i := 0; i < ccxt.GetArrayLength(ccxtMarkets); i++ {
				var mkt any = ccxt.GetValue(ccxtMarkets, i)
				if ccxt.IsEqual(mkt, nil) {
					panic(ccxt.ExchangeError(this.Id + " fetchOutcomes() could not resolve mkt"))
				}
				ccxt.AddElementToObject(this.Markets, ccxt.GetValue(mkt, "market"), mkt)
			}
...
func (this *Polymarket) fetchTickersBody(ch chan any, optionalArgs ...any) any {
	...
	for ccxt.IsLessThan(startIndex, tokenIdsLength) {
		...
		for i := int(ccxt.ParseInt(startIndex)); ccxt.IsLessThan(i, endIndex); i++ {
			bookParams = append(bookParams, map[string]any{
...
func (this *Polymarket) fetchOHLCVBody(ch chan any, outcome any, optionalArgs ...any) any {
	...
	for i := 0; i < ccxt.GetArrayLength(history); i++ {
		...
		if !(func() bool { _, ok := buckets[bucketKey]; return ok }()) {
			buckets[bucketKey] = []any{snappedMs, price, price, price, price, vol}
		} else {
			var candle any = buckets[bucketKey]
...
func (this *Polymarket) fetchPositionsBody(ch chan any, optionalArgs ...any) any {
	...
	if ccxt.IsEqual(this.WalletAddress, nil) {
		panic(ccxt.ArgumentsRequired(this.Id + " walletAddress is required to fetchPositions"))
	}
	var request map[string]any = map[string]any{
...
func (this *Polymarket) createOrdersBody(ch chan any, orders any, optionalArgs ...any) any {
	...
	for i := 0; i < ccxt.GetArrayLength(orders); i++ {
		...
		bodies = append(bodies, this.SafeDict(built, "body", map[string]any{}))
		outcomes = append(outcomes, this.SafeDict(built, "outcome", map[string]any{}))
...
func (this *Polymarket) createMarketBuyOrderWithCostBody(ch chan any, outcome any, cost any, optionalArgs ...any) any {
	...
	retRes220215 := (<-this.CreateOrderAsync(outcome, "market", "buy", cost, nil, request))
	ccxt.PanicOnError(retRes220215)
	ch <- retRes220215
	return nil
}
...
func (this *Polymarket) cancelOrderBody(ch chan any, id any, optionalArgs ...any) any {
	...
	retRes23428 := (<-this.LoadApiCredentialsAsync())
...
func (this *Polymarket) cancelOrdersBody(ch chan any, ids any, optionalArgs ...any) any {
	...
	retRes23658 := (<-this.LoadApiCredentialsAsync())
...
func (this *Polymarket) cancelAllOrdersBody(ch chan any, optionalArgs ...any) any {
	...
	retRes23878 := (<-this.LoadApiCredentialsAsync())
	ccxt.PanicOnError(retRes23878)
...
func (this *Polymarket) loadApiCredentialsBody(ch chan any) any {
	...
	if !ccxt.IsEqual(this.PrivateKey, nil) {
		...
		if alreadyDerived == nil {
			...
			retRes295316 := (<-this.CreateOrDeriveApiKeyAsync())
			ccxt.PanicOnError(retRes295316)
		}
	...
	panic(ccxt.AuthenticationError(this.Id + " requires L2 api credentials (apiKey, secret, password) or a privateKey to derive them"))
}
...
func (this *Polymarket) watchTickerBody(ch chan any, outcome any, optionalArgs ...any) any {
	...
	if ccxt.IsEqual(outcome, nil) {
		panic(ccxt.ExchangeError(this.Id + " watchTicker() missing outcome"))
	}
	if !(ccxt.InOp(this.Orderbooks, outcome)) {
		var seededBook any = this.OrderBook(map[string]any{})
		if !ccxt.IsEqual(outcome, nil) {
	...
	if (!ccxt.IsEqual(bids, nil)) && (bidsLength > 0) {
	...
	if (!ccxt.IsEqual(asks, nil)) && (asksLength > 0) {
...
func (this *Polymarket) watchOrdersBody(ch chan any, optionalArgs ...any) any {
	...
	retRes32548 := (<-this.LoadApiCredentialsAsync())
	ccxt.PanicOnError(retRes32548)
...
func (this *Polymarket) watchMyTradesBody(ch chan any, optionalArgs ...any) any {
	...
	retRes32808 := (<-this.LoadApiCredentialsAsync())
...
func (this *Polymarket) subscribeUserChannelBody(ch chan any, messageHash any, optionalArgs ...any) any {
	...
	retRes330415 := (<-this.Watch(url, messageHash, this.Extend(subscribeMsg, params), subscribeHash))
	ccxt.PanicOnError(retRes330415)
	ch <- retRes330415
	return nil
}
