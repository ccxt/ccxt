...
func TestOrderRouterRetryFailedStepsUsesANewClientOrderIdAndNeverRetriesAnUnknownOutcome(t *testing.T) {
	...
	first := routerStringAt(relents.paramsSeen[0], "clientOrderId", "")
	second := routerStringAt(relents.paramsSeen[1], "clientOrderId", "")
	if first == second {
		t.Fatalf("a retry must not reuse the rejected order id, got %v", first)
	}
	if strings.Index(second, "-r1") == -1 {
		t.Fatalf("the retry is marked as such, got %v", second)
	}
