...
func (this *BinanceCore) fetchCurrenciesBody(ch chan any, optionalArgs ...any) any {
	...
	if !IsEqual(fetchCurrenciesEnabled, true) {
...
func (this *BinanceCore) ParseCurrency(rawCurrency any) any {
	...
	for j := 0; IsLessThan(j, GetArrayLength(networkList)); j++ {
		...
		if (IsEqual(isDefault, true)) || (IsEqual(fee, nil)) {
			fee = withdrawFee
		}
		...
		if IsEqual(withdrawPrecision, nil) && (IsEqual(isFiat, true)) {
...
func (this *BinanceCore) fetchMarketsBody(ch chan any, optionalArgs ...any) any {
	...
	var isDemoEnv bool = (demoMode == true) || (sandboxMode == true)
	...
	for i := 0; IsLessThan(i, GetArrayLength(fetchMarkets)); i++ {
		...
		if IsEqual(marketType, "spot") {
			...
			if (fetchMargins == true) && EvalTruthy(this.CheckRequiredCredentials(false)) && (isDemoEnv != true) {
	...
	for i := 0; IsLessThan(i, GetArrayLength(results)); i++ {
		...
		if (fetchMargins == true) && IsArray(res) {
...
func (this *BinanceCore) fetchBalanceBody(ch chan any, optionalArgs ...any) any {
	...
	var response any = nil
	var request map[string]any = map[string]any{}
	if EvalTruthy(isPortfolioMargin) || (typeVar == "papi") {
		if EvalTruthy(this.IsLinear(typeVar, subType)) {
			typeVar = "linear"
		} else if EvalTruthy(this.IsInverse(typeVar, subType)) {
			typeVar = "inverse"
		...
		PanicOnError(response)
	} else if (typeVar == "margin") || (IsEqual(marginMode, "cross")) {

		response = (<-this.SapiGetMarginAccount(this.Extend(request, query)))
		PanicOnError(response)
	} else if typeVar == "savings" {
...
func (this *BinanceCore) fetchBidsAsksBody(ch chan any, optionalArgs ...any) any {
	...
	params = GetValue(subTypeparamsVariable, 1)
	var request map[string]any = map[string]any{}
	if (!IsEqual(symbols, nil)) && (EvalTruthy(this.IsLinear(typeVar, subType)) || EvalTruthy(this.IsInverse(typeVar, subType))) {
...
func (this *BinanceCore) ParseOHLCV(ohlcv any, optionalArgs ...any) any {
	...
	var volumeIndex any = Ternary((IsEqual(inverse, true)), 7, 5)
...
func (this *BinanceCore) fetchTradesBody(ch chan any, symbol any, optionalArgs ...any) any {
	...
	if !IsEqual(limit, nil) {
		...
		AddElementToObject(request, "limit", Ternary((isFutureOrSwap == true), mathMin(limit, maxLimitForContractHistorical), limit)) // default = 500, maximum = 1000
	}
...
func (this *BinanceCore) createOrderBody(ch chan any, symbol any, typeVar any, side any, amount any, optionalArgs ...any) any {
	...
	} else if (marketType != nil && *marketType == "margin") || (marginMode != nil) || (isPortfolioMargin == true) {
		if isPortfolioMargin == true {
...
func (this *BinanceCore) CreateOrderRequest(symbol any, typeVar any, side any, amount any, optionalArgs ...any) any {
	...
	if reduceOnly == true {
		if (marketType != nil && *marketType == "margin") || ((!IsEqual(GetValue(market, "contract"), true)) && (!IsEqual(marginMode, nil))) {
	...
	if isTrailingPercentOrder {
		...
		if isMarketOrder {
			...
			uppercaseType = Ternary((IsEqual(GetValue(market, "contract"), true)), "STOP_MARKET", "STOP_LOSS")
		} else if isLimitOrder {
			uppercaseType = Ternary((IsEqual(GetValue(market, "contract"), true)), "STOP", "STOP_LOSS_LIMIT")
		}
	} else if isTakeProfit {
		stopPrice = takeProfitPrice
		if isMarketOrder {
			// spot TAKE_PROFIT market orders are not a valid order type
			uppercaseType = Ternary((IsEqual(GetValue(market, "contract"), true)), "TAKE_PROFIT_MARKET", "TAKE_PROFIT")
		} else if isLimitOrder {
			uppercaseType = Ternary((IsEqual(GetValue(market, "contract"), true)), "TAKE_PROFIT", "TAKE_PROFIT_LIMIT")
		}
	}
	if IsEqual(GetValue(market, "option"), true) {
		if typeVar == "market" {
	...
	if (IsEqual(GetValue(market, "linear"), true)) && (IsEqual(GetValue(market, "swap"), true)) && isConditional && !EvalTruthy(isPortfolioMargin) {
	...
	if clientOrderId == nil {
		...
		var defaultId any = Ternary((IsEqual(GetValue(market, "contract"), true)), "x-xcKtGhcu", "x-TKT5PX2F")
		var idMarketType any = "spot"
		if IsEqual(GetValue(market, "contract"), true) {
			var isLinearSwap bool = (IsEqual(GetValue(market, "swap"), true)) && (IsEqual(GetValue(market, "linear"), true))
			idMarketType = Ternary(isLinearSwap, "swap", "inverse")
		}
	...
	if !EvalTruthy(isPortfolioMargin) {
		...
		if (IsEqual(GetValue(market, "spot"), true)) || (marketType != nil && *marketType == "margin") {
	...
	if ((marketType != nil && *marketType == "spot") || (marketType != nil && *marketType == "margin")) && !EvalTruthy(isPortfolioMargin) && (stock != true) {
	...
	if triggerPriceIsRequired {
		...
		if !IsEqual(stopPrice, nil) {
			if (IsEqual(GetValue(market, "swap"), true)) && !EvalTruthy(isPortfolioMargin) {
	...
	if !EvalTruthy(isPortfolioMargin) && (IsEqual(GetValue(market, "contract"), true)) && EvalTruthy(postOnly) {
		AddElementToObject(request, "timeInForce", "GTX")
	}
	// remove timeInForce from params because PO is only used by this.isPostOnly and it's not a valid value for Binance
	if IsEqual(this.SafeString(params, "timeInForce"), "PO") {
...
func (this *BinanceCore) fetchOrderBody(ch chan any, id any, optionalArgs ...any) any {
	...
	var isOptionType bool = (typeVar == "option")
	var isLinearType any = this.IsLinear(typeVar, subType)
	var isInverseType any = this.IsInverse(typeVar, subType)
	var isLinearSwapConditional bool = EvalTruthy(isLinearType) && (!IsEqual(market, nil)) && (IsEqual(GetValue(market, "swap"), true)) && (IsEqual(isConditional, true)) && (isPortfolioMargin != true)
	...
	} else if (typeVar == "margin") || (!IsEqual(marginMode, nil)) || EvalTruthy(isPortfolioMargin) {
		if EvalTruthy(isPortfolioMargin) {
...
func (this *BinanceCore) fetchOrdersBody(ch chan any, optionalArgs ...any) any {
	...
	if isOptionType {
		...
		if EvalTruthy(isPortfolioMargin) {
			...
			PanicOnError(response)
		} else if (typeVar == "margin") || !IsEqual(marginMode, nil) {
			if IsEqual(marginMode, "isolated") {
				AddElementToObject(request, "isIsolated", true)
			}
...
func (this *BinanceCore) fetchOpenOrdersBody(ch chan any, optionalArgs ...any) any {
	...
	if !IsEqual(symbol, nil) {
		...
		if (IsEqual(optValue, true)) || (IsEqual(optValue, nil) && (IsEqual(warnWithoutSymbol, true))) {
...
func (this *BinanceCore) fetchOpenOrderBody(ch chan any, id any, optionalArgs ...any) any {
	...
	var orderIdRequest any = Ternary((IsEqual(isPortfolioMarginConditional, true)), "strategyId", "orderId")
...
func (this *BinanceCore) cancelOrderBody(ch chan any, id any, optionalArgs ...any) any {
	...
	var isLinearType any = this.IsLinear(typeVar, subType)
	var isInverseType any = this.IsInverse(typeVar, subType)
	var isSwapConditional bool = (!IsEqual(market, nil)) && (IsEqual(GetValue(market, "swap"), true)) && (IsEqual(isConditional, true)) && (isPortfolioMargin != true)
	...
	if clientOrderId != nil {
		if isOptionType {
			...
			if EvalTruthy(isPortfolioMargin) && (IsEqual(isConditional, true)) {
...
func (this *BinanceCore) fetchMyTradesBody(ch chan any, optionalArgs ...any) any {
	...
	typeVar = GetValue(typeVarparamsVariable, 0)
	params = GetValue(typeVarparamsVariable, 1)
	if (stock != true) && (typeVar != "option") && (IsEqual(symbol, nil)) {
	...
	if !IsEqual(limit, nil) {
		if (typeVar == "option") || (IsEqual(this.SafeBool(market, "contract"), true)) {
...
func (this *BinanceCore) ParseDustTrade(trade any, optionalArgs ...any) any {
	...
	if !IsEqual(costString, nil) {
		if (!IsEqual(amountString, nil)) && (amountString != "") {
...
func (this *BinanceCore) fetchDepositsBody(ch chan any, optionalArgs ...any) any {
	...
	if (fiatOnly == true) || ((!IsEqual(code, nil)) && (InOp(legalMoney, code))) {
		if !IsEqual(code, nil) {
...
func (this *BinanceCore) fetchTransfersBody(ch chan any, optionalArgs ...any) any {
	...
	paginate = GetValue(paginateparamsVariable, 0)
	params = GetValue(paginateparamsVariable, 1)
	if EvalTruthy(paginate) && (!IsEqual(internal, true)) {
...
func (this *BinanceCore) fetchTradingFeesBody(ch chan any, optionalArgs ...any) any {
	...
	subType = GetValue(subTypeparamsVariable, 0)
	params = GetValue(subTypeparamsVariable, 1)
	var isSpotOrMargin bool = (typeVar == "spot") || (typeVar == "margin")
	var isLinear any = this.IsLinear(typeVar, subType)
	var isInverse any = this.IsInverse(typeVar, subType)
	var response any = nil
	if isSpotOrMargin {
...
func (this *BinanceCore) ParseAccountPosition(position any, optionalArgs ...any) any {
	...
	if Precise.StringEquals(notionalString, "0") {
		...
		if !IsEqual(truncatedLiquidationPrice, nil) && (GetValue(truncatedLiquidationPrice, 0) == "-") {
...
func (this *BinanceCore) Sign(path any, optionalArgs ...any) any {
	...
	if userDataStream {
		...
		if (IsGreaterThan(GetIndexOf(url, "testnet.binancefuture.com"), OpNeg(1))) && EvalTruthy(this.IsSandboxModeEnabled) && (!IsEqual(this.SafeBool(this.Options, "disableFuturesSandboxWarning"), true)) {
...
func (this *BinanceCore) fetchOpenInterestHistoryBody(ch chan any, symbol any, optionalArgs ...any) any {
	...
	var symbolKey any = Ternary((IsEqual(GetValue(market, "linear"), true)), "symbol", "pair")
	...
	if (endTime != nil) && (endTime == nil || *endTime != 0) {
		AddElementToObject(request, "endTime", endTime)
	} else if (!IsEqual(since, nil)) && (since != 0) {
		if IsEqual(limit, nil) {
...
func (this *BinanceCore) ParseOpenInterest(interest any, optionalArgs ...any) any {
	...
	var baseVolume any = Ternary(isInverse, nil, amount)
...
func (this *BinanceCore) fetchMyLiquidationsBody(ch chan any, optionalArgs ...any) any {
	...
	if !IsEqual(market, nil) {
		var symbolKey any = Ternary((IsEqual(GetValue(market, "spot"), true)), "isolatedSymbol", "symbol")
		if !EvalTruthy(isPortfolioMargin) {
			AddElementToObject(request, symbolKey, GetValue(market, "id"))
		}
	}
