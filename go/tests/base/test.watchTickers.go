...
func TestWatchTickersAsync(exchange ccxt.ICoreExchange, skippedProperties any, symbol any) <-chan any {
	ch := make(chan any, 1)
	...
	for IsTrue((IsLessThan(now, ends))) && !IsTrue(idle) {
...

		{
			func() (ret_ any) {
				...
				response = (UnWrapType(<-exchange.WatchTickersAsync(argSymbols, argParams)))
