...
func TestFetchOrdersAsync(exchange ccxt.ICoreExchange, skippedProperties any, symbol any) <-chan any {
	ch := make(chan any, 1)
...
func testFetchOrdersBody(ch chan any, exchange ccxt.ICoreExchange, skippedProperties any, symbol any) any {
	...
	var method string = "fetchOrders"

	orders := (<-exchange.FetchOrdersAsync(symbol))
	PanicOnError(orders)
