...
func TestFetchPositionsAsync(exchange ccxt.ICoreExchange, skippedProperties any, symbol any) <-chan any {
	ch := make(chan any, 1)
	go testFetchPositionsBody(ch, exchange, skippedProperties, symbol)
	return ch
}
func testFetchPositionsBody(ch chan any, exchange ccxt.ICoreExchange, skippedProperties any, symbol any) any {
	...
	positionsForSymbol := (<-exchange.(ccxt.IFetchPositions).FetchPositionsAsync([]any{symbol}))
