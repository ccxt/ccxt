...
func AssertStructure(exchange ccxt.ICoreExchange, skippedProperties any, method any, entry any, format any, optionalArgs ...any) {
	...
	if IsArray(format) {
		...
		Assert((realLength == expectedLength), Add("entry length is not equal to expected length of "+strconv.Itoa(expectedLength), logText))
...
func AssertTimestamp(exchange ccxt.ICoreExchange, skippedProperties any, method any, entry any, optionalArgs ...any) {
	...
	if !IsEqual(ts, nil) {
		...
		Assert(IsGreaterThan(ts, minTs), Add("timestamp is impossible to be before "+strconv.Itoa(minTs)+" (03.01.2009)", logText)) // 03 Jan 2009 - first block
		Assert(IsLessThan(ts, maxTs), Add("timestamp more than "+strconv.Itoa(maxTs)+" (19.01.2038)", logText))                     // 19 Jan 2038 - int32 overflows // 7258118400000  -> Jan 1 2200
...
func AssertDictionaryResponse(exchange ccxt.ICoreExchange, method any, response any, optionalArgs ...any) {
	...
	var hint *string = GetArgStringPtr(optionalArgs, 0, nil)
	_ = hint
