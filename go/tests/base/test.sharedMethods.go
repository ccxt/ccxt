    ...
    func AssertTimestampAndDatetime(exchange ccxt.ICoreExchange, skippedProperties interface{}, method interface{}, entry interface{}, optionalArgs ...interface{})  {
        ...
        if IsTrue(isDateTimeObject) {
            ...
            if IsTrue(!IsEqual(dt, nil)) {
                ...
                // Allow for 1ms difference in datetime strings due to floating-point precision and truncation
                var isExactMatch interface{} = IsEqual(dtParsedString, dtEntryString)
                var isWithin1ms interface{} = IsLessThanOrEqual(mathAbs(Subtract(exchange.Parse8601(dtParsedString), exchange.Parse8601(dtEntryString))), 1)
                Assert(IsTrue(isExactMatch) || IsTrue(isWithin1ms), Add(Add(Add(Add(Add("datetime is not iso8601 of timestamp:", dtParsedString), "(string) != "), dtEntryString), "(from ts)"), logText))
            }
        }
    }
