    ...
    func AssertTimestampAndDatetime(exchange ccxt.ICoreExchange, skippedProperties interface{}, method interface{}, entry interface{}, optionalArgs ...interface{})  {
        ...
        if IsTrue(isDateTimeObject) {
            ...
            if IsTrue(!IsEqual(dt, nil)) {
                ...
                // so, we have to compare with millisecond accuracy
                var dtParsed interface{} = exchange.Parse8601(dt)
                var timestamp interface{} = GetValue(entry, "timestamp")
                // Allow for 1ms difference due to floating-point precision and processing delays
                var timeDiff interface{} = OpSub(dtParsed, timestamp)
                if timeDiff == nil {
                    timeDiff = 0
                }
                var absDiff interface{} = OpAbs(timeDiff)
                Assert(IsLessThanOrEqual(absDiff, 1), Add(Add(Add(Add(Add("datetime is not iso8601 of timestamp (diff: ", absDiff), "ms): ", exchange.Iso8601(dtParsed)), " != "), exchange.Iso8601(timestamp)), logText))
            }
        }
    }
