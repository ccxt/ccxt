...
func TestTicker(exchange ccxt.ICoreExchange, skippedProperties any, method any, entry any, symbol any) {
	...
	if (!IsEqual(changeString, nil)) && (!IsEqual(open, nil)) && (!IsEqual(close, nil)) && !(InOp(skippedProperties, "compareChange")) {
		...
		if IsGreaterThanOrEqual(changeDecimals, 0) {
			...
			var quantumCap *string = ccxt.Precise.StringDiv(ccxt.Precise.StringAbs(close), "100")
	...
	if (!IsEqual(changeString, nil)) && (!IsEqual(percentageString, nil)) && (!IsEqual(open, nil)) && !(InOp(skippedProperties, "comparePercentage")) {
		...
		var relative *string = ccxt.Precise.StringDiv(ccxt.Precise.StringAbs(derived), "50")
		var allowed *string = ccxt.Precise.StringMax(relative, "0.01")
		var gap *string = ccxt.Precise.StringAbs(ccxt.Precise.StringSub(percentageString, derived))
