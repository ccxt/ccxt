...
func FetchTickersAmountsTest(exchange ccxt.ICoreExchange, skippedProperties any, tickers any) {
	...
	if !(InOp(skippedProperties, "checkActiveSymbols")) {
		...
		Assert(IsGreaterThanOrEqual(obtainedTickersLength, Multiply(notInactiveSymbolsLength, minRatio)), Add(Add(Add(Add(Add(Add(Add(exchange.GetId(), " "), "fetchTickers"), " must return tickers for all active markets. but returned: "), strconv.Itoa(obtainedTickersLength)), " tickers, "), strconv.Itoa(notInactiveSymbolsLength)), " active markets"))
		//
		...
		Assert((obtainedTickersLength <= allMarketsLength), Add(Add(Add(Add(Add(Add(Add(exchange.GetId(), " "), "fetchTickers"), " must return <= than all markets, but returned: "), strconv.Itoa(obtainedTickersLength)), " tickers, "), strconv.Itoa(allMarketsLength)), " markets"))
	}
}
