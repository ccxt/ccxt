...
func TestWatchPositionsAsync(exchange ccxt.ICoreExchange, skippedProperties any, symbol any) <-chan any {
	ch := make(chan any, 1)
	...
	for IsLessThan(now, ends) {
...

		{
			func() (ret_ any) {
				...
				response = (UnWrapType(<-exchange.WatchPositionsAsync([]any{symbol})))
...

		{
			func() (ret_ any) {
				...
				positionsForSymbols = (UnWrapType(<-exchange.WatchPositionsAsync([]any{symbol})))
