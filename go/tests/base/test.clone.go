...
func TestClone() {
	...
	assert((len(ccxt.ObjectKeys(emptyClone)) == 0), "clone C: cloning empty object gives empty object")
	...
	assert(!(func() bool { _, ok := masterOrig["d"]; return ok }()), "clone E: extra key must not appear in original")
