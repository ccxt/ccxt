...
func TestFeaturesAsync(exchange ccxt.ICoreExchange, skippedProperties any) <-chan any {
	ch := make(chan any, 1)
...
func testFeaturesBody(ch chan any, exchange ccxt.ICoreExchange, skippedProperties any) any {
	...
	for i := 0; IsLessThan(i, GetArrayLength(keys)); i++ {
		...
		var marketType string = GetValue(keys, i).(string)
		...
		if marketType == "spot" {
			...
			for j := 0; IsLessThan(j, GetArrayLength(subKeys)); j++ {
				var subKey string = GetValue(subKeys, j).(string)
