...
func testFetchCurrenciesBody(ch chan any, exchange ccxt.ICoreExchange, skippedProperties any) any {
	...
	if isFetchCurrenciesPrivate != true {
		...
		Assert(skipAmount || (currenciesLength > 5), Add(Add(Add(Add(exchange.GetId(), " "), method), " must return at least several currencies, but it returned "), strconv.Itoa(currenciesLength)))
