...
class kalshi extends kalshi$1["default"] {
    ...
    async fetchOutcome(outcomeSymbol) {
        ...
        try {
            ...
            if (e instanceof errors.BadSymbol) {
                throw new errors.BadSymbol(this.id + ' could not resolve outcome ' + outcomeSymbol + ' — pass an outcomeId, or call fetchEvents ()/loadOutcomes () first');
            }
            throw e;
        }
    ...
    async createOrder(outcome, type, side, amount, price = undefined, params = {}) {
        ...
        const order = this.parseOrder(response, outcomeObj);
        order['side'] = side;
        order['amount'] = amount;
        order['price'] = price;
    ...
    async fetchEvents(params = {}) {
        ...
        let page = 0;
        while (page < maxPages) {
            const request = { 'status': status, 'limit': pageLimit, 'with_nested_markets': false };
            if (cursor) {
                request['cursor'] = cursor;
            }
