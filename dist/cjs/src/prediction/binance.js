...
var binance$1 = require('../abstract/prediction/binance.js');
...
class binance extends binance$1["default"] {
        ...
        return this.deepExtend(super.describe(), {
            ...
            'exceptions': {
                'exact': {
                    ...
                    '-1021': errors.InvalidNonce, // timestamp outside recvWindow
                    '-1022': errors.AuthenticationError, // invalid signature
                    '-1102': errors.BadRequest, // mandatory parameter missing
                    '-1121': errors.BadSymbol,
                    '-2008': errors.AuthenticationError, // invalid api-key id
                    '-2014': errors.AuthenticationError, // api-key format invalid
                    '-2015': errors.PermissionDenied, // invalid key, ip or permissions
    ...
    parseTopicMarket(rawMarket, rawTopic) {
        ...
        for (let oi = 0; oi < rawOutcomesLength; oi++) {
            ...
            if (resolved && (price !== undefined)) {
                winnerRaw = Precise["default"].stringEq(price, '1');
    ...
    parsePredictionTicker(raw, market = undefined) {
        ...
        if (lastString !== undefined) {
            if (isMirrored) {
                last = this.parseNumber(Precise["default"].stringSub('1', lastString));
            }
            else {
     ...
     */
    parsePredictionOrder(order, outcomeObj = undefined) {
        //
        // {
        //     "orderId": "54124",
     ...
     */
    parsePredictionTrade(trade, outcomeObj = undefined) {
        //
        // {
        //     "orderId": "54124",
        //     "vendorOrderId": "0x1234abcd...",
        ...
        if ((orderType === 'market') && (cost !== undefined) && (price !== undefined) && (filled !== undefined)) {
            ...
            const feeCost = Precise["default"].stringAbs(Precise["default"].stringSub(cost, Precise["default"].stringMul(price, filled)));
            fee = {
                'currency': 'USDT',
     ...
     */
    async fetchWallet(methodName, params = {}) {
        ...
        if (cachedWallet === undefined) {
            throw new errors.NotSupported(this.id + 'fetchWallet could\'n find wallet ' + walletAddress);
        }
        this.options['wallet'] = cachedWallet;
        return cachedWallet;
    }
    ...
    priceToPrecision(outcome, price) {
        ...
        return this.decimalToPrecision(price, number.ROUND, decimals, number.DECIMAL_PLACES, this.paddingMode);
    }
    amountToPrecision(outcome, amount) {
        const market = this.market(outcome);
        const prec = this.safeNumber(this.safeDict(market, 'precision', {}), 'amount', 0.01);
        let decimals = 2;
        if ((prec !== undefined) && (prec > 0)) {
            decimals = this.precisionFromString(this.numberToString(prec));
        }
        // amounts truncate so a rounded-up value can never exceed the caller's balance
        return this.decimalToPrecision(amount, number.TRUNCATE, decimals, number.DECIMAL_PLACES, this.paddingMode);
    }
    /**
     * @method
    ...
    async createOrder(outcome, type, side, amount, price = undefined, params = {}) {
        ...
        const slippageBps = this.parseToInt(Precise["default"].stringMul(slippage, '10000'));
        const commonRequest = {
            'walletAddress': wallet['walletAddress'],
            'orderType': typeUpper,
        ...
        if (typeUpper === 'LIMIT') {
            if (price === undefined) {
                throw new errors.ArgumentsRequired(this.id + 'createOrder requires price for limit order');
            }
        ...
        if (sideUpper === 'BUY') {
            if (cost !== undefined) {
                ...
                if (typeUpper === 'LIMIT') {
                    ...
                    if (price === undefined) {
                        throw new errors.ArgumentsRequired(this.id + ' createOrder requires price for ' + side + ' order');
                    }
                }
                const feeRate = Precise["default"].stringDiv(feeRateBps, '10000');
                const minPrice = Precise["default"].stringMin(priceStr, Precise["default"].stringSub('1', priceStr));
                const fee = Precise["default"].stringMul(Precise["default"].stringMul(feeRate, minPrice), amountStr);
                amountStr = Precise["default"].stringAdd(Precise["default"].stringMul(amountStr, priceStr), fee);
            }
        }
    ...
    async cancelOrders(ids, outcome = undefined, params = {}) {
        ...
        for (let i = 0; i < failedOrdersLength; i++) {
            ...
            throw new errors.OrderNotFound(this.id + ' cancelOrders() failed for ' + this.safeString(failedOrder, 'orderId') + ': ' + error);
        }
        const orders = [];
    ...
    handleErrors(code, reason, url, method, headers, body, response, requestHeaders, requestBody) {
        ...
        if ((errorCode !== undefined) && Precise["default"].stringLt(errorCode, '0')) {
