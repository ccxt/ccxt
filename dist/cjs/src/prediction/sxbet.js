...
var sxbet$1 = require('../abstract/prediction/sxbet.js');
 ...
 * @augments Exchange
 */
class sxbet extends sxbet$1["default"] {
    describe() {
        return this.deepExtend(super.describe(), {
            ...
            'exceptions': {
                ...
                'exact': {
                    'INSUFFICIENT_BALANCE': errors.InsufficientFunds, // maker token balance too low
                    'INVALID_MARKET': errors.BadRequest, // non-unique marketHashes in one request
                    'ORDERS_ALREADY_EXIST': errors.DuplicateOrderId,
                    'TOO_MANY_DIFFERENT_MARKETS': errors.BadRequest, // more than 3 markets in a single request
                    'ORDERS_MUST_HAVE_IDENTICAL_MARKET': errors.BadRequest,
                    'BAD_BASE_TOKEN': errors.BadRequest, // all orders must share one base token
                    'INSUFFICIENT_KYC': errors.PermissionDenied,
                    'PROXY_NOT_DEPLOYED': errors.AccountNotEnabled, // live-verified v3: the account's obv3 proxy wallet is not deployed yet - call approve() (deploys and funds it)
                    'INVALID_USER': errors.AuthenticationError, // live-verified v3: the x-sx-api-key does not belong to a registered user on the target network
                    'BAD_AUTH': errors.AuthenticationError, // live-verified v3: missing or malformed x-sx-api-key
                    'ERC20_PERMIT_BAD_SPENDER': errors.BadRequest, // live-verified v3: transfer-to-proxy permit signed for the wrong executor
                    'BASE_TOKENS_NOT_SAME': errors.BadRequest,
                    'MARKETS_NOT_SAME': errors.BadRequest,
                    'DIRECTIONS_NOT_SAME': errors.BadRequest,
                    'INVALID_ORDERS': errors.OrderNotFillable, // a targeted order is no longer active
                    'INVALID_ODDS': errors.InvalidOrder, // desiredOdds must be below 10^20
                    'PERCENTAGEODDS_UNDEFINED_OR_MALFORMED': errors.InvalidOrder, // live-verified: maker percentageOdds above the 10^20 cap (price > 1) or malformed
                    'TOTAL_BET_SIZE_TOO_LOW': errors.InvalidOrder, // live-verified: stake below the venue's minimum bet size
                    'MATCH_STATE_INVALID': errors.MarketClosed, // fixture not available for betting
                    'PROXY_ACCOUNT_INVALID': errors.BadRequest,
                    'RATE_LIMIT_ORDER_REQUEST_MARKET_COUNT': errors.BadRequest, // more than 1000 marketHashes queried
                    'BOTH_SPORTXEVENTID_MARKETHASHES_PRESENT': errors.BadRequest,
                    'BAD_MARKET_HASHES': errors.BadRequest, // invalid or more than 30 hashes on /markets/find
                    'Order not found': errors.OrderNotFound, // live-verified v3: GET /orders-v3/{orderId} 404s for a missing or foreign id
                },
                'broad': {
                    'no longer supported': errors.NotSupported, // e.g. "OrderBook V2 is no longer supported" (testnet V3 canary gate)
                    'must be': errors.BadRequest, // request validation messages like "orders must be an array"
                    'Insufficient available balance': errors.InsufficientFunds, // live-verified v3: the proxy wallet holds less than the order size
                },
            },
            'options': {
    ...
    async fetchEvent(id, params = {}) {
        ...
        if (rawMarketsLength === 0) {
            throw new errors.BadSymbol(this.id + ' fetchEvent() could not find an active fixture ' + id);
        }
    ...
    parseEvent(fixtureId, rawMarkets) {
        ...
        for (let i = 0; i < rawMarketsLength; i++) {
            ...
            if (parsed === undefined) {
                throw new errors.ExchangeError(this.id + ' parseEvent() could not resolve parsed market');
            }
    ...
    roundOddsToLadder(probability, oddsLadderStepSize) {
        const tickSize = Precise["default"].stringDiv(oddsLadderStepSize, '100000', 10);
        return this.decimalToPrecision(probability, number.ROUND, tickSize, number.TICK_SIZE);
    }
    /**
     * @ignore
     * @method
    ...
    hashEip712Digest(encoded) {
        return '0x' + this.hash(encoded, sha3_js.keccak_256, 'hex');
    }
    /**
     * @ignore
     * @method
    ...
    signDigest(digest, privateKey) {
        const signature = crypto.ecdsa(digest.slice(-64), privateKey.slice(-64), secp256k1_js.secp256k1, undefined);
    ...
    async approve(params = {}) {
        ...
        if (amount === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' approve() requires params.amount - the USDC amount to move into the proxy wallet');
        }
        ...
        if (tokenAddress === undefined) {
            throw new errors.BadRequest(this.id + ' approve() could not resolve the base token address from /metadata/obv3');
        }
        ...
        if (spender === undefined) {
            throw new errors.BadRequest(this.id + ' approve() could not resolve the transfer-to-proxy executor from /metadata/obv3 - pass params.spender');
        }
        ...
        if (rpcUrl === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' approve() has no RPC endpoint configured for chainId ' + this.numberToString(chainId) + ' - pass params.rpcUrl');
        }
        ...
        const value = this.decimalToPrecision(Precise["default"].stringMul(this.numberToString(amount), '1000000'), number.ROUND, 0, number.DECIMAL_PLACES);
    ...
    async createOrder(outcome, type, side, amount, price = undefined, params = {}) {
        ...
        if ((type !== 'limit') && (type !== 'market')) {
            throw new errors.InvalidOrder(this.id + " createOrder() type must be 'limit' or 'market', got " + type);
        }
        if (price === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' createOrder() requires a price - the implied probability of the requested outcome');
        }
        ...
        if (postOnly === true) {
            throw new errors.NotSupported(this.id + ' createOrder() does not support postOnly - GTC orders may cross on entry');
        }
        const triggerPrice = this.safeStringN(params, ['triggerPrice', 'stopLossPrice', 'takeProfitPrice']);
        if (triggerPrice !== undefined) {
            throw new errors.NotSupported(this.id + ' createOrder() does not support trigger, stop-loss or take-profit orders');
        }
        ...
        const probability = (isBuy) ? priceStr : Precise["default"].stringSub('1', priceStr);
        const obv3 = await this.loadSxObv3Metadata();
        ...
        const percentageOdds = this.decimalToPrecision(Precise["default"].stringMul(roundedProbability, '100000000000000000000'), number.ROUND, 0, number.DECIMAL_PLACES);
        const amountStr = this.numberToString(amount);
        const totalBetSize = this.decimalToPrecision(Precise["default"].stringMul(amountStr, '1000000'), number.ROUND, 0, number.DECIMAL_PLACES);
        ...
        if ((type === 'market') && (timeInForce === 'GTC')) {
            throw new errors.InvalidOrder(this.id + " createOrder() market orders cannot be GTC - use type 'limit' for a resting order");
        }
        ...
        if (matchOutcome !== undefined) {
            ...
            if (fillAmountRaw !== undefined) {
                filled = this.parseNumber(Precise["default"].stringDiv(fillAmountRaw, usdcDecimals, 6));
            }
            if (remainingRaw !== undefined) {
                remaining = this.parseNumber(Precise["default"].stringDiv(remainingRaw, usdcDecimals, 6));
            }
    ...
    async cancelOrders(ids, outcome = undefined, params = {}) {
        ...
        if (idsLength === 0) {
            throw new errors.ArgumentsRequired(this.id + ' cancelOrders() requires a non-empty ids argument');
        }
    ...
    parsePredictionOrder(order, market = undefined) {
        ...
        const price = (percentageOdds !== undefined) ? this.parseNumber(Precise["default"].stringDiv(percentageOdds, oneDenom)) : undefined;
        ...
        const filledRaw = Precise["default"].stringSub(totalBetSize, remainingSize);
        const amount = this.parseNumber(Precise["default"].stringDiv(totalBetSize, usdcDecimals, 6));
        const filled = this.parseNumber(Precise["default"].stringDiv(filledRaw, usdcDecimals, 6));
        const remaining = this.parseNumber(Precise["default"].stringDiv(remainingSize, usdcDecimals, 6));
    ...
    async fetchTrades(outcome, since = undefined, limit = undefined, params = {}) {
        if (outcome === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' fetchTrades() requires an outcome argument - the venue requires the trades listing to be scoped');
        }
        await this.loadOutcome(outcome);
        const outcomeObj = this.outcome(outcome);
    ...
    parseSxbetV3Fill(fill, market = undefined) {
        ...
        const price = (fillOdds !== undefined) ? this.parseNumber(Precise["default"].stringDiv(fillOdds, oneDenom)) : undefined;
        const fillAmount = this.safeString(fill, 'fillAmount', '0');
        const amount = this.parseNumber(Precise["default"].stringDiv(fillAmount, usdcDecimals, 6));
    ...
    async fetchBalance(params = {}) {
        ...
        for (let i = 0; i < balancesLength; i++) {
            ...
            const free = Precise["default"].stringDiv(this.safeString(row, 'availableAmount', '0'), usdcDecimals, 6);
            const used = Precise["default"].stringDiv(this.safeString(row, 'escrowedAmount', '0'), usdcDecimals, 6);
            result[code] = {
                'free': this.parseNumber(free),
                'used': this.parseNumber(used),
                'total': this.parseNumber(Precise["default"].stringAdd(free, used)),
            };
        }
    ...
    parseSxbetV3Position(raw) {
        ...
        const entryPrice = (ownOdds !== undefined) ? this.parseNumber(Precise["default"].stringDiv(ownOdds, oneDenom)) : undefined;
        ...
        return this.safePredictionPosition({
            ...
            'contracts': this.parseNumber(Precise["default"].stringDiv(totalStake, usdcDecimals, 6)),
            'entryPrice': entryPrice,
            'markPrice': undefined,
            'unrealizedPnl': undefined,
            'realizedPnl': (pnl !== undefined) ? this.parseNumber(Precise["default"].stringDiv(pnl, usdcDecimals, 6)) : undefined,
            'side': 'long',
            'info': raw,
        });
    }
    ...
    parseSettlement(trade, market = undefined) {
        ...
        const usdcDecimals = '1000000';
        const stake = Precise["default"].stringDiv(this.safeString(trade, 'totalStake', '0'), usdcDecimals, 6);
        const payout = Precise["default"].stringDiv(this.safeString(settlement, 'settleReturnAmount', '0'), usdcDecimals, 6);
        const pnl = Precise["default"].stringSub(payout, stake);
    ...
    async fetchTickers(outcomes = undefined, params = {}) {
        if (outcomes === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' fetchTickers() requires an outcomes argument - sx.bet has thousands of markets and serves best odds per market list');
        }
    ...
    parsePredictionTicker(raw, market = undefined) {
        ...
        const bid = (ownPercentage !== undefined) ? this.parseNumber(Precise["default"].stringDiv(ownPercentage, oneDenom)) : undefined;
        const ask = (oppositePercentage !== undefined) ? this.parseNumber(Precise["default"].stringSub('1', Precise["default"].stringDiv(oppositePercentage, oneDenom))) : undefined;
    ...
    parseSxbetV3BookSides(snapshot, isOutcomeOne) {
        ...
        for (let i = 0; i < ownLevelsLength; i++) {
            ...
            const price = this.parseNumber(Precise["default"].stringDiv(percentageOdds, oneDenom));
            const amount = this.parseNumber(Precise["default"].stringDiv(size, usdcDecimals, 6));
        ...
        for (let i = 0; i < oppositeLevelsLength; i++) {
            ...
            const price = this.parseNumber(Precise["default"].stringSub('1', Precise["default"].stringDiv(percentageOdds, oneDenom)));
            const ratio = Precise["default"].stringDiv(oneDenom, percentageOdds, 12);
            const remainingTaker = Precise["default"].stringSub(Precise["default"].stringMul(size, ratio), size);
            const amount = this.parseNumber(Precise["default"].stringDiv(remainingTaker, usdcDecimals, 6));
    ...
    async fetchSxbetRealtimeToken() {
        if (this.apiKey === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' websocket streaming requires the apiKey credential - the realtime token endpoint authenticates with the X-Api-Key header');
        }
    ...
    handleCentrifugoFrame(client, msg) {
        ...
        if (errorReply !== undefined) {
            ...
            const error = new errors.ExchangeError(this.id + ' ' + this.json(msg));
    ...
    parseSxbetV3PublicTrade(trade) {
        ...
        const price = (odds !== undefined) ? this.parseNumber(Precise["default"].stringDiv(odds, oneDenom)) : undefined;
        const stake = this.safeString(trade, 'totalStake', '0');
        const amount = this.parseNumber(Precise["default"].stringDiv(stake, usdcDecimals, 6));
    ...
    async watchMyTrades(outcome = undefined, since = undefined, limit = undefined, params = {}) {
        if (this.walletAddress === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' watchMyTrades() requires a walletAddress');
        }
    ...
    async watchOrders(outcome = undefined, since = undefined, limit = undefined, params = {}) {
        if (this.walletAddress === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' watchOrders() requires a walletAddress');
        }
    ...
    handleOrder(client, rows) {
        ...
        for (let i = 0; i < rowsLength; i++) {
            ...
            if (this.orders === undefined) {
                ...
                this.orders = new Cache.ArrayCacheByOutcomeById(cacheLimit);
            }
    ...
    sign(path, api = 'sxbet', method = 'GET', params = {}, headers = undefined, body = undefined) {
        ...
        if ((accessLevel === 'private') && (this.apiKey === undefined)) {
            throw new errors.AuthenticationError(this.id + ' ' + path + ' is a private endpoint and requires the apiKey credential (the x-sx-api-key header)');
        }
