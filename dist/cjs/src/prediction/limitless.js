...
var limitless$1 = require('../abstract/prediction/limitless.js');
 ...
 * @augments Exchange
 */
class limitless extends limitless$1["default"] {
    describe() {
        return this.deepExtend(super.describe(), {
            ...
            'exceptions': {
                ...
                'broad': {
                    ...
                    'Orders not found': errors.OrderNotFound,
                    'already canceled': errors.OrderNotFound,
                    'must be a UUIDv4': errors.BadRequest,
                    'Invalid signature': errors.InvalidOrder,
                },
            },
        });
    }
    ...
    parsePredictionTicker(ticker, market = undefined) {
        ...
        if (book !== undefined) {
            ...
            if (isYes) {
                ...
                if (yesAskPrice !== undefined) {
                    bidStr = Precise["default"].stringSub('1', yesAskPrice);
                }
                bidSizeStr = yesAskSize;
                if (yesBidPrice !== undefined) {
                    askStr = Precise["default"].stringSub('1', yesBidPrice);
                }
                askSizeStr = yesBidSize;
                if (yesLast !== undefined) {
                    lastStr = Precise["default"].stringSub('1', yesLast);
                }
                if (yesMid !== undefined) {
                    midStr = Precise["default"].stringSub('1', yesMid);
                }
            }
        }
        ...
        if (rawVolume !== undefined) {
            volumeStr = Precise["default"].stringDiv(rawVolume, '1000000');
        }
        if (bidSizeStr !== undefined) {
            bidSizeStr = Precise["default"].stringDiv(bidSizeStr, '1000000');
        }
    ...
    async fetchOrderBook(outcome, limit = undefined, params = {}) {
        ...
        for (let bi = 0; bi < bidsSource.length; bi++) {
            ...
            if (!isYes && (priceStr !== undefined)) {
                priceStr = Precise["default"].stringSub('1', priceStr);
            }
            let sizeStr = this.safeString(bidsSource[bi], 'size');
            if (sizeStr !== undefined) {
                sizeStr = Precise["default"].stringDiv(sizeStr, scaleStr);
            }
    ...
    async fetchOrders(outcome = undefined, since = undefined, limit = undefined, params = {}) {
        if (outcome === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' fetchOrders requires an outcome argument');
        }
        await this.loadOutcome(outcome);
        const outcomeObj = this.outcome(outcome);
    ...
    async fetchOrdersByIds(ids, outcome = undefined, params = {}) {
        ...
        if (length > 50) {
            throw new errors.BadRequest(this.id + ' fetchOrdersByIds can only fetch up to 50 orders at a time');
        }
        const items = [];
    ...
    applyScale(amount, multiply = false) {
        ...
        if (multiply) {
            return Precise["default"].stringMul(amount, scale);
        }
        else {
            return Precise["default"].stringDiv(amount, scale);
        }
    }
    ...
    async createOrder(outcome, type, side, amount, price = undefined, params = {}) {
        ...
        try {
            ...
            throw new errors.InvalidAddress(this.id + ' createOrder requires a valid maker address. Set the "maker" parameter to a valid address or set the "walletAddress" property in the constructor options.');
        }
        ...
        if (isMarket && (side === 'buy')) {
            ...
            const calculatedCost = Precise["default"].stringMul(amountString, priceString);
            if (side === 'buy') {
    ...
    signOrderRequest(signRequest, marketSymbol) {
        ...
        if (this.privateKey === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' createOrder() requires a privateKey (the embedded/trading wallet key) to sign orders');
        }
    ...
    async approve(params = {}) {
        ...
        if (amount !== undefined) {
            ...
            const scaled = Precise["default"].stringDiv(amount, this.parsePrecision(this.numberToString(decimals)));
    ...
    async redeem(outcome = undefined, params = {}) {
        ...
        if (conditionId === undefined) {
            if (outcome === undefined) {
                throw new errors.ArgumentsRequired(this.id + ' redeem() requires an outcome or a params.conditionId');
            }
            await this.loadOutcome(outcome);
            const outcomeObj = this.outcome(outcome);
        ...
        if (conditionId === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' redeem() could not resolve the market conditionId - pass params.conditionId (a bytes32 hex string)');
        }
        const request = {
            'conditionId': conditionId,
        };
    ...
    async cancelOrders(ids, outcome = undefined, params = {}) {
        ...
        if (failedLethgn > 0) {
            ...
            throw new errors.OrderNotFound(feedback);
        }
    ...
    parsePredictionTrade(trade, market = undefined) {
        ...
        if (matchedSize !== undefined) {
            ...
            const amountStr = Precise["default"].stringDiv(matchedSize, '1000000');
            const priceStr = this.safeString(trade, 'price');
            let costStr = undefined;
            if (priceStr !== undefined) {
                costStr = Precise["default"].stringMul(priceStr, amountStr);
            }
    ...
    async fetchRawMarketsByTags(tags, params = {}) {
        ...
        if (categoryIdsLength === 0) {
            throw new errors.BadRequest(this.id + ' fetchEvents() could not match the requested tags to any limitless category — GET /categories lists the valid names');
        }
        const seen = {};
        const allRaw = [];
