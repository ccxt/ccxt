...
var predictfun$1 = require('../abstract/prediction/predictfun.js');
 ...
 * @augments Exchange
 */
class predictfun extends predictfun$1["default"] {
    describe() {
        return this.deepExtend(super.describe(), {
            ...
            'exceptions': {
                'exact': {
                    ...
                    'create_order_hash_mismatch': errors.InvalidOrder,
                    'create_order_insufficient_collateral_allowance': errors.InsufficientFunds,
                    'create_order_insufficient_shares_allowance': errors.InsufficientFunds,
                    'create_order_min_order_value_not_met': errors.InvalidOrder,
                    'create_order_price_out_of_range': errors.InvalidOrder,
                    'create_order_market_not_trading': errors.MarketClosed,
                },
                'broad': {
                    ...
                    'do not belong to': errors.OrderNotFound,
                    'order hash must be a': errors.BadRequest,
                    'Orderbook not found': errors.BadSymbol,
                    'market not found': errors.BadSymbol,
                    'not currently trading': errors.MarketClosed,
                    'allowance is less than': errors.InsufficientFunds,
                    'price per share must be': errors.InvalidOrder,
                    'must have a value of at least': errors.InvalidOrder,
                },
            },
            'options': {
    ...
    async fetchEvent(id, params = {}) {
        ...
        if (slug === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' fetchEvent() requires the "id" argument or the "slug" parameter');
        }
     ...
     */
    async fetchRawTopicsByQueries(queries, params = {}) {
        ...
        for (let i = 0; i < queriesLength; i++) {
            ...
            const rawMarkets = this.safeList(data, 'markets', []);
     ...
     */
    titleForMarketSymbol(topicSlug, title, marketCount = undefined) {
    ...
    parseTopicMarket(rawMarket, rawTopic) {
        ...
        const feeRate = this.parseNumber(Precise["default"].stringDiv(feeRateBps, '10000'));
    ...
    parsePredictionTicker(ticker, market = undefined) {
        ...
        if (!outcomeFound) {
            ...
            throw new errors.BadSymbol(this.id + ' fetchTicker() did not find ' + requested + ' among the outcomes of its market');
        }
    ...
    async fetchMyTrades(outcome = undefined, since = undefined, limit = undefined, params = {}) {
        ...
        if (signerAddress === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' fetchMyTrades() requires a walletAddress, or a "signerAddress" parameter for any other address');
        }
        const request = {
    ...
    parsePredictionTrade(trade, market = undefined) {
        ...
        priceStr = Precise["default"].stringDiv(priceStr, '1000000000000000000');
        let amountStr = this.safeString(party, 'amount');
        amountStr = Precise["default"].stringDiv(amountStr, '1000000000000000000');
        let side = undefined;
        let order = undefined;
        let fee = undefined;
        ...
        if (rawFee !== undefined) {
            ...
            fee = {
                ...
                'cost': this.parseNumber(Precise["default"].stringDiv(feeCost, '1000000000000000000')),
            };
        }
    ...
    async authenticate(params = {}) {
        if ((this.walletAddress === undefined) || (this.privateKey === undefined)) {
            throw new errors.ArgumentsRequired(this.id + ' authenticate() requires a walletAddress and a privateKey');
        }
        ...
        if (message === undefined) {
            throw new errors.AuthenticationError(this.id + ' authenticate() got an auth reply without the "message" field to sign');
        }
        ...
        if (token === undefined) {
            throw new errors.AuthenticationError(this.id + ' authenticate() did not return a token');
        }
     ...
     */
    signPredictfunOrder(order, isNegRisk, isYieldBearing) {
    ...
    async createOrder(outcome, type, side, amount, price = undefined, params = {}) {
        ...
        if (price === undefined) {
            ...
            if (warnOnMarketOrderWithoutPrice) {
                throw new errors.ArgumentsRequired(this.id + ' createOrder() market orders require a "price" argument. To use default values turn "warnOnMarketOrderWithoutPrice" off in options');
            }
        ...
        const quantityWei = Precise["default"].stringMul(amountString, '1000000000000000000');
        const priceWei = Precise["default"].stringMul(priceToProvide, '1000000000000000000');
        ...
        const costWei = Precise["default"].stringMul(priceToProvide, quantityWei);
        ...
        if (Precise["default"].stringGt(slippageBps, '0')) {
            if (isBuy) {
                ...
                makerAmount = Precise["default"].stringMin(Precise["default"].stringDiv(Precise["default"].stringMul(makerAmount, Precise["default"].stringAdd('10000', slippageBps)), '10000'), quantityWei);
            }
            else {
                takerAmount = Precise["default"].stringMax(Precise["default"].stringDiv(Precise["default"].stringMul(takerAmount, Precise["default"].stringSub('10000', slippageBps)), '10000'), '0');
            }
        }
    ...
    parsePredictionPosition(position, market = undefined) {
        ...
        const contracts = Precise["default"].stringDiv(this.safeString(position, 'amount'), '1000000000000000000');
        ...
        const collateral = Precise["default"].stringMul(entryPrice, contracts);
        let markPrice = undefined;
        if ((notional !== undefined) && (contracts !== undefined) && Precise["default"].stringGt(contracts, '0')) {
            markPrice = Precise["default"].stringDiv(notional, contracts);
        }
        let percentage = undefined;
        if ((pnl !== undefined) && (collateral !== undefined) && Precise["default"].stringGt(collateral, '0')) {
            percentage = Precise["default"].stringMul(Precise["default"].stringDiv(pnl, collateral), '100');
        }
    ...
    async cancelOrder(id, outcome = undefined, params = {}) {
        ...
        if (order === undefined) {
            throw new errors.OrderNotFound(this.id + ' cancelOrder() could not remove ' + id);
        }
        return order;
    }
    /**
    ...
    async cancelOrders(ids, outcome = undefined, params = {}) {
        ...
        if (idsLength === 0) {
            throw new errors.ArgumentsRequired(this.id + ' cancelOrders() requires at least one order hash');
        }
        if (idsLength > 100) {
            throw new errors.BadRequest(this.id + ' cancelOrders() takes at most 100 order hashes per call');
        }
        ...
        if (rejectedLength > 0) {
            throw new errors.OrderNotFound(this.id + ' cancelOrders() was refused for ' + this.json(rejected));
        }
     ...
     * @returns {object[]} a list of [order structures](https://docs.ccxt.com/#/?id=order-structure)
     */
    async fetchOrdersHelper(outcome = undefined, since = undefined, limit = undefined, params = {}) {
        let outcomeObj = undefined;
        if (outcome !== undefined) {
            await this.loadOutcome(outcome);
            outcomeObj = this.outcome(outcome);
    ...
    parsePredictionOrder(order, market = undefined) {
        ...
        const amount = Precise["default"].stringDiv(amountWei, '1000000000000000000');
        const cost = Precise["default"].stringDiv(costWei, '1000000000000000000');
        ...
        if (type === 'limit') {
            price = Precise["default"].stringDiv(cost, amount);
        }
        const amountFilled = this.safeString(order, 'amountFilled');
        const filled = Precise["default"].stringDiv(amountFilled, '1000000000000000000');
        const filledCost = Precise["default"].stringMul(filled, price);
        return this.safePredictionOrder({
            'id': orderHash,
            'clientOrderId': undefined,
            'timestamp': undefined,
     ...
     */
    exchangeAddress(isNegRisk, isYieldBearing) {
        const chainIdValue = this.safeInteger(this.options, 'chainId', 56);
     ...
     * @returns {string} the contract address on the active chain
     */
    conditionalTokensAddress(isNegRisk, isYieldBearing) {
        const chainIdValue = this.safeInteger(this.options, 'chainId', 56);
     ...
     */
    async approve(outcome = undefined, params = {}) {
        if (this.privateKey === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' approve() requires a privateKey to sign the on-chain transaction');
        }
        ...
        if (side === 'sell') {
            ...
            if (explicitOperator !== undefined) {
                ...
                throw new errors.ArgumentsRequired(this.id + ' approve() requires an "outcome" to resolve the contracts to approve for selling, or an explicit "spender" parameter');
            }
            else {
            ...
            if ((rpcUrl === undefined) || (ctfToken === undefined)) {
                throw new errors.ArgumentsRequired(this.id + ' approve() could not resolve the "rpcUrl" or the conditional tokens for chain ' + chainKey);
            }
            ...
            for (let i = 0; i < operatorsLength; i++) {
                ...
                if (operatorAddress === undefined) {
                    throw new errors.ArgumentsRequired(this.id + ' approve() could not resolve the operator to approve for chain ' + chainKey);
                }
        ...
        if (spender === undefined) {
            if (outcome === undefined) {
                throw new errors.ArgumentsRequired(this.id + ' approve() requires an "outcome" to resolve the exchange to approve, or an explicit "spender" parameter');
            }
            spender = this.exchangeAddress(isNegRisk, isYieldBearing);
        }
        if ((rpcUrl === undefined) || (token === undefined) || (spender === undefined)) {
            throw new errors.ArgumentsRequired(this.id + ' approve() could not resolve the "rpcUrl", the "token" or the "spender" for chain ' + chainKey);
        }
        ...
        if (amount !== undefined) {
            ...
            const scaled = Precise["default"].stringMul(amount, '1000000000000000000');
            const amountInt = this.decimalToPrecision(scaled, number.TRUNCATE, 0, number.DECIMAL_PLACES);
    ...
    async watchOrderBook(outcome, limit = undefined, params = {}) {
        ...
        if (marketId === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' watchOrderBook() could not resolve the market id of ' + outcome);
        }
     ...
     */
    async unWatchOrderBook(outcome, params = {}) {
        await this.loadOutcome(outcome);
        const outcomeObj = this.outcome(outcome);
        const info = this.safeDict(outcomeObj, 'info', {});
        const marketId = this.safeString(info, 'marketId');
        if (marketId === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' unWatchOrderBook() could not resolve the market id of ' + outcome);
        }
        const topic = 'predictOrderbook/' + marketId;
     ...
     */
    orderBookMessageHashes(marketId) {
        const outcomes = this.outcomesByMarketId(marketId);
        const outcomesLength = outcomes.length;
        const hashes = [];
        for (let i = 0; i < outcomesLength; i++) {
     ...
     */
    handleSubscriptionError(client, message, subscription) {
        const rawError = this.safeDict(message, 'error', {});
        const error = new errors.ExchangeError(this.id + ' subscribe request rejected ' + this.json(rawError));
     ...
     * @returns {any} the venue's acknowledgement
     */
    async unWatchOrders(outcome = undefined, params = {}) {
     ...
     * @returns {any} the venue's acknowledgement
     */
    async unWatchMyTrades(outcome = undefined, params = {}) {
     ...
     */
    async watchWalletEvents(messageHash, params = {}) {
     ...
     * @returns {string[]} the message hashes
     */
    walletEventMessageHashes(client, messageHash = undefined) {
     ...
     * @returns {any} the venue's acknowledgement
     */
    async unWatchWalletEvents(channel, params = {}) {
        const topic = await this.walletEventsTopic();
        const requestId = this.requestId();
        const request = {
            'method': 'unsubscribe',
    ...
    socketUrl() {
        ...
        if (base === undefined) {
            ...
            throw new errors.NotSupported(this.id + ' does not have a sandbox websocket endpoint');
        }
        if (this.apiKey === undefined) {
            throw new errors.AuthenticationError(this.id + ' requires an apiKey for all methods - the venue answers the socket handshake with a 401 without one');
        }
     ...
     */
    outcomeForToken(tokenId, market = undefined) {
    ...
    handleOrderBook(client, message) {
        ...
        for (let i = 0; i < bidsLength; i++) {
            ...
            noAsks.push([this.parseNumber(Precise["default"].stringSub('1', bidPrice)), bidSize]);
        }
        ...
        for (let i = 0; i < asksLength; i++) {
            ...
            noBids.push([this.parseNumber(Precise["default"].stringSub('1', askPrice)), askSize]);
        }
        const outcomes = this.outcomesByMarketId(marketId);
        const outcomesLength = outcomes.length;
        for (let i = 0; i < outcomesLength; i++) {
     ...
     * @param {object} message the raw message
     */
    handleWalletEvent(client, message) {
        //
        //     {
        //         "type": "M",
     ...
     */
    handleWalletEventOrder(client, order) {
     ...
     */
    handleWalletEventTrade(client, event, order) {
    ...
    parseWalletEventOrder(event) {
        ...
        if ((eventType === 'orderAccepted') || (eventType === 'orderMarketMatchPlan') || (eventType === 'orderTransactionSubmitted') || (eventType === 'orderTransactionFailed')) {
            ...
            status = 'open';
            if ((amount !== undefined) && (filled !== undefined) && !Precise["default"].stringLt(filled, amount)) {
                status = 'closed';
            }
        }
     ...
     * @returns {object} a [prediction trade structure](https://docs.ccxt.com/#/?id=prediction-trade-structure)
     */
    parseWalletEventTrade(event, order) {
        ...
        const price = Precise["default"].stringDiv(this.safeString(fill, 'executedPriceWei'), '1000000000000000000');
        const amount = Precise["default"].stringDiv(this.safeString(fill, 'executedSizeWei'), '1000000000000000000');
        const cost = Precise["default"].stringDiv(this.safeString(fill, 'executedValueWei'), '1000000000000000000');
        ...
        if (rawFee !== undefined) {
            ...
            fee = {
                // a SHARES fee is charged in outcome tokens rather than in collateral
                'currency': (feeType === 'COLLATERAL') ? 'USDT' : undefined,
                'cost': this.parseNumber(Precise["default"].stringDiv(this.safeString(rawFee, 'amountWei'), '1000000000000000000')),
            };
        }
        const timestamp = this.safeInteger(event, 'timestamp');
        return this.safePredictionTrade({
     ...
     */
    sign(path, api = 'predictfun', method = 'GET', params = {}, headers = undefined, body = undefined) {
        ...
        if ((apiKey === undefined) && !sandboxMode) {
            throw new errors.AuthenticationError(this.id + ' sign() requires the "apiKey" credential for all endpoints');
        }
        const apiGroup = typeof api === 'string' ? api : api[0];
        const baseUrls = this.urls['api'];
        const baseUrl = this.safeString(baseUrls, apiGroup, baseUrls['predictfun']);
