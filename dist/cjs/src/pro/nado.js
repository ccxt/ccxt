...
class nado extends nado$1["default"] {
    ...
    async watchOHLCVForSymbols(symbolsAndTimeframes, since = undefined, limit = undefined, params = {}) {
        ...
        if (symbolsLength === 0 || !Array.isArray(symbolsAndTimeframes[0])) {
            throw new errors.ArgumentsRequired(this.id + " watchOHLCVForSymbols() requires a an array of symbols and timeframes, like  [['BTC/USDT0:USDT0', '1m'], ['ETH/USDT0:USDT0', '5m']]");
        }
        await this.loadMarkets();
        const markets = [];
        const messageHashes = [];
    ...
    async unWatchOHLCVForSymbols(symbolsAndTimeframes, params = {}) {
        const symbolsLength = symbolsAndTimeframes.length;
        if (symbolsLength === 0 || !Array.isArray(symbolsAndTimeframes[0])) {
            throw new errors.ArgumentsRequired(this.id + " unWatchOHLCVForSymbols() requires a an array of symbols and timeframes, like  [['BTC/USDT0:USDT0', '1m'], ['ETH/USDT0:USDT0', '5m']]");
        }
        await this.loadMarkets();
        const markets = [];
        const messageHashes = [];
    ...
    async authenticate(params = {}) {
        ...
        if (endpointAddress === undefined) {
            throw new errors.ExchangeError(this.id + ' authenticate() requires endpoint_addr from contracts query');
        }
    ...
    parseWsOrder(order, market = undefined) {
        ...
        if (reason === 'placed') {
            ...
            status = 'open';
            if ((amountString !== undefined) && Precise["default"].stringEq(amountString, '0')) {
                status = 'closed';
            }
        }
    ...
    parseWsPosition(position, market = undefined) {
        ...
        if (amountString !== undefined) {
            ...
            if ((vQuoteAmount !== undefined) && !Precise["default"].stringEquals(absoluteAmount, '0')) {
                entryPrice = this.parseNumber(Precise["default"].stringDiv(Precise["default"].stringAbs(vQuoteAmount), absoluteAmount));
            }
        }
    ...
    parseWsAllBidsAsks(message) {
        ...
        for (let i = 0; i < marketIds.length; i++) {
            ...
            if (Precise["default"].stringGt(bid, '0') && Precise["default"].stringGt(ask, '0') && !Precise["default"].stringEquals(bid, maxPrice) && !Precise["default"].stringEquals(ask, maxPrice)) {
