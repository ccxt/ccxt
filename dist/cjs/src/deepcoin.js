...
class deepcoin extends deepcoin$1["default"] {
    ...
    async fetchDepositAddresses(codes = undefined, params = {}) {
        await this.loadMarkets();
        if (codes === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' fetchDepositAddresses requires a list with one currency code');
        }
        const length = codes.length;
        if (length !== 1) {
            throw new errors.NotSupported(this.id + ' fetchDepositAddresses requires a list with one currency code');
        }
        const code = codes[0];
    ...
    createRegularOrderRequest(symbol, type, side, amount, price = undefined, params = {}) {
        ...
        if (price !== undefined) {
            if (isMarketOrder) {
                throw new errors.BadRequest(this.id + ' createOrder() does not require a price argument for market orders');
            }
    ...
    async fetchClosedOrder(id, symbol = undefined, params = {}) {
        await this.loadMarkets();
        if (symbol === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' fetchClosedOrder() requires a symbol argument');
        }
        const market = this.market(symbol);
        const request = {
            'instId': market['id'],
    ...
    async setLeverage(leverage, symbol = undefined, params = {}) {
        ...
        if (leverage < 1) {
            throw new errors.BadRequest(this.id + ' setLeverage() leverage should be minimum 1');
        }
        await this.loadMarkets();
        const market = this.market(symbol);
        let marginMode = 'cross';
