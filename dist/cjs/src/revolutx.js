...
var revolutx$1 = require('./abstract/revolutx.js');
 ...
 * @augments Exchange
 */
class revolutx extends revolutx$1["default"] {
    describe() {
        return this.deepExtend(super.describe(), {
            ...
            'exceptions': {
                ...
                'broad': {
                    ...
                    'timestamp is in the future': errors.InvalidNonce,
                },
            },
        });
    }
    sign(path, api = 'public', method = 'GET', params = {}, headers = undefined, body = undefined) {
        ...
        if (api === 'private') {
            ...
            const signature = crypto.eddsa(this.encode(message), this.privateKey, ed25519_js.ed25519);
            headers = {
    ...
    parseTicker(ticker, market = undefined) {
        ...
        if (last !== undefined && priceChange !== undefined) {
            open = Precise["default"].stringSub(last, priceChange);
        }
        let percentage = undefined;
        if (open !== undefined && priceChange !== undefined) {
            const percentageString = Precise["default"].stringDiv(priceChange, open, 8);
            percentage = this.parseNumber(Precise["default"].stringMul(percentageString, '100'));
        }
    ...
    async fetchTicker(symbol, params = {}) {
        ...
        if (ticker === undefined) {
            throw new errors.ExchangeError(this.id + ' fetchTicker() could not find ticker for symbol ' + symbol);
        }
        return ticker;
    }
    /**
    ...
    async fetchBalance(params = {}) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            let used = reserved;
            if (staked !== undefined) {
                used = (reserved === undefined) ? staked : Precise["default"].stringAdd(reserved, staked);
            }
            account['used'] = used;
    ...
    async createOrder(symbol, type, side, amount, price = undefined, params = {}) {
        ...
        if (type === 'limit') {
            ...
            if (timeInForce !== undefined) {
                throw new errors.InvalidOrder(this.id + ' createOrder() timeInForce is only supported for limit orders');
            }
            if (executionInstructions !== undefined) {
                throw new errors.InvalidOrder(this.id + ' createOrder() executionInstructions are only supported for limit orders');
            }
            const marketConfig = {};
            if (cost !== undefined) {
