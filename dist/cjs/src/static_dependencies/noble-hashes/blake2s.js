...
class BLAKE2s extends _blake2.BLAKE2 {
    constructor(opts = {}) {
        ...
        if (opts.salt) {
            const salt = utils.u32(utils.toBytes(opts.salt));
            this.v4 ^= salt[0];
            this.v5 ^= salt[1];
        }
        if (opts.personalization) {
            const pers = utils.u32(utils.toBytes(opts.personalization));
            this.v6 ^= pers[0];
            this.v7 ^= pers[1];
        }
        if (opts.key) {
            ...
            tmp.set(utils.toBytes(opts.key));
            this.update(tmp);
        }
    }
    get() {
    ...
    compress(msg, offset, isLast) {
        const { h, l } = _u64["default"].fromBig(BigInt(this.length));
        // prettier-ignore
        const { v0, v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12, v13, v14, v15 } = compress(_blake2.SIGMA, offset, msg, 10, this.v0, this.v1, this.v2, this.v3, this.v4, this.v5, this.v6, this.v7, IV[0], IV[1], IV[2], IV[3], l ^ IV[4], h ^ IV[5], isLast ? ~IV[6] : IV[6], IV[7]);
        this.v0 ^= v0 ^ v8;
        this.v1 ^= v1 ^ v9;
        this.v2 ^= v2 ^ v10;
        this.v3 ^= v3 ^ v11;
 ...
 */
utils.wrapConstructorWithOpts((opts) => new BLAKE2s(opts));
