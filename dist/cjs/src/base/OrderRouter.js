...
class OrderRouter {
    ...
    static { this.DEFAULT_RETRY_DELAY_MS = 1000; }
    ...
    constructor(config = {}) {
        ...
        if (maxNotionalUsd < 0) {
            ...
            throw new errors.BadRequest('OrderRouter maxNotionalUsd must not be negative; omit it, or pass 0, for no cap');
        }
    ...
    formatNumber(value) {
        ...
        if (Math.abs(value) >= 1e18) {
            ...
            throw new errors.BadRequest('OrderRouter: a number this large cannot be rendered identically in all six languages');
        }
    ...
    async fetchRoute(fromAsset, toAsset, params = {}) {
        if (fromAsset === undefined || toAsset === undefined || fromAsset === '' || toAsset === '') {
            throw new errors.ArgumentsRequired('fetchRoute requires fromAsset and toAsset');
        }
     ...
     * @returns {undefined}
     */
    assertRouteAmounts(params, method) {
        ...
        if (hasAmountIn === hasAmountOut) {
            ...
            throw new errors.BadRequest(method + ' requires exactly one of amountIn or amountOut');
        }
    }
    /**
     * @ignore
     ...
     */
    streamUrl(fromAsset, toAsset, params) {
        if (fromAsset === undefined || toAsset === undefined || fromAsset === '' || toAsset === '') {
            throw new errors.ArgumentsRequired('watchRoute requires fromAsset and toAsset');
        }
        ...
        if (params['balances'] !== undefined && params['balances'] !== null) {
            throw new errors.BadRequest('OrderRouter: /stream/route does not accept balances — a socket outlives the holdings it was opened with. Use fetchRoute');
        }
        if (params['balanceMode'] !== undefined && params['balanceMode'] !== null) {
            throw new errors.BadRequest('OrderRouter: /stream/route does not accept balanceMode, because it does not accept balances. Use fetchRoute');
        }
    ...
    streamHandshakeError(message) {
        ...
        if (at >= 0) {
            ...
            if (status === 401) {
                return new errors.AuthenticationError('OrderRouter: unauthorized');
            }
            if (status === 403) {
                ...
                return new errors.PermissionDenied('OrderRouter: the router resolved this client to a non-IPv4 address and refused it; reach it over IPv4');
            }
            if (status === 429) {
                return new errors.RateLimitExceeded('OrderRouter: rate limit exceeded');
            }
        }
        return new errors.ExchangeNotAvailable('OrderRouter: ' + message);
    }
    /**
     * @ignore
     * @method
     ...
     */
    streamCloseError(code, lastFrame) {
        if (code === 1008) {
            ...
            return new errors.BadRequest('OrderRouter: ' + message);
        }
        if (code === 1013) {
            ...
            return new errors.ExchangeNotAvailable('OrderRouter: ' + message + this.coldCacheSuffix(lastFrame));
        }
        ...
        return new errors.ExchangeNotAvailable('OrderRouter: the route stream closed with code ' + code.toString());
    }
    /**
     * @ignore
     * @method
    ...
    assertBalancesApplied(route, params) {
        ...
        throw new errors.ExchangeError('OrderRouter did not echo balancesApplied: the balances were ignored, so this route is not funded-aware');
    }
    /**
     * @ignore
     * @method
    ...
    routeParamText(value) {
        ...
        throw new errors.BadRequest('OrderRouter: a route parameter must be a string, number, boolean or array; got ' + typeof value);
    }
    /**
     * @ignore
     * @method
    ...
    async request(url, method = 'GET', requestBody = {}, requestId = '') {
        ...
        try {
            ...
            if (name === 'AbortError' || name === 'TimeoutError') {
                throw new errors.RequestTimeout('OrderRouter request timed out after ' + this.timeoutMs.toString() + 'ms');
            }
            throw new errors.ExchangeNotAvailable('OrderRouter request failed: ' + e.message);
        }
        finally {
            clearTimeout(timer);
        }
        ...
        try {
            ...
            throw new errors.ExchangeError('OrderRouter returned a non-JSON body');
        }
        ...
        if (status === 400) {
            throw new errors.BadRequest('OrderRouter: ' + message);
        }
        if (status === 401) {
            ...
            throw new errors.AuthenticationError('OrderRouter: ' + message);
        }
        if (status === 403) {
            ...
            throw new errors.PermissionDenied('OrderRouter: ' + message + ' — the router resolved this client to a non-IPv4 address and refused it; reach it over IPv4');
        }
        if (status === 429) {
            ...
            throw new errors.RateLimitExceeded('OrderRouter: ' + message + this.retrySuffix(limitInterval));
        }
        if (status === 408 || status === 504) {
            throw new errors.RequestTimeout('OrderRouter: ' + message);
        }
        if (status === 503) {
            ...
            throw new errors.ExchangeNotAvailable('OrderRouter: ' + message + this.coldCacheSuffix(body) + this.retrySuffix(retryAfter));
        }
        throw new errors.ExchangeError('OrderRouter: ' + message);
    }
    /**
     * @method
    ...
    async fetchCachedOrderBook(exchangeId, symbol) {
        if (exchangeId === undefined || symbol === undefined || exchangeId === '' || symbol === '') {
            throw new errors.ArgumentsRequired('fetchCachedOrderBook requires an exchangeId and a symbol');
        }
     ...
     */
    async watchRoute(fromAsset, toAsset, params = {}, onRoute = undefined) {
        if (onRoute === undefined) {
            ...
            throw new errors.ArgumentsRequired('watchRoute requires an onRoute callback');
        }
        ...
        return await new Promise((resolve, reject) => {
            ...
            let handshakeTimer = setTimeout(() => {
                ...
                if (!settled) {
                    ...
                    reject(new errors.RequestTimeout('OrderRouter stream did not open within ' + this.timeoutMs.toString() + 'ms'));
                }
            }, this.timeoutMs);
            ...
            socket.onmessage = (event) => {
                ...
                try {
                    ...
                    reject(new errors.ExchangeError('OrderRouter stream sent a non-JSON frame'));
                    return;
                }
    ...
    async loadWebSocket() {
        ...
        try {
            const loaded = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('ws')); });
            this.webSocketImpl = (loaded['default'] !== undefined) ? loaded['default'] : loaded;
            return this.webSocketImpl;
        }
        catch (e) {
            throw new errors.NotSupported('OrderRouter.watchRoute needs the ws package, which ccxt depends on but a browser bundle cannot provide — a browser WebSocket cannot send the x-api-key header the router requires. Use fetchRoute, or proxy the stream server-side');
        }
    }
    /**
     * @method
     ...
     * @returns {undefined}
     */
    assertNoPlanShapingOptions(options) {
        const shaping = ['slippageBps', 'reconcileToleranceRatio'];
        for (let i = 0; i < shaping.length; i++) {
            const key = shaping[i];
            if ((options[key] !== undefined) && (options[key] !== null)) {
                throw new errors.BadRequest('OrderRouter: ' + key + ' shapes a plan and this plan is already built, so it cannot be applied — pass it to buildExecutionPlan, or hand execute the route instead of the plan');
            }
        }
        return undefined;
    }
    ...
    renderBalances(value) {
        ...
        if ((value === null) || (typeof value !== 'object')) {
            throw new errors.BadRequest('OrderRouter: balances must be "[exchangeId.]ASSET:amount" entries, or a wallet like { mexc: { USDT: 100 } } or { USDT: 100 }');
        }
    ...
    assertRouteChainIsCoherent(route, hops) {
        ...
        for (let i = 0; i < hops.length; i++) {
            ...
            if (base === '' || quote === '' || (side !== 'buy' && side !== 'sell')) {
                throw new errors.ExchangeError('OrderRouter: hop ' + i.toString() + ' does not name a market and a side');
            }
            ...
            if (i > 0 && spends !== carried) {
                ...
                throw new errors.ExchangeError('OrderRouter: hop ' + i.toString() + ' spends ' + spends + ' but the previous hop produced ' + carried);
            }
            if (i === 0) {
                carried = spends;
                const requestedFrom = this.stringAt(route, 'clientRequestedFrom', '');
                if (requestedFrom !== '' && spends !== requestedFrom) {
                    throw new errors.ExchangeError('OrderRouter: the route spends ' + spends + ', not the requested ' + requestedFrom);
                }
            }
            carried = produces;
        }
        ...
        if (requestedTo !== '' && carried !== requestedTo) {
            throw new errors.ExchangeError('OrderRouter: the route produces ' + carried + ', not the requested ' + requestedTo);
        }
    }
    /**
     * @method
    ...
    reconcileExecutionStep(plan, stepIndex, realisedOut) {
        ...
        if (stepIndex < 0 || stepIndex >= steps.length) {
            throw new errors.BadRequest('reconcileExecutionStep: stepIndex is out of range');
        }
    ...
    async execute(plan, venues = {}, options = {}) {
        ...
        if (KNOWN_STRATEGIES.indexOf(requestedStrategy) < 0) {
            throw new errors.BadRequest('OrderRouter: unknown execution strategy ' + requestedStrategy);
        }
        ...
        if (options['live'] !== undefined && options['live'] !== null) {
            throw new errors.BadRequest('OrderRouter: options.live is gone — execute places orders, and a rehearsal is asked for with dryRun. Remove live, or pass dryRun to place nothing');
        }
        ...
        if (!dryRun && maxPlanAgeMs > 0) {
            if (planAgeMs < 0) {
                throw new errors.ExchangeError('OrderRouter: refusing to execute, the plan carries no calculatedAt and maxPlanAgeMs was set');
            }
            if (planAgeMs > maxPlanAgeMs) {
                throw new errors.ExchangeError('OrderRouter: refusing to execute a plan older than maxPlanAgeMs, recompute the route');
            }
        }
        if (dryRun) {
        ...
        if (Object.keys(venues).length === 0) {
            throw new errors.ArgumentsRequired('OrderRouter.execute requires a venues dictionary when live');
        }
        ...
        if (planId === '') {
            ...
            throw new errors.BadRequest('OrderRouter: refusing to execute live without an identity, the plan carries no requestId — pass options.idempotencyKey');
        }
        if (options['allowReexecution'] !== true) {
            if (this.hasExecutedPlan(planId)) {
                throw new errors.BadRequest('OrderRouter: refusing to re-execute a plan this instance already executed, pass allowReexecution to override');
            }
        }
        ...
        if (strategy === 'best_effort') {
            if (hopCount > 1) {
                ...
                throw new errors.NotSupported('OrderRouter: best_effort refuses multi-hop routes');
            }
            if (options['acknowledgeDispersion'] !== true) {
                throw new errors.BadRequest('OrderRouter: best_effort requires acknowledgeDispersion');
            }
            if (this.numberAt(options, 'maxOrders', 0) <= 0) {
                throw new errors.BadRequest('OrderRouter: best_effort requires a positive maxOrders');
            }
        }
        if (strategy === 'limit_protected') {
            ...
            if (this.hasNumberAt(options, 'pollIntervalMs') && this.numberAt(options, 'pollIntervalMs', 0) <= 0) {
                throw new errors.BadRequest('OrderRouter: pollIntervalMs must be positive, a resting order is polled on that clock');
            }
        }
        ...
        if (blockers !== '') {
            ...
            throw new errors.ExchangeError('OrderRouter: refusing to execute, blocking safety violations: ' + blockers + hint);
        }
     ...
     */
    emptyReport(plan, strategy, dryRun, steps) {
        const results = [];
        for (let i = 0; i < steps.length; i++) {
            const step = steps[i];
            results.push({
     ...
     */
    async placeStepWithRetry(step, venues, options, usdRates, strategy, report) {
     ...
     */
    callOnStep(options, report, result, reconciliation, stepIndex, stepsTotal) {
    ...
    isOutcomeUnknownError(errorCode) {
        ...
        //  instanceof/isa mechanics. The list is NOT the whole OperationFailed subtree, and the
        //  exclusions are as deliberate as the inclusions.
        //
        //  IN: the call came back without an answer, so an order may be live. binance maps
        //  -1006 — literally "An unexpected response was received from the message bus.
        //  Execution status unknown." — to OperationFailed on one endpoint and BadResponse on
        //  another. Both were previously read as "the venue said no" and RETRIED under
        //  retryFailedSteps, which is the double-fill this class exists to prevent.
        //
        //  OUT: DDoSProtection, RateLimitExceeded, InvalidNonce. Those are the venue ANSWERING —
        //  a throttled request is a definite "no order was placed" — and marking them unknown
        //  halts a route the other ports complete and sends an operator hunting for an order
        //  that never existed. That was measured once already; do not widen this to a subtree
        //  walk without re-measuring it.
    ...
    async placeImmediateOrder(venue, symbol, side, amount, price, orderParams, options, result) {
        ...
        if (options['allowMarketOrders'] !== true) {
            ...
            throw new errors.NotSupported('OrderRouter: venue cannot do IOC and allowMarketOrders was not set');
        }
        ...
        if (this.numberAt(options, 'maxNotionalUsd', this.maxNotionalUsd) > 0) {
            throw new errors.NotSupported('OrderRouter: allowMarketOrders cannot be honoured under a maxNotionalUsd cap, because a market order has no price to check');
        }
        result['placementAttempted'] = true;
    ...
    async placeProtectedLimit(venue, step, symbol, side, amount, price, orderParams, options, report, result) {
        ...
        try {
            ...
            throw new errors.ExchangeError('OrderRouter: cancelOrder failed and an order is left OPEN, refusing to proceed');
        }
    ...
    assertUnderCap(step, amount, price, usdRates, options) {
        ...
        if (usdValue <= 0) {
            throw new errors.ExchangeError('OrderRouter: refusing to place an order that cannot be valued in USD');
        }
        if (usdValue > cap * (1 + OrderRouter.TOLERANCE)) {
            throw new errors.ExchangeError('OrderRouter: refusing to place an order above the per-trade USD notional cap');
        }
    }
    /**
     * @ignore
    ...
    async assertPrefunded(steps, venues) {
        ...
        for (let i = 0; i < required.length; i++) {
            ...
            if (available < this.numberAt(required[i], 'amount', 0)) {
                ...
                throw new errors.InsufficientFunds('OrderRouter: atomic_ish requires the whole route pre-funded, and ' + exchangeId + ' is short of ' + asset);
            }
        }
    }
    /**
