...
var fswap$1 = require('./abstract/fswap.js');
var Precise = require('./base/Precise.js');
var crypto = require('./base/functions/crypto.js');
var sha256 = require('./static_dependencies/noble-hashes/sha256.js');
var sha512 = require('./static_dependencies/noble-hashes/sha512.js');
var blake3 = require('./static_dependencies/noble-hashes/blake3.js');
var ed25519 = require('./static_dependencies/noble-curves/ed25519.js');
var index = require('./static_dependencies/scure-base/index.js');
var modular = require('./static_dependencies/noble-curves/abstract/modular.js');
...
class fswap extends fswap$1 {
        ...
        return this.deepExtend(super.describe(), {
            ...
            'exceptions': {
                'exact': {
                    ...
                    '10006': errors.ExchangeError,
                    '20116': errors.ExchangeError,
                    '20117': errors.InsufficientFunds,
                    '20120': errors.InvalidOrder,
                    '20123': errors.ExchangeError,
                    '20124': errors.InsufficientFunds,
                    '20125': errors.InvalidOrder,
                    '20127': errors.InvalidOrder,
                    '20131': errors.InvalidAddress,
                    '20133': errors.ExchangeError,
                    '20134': errors.InvalidOrder,
                    '20135': errors.InvalidOrder,
                    '20150': errors.InvalidAddress,
                },
                'broad': {
    ...
    parseBalance(outputs) {
        ...
        for (let i = 0; i < outputs.length; i++) {
            ...
            result[symbol].free = Precise["default"].stringAdd(result[symbol].free, amount);
            result[symbol].total = Precise["default"].stringAdd(result[symbol].total, amount);
        }
    ...
    parseOrder(order) {
        ...
        const cost = Precise["default"].stringMul(amount, price);
        const fee = {
    ...
    parseTrade(trade, market) {
        ...
        const price = Precise["default"].stringDiv(quoteAmount, baseAmount);
    ...
    getPublicFromMainnetAddress(address) {
        ...
        const decodedData = index.base58.decode(address.slice(3));
        if (decodedData.length !== 68) {
            return undefined;
        }
        // this.binaryConcatArray([])
    ...
    buildMixAddress(members, threshold) {
        const result = this.ccxtProxyPostMixinMixaddress({ 'members': members, 'threshold': threshold });
        return this.safeString(result, 'address');
    }
    getUnspentOutputsForRecipients(outputs, rs) {
        let totalOutput = '0';
        for (let i = 0; i < rs.length; i++) {
            const cur = rs[i];
            totalOutput = Precise["default"].stringAdd(totalOutput, cur.amount);
        }
        let totalInput = '0';
        for (let i = 0; i < outputs.length; i++) {
            const o = outputs[i];
            if (o.state !== 'unspent')
                continue;
            totalInput = Precise["default"].stringAdd(totalInput, o.amount);
            if (Precise["default"].stringLt(totalInput, totalOutput))
                continue;
            const getUtxos = () => {
                const utxos = [];
                for (let j = 0; j <= i; j++) {
            ...
            return {
                'utxos': getUtxos(),
                'change': Precise["default"].stringSub(totalInput, totalOutput),
            };
        }
    ...
    async getSafeTx(asset_id, amount, memo) {
        ...
        if (!Precise["default"].stringEq(change, '0') && !Precise["default"].stringLt(change, '0')) {
    ...
    async signSafeTx(tx, views, privateKey) {
        const index = 0;
        const sha512Hash = (data) => Buffer.from(sha512.sha512.create().update(data).digest());
        const blake3Hash = (data) => Buffer.from(blake3.blake3.create({}).update(data).digest());
        const ed = this.ed();
    ...
    ed() {
        ...
        const fn = modular.Fp(ed25519.ed25519.CURVE.n, undefined, true);
        return {
            'scalar': fn,
            'setBytesWithClamping': (x) => {
                ...
                const adjustedBytes = ed25519.ed25519.CURVE.adjustScalarBytes(wideBytesUint8Array);
                const adjustedBuffer = Buffer.from(adjustedBytes);
                const m = fn.create(utils.bytesToNumberLE(adjustedBuffer.subarray(0, 32)));
                return m;
            },
            'setCanonicalBytes': (x) => {
                if (x.byteLength !== 32)
                ...
                const s = fn.create(utils.bytesToNumberLE(x));
                return s;
            },
            'scalar.add': fn.add,
            ...
            'setUniformBytes': (x) => {
                ...
                const wideBytes = Buffer.alloc(64);
                x.copy(wideBytes);
                const m = fn.create(utils.bytesToNumberLE(wideBytes));
                return m;
            },
            'scalarBaseMult': (x) => {
                const base = ed25519.ed25519.ExtendedPoint.fromHex('5866666666666666666666666666666666666666666666666666666666666666');
            ...
            'sign': (msg, key) => {
                ...
                const s = utils.numberToBytesLE(fn.add(fn.mul(x, y), z), 32);
    ...
    signToken(payload, private_key) {
        ...
        const signData = crypto.eddsa(this.encode(message), privateKey, ed25519.ed25519);
        const sign = this.base64RawURLEncode(index.base64.decode(signData));
    ...
    signAuthenticationToken(methodRaw, uri, params = {}, requestID = '') {
        ...
        const exp = iat + 3600;
        const sig = this.hash(new TextEncoder().encode(method + uri + data), sha256.sha256, 'hex');
        const payload = {
            'uid': app_id,
            'sid': session_id,
            'iat': iat,
