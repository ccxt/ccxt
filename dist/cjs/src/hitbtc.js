...
class hitbtc extends hitbtc$1 {
    describe() {
        return this.deepExtend(super.describe(), {
            ...
            'exceptions': {
                ...
                '20010': errors.BadSymbol,
                '20045': errors.InvalidOrder, // {"error":{"code":20045,"message":"Fat finger limit exceeded"}}
            },
        });
    }
    feeToPrecision(symbol, fee) {
        return this.decimalToPrecision(fee, number.TRUNCATE, 0.00000001, number.TICK_SIZE);
    }
    ...
    async transfer(code, amount, fromAccount, toAccount, params = {}) {
        ...
        if (type === undefined) {
            ...
            if (fromId === toId) {
                throw new errors.ExchangeError(this.id + ' transfer() from and to cannot be the same account');
            }
    ...
    async fetchBalance(params = {}) {
        ...
        if (typeId === undefined) {
            throw new errors.ExchangeError(this.id + ' fetchBalance() account type must be either main or trading');
        }
    ...
    async fetchTicker(symbol, params = {}) {
        ...
        if ('message' in response) {
            throw new errors.ExchangeError(this.id + ' ' + response['message']);
        }
    ...
    async createOrder(symbol, type, side, amount, price = undefined, params = {}) {
        ...
        if (order['status'] === 'rejected') {
            throw new errors.InvalidOrder(this.id + ' order was rejected by the exchange ' + this.json(order));
        }
        return order;
    }
    ...
    async fetchOrderTrades(id, symbol = undefined, since = undefined, limit = undefined, params = {}) {
        ...
        throw new errors.OrderNotFound(this.id + ' order ' + id + ' not found, ' + this.id + '.fetchOrderTrades() requires an exchange-specific order id, you need to grab it from order["info"]["id"]');
    }
    ...
    async convertCurrencyNetwork(code, amount, fromNetwork, toNetwork, params) {
        ...
        if (fromNetwork === toNetwork) {
            throw new errors.ExchangeError(this.id + ' convertCurrencyNetwork() fromNetwork cannot be the same as toNetwork');
        }
        const request = {
