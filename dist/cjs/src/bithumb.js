...
class bithumb extends bithumb$1["default"] {
    ...
    parseTicker(ticker, market = undefined) {
        ...
        if ((marketId !== undefined) && (nonZeroOpen !== undefined) && (close !== undefined)) {
            const computedChange = Precise["default"].stringSub(close, open);
            // Some v2 payloads return signed_change_price as 0 while open/last imply a non-zero move.
            if ((change !== undefined) && Precise["default"].stringEq(change, '0') && !Precise["default"].stringEq(computedChange, '0')) {
        ...
        if ((close !== undefined) && (high !== undefined) && Precise["default"].stringGt(close, high)) {
            high = close;
        }
        if ((close !== undefined) && (low !== undefined) && Precise["default"].stringLt(close, low)) {
            low = close;
        }
    ...
    async createOrders(orders, params = {}) {
        ...
        if (ordersCount === 0) {
            throw new errors.ArgumentsRequired(this.id + ' createOrders() requires a non-empty orders array');
        }
        ...
        for (let i = 0; i < orders.length; i++) {
            ...
            if (type === undefined) {
                throw new errors.ArgumentsRequired(this.id + ' createOrders() requires each order to have a type');
            }
            const side = this.safeString(rawOrder, 'side');
            if (side === undefined) {
                throw new errors.ArgumentsRequired(this.id + ' createOrders() requires each order to have a side');
            }
    ...
    async createTwapOrder(symbol, side, amount, duration, params = {}) {
        ...
        const durationSeconds = Precise["default"].stringDiv(durationString, '1000');
        const request = {
            'market': this.getGen2MarketId(market),
    ...
    async fetchOrder(id, symbol = undefined, params = {}) {
        ...
        if (generation === 2) {
            ...
            if ((base === undefined) || (quote === undefined)) {
                throw new errors.ArgumentsRequired(this.id + ' fetchOrder() requires a market with defined base and quote');
            }
            request['order_id'] = id;
        ...
        if (generation === 2) {
            if (code === 'KRW') {
                ...
                if (network === undefined) {
                    throw new errors.ArgumentsRequired(this.id + ' ' + code + ' withdraw() requires a network parameter');
                }
    ...
    async fetchWithdrawal(id, code = undefined, params = {}) {
        ...
        if (code === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' fetchWithdrawal() requires a code argument');
        }
        const currency = this.currency(code);
        const request = {
    ...
    async fetchDeposit(id, code = undefined, params = {}) {
        ...
        if (code === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' fetchDeposit() requires a code argument');
        }
        const currency = this.currency(code);
        const request = {
            'currency': currency['id'],
    ...
    parseDepositAddress(response, currency = undefined) {
        ...
        if (address === undefined) {
            throw new errors.ExchangeError(this.id + ' parseDepositAddress() could not find deposit_address');
        }
        this.checkAddress(address);
        return {
            'info': response,
        ...
        if (api === 'public') {
            ...
            if (isVersionedApi) {
                ...
                if (hasQuery) {
                    ...
                    request['query_hash'] = this.hash(this.encode(authString), sha2_js.sha512);
