...
class btse extends btse$1["default"] {
        ...
        return this.deepExtend(super.describe(), {
            ...
            'exceptions': {
                'exact': {
                    ...
                    '-2': errors.BadRequest, // INVALID_REQUEST {"status":400,"errorCode":-2,"message":"symbol parameter is mandatory","extraData":null}
                    '-7': errors.AuthenticationError, // {"status":400,"errorCode":-7,"message":"Authenticate failed","extraData":null}
                    '-7006': errors.BadSymbol, // {"status":400,"errorCode":-7006,"message":"Unsupported symbol","extraData":null} observed live for a full contract id sent to the unified futures api
                    '-11': errors.ExchangeNotAvailable, // 400 Bad Request {"code":-11,"msg":"System error","success":false,"time":1770451790797,"data":[]}
                    ...
                    '1': errors.ExchangeNotAvailable, // MARKET_UNAVAILABLE
                    '8': errors.InsufficientFunds, // INSUFFICIENT_BALANCE
                    '11': errors.BadRequest, // ERROR_INVALID_CURRENCY
                    '12': errors.BadRequest, // ERROR_UPDATE_RISK_LIMIT
                    '13': errors.BadRequest, // ERROR_INVALID_LEVERAGE
                    '15': errors.InvalidOrder, // ORDER_REJECTED
                    '16': errors.OrderNotFound, // ORDER_NOTFOUND
                    '17': errors.ExchangeError, // REQUEST_FAILED
                    '28': errors.ExchangeError, // TRANSFER_UNSUCCESSFUL
                    '41': errors.BadRequest, // ERROR_INVALID_RISK_LIMIT
                    '64': errors.ExchangeError, // STATUS_LIQUIDATION
                    '101': errors.InvalidOrder, // FUTURES_ORDER_PRICE_OUTSIDE_LIQUIDATION_PRICE
                    '133': errors.InvalidOrder, // {"status":400,"errorCode":133,"message":"Position mode invalid","extraData":["0","150"]}
                    '134': errors.BadRequest, // {"status":400,"errorCode":134,"message":"failure","extraData":"Remaining positions."}
                    '135': errors.BadRequest, // invalid position id, observed live with an embedded json message
                    '300': errors.InvalidOrder, // ERROR_MAX_ORDER_SIZE_EXCEEDED
                    '301': errors.InvalidOrder, // ERROR_INVALID_ORDER_SIZE
                    '302': errors.InvalidOrder, // ERROR_INVALID_ORDER_PRICE
                    '303': errors.RateLimitExceeded, // ERROR_RATE_LIMITS_EXCEEDED
                    '304': errors.InvalidOrder, // ERROR_MAX_OPEN_ORDER_EXCEEDED
                    '305': errors.InvalidOrder, // ERROR_ORDER_PRICE_OUT_OF_PRICE_PROTECTION_RANGE
                    '1003': errors.InvalidOrder, // ORDER_LIQUIDATION
                    '1004': errors.InvalidOrder, // ORDER_ADL
                    '4003': errors.InvalidOrder, // {"code":4003,"msg":"BADREQUEST: The order amount cannot surpass 100.0 BTC. Please adjust your order size and try again.","time":1786509292895,"data":null,"success":false}
                    '4005': errors.InvalidOrder, // {"code":4005,"msg":"BADREQUEST: order price must be at least 0.01 USDT","time":1786622000000,"data":null,"success":false} observed live on the unified spot api
                    '4051': errors.InvalidOrder, // {"status":400,"errorCode":4051,"message":"[RAVE-PERP] The order size cannot surpass 50000.0 contracts. Please adjust your order size and try again.","extraData":null}
                    '10002': errors.AuthenticationError, // {"code":10002,"msg":"UNAUTHORIZED: Authentication Failed","time":1770477230034,"data":null,"success":false}
                    '10010004': errors.BadRequest, // {"code":10010004,"msg":"BADREQUEST: resolution too small for the requested time range. Records returned exceeds 300","success":false} observed live on the unified markets api
                    '11000001': errors.BadRequest, // {"code":11000001,"msg":"Request parameter is error, 'asset' is required in spot wallet history query","time":1786624306820,"data":null,"success":false} observed live on the unified wallet api
                    '51523': errors.InsufficientFunds, // {"code":51523,"msg":"BADREQUEST: Insufficient wallet balance","time":1770814875493,"data":null,"success":false}
                    '33001001': errors.InvalidOrder, // {"code":33001001,"msg":"BADREQUEST: The distance between Trigger Price and Limit Price cannot exceed 5.0 %","time":1770815167145,"data":["5.0 %"],"success":false}
                    '33001003': errors.InvalidOrder, // {"status":400,"errorCode":33001003,"message":"You can not SELL ETH lower than 1825.24 USDT","extraData":["SELL","ETH","lower","1825.24","USDT"]}
                    '33199101': errors.InsufficientFunds, // {"status":400,"errorCode":33199101,"message":"Available balance is insufficient to meet this order.","extraData":["0.013553333"]}
                    '33199120': errors.InvalidOrder, // {"status":400,"errorCode":33199120,"message":"Reduce only open order canceled because no active position exists","extraData":null}
                },
                'broad': {
                    // the specific strings come from the official error codes reference and
                    // must stay above the generic BADREQUEST catch all
                    'Order not found': errors.OrderNotFound, // {"status":429,"errorCode":-1,"message":"Order not found","extraData":["ETHPFC-USD","0","117"]}
                    'Insufficient wallet balance': errors.InsufficientFunds,
                    'Insufficient balance': errors.InsufficientFunds, // official error string for enum code 8
                    'Authentication Failed': errors.AuthenticationError,
                    'Authenticate failed': errors.AuthenticationError,
                    'Signature verification failed': errors.AuthenticationError, // official error string, signed payload mismatch
                    'api parameter is mandatory': errors.AuthenticationError, // official error string, wrong auth header names
                    'Invalid nonce': errors.InvalidNonce, // official error string, clock drift or nonce reuse
                    'Invalid order size': errors.InvalidOrder, // official error string for enum code 301
                    'Invalid order price': errors.InvalidOrder, // official error string for enum code 302
                    'Maximum open orders exceeded': errors.InvalidOrder, // official error string for enum code 304
                    'Rate limit exceeded': errors.RateLimitExceeded, // official error string for enum code 303
                    'auto-deleveraging': errors.InvalidOrder, // official error string for enum code 1004
        ...
        for (let i = 0; i < response.length; i++) {
            ...
            if (assets !== undefined) {
                ...
                for (let j = 0; j < inUse.length; j++) {
                    ...
                    useds[usedCode] = Precise["default"].stringAdd(this.safeString(useds, usedCode, '0'), this.safeString(usedRow, 'balance'));
                }
                for (let j = 0; j < assets.length; j++) {
                    ...
                    totals[code] = Precise["default"].stringAdd(this.safeString(totals, code, '0'), this.safeString(assetRow, 'balance'));
                ...
                totals[code] = Precise["default"].stringAdd(this.safeString(totals, code, '0'), this.safeString2(row, 'totalAmount', 'total'));
                frees[code] = Precise["default"].stringAdd(this.safeString(frees, code, '0'), this.safeString2(row, 'availableAmount', 'available'));
            }
        }
    ...
    parseTicker(ticker, market = undefined) {
        ...
        if ((baseVolume !== undefined) && (market !== undefined) && (market['contract'] === true)) {
            ...
            if (contractSizeString !== undefined) {
                baseVolume = Precise["default"].stringMul(baseVolume, contractSizeString);
            }
        }
    ...
    async fetchOrderTrades(id, symbol = undefined, since = undefined, limit = undefined, params = {}) {
        ...
        if (clientOrderId === undefined) {
            if (id === undefined) {
                throw new errors.ArgumentsRequired(this.id + ' fetchOrderTrades() requires an id argument or a clientOrderId parameter');
            }
            else {
    ...
    async createSpotOrder(symbol, type, side, amount, price = undefined, params = {}) {
        ...
        if (isLimitOrder || (type === 'PEG') || (type === 'OCO')) {
            if (price === undefined) {
                throw new errors.InvalidOrder(this.id + ' createOrder() requires a price argument for ' + type + ' orders');
            }
        }
        ...
        if (needsQuoteSize) {
            ...
            if (cost !== undefined) {
                ...
                if (price === undefined) {
                    throw new errors.InvalidOrder(this.id + ' createOrder() requires the price argument for market buy orders to calculate the total cost to spend, alternatively set the createMarketBuyOrderRequiresPrice option or param to false and pass the cost to spend in the amount argument');
                }
                else {
                    const amountString = this.numberToString(amount);
                    const priceString = this.numberToString(price);
                    quoteAmount = this.costToPrecision(symbol, Precise["default"].stringMul(amountString, priceString));
                }
            }
            else {
    ...
    async createContractOrder(symbol, type, side, amount, price = undefined, params = {}) {
        ...
        if (positionMode === undefined) {
            ...
            if (marginMode === 'isolated') {
                if (hedged) {
                    throw new errors.BadRequest(this.id + ' createOrder() cannot use isolated margin with hedged positions');
                }
    ...
    async fetchOpenOrder(id, symbol = undefined, params = {}) {
        ...
        if (clientOrderId !== undefined) {
            ...
            throw new errors.ArgumentsRequired(this.id + ' fetchOpenOrder() requires an id argument or a clientOrderId parameter');
        }
        else {
            request['orderId'] = id;
        }
    ...
    async editOrder(id, symbol, type, side, amount = undefined, price = undefined, params = {}) {
        ...
        if (clientOrderId !== undefined) {
            ...
            throw new errors.ArgumentsRequired(this.id + ' editOrder() requires an id argument or a clientOrderId parameter');
        }
        else {
            request['orderId'] = id;
        }
        ...
        if (market['spot'] === true) {
            ...
            if (triggerPrice !== undefined) {
                if ((amount !== undefined) || (price !== undefined)) {
                    throw new errors.BadRequest(this.id + ' editOrder() can not amend the trigger price together with the price or the amount on contract markets');
                }
    ...
    async cancelOrder(id, symbol = undefined, params = {}) {
        ...
        if (clientOrderId !== undefined) {
            ...
            throw new errors.ArgumentsRequired(this.id + ' cancelOrder() requires an id argument or a clientOrderId parameter');
        }
        else {
            request['orderId'] = id;
        }
    ...
    async cancelAllOrders(symbol = undefined, params = {}) {
        ...
        if (marketType === 'spot') {
            ...
            if (market === undefined) {
                throw new errors.ArgumentsRequired(this.id + ' cancelAllOrders() requires a symbol argument for contract markets');
            }
    ...
    async requestWalletHistoryRows(methodName, historyTypes, code = undefined, since = undefined, limit = undefined, params = {}) {
        ...
        if (code !== undefined) {
            ...
            throw new errors.ArgumentsRequired(this.id + ' ' + methodName + '() requires a code argument for the spot wallet history');
        }
        if (since !== undefined) {
            request['startTime'] = since;
        }
    ...
    async fetchLedger(code = undefined, since = undefined, limit = undefined, params = {}) {
        ...
        if (code !== undefined) {
            ...
            // the exchange rejects spot wallet history queries without an asset,
            // verified live, and omitting walletType still defaults to spot
            throw new errors.ArgumentsRequired(this.id + ' fetchLedger() requires a code argument for the spot wallet history');
        }
        if (since !== undefined) {
            request['startTime'] = since;
        }
    ...
    async setMarginMode(marginMode, symbol = undefined, params = {}) {
        ...
        if ((marginMode !== 'cross') && (marginMode !== 'isolated')) {
            throw new errors.BadRequest(this.id + ' setMarginMode() marginMode argument should be either cross or isolated');
        }
    ...
    async closePosition(symbol, side = undefined, params = {}) {
        ...
        if (positionId === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' closePosition() requires a positionId parameter');
        }
        const request = {
            'symbol': this.futuresRequestId(market),
        };
        ...
        if (type === 'LIMIT') {
            ...
            if (price === undefined) {
                throw new errors.ArgumentsRequired(this.id + ' closePosition() requires a price parameter for limit orders');
            }
            request['orderPrice'] = this.priceToPrecision(symbol, price);
