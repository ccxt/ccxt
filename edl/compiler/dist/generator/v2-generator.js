 ...
 */
export function generateExchangeV2(doc, options = {}) {
    const lines = [];
    const exchangeId = doc.exchange.id;
    const className = capitalize(exchangeId);
    // Imports
    lines.push(`import Exchange from './base/Exchange.js';`);
...
function generateSign(auth, exchangeId, options) {
    ...
    if (auth.select && auth.select.length > 0) {
        ...
        for (const selector of auth.select) {
            ...
            const variant = typeof selector.variant === 'string' ? `'${selector.variant}'` : generateExpressionCode(selector.variant, 'this');
...
function generateSigningPipeline(pipeline, indent) {
    const lines = [];
...
function generateSimpleSigning(auth, indent) {
    const lines = [];
...
function generateNonce(auth, options) {
    const lines = [];
    const indent = '    ';
...
function generateParser(name, parser, doc, options) {
    const lines = [];
    const indent = '    ';
    ...
    if (typeof parser.mapping === 'object' && '_ref' in parser.mapping) {
        lines.push(`${indent}${indent}return this.parse${capitalize(parser.mapping._ref)}(data, market);`);
        lines.push(`${indent}}`);
        return lines.join('\n');
    }
    ...
    if (parser.isArray) {
        ...
        if (Array.isArray(parser.mapping)) {
            ...
            const elements = parser.mapping.map((m) => {
    ...
    if (parser.iterate && parser.iterate.keys) {
        const itemVar = parser.iterate.itemVar || 'key';
        const valueVar = parser.iterate.valueVar || 'value';
        lines.push(`${indent}${indent}const result: any = {};`);
        ...
        // Filter if specified
        if (parser.iterate.filter) {
            const filterCode = generateExpressionCode(parser.iterate.filter, 'this');
...
function generateMappingCode(mapping, dataVar, indent) {
    ...
    if (typeof mapping === 'object' && mapping !== null) {
        ...
        if ('path' in mapping) {
            ...
            if (mapping.default !== undefined) {
                code = `(${code} ?? ${JSON.stringify(mapping.default)})`;
            }
            return code;
        }
        // Compute mapping
        if ('compute' in mapping) {
            if (mapping.compute === null) {
                return 'undefined';
            }
            return generateExpressionCode(mapping.compute, 'this');
        }
        // Conditional mapping
        if ('if' in mapping) {
            const condition = generateExpressionCode(mapping.if, 'this');
            const thenValue = generateMappingCode(mapping.then, dataVar, indent);
            const elseValue = mapping.else ? generateMappingCode(mapping.else, dataVar, indent) : 'undefined';
            return `(${condition}) ? ${thenValue} : ${elseValue}`;
        }
        // Switch mapping
        if ('switch' in mapping) {
            const switchExpr = generateExpressionCode(mapping.switch, 'this');
            const cases = mapping.cases || {};
            let code = `(() => { switch (${switchExpr}) {`;
            for (const [caseVal, caseMapping] of Object.entries(cases)) {
                const caseCode = generateMappingCode(caseMapping, dataVar, indent);
                code += ` case '${caseVal}': return ${caseCode};`;
            }
            if (mapping.default) {
                const defaultCode = generateMappingCode(mapping.default, dataVar, indent);
        ...
        if ('literal' in mapping) {
            return JSON.stringify(mapping.literal);
        }
    }
    // Fallback
    return 'undefined';
...
function applyTransform(code, transform) {
    ...
    if (typeof transform === 'object' && transform !== null) {
        ...
        if ('call' in transform) {
            const args = transform.args?.map((a) => a === '$value' ? code : generateExpressionCode(a, 'this')).join(', ') || code;
            return `${transform.call}(${args})`;
        }
    }
    return code;
}
function generateExpressionCode(expr, context) {
    ...
    if (typeof expr === 'object') {
        ...
        switch (expr.type) {
            ...
            case 'call':
                ...
                const args = (expr.args || []).map((a) => generateExpressionCode(a, context)).join(', ');
                // Map common function names
                if (func === 'extend')
                    return `${context}.extend(${args})`;
                if (func === 'length')
                    return `(${args})?.length`;
                if (func === 'Array.isArray')
            ...
            case 'object':
                ...
                return `{ ${props} }`;
            case 'array':
                const elements = (expr.elements || []).map((e) => generateExpressionCode(e, context)).join(', ');
                return `[${elements}]`;
            default:
                ...
                if ('call' in expr) {
                    const fn = expr.call;
                    const fnArgs = (expr.args || []).map((a) => generateExpressionCode(a, context)).join(', ');
                    // Don't add context if function already has `this.`
...
function generateBinaryExpression(op, left, right) {
    switch (op) {
        case 'contains':
...
function generateMethod(name, method, doc, options) {
    const lines = [];
    const indent = '    ';
    ...
    if (method.selection || method.variants) {
        ...
        if (method.variants && method.variants.length > 0) {
            ...
            const defaultVariant = method.variants.find((v) => v.name === method.selection?.default);
            const otherVariants = method.variants.filter((v) => v.name !== method.selection?.default);
            ...
            for (let i = 0; i < otherVariants.length; i++) {
                ...
                const apiMethod = endpoint.map((p, idx) => {
        ...
        if (method.endpoint) {
            ...
            const apiMethod = endpoint.map((p, i) => i === 0 ? p : capitalize(p)).join('');
...
function generateTransformMethod(name, transform, options) {
    const lines = [];
    const indent = '    ';
    ...
    if (transform.type === 'switch') {
        ...
        if (Array.isArray(mapping)) {
            ...
            for (const [field, fieldMapping] of Object.entries(mapping)) {
                const fieldCode = generateMappingCode(fieldMapping, 'item', indent + indent + indent + indent);
...
function generateStandardMethods(doc, options) {
    const lines = [];
    const indent = '    ';
...
function generateAPIStructure(api, indent) {
    const lines = ['{'];
    for (const [category, endpoints] of Object.entries(api)) {
        if (category === 'baseUrls')
            continue;
...
function generateExceptions(errors, indent) {
    const lines = ['{'];
    if (errors.default) {
...
//# sourceMappingURL=v2-generator.js.map