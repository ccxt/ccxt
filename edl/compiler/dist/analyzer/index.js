...
export function analyzeEDL(doc) {
    const errors = [];
    const warnings = [];
...
function validateExchangeMetadata(doc, errors, warnings) {
    const { exchange } = doc;
...
function validateAuth(doc, errors, warnings) {
    const auth = doc.auth;
...
function validateAPI(api, errors, warnings) {
...
function validateParsers(doc, errors, warnings) {
...
function validateMappings(mapping, pathPrefix, errors, warnings) {
    for (const [field, fieldMapping] of Object.entries(mapping)) {
        ...
        if ('compute' in fieldMapping) {
            const deps = fieldMapping.dependencies || [];
            // Basic check for referenced fields in expression
            const expr = fieldMapping.compute;
            const refs = expr.match(/\{([^}]+)\}/g) || [];
            for (const ref of refs) {
                const refName = ref.slice(1, -1);
                if (!deps.includes(refName) && !Object.keys(mapping).includes(refName)) {
                    warnings.push({
                        path: `${path}.compute`,
                        message: `Computed field references "{${refName}}" but it's not in dependencies or mapping`,
                        severity: 'warning',
                    });
                }
            }
...
function validateConditionalMapping(conditional, path, errors, warnings) {
    if (!conditional.if) {
        errors.push({
            path: `${path}.if`,
...
function validateErrors(doc, errors, warnings) {
...
function validateOverrides(doc, errors, warnings) {
...
function validateCrossReferences(doc, errors, warnings) {
