@kwdef mutable struct Woofipro <: CcxtExchange
...
function describe(self::Woofipro, )
    return deepExtend(describe(self.parent), Dict{Symbol, Any}(
    Symbol("id") => "woofipro",
    Symbol("name") => "WOOFI PRO",
    Symbol("countries") => ["KY"],
    ...
    Symbol("urls") => Dict{Symbol, Any}(
        Symbol("logo") => "https://github.com/user-attachments/assets/9ba21b8a-a9c7-4770-b7f1-ce3bcbde68c1",
...
function setSandboxMode(self::Woofipro, enable)
...
function fetchCurrencies(self::Woofipro, params=Dict())
    ...
    while functions.ccxtruthy(functions.ccxt_lt(i, length(tokenRows)))
        token = get(tokenRows, i + 1, nothing);
        parsed = self.parseCurrency(Dict{Symbol, Any}(
            Symbol("_token") => token,
...
function parseCurrency(self::Woofipro, rawCurrency)
    ...
    Symbol("limits") => Dict{Symbol, Any}(
        ...
        Symbol("withdraw") => Dict{Symbol, Any}(
            Symbol("min") => self.safeNumber(token, "minimum_withdraw_amount"),
            Symbol("max") => nothing
        )
    ),
    Symbol("info") => token
...
function parseTokenAndFeeTemp(self::Woofipro, item, feeTokenKey, feeAmountKey)
...
function parseFundingInterval(self::Woofipro, interval)
...
function parseOrderStatus(self::Woofipro, status)
    if functions.ccxtruthy(status != nothing)
...
function parseOrderType(self::Woofipro, type_var)
...
function parseTransactionStatus(self::Woofipro, status)
    statuses = Dict{Symbol, Any}(
        Symbol("NEW") => "pending",
...
function hashMessage(self::Woofipro, message)
...
function withdraw(self::Woofipro, code, amount, address, tag=nothing, params=Dict())
    ...
    withdrawRequest = Dict{Symbol, Any}(
        Symbol("brokerId") => safeString(self.options, "keyBrokerId", "woofi_pro"),
...
function setLeverage(self::Woofipro, leverage, symbol=nothing, params=Dict())
    ...
    if functions.ccxtruthy(@functions.ccxt_or((functions.ccxt_lt(leverage, 1)), (functions.ccxt_gt(leverage, 50))))
...
function handleErrors(self::Woofipro, httpCode, reason, url, method, headers, body, response, requestHeaders, requestBody)
