using Test
using Ccxt
function testFetchOpenOrders(exchange, skippedProperties, symbol)

    method = "fetchOpenOrders";
    orders = Base.fetch(fetchOpenOrders(exchange, symbol));
    ...
    while functions.ccxtruthy(functions.ccxt_lt(i, length(orders)))
        ...
        assertInArray(testSharedMethods, exchange, skippedProperties, method, order, "status", ["open"]);
        i += 1
    end
