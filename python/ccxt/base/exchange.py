...
class Exchange(object):
    ...
    currencies = None
    options = (
        None  # Python does not allow to define properties in run-time with setattr
    )
    accounts = None
    positions = None
    ...
    def fetch(self, url, method="GET", headers=None, body=None):
        ...
        proxyUrl, httpProxy, httpsProxy, socksProxy = self.check_proxy_settings(
        ...
        if (proxies is not None) and (self.proxies is not None):
            ...
            raise NotSupported(
                self.id + " you have set multiple proxies, please use one or another"
            )
    ...
    def get_object_value_from_key_list(dictionary, key_list):
        filtered_list = list(
            filter(
                lambda el: el in dictionary
                and dictionary[el] != ""
                and dictionary[el] is not None,
                key_list,
            )
        )
    ...
    def sort_by(array, key, descending=False):
        return sorted(
            array,
            key=lambda k: k[key] if k[key] is not None else "",
            reverse=descending,
        )
    ...
    def parse_date(timestamp=None):
        ...
        if "GMT" in timestamp:
            try:
                string = (
                    "".join([str(value) for value in parsedate(timestamp)[:6]])
                    + ".000Z"
                )
    ...
    def ecdsa(request, secret, algorithm="p256", hash=None, fixed_length=False):
        ...
        key = ecdsa.SigningKey.from_string(
            base64.b16decode(Exchange.encode(secret), casefold=True),
            curve=curve_info[0],
        )
    ...
    def check_address(self, address):
        ...
        if (
            all(letter == address[0] for letter in address)
    ...
    def check_order_arguments(self, market, type, side, amount, price, params):
        ...
        if amount <= 0:
            raise ArgumentsRequired(
                self.id + " create_order() amount should be above 0"
            )
    ...
    def check_proxy_settings(self, url, method, headers, body):
        ...
        proxyUrlCallback = (
            self.proxyUrlCallback
            if (self.proxyUrlCallback is not None)
            else self.proxy_url_callback
        )
        ...
        httpProxyCallback = (
            self.httpProxyCallback
            if (self.httpProxyCallback is not None)
            else self.http_proxy_callback
        )
        if httpProxyCallback is not None:
            httpProxy = httpProxyCallback(url, method, headers, body)
        httpsProxy = (
            self.httpsProxy if (self.httpsProxy is not None) else self.https_proxy
        )
        httpsProxyCallback = (
            self.httpsProxyCallback
            if (self.httpsProxyCallback is not None)
            else self.https_proxy_callback
        )
        if httpsProxyCallback is not None:
            httpsProxy = httpsProxyCallback(url, method, headers, body)
        socksProxy = (
            self.socksProxy if (self.socksProxy is not None) else self.socks_proxy
        )
        socksProxyCallback = (
            self.socksProxyCallback
            if (self.socksProxyCallback is not None)
            else self.socks_proxy_callback
        )
        ...
        if val > 1:
            raise ExchangeError(
                self.id
                + " you have multiple conflicting proxy settings, please use only one from : proxyUrl, httpProxy, httpsProxy, socksProxy, userAgent"
            )
    ...
    def filter_by_limit(
        ...
        if self.valueIsDefined(limit):
            ...
            if arrayLength > 0:
                ...
                array = (
                    self.arraySlice(array, -limit)
                    if ascending
                    else self.arraySlice(array, 0, limit)
                )
        return array
    ...
    def set_markets(self, markets, currencies=None):
        ...
        if currencies is not None:
            ...
            self.currencies = self.deep_extend(
    ...
    def parse_orders(
        ...
        if isinstance(orders, list):
            ...
            for i in range(0, len(ids)):
                id = ids[i]
                order = self.extend(
                    self.parse_order(self.extend({"id": id}, orders[id]), market),
                    params,
                )
                results.append(order)
    ...
    def parse_bids_asks(
        self, bidasks, priceKey: IndexType = 0, amountKey: IndexType = 1
    ):
    ...
    def get_network(self, network: str, code: str):
        ...
        if network == code:
            ...
            raise NotSupported(
                self.id + " network " + network + " is not yet supported"
            )
    ...
    def network_id_to_code(self, networkId, currencyCode=None):
        ...
        if currencyCode is not None:
            ...
            if currencyCode in defaultNetworkCodeReplacements:
                replacementObject = self.safe_value(
    ...
    def select_network_code_from_unified_networks(
        ...
        return self.selectNetworkKeyFromNetworks(
        ...
        if networkCode is not None:
            if responseNetworksLength == 0:
                ...
                networkId = (
                    ...
                    else self.networkCodeToId(networkCode, currencyCode)
                )
            ...
            if responseNetworksLength == 0:
                ...
                defaultNetworkId = (
                    defaultNetworkCode
                    if isIndexedByUnifiedNetworkCode
                    else self.networkCodeToId(defaultNetworkCode, currencyCode)
                )
                chosenNetworkId = (
                    defaultNetworkId
                    if (defaultNetworkId in indexedNetworkEntries)
                    else availableNetworkIds[0]
                )
    ...
    def parse_order_book(
        ...
        bids = self.parse_bids_asks(
            self.safe_value(orderbook, bidsKey, []), priceKey, amountKey
        )
        asks = self.parse_bids_asks(
            self.safe_value(orderbook, asksKey, []), priceKey, amountKey
        )
        return {
            "symbol": symbol,
    ...
    def parse_leverage_tiers(
        self, response, symbols: Optional[List[str]] = None, marketIdKey=None
    ):
        ...
        for i in range(0, len(response)):
            ...
            market = self.safe_market(
                id, None, None, self.safe_string(self.options, "defaultType")
            )
    ...
    def load_trading_limits(
        self, symbols: Optional[List[str]] = None, reload=False, params={}
    ):
    ...
    def parse_positions(
        self, positions, symbols: Optional[List[str]] = None, params={}
    ):
        symbols = self.market_symbols(symbols)
    ...
    def build_ohlcvc(
        ...
        for i in range(0, oldest):
            ...
            if (candle == -1) or (
                openingTime >= self.sum(ohlcvs[candle][i_timestamp], ms)
            ):
    ...
    def edit_order(
        self, id: str, symbol, type, side, amount=None, price=None, params={}
    ):
        ...
        if marketId is not None:
            if (self.markets_by_id is not None) and (marketId in self.markets_by_id):
                ...
                if numMarkets == 1:
                    ...
                    inferredMarketType = (
                        market["type"] if (marketType is None) else marketType
                    )
    ...
    def fetch_funding_fee(self, code: str, params={}):
        warnOnFetchFundingFee = self.safe_value(
            self.options, "warnOnFetchFundingFee", True
        )
        if warnOnFetchFundingFee:
            raise NotSupported(
                self.id
                + ' fetchFundingFee() method is deprecated, it will be removed in July 2022, please, use fetchTransactionFee() or set exchange.options["warnOnFetchFundingFee"] = False to suppress self warning'
            )
        return self.fetch_transaction_fee(code, params)

    def fetch_funding_fees(self, codes: Optional[List[str]] = None, params={}):
        warnOnFetchFundingFees = self.safe_value(
            self.options, "warnOnFetchFundingFees", True
        )
        if warnOnFetchFundingFees:
            raise NotSupported(
                self.id
                + ' fetchFundingFees() method is deprecated, it will be removed in July 2022. Please, use fetchTransactionFees() or set exchange.options["warnOnFetchFundingFees"] = False to suppress self warning'
            )
    ...
    def handle_option_and_params(
        self, params, methodName, optionName, defaultValue=None
    ):
    ...
    def handle_option(self, methodName, optionName, defaultValue=None):
        ...
        result, empty = self.handle_option_and_params(
            {}, methodName, optionName, defaultValue
        )
        return result
    ...
    def handle_market_type_and_params(self, methodName, market=None, params={}):
        ...
        if methodOptions is not None:
            if isinstance(methodOptions, str):
                ...
                methodType = self.safe_string_2(
                    methodOptions, "defaultType", "type", methodType
                )
    ...
    def handle_sub_type_and_params(
        self, methodName, market=None, params={}, defaultValue=None
    ):
        subType = None
        ...
        if subTypeInParams is not None:
            ...
            if subType is None:
                values = self.handle_option_and_params(
                    None, methodName, "subType", defaultValue
    ...
    def fetch_unified_order(self, order, params={}):
        return self.fetch_order(
            self.safe_value(order, "id"), self.safe_value(order, "symbol"), params
        )
    ...
    def create_limit_order(
        self, symbol: str, side: OrderSide, amount, price, params={}
    ):
        return self.create_order(symbol, "limit", side, amount, price, params)

    def create_market_order(
        self, symbol: str, side: OrderSide, amount, price=None, params={}
    ):
    ...
    def currency_to_precision(self, code: str, fee, networkCode=None):
        ...
        if precision is None:
            ...
            return self.decimal_to_precision(
                fee, ROUND, precision, self.precisionMode, self.paddingMode
            )
    ...
    def safe_number(
        self, obj: object, key: IndexType, defaultNumber: Optional[float] = None
    ):
        value = self.safe_string(obj, key)
        return self.parse_number(value, defaultNumber)

    def safe_number_n(
        self, obj: object, arr: List[IndexType], defaultNumber: Optional[float] = None
    ):
    ...
    def create_post_only_order(
        self, symbol: str, type: OrderType, side: OrderSide, amount, price, params={}
    ):
    ...
    def create_stop_limit_order(
        self, symbol: str, side: OrderSide, amount, price, stopPrice, params={}
    ):
    ...
    def create_stop_market_order(
        self, symbol: str, side: OrderSide, amount, stopPrice, params={}
    ):
    ...
    def safe_currency_code(
        self, currencyId: Optional[str], currency: Optional[Any] = None
    ):
    ...
    def parse_last_prices(
        self, pricesData, symbols: Optional[List[str]] = None, params={}
    ):
        #
        ...
        if isinstance(pricesData, list):
            ...
            for i in range(0, len(marketIds)):
                ...
                priceData = self.extend(
                    self.parseLastPrice(pricesData[marketId], market), params
                )
                results.append(priceData)
        symbols = self.market_symbols(symbols)
    ...
    def parse_tickers(self, tickers, symbols: Optional[List[str]] = None, params={}):
        ...
        if isinstance(tickers, list):
            ...
            for i in range(0, len(marketIds)):
                ...
                market = self.safe_market(marketId)
                ticker = self.extend(
                    self.parse_ticker(tickers[marketId], market), params
                )
                results.append(ticker)
        symbols = self.market_symbols(symbols)
        return self.filter_by_array(results, "symbol", symbols)
    ...
    def parse_deposit_addresses(
        self, addresses, codes: Optional[List[str]] = None, indexed=True, params={}
    ):
        result = []
    ...
    def parse_deposit_withdraw_fees(
        self, response, codes: Optional[List[str]] = None, currencyIdKey=None
    ):
        """
         * @ignore
        ...
        for i in range(0, len(responseKeys)):
            ...
            if (codes is None) or (self.in_array(code, codes)):
                depositWithdrawFees[code] = self.parseDepositWithdrawFee(
                    dictionary, currency
                )
    ...
    def fetch_deposits_withdrawals(self, code=None, since=None, limit=None, params={}):
        ...
        if self.has["fetchTransactions"]:
            ...
            raise NotSupported(
                self.id + " fetchDepositsWithdrawals() is not supported yet"
            )
