...
class lbank2(Exchange):
    ...
    def fetch_ticker(self, symbol, params={}):
        ...
        #      }
        #
        result = self.safe_value(response, 'data')[0]
    ...
    def fetch_trades(self, symbol, since=None, limit=None, params={}):
        self.load_markets()
        if limit is None:
            raise ArgumentsRequired(self.id + 'fetchTrades() requires a limit argument')
        market = self.market(symbol)
        request = {
            'symbol': market['id'],
            'size': limit,  # max
    ...
    def fetch_closed_orders(self, symbol=None, since=None, limit=None, params={}):
        ...
        self.load_markets()
        market = self.market(symbol)
        since = self.safe_value(params, 'startTime', since)
        params = self.omit(params, 'startTime')
        request = {
            'symbol': market['id'],
        ...
        if limit is not None:
            request['limit'] = limit
        if since is not None:
            request['startTime'] = self.ymd(since, '-')  # max query 2 days ago
        response = self.privatePostSupplementTransactionHistory(self.extend(request, params))
        #
        #      {
        #          "result":true,
        #          "data":[
        ...
        if api == 'public':
            ...
            if signatureMethod == 'RSA':
                ...
                if cacheSecretAsPem:
                    ...
                    pem = self.convert_secret_to_pem(self.secret)
                sign = self.binary_to_base64(self.rsa(hash, self.encode(pem), 'RS256'))
                # TODO fix RSA signing
            elif signatureMethod == 'HmacSHA256':
                sign = self.hmac(self.encode(hash), self.secret)
            body = self.urlencode(self.keysort(query)) + '&sign=' + sign
            headers = {
