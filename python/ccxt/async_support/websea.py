...
class websea(Exchange, ImplicitAPI):
    ...
    async def fetch_markets_by_type(self, type: str, params={}) -> List[Market]:
        ...
        currenciesResponse = await self.publicGetOpenApiMarketCurrencies(params)
        ...
        if type == 'spot':
            ...
            # 尝试获取合约市场数据
            try:
                swapResponse = await self.contractGetOpenApiContractSymbols(params)
    ...
    async def fetch_tickers(self, symbols: Strings = None, params={}) -> Tickers:
        ...
        await self.load_markets()
        # 获取现货市场ticker
        spotResponse = await self.publicGetOpenApiMarket24kline(params)
        spotResult = self.safe_value(spotResponse, 'result', [])
        # 获取合约市场ticker
        swapResponse = await self.contractGetOpenApiContract24kline(params)
    ...
    async def create_order(self, symbol: str, type: OrderType, side: OrderSide, amount: float, price: Num = None, params={}) -> Order:
        ...
        if marketType == 'swap':
            ...
            if reduceOnly:
                ...
                # 期货下单
                response = await self.privatePostOpenApiFuturesEntrustAdd(self.extend(request, queryWithoutReduceOnly))
            else:
                # 期货下单
