...
class websea(Exchange, ImplicitAPI):
    ...
    async def fetch_ticker(self, symbol: str, params={}) -> Ticker:
        ...
        if market['type'] == 'swap':
            # 合约市场使用合约API
            response = await self.publicGetOpenApiContract24kline(self.extend(request, params))
        else:
            # 现货市场使用现货API
    ...
    async def create_order(self, symbol: str, type: OrderType, side: OrderSide, amount: float, price: Num = None, params={}) -> Order:
        ...
        if marketType == 'swap':
            ...
            # 合约下单
            response = await self.privatePostOpenApiContractAdd(self.extend(request, cleanParams))
    ...
    async def fetch_order(self, id: str, symbol: Str = None, params={}) -> Order:
        ...
        if marketType == 'swap':
            ...
            request['order_id'] = id
            response = await self.privateGetOpenApiContractGetOrderDetail(self.extend(request, query))
        else:
    ...
    async def fetch_open_orders(self, symbol: Str = None, since: Int = None, limit: Int = None, params={}) -> List[Order]:
        ...
        if marketType == 'swap':
            # 期货当前订单列表
            response = await self.privateGetOpenApiContractCurrentList(self.extend(request, query))
    ...
    async def fetch_closed_orders(self, symbol: Str = None, since: Int = None, limit: Int = None, params={}) -> List[Order]:
        ...
        if marketType == 'swap':
            ...
            response = await self.privateGetOpenApiContractHistoryList(self.extend(request, query))
        else:
            # 现货历史订单列表
