...
class websea(Exchange, ImplicitAPI):
    ...
    async def set_leverage(self, leverage: int, symbol: Str = None, params={}):
        ...
        return await self.privatePostOpenApiFuturesPositionSetLeverage(self.extend(request, params))
    ...
    async def fetch_markets_by_type(self, type: str, params={}) -> List[Market]:
        markets = []
        if type == 'spot':
            ...
            symbolsResponse, precisionResponse = await asyncio.gather(*promises)
    ...
    async def load_markets(self, reload=False, params={}):
        markets = await super(websea, self).load_markets(reload, params)
    ...
    async def fetch_ticker(self, symbol: str, params={}) -> Ticker:
        ...
        if market['type'] == 'swap':
            # 合约市场使用合约API
            response = await self.contractGetOpenApiContract24kline(self.extend(request, params))
        else:
            # 现货市场使用现货API
    ...
    async def fetch_tickers(self, symbols: Strings = None, params={}) -> Tickers:
        ...
        spotResponse, swapResponse = await asyncio.gather(*promises)
    ...
    async def create_order(self, symbol: str, type: OrderType, side: OrderSide, amount: float, price: Num = None, params={}) -> Order:
        ...
        if marketType == 'swap':
            ...
            # 合约下单
            response = await self.privatePostOpenApiFuturesEntrustAdd(self.extend(request, cleanParams))
        ...
        if orderId is not None:
            return await self.fetch_order(orderId, symbol, {'type': marketType})
        # 如果没有订单ID，返回解析后的结果
    ...
    async def cancel_order(self, id: str, symbol: Str = None, params={}):
        ...
        if marketType == 'swap':
            # 合约取消订单
            response = await self.privatePostOpenApiContractCancel(self.extend(request, query))
        else:
            # 现货取消订单
    ...
    async def fetch_order(self, id: str, symbol: Str = None, params={}) -> Order:
        ...
        if marketType == 'swap':
            ...
            request['order_id'] = id
            response = await self.contractGetOpenApiContractGetOrderDetail(self.extend(request, query))
        else:
            # 现货订单详情 - 使用GET方法和正确的端点
            request['order_sn'] = id
            if symbol is not None:
                request['symbol'] = market['id']
            response = await self.privateGetOpenApiEntrustStatus(self.extend(request, query))
        #
        # 合约返回示例：
        # {
        #     "errno": 0,
    ...
    async def fetch_open_orders(self, symbol: Str = None, since: Int = None, limit: Int = None, params={}) -> List[Order]:
        ...
        if marketType == 'swap':
            # 期货当前订单列表
            response = await self.contractGetOpenApiContractCurrentList(self.extend(request, query))
        elif marketType == 'spot':
            # 现货当前委托列表 - 使用正确的API端点
            response = await self.privateGetOpenApiEntrustCurrentList(self.extend(request, query))
        else:
