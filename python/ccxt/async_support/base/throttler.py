...
class Throttler:
    ...
    def __call__(self, cost=None):
        ...
        if len(self.queue) > self.config['maxCapacity']:
            raise RuntimeError('throttle queue is over maxCapacity (' + int(self.config['maxCapacity']) + '). For recommendation, see: https://github.com/ccxt/ccxt/issues/11645#issuecomment-1195695526')
