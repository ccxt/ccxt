...
class krakenfutures(Exchange):
    ...
    def parse_order(self, order, market=None):
        ...
        if (filled is not None) and (market is not None):
            ...
            if whichPrice is not None:
                ...
                # cost = Precise.string_mul(cost, market['lotSize'])  # TODO: What is lotSize supposed to be
        ...
        return self.safe_order({
            ...
            'price': self.parse_number(price),
            'stopPrice': self.safe_number(details, 'triggerPrice'),
            'amount': amount,
            'cost': self.parse_number(cost),
            'average': self.parse_number(average),
    ...
    async def fetch_balance(self, params={}):
        ...
        if type == 'marginAccount' or type == 'margin':
            if symbol is None:
                raise ArgumentsRequired(self.id + ' fetchBalance')
            type = symbol
        if type is None:
    ...
    def parse_position(self, position, market=None):
        ...
        return {
            'info': position,
            'symbol': self.safe_currency_code(self.safe_string(position, 'symbol')),
            'timestamp': self.parse8601(datetime),
            'datetime': datetime,
            'initialMargin': None,
            ...
            'markPrice': None,
            'collateral': self.safe_string(position, 'size'),
            'marginType': 'cross',
    ...
    def sign(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if api == 'private' or access == 'private':
            nonce = ''  # self.nonce()
            auth = postData + nonce + '/api/' + endpoint  # 1
