...
class hitbtc(Exchange, ImplicitAPI):
    ...
    async def fetch_transactions_helper(self, types: Any, code: Any, since: Any, limit: Any, params: Any) -> List[Transaction]:
        if self.markets is None:
            await self.load_markets()
        request = {
            'types': types,
    ...
    async def convert_currency_network(self, code: str, amount: Any, fromNetwork: Any, toNetwork: Any, params: Any):
        if self.markets is None:
            await self.load_markets()
        if code != 'USDT':
