...
class mudrex(Exchange, ImplicitAPI):
    ...
    async def fetch_ohlcv(self, symbol: str, timeframe: str = '1m', since: Int = None, limit: Int = None, params={}) -> List[list]:
        ...
        response = await self.request(path, 'market', 'GET', self.extend(request, params))
    ...
    async def fetch_markets(self, params={}) -> List[Market]:
        ...
        while(paging):
            ...
            response = await self.request('futures', 'private', 'GET', q)
            data = self.safe_value(response, 'data', [])
            items = []
    ...
    async def fetch_balance(self, params={}) -> Balances:
        ...
        spot = await self.request('wallet/funds', 'private', 'GET', self.extend(spotReq, p1))
        fut = await self.request('futures/funds', 'private', 'GET', self.extend(futReq, p1))
    ...
    async def fetch_leverage(self, symbol: str, params={}) -> Leverage:
        ...
        response = await self.request(path, 'private', 'GET', self.extend(request, self.omit(params, ['trade_currency', 'tradeCurrency'])))
        data = self.safe_dict(response, 'data', {})
        return {
            'info': response,
            'symbol': symbol,
    ...
    async def set_leverage(self, leverage: float, symbol: Str = None, params={}) -> Any:
        ...
        response = await self.request(path, 'private', 'POST', self.extend(body, self.omit(params, ['trade_currency', 'tradeCurrency', 'marginType'])))
    ...
    async def create_order(self, symbol: str, type: OrderType, side: OrderSide, amount: float, price: Num = None, params={}) -> Order:
        ...
        response = await self.request(path, 'private', 'POST', self.extend(body, self.omit(params, ['leverage', 'reduceOnly', 'takeProfit', 'stopLoss', 'takeprofit_price', 'takeProfitPrice', 'stoploss_price', 'stopLossPrice', 'trade_currency', 'tradeCurrency'])))
        leverages = self.safe_dict(self.options, 'leverages', {})
    ...
    async def edit_order(self, id: str, symbol: str, type: OrderType, side: OrderSide, amount: Num = None, price: Num = None, params={}) -> Order:
        ...
        if tradeCurrency is not None:
            request['trade_currency'] = tradeCurrency
        response = await self.request('futures/orders/' + id, 'private', 'PATCH', self.extend(request, self.omit(params, ['trade_currency', 'tradeCurrency'])))
        data = self.safe_dict(response, 'data', response)
        return self.parse_order(data, market)
    ...
    async def cancel_order(self, id: str, symbol: Str = None, params={}) -> Order:
        ...
        if tradeCurrency is not None:
            request['trade_currency'] = tradeCurrency
        response = await self.request(path, 'private', 'DELETE', self.extend(request, self.omit(params, ['trade_currency', 'tradeCurrency'])))
        data = self.safe_dict(response, 'data', response)
        return self.parse_order(data, market)
    ...
    async def fetch_positions(self, symbols: Strings = None, params={}) -> List[Position]:
        ...
        if tradeCurrency is not None:
            q['trade_currency'] = tradeCurrency
        response = await self.request('futures/positions', 'private', 'GET', self.extend(q, self.omit(params, ['trade_currency', 'tradeCurrency'])))
        data = self.safe_value(response, 'data', [])
        if data is None:
            return []
        rows = self.to_array(data)
    ...
    async def close_position(self, symbol: str, side: OrderSide = None, params={}) -> Any:
        self.check_required_credentials()
        await self.load_markets()
        positionId = self.safe_string(params, 'position_id')
        amount = self.safe_value(params, 'amount')
        if positionId is None:
            positions = await [symbol] if self.fetch_positions(symbol else None, params)
        ...
        if amount is not None:
            ...
            if orderType == 'LIMIT' and lp is not None:
                body['limit_price'] = lp
            return await self.request(partialPath, 'private', 'POST', self.extend(body, self.omit(params, ['trade_currency', 'tradeCurrency', 'order_type', 'limit_price', 'amount'])))
        closePath = 'futures/positions/' + positionId + '/close'
        return await self.request(closePath, 'private', 'POST', self.extend(request, self.omit(params, ['trade_currency', 'tradeCurrency'])))
    ...
    async def add_margin(self, symbol: str, amount: float, params={}):
        ...
        if tradeCurrency is not None:
            request['trade_currency'] = tradeCurrency
        return await self.request('futures/positions/' + positionId + '/add-margin', 'private', 'POST', self.extend(request, self.omit(params, ['trade_currency', 'tradeCurrency', 'position_id'])))
    ...
    async def fetch_my_trades(self, symbol: Str = None, since: Int = None, limit: Int = None, params={}) -> List[Trade]:
        ...
        if tradeCurrency is not None:
            request['trade_currency'] = tradeCurrency
        response = await self.request('futures/fee/history', 'private', 'GET', self.extend(request, self.omit(params, ['trade_currency', 'tradeCurrency'])))
        data = self.safe_value(response, 'data', [])
        rows = self.to_array(data)
    ...
    async def transfer(self, code: str, amount: float, fromAccount: str, toAccount: str, params={}) -> TransferEntry:
        ...
        if code == 'INR':
            ...
            if tradeCurrency == 'INR':
                path = 'futures/transfers/inr'
        response = await self.request(path, 'private', 'POST', body, None, None, {'cost': 5})
        data = self.safe_dict(response, 'data', response)
        return {
            'info': response,
