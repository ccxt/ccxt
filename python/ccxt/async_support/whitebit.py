...
class whitebit(Exchange, ImplicitAPI):
 ...
 @deprecated
    ...
    async def fetch_funding_limits(self, codes: Strings = None, params: dict = {}) -> dict:
        ...
        for i in range(0, len(currencyKeys)):
            ...
            currencyLimits = self.safe_dict(currency, 'limits', {})
            limits = {
                'deposit': {
    ...
    async def fetch_order(self, id: str, symbol: Str = None, params: dict = {}) -> Order:
        ...
        if checkActive is True:
            try:
                response = await self.v4PrivatePostOrders(self.extend(request, paramsOmitted))
        ...
        if checkExecuted is True:
            try:
                response = await self.v4PrivatePostTradeAccountOrderHistory(self.extend(request, paramsOmitted))
    ...
    async def fetch_tickers(self, symbols: Strings = None, params: dict = {}) -> Tickers:
        ...
        if method == 'v4PublicGetTicker':
            ...
            #      },
            #
            response = await self.v4PublicGetTicker(paramsMethod)
            ...
            #     }
            #
            response = await self.v4PublicGetFutures(paramsMethod)
        else:
            response = await self.v2PublicGetTicker(paramsMethod)
    ...
    async def create_order(self, symbol: str, type: OrderType, side: OrderSide, amount: float, price: Num = None, params: dict = {}) -> Order:
        ...
        if isStopOrder:
            ...
            if isLimitOrder:
                ...
                response = await self.v4PrivatePostOrderStopLimit(self.extend(request, orderParams))
            else:
                # stop market order
                if useCollateralEndpoint:
                    response = await self.v4PrivatePostOrderCollateralTriggerMarket(self.extend(request, orderParams))
                else:
                    response = await self.v4PrivatePostOrderStopMarket(self.extend(request, orderParams))
        else:
            if isLimitOrder:
                # limit order
                request['price'] = self.price_to_precision(symbol, price)
                if useCollateralEndpoint:
                    response = await self.v4PrivatePostOrderCollateralLimit(self.extend(request, orderParams))
                else:
                    response = await self.v4PrivatePostOrderNew(self.extend(request, orderParams))
            else:
                # market order
                if useCollateralEndpoint:
                    response = await self.v4PrivatePostOrderCollateralMarket(self.extend(request, orderParams))
                else:
                    if cost is not None:
                        response = await self.v4PrivatePostOrderMarket(self.extend(request, orderParams))
                    else:
                        response = await self.v4PrivatePostOrderStockMarket(self.extend(request, orderParams))
    ...
    async def edit_order(self, id: str, symbol: str, type: OrderType, side: OrderSide, amount: Num = None, price: Num = None, params: dict = {}) -> Order:
        ...
        response = await self.v4PrivatePostOrderModify(self.extend(request, paramsOmitted))
        return self.parse_order(response)
    ...
    async def cancel_all_orders(self, symbol: Str = None, params: dict = {}) -> list[Order]:
        ...
        response = await self.v4PrivatePostOrderCancelAll(self.extend(request, requestParams))
        #
        # []
        #
    ...
    async def cancel_all_orders_after(self, timeout: Int, params: dict = {}):
        ...
        response = await self.v4PrivatePostOrderKillSwitch(self.extend(request, paramsOmitted))
        #
        #     {
    ...
    async def fetch_balance(self, params: dict = {}) -> Balances:
        ...
        response: dict
        if marketType == 'swap':
            response = await self.v4PrivatePostCollateralAccountBalance(paramsMarketType)
        else:
            ...
            if account == 'main' or account == 'funding':
                response = await self.v4PrivatePostMainAccountBalance(paramsOmitted)
            else:
                response = await self.v4PrivatePostTradeAccountBalance(paramsOmitted)
        #
        # main account
        #
        #     {
    ...
    async def fetch_funding_history(self, symbol: Str = None, since: Int = None, limit: Int = None, params: dict = {}) -> list[FundingHistory]:
        ...
        response = await self.v4PrivatePostCollateralAccountFundingHistory(self.extend(requestUntil, paramsUntil))
        #
        #     {
        #         "records": [
        #             {
 ...
 EXCHANGE SPECIFIC PARAMETERS
    ...
    async def fetch_convert_trade_history(self, code: Str = None, since: Int = None, limit: Int = None, params: dict = {}) -> list[Conversion]:
        ...
        response = await self.v4PrivatePostConvertHistory(self.extend(requestUntil, paramsUntil))
        #
        #     {
        #         "records": [
        #             {
    ...
    async def fetch_position_history(self, symbol: str, since: Int = None, limit: Int = None, params: dict = {}) -> list[Position]:
        ...
        if limit is not None:
            request['limit'] = since
        requestUntil, paramsUntil = self.handle_until_option('endDate', request, params)
        response = await self.v4PrivatePostCollateralAccountPositionsHistory(self.extend(requestUntil, paramsUntil))
        #
        #     [
        #         {
    ...
    async def fetch_funding_rate_history(self, symbol: Str = None, since: Int = None, limit: Int = None, params: dict = {}) -> list[FundingRateHistory]:
        ...
        if paginate:
            return await self.fetch_paginated_call_deterministic('fetchFundingRateHistory', symbol, since, limit, '8h', paramsPaginate, maxLimit)
        if self.markets is None:
            await self.load_markets()
        market = self.market(symbol)
        request = {
        ...
        response = await self.v4PublicGetFundingHistoryMarket(self.extend(requestUntil, paramsUntil))
        #
        #     [
        #         {
