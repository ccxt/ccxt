...
class b2c2(Exchange):
    ...
    async def fetch_currencies(self, params={}):
        ccys = await self.privateGetCurrency(params)
        account_info = await self.privateGetAccountInfo(params)
        #
        # currencies endpoint
        # {
        #     "AUD": {
    ...
    async def fetch_markets(self, params={}):
        ...
        response = await self.privateGetInstruments(params)
        #
        # [
        #     {
        #       "name": "LTCUSD.SPOT",
    ...
    async def create_quote(self, symbol, side, amount, params={}):
        await self.load_markets()
        ...
        response = await self.privatePostRequestForQuote(self.extend(request, params))
    ...
    async def fetch_order(self, id, symbol=None, params={}):
        ...
        response = await self.privateGetOrderOrderIdOrClientOrderId(self.extend(request, params))
    ...
    async def fetch_orders(self, symbol=None, since=None, limit=None, params={}):
        ...
        response_next = await self.fetch_recursively()
        if response_next is not None:
            response.append(response_next)
        return self.parse_orders(response, None, since, limit)

    async def fetch_recursively(self):
        ...
        print(next)
        if next is not None:
            response = await self.request(next, 'private', 'GET')
            response_next = await self.fetch_recursively()
            if response_next is not None:
                response.append(response_next)
            return response
    ...
    async def fetch_my_trades(self, symbol=None, since=None, limit=None, params={}):
        ...
        response = await self.privateGetTrade(self.extend(request, params))
        response_next = await self.fetch_recursively()
        if response_next is not None:
            response.append(response_next)
    ...
    async def fetch_withdrawals(self, code=None, since=None, limit=None, params={}):
        ...
        response = await self.privateGetWithdrawal(self.extend(request, params))
        response_next = await self.fetch_recursively()
        if response_next is not None:
            response.append(response_next)
