...
class coinmetro(Exchange, ImplicitAPI):
        ...
        return self.deep_extend(super(coinmetro, self).describe(), {
            ...
            'exceptions': {
                ...
                'exact': {
                    ...
                    # todo: add self one - 422 Unprocessable Entity {"message":"Invalid 'from'"}
                ...
                'broad': {
                    ...
                    'available to allocate': InsufficientFunds,  # todo: check 403 Forbidden {"message":"Insufficient EUR available to allocate"}
    ...
    async def fetch_ohlcv(self, symbol: str, timeframe='1m', since: Int = None, limit: Int = None, params={}) -> List[list]:
        ...
        if since is not None:
            if (since < 0) or (not isinstance(since, int)):
                raise BadRequest(self.id + ' fetchOHLCV() param "since" must be an integer greater than 0 or equal to 0')  # the exchange returns 422 "No more than 1000 candles per call" for float or negative values
            request['from'] = since
    ...
    def parse_ohlcv(self, ohlcv, market: Market = None) -> list:
        return [
            self.safe_number(ohlcv, 'timestamp'),
    ...
    async def fetch_trades(self, symbol: str, since: Int = None, limit: Int = None, params={}) -> List[Trade]:
        ...
        return self.parse_trades(tickHistory, market, since, limit)  # todo = 0
    ...
    def parse_trade(self, trade, market: Market = None) -> Trade:
        ...
        id = self.safe_string_n(trade, ['_id', 'seqNum', 'seqNumber'])  # todo: check
    ...
    async def fetch_order_book(self, symbol: str, limit: Int = None, params={}) -> OrderBook:
        ...
        orderbook['nonce'] = self.safe_integer(book, 'checksum')  # todo: check
        return orderbook
    ...
    def parse_ticker(self, ticker, market: Market = None) -> Ticker:
        ...
        close = self.safe_string(ticker, 'price')  # todo: check the GUI shows it last price
    ...
    async def fetch_ledger(self, code: Str = None, since: Int = None, limit: Int = None, params={}):
        ...
        return self.parse_ledger(ledger, currency, since, limit)  # todo = 0
    ...
    async def create_order(self, symbol: str, type: OrderType, side: OrderSide, amount, price=None, params={}):
        ...
        if type == 'limit':
            if (price is None) and (cost is None):
                ...
                costString = str(Precise.string_mul(price), str(precisedAmount))
                cost = self.number(costString)
        precisedCost = None
        if cost is not None:
            precisedCost = self.cost_to_precision(symbol, cost)
        if side == 'sell':
            self.handle_create_order_side(market['baseId'], market['quoteId'], precisedAmount, precisedCost, request)
        elif side == 'buy':
            self.handle_create_order_side(market['quoteId'], market['baseId'], precisedCost, precisedAmount, request)
        timeInForce = self.safe_value(params, 'timeInForce')
        # todo: the exchange accepts limit IOC orders and cancels them, should we check it here and raise an exeption?
        if timeInForce is not None:
            params = self.omit(params, 'timeInForce')
            request['timeInForce'] = self.encode_order_time_in_force(timeInForce)
        expirationTime = self.safe_integer(params, 'expirationTime')
        if expirationTime is not None:
            params = self.omit(params, 'expirationTime')
        ...
        margin = self.safe_value(params, 'margin')
        params = self.omit(params, 'margin')  # if request has property 'margin' with any value, the exchange counts the order
        if margin is True:
            request['margin'] = True
        userData = self.safe_value(params, 'userData', {})
        comment = self.safe_string_2(params, 'clientOrderId', 'comment')
        if comment is not None:
            params = self.omit(params, ['clientOrderId', 'comment'])
    ...
    def parse_order(self, order, market: Market = None) -> Order:
        ...
        if isCanceled is True:
            if timestamp is None:
                timestamp = self.safe_integer(order, 'completionTime')  # todo: check - market orders with bad price gain IOC - should we mark them as 'rejected'?
