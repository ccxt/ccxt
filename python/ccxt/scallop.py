...
class scallop(Exchange):

    def describe(self):
        return self.deep_extend(super(scallop, self).describe(), {
            'id': 'scallop',
            'name': 'Scallop',
            'countries': ['SG'],
            'version': 'v1',
    ...
    def parse_trade(self, trade, market=None):
        ...
        symbol = self.safe_symbol(marketId, market, '_')
        takerOrMaker = self.safe_value(trade, 'isMaker')
        feeCostString = self.safe_string(trade, 'fee')
        fee = None
        if feeCostString is not None:
            feeCurrencyId = self.safe_string(trade, 'feeCoin')
    ...
    def create_order(self, symbol, type, side, amount, price=None, params={}):
        ...
        request['newClientOrderId'] = timestamp
        request['recvWindow'] = None
    ...
    def cancel_orders(self, ids, symbol=None, params={}):
        ...
        response = self.privatePostBatchCancel(self.extend(request, params))
        #
        #     {
        #         "code": 0,
        #         "success": [
    ...
    def parse_order(self, order, market=None):
        ...
        symbol = self.safe_symbol(marketId, market, '_')
        amountString = self.safe_string(order, 'origQty')
        filledString = self.safe_string(order, 'executedQty')
    ...
    def fetch_order(self, id, symbol=None, newClientOrderId=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        request = {
            'orderId': id,
    ...
    def fetch_my_trades(self, symbol=None, since=None, limit=None, fromId=None, params={}):
        # defaultType = self.safe_string(self.options, 'defaultType', 'spot')
        # orderType = self.safe_string(params, 'type', defaultType)
        params = self.omit(params, 'type')
        self.load_markets()
    ...
    def sign(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if api == 'private':
            ...
            if method == 'GET':
                if urlencoded:
                    url += '?' + urlencoded
                    signatureBody = timestamp + method + "/sapi/v1/{path}?{urlencoded}".format(path=path, urlencoded=urlencoded)
                else:
                    signatureBody = timestamp + method + "/sapi/v1/{path}".format(path=path)

            elif method == 'POST':
                headers['Content-Type'] = 'application/json'
                body = self.json(query)
                signatureBody = timestamp + method + "/sapi/v1/{path}{body}".format(path=path, body=body)
