   ...
   public BingxCore (Object options) {
    ...
    public java.util.concurrent.CompletableFuture<Object> fetchCurrencies(Object... optionalArgs)
    {
        ...
        put( "type", "crypto" );
                ...
                {
                    Object existing = Helpers.GetValue(result, code);
                    Object existingNetworks = this.safeDict(existing, "networks", new java.util.HashMap<String, Object>() {{}});
                    Object newNetworkCodes = Helpers.objectKeys(networks);
                    for (var j = 0; Helpers.isLessThan(j, Helpers.getArrayLength(newNetworkCodes)); j++)
                    {
                        Object newNetworkCode = Helpers.GetValue(newNetworkCodes, j);
                        if (!Helpers.isTrue((Helpers.inOp(existingNetworks, newNetworkCode))))
                        {
                            Helpers.addElementToObject(existingNetworks, newNetworkCode, Helpers.GetValue(networks, newNetworkCode));
                        }
                    }
                    Helpers.addElementToObject(Helpers.GetValue(result, code), "networks", existingNetworks);
                }
            }
            Object codes = Helpers.objectKeys(result);
            for (var i = 0; Helpers.isLessThan(i, Helpers.getArrayLength(codes)); i++)
            {
                Object code = Helpers.GetValue(codes, i);
                Object currency = Helpers.GetValue(result, code);
                Helpers.addElementToObject(result, code, this.safeCurrencyStructure(currency));
            }
            return result;
        });
