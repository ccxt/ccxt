   ...
   public PoloniexCore (Object options) {
    ...
    public java.util.concurrent.CompletableFuture<Object> fetchCurrencies(Object... optionalArgs)
    {
        ...
        return java.util.concurrent.CompletableFuture.supplyAsync(() -> {
            ...
            Object currenciesDict = new java.util.HashMap<String, Object>() {{}};
            for (var i = 0; Helpers.isLessThan(i, Helpers.getArrayLength(response)); i++)
            {
                Object item = this.safeDict(response, i);
                Object ids = Helpers.objectKeys(item);
                Object id = this.safeString(ids, 0);
                Helpers.addElementToObject(currenciesDict, id, Helpers.GetValue(item, id));
            }
            Object keys = Helpers.objectKeys(currenciesDict);
            for (var i = 0; Helpers.isLessThan(i, Helpers.getArrayLength(keys)); i++)
            {
                Object id = Helpers.GetValue(keys, i);
                Object entry = Helpers.GetValue(currenciesDict, id);
                Object code = this.safeCurrencyCode(id);
                // skip childChains, as they are collected in parentChain loop
                if (Helpers.isTrue(this.safeBool(entry, "isChildChain")))
                {
                    continue;
                }
                Object allChainEntries = new java.util.ArrayList<Object>(java.util.Arrays.asList());
                Object childChains = this.safeList(entry, "childChains", new java.util.ArrayList<Object>(java.util.Arrays.asList()));
                if (Helpers.isTrue(!Helpers.isEqual(childChains, null)))
                {
                    for (var j = 0; Helpers.isLessThan(j, Helpers.getArrayLength(childChains)); j++)
                    {
                        Object childChainId = Helpers.GetValue(childChains, j);
                        Object childNetworkEntry = this.safeDict(currenciesDict, childChainId);
                        ((java.util.List<Object>)allChainEntries).add(childNetworkEntry);
                    }
                }
                ((java.util.List<Object>)allChainEntries).add(entry);
                Object networks = new java.util.HashMap<String, Object>() {{}};
                for (var j = 0; Helpers.isLessThan(j, Helpers.getArrayLength(allChainEntries)); j++)
                {
                    Object chainEntry = Helpers.GetValue(allChainEntries, j);
        ...
        put( "info", chainEntry );
        put( "id", specialNetworkId );
        put( "numericId", PoloniexCore.this.safeInteger(chainEntry, "id") );
        put( "network", networkCode );
        put( "active", PoloniexCore.this.safeBool(chainEntry, "walletState") );
        put( "deposit", Helpers.isEqual(PoloniexCore.this.safeString(chainEntry, "walletDepositState"), "ENABLED") );
        put( "withdraw", Helpers.isEqual(PoloniexCore.this.safeString(chainEntry, "walletWithdrawalState"), "ENABLED") );
        put( "fee", PoloniexCore.this.safeNumber(chainEntry, "withdrawalFee") );
        put( "precision", null );
        put( "limits", new java.util.HashMap<String, Object>() {{
        ...
        put( "code", code );
        put( "id", id );
        put( "numericId", PoloniexCore.this.safeInteger(entry, "id") );
        put( "type", "crypto" );
